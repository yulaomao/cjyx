...
- Create source folder. This folder will be referred to as `<Cjyx_SOURCE>` in the following. Recommended path: `C:\D\S4`
...
- Create build folder. This folder will be referred to as `<Cjyx_BUILD>` in the following. Recommended path: `C:\D\S4R` for release-mode build, `C:\D\S4D` for debug-mode build.
...
- Download source code into _Cjyx source_ folder from GitHub: https://github.com/Slicer/Slicer.git
  - The following command can be executed in _Cjyx source_ folder to achieve this: `git clone https://github.com/Slicer/Slicer.git .` (note the dot at the end of the command; the `.` is needed because without that git would create a `Slicer` subfolder in the current directory)
- Configure the repository for developers (optional): Needed if changes need to be contributed to Cjyx repository.
  - Right-click on `<Cjyx_SOURCE>/Utilities` folder in Windows Explorer and select `Git bash here`
  - Execute this command in the terminal (and answer all questions): `./SetupForDevelopment.sh`
  - Note: more information about how to use git in Cjyx can be found on [this page](https://www.slicer.org/wiki/Documentation/Nightly/Developers/DevelopmentWithGit)
...
- Set `Where is the source code` to `<Cjyx_SOURCE>` location
- Set `Where to build the binaries` to `<Cjyx_BUILD>` location. Do not configure yet!
...
- If building in release mode:
  - Open the top-level Cjyx.sln file in the build directory in Visual Studio
  - Set active configuration to Release. Visual Studio will select Debug build configuration by default when you first open the solution in the Visual Studio GUI. If you build Cjyx in release mode and accidentally forget to switch the build configuration to Release then the build will fail. Note: you can avoid this manual configuration mode selection by setting `CMAKE_CONFIGURATION_TYPES` to Release in cmake-gui.
- Build the `ALL_BUILD` project

## Run Cjyx

Run `<Cjyx_BUILD>/Cjyx-build/Cjyx.exe` application.

Note: `Cjyx.exe` is a "launcher", which sets environment variables and launches the real executable: `<Cjyx_BUILD>/Cjyx-build\bin\Release\CjyxApp-real.exe` (use `Debug` instead of `Release` for debug-mode builds).
...
## Package Cjyx (create installer package)
...
Errors due to missing Python libraries (or other Python related errors, such as building a `python-...-requirements` project or Python-wrapping SimpleITK) may be caused by the build system detecting Python installations somewhere on the system, instead of Cjyx's own Python environment. To resolve such issues, remove all references to Python in the environment variables (PATH, PYTHONPATH, PYTHONHOME). Alternatively, temporarily rename or remove other Python installations before starting to build Cjyx; they can be restored after Cjyx build is completed.
