...
The instructions to build Cjyx for GNU/Linux systems are slightly different
...
related to the building process in the [Cjyx forum](https://discourse.slicer.org).

## Pre-requisites

First, you need to install the tools that will be used for fetching the source
code of cjyx, generating the project files and build the project.
...
In addition, Cjyx requires a set of support libraries that are not includes as
...
:::{note} Warning
:class: warning
Since the default Qt5 packages available on Ubuntu 20.04 correspond to version 5.12.8 and version 5.15.2 is used to build and test the packages available for download. Compiling Cjyx against version 5.12.8 may not succeed, and if it does, the compiled Cjyx application may behave differently.
...
### CentOS 7
:::{note}
Cjyx built on CentOS 7 will be available for many Linux distributions and releases
:::
...
```
You can run the installer and follow the instructions in the GUI. Keep in mind that the components needed by 3D Cjyx are: `qt.qt5.5152.gcc_64`, `qt.qt5.5152.qtscript` `qt.qt5.5152.qtscript.gcc_64`, `qt.qt5.5152.qtwebengine` and `qt.qt5.5152.qtwebengine.gcc_64`.
...
When configuring the Cjyx build project, the CMake variable `Qt5_DIR` need to be set using the full path to the Qt5 installation directory ending with `5.15.2/gcc_64/lib/cmake/Qt5`. For example, assuming you installed Qt in `/opt/qt`, you may use `cmake -DCMAKE_BUILD_TYPE:STRING=Release -DQt5_DIR:PATH=/opt/qt/5.15.2/gcc_64/lib/cmake/Qt5 ../Cjyx`.

:::

## Checkout Cjyx source files

The recommended way to obtain the source code of CJyx is cloning the repository using `git`:

```console
git clone https://github.com/Slicer/Slicer.git
```

This will create a `Cjyx` directory containing the source code of Cjyx.
...
## Configure and generate the Cjyx build project files

Cjyx is highly configurable and multi-platform. To support this,
Cjyx needs a configuration of the build parameters before the build process
...
the software components and functionalities of Cjyx.
...
|-----------------|--------|
| **source**      | `~/Cjyx` |
| **build**       |  `~/Cjyx-SuperBuild-Debug` |
| **inner-build** |  `~/Cjyx-SuperBuild-Debug/Cjyx-build` |

To obtain a default configuration of the Cjyx build project, create the **build** folder and use `cmake`:

```console
mkdir Cjyx-SuperBuild-Debug
cd Cjyx-SuperBuild-Debug
cmake ../Cjyx
```
...
```console
cmake -DCMAKE_BUILD_TYPE:STRING=Release ../Cjyx
```

:::{admonition} Tip -- Interfaces to change 3D Cjyx configuration variables
...
:::{admonition} Tip -- Speed up 3D Cjyx build with `ccache`

`ccache` is a compiler cache that can speed up subsequent build of 3D Cjyx. This can be useful if 3D Cjyx is built often and there are no large divergences between subsequent builds. This requires `ccache` installed on the system (e.g., `sudo apt install ccache`).
...
`ccache` is not detected as a valid compiler by the 3D Cjyx building process. You can generate local symbolic links to disguise the use of `ccache` as valid compilers:
...
Then, the Cjyx build can be configured to use these compilers:
...
Once the Cjyx build project files have been generated, the Cjyx project can
...
Building Cjyx will generally take long time, particularly on the first build or upon code/configuration changes. To help speeding up the process one can use `make -j<N>`, where `<N>` is the number of parallel builds. As a rule of thumb, many uses the `number of CPU threads + 1` as the number of parallel builds.
...
run Cjyx will be located in the **inner-build** folder.
