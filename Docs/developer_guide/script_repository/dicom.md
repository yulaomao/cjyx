...
This code loads all DICOM objects into the scene from a file folder. All the registered plugins are evaluated and the one with the highest confidence will be used to load the data. Files are imported into a temporary DICOM database, so the current Cjyx DICOM database is not impacted.
...
This code snippet uses Cjyx DICOM browser built-in indexer to import DICOM files into the database. Images are not loaded into the scene, but they show up in the DICOM browser. After import, data sets can be loaded using DICOMUtils functions (e.g., loadPatientByUID) - see above for an example.

```python
# instantiate a new DICOM browser
cjyx.util.selectModule("DICOM")
dicomBrowser = cjyx.modules.DICOMWidget.browserWidget.dicomBrowser
...
```python
cjyx.util.selectModule("DICOM")  # ensure DICOM database is initialized and
...
### Access top level tags of DICOM images imported into Cjyx
...
volumeNode = cjyx.util.getNode(volumeName)
instUids = volumeNode.GetAttribute("DICOM.instanceUIDs").split()
filename = cjyx.dicomDatabase.fileForInstance(instUids[0])
print(cjyx.dicomDatabase.fileValue(filename, "0018,5100"))  # patient position
```
...
instUids = cjyx.dicomDatabase.instancesForSeries(seriesInstanceUID)
print(cjyx.dicomDatabase.instanceValue(instUids[0], '0010,0010')) # patient name
```
...
rtStructNode = cjyx.util.getNode(rtStructName)
shNode = cjyx.vtkDMMLSubjectHierarchyNode.GetSubjectHierarchyNode(cjyx.dmmlScene)
...
filename = cjyx.dicomDatabase.fileForInstance(ctSliceInstanceUids[0])
print(cjyx.dicomDatabase.fileValue(filename, "0008,0033"))  # content time
```
...
def pathFromNode(node):
  ...
  if storageNode is not None: # loaded via drag-drop
    ...
    filepath = cjyx.dicomDatabase.fileForInstance(instUids[0])
  return filepath

# Example:
node = cjyx.util.getNode("volume1")
...
```console
/Applications/Cjyx-4.6.2.app/Contents/MacOS/Cjyx --no-main-window --python-code "node=cjyx.util.loadVolume('/tmp/series/im0.dcm'); cjyx.util.saveNode(node, "/tmp/output.nrrd"); exit()"
```

The same can be done on windows by using the top level Cjyx.exe. Be sure to use forward slashes in the pathnames within quotes on the command line.
...
dicomBrowser = cjyx.modules.dicom.widgetRepresentation().self().browserWidget.dicomBrowser
...
dicomQuery.callingAETitle = "CJYX"
...
dicomRetrieve.setMoveDestinationAETitle("CJYX");
dicomRetrieve.setDatabase(cjyx.dicomDatabase)
...
cjyx.dicomDatabase.updateDisplayedFields()
```

### Convert RT structure set to labelmap NRRD files

[CjyxRT batch processing](https://github.com/SlicerRt/SlicerRT/tree/master/BatchProcessing) to batch convert RT structure sets to labelmap NRRD files.
