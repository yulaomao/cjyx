...
```python
cjyx.util.loadSegmentation("c:/tmp/tmp/Segmentation.nrrd")
cjyx.util.loadSegmentation("c:/tmp/tmp/Segmentation.nii")
cjyx.util.loadSegmentation("c:/tmp/Segment_1.stl")
```
...
seg = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLSegmentationNode")
cjyx.modules.segmentations.logic().ImportLabelmapToSegmentationNode(labelmapVolumeNode, seg)
...
sphere.SetRadius(10)
modelNode = cjyx.modules.models.logic().AddModel(sphere.GetOutputPort())

# Create segmentation
segmentationNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLSegmentationNode")
segmentationNode.CreateDefaultDisplayNodes() # only needed for display

# Import the model into the segmentation node
cjyx.modules.segmentations.logic().ImportModelToSegmentationNode(modelNode, segmentationNode)
```
...
cjyx.modules.segmentations.logic().ExportAllSegmentsToLabelmapNode(segmentationNode, labelmapVolumeNode, cjyx.vtkSegmentation.EXTENT_REFERENCE_GEOMETRY)
```
...
segmentationNode = getNode("Segmentation")
labelmapVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLLabelMapVolumeNode")
cjyx.modules.segmentations.logic().ExportAllSegmentsToLabelmapNode(segmentationNode, labelmapVolumeNode)
```
...
segmentationNode = getNode("Segmentation")
labelmapVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLLabelMapVolumeNode")
cjyx.modules.segmentations.logic().ExportVisibleSegmentsToLabelmapNode(segmentationNode, labelmapVolumeNode, referenceVolumeNode)
```
...
cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentsToLabelmapNode(segmentationNode, segmentIds, labelmapVolumeNode, referenceVolumeNode)
```
...
def exportLabelmap():
  segmentationNode = cjyx.dmmlScene.GetFirstNodeByClass("vtkDMMLSegmentationNode")
  referenceVolumeNode = cjyx.dmmlScene.GetFirstNodeByClass("vtkDMMLScalarVolumeNode")
  labelmapVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLLabelMapVolumeNode")
  cjyx.modules.segmentations.logic().ExportVisibleSegmentsToLabelmapNode(segmentationNode, labelmapVolumeNode, referenceVolumeNode)
  filepath = outputPath + "/" + referenceVolumeNode.GetName() + "-label.nrrd"
  cjyx.util.saveNode(labelmapVolumeNode, filepath)
  cjyx.dmmlScene.RemoveNode(labelmapVolumeNode.GetDisplayNode().GetColorNode())
  cjyx.dmmlScene.RemoveNode(labelmapVolumeNode)
  cjyx.util.delayDisplay("Segmentation saved to " + filepath)
...
colorTableNode = cjyx.dmmlScene.CreateNodeByClass("vtkDMMLColorTableNode")
colorTableNode.SetTypeToUser()
colorTableNode.HideFromEditorsOff()  # make the color table selectable in the GUI outside Colors module
cjyx.dmmlScene.AddNode(colorTableNode); colorTableNode.UnRegister(None)
...
labelmapVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLLabelMapVolumeNode")  # export to new labelmap volume
...
cjyx.modules.segmentations.logic().ExportSegmentsToLabelmapNode(segmentationNode, segmentIds, labelmapVolumeNode, referenceVolumeNode, cjyx.vtkSegmentation.EXTENT_REFERENCE_GEOMETRY, colorTableNode)
```
...
segmentationNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLSegmentationNode")  # import into new segmentation node
colorTableNode = ...  # created from scratch or loaded from file
...
cjyx.modules.segmentations.logic().ImportLabelmapToSegmentationNode(labelmapVolumeNode, segmentationNode)
```
...
cjyx.modules.segmentations.logic().ExportAllSegmentsToModels(segmentationNode, exportFolderItemId)
```
...
# Set up Hollow tool
hollowModeler = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLDynamicModelerNode")
hollowModeler.SetToolName("Hollow")
hollowModeler.SetNodeReferenceID("Hollow.InputModel", modelNode.GetID())
hollowedModelNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLModelNode")  # this node will store the hollow model
...
cjyx.modules.models.logic().AddModel(normals.GetOutputPort())
```
...
```python
image = cjyx.vtkOrientedImageData()
...
cjyx.vtkCjyxSegmentationsModuleLogic.GetSegmentBinaryLabelmapRepresentation(segmentationNode, segmentID, outputOrientedImageData)
```
...
```python
outputPolyData = vtk.vtkPolyData()
cjyx.vtkCjyxSegmentationsModuleLogic.GetSegmentClosedSurfaceRepresentation(segmentationNode, segmentID, outputPolyData)
```
...
```python
referenceGeometry = cjyx.vtkSegmentationConverter.SerializeImageGeometry(referenceImageData)
segmentation.SetConversionParameter(cjyx.vtkSegmentationConverter.GetReferenceImageGeometryParameterName(), referenceGeometry)
```
...
def toggleSphereBrush():
  segmentEditorWidget = cjyx.modules.segmenteditor.widgetRepresentation().self().editor
...
```python
cjyx.segmentationNode = getNode('Segmentation')
cjyx.toggledSegmentState="completed"  # it could be "inprogress", "completed", "flagged"
cjyx.visibility = True

def toggleSegmentVisibility():
    cjyx.visibility = not cjyx.visibility
    segmentation = cjyx.segmentationNode.GetSegmentation()
    for segmentIndex in range(segmentation.GetNumberOfSegments()):
        ...
        if not segmentation.GetSegment(segmentId).GetTag("Segmentation.Status", segmentationStatus):
            continue
        if segmentationStatus != cjyx.toggledSegmentState:
            continue
        cjyx.segmentationNode.GetDisplayNode().SetSegmentVisibility(segmentId, cjyx.visibility)
...
segmentArray = cjyx.util.arrayFromSegmentBinaryLabelmap(segmentationNode, segmentId, volumeNode)

# Modify the segmentation
segmentArray[:] = 0  # clear the segmentation
segmentArray[ cjyx.util.arrayFromVolume(volumeNode) > 80 ] = 1  # create segment by simple thresholding of an image
segmentArray[20:80, 40:90, 30:70] = 1  # fill a rectangular region using numpy indexing
cjyx.util.updateSegmentBinaryLabelmapFromArray(segmentArray, segmentationNode, segmentId, volumeNode)
```
...
cjyx.vtkDMMLTransformNode.GetTransformBetweenNodes(segmentationNode.GetParentTransformNode(), None, transformWorldToRas)
mean_Ras = transformWorldToRas.TransformPoint(mean_World)

# Show mean position value and jump to it in all slice viewers
print(mean_Ras)
cjyx.modules.markups.logic().JumpSlicesToLocation(mean_Ras[0], mean_Ras[1], mean_Ras[2], True)
```
...
# Create segmentation
segmentationNode = cjyx.vtkDMMLSegmentationNode()
cjyx.dmmlScene.AddNode(segmentationNode)
segmentationNode.CreateDefaultDisplayNodes() # only needed for display
...
cjyx.util.plot(histogram, xColumnIndex = 1)
```
...
```python
segmentationNode = cjyx.dmmlScene.GetFirstNodeByClass("vtkDMMLSegmentationNode")
pointListNode = cjyx.dmmlScene.GetFirstNodeByClass("vtkDMMLMarkupsFiducialNode")
...
def printSegmentNames(unused1=None, unused2=None):
  ...
  segmentationsDisplayableManager = sliceViewWidget.sliceView().displayableManagerByClassName("vtkDMMLSegmentationsDisplayableManager2D")
  ras = [0,0,0]
...
pointListNode.AddObserver(cjyx.vtkDMMLMarkupsPlaneNode.PointModifiedEvent, printSegmentNames)
printSegmentNames()
```
...
```python
defaultSegmentEditorNode = cjyx.vtkDMMLSegmentEditorNode()
defaultSegmentEditorNode.SetOverwriteMode(cjyx.vtkDMMLSegmentEditorNode.OverwriteNone)
cjyx.dmmlScene.AddDefaultNode(defaultSegmentEditorNode)
```

To always make this the default, add the lines above to your [.cjyxrc.py file](../user_guide/settings.md#application-startup-file).

### How to run segment editor effects from a script

Editor effects are complex because they need to handle changing master volumes, undo/redo, masking operations, etc. Therefore, it is recommended to use the effect by instantiating a qDMMLSegmentEditorWidget or use/extract processing logic of the effect and use that from a script.

#### Use Segment editor effects from script (qDMMLSegmentEditorWidget)
...
segmentIds.InsertNextValue(segmentId)
labelmapVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLLabelMapVolumeNode")
cjyx.modules.segmentations.logic().ExportSegmentsToLabelmapNode(segmentationNode, segmentIds, labelmapVolumeNode)
...
cjyx.modules.segmentations.logic().ImportLabelmapToSegmentationNode(labelmapVolumeNode, segmentationNode, segmentIds)
...
### Use segmentation files in Python - outside Cjyx

You can use [cjyxio](https://pypi.org/project/slicerio/) Python package (in any Python environment, not just within Slicer) to get information from segmentation (.seg.nrrd) files.
...
extracted_voxels, extracted_header = cjyxio.extract_segments(voxels, header, segmentation_info, segment_names_to_labels)
...
pointListNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLMarkupsFiducialNode")
...
for segmentId in stats["SegmentIDs"]:
  ...
  roi=cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLMarkupsROINode")
  ...
  boundingBoxToRasTransformMatrix = cjyx.util.vtkMatrixFromArray(boundingBoxToRasTransform)
...
for segmentId in stats["SegmentIDs"]:
  ...
  # Create ROI
  segment = segmentationNode.GetSegmentation().GetSegment(segmentId)
  roi=cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLAnnotationROINode")
  ...
  boundingBoxToRasTransform = np.row_stack((np.column_stack((obb_direction_ras_x, obb_direction_ras_y, obb_direction_ras_z, obb_center_ras)), (0, 0, 0, 1)))
  boundingBoxToRasTransformMatrix = cjyx.util.vtkMatrixFromArray(boundingBoxToRasTransform)
  transformNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLTransformNode")
