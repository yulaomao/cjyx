...
boxNode = cjyx.modules.models.logic().AddModel(box.GetOutputPort())
...
if modelNode.GetParentTransformNode():
  ...
  cjyx.vtkDMMLTransformNode.GetTransformBetweenNodes(modelNode.GetParentTransformNode(), None, transformModelToWorld)
...
resultTableNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLTableNode", "Points from surface distance")
...
cjyx.app.layoutManager().setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutFourUpTableView)
cjyx.app.applicationLogic().GetSelectionNode().SetReferenceActiveTableID(resultTableNode.GetID())
...
model = cjyx.modules.models.logic().AddModel(planeSource.GetOutputPort())
...
```python
# Get input nodes
modelNode = cjyx.util.getNode("Segment_1") # select cells in this model
pointListNode = cjyx.util.getNode("F") # points will be selected at positions specified by this markups point list node
...
modelNode.GetDisplayNode().SetAndObserveColorNodeID("vtkDMMLColorTableNodeWarm1")
modelNode.GetDisplayNode().SetScalarVisibility(True)
...
def onPointsModified(observer=None, eventid=None):
  ...
  markupPoints = cjyx.util.arrayFromMarkupsControlPoints(pointListNode)
...
pointListNodeObserverTag = markupsNode.AddObserver(cjyx.vtkDMMLMarkupsFiducialNode.PointModifiedEvent, onPointsModified)
...
[CjyxOpenAnatomy extension](https://github.com/PerkLab/SlicerOpenAnatomy/) provides rich export of models and segmentations (preserving names, hierarchy, etc.), but for a basic export operation this code snippet can be used:

```python
exporter = vtk.vtkGLTFExporter()
exporter.SetRenderWindow(cjyx.app.layoutManager().threeDWidget(0).threeDView().renderWindow())
...
for rowIndex in range(numberOfRows):
  ...
  for colIndex in range(numberOfColumns):
    ...
    customLayout += '<item><view class="vtkDMMLViewNode" singletontag="'+name
    customLayout += '"><property name="viewlabel" action="default">'+name+'</property></view></item>'
    viewIndex += 1
  customLayout += '</layout></item>'

customLayout += '</layout>'
if not cjyx.app.layoutManager().layoutLogic().GetLayoutNode().SetLayoutDescription(customLayoutId, customLayout):
    cjyx.app.layoutManager().layoutLogic().GetLayoutNode().AddLayoutDescription(customLayoutId, customLayout)

cjyx.app.layoutManager().setLayout(customLayoutId)
...
for modelIndex, modelFile in enumerate(modelFiles):
  ...
  viewNode = cjyx.dmmlScene.GetSingletonNode(name, "vtkDMMLViewNode")
  viewNode.LinkedControlOn()
  modelNode = cjyx.util.loadModel(modelDir + "/" + modelFile)
  modelNode.GetDisplayNode().AddViewNodeID(viewNode.GetID())

cjyx.app.setRenderPaused(False)
```
...
# Read model
inputModel = cjyx.util.loadModel(inputModelFile)
...
referenceVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLScalarVolumeNode")
...
cjyx.modules.segmentations.logic().ImportModelToSegmentationNode(inputModel, seg)
seg.CreateBinaryLabelmapRepresentation()
outputLabelmapVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLLabelMapVolumeNode")
cjyx.modules.segmentations.logic().ExportVisibleSegmentsToLabelmapNode(seg, outputLabelmapVolumeNode, referenceVolumeNode)
outputLabelmapVolumeArray = (cjyx.util.arrayFromVolume(outputLabelmapVolumeNode) * outputVolumeLabelValue).astype("int8")
