...
There are different approaches to install Cjyx and extensions programmatically:

- Install Cjyx manually and install extensions by using `cjyx.app.extensionsManagerModel()`. See example [below](#download-and-install-extension) and in [install-cjyx-extension.py](https://github.com/pieper/SlicerDockers/blob/master/slicer-plus/install-slicer-extension.py)
...
### Open a file with Cjyx at the command line

Open Cjyx to view the `c:\some\folder\MRHead.nrrd` image file:

```console
"c:\Users\myusername\AppData\Local\NA-MIC\Cjyx 4.11.20210226\Cjyx.exe" c:\some\folder\MRHead.nrrd
```

:::{note}

It is necessary to specify full path to the Cjyx executable and to the file that needs to be loaded.

:::

On Windows, the installer registers the `Cjyx` application during install. This makes it possible to use `start` command to launch the most recently installed Cjyx. For example, this command on the command-line starts Cjyx and loads an image:

```console
start Cjyx c:\some\folder\MRHead.nrrd
```

To load a file with non-default options, you can use `--python-code` option to run `cjyx.util.load...` commands.

### Open an .mrb file with Cjyx at the command line

```console
Cjyx.exe --python-code "cjyx.util.loadScene('f:/2013-08-23-Scene.mrb')"
```

### Run Python commands in the Cjyx environment

Run Python commands, without showing any graphical user interface:

```console
Cjyx.exe --python-code "doSomething; doSomethingElse; etc." --testing --no-splash --no-main-window
```

Cjyx exits when the commands are completed because `--testing` options is specified.

### Run a Python script file in the Cjyx environment

Run a Python script on Windows (stored in script file), without showing any graphical user interface:

 ```console
 Cjyx.exe --python-script "/full/path/to/myscript.py" --no-splash --no-main-window
 ```

 Run a Python script on MacOS (stored in script file), without showing any graphical user interface:

 ```console
 /Applications/Cjyx.app/Contents/MacOS/Cjyx --no-splash --no-main-window --python-script "/full/path/to/myscript.py"
 ```

To make Cjyx exit when the script execution is completed, call `sys.exit(errorCode)` (where `errorCode` is set 0 for success and other value to indicate error).

### Launch Cjyx directly from a web browser

Cjyx can be associated with the `cjyx:` custom URL protocol in the operating system or web browser. This is done automatically in the Windows installer and can be set up on other operating systems manually. After this when the user clicks on a `cjyx://...` URL in the web browser, Cjyx will start and the `cjyx.app` object emits a signal with the URL that modules can process. The DICOM module processes DICOMweb URLs, but any module can specify additional actions.
...
```
cjyx://viewer/?download=https%3A%2F%2Fgithub.com%2Frbumm%2FSlicerLungCTAnalyzer%2Freleases%2Fdownload%2FSampleData%2FLungCTAnalyzerChestCT.nrrd
```
...
```
cjyx://viewer/?studyUID=2.16.840.1.113669.632.20.121711.10000158860
...
### Get DMML node from the scene

Get markups point list node named `F` (useful for quickly getting access to a DMML node in the Python console):
...
- {func}`cjyx.util.getNode()` is recommended **only for interactive debugging** in the Python console/Jupyter notebook
...
- `cjyx.dmmlScene.GetNodeByID()` is more appropriate when a module needs to access a DMML node:
  - its behavior is more predictable: it only accepts node ID as input. `cjyx.dmmlScene.GetFirstNodeByName()` can be used to get a node by its name, but since multiple nodes in the scene can have the same name, it is not recommended to keep reference to a node by its name. Since node IDs may change when a scene is saved and reloaded, node ID should not be stored persistently, but [node references](dmml_overview.md#dmml-node-references) must be used instead
...
clonedItemID = cjyx.modules.subjecthierarchy.logic().CloneSubjectHierarchyItem(shNode, itemIDToClone)
...
# Generate file name
import time
sceneSaveFilename = cjyx.app.temporaryPath + "/saved-scene-" + time.strftime("%Y%m%d-%H%M%S") + ".mrb"

# Save scene
if cjyx.util.saveScene(sceneSaveFilename):
...
import time
sceneSaveDirectory = cjyx.app.temporaryPath + "/saved-scene-" + time.strftime("%Y%m%d-%H%M%S")
if not os.access(sceneSaveDirectory, os.F_OK):
  os.makedirs(sceneSaveDirectory)

# Save the scene
if cjyx.app.applicationLogic().SaveSceneToCjyxDataBundleDirectory(sceneSaveDirectory, None):
...
class MyModuleFileDialog ():
  ...
  See: Base/QTGUI/qCjyxScriptedFileDialog.h.
  ...
  def __init__(self,qCjyxFileDialog ):
    ...
    qCjyxFileDialog.fileType = "NoFile"
    qCjyxFileDialog.description = "Save scene"
    qCjyxFileDialog.action = cjyx.qCjyxFileDialog.Write
...
cjyx.util.mainWindow().installEventFilter(filter)
```
...
```python
defaultModelStorageNode = cjyx.vtkDMMLModelStorageNode()
defaultModelStorageNode.SetDefaultWriteFileExtension("ply")
cjyx.dmmlScene.AddDefaultNode(defaultModelStorageNode)
```
...
existingModelStorageNodes = cjyx.util.getNodesByClass("vtkDMMLModelStorageNode")
for modelStorageNode in existingModelStorageNodes:
  cjyx.dmmlScene.RemoveNode(modelStorageNode)
```
...
volumeNodes = cjyx.util.getNodesByClass("vtkDMMLScalarVolumeNode")
...
To set all volume nodes to save uncompressed by default (add this to [.cjyxrc.py file ](../user_guide/settings.md#application-startup-file) so it takes effect for the whole session):

```python
#set the default volume storage to not compress by default
defaultVolumeStorageNode = cjyx.vtkDMMLVolumeArchetypeStorageNode()
defaultVolumeStorageNode.SetUseCompression(0)
cjyx.dmmlScene.AddDefaultNode(defaultVolumeStorageNode)
...
```python
#set the default volume storage to not compress by default
defaultVolumeStorageNode = cjyx.vtkDMMLSegmentationStorageNode()
defaultVolumeStorageNode.SetUseCompression(0)
cjyx.dmmlScene.AddDefaultNode(defaultVolumeStorageNode)
...
```python
view=cjyx.app.layoutManager().sliceWidget("Red").sliceView()
```

### Show orientation marker in all views

```python
viewNodes = cjyx.util.getNodesByClass("vtkDMMLAbstractViewNode")
for viewNode in viewNodes:
  viewNode.SetOrientationMarkerType(cjyx.vtkDMMLAbstractViewNode.OrientationMarkerTypeAxes)
```
...
```python
labels = ["x", "X", "y", "Y", "z", "Z"]
viewNode = cjyx.app.layoutManager().threeDWidget(0).dmmlViewNode()
# for slice view:
# viewNode = cjyx.app.layoutManager().sliceWidget("Red").dmmlSliceNode()
...
```python
cjyx.app.layoutManager().threeDWidget(0).threeDController().setVisible(False)
cjyx.app.layoutManager().sliceWidget("Red").sliceController().setVisible(False)
cjyx.app.layoutManager().plotWidget(0).plotController().setVisible(False)
cjyx.app.layoutManager().tableWidget(0).tableController().setVisible(False)
```

### Hide Cjyx logo from main window

This script increases vertical space available in the module panel by hiding the Cjyx application logo.

```python
cjyx.util.findChild(cjyx.util.mainWindow(), "LogoLabel").visible = False
```

### Customize widgets in view controller bars

```python
sliceController = cjyx.app.layoutManager().sliceWidget("Red").sliceController()
...
crosshairNode=cjyx.util.getNode("Crosshair")
crosshairNode.AddObserver(cjyx.vtkDMMLCrosshairNode.CursorPositionModifiedEvent, onMouseMoved)
```
...
cjyx.vtkDMMLSliceNode.JumpAllSlices(cjyx.dmmlScene, *position_RAS, cjyx.vtkDMMLSliceNode.CenteredJumpSlice)
# Make the crosshair visible
crosshairNode.SetCrosshairMode(cjyx.vtkDMMLCrosshairNode.ShowBasic)
```
...
def onMouseMoved(observer,eventid):
  ...
  cjyx.util.showStatusMessage(f"RAS={_ras}   MNI={_mni}")

crosshairNode=cjyx.util.getNode("Crosshair")
observationId = crosshairNode.AddObserver(cjyx.vtkDMMLCrosshairNode.CursorPositionModifiedEvent, onMouseMoved)
...
```python
def printDataProbe():
  infoWidget = cjyx.modules.DataProbeInstance.infoWidget
  for layer in ("B", "F", "L"):
...
```python
invertedocean = cjyx.vtkDMMLColorTableNode()
...
cjyx.dmmlScene.AddNode(invertedocean)
```
...
colorLegendDisplayNode = cjyx.modules.colors.logic().AddDefaultColorLegendDisplayNode(volumeNode)
```
...
# Create color node
colorNode = cjyx.dmmlScene.CreateNodeByClass("vtkDMMLProceduralColorNode")
colorNode.UnRegister(None)  # to prevent memory leaks
colorNode.SetName(cjyx.dmmlScene.GenerateUniqueName("MyColormap"))
...
colorLegendDisplayNode = cjyx.modules.colors.logic().AddDefaultColorLegendDisplayNode(modelNode)
...
See description of standard layouts (that can be used as examples) here: https://github.com/Slicer/Slicer/blob/master/Libs/DMML/Logic/vtkDMMLLayoutLogic.cxx
...
```python
sliceDisplayNodes = cjyx.util.getNodesByClass("vtkDMMLSliceDisplayNode")
...
sliceNodes = cjyx.util.getNodesByClass('vtkDMMLSliceNode')
...
For this one I searched for "slice intersections" text in the whole Cjyx source code, found that the function is implemented in `Base\QTGUI\qCjyxViewersToolBar.cxx`, then translated the `qCjyxViewersToolBarPrivate::setSliceIntersectionVisible(bool visible)` method to Python.
...
sliceAnnotations = cjyx.modules.DataProbeInstance.infoWidget.sliceAnnotations
sliceAnnotations.sliceViewAnnotationsEnabled=False
sliceAnnotations.updateSliceViewFromGUI()
# Disable slice annotations persistently (after Cjyx restarts)
...
```python
sliceNode = cjyx.app.layoutManager().sliceWidget("Red").dmmlSliceNode()
...
```python
sliceNodeIds = ["vtkDMMLSliceNodeRed", "vtkDMMLSliceNodeYellow"]

# Print angles between slice nodes
def ShowAngle(unused1=None, unused2=None):
  sliceNormalVector = []
  for sliceNodeId in sliceNodeIds:
    sliceToRAS = cjyx.dmmlScene.GetNodeByID(sliceNodeId).GetSliceToRAS()
...
for sliceNodeId in sliceNodeIds:
  cjyx.dmmlScene.GetNodeByID(sliceNodeId).AddObserver(vtk.vtkCommand.ModifiedEvent, ShowAngle)
...
# Example usage:
sliceNode = getNode("vtkDMMLSliceNodeRed")
...
You can left-right "flip" slice view orientation presets (show patient left side on left/right side of the screen) by copy-pasting the script below to your [.cjyxrc.py file](../user_guide/settings.md#application-startup-file).
...
sliceNodes = cjyx.util.getNodesByClass("vtkDMMLSliceNode")
sliceNodes.append(cjyx.dmmlScene.GetDefaultNodeByClass("vtkDMMLSliceNode"))
for sliceNode in sliceNodes:
...
You can make slice views linked by default (when application starts or the scene is cleared) by copy-pasting the script below to your [.cjyxrc.py file ](../user_guide/settings.md#application-startup-file).

```python
# Set linked slice views  in all existing slice composite nodes and in the default node
sliceCompositeNodes = cjyx.util.getNodesByClass("vtkDMMLSliceCompositeNode")
defaultSliceCompositeNode = cjyx.dmmlScene.GetDefaultNodeByClass("vtkDMMLSliceCompositeNode")
if not defaultSliceCompositeNode:
  defaultSliceCompositeNode = cjyx.dmmlScene.CreateNodeByClass("vtkDMMLSliceCompositeNode")
  ...
  cjyx.dmmlScene.AddDefaultNode(defaultSliceCompositeNode)
...
You can change default slice jump mode (when application starts or the scene is cleared) by copy-pasting the script below to your [.cjyxrc.py file ](../user_guide/settings.md#application-startup-file).

```python
crosshair=cjyx.dmmlScene.GetFirstNodeByClass("vtkDMMLCrosshairNode")
...
for sliceViewName in lm.sliceViewNames():
  ...
  displayableManager = sliceView.displayableManagerByClassName("vtkDMMLRulerDisplayableManager")
...
viewOwnerNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLScriptedModuleNode")

# Create DMML nodes
viewLogic = cjyx.vtkDMMLSliceLogic()
viewLogic.SetDMMLScene(cjyx.dmmlScene)
...
# Create widget
viewWidget = cjyx.qDMMLSliceWidget()
viewWidget.setDMMLScene(cjyx.dmmlScene)
viewWidget.setDMMLSliceNode(viewNode)
sliceLogics = cjyx.app.applicationLogic().GetSliceLogics()
...
# Create DMML node
viewLogic = cjyx.vtkDMMLViewLogic()
viewLogic.SetDMMLScene(cjyx.dmmlScene)
...
# Create widget
viewWidget = cjyx.qDMMLThreeDWidget()
viewWidget.setDMMLScene(cjyx.dmmlScene)
viewWidget.setDMMLViewNode(viewNode)
viewWidget.show()
```
...
for threeDViewIndex in range(layoutManager.threeDViewCount) :
  ...
  threeDViewNode = view.dmmlViewNode()
  cameraNode = cjyx.modules.cameras.logic().GetViewActiveCameraNode(threeDViewNode)
...
for sliceViewName in layoutManager.sliceViewNames():
  ...
  sliceNode = view.dmmlSliceNode()
  sliceLogic = cjyx.app.applicationLogic().GetSliceLogic(sliceNode)
...
Displayable managers are responsible for creating VTK filters, mappers, and actors to display DMML nodes in renderers. Input to filters and mappers are VTK objects stored in DMML data nodes. Filter and actor properties are set based on display options specified in DMML display nodes.

Accessing displayable managers is useful for troubleshooting or for testing new features that are not exposed via DMML classes yet, as they provide usually allow low-level access to VTK actors.

```python
threeDViewWidget = cjyx.app.layoutManager().threeDWidget(0)
modelDisplayableManager = threeDViewWidget.threeDView().displayableManagerByClassName("vtkDMMLModelDisplayableManager")
...
```python
modelNode = cjyx.util.getNode("MyModel")
...
Keyboard shortcuts can be specified for activating any Cjyx feature by adding a couple of lines to your [.cjyxrc.py file](../user_guide/settings.md#application-startup-file).
...
```python
shortcuts = [
  ("Ctrl+b", lambda: cjyx.app.layoutManager().setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutOneUpRedSliceView)),
  ("Ctrl+n", lambda: cjyx.app.layoutManager().setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutOneUpYellowSliceView)),
  ("Ctrl+m", lambda: cjyx.app.layoutManager().setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutOneUpGreenSliceView)),
  ("Ctrl+,", lambda: cjyx.app.layoutManager().setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutFourUpView))
  ]

for (shortcutKey, callback) in shortcuts:
  shortcut = qt.QShortcut(cjyx.util.mainWindow())
  shortcut.setKey(qt.QKeySequence(shortcutKey))
  shortcut.connect( "activated()", callback)
```

Here's an example for cycling through Segment Editor effects (requested [on the Cjyx forum](https://discourse.slicer.org/t/is-there-a-keystroke-to-cycle-through-effects-in-segment-editor/10117/2) for the [SlicerMorph](https://slicermorph.github.io/) project).

```python
def cycleEffect(delta=1):
  try:
    orderedNames = list(cjyx.modules.SegmentEditorWidget.editor.effectNameOrder())
    allNames = cjyx.modules.SegmentEditorWidget.editor.availableEffectNames()
    ...
    activeEffect = cjyx.modules.SegmentEditorWidget.editor.activeEffect()
    if activeEffect:
      activeName = cjyx.modules.SegmentEditorWidget.editor.activeEffect().name
    else:
      activeName = None
    newIndex = (orderedNames.index(activeName) + delta) % len(orderedNames)
    cjyx.modules.SegmentEditorWidget.editor.setActiveEffectByName(orderedNames[newIndex])
...
```python
threeDViewWidget = cjyx.app.layoutManager().threeDWidget(0)
cameraDisplayableManager = threeDViewWidget.threeDView().displayableManagerByClassName("vtkDMMLCameraDisplayableManager")
...
# Red slice view
sliceViewLabel = "Red"
sliceViewWidget = cjyx.app.layoutManager().sliceWidget(sliceViewLabel)
displayableManager = sliceViewWidget.sliceView().displayableManagerByClassName("vtkDMMLCrosshairDisplayableManager")
widget = displayableManager.GetSliceIntersectionWidget()

# Set crosshair position by left-click
widget.SetEventTranslation(widget.WidgetStateIdle, cjyx.vtkDMMLInteractionEventData.LeftButtonClickEvent, vtk.vtkEvent.NoModifier, widget.WidgetEventSetCrosshairPosition)
widget.SetEventTranslation(widget.WidgetStateIdle, cjyx.vtkDMMLInteractionEventData.LeftButtonClickEvent, vtk.vtkEvent.NoModifier, widget.WidgetEventSetCrosshairPosition)
...
sliceViewLabel = "Red"
sliceViewWidget = cjyx.app.layoutManager().sliceWidget(sliceViewLabel)
displayableManager = sliceViewWidget.sliceView().displayableManagerByClassName("vtkDMMLScalarBarDisplayableManager")
...
```python
interactorStyle = cjyx.app.layoutManager().sliceWidget("Red").sliceView().sliceViewInteractorStyle()
...
```python
lm = cjyx.app.layoutManager()
for viewIndex in range(cjyx.app.layoutManager().threeDViewCount):
...
```python
shortcut = qt.QShortcut(qt.QKeySequence("Ctrl+e"), cjyx.util.mainWindow())
shortcut.connect("activated()",
  lambda: cjyx.modules.markups.logic().JumpSlicesToLocation(0,0,0, True))
```

## Launch external applications

How to run external applications from Cjyx.

### Launch external process in startup environment

When a process is launched from Cjyx then by default Cjyx"s ITK, VTK, Qt, etc. libraries are used. If an external application has its own version of these libraries, then the application is expected to crash. To prevent crashing, the application must be run in the environment where Cjyx started up (without all Cjyx-specific library paths). This startup environment can be retrieved using {func}`cjyx.util.startupEnvironment()`.

Example: run Python3 script from Cjyx:
...
check_output(
  command_to_execute,
  env=cjyx.util.startupEnvironment()
  )
```
...
```python
extensionName = 'CjyxIGT'
em = cjyx.app.extensionsManagerModel()
if not em.isExtensionInstalled(extensionName):
  ...
  extensionPackageFilename = cjyx.app.temporaryPath+'/'+extensionMetaData['_id']
  cjyx.util.downloadFile(url, extensionPackageFilename)
...
try:
  ...
  if cjyx.util.confirmOkCancelDisplay("This module requires 'flywheel-sdk' Python package. Click OK to install it now."):
    cjyx.util.pip_install("flywheel-sdk")
    import flywheel
```
