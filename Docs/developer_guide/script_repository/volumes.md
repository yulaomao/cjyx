...
```python
loadedVolumeNode = cjyx.util.loadVolume("c:/Users/abc/Documents/MRHead.nrrd")
```
...
```python
loadedVolumeNode = cjyx.util.loadVolume("c:/Users/abc/Documents/SomeImage/file001.png", {"singleFile": False})
```

:::{note}

The following options can be passed to load volumes programmatically when using `qCjyxVolumesReader`:
...
- `colorNodeID` (string): ID of the color node used to display the volume. Default is `vtkDMMLColorTableNodeGrey` for scalar volume and `vtkDMMLColorTableNodeFileGenericColors.txt` for labelmap volume.
...
cjyx.util.exportNode(volumeNode, "c:/tmp/test.nrrd")
```
...
```python
cjyx.util.exportNode(volumeNode, imagePath, {"useCompression": 0})
```
...
```python
cjyx.util.exportNode(volumeNode, imagePath, {"useCompression": 0}, world=True)
```
...
Cjyx does not provide reader for VTK XML image data file format (as they are not commonly used for storing medical images and they cannot store image axis directions) but such files can be read by using this script:
...
volumeNode=cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLScalarVolumeNode")
volumeNode.SetAndObserveImageData(imageData)
volumeNode.SetSpacing(spacing)
volumeNode.SetOrigin(origin)
cjyx.util.setSliceViewerLayers(volumeNode, fit=True)
```
...
again. To compute digest with algo *SHA256*, you can run {func}`cjyx.util.computeChecksum("SHA256", "path/to/file")`.
...
def reportProgress(msg, level=None):
  ...
  if cjyx.progressWindow.wasCanceled:
    raise Exception("download aborted")
  # Update progress window
  cjyx.progressWindow.show()
  cjyx.progressWindow.activateWindow()
  cjyx.progressWindow.setValue(int(sampleDataLogic.downloadPercent))
  cjyx.progressWindow.setLabelText("Downloading...")
  # Process events to allow screen to refresh
  cjyx.app.processEvents()

try:
  volumeNode = None
  cjyx.progressWindow = cjyx.util.createProgressDialog()
  sampleDataLogic = SampleData.SampleDataLogic()
  ...
  cjyx.progressWindow.close()
```

### Show volume rendering automatically when a volume is loaded

To show volume rendering of a volume automatically when it is loaded, add the lines below to your [.cjyxrc.py file](../user_guide/settings.md#application-startup-file).
...
cjyx.dmmlScene.AddObserver(cjyx.vtkDMMLScene.NodeAddedEvent, onNodeAdded)
```
...
def showVolumeRenderingMIP(volumeNode, useSliceViewColors=True):
  ...
  for viewNode in cjyx.util.getNodesByClass("vtkDMMLViewNode"):
    viewNode.SetRaycastTechnique(cjyx.vtkDMMLViewNode.MaximumIntensityProjection)
...
def randomSlices(volume, sliceCount, sliceShape):
  ...
  sliceNode = redWidget.dmmlSliceNode()
...
```python
sourceVolumeNode = cjyx.util.getNode("MRHead")
volumesLogic = cjyx.modules.volumes.logic()
clonedVolumeNode = volumesLogic.CloneVolume(cjyx.dmmlScene, sourceVolumeNode, "Cloned volume")
```
...
# Create volume node
volumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLScalarVolumeNode", nodeName)
...
dmmlScene->AddNode( volumeNode.GetPointer() );
...
```python
volumeNode = cjyx.util.getNode("MRHead")
ijk = [20,40,30]  # volume voxel coordinates

voxels = cjyx.util.arrayFromVolume(volumeNode)  # get voxels as a numpy array
...
getNode(nodeName).Modified() # at the end of all processing, notify Cjyx that the image modification is completed
```
...
cjyx.vtkDMMLTransformNode.GetTransformBetweenNodes(None, volumeNode.GetParentTransformNode(), transformRasToVolumeRas)
...
import numpy as np
volumeArray = cjyx.util.arrayFromVolume(volumeNode)
...
cjyx.vtkDMMLTransformNode.GetTransformBetweenNodes(volumeNode.GetParentTransformNode(), None, transformVolumeRasToRas)
...
```python
volumeNode=cjyx.util.getNode("Output DTI Volume")
imageData=volumeNode.GetImageData()
...
```python
displayNode.SetAndObserveColorNodeID("vtkDMMLColorTableNodeRainbow")
```

### Make mouse left-click and drag on the image adjust window/level

In older Cjyx versions, by default, left-click and drag in a slice view adjusted window/level of the displayed image. Window/level adjustment is now a new mouse mode that can be activated by clicking on its toolbar button or running this code:

```python
cjyx.app.applicationLogic().GetInteractionNode().SetCurrentInteractionMode(cjyx.vtkDMMLInteractionNode.AdjustWindowLevel)
```
...
```python
cjyx.util.setSliceViewerLayers(background=mrVolume, foreground=ctVolume)
```
...
```python
volumeNode = cjyx.util.getNode("YourVolumeNode")
cjyx.util.setSliceViewerLayers(background=volumeNode)
```
...
```python
n =  cjyx.util.getNode("YourVolumeNode")
for color in ["Red", "Yellow", "Green"]:
  cjyx.app.layoutManager().sliceWidget(color).sliceLogic().GetSliceCompositeNode().SetForegroundVolumeID(n.GetID())
```

### Change opacity of foreground volume in slice views

```python
cjyx.util.setSliceViewerLayers(foregroundOpacity=0.4)
```
...
def NoInterpolate(caller,event):
  for node in cjyx.util.getNodes("*").values():
    if node.IsA("vtkDMMLScalarVolumeDisplayNode"):
      node.SetInterpolate(0)

cjyx.dmmlScene.AddObserver(cjyx.dmmlScene.NodeAddedEvent, NoInterpolate)
```

You can place this code snippet in your [.cjyxrc.py file](../user_guide/settings.md#application-startup-file) to always disable interpolation by default.
...
outputVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLScalarVolumeNode", "MRHeadFiltered")

# Run processing
inputImage = sitkUtils.PullVolumeFromCjyx(inputVolumeNode)
filter = sitk.SignedMaurerDistanceMapImageFilter()
outputImage = filter.Execute(inputImage)
sitkUtils.PushVolumeToCjyx(outputImage, outputVolumeNode)

# Show processing result
cjyx.util.setSliceViewerLayers(background=outputVolumeNode)
```

More information:

- See the SimpleITK documentation for SimpleITK examples: https://simpleitk.org/doxygen/latest/html/examples.html
- sitkUtils in Cjyx is used for pushing and pulling images from Cjyx to SimpleITK: https://github.com/Slicer/Slicer/blob/master/Base/Python/sitkUtils.py
...
voxels = cjyx.util.arrayFromVolume(volumeNode)  # Get volume as numpy array
...
```python
sliceNodeID = "vtkDMMLSliceNodeRed"

# Get image data from slice view
sliceNode = cjyx.dmmlScene.GetNodeByID(sliceNodeID)
...
voxels = cjyx.util.arrayFromVolume(volumeNode)
print(voxels.shape)
```
...
import cjyx.util
a = cjyx.util.arrayFromVolume(input1Volume)
b = cjyx.util.arrayFromVolume(input2Volume)
...
volumeNode = cjyx.modules.volumes.logic().CloneVolume(input1Volume, "Difference")
cjyx.util.updateVolumeFromArray(volumeNode, c)
...
cjyx.util.arrayFromVolumeModified(volumeNode)
```
...
labelmapVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLLabelMapVolumeNode")
cjyx.modules.segmentations.logic().ExportVisibleSegmentsToLabelmapNode(segmentationNode, labelmapVolumeNode, volumeNode)

# Masking
import numpy as np
voxels = cjyx.util.arrayFromVolume(volumeNode)
mask = cjyx.util.arrayFromVolume(labelmapVolumeNode)
...
maskedVolumeNode = cjyx.modules.volumes.logic().CloneVolume(volumeNode, "Masked")
cjyx.util.updateVolumeFromArray(maskedVolumeNode, maskedVoxels)
cjyx.util.setSliceViewerLayers(maskedVolumeNode)
```
...
```python
sliceNode = cjyx.dmmlScene.GetNodeByID("vtkDMMLSliceNodeRed")
appLogic = cjyx.app.applicationLogic()
sliceLogic = appLogic.GetSliceLogic(sliceNode)
sliceLayerLogic = sliceLogic.GetBackgroundLayer()
reslice = sliceLayerLogic.GetReslice()
...
```python
  logic = cjyx.modules.volumerendering.logic()
  volumeNode = cjyx.dmmlScene.GetNodeByID('vtkDMMLScalarVolumeNode1')
...
```cpp
qCjyxAbstractCoreModule* volumeRenderingModule =
  qCjyxCoreApplication::application()->moduleManager()->module("VolumeRendering");
vtkCjyxVolumeRenderingLogic* volumeRenderingLogic =
  volumeRenderingModule ? vtkCjyxVolumeRenderingLogic::SafeDownCast(volumeRenderingModule->logic()) : 0;
vtkDMMLVolumeNode* volumeNode = dmmlScene->GetNodeByID('vtkDMMLScalarVolumeNode1');
if (volumeRenderingLogic)
  {
  vtkSmartPointer<vtkDMMLVolumeRenderingDisplayNode> displayNode =
    vtkSmartPointer<vtkDMMLVolumeRenderingDisplayNode>::Take(volumeRenderingLogic->CreateVolumeRenderingDisplayNode());
  dmmlScene->AddNode(displayNode);
  volumeNode->AddAndObserveDisplayNodeID(displayNode->GetID());
...
vtkDMMLVolumePropertyNode* propertyNode = displayNode->GetVolumePropertyNode();
...
You need to derive from [vtkDMMLVolumeRenderingDisplayNode](https://apidocs.slicer.org/master/classvtkDMMLVolumeRenderingDisplayNode.html) and register your class within [vtkSlicerVolumeRenderingLogic](https://apidocs.slicer.org/master/classvtkSlicerVolumeRenderingLogic.html).

C++:

```cpp
void qCjyxMyABCVolumeRenderingModule::setup()
{
  vtkDMMLThreeDViewDisplayableManagerFactory::GetInstance()->
    RegisterDisplayableManager("vtkDMMLMyABCVolumeRenderingDisplayableManager");
  ...
  if (volumeRenderingModule)
    {
    vtkNew<vtkDMMLMyABCVolumeRenderingDisplayNode> displayNode;
    vtkCjyxVolumeRenderingLogic* volumeRenderingLogic =
      vtkCjyxVolumeRenderingLogic::SafeDownCast(volumeRenderingModule->logic());
...
Custom presets can be added to the volume rendering module by calling AddPreset() method of the volume rendering module logic. The example below shows how to define multiple custom volume rendering presets in an external DMML scene file and add them to the volume rendering module user interface.

Create a *MyPresets.dmml* file that describes two custom volume rendering presets:

```xml
<DMML version="Cjyx4.4.0">
  <VolumeProperty id="vtkDMMLVolumeProperty1" name="MyPreset1"     references="IconVolume:vtkDMMLVectorVolumeNode1;" interpolation="1" shade="1" diffuse="0.66" ambient="0.1" specular="0.62" specularPower="14" scalarOpacity="10 -3.52844023704529 0 56.7852325439453 0 79.2550277709961 0.428571432828903 415.119384765625 1 641 1" gradientOpacity="4 0 1 160.25 1" colorTransfer="16 0 0 0 0 98.7223 0.196078431372549 0.945098039215686 0.956862745098039 412.406 0 0.592157 0.807843 641 1 1 1" />
  <VectorVolume id="vtkDMMLVectorVolumeNode1" references="storage:vtkDMMLVolumeArchetypeStorageNode1;" />
  <VolumeArchetypeStorage id="vtkDMMLVolumeArchetypeStorageNode1" fileName="MyPreset1.png"  fileListMember0="MyPreset1.png" />

  <VolumeProperty id="vtkDMMLVolumeProperty2" name="MyPreset2"     references="IconVolume:vtkDMMLVectorVolumeNode2;" interpolation="1" shade="1" diffuse="0.66" ambient="0.1" specular="0.62" specularPower="14" scalarOpacity="10 -3.52844023704529 0 56.7852325439453 0 79.2550277709961 0.428571432828903 415.119384765625 1 641 1" gradientOpacity="4 0 1 160.25 1" colorTransfer="16 0 0 0 0 98.7223 0.196078431372549 0.945098039215686 0.956862745098039 412.406 0 0.592157 0.807843 641 1 1 1" />
  <VectorVolume id="vtkDMMLVectorVolumeNode2" references="storage:vtkDMMLVolumeArchetypeStorageNode2;" />
  <VolumeArchetypeStorage id="vtkDMMLVolumeArchetypeStorageNode2" fileName="MyPreset2.png"  fileListMember0="MyPreset2.png" />
</DMML>
```

For this example, thumbnail images for the presets should be located in the same directory as `MyPresets.dmml`, with the file names `MyPreset1.png` and `MyPreset2.png`.

Use the following code to read all the custom presets from *MyPresets.dmml* and load it into the scene:

```python
presetsScenePath = "MyPresets.dmml"

# Read presets scene
customPresetsScene = cjyx.vtkDMMLScene()
vrPropNode = cjyx.vtkDMMLVolumePropertyNode()
...
vrLogic = cjyx.modules.volumerendering.logic()
presetsScene = vrLogic.GetPresetsScene()
vrNodes = customPresetsScene.GetNodesByClass("vtkDMMLVolumePropertyNode")
