...
```python
markupsNode = cjyx.util.getNode('F')
cjyx.util.saveNode(markupsNode, "/path/to/MyMarkups.mkp.json")
```
...
```python
cjyx.modules.markups.logic().ExportControlPointsToCSV(markupsNode, "/path/to/MyControlPoints.csv")
```
...
```python
markupsNode = cjyx.util.loadMarkups("/path/to/MyMarkups.mkp.json")
```
...
```python
markupsNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLMarkupsCurveNode")
cjyx.modules.markups.logic().ImportControlPointsFromCSV(markupsNode, "/path/to/MyControlPoints.csv")
```
...
```python
cjyx.util.loadMarkupsFiducialList("/path/to/list/F.fcsv")
```
...
curveNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLMarkupsCurveNode")
cjyx.util.updateMarkupsControlPointsFromArray(curveNode, pointPositions)
```
...
The [qCjyxMarkupsPlaceWidget widget](https://apidocs.slicer.org/master/classqSlicerMarkupsPlaceWidget.html) can automatically activate placement of multiple points and can show buttons for deleting points, changing colors, lock, and hide points.

```python
w=cjyx.qCjyxMarkupsPlaceWidget()
w.setDMMLScene(cjyx.dmmlScene)
markupsNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLMarkupsCurveNode")
w.setCurrentNode(cjyx.dmmlScene.GetNodeByID(markupsNode.GetID()))
...
cjyx.modules.markups.logic().StartPlaceMode(placeModePersistence)
```
...
interactionNode = cjyx.dmmlScene.GetNodeByID("vtkDMMLInteractionNodeSingleton")
placeModePersistence = 1
interactionNode.SetPlaceModePersistence(placeModePersistence)
# mode 1 is Place, can also be accessed via cjyx.vtkDMMLInteractionNode().Place
...
Each vtkDMMLMarkupsFiducialNode has a vector of control points in it which can be accessed from python:

```python
pointListNode = getNode("vtkDMMLMarkupsFiducialNode1")
...
modelsLogic = cjyx.modules.models.logic()
...
pointListNode.AddObserver(cjyx.vtkDMMLMarkupsNode.PointModifiedEvent, UpdateSphere, 2)
```
...
markupsPositions = cjyx.util.arrayFromMarkupsControlPoints(markups)
import numpy as np
# initial guess
...
roiNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLMarkupsROINode")

cropVolumeParameters = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLCropVolumeParametersNode")
cropVolumeParameters.SetInputVolumeNodeID(volumeNode.GetID())
cropVolumeParameters.SetROINodeID(roiNode.GetID())
cjyx.modules.cropvolume.logic().SnapROIToVoxelGrid(cropVolumeParameters)  # optional (rotates the ROI to match the volume axis directions)
cjyx.modules.cropvolume.logic().FitROIToInputVolume(cropVolumeParameters)
cjyx.dmmlScene.RemoveNode(cropVolumeParameters)
```
...
planeNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLMarkupsPlaneNode')
...
def ShowAngle(unused1=None, unused2=None):
  ...
  for planeNodeName in planeNodeNames:
    planeNode = cjyx.util.getFirstNodeByClassByName("vtkDMMLMarkupsPlaneNode", planeNodeName)
...
for planeNodeName in planeNodeNames:
  planeNode = cjyx.util.getFirstNodeByClassByName("vtkDMMLMarkupsPlaneNode", planeNodeName)
  planeNode.AddObserver(cjyx.vtkDMMLMarkupsPlaneNode.PointModifiedEvent, ShowAngle)
...
def ShowAngle(unused1=None, unused2=None):
  ...
  for lineNodeName in lineNodeNames:
    lineNode = cjyx.util.getFirstNodeByClassByName("vtkDMMLMarkupsLineNode", lineNodeName)
...
for lineNodeName in lineNodeNames:
  lineNode = cjyx.util.getFirstNodeByClassByName("vtkDMMLMarkupsLineNode", lineNodeName)
  lineNode.AddObserver(cjyx.vtkDMMLMarkupsLineNode.PointModifiedEvent, ShowAngle)
...
projectedLineNode = cjyx.dmmlScene.AddNewNodeByClass(lineNode.GetClassName(), lineNode.GetName()+" projected")
...
points = cjyx.util.arrayFromMarkupsControlPoints(pointListNode)
line = cjyx.util.arrayFromMarkupsControlPoints(lineNode)
...
pointListObservation = [pointListNode, pointListNode.AddObserver(cjyx.vtkDMMLMarkupsNode.PointModifiedEvent, UpdateSlicePlane, 2)]
```
...
```python
interactionNode = cjyx.app.applicationLogic().GetInteractionNode()
selectionNode = cjyx.app.applicationLogic().GetSelectionNode()
selectionNode.SetReferenceActivePlaceNodeClassName("vtkDMMLMarkupsFiducialNode")
...
Alternatively, *qCjyxMarkupsPlaceWidget* widget can be used to initiate markup placement:
...
placeWidget = cjyx.qCjyxMarkupsPlaceWidget()
placeWidget.setDMMLScene(cjyx.dmmlScene)
...
pointListDisplayNode.SetViewNodeIDs(["vtkDMMLSliceNodeRed", "vtkDMMLViewNode1"]) # Only show in red slice view and first 3D view
```

### Get a notification if a markup control point position is modified

Event management of Cjyx-4.11 version is still subject to change. The example below shows how control point manipulation can be observed now.
...
def onMarkupChanged(caller,event):
  ...
  if movingMarkupIndex >= 0:
    ...
    isPreview = markupsNode.GetNthControlPointPositionStatus(movingMarkupIndex) == cjyx.vtkDMMLMarkupsNode.PositionPreview
    if isPreview:
...
pointListNode.AddObserver(cjyx.vtkDMMLMarkupsNode.PointModifiedEvent, onMarkupChanged)
pointListNode.AddObserver(cjyx.vtkDMMLMarkupsNode.PointStartInteractionEvent, onMarkupStartInteraction)
pointListNode.AddObserver(cjyx.vtkDMMLMarkupsNode.PointEndInteractionEvent, onMarkupEndInteraction)
```
...
pointListNode = cjyx.dmmlScene.GetFirstNodeByName("F")
...
1. Customize the code (replace A, B, C, D with your measurement names) and copy-paste the code into the Python console. This has to be done only once after Cjyx is started. Add it to [.cjyxrc.py file](../user_guide/settings.md#application-startup-file) so that it persists even if Cjyx is restarted.
...
def createMeasurements():
  for nodeName in ['A', 'B', 'C', 'D']:
    lineNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLMarkupsLineNode", nodeName)
...
shortcut1 = qt.QShortcut(cjyx.util.mainWindow())
...
1. Copy-paste the code into the Python console. This has to be done only once after Cjyx is started. Add it to [.cjyxrc.py file](../user_guide/settings.md#application-startup-file) so that it persists even if Cjyx is restarted.
...
def copyLineMeasurementsToClipboard():
  ...
  lineNodes = getNodesByClass('vtkDMMLMarkupsLineNode')
  for lineNode in lineNodes:
    # Get node filename that the length was measured on
    try:
      volumeNode = cjyx.dmmlScene.GetNodeByID(lineNode.GetNthControlPointAssociatedNodeID(0))
  ...
  cjyx.app.clipboard().setText("\n".join(measurements))
  cjyx.util.delayDisplay(f"Copied {len(measurements)} length measurements to the clipboard.")

shortcut2 = qt.QShortcut(cjyx.util.mainWindow())
shortcut2.setKey(qt.QKeySequence("Ctrl+m"))
shortcut2.connect( 'activated()', copyLineMeasurementsToClipboard)
```

### Use markups json files in Python - outside Cjyx

The examples below show how to use markups json files outside Cjyx, in any Python environment.
...
markupsDisplayableManager = threeDViewWidget.threeDView().displayableManagerByClassName('vtkDMMLMarkupsDisplayableManager')
...
markupsDisplayNodes = cjyx.util.getNodesByClass("vtkDMMLMarkupsDisplayNode")
