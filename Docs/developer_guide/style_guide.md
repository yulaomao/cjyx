...
Cjyx uses and extends libraries and toolkits multiple languages, which each has its own conventions. As a general rule, follow these conventions. For example, to get number of items in a VTK class use GetNumberOfXXX, but use XXXCount in a Qt class.
...
- Follow the Cjyx repository Flake8 configuration and run `python -m pre_commit run --all-files` to confirm compliance.
...
- Imports
  - Grouped in order of scope/commonallity
    ...
    - Local apps/library specific imports
      - Cjyx application imports and local/module imports may be grouped independently.
...
- Use the old-style VTK indentation (braces are in new line, indented by two spaces), until the entire Cjyx code base will be updated to use current VTK indentation style.
- Use VTK naming conventions:
  - Local variable should start with a lower case.
    Use:
    `void vtkCjyxSliceLogic::SetForegroundLayer(vtkCjyxSliceLayerLogic *foregroundLayer)`
    Instead of:
    ` void vtkCjyxSliceLogic::SetForegroundLayer(vtkCjyxSliceLayerLogic *ForegroundLayer) // wrong!`
...
- Order groups from local to global: for example, module then DMML then CTK then Qt then VTK then ITK then STL.
...
```
 ...
 #include "qCjyxMyModule.h"

 // MyModule includes
 #include "qCjyxMyModuleWidget.h"
 #include "vtkCjyxMyModuleLogic.h"
...
- DMML classes should only depend on vtk and itk (not Cjyx Logic or Qt)
- Logic classes depend on DMML to store state
- Logic classes should encapsulate vtk and itk pipelines to accomplish specific cjyx tasks (such as resampling volumes for display)
- GUI classes can depend on DMML and Logic and Qt
...
- When possible, one keyword to scope the change in the subject (i.e. "STYLE: README: ...", "BUG: vtkDMMLSliceLogic: ...")

### Commit message prefix

Subversion Commits to Cjyx require commit type in the comment.
...
- Bad: `ENH: More work in qCjyxXModuleWidget` -> `more work` is *too vague*, `qCjyxXModuleWidget` is too *low level*
...
vtkSetAndObserveDMMLNodeEventsMacro can't be used for observing all types of vtkObjects,
only vtkDMMLNode is expected by vtkDMMLAbstractLogic::OnDMMLNodeModified(...)
...
This will avoid having a Cjyx commit history made of uninformative messages such as:
...
The following log levels are used in Cjyx:
