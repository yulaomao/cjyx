...
def createModelFromVolume(inputVolumeNode):
  ...
  outputModelNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLModelNode")
  ...
  if cliNode.GetStatus() & cliNode.ErrorsMask:
    ...
    cjyx.dmmlScene.RemoveNode(cliNode)
...
headLabel = volumesLogic.CreateLabelVolume(cjyx.dmmlScene, head, 'head-label')
...
If the CLI module is executed using `cjyx.cli.run` method then the CLI module runs in a background thread, so the call to `startProcessing` will return right away and the user interface will not be blocked. The `cjyx.cli.run` call returns a cliNode (an instance of [vtkDMMLCommandLineModuleNode](https://slicer.org/doc/html/classvtkDMMLCommandLineModuleNode.html)) which can be used to monitor the progress of the module.
...
def onProcessingStatusUpdate(cliNode, event):
  ...
  if cliNode.IsA('vtkDMMLCommandLineModuleNode'):
...
Both Python scripted loadable modules and C++ loadable modules can be used from Python in multiple ways: by modifying DMML nodes, calling methods of the module logic object, and adding reusable GUI widgets provided by the module.
...
- Usually the text is found in a .ui file, in this case it is in [qDMMLSliceControllerWidget.ui](https://github.com/Slicer/Slicer/blob/dfef9574096a10c4f02337b59c5edfd6810b55db/Libs/DMML/Widgets/Resources/UI/qDMMLSliceControllerWidget.ui), open the file
...
- Search for that widget or action name in the repository, you should find a source file(s) that use it. In this case it will is [qDMMLSliceControllerWidget.cxx](https://github.com/Slicer/Slicer/blob/46345e8a3dba3d591a7f06767aff83a2beefad6a/Libs/DMML/Widgets/qDMMLSliceControllerWidget.cxx)
- Search for the action/widget name, and you'll find what it does - in this case it calls `setSliceModelModeVolumes` method, which calls `this->setSliceModelMode(vtkDMMLSliceNode::SliceResolutionMatchVolumes)`, which then calls `d->DMMLSliceNode->SetSliceResolutionMode(mode)`
- This means that this action calls `someSliceNode->SetSliceResolutionMode(vtkDMMLSliceNode::SliceResolutionMatchVolumes)` in Python syntax it is `someSliceNode.SetSliceResolutionMode(cjyx.vtkDMMLSliceNode.SliceResolutionMatchVolumes)`. For example, for the red slice node this will be:

```python
sliceNode = cjyx.dmmlScene.GetNodeByID('vtkDMMLSliceNodeRed')
sliceNode.SetSliceResolutionMode(cjyx.vtkDMMLSliceNode.SliceResolutionMatchVolumes)
```
