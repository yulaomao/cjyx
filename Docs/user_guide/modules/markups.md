...
- Buttons: These buttons apply changes to control points in the selected list.
  ...
  - Transformed: Check to show the transformed coordinates in the table. This will apply any transform to the points in the list and show that result. Keep unchecked to show the raw RAS values that are stored in DMML. If you harden the transform the transformed coordinates will be the same as the non transformed coordinates.
