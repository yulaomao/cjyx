...
To avoid the need to always manually select `Segmentation`, save the `.nrrd` file using the `.seg.nrrd` file extension. It makes Cjyx load the image as a segmentation by default.
...
- Legacy DICOM RT structure set export is available if `CjyxRT` extension is installed. RT structure sets are not recommended for storing segmentations, as they cannot store arbitrarily complex 3D shapes.
...
- [Segmentations Cjyx module on GitHub](https://github.com/Slicer/Slicer/tree/master/Modules/Loadable/Segmentations)
