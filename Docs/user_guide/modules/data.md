...
- **Show DMML ID's**: Show/hide in the tree view a second column containing the node ID of the nodes. Hidden by default
...
- **DMML node information**: Attribute list of the currently selected node. Attributes can be edited (double click in the "Attribute value" cell), added (with the "Add" button) or removed (with the "Remove" button).
...
To create a C++ plugin, implement a child class of qCjyxSubjectHierarchyAbstractPlugin, for Python plugin see below. Many examples can be found in Cjyx core and in the CjyxRT extension, look for folders named SubjectHierarchyPlugins.
    - Writing plugins in **Python**:
        - Child class of AbstractScriptedSubjectHierarchyPlugin which is a Python adaptor of the C++ qCjyxSubjectHierarchyScriptedPlugin class
    ...
    - **Role** plugins: add support for new data node types
        ...
        - Existing plugins in Cjyx core: Markups, Models, SceneViews, Charts, Folder, Tables, Transforms, LabelMaps, Volumes
    -  **Function** plugins: add feature in right-click context menu for certain types of nodes
        ...
        - Existing plugins in Cjyx core: CloneNode, ParseLocalData, Register, Segment, DICOM, Volumes, Markups, Models, Annotations, Segmentations, Segments, etc.
