...
"""
Usage:
    MeasureStartupTimes.py /path/to/Cjyx
"""
...
def collect_startup_times_normal(output_file, drop_cache=False, display_output=False):
    results = {}
    test = []
    (duration, result) = runCjyxAndExitWithTime(cjyx_executable, test, drop_cache=drop_cache)
...
for name in dir(cjyx.moduleNames):
  module = moduleManager.module(name)
  if module is None:
    continue
  moduleType = None
  if module.inherits('qCjyxCLIModule'):
    module_type = "CLI"
  elif module.inherits('qCjyxScriptedLoadableModule'):
    module_type = "Scripted"
  elif module.inherits('qCjyxLoadableModule'):
    module_type = "Loadable"
  elif module.inherits('qCjyxCoreModule'):
    module_type = "Core"
  else:
...
""".format(output_file))
    ...
    (returnCode, stdout, stderr) = runCjyxAndExit(cjyx_executable, ['--python-script', python_script.name])
...
def cjyxRevision():
    (returnCode, stdout, stderr) = runCjyxAndExit(cjyx_executable, [
        '--no-main-window', '--ignore-cjyxrc', '--disable-modules',
        '--python-code', 'print(cjyx.app.repositoryRevision)'
    ])
    assert returnCode == EXIT_SUCCESS
...
def collect_startup_times_excluding_one_module(output_file, module_list, drop_cache=False, display_output=False):
    ...
    for (idx, (moduleName, moduleType)) in enumerate(modules.iteritems(), start=1):
        ...
        (duration, result) = runCjyxAndExitWithTime(cjyx_executable, ['--testing', '--modules-to-ignore', moduleName], drop_cache=drop_cache)
        (returnCode, stdout, stderr) = result
        if display_output:
            if stdout:
                print("STDOUT [%s]\n" % stdout)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--display-cjyx-output", action="store_true")
    ...
    runCjyxAndExitWithTime = timecall(runCjyxAndExit, repeat=args.repeat)

    sliver_revision = cjyxRevision()
    ...
    common_kwargs = {
        'display_output': args.display_cjyx_output,
