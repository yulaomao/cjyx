...
Usage:
    CLISerializationTest.py
      /path/to/Cjyx /path/to/CLIExecutables /path/to/data_dir /path/to/temp_dir
"""


class CLISerializationTest:
    def __init__(self):
        self.CjyxExecutable = None
    ...
    def _runCLI(self, cli_name, option, json_file_path, parameters=[]):
        ...
        return run(self.CjyxExecutable, args)
...
if __name__ == '__main__':
    ...
    # Copy .dmml file to prevent modification of source tree
    dmml_source_path = os.path.join(data_dir, 'ExecutionModelTourTest.dmml')
    dmml_dest_path = os.path.join(temp_dir, 'ExecutionModelTourTestPython.dmml')
    shutil.copyfile(dmml_source_path, dmml_dest_path)

    # -- Test ExecutionModelTour --
    EMTSerializer = CLISerializationTest()
    EMTSerializer.CjyxExecutable = cjyx_executable
    CLIName = 'ExecutionModelTour'
    required_inputs = [
        '--transform1', '%s/ExecutionModelTourTestPython.dmml#vtkDMMLLinearTransformNode1' % (temp_dir),
        '--transform2', '%s/ExecutionModelTourTestPython.dmml#vtkDMMLLinearTransformNode2' % (temp_dir),
        mrHeadResampled,
        ctHeadAxial,
    ]
    serialize_options = [
