...
class TwoCLIsInParallelTest(ScriptedLoadableModule):
    ...
    """
        ...
        try:
            ...
            cjyx.selfTests = {}
        cjyx.selfTests['TwoCLIsInParallelTest'] = self.runTest
...
class TwoCLIsInParallelTestLogic(ScriptedLoadableModuleLogic):
    def __init__(self):
        self.Observations = []
        self.StatusModifiedEvent = cjyx.vtkDMMLCommandLineModuleNode().StatusModifiedEvent

        self.parameters = {}
        self.success = False

    def runModule1(self):
        cliModule = cjyx.modules.cli4test
        cliNode = cjyx.cli.createNode(cliModule)
        cliNode.SetName("CLIModule1")
        self.addObserver(cliNode, self.StatusModifiedEvent, self.onModule1Modified)
        cliNode = cjyx.cli.run(cliModule, cliNode, self.parameters, False)
    ...
    def runModule2(self):
        ...
        cliNode = cjyx.cli.run(cliModule, cliNode, self.parameters, True)
