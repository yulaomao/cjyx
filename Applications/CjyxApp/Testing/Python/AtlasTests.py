...
class AtlasTestsTest(ScriptedLoadableModuleTest):
    ...
    def perform_AtlasTest(self, downloads, testVolumePattern):
        ...
        volumeNode = cjyx.util.getNode(pattern=testVolumePattern)
        ...
        numModelHierarchies = cjyx.dmmlScene.GetNumberOfNodesByClass("vtkDMMLModelHierarchyNode")
        # get the number that we'll be manipulating
        numModelHierarchiesToManipulate = 0
        for h in range(numModelHierarchies):
            mh = cjyx.dmmlScene.GetNthNodeByClass(h, "vtkDMMLModelHierarchyNode")
        ...
        numSceneViews = cjyx.dmmlScene.GetNumberOfNodesByClass("vtkDMMLSceneViewNode")
        for s in range(numSceneViews):
            sv = cjyx.dmmlScene.GetNthNodeByClass(s, "vtkDMMLSceneViewNode")
