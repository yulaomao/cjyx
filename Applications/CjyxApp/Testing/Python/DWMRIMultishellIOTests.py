...
dmmlcore_testdata_path = "Libs/DMML/Core/Testing/TestData/"


multishell_dwi_451 = os.path.join(dmmlcore_testdata_path, "multishell-DWI-451dir.nhdr")
...
def test_nrrd_dwi_load(first_file, second_file=None):
    """
    - load a DWI NRRD file into Cjyx
    - validate b values and gradient vectors against original header
        - check the values in the vtkDMMLDiffusionWeightedVolumeNode
        - check the values in the vtkDMMLDWVNode attribute dictionary
    """
    if second_file is None:
        second_file = first_file

    # load NRRD into Cjyx
    storagenode = cjyx.vtkDMMLNRRDStorageNode()
    storagenode.SetFileName(first_file)
    dw_node = cjyx.vtkDMMLDiffusionWeightedVolumeNode()
    storagenode.ReadData(dw_node)

    cjyx_grads = numpy_support.vtk_to_numpy(dw_node.GetDiffusionGradients())
    cjyx_numgrads = cjyx_grads.shape[0]
    ...
    assert(len(parsed_nrrd.gradients) == cjyx_numgrads)
    ...
    for i in range(0, cjyx_numgrads):
        ...
        np.testing.assert_almost_equal(bval_parsed, dw_node.GetBValue(i), decimal=7,
                                       err_msg="DMMLNode b value does not match NRRD header")

        g_from_node = cjyx_grads[i, :]

        # gradients stored in the vtkDMMLDiffusionWeightedVolumeNode must be *normalized*.
...
def test_nrrd_dwi_roundtrip(test_nrrd_path):
    ...
    - loads and saves a NRRD file via Cjyx's I/O, twice
    ...
    storagenode1 = cjyx.vtkDMMLNRRDStorageNode()
    storagenode1.SetFileName(test_nrrd_path)
    dw_node1 = cjyx.vtkDMMLDiffusionWeightedVolumeNode()
    ...
    # re-save NRRD again
    storagenode2 = cjyx.vtkDMMLNRRDStorageNode()
