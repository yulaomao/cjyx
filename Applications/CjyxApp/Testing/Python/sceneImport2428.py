...
class sceneImport2428Test(ScriptedLoadableModuleTest):
    ...
    def test_sceneImport24281(self):
        ...
        headLabel = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLLabelMapVolumeNode')
        cjyx.modules.segmentations.logic().ExportVisibleSegmentsToLabelmapNode(segmentationNode, headLabel, head)
        ...
        outHierarchy = cjyx.vtkDMMLModelHierarchyNode()
        outHierarchy.SetScene(cjyx.dmmlScene)
        outHierarchy.SetName("sceneImport2428Hierachy")
        cjyx.dmmlScene.AddNode(outHierarchy)
        parameters["ModelSceneFile"] = outHierarchy

        modelMaker = cjyx.modules.modelmaker
        self.CLINode = None
        self.CLINode = cjyx.cli.runSync(modelMaker, self.CLINode, parameters, delete_temporary_files=False)

        self.delayDisplay("Models built")

        success = self.verifyModels()

        success = success and (cjyx.dmmlScene.GetNumberOfNodesByClass("vtkDMMLModelNode") > 3)
    ...
    # paste this in the cjyx console for testing/verifying any scene
    def verifyModels():
    ...
    verifyModels()
        ...
        fileNamesInScene = []
        success = True
        numModels = cjyx.dmmlScene.GetNumberOfNodesByClass("vtkDMMLModelNode")
        for n in range(numModels):
            modelNode = cjyx.dmmlScene.GetNthNodeByClass(n, "vtkDMMLModelNode")
