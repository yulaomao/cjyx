...
class DICOMReaders(ScriptedLoadableModule):
    def __init__(self, parent):
        ...
        parent.helpText = """
    This module was developed to confirm that different DICOM reading approaches result in the same volumes loaded in Cjyx (or that old readers fail but fixed readers succeed).
    """
    ...
    def test_AlternateReaders(self):
        ...
        for dataset in referenceData:
            try:
                ...
                #
                scalarVolumePlugin = cjyx.modules.dicomPlugins['DICOMScalarVolumePlugin']()
                ...
                for approachIndex in range(len(approachesThatLoaded)):
                    ...
                    for secondApproachIndex in range(approachIndex + 1, len(approachesThatLoaded)):
                        ...
                        comparison = cjyx.modules.dicomPlugins['DICOMScalarVolumePlugin'].compareVolumeNodes(firstVolume, secondVolume)
                        if comparison != "":
        ...
        cjyx.util.selectModule('DICOMReaders')
