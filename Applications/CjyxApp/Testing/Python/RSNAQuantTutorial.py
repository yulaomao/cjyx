...
class RSNAQuantTutorialTest(ScriptedLoadableModuleTest):
    ...
    def test_Part3PETCT(self, enableScreenshotsFlag=0, screenshotScaleFactor=1):
        ...
        cjyx.util.loadScene(preTreatmentPath)
        ...
        try:
            ...
            compositNode = redWidget.dmmlSliceCompositeNode()
            ...
            if not hasattr(cjyx.modules, 'petstandarduptakevaluecomputation'):
                self.delayDisplay("PET SUV Computation not available, skipping the test.")
                return

            cjyx.util.selectModule('PETStandardUptakeValueComputation')

            parameters = {
                "PETDICOMPath": extractPath + '/' + 'PET1',
                "PETVolume": cjyx.util.getNode('PET1'),
                "VOIVolume": cjyx.util.getNode('PET1-label'),
            }

            suvComputation = cjyx.modules.petstandarduptakevaluecomputation
            self.CLINode1 = None
            self.CLINode1 = cjyx.cli.runSync(suvComputation, self.CLINode1, parameters, delete_temporary_files=False)
            ...
            cjyx.util.loadScene(postTreatmentPath)
    ...
    def test_Part2ChangeTracker(self, enableScreenshotsFlag=0, screenshotScaleFactor=1):
        ...
        if not hasattr(cjyx.modules, 'changetracker'):
        ...
        try:
            ...
            changeTracker = cjyx.modules.changetracker.widgetRepresentation().self()

            baselineNode = cjyx.util.getNode('2006-spgr1')
            followupNode = cjyx.util.getNode('2007-spgr1')
            ...
            cjyx.util.clickAndDrag(redWidget, button='Right')
            self.takeScreenshot('ChangeTracker-Zoom', 'Inspect - zoom', -1)

            cjyx.util.clickAndDrag(redWidget, button='Middle')
            ...
            cjyx.util.clickAndDrag(compareWidget, button='Right')

            self.delayDisplay('Pan')
            cjyx.util.clickAndDrag(compareWidget, button='Middle')
