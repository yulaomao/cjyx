...
class UtilTest(ScriptedLoadableModule):
    def __init__(self, parent):
        ...
        parent.helpText = """
    This is a self test that tests the methods of cjyx.util
    """
...
class UtilTestWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        self.Widget = cjyx.util.loadUI(path)
...
class UtilTestTest(ScriptedLoadableModuleTest):
    ...
    def test_setSliceViewerLayers(self):
        self.delayDisplay('Testing cjyx.util.setSliceViewerLayers')

        layoutManager = cjyx.app.layoutManager()
        layoutManager.layout = cjyx.vtkDMMLLayoutNode.CjyxLayoutOneUpRedSliceView

        redSliceCompositeNode = cjyx.dmmlScene.GetNodeByID('vtkDMMLSliceCompositeNodeRed')
        ...
        labelmapNode = volumesLogic.CreateAndAddLabelVolume(cjyx.dmmlScene, backgroundNode, 'Labelmap')
        ...
        # Try with nothing
        cjyx.util.setSliceViewerLayers(background=None, foreground=None, label=None)
        self.assertIsNone(redSliceCompositeNode.GetBackgroundVolumeID())
        self.assertIsNone(redSliceCompositeNode.GetForegroundVolumeID())
        self.assertIsNone(redSliceCompositeNode.GetLabelVolumeID())

        # Try with nodes
        cjyx.util.setSliceViewerLayers(
        ...
        otherLabelmapNode = volumesLogic.CreateAndAddLabelVolume(cjyx.dmmlScene, backgroundNode, 'OtherLabelmap')
        ...
        self.delayDisplay('Testing cjyx.util.setSliceViewerLayers passed')

    def test_loadUI(self):
        # Try to load a UI that does not exist and catch exception
        caughtException = False
        try:
            cjyx.util.loadUI('does/not/exists.ui')
        except RuntimeError:
        ...
        label = cjyx.util.findChild(utilWidget.parent, 'UtilTest_Label')
    ...
    def test_findChild(self):
        ...
        # Try to get a widget that exists
        label = cjyx.util.findChild(utilWidget.Widget, 'UtilTest_Label')
        self.assertIsNotNone(label, qt.QLabel)
        self.assertEqual(label.text, 'My custom UI')
        ...
        caughtException = False
        try:
            cjyx.util.findChild(utilWidget.Widget, 'Unexistant_Label')
        except RuntimeError:
            caughtException = True
        self.assertTrue(caughtException)
    ...
    def test_arrayFromVolume(self):
        ...
        narray = cjyx.util.arrayFromVolume(volumeNode)
        ...
        self.delayDisplay('Testing cjyx.util.test_arrayFromVolume passed')
    ...
    def test_updateVolumeFromArray(self):
        ...
        cjyx.util.updateVolumeFromArray(volumeNode, f)
        ...
        self.delayDisplay('Testing cjyx.util.test_updateVolumeFromArray passed')
    ...
    def test_updateTableFromArray(self):
        ...
        tableNode1 = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLTableNode")
        cjyx.util.updateTableFromArray(tableNode1, a)
        ...
        histogram = np.histogram(cjyx.util.arrayFromVolume(volumeNode))

        self.delayDisplay('Test table update')
        tableNode2 = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLTableNode")
        cjyx.util.updateTableFromArray(tableNode2, histogram)
        self.assertEqual(tableNode2.GetNumberOfColumns(), 2)
        self.assertEqual(tableNode2.GetNumberOfRows(), 11)

        self.delayDisplay('Testing cjyx.util.test_updateTableFromArray passed')
    ...
    def test_arrayFromModelPoints(self):
        ...
        sphere.Update()
        modelNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLModelNode')
        modelNode.SetAndObservePolyData(sphere.GetOutput())
        modelNode.CreateDefaultDisplayNodes()
        a = cjyx.util.arrayFromModelPoints(modelNode)

        self.delayDisplay('Change Y scaling')
        a[:, 2] = a[:, 2] * 2.5
        modelNode.GetPolyData().Modified()

        self.delayDisplay('Testing cjyx.util.test_arrayFromModelPoints passed')
    ...
    def test_arrayFromVTKMatrix(self):
        ...
        vmatrix = cjyx.util.vtkMatrixFromArray(a)
        ...
        narray = cjyx.util.arrayFromVTKMatrix(vmatrix)
        np.testing.assert_array_equal(a, narray)

        vmatrixExisting = vtk.vtkMatrix4x4()
        cjyx.util.updateVTKMatrixFromArray(vmatrixExisting, a)
        narray = cjyx.util.arrayFromVTKMatrix(vmatrixExisting)
        np.testing.assert_array_equal(a, narray)
        ...
        caughtException = False
        try:
            vmatrix = cjyx.util.vtkMatrixFromArray(np.zeros([3, 4]))
        except RuntimeError:
            caughtException = True
        self.assertTrue(caughtException)
    ...
    def test_arrayFromTransformMatrix(self):
        ...
        narray = cjyx.util.arrayFromTransformMatrix(transformNode)
        ...
        parentTransformNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLTransformNode')
        ...
        narrayParent = cjyx.util.arrayFromTransformMatrix(parentTransformNode)

        transformNode.SetAndObserveTransformNodeID(parentTransformNode.GetID())

        narrayToWorld = cjyx.util.arrayFromTransformMatrix(transformNode, toWorld=True)
        ...
        cjyx.util.updateTransformMatrixFromArray(transformNode, narrayUpdated)
        ...
        cjyx.util.updateTransformMatrixFromArray(transformNode, narrayUpdated, toWorld=True)
        transformMatrixUpdated = vtk.vtkMatrix4x4()
    ...
    def test_arrayFromMarkupsControlPoints(self):
        ...
        markupsNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLMarkupsFiducialNode')
        ...
        narray = cjyx.util.arrayFromMarkupsControlPoints(markupsNode)
        ...
        narray = cjyx.util.arrayFromMarkupsControlPoints(markupsNode, world=True)
        self.assertEqual(narray.shape, (5, 3))
        ...
        cjyx.util.updateMarkupsControlPointsFromArray(markupsNode, narray)
        ...
        narray = np.array([[2, 3, 4], [6, 7, 8]])
        cjyx.util.updateMarkupsControlPointsFromArray(markupsNode, narray, world=True)
        self.assertEqual(markupsNode.GetNumberOfControlPoints(), 2)
    ...
    def test_array(self):
        ...
        # narray = cjyx.util.array(tensorVolumeNode.GetName())
        ...
        narray = cjyx.util.array(modelNode.GetName())
        ...
        markupsNode.AddControlPoint(vtk.vtkVector3d(45, 45, 55))
        markupsNode.AddControlPoint(vtk.vtkVector3d(51, 41, 59))
        narray = cjyx.util.array(markupsNode.GetName())
        self.assertEqual(narray.shape, (5, 3))
        ...
        transformMatrix.SetElement(2, 3, 44.0)
        transformNode.SetMatrixTransformToParent(transformMatrix)
        narray = cjyx.util.array(transformNode.GetName())
        self.assertEqual(narray.shape, (4, 4))

        self.delayDisplay('Testing cjyx.util.test_array passed')
