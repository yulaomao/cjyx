...
__all__ = ['EXIT_FAILURE', 'EXIT_SUCCESS', 'run', 'runCjyx', 'runCjyxAndExit', 'timecall']
...
def runCjyx(cjyx_executable, arguments=[], verbose=True, **kwargs):
    """Run ``cjyx_executable`` with provided ``arguments``.
    """
    args = ['--no-splash']
    args.extend(arguments)
    return run(cjyx_executable, args, verbose, **kwargs)


def runCjyxAndExit(cjyx_executable, arguments=[], verbose=True, **kwargs):
    """Run ``cjyx_executable`` with provided ``arguments`` and exit.
    """
    args = ['--exit-after-startup']
    args.extend(arguments)
    return runCjyx(cjyx_executable, args, verbose, **kwargs)
