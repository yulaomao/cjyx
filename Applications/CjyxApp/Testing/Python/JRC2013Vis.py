...
class JRC2013VisWidget(ScriptedLoadableModuleWidget):
    ...
    def onStartStopDicomPeer(self, flag):
        if flag:
            ...
            dicomFilesDirectory = cjyx.app.temporaryPath
            configFilePath = dicomFilesDirectory + '/Dcmtk-db/dcmqrscp.cfg'
            processCurrentPath = dicomFilesDirectory + '/Dcmtk-db/'

            if cjyx.util.confirmYesNoDisplay('Do you want to choose local DCMTK database folder?'):
                print('Yes')
            ...
            for path in dcmqrscpExeOptions:
                testPath = cjyx.app.cjyxHome + path + dcmqrscpExeName
...
class JRC2013VisTest(ScriptedLoadableModuleTest):
    ...
    def test_Part1DICOM(self):
        ...
        try:
            ...
            dicomRetrieve.setCallingAETitle('CjyxAE')
            ...
            browserWidget = cjyx.modules.DICOMWidget.browserWidget
            ...
            cjyx.util.clickAndDrag(redWidget, start=(10, 10), end=(10, 40))

            self.delayDisplay('Change Window')
            cjyx.util.clickAndDrag(redWidget, start=(10, 10), end=(40, 10))

            self.delayDisplay('Change Layout')
            layoutManager = cjyx.app.layoutManager()
            layoutManager.setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutOneUpRedSliceView)

            self.delayDisplay('Zoom')
            cjyx.util.clickAndDrag(redWidget, button='Right', start=(10, 10), end=(10, 40))

            self.delayDisplay('Pan')
            cjyx.util.clickAndDrag(redWidget, button='Middle', start=(10, 10), end=(40, 40))
            ...
            cjyx.util.clickAndDrag(redWidget, button='None', start=(100, 100), end=(140, 140), modifiers=['Shift'])
            ...
            cjyx.util.clickAndDrag(threeDView)

            self.delayDisplay('Zoom')
            threeDView = layoutManager.threeDWidget(0).threeDView()
            cjyx.util.clickAndDrag(threeDView, button='Right')
    ...
    def test_Part2Head(self):
        """ Test using the head atlas - may not be needed - Cjyx4Minute is already tested
        """
        ...
        try:
            ...
            layoutManager = cjyx.app.layoutManager()
            threeDView = layoutManager.threeDWidget(0).threeDView()
            redWidget = layoutManager.sliceWidget('vtkDMMLSliceNode1')  # it would be 'Red' in a recent scene
            redController = redWidget.sliceController()
            greenWidget = layoutManager.sliceWidget('vtkDMMLSliceNode2')  # it would be 'Green' in a recent scene
            ...
            hemispheric_white_matter = cjyx.util.getNode(pattern='hemispheric_white_matter.vtk')
            hemispheric_white_matter.GetDisplayNode().SetClipping(1)
            clip = cjyx.dmmlScene.GetFirstNodeByClass('vtkDMMLClipModelsNode')
            clip.SetRedSliceClipState(0)
            clip.SetYellowSliceClipState(0)
            clip.SetGreenSliceClipState(2)

            viewNode = threeDView.dmmlViewNode()
            cameras = cjyx.util.getNodes('vtkDMMLCameraNode*')
    ...
    def test_Part3Liver(self):
        ...
        try:
            ...
            mainWindow.moduleSelector().selectModule('Models')
            segmentII = cjyx.util.getNode('LiverSegment_II')
            segmentII.GetDisplayNode().SetVisibility(0)
            cjyx.util.clickAndDrag(threeDView, start=(10, 200), end=(10, 10))
            ...
            segmentVII = cjyx.util.getNode('LiverSegment_II')
            redWidget.sliceController().setSliceVisible(True)

            self.delayDisplay('Middle Hepatic')
            models = cjyx.util.getNodes('vtkDMMLModelNode*')
            ...
            for nodeName in transparentNodes:
                modelNode = cjyx.util.getNode(nodeName)
    ...
    def test_Part4Lung(self):
        ...
        try:
            ...
            redWidget.sliceController().setSliceVisible(True)
            lungs = cjyx.util.getNode('chestCT_lungs')
            ...
            cameraNode.GetCamera().Elevation(-10)
            lowerLobeNodes = cjyx.util.getNodes('*LowerLobe*')
