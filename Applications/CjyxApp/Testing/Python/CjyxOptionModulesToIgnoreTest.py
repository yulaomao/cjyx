...
"""
Usage:
    CjyxOptionModulesToIgnoreTest.py /path/to/Cjyx
"""
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print(os.path.basename(sys.argv[0]) + " /path/to/Cjyx /path/to/cjyxExtensionWizard")
        exit(EXIT_FAILURE)

    cjyx_executable = os.path.expanduser(sys.argv[1])
    cjyx_extension_wizard = os.path.expanduser(sys.argv[2])
    ...
    (returnCode, stdout, stderr) = run(cjyx_extension_wizard, args, shell=True)
    ...
    try:
        ...
        args.extend(['--python-code', 'assert len([module for module in dir(cjyx.moduleNames) if module in ["A", "B", "C", "D"]]) == 4, "Failed to load modules"'])
        ...
        args.extend(['--python-code', 'cjyx.app.moduleManager().factoryManager().modulesToIgnore = ["B"]'])
        args.extend(additional_module_paths)
        (returnCode, stdout, stderr) = runCjyxAndExit(cjyx_executable, args)
        assert returnCode == EXIT_SUCCESS
        print("=> ok\n")
        ...
        args = list(common_args)
        args.extend(['--keep-temporary-settings'])
        args.extend(['--python-code', 'assert len([module for module in dir(cjyx.moduleNames) if module in ["A", "C", "D"]]) == 3, "Failed to disable modules"'])
        args.extend(additional_module_paths)
        (returnCode, stdout, stderr) = runCjyxAndExit(cjyx_executable, args)
        assert returnCode == EXIT_SUCCESS
        print("=> ok\n")
        ...
        args.extend(['--python-code', 'assert len([module for module in dir(cjyx.moduleNames) if module in ["C"]]) == 1, "Failed to disable modules"'])
        args.extend(additional_module_paths)
        (returnCode, stdout, stderr) = runCjyxAndExit(cjyx_executable, args)
        assert returnCode == EXIT_SUCCESS
        print("=> ok\n")
