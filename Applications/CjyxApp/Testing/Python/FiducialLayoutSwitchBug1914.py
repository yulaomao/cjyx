...
class FiducialLayoutSwitchBug1914Logic(ScriptedLoadableModuleLogic):
    ...
    def getPointSliceDisplayableManagerHelper(self, sliceName='Red'):
        sliceWidget = cjyx.app.layoutManager().sliceWidget(sliceName)
        sliceView = sliceWidget.sliceView()
        collection = vtk.vtkCollection()
        sliceView.getDisplayableManagers(collection)
        for i in range(collection.GetNumberOfItems()):
            m = collection.GetItemAsObject(i)
            if m.GetClassName() == "vtkDMMLMarkupsFiducialDisplayableManager2D":
                return m.GetHelper()
        return None
...
class FiducialLayoutSwitchBug1914Test(ScriptedLoadableModuleTest):
    ...
    def test_FiducialLayoutSwitchBug1914(self):
        ...
        # without this delayed display, when running from the cmd line Cjyx starts
        ...
        # Pan and zoom
        sliceWidget = cjyx.app.layoutManager().sliceWidget('Red')
        ...
        self.takeScreenshot('FiducialLayoutSwitchBug1914-StartingPosition', 'Point starting position', cjyx.qDMMLScreenShotDialog.Red)
        ...
        self.takeScreenshot('FiducialLayoutSwitchBug1914-EndingPosition', 'Point ending position', cjyx.qDMMLScreenShotDialog.Red)
        ...
        if enableScreenshots == 1:
            ...
            startView = cjyx.dmmlScene.GetFirstNodeByName('FiducialLayoutSwitchBug1914-StartingPosition')
            startShot = startView.GetScreenShot()
            endView = cjyx.dmmlScene.GetFirstNodeByName('FiducialLayoutSwitchBug1914-EndingPosition')
            ...
            annotationLogic.CreateSnapShot("FiducialLayoutSwitchBug1914-Diff", "Difference between starting and ending point positions",
                                           cjyx.qDMMLScreenShotDialog.Red, screenshotScaleFactor, shotDiff)
