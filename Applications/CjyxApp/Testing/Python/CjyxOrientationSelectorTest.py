...
#
# CjyxOrientationSelectorTest
#

class CjyxOrientationSelectorTest(ScriptedLoadableModule):
...
#
# CjyxOrientationSelectorTestWidget
#

class CjyxOrientationSelectorTestWidget(ScriptedLoadableModuleWidget):
...
#
# CjyxOrientationSelectorTestLogic
#

class CjyxOrientationSelectorTestLogic(ScriptedLoadableModuleLogic):
...
class CjyxOrientationSelectorTestTest(ScriptedLoadableModuleTest):
    """
    ...
    def runTest(self):
        ...
        """
        self.setUp()
        self.test_CjyxOrientationSelectorTest()

    def test_CjyxOrientationSelectorTest(self):
        ...
        logic = CjyxOrientationSelectorTestLogic()

        self.delayDisplay("Starting the test")
        import SampleData
        mrHeadVolume = SampleData.downloadSample("MRHead")

        cjyx.util.selectModule('Reformat')

        # Select Red slice
        widget = cjyx.modules.reformat.widgetRepresentation()
        sliceNodeSelector = cjyx.util.findChildren(widget, "SliceNodeSelector")[0]
        sliceNodeSelector.setCurrentNodeID("vtkDMMLSliceNodeRed")

        # Set LR value using Reformat module
        lrslider = cjyx.util.findChildren(widget, "LRSlider")[0]
        lrslider.value = 1

        # Get reference to the Red slice controller
        lm = cjyx.app.layoutManager()
        sliceWidget = lm.sliceWidget("Red")
        sliceOrientationSelector = cjyx.util.findChildren(sliceWidget, "SliceOrientationSelector")[0]
