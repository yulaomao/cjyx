...
#ifndef __qDMMLSubjectHierarchyTreeView_h
#define __qDMMLSubjectHierarchyTreeView_h
...
class qDMMLSortFilterSubjectHierarchyProxyModel;
...
class Q_CJYX_MODULE_SUBJECTHIERARCHY_WIDGETS_EXPORT qDMMLSubjectHierarchyTreeView : public QTreeView
{
  Q_OBJECT
  QVTK_OBJECT
...
public:
  ...
  qDMMLSubjectHierarchyTreeView(QWidget *parent=nullptr);
  ~qDMMLSubjectHierarchyTreeView() override;
...
public:
  ...
  virtual vtkDMMLNode* currentNode()const;
  ...
  Q_INVOKABLE qDMMLSortFilterSubjectHierarchyProxyModel* sortFilterProxyModel()const;
  Q_INVOKABLE qDMMLSubjectHierarchyModel* model()const;
...
public slots:
  ...
  /// Convenience method to set current item by associated data node
  virtual void setCurrentNode(vtkDMMLNode* node);
...
protected:
  QScopedPointer<qDMMLSubjectHierarchyTreeViewPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLSubjectHierarchyTreeView);
  Q_DISABLE_COPY(qDMMLSubjectHierarchyTreeView);
  friend class qDMMLSubjectHierarchyComboBox;
};
