...
#ifndef __qDMMLSubjectHierarchyModel_h
#define __qDMMLSubjectHierarchyModel_h
...
class qDMMLSubjectHierarchyModelPrivate;
...
/// vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemModifiedEvent)
///
class Q_CJYX_MODULE_SUBJECTHIERARCHY_WIDGETS_EXPORT qDMMLSubjectHierarchyModel : public QStandardItemModel
{
  Q_OBJECT
  QVTK_OBJECT

  /// Control in which column the data DMML node names or if not assigned then subject hierarchy
  ...
  /// Control in which column data DMML node visibility are displayed (Qt::DecorationRole).
  /// A value of -1 (default) hides the column
  Q_PROPERTY (int visibilityColumn READ visibilityColumn WRITE setVisibilityColumn)
  /// Control in which column data DMML node color is displayed.
  /// A value of -1 (default) hides the column
  Q_PROPERTY(int colorColumn READ colorColumn WRITE setColorColumn)
    /// Control in which column the parent transforms are displayed
  /// A DMML node combobox is displayed in the row of the transformable nodes, in which
  ...
  /// Control in which column the data DMML node IDs are displayed (Qt::DisplayRole).
...
public:
  ...
  qDMMLSubjectHierarchyModel(QObject *parent=nullptr);
  ~qDMMLSubjectHierarchyModel() override;
  ...
  enum ItemDataRole
    ...
    VisibilityRole,
    /// DMML node ID of the parent transform
    TransformIDRole,
  ...
  vtkDMMLSubjectHierarchyNode* subjectHierarchyNode()const;
...
protected slots:
  ...
  virtual void onDMMLNodeRemoved(vtkDMMLNode* node);
...
protected:
  qDMMLSubjectHierarchyModel(qDMMLSubjectHierarchyModelPrivate* pimpl, QObject *parent=nullptr);

  /// Set the subject hierarchy node found in the given scene. Called only internally.
  virtual void setSubjectHierarchyNode(vtkDMMLSubjectHierarchyNode* shNode);
...
protected:
  QScopedPointer<qDMMLSubjectHierarchyModelPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLSubjectHierarchyModel);
  Q_DISABLE_COPY(qDMMLSubjectHierarchyModel);
};
