...
#ifndef __qCjyxSubjectHierarchyPluginHandler_h
#define __qCjyxSubjectHierarchyPluginHandler_h
...
class qCjyxSubjectHierarchyDefaultPlugin;
...
/// \class qCjyxSubjectHierarchyPluginHandler
/// \brief Singleton class managing Subject Hierarchy plugins
class Q_CJYX_MODULE_SUBJECTHIERARCHY_WIDGETS_EXPORT qCjyxSubjectHierarchyPluginHandler : public QObject
{
  Q_OBJECT
...
public:
  ...
  Q_INVOKABLE static qCjyxSubjectHierarchyPluginHandler* instance();

  /// Get subject hierarchy node
  Q_INVOKABLE vtkDMMLSubjectHierarchyNode* subjectHierarchyNode()const;
  /// Set DMML scene
...
public:
  ...
  Q_INVOKABLE bool registerPlugin(qCjyxSubjectHierarchyAbstractPlugin* plugin);

  /// Get list of registered plugins
  Q_INVOKABLE QList<qCjyxSubjectHierarchyAbstractPlugin*> registeredPlugins() { return m_RegisteredPlugins; };

  /// Get default plugin instance
  Q_INVOKABLE qCjyxSubjectHierarchyDefaultPlugin* defaultPlugin();

  /// Returns all plugins (registered plugins and default plugin)
  Q_INVOKABLE QList<qCjyxSubjectHierarchyAbstractPlugin*> allPlugins();

  /// Get a plugin by name
  /// \return The plugin instance if exists, nullptr otherwise
  Q_INVOKABLE qCjyxSubjectHierarchyAbstractPlugin* pluginByName(QString name);
  ...
  QList<qCjyxSubjectHierarchyAbstractPlugin*> pluginsForAddingNodeToSubjectHierarchy(
    vtkDMMLNode* node,
    vtkIdType parentItemID=vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID );
  ...
  /// \return The most suitable plugins if found, empty list otherwise
  QList<qCjyxSubjectHierarchyAbstractPlugin*> pluginsForReparentingItemInSubjectHierarchy(
  ...
  qCjyxSubjectHierarchyAbstractPlugin* findOwnerPluginForSubjectHierarchyItem(vtkIdType itemID);

  /// Find and set plugin that is most suitable to own a subject hierarchy item
  /// The best plugins are found based on the confidence numbers they return for the inputs.
  /// \param item Item to be owned
  qCjyxSubjectHierarchyAbstractPlugin* findAndSetOwnerPluginForSubjectHierarchyItem(vtkIdType itemID);
  ...
  Q_INVOKABLE qCjyxSubjectHierarchyAbstractPlugin* getOwnerPluginForSubjectHierarchyItem(vtkIdType itemID);
  ...
  qCjyxSubjectHierarchyAbstractPlugin* selectPluginFromDialog(QString textToDisplay, QList<qCjyxSubjectHierarchyAbstractPlugin*> candidatePlugins);

  /// Add an observer on the given SH node to watch for changes and react
  /// as needed
  /// \param shNode the subject hierarchy node to observe
  void observeSubjectHierarchyNode(vtkDMMLSubjectHierarchyNode* shNode);

  /// Show a list of items in a selected view (used for drag&drop of items into a view node)
  Q_INVOKABLE void showItemsInView(vtkIdList* itemIDsToShow, vtkDMMLAbstractViewNode* viewNode);
...
protected:
  ...
  QList<qCjyxSubjectHierarchyAbstractPlugin*> m_RegisteredPlugins;
  ...
  qCjyxSubjectHierarchyDefaultPlugin* m_DefaultPlugin;
  ...
  vtkWeakPointer<vtkDMMLScene> m_DMMLScene;

  /// Plugin logic
  qCjyxSubjectHierarchyPluginLogic* m_PluginLogic{nullptr};
...
private:
  qCjyxSubjectHierarchyPluginHandler(QObject* parent=nullptr);
  ~qCjyxSubjectHierarchyPluginHandler() override;

  Q_DISABLE_COPY(qCjyxSubjectHierarchyPluginHandler);
  friend class qCjyxSubjectHierarchyPluginHandlerCleanup;
  friend class PythonQtWrapper_qCjyxSubjectHierarchyPluginHandler; // Allow Python wrapping without enabling direct instantiation

private:
  /// Instance of the singleton
  static qCjyxSubjectHierarchyPluginHandler* m_Instance;
};
