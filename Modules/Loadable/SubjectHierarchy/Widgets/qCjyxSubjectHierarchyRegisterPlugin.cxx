...
class qCjyxSubjectHierarchyRegisterPluginPrivate: public QObject
{
  Q_DECLARE_PUBLIC(qCjyxSubjectHierarchyRegisterPlugin);
protected:
  qCjyxSubjectHierarchyRegisterPlugin* const q_ptr;
public:
  qCjyxSubjectHierarchyRegisterPluginPrivate(qCjyxSubjectHierarchyRegisterPlugin& object);
  ~qCjyxSubjectHierarchyRegisterPluginPrivate() override;
  void init();
public:
...
//-----------------------------------------------------------------------------
// qCjyxSubjectHierarchyRegisterPluginPrivate methods

//-----------------------------------------------------------------------------
qCjyxSubjectHierarchyRegisterPluginPrivate::qCjyxSubjectHierarchyRegisterPluginPrivate(qCjyxSubjectHierarchyRegisterPlugin& object)
: q_ptr(&object)
...
//------------------------------------------------------------------------------
void qCjyxSubjectHierarchyRegisterPluginPrivate::init()
{
  Q_Q(qCjyxSubjectHierarchyRegisterPlugin);
...
//-----------------------------------------------------------------------------
qCjyxSubjectHierarchyRegisterPluginPrivate::~qCjyxSubjectHierarchyRegisterPluginPrivate() = default;

//-----------------------------------------------------------------------------
// qCjyxSubjectHierarchyRegisterPlugin methods

//-----------------------------------------------------------------------------
qCjyxSubjectHierarchyRegisterPlugin::qCjyxSubjectHierarchyRegisterPlugin(QObject* parent)
 : Superclass(parent)
 , m_RegisterFromItem(vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
 , d_ptr( new qCjyxSubjectHierarchyRegisterPluginPrivate(*this) )
{
  this->m_Name = QString("Register");

  Q_D(qCjyxSubjectHierarchyRegisterPlugin);
  d->init();
}

//-----------------------------------------------------------------------------
qCjyxSubjectHierarchyRegisterPlugin::~qCjyxSubjectHierarchyRegisterPlugin() = default;

//---------------------------------------------------------------------------
QList<QAction*> qCjyxSubjectHierarchyRegisterPlugin::itemContextMenuActions()const
{
  Q_D(const qCjyxSubjectHierarchyRegisterPlugin);
...
//---------------------------------------------------------------------------
void qCjyxSubjectHierarchyRegisterPlugin::showContextMenuActionsForItem(vtkIdType itemID)
{
  Q_D(qCjyxSubjectHierarchyRegisterPlugin);
...
//---------------------------------------------------------------------------
void qCjyxSubjectHierarchyRegisterPlugin::registerCurrentItemTo()
{
  Q_D(qCjyxSubjectHierarchyRegisterPlugin);
...
//---------------------------------------------------------------------------
void qCjyxSubjectHierarchyRegisterPlugin::registerImageBasedRigid()
{
#ifdef Cjyx_BUILD_CLI_SUPPORT
  vtkDMMLSubjectHierarchyNode* shNode = qCjyxSubjectHierarchyPluginHandler::instance()->subjectHierarchyNode();
  if (!shNode)
  ...
  if (!scene)
    ...
    this->m_RegisterFromItem = vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID;
    return;
    }

  // Get volume nodes from selected subject hierarchy items
  vtkDMMLNode* registerFromVolumeNode = shNode->GetItemDataNode(this->m_RegisterFromItem);
  vtkDMMLNode* registerToVolumeNode = shNode->GetItemDataNode(currentItemID);
  ...
  qCjyxAbstractModuleWidget* moduleWidget = qCjyxSubjectHierarchyAbstractPlugin::switchToModule("BRAINSFit");
  if (moduleWidget)
    {
    // Create parameter set node
    vtkSmartPointer<vtkDMMLCommandLineModuleNode> registrationParameterSetNode =
      vtkSmartPointer<vtkDMMLCommandLineModuleNode>::New();
    ...
    vtkSmartPointer<vtkDMMLLinearTransformNode> outputTransform =
      vtkSmartPointer<vtkDMMLLinearTransformNode>::New();
    ...
    qDMMLNodeComboBox* parameterSetNodeSelector = moduleWidget->findChild<qDMMLNodeComboBox*>("DMMLCommandLineModuleNodeSelector");
...
//---------------------------------------------------------------------------
void qCjyxSubjectHierarchyRegisterPlugin::registerImageBasedBSpline()
{
#ifdef Cjyx_BUILD_CLI_SUPPORT
  vtkDMMLSubjectHierarchyNode* shNode = qCjyxSubjectHierarchyPluginHandler::instance()->subjectHierarchyNode();
  if (!shNode)
  ...
  if (moduleWidget)
    ...
    // Set output transform
    vtkSmartPointer<vtkDMMLBSplineTransformNode> outputTransform =
      vtkSmartPointer<vtkDMMLBSplineTransformNode>::New();
...
//---------------------------------------------------------------------------
void qCjyxSubjectHierarchyRegisterPlugin::registerInteractiveLandmark()
{
  vtkDMMLSubjectHierarchyNode* shNode = qCjyxSubjectHierarchyPluginHandler::instance()->subjectHierarchyNode();
  if (!shNode)
    {
  ...
  qCjyxAbstractModuleWidget* moduleWidget = qCjyxSubjectHierarchyAbstractPlugin::switchToModule("LandmarkRegistration");
  QApplication::processEvents(); // Make sure the volume selector dialog shows up
  if (moduleWidget)
    {
    // Get volume selector dialog
    QDialog* volumeSelectorDialog = qCjyxApplication::application()->mainWindow()->findChild<QDialog*>("LandmarkRegistrationVolumeSelect");
    if (volumeSelectorDialog)
      {
      // Get volume selector comboboxes
      QList<qDMMLNodeComboBox*> comboboxes = volumeSelectorDialog->findChildren<qDMMLNodeComboBox*>();

      // Set fixed and moving image. Unfortunately currently only the tooltip refers to the role
      foreach (qDMMLNodeComboBox* combobox, comboboxes)
        {
...
//---------------------------------------------------------------------------
void qCjyxSubjectHierarchyRegisterPlugin::cancel()
{
  // Reset saved 'from' item
  this->m_RegisterFromItem = vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID;
}
