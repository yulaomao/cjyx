...
#ifndef __qCjyxSubjectHierarchyPluginLogic_h
#define __qCjyxSubjectHierarchyPluginLogic_h
...
class Q_CJYX_MODULE_SUBJECTHIERARCHY_WIDGETS_EXPORT qCjyxSubjectHierarchyPluginLogic :
  public QObject, public virtual qCjyxObject
{
  Q_OBJECT
  QVTK_OBJECT
...
public:
  ...
  qCjyxSubjectHierarchyPluginLogic(QObject *parent=nullptr);
  ~qCjyxSubjectHierarchyPluginLogic() override;

public:
  /// Set the current DMML scene to the logic
  void setDMMLScene(vtkDMMLScene* scene) override;

  /// Assessor function for getting subject hierarchy plugin by name (for python)
  Q_INVOKABLE qCjyxSubjectHierarchyAbstractPlugin* subjectHierarchyPluginByName(QString name)const;
...
protected:
  ...
  void observeNode(vtkDMMLNode* node);
...
protected:
  QScopedPointer<qCjyxSubjectHierarchyPluginLogicPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qCjyxSubjectHierarchyPluginLogic);
  Q_DISABLE_COPY(qCjyxSubjectHierarchyPluginLogic);
  friend class qCjyxSubjectHierarchyPluginHandler;
};
