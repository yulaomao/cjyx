...
#ifndef __qDMMLSubjectHierarchyComboBox_h
#define __qDMMLSubjectHierarchyComboBox_h
...
class Q_CJYX_MODULE_SUBJECTHIERARCHY_WIDGETS_EXPORT qDMMLSubjectHierarchyComboBox : public ctkComboBox
{
  Q_OBJECT
  QVTK_OBJECT
  ...
  /// Else, the popup tree appears below the combobox, like for a qDMMLNodeComboBox.
...
public:
  ...
  qDMMLSubjectHierarchyComboBox(QWidget *parent=nullptr);
  ~qDMMLSubjectHierarchyComboBox() override;
...
protected slots:
  ...
  virtual void onDMMLSceneCloseEnded(vtkObject* sceneObject);
...
protected:
  QScopedPointer<qDMMLSubjectHierarchyComboBoxPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLSubjectHierarchyComboBox);
  Q_DISABLE_COPY(qDMMLSubjectHierarchyComboBox);
};
