...
This file was originally developed by Csaba Pinter, PerkLab, Queen's University and was supported through the Applied Cancer
 Research Unit program of Cancer Care Ontario with funds provided by the Ontario Ministry of Health and Long-Term Care"""
        ...
        try:
            ...
            cjyx.selfTests = {}
        cjyx.selfTests['SubjectHierarchyGenericSelfTest'] = self.runTest
...
class SubjectHierarchyGenericSelfTestTest(ScriptedLoadableModuleTest):
    ...
    def setUp(self):
        ...
        # logFile.write(repr(cjyx.modules.subjecthierarchygenericselftest) + '\n')
    ...
    def test_SubjectHierarchyGenericSelfTest_FullTest1(self):
        ...
        self.assertIsNotNone(cjyx.modules.subjecthierarchy)
    ...
    def section_SetupPathsAndNames(self):
        # Set constants
        subjectHierarchyGenericSelfTestDir = cjyx.app.temporaryPath + '/SubjectHierarchyGenericSelfTest'
        ...
        self.genericTestSceneFileName = self.tempDir + '/SubjectHierarchyGenericSelfTestScene.dmml'
    ...
    def section_ClearScene(self):
        ...
        self.assertEqual(cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLSubjectHierarchyNode'), 1)
    ...
    def section_LoadDicomDataWitchBatchProcessing(self):
        try:
            ...
            with DICOMUtils.TemporaryDICOMDatabase(self.dicomDatabaseDir) as db:
                ...
                self.assertEqual(cjyx.dicomDatabase, db)

                cjyx.dmmlScene.StartState(cjyx.vtkDMMLScene.BatchProcessState)

                # Download, unzip, import, and load data. Verify loaded nodes.
                loadedNodes = {'vtkDMMLScalarVolumeNode': 1}
                ...
                cjyx.dmmlScene.EndState(cjyx.vtkDMMLScene.BatchProcessState)

            self.assertEqual(len(cjyx.util.getNodes('vtkDMMLSubjectHierarchyNode*')), 1)
    ...
    def section_SaveScene(self):
        ...
        # Save DMML scene into file
        cjyx.dmmlScene.Commit(self.genericTestSceneFileName)
    ...
    def section_AddNodeToSubjectHierarchy(self):
        ...
        volumeNodes = list(cjyx.util.getNodes('vtkDMMLScalarVolumeNode*').values())
        ...
        dataWidget = cjyx.modules.data.widgetRepresentation()
        self.assertIsNotNone(dataWidget)
        shTreeView = cjyx.util.findChild(dataWidget, name='SubjectHierarchyTreeView')
        ...
        # Verify DICOM levels
        self.assertEqual(shNode.GetItemLevel(self.patientItemID), cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMLevelPatient())
        self.assertEqual(shNode.GetItemLevel(self.studyItemID), cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMLevelStudy())
    ...
    def section_CLI(self):
        ...
        resampledVolumeNode = cjyx.vtkDMMLScalarVolumeNode()
        resampledVolumeNode.SetName(ctVolumeNode.GetName() + '_Resampled_10x10x10mm')
        cjyx.dmmlScene.AddNode(resampledVolumeNode)
        ...
        cjyx.cli.run(cjyx.modules.resamplescalarvolume, None, resampleParameters, wait_for_completion=True)
    ...
    def section_LoadScene(self):
        ...
        cjyx.util.loadScene(self.genericTestSceneFileName)

        # Check number of nodes in the scene
        self.assertEqual(cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLScalarVolumeNode'), 4)
        self.assertEqual(cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLModelNode'), 4)  # Including the three slice view models
        self.assertEqual(cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLSubjectHierarchyNode'), 1)
    ...
    def section_TestCircularParenthood(self):
        ...
        shNode.SetItemParent(mainfolder_ID, subfolder_ID)  # Makes cjyx crash instead of returning an error
    ...
    def section_AttributeFilters(self):
        ...
        # Scene
        #  +----- NewFolder
        #  |       +----------- MarkupsAngle (DataNode:vtkDMMLMarkupsAngleNode1)
        #  |       |             (ItemAttributes: ItemAttribute1:'1')
        #  |       |             (NodeAttributes: Markups.MovingInSliceView:'Red', Markups.MovingMarkupIndex:'1')
        #  |       +----------- MarkupsAngle_1 (DataNode:vtkDMMLMarkupsAngleNode2)
        #  |       |             (NodeAttributes: Markups.MovingInSliceView:'Red', Markups.MovingMarkupIndex:'1')
        #  |       +----------- MarkupsAngle_2 (DataNode:vtkDMMLMarkupsAngleNode3)
        ...
        #  |       +----------- MarkupsCurve_1 (DataNode:vtkDMMLMarkupsCurveNode2)
        #  |       |             (NodeAttributes: Markups.MovingMarkupIndex:'3', Sajt:'Green')
        #  |       +----------- MarkupsCurve (DataNode:vtkDMMLMarkupsCurveNode1)
        #  |                     (ItemAttributes: ItemAttribute2:'2')
        #  |                     (NodeAttributes: Markups.MovingMarkupIndex:'2', Sajt:'Green')
        #  +----- MarkupsCurve_2 (DataNode:vtkDMMLMarkupsCurveNode1)
        #          (NodeAttributes: Markups.MovingMarkupIndex:'3', Sajt:'Green')
        ...
        self.assertEqual(cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLMarkupsNode'), 7)

        # Create test SH tree view
        shTreeView = cjyx.qDMMLSubjectHierarchyTreeView()
        shTreeView.setDMMLScene(cjyx.dmmlScene)
        shTreeView.show()
        ...
        def testAttributeFilters(filteredObject, proxyModel):
            ...
            filteredObject.addNodeAttributeFilter('Markups.MovingMarkupIndex', 3, True, 'vtkDMMLMarkupsCurveNode')
            self.assertEqual(shProxyModel.acceptedItemCount(shNode.GetSceneItemID()), 3)
            filteredObject.addNodeAttributeFilter('ParentAttribute', '', True, 'vtkDMMLMarkupsAngleNode')
            self.assertEqual(shProxyModel.acceptedItemCount(shNode.GetSceneItemID()), 5)
            filteredObject.addNodeAttributeFilter('ChildAttribute', '', True, 'vtkDMMLMarkupsAngleNode')
            self.assertEqual(shProxyModel.acceptedItemCount(shNode.GetSceneItemID()), 6)
            filteredObject.includeNodeAttributeNamesFilter = []
            self.assertEqual(shProxyModel.acceptedItemCount(shNode.GetSceneItemID()), 9)
            # Check with empty attribute value
            filteredObject.addNodeAttributeFilter('Markups.MovingMarkupIndex', '', True, 'vtkDMMLMarkupsCurveNode')
            self.assertEqual(shProxyModel.acceptedItemCount(shNode.GetSceneItemID()), 4)
            filteredObject.includeNodeAttributeNamesFilter = []
            self.assertEqual(shProxyModel.acceptedItemCount(shNode.GetSceneItemID()), 9)
    ...
    def section_ComboboxFeatures(self):
        ...
        comboBox = cjyx.qDMMLSubjectHierarchyComboBox()
        comboBox.setDMMLScene(cjyx.dmmlScene)
        comboBox.show()
    ...
    def createSampleLabelmapVolumeNode(self, volumeNode, name, label, colorNode=None):
        ...
        self.assertTrue(volumeNode.IsA('vtkDMMLScalarVolumeNode'))
        self.assertTrue(label > 0)

        sampleLabelmapNode = cjyx.vtkDMMLLabelMapVolumeNode()
        sampleLabelmapNode.SetName(name)
        sampleLabelmapNode = cjyx.dmmlScene.AddNode(sampleLabelmapNode)
        ...
        # Display labelmap
        labelmapVolumeDisplayNode = cjyx.vtkDMMLLabelMapVolumeDisplayNode()
        cjyx.dmmlScene.AddNode(labelmapVolumeDisplayNode)
        if colorNode is None:
            colorNode = cjyx.util.getNode('GenericAnatomyColors')
        ...
        sampleLabelmapName = cjyx.dmmlScene.GenerateUniqueName(name)
    ...
    def createSampleModelNode(self, name, color, volumeNode=None):
        ...
        modelNode = cjyx.vtkDMMLModelNode()
        modelNode.SetName(name)
        modelNode = cjyx.dmmlScene.AddNode(modelNode)
        modelNode.SetPolyDataConnection(sphere.GetOutputPort())
        modelNode.SetHideFromEditors(0)

        displayNode = cjyx.vtkDMMLModelDisplayNode()
