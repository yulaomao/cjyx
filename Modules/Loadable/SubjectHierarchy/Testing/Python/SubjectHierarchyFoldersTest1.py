...
import cjyx
from cjyx.util import DATA_STORE_URL
...
class SubjectHierarchyFoldersTest1(unittest.TestCase):
    ...
    def TestSection_InitializeTest(self):
        ...
        self.shNode = cjyx.vtkDMMLSubjectHierarchyNode.GetSubjectHierarchyNode(cjyx.dmmlScene)
        ...
        self.viewOwnerNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLScriptedModuleNode")
        self.viewNode = cjyx.vtkDMMLViewNode()
        ...
        self.viewNode = cjyx.dmmlScene.AddNode(self.viewNode)
        self.viewWidget = cjyx.qDMMLThreeDWidget()
        self.viewWidget.setDMMLScene(cjyx.dmmlScene)
        self.viewWidget.setDMMLViewNode(self.viewNode)
    ...
    def TestSection_LoadTestData(self):
        ...
        sceneFile = SampleData.downloadFromURL(
            ...
            # Note: this data set is from CjyxDataStore (not from CjyxTestingData) repository
        ...
        self.assertEqual(298, cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLModelNode'))  # 301 with main window due to the slice views

        # Check number of model hierarchy nodes to make sure all of them were converted
        self.assertEqual(0, cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLModelHierarchyNode'))
        # Check number of folder display nodes, which is zero until branch display related functions are used
        self.assertEqual(0, cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLFolderDisplayNode'))
        ...
        for index in range(allItems.GetNumberOfIds()):
            ...
            if self.shNode.IsItemLevel(currentItem, cjyx.vtkDMMLSubjectHierarchyConstants.GetSubjectHierarchyLevelFolder()):
    ...
    def TestSection_FolderVisibility(self):
        ...
        modelNodes = cjyx.util.getNodes('vtkDMMLModelNode*')
        ...
        self.testModelNode = cjyx.util.getNode(self.testModelNodeName)
        self.assertIsNotNone(self.testModelNode)

        testModelHierarchyVisibility = cjyx.vtkDMMLFolderDisplayNode.GetHierarchyVisibility(self.testModelNode)
        ...
        self.assertEqual(1, cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLFolderDisplayNode'))
    ...
    def TestSection_ApplyDisplayPropertiesOnBranch(self):
        ...
        testModelHierarchyOpacity = cjyx.vtkDMMLFolderDisplayNode.GetHierarchyOpacity(self.testModelNode)
    ...
    def getModelDisplayableManager(self):
        ...
        for i in range(managers.GetNumberOfItems()):
            ...
            if obj.IsA('vtkDMMLModelDisplayableManager'):
                return obj
