...
void CheckTextColor(vtkDMMLAnnotationRulerNode *rnode)
{
...
int vtkCjyxAnnotationModuleLogicTest1(int , char * [] )
{
  ...
  vtkSmartPointer<vtkDMMLAnnotationRulerNode> rnode1 = vtkSmartPointer<vtkDMMLAnnotationRulerNode>::New();
  dmmlScene->AddNode(rnode1 );
  ...
  // TODO: test dmml event processing?
  ...
  vtkDMMLNode *node = dmmlScene->GetNodeByID("vtkDMMLAnnotationFiducialNode");
  if (node)
    {
  ...
  vtkSmartPointer<vtkDMMLAnnotationRulerNode> rnode = vtkSmartPointer<vtkDMMLAnnotationRulerNode>::New();
  dmmlScene->AddNode(rnode);
...
//    return EXIT_FAILURE;
  ...
  vtkSmartPointer<vtkDMMLAnnotationFiducialNode> fnode = vtkSmartPointer<vtkDMMLAnnotationFiducialNode>::New();
  double pos[3]={1,2,5};
  fnode->SetControlPoint(0, pos, 0, 1);
  dmmlScene->AddNode(fnode);
  ...
  vtkDMMLAnnotationHierarchyNode *activeHierarchy = logic->GetActiveHierarchyNode();
  if (activeHierarchy)
    {
    std::cout << "Active hierarchy = " << (activeHierarchy->GetName() ? activeHierarchy->GetName() : "null") << std::endl;
    }

  vtkDMMLAnnotationNode *nullNode = nullptr;
  TESTING_OUTPUT_ASSERT_ERRORS_BEGIN();
