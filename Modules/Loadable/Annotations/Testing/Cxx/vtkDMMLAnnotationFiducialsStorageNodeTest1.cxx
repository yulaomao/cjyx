...
int vtkDMMLAnnotationFiducialsStorageNodeTest1(int argc, char * argv[] )
{
  ...
  vtkNew<vtkDMMLAnnotationFiducialsStorageNode> node2;
  EXERCISE_ALL_BASIC_DMML_METHODS(node2.GetPointer());

  vtkDMMLAnnotationFiducialsStorageNode* node1 = dynamic_cast <  vtkDMMLAnnotationFiducialsStorageNode *> (node2->CreateNodeInstance());
  if( node1 == nullptr )
    {
  ...
  vtkSmartPointer< vtkDMMLAnnotationFiducialNode > annNode = vtkSmartPointer< vtkDMMLAnnotationFiducialNode >::New();
  annNode->StartModify();
  ...
  if (!annNode->GetAnnotationPointDisplayNode())
    {
       std::cerr << "Error in vtkDMMLAnnotationFiducialNode::AnnotationPointDisplayNode() " << std::endl;
       return EXIT_FAILURE;
    }
