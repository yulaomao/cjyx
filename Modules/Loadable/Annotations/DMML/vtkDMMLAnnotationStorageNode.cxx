...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLAnnotationStorageNode);

//----------------------------------------------------------------------------
vtkDMMLAnnotationStorageNode::vtkDMMLAnnotationStorageNode()
{
  //this->Debug =1;
...
//----------------------------------------------------------------------------
vtkDMMLAnnotationStorageNode::~vtkDMMLAnnotationStorageNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLAnnotationStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
  vtkDMMLStorageNode::PrintSelf(os,indent);
}

//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::ReadAnnotationDisplayProperties(vtkDMMLAnnotationDisplayNode *annotationDisplayNode, std::string lineString, std::string preposition)
{
...
//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::ReadAnnotationTextDisplayProperties(vtkDMMLAnnotationTextDisplayNode *annotationDisplayNode, std::string lineString, std::string preposition)
{
  if (annotationDisplayNode == nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::OpenFileToRead(fstream& fstr, vtkDMMLNode *refNode)
{
  vtkDebugMacro("Reading Annotation data");
  // test whether refNode is a valid node to hold a text list
  if ( !refNode ||
       !( refNode->IsA("vtkDMMLAnnotationNode"))
     )
    {
    vtkErrorMacro("Reference node is not a proper vtkDMMLAnnotationNode");
    return 0;
    }
  ...
  if (fullName.empty())
    {
    vtkErrorMacro("vtkDMMLAnnotationStorageNode: File name not specified");
    return 0;
    }
...
//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::ReadAnnotationTextData(vtkDMMLAnnotationNode *refNode, char line[1024], int typeColumn, int annotationColumn,  int selColumn,  int visColumn, int numColumns)
{
  if (!refNode)
    {
...
int vtkDMMLAnnotationStorageNode::ReadAnnotationTextProperties(vtkDMMLAnnotationNode *refNode, char line[1024], int &typeColumn,
...
int vtkDMMLAnnotationStorageNode::ReadAnnotation(vtkDMMLAnnotationNode *annotationNode)
{
...
//----------------------------------------------------------------------------
bool vtkDMMLAnnotationStorageNode::CanReadInReferenceNode(vtkDMMLNode* refNode)
{
  return refNode->IsA("vtkDMMLAnnotationNode");
}

//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::ReadDataInternal(vtkDMMLNode *refNode)
{
  /*
  // special case: if this annotation is in a hierarchy, the hierarchy took
  // care of reading it already
  vtkDMMLHierarchyNode *hnode = vtkDMMLHierarchyNode::GetAssociatedHierarchyNode(refNode->GetScene(), refNode->GetID());
  if (hnode &&
  ...
  */
  ...
  vtkDMMLAnnotationNode *annotationNode =
    vtkDMMLAnnotationNode::SafeDownCast (refNode);
  ...
  this->InvokeEvent(vtkDMMLScene::NodeAddedEvent, annotationNode);

  return 1;
}

//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::WriteAnnotationDisplayProperties(fstream& of, vtkDMMLAnnotationDisplayNode *refNode, std::string preposition)
{
 if (!refNode)
   {
   return 0;
...
//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::WriteAnnotationTextDisplayProperties(fstream& of, vtkDMMLAnnotationTextDisplayNode *refNode, std::string preposition)
{
 if (!refNode)
   {
...
//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::WriteAnnotationTextProperties(fstream& of, vtkDMMLAnnotationNode *refNode)
{
  // put down a header
  if (refNode == nullptr)
    {
  ...
  vtkDMMLAnnotationTextDisplayNode *annDisNode = refNode->GetAnnotationTextDisplayNode();
...
//--------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::WriteAnnotationData(fstream& of, vtkDMMLAnnotationNode *refNode)
{
  if (!refNode)
    {
  ...
  for (int i = 0; i < refNode->GetNumberOfTexts(); i++)
    ...
    int sel = refNode->GetAnnotationAttribute(i, vtkDMMLAnnotationNode::TEXT_SELECTED);
    int vis = refNode->GetAnnotationAttribute(i, vtkDMMLAnnotationNode::TEXT_VISIBLE);
...
//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::OpenFileToWrite(fstream& of)
{
...
//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::WriteAnnotationDataInternal(vtkDMMLNode *refNode, fstream &of)
{
  vtkDebugMacro("vtkDMMLAnnotationStorageNode::WriteData");
...
//----------------------------------------------------------------------------
int vtkDMMLAnnotationStorageNode::WriteDataInternal(vtkDMMLNode *refNode)
{
  if (!refNode)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLAnnotationStorageNode::InitializeSupportedReadFileTypes()
{
...
//----------------------------------------------------------------------------
void vtkDMMLAnnotationStorageNode::InitializeSupportedWriteFileTypes()
{
