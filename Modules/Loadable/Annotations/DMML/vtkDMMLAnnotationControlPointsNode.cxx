...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLAnnotationControlPointsNode);

vtkDMMLAnnotationControlPointsNode::vtkDMMLAnnotationControlPointsNode()
{
  this->NumberingScheme = UseID;
}

//----------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::WriteXML(ostream& of, int nIndent)
{
  /*
  // special case: if this annotation is in a hierarchy, the hierarchy took
  // care of writing it already
  vtkDMMLHierarchyNode *hnode = vtkDMMLHierarchyNode::GetAssociatedHierarchyNode(this->GetScene(), this->GetID());
  ...
  */
  // cout << "vtkDMMLAnnotationControlPointsNode::WriteXML start" << endl;
...
//----------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::
         ...
         int coordinateSystem, int multipleFlag)
{
  ...
  if (this->GetPoints())
    ...
    for (int i = 0; i < n; i++ )
      ...
      else if (coordinateSystem == vtkDMMLStorageNode::CoordinateSystemLPS)
        {
        // LPS
        double lps[3];
        lps[0] = -1.0 * ptr[0];
...
//----------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::ReadXMLAttributes(const char** atts)
{
  // cout << "vtkDMMLAnnotationControlPointsNode::ReadXMLAttributes start"<< endl;
...
//-----------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::UpdateScene(vtkDMMLScene *scene)
{
  Superclass::UpdateScene(scene);
}

//---------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::ProcessDMMLEvents ( vtkObject *caller,
                                           unsigned long event,
                                           void *callData )
{
...
//----------------------------------------------------------------------------
//void vtkDMMLAnnotationControlPointsNode::PrintSelf(ostream& os, vtkIndent indent)
//{
//  vtkDMMLModelNode::PrintSelf(os,indent);
//  os << endl;
//  this->PrintAnnotationInfo(os,indent,0);
//}
//----------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::PrintAnnotationInfo(ostream& os, vtkIndent indent, int titleFlag)
{
  if (titleFlag)
    {

      os <<indent << "vtkDMMLAnnotationControlPointsNode: Annotation Summary";
      if (this->GetName())
    {
      os << " of " << this->GetName();
    }
...
//----------------------------------------------------------------------------
vtkDMMLAnnotationPointDisplayNode* vtkDMMLAnnotationControlPointsNode::GetAnnotationPointDisplayNode()
{
  int nnodes = this->GetNumberOfDisplayNodes();
  vtkDMMLAnnotationPointDisplayNode *node = nullptr;
  for (int n=0; n<nnodes; n++)
    {
      // vtkDMMLDisplayableNode stores the ids in DisplayNodeIDs
    node = vtkDMMLAnnotationPointDisplayNode::SafeDownCast(this->GetNthDisplayNode(n));
    if (node && node->IsA("vtkDMMLAnnotationPointDisplayNode"))
      {
    return node;
      }
    }
...
//---------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::CreateAnnotationPointDisplayNode()
{
  vtkDMMLAnnotationPointDisplayNode *node = this->GetAnnotationPointDisplayNode();
  if (node) return;
  if (!this->GetScene())
    {
      vtkErrorMacro("vtkDMMLAnnotationControlPointsNode::CreateAnnotationControlPointDisplayNode Annotation: No scene defined" ) ;
      return;
    }

  node = vtkDMMLAnnotationPointDisplayNode::New();
...
//---------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::ResetAnnotations()
{
...
//---------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::CreatePolyData()
{
...
//---------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::ResetControlPoints()
{
  this->CreatePolyData();
...
//---------------------------------------------------------------------------
int vtkDMMLAnnotationControlPointsNode::GetNumberOfControlPoints()
{
  if (!this->GetPoints())
    {
    this->ResetControlPoints();
...
//---------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::DeleteControlPoint(int id)
{
  if (!this->GetPoints())
    {
    this->ResetControlPoints();
...
double* vtkDMMLAnnotationControlPointsNode::GetControlPointCoordinates(vtkIdType id)
{

  if (!this->GetPoints() ||
      this->GetPoints()->GetNumberOfPoints() <= id)
    {
    vtkErrorMacro("vtkDMMLAnnotationControlPointsNode::GetControlPointWorldCoordinates() no control point with index" << id) ;
    return nullptr;
    }
...
/// Try to limit calling this function because it is performance critical.
/// Also, when queried again it resets all former pointers. Copying is therefore necessary.
void vtkDMMLAnnotationControlPointsNode::GetControlPointWorldCoordinates(vtkIdType id, double *point)
{
...
//---------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::ResetControlPointsAttributesAll() {
  for (int j = NUM_TEXT_ATTRIBUTE_TYPES ; j < NUM_CP_ATTRIBUTE_TYPES; j ++) {
...
//-------------------------------------------------------------------------
const char *vtkDMMLAnnotationControlPointsNode::GetAttributeTypesEnumAsString(int val)
{
  if (val < vtkDMMLAnnotationNode::NUM_TEXT_ATTRIBUTE_TYPES) {
    return vtkDMMLAnnotationNode::GetAttributeTypesEnumAsString(val);
  }
...
//-------------------------------------------------------------------------
vtkDMMLStorageNode* vtkDMMLAnnotationControlPointsNode::CreateDefaultStorageNode()
{
  ...
  return vtkDMMLStorageNode::SafeDownCast(
    scene->CreateNodeByClass("vtkDMMLAnnotationControlPointsStorageNode"));
}

//---------------------------------------------------------------------------
int vtkDMMLAnnotationControlPointsNode::SetControlPoint(int id, double newControl[3],int selectedFlag, int visibleFlag)
{
  // cout << "vtkDMMLAnnotationControlPointsNode::SetControlPoint: ID:  " << id << " CtrlPt: " <<  newControl[0] << " " <<  newControl[1] << " " <<  newControl[2] << endl;
  if (id < 0)
  {
...
//---------------------------------------------------------------------------
int vtkDMMLAnnotationControlPointsNode::SetControlPointWorldCoordinates(int id, double p[3])
{
...
//---------------------------------------------------------------------------
int vtkDMMLAnnotationControlPointsNode::SetControlPointWorldCoordinates(int id, double p[3], int selectedFlag, int visibleFlag)
{
  double worldPoint[] = {p[0],p[1],p[2],1};
  double localPoint[4];
  this->TransformPointFromWorld(worldPoint, localPoint);
...
//---------------------------------------------------------------------------
int vtkDMMLAnnotationControlPointsNode::SetControlPoint(int id, double newControl[3])
{
  // cout << "vtkDMMLAnnotationControlPointsNode::SetControlPoint: ID:  " << id << " CtrlPt: " <<  newControl[0] << " " <<  newControl[1] << " " <<  newControl[2] << endl;
  if (id < 0)
  {
...
//---------------------------------------------------------------------------
int vtkDMMLAnnotationControlPointsNode::AddControlPoint(double newControl[3],int selectedFlag, int visibleFlag)
{
  // Create if not there
  if (!this->GetPoints())
    {
...
//----------------------------------------------------------------------------
const char* vtkDMMLAnnotationControlPointsNode::GetNumberingSchemeAsString()
{
  return this->GetNumberingSchemeAsString(this->NumberingScheme);
}

//----------------------------------------------------------------------------
const char* vtkDMMLAnnotationControlPointsNode::GetNumberingSchemeAsString(int scheme)
{
...
//----------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::SetNumberingSchemeFromString(const char *schemeString)
{
...
//----------------------------------------------------------------------------
void vtkDMMLAnnotationControlPointsNode::Initialize(vtkDMMLScene* dmmlScene)
{
  if (!dmmlScene)
  {
