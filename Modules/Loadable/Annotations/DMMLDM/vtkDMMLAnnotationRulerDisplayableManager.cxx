...
//---------------------------------------------------------------------------
vtkStandardNewMacro (vtkDMMLAnnotationRulerDisplayableManager);

//---------------------------------------------------------------------------
// vtkDMMLAnnotationRulerDisplayableManager Callback
...
//---------------------------------------------------------------------------
// vtkDMMLAnnotationRulerDisplayableManager methods

//---------------------------------------------------------------------------
void vtkDMMLAnnotationRulerDisplayableManager::PrintSelf(ostream& os, vtkIndent indent)
{
...
vtkAbstractWidget * vtkDMMLAnnotationRulerDisplayableManager::CreateWidget(vtkDMMLAnnotationNode* node)
{
  ...
  vtkDMMLAnnotationRulerNode* rulerNode = vtkDMMLAnnotationRulerNode::SafeDownCast(node);
  vtkDMMLAnnotationLineDisplayNode* lineDisplayNode =
...
//    handle2->GetProperty()->SetColor(1,1,0);
  ...
  this->PropagateDMMLToWidget(rulerNode, rulerWidget);
...
void vtkDMMLAnnotationRulerDisplayableManager::OnWidgetCreated(vtkAbstractWidget * widget, vtkDMMLAnnotationNode * node)
{
  ...
  // widget thinks the interaction ended, now we can place the points from DMML
...
void vtkDMMLAnnotationRulerDisplayableManager::PropagateDMMLToWidget(vtkDMMLAnnotationNode* node, vtkAbstractWidget * widget)
{
  ...
  if (!rulerWidget)
    {
    vtkErrorMacro("PropagateDMMLToWidget: Could not get ruler widget!");
    return;
    }
  ...
  if (!rulerNode)
    {
    vtkErrorMacro("PropagateDMMLToWidget: Could not get ruler node!");
    return;
    }
  ...
  vtkDMMLAnnotationTextDisplayNode *textDisplayNode = rulerNode->GetAnnotationTextDisplayNode();
  vtkDMMLAnnotationPointDisplayNode *pointDisplayNode = rulerNode->GetAnnotationPointDisplayNode();
  ...
  if (this->Is2DDisplayableManager())
    ...
    // now get the widget properties (coordinates, measurement etc.) and if the dmml node
...
std::string vtkDMMLAnnotationRulerDisplayableManager
::GetLabelFormat(vtkDMMLAnnotationRulerNode* rulerNode)
{
  if (!rulerNode)
    {
    return "";
  ...
  if (this->GetDMMLScene())
    ...
    if (selectionNode)
      {
      vtkDMMLUnitNode* lengthUnit = vtkDMMLUnitNode::SafeDownCast(
...
//---------------------------------------------------------------------------
double vtkDMMLAnnotationRulerDisplayableManager::GetDistance(
...
//---------------------------------------------------------------------------
double vtkDMMLAnnotationRulerDisplayableManager
...
void vtkDMMLAnnotationRulerDisplayableManager::PropagateWidgetToDMML(vtkAbstractWidget * widget, vtkDMMLAnnotationNode* node)
{
  ...
  if (!rulerWidget)
    {
    vtkErrorMacro("PropagateWidgetToDMML: Could not get ruler widget!");
    return;
    }
  ...
  if (!rulerNode)
    {
    vtkErrorMacro("PropagateWidgetToDMML: Could not get ruler node!");
    return;
    }
  ...
  // save worldCoordinates to DMML if changed
  double p1[4]={0,0,0,1};
  double p2[4]={0,0,0,1};
...
void vtkDMMLAnnotationRulerDisplayableManager::OnClickInRenderWindow(double x, double y, const char *associatedNodeID)
{
  ...
  if (!this->m_ClickCounter->HasEnoughClicks(2))
  ...
  else
   {
    ...
    rulerNode->SetName(this->GetDMMLScene()->GetUniqueNameByString("M"));
    ...
    rulerNode->Initialize(this->GetDMMLScene());
...
//---------------------------------------------------------------------------
void vtkDMMLAnnotationRulerDisplayableManager::UpdatePosition(vtkAbstractWidget *widget, vtkDMMLNode *node)
{
    if (!widget)
    {
  ...
  if (!node)
    {
    vtkErrorMacro("UpdatePosition: DMML node was null!");
    return;
    }
...
//---------------------------------------------------------------------------
void vtkDMMLAnnotationRulerDisplayableManager
::OnDMMLSelectionNodeUnitModifiedEvent(vtkDMMLSelectionNode* selectionNode)
{
  if (!selectionNode)
    {
    return;
  ...
  for (it = this->Helper->AnnotationNodeList.begin();
    ...
    vtkDMMLAnnotationRulerNode* rulerNode =
      vtkDMMLAnnotationRulerNode::SafeDownCast((*it));
    if (rulerNode)
      {
