...
class NeurosurgicalPlanningTutorialMarkupsSelfTestTest(ScriptedLoadableModuleTest):
    ...
    def setUp(self):
        ...
        lm = cjyx.app.layoutManager()
        lm.setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutConventionalView)
    ...
    def test_NeurosurgicalPlanningTutorialMarkupsSelfTest1(self):
        ...
        if self.enableScreenshots:
            ...
            cjyx.util.selectModule('Welcome')
        ...
        #
        cjyx.util.selectModule('Volumes')
        ...
        #
        lm.setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutOneUpRedSliceView)
        cjyx.app.processEvents()
        sliceLogic.FitSliceToAll()
        ...
        # Create segmentation
        segmentationNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLSegmentationNode", baselineVolume.GetName() + '-segmentation')
        ...
        #
        cjyx.util.selectModule('SegmentEditor')
        ...
        for clickCoords in clickCoordsList:
            if currentCoords:
                cjyx.util.clickAndDrag(sliceWidget, start=logic.rasToDisplay(*currentCoords), end=logic.rasToDisplay(*clickCoords), steps=10)
        ...
        cjyx.util.clickAndDrag(sliceWidget, start=logic.rasToDisplay(-0.5, 118.5, sliceOffset), end=logic.rasToDisplay(-7.4, 116, sliceOffset), steps=10)
        ...
        for clickCoords in clickCoordsList:
            if currentCoords:
                cjyx.util.clickAndDrag(sliceWidget, start=logic.rasToDisplay(*currentCoords), end=logic.rasToDisplay(*clickCoords), steps=30)
            currentCoords = clickCoords
        ...
        segmentEditorNode.SetOverwriteMode(cjyx.vtkDMMLSegmentEditorNode.OverwriteNone)
        ...
        cjyx.util.clickAndDrag(sliceWidget, start=logic.rasToDisplay(25.3, 5.8, sliceOffset), end=logic.rasToDisplay(25.3, 5.8, sliceOffset), steps=1)
