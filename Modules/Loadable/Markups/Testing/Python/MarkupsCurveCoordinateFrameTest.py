...
def createCoordinateSystemsModel(curve, axisLength=5):
    ...
    # model = cjyx.modules.models.logic().AddModel(coordinateSystemAppender.GetOutputPort())
    model = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLModelNode')
...
def addCoordinateSystemUpdater(updateInfo):
    model, curve, coordinateSystemAppender, curvePointToWorldTransform, transform, transformer = updateInfo
    observation = curve.AddObserver(cjyx.vtkDMMLMarkupsNode.PointModifiedEvent,
...
curveMeasurementsTestDir = cjyx.app.temporaryPath + '/curveMeasurementsTest'
...
cjyx.util.loadScene(testSceneFilePath)
planarCurveNode = cjyx.util.getNode('C')
...
curvePointToWorldMatrix = cjyx.util.arrayFromVTKMatrix(curvePointToWorldTransform)
...
closedCurveNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLMarkupsClosedCurveNode')
pos = np.zeros(3)
...
# of a model using the ExtractCenterline module in the CjyxVMTK extension.
#
...
centerlineModel = cjyx.util.getNode('Centerline model')
centerlinePolyData = centerlineModel.GetPolyData()

# Create curve node from centerline model
# (markups curve with radius array output is not yet available in CjyxVMTK)
centerlineCurve = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLMarkupsCurveNode')
...
import math
circleCurveNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLMarkupsClosedCurveNode")
