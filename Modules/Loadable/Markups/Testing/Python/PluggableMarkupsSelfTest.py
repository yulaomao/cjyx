...
class PluggableMarkupsSelfTestLogic(ScriptedLoadableModuleLogic):
    ...
    def setUp(self):
        ...
        markupsWidget = cjyx.modules.markups.widgetRepresentation()
        if markupsWidget is None:
            raise Exception("Couldn't get the Markups module widget")

        markupsLogic = cjyx.modules.markups.logic()
        ...
        #
        additionalOptionsWidgetsFactory = cjyx.qDMMLMarkupsOptionsWidgetsFactory().instance()
    ...
    def __checkPushButtonExists(self, widget, name):
        ...
        # cjyx.util.delayDisplay("Checking whether '%s' exists" % pushButtonObjectName)
        if widget.findChild(qt.QPushButton, pushButtonObjectName):
            return True
        return False

    def __checkWidgetExists(self, widget, name):
        # cjyx.util.delayDisplay("Checking whether '%s' exists" % name)
        if widget.findChild(qt.QWidget, name):
            return True
        return False

    def __checkWidgetVisibility(self, widget, name):
        # cjyx.util.delayDisplay("Checking whether '%s' is visible" % pushButtonObjectName)
        w = widget.findChild(qt.QWidget, name)
        return w.isVisible()

    def markupsNodes(self):
        return {
            cjyx.vtkDMMLMarkupsAngleNode(): cjyx.vtkCjyxAngleWidget(),
            cjyx.vtkDMMLMarkupsClosedCurveNode(): cjyx.vtkCjyxCurveWidget(),
            cjyx.vtkDMMLMarkupsCurveNode(): cjyx.vtkCjyxCurveWidget(),
            cjyx.vtkDMMLMarkupsFiducialNode(): cjyx.vtkCjyxPointsWidget(),
            cjyx.vtkDMMLMarkupsLineNode(): cjyx.vtkCjyxLineWidget(),
            cjyx.vtkDMMLMarkupsPlaneNode(): cjyx.vtkCjyxPlaneWidget(),
            cjyx.vtkDMMLMarkupsROINode(): cjyx.vtkCjyxROIWidget(),
            cjyx.vtkDMMLMarkupsTestLineNode(): cjyx.vtkCjyxTestLineWidget()
        }

    def additionalOptionsWidgets(self):
        return [
            cjyx.qDMMLMarkupsCurveSettingsWidget(),
            cjyx.qDMMLMarkupsAngleMeasurementsWidget(),
            cjyx.qDMMLMarkupsPlaneWidget(),
            cjyx.qDMMLMarkupsROIWidget(),
            cjyx.qDMMLMarkupsTestLineWidget()
        ]
    ...
    def test_unregister_existing_markups(self):
        ...
        for markupNode in self.markupsNodes():
            cjyx.util.delayDisplay("Unregistering %s" % markupNode.GetMarkupType())
    ...
    def test_register_markups(self):
        ...
        # Check Markups module standard nodes are registered
        for markupNode in markupsNodes:
            cjyx.util.delayDisplay("Registering %s" % markupNode.GetMarkupType())
            markupsLogic.RegisterMarkupsNode(markupNode, markupsNodes[markupNode])
            if self.__checkPushButtonExists(markupsWidget, markupNode.GetMarkupType()) is None:
                raise Exception("Create PushButton for %s is not present" % markupNode.GetMarkupType())
    ...
    def test_register_additional_options_widgets(self):
        ...
        for additionalOptionsWidget in self.additionalOptionsWidgets():
            ...
            cjyx.util.delayDisplay("Registering %s" % additionalOptionsWidget.objectName)
            additionalOptionsWidgetsFactory.registerOptionsWidget(additionalOptionsWidget)
    ...
    def run(self):
        ...
        Run the tests
        """
        cjyx.util.delayDisplay('Running integration tests for Pluggable Markups')
...
class PluggableMarkupsSelfTestTest(ScriptedLoadableModuleTest):
    ...
    def test_PluggableMarkupsSelfTest1(self):
        ...
        cjyx.util.mainWindow().moduleSelector().selectModule('Markups')
