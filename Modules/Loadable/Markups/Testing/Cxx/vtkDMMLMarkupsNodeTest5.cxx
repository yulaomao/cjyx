...
bool CompareROI(double xAxisExpected_World[3], double yAxisExpected_World[3], double zAxisExpected_World[3],
  double originExpected_World[3], double sizeExpected_World[3], vtkDMMLMarkupsROINode* roiNode, double epsilon)
{
...
//----------------------------------------------------------------------------
int vtkDMMLMarkupsNodeTest5(int , char * [])
{
  std::cout << "Testing vtkDMMLMarkupsROINode" << std::endl;
  ...
  vtkDMMLMarkupsROINode::GenerateOrthogonalMatrix(xAxis_Node, yAxis_Node, zAxis_Node, origin_Node, matrix, linearTransform);
  for (int i = 0; i < 3; ++i)
    {
  ...
  vtkNew<vtkDMMLBSplineTransformNode> bSplineTransformNode;
  ...
  vtkDMMLMarkupsROINode::GenerateOrthogonalMatrix(xAxis, yAxis, zAxis, origin, matrix, bSplineTransform, false);
  for (int i = 0; i < 3; ++i)
    {
    xAxis_World[i] = matrix->GetElement(i, 0);
    yAxis_World[i] = matrix->GetElement(i, 1);
