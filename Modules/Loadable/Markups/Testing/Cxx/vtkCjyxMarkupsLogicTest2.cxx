...
static void PrintLabels(vtkDMMLMarkupsNode *m)
{
  if (!m)
    {
    return;
...
int vtkCjyxMarkupsLogicTest2(int argc, char *argv [] )
{
  ...
  vtkSmartPointer<vtkDMMLMarkupsFiducialNode> source = vtkSmartPointer<vtkDMMLMarkupsFiducialNode>::New();
  vtkSmartPointer<vtkDMMLMarkupsFiducialNode> dest = vtkSmartPointer<vtkDMMLMarkupsFiducialNode>::New();
  ...
  std::string filename = tempFolder + "/vtkDMMLMarkupsLogicTest2-export-temp.csv";
  CHECK_BOOL(logic1->ExportControlPointsToCSV(dest, filename), true);

  vtkNew<vtkDMMLMarkupsCurveNode> importedNode;
