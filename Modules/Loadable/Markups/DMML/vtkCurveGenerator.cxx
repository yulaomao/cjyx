...
vtkCurveGenerator::vtkCurveGenerator()
{
  ...
  this->SurfacePathFilter = vtkSmartPointer<vtkCjyxDijkstraGraphGeodesicPath>::New();
...
void vtkCurveGenerator::PrintSelf(std::ostream &os, vtkIndent indent)
{
  ...
  os << indent << "SurfaceCostFunctionType: " <<
    vtkCjyxDijkstraGraphGeodesicPath::GetCostFunctionTypeAsString(this->GetSurfaceCostFunctionType()) << std::endl;
}
...
void vtkCurveGenerator::SortByMinimumSpanningTreePosition(vtkPoints* points, vtkDoubleArray* parameters)
{
  ...
  // vtk boost algorithms cannot be used because they are not built with 3D Cjyx
