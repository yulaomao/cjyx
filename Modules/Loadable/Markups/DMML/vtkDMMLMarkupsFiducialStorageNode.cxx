...
//------------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLMarkupsFiducialStorageNode);

//----------------------------------------------------------------------------
vtkDMMLMarkupsFiducialStorageNode::vtkDMMLMarkupsFiducialStorageNode()
{
...
//----------------------------------------------------------------------------
vtkDMMLMarkupsFiducialStorageNode::~vtkDMMLMarkupsFiducialStorageNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLMarkupsFiducialStorageNode::WriteXML(ostream& of, int nIndent)
{
  Superclass::WriteXML(of,nIndent);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsFiducialStorageNode::ReadXMLAttributes(const char** atts)
{
  Superclass::ReadXMLAttributes(atts);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsFiducialStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
  Superclass::PrintSelf(os,indent);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsFiducialStorageNode::Copy(vtkDMMLNode *anode)
{
  Superclass::Copy(anode);
}

//----------------------------------------------------------------------------
bool vtkDMMLMarkupsFiducialStorageNode::CanReadInReferenceNode(vtkDMMLNode *refNode)
{
  return refNode->IsA("vtkDMMLMarkupsNode");
}

//----------------------------------------------------------------------------
bool vtkDMMLMarkupsFiducialStorageNode::SetPointFromString(vtkDMMLMarkupsNode *markupsNode, int pointIndex, const char* line)
{
  if (!markupsNode)
    {
    vtkGenericWarningMacro("vtkDMMLMarkupsFiducialStorageNode::SetMarkupFromString failed: invalid markupsnode");
    return false;
    }

  if (pointIndex<0)
    {
    vtkGenericWarningMacro("vtkDMMLMarkupsFiducialStorageNode::SetMarkupFromString failed: invalid pointIndex");
    return false;
    }

  if (markupsNode->GetMaximumNumberOfControlPoints() >= 0 && pointIndex >= markupsNode->GetMaximumNumberOfControlPoints())
    {
    vtkGenericWarningMacro("vtkDMMLMarkupsFiducialStorageNode::SetMarkupFromString failed: index beyond control point maximum");
    return false;
    }

  if (this->GetCoordinateSystem() != vtkDMMLStorageNode::CoordinateSystemRAS
    && this->GetCoordinateSystem() != vtkDMMLStorageNode::CoordinateSystemLPS)
    {
    vtkGenericWarningMacro("vtkDMMLMarkupsFiducialStorageNode::SetMarkupFromString failed: invalid coordinate system");
    return false;
    }
  ...
  for (int i = 0; i < 3; i++)
    ...
    if (!parser.GetDoubleField(FIELD_XYZ + i, xyz[i]))
      {
      vtkGenericWarningMacro("vtkDMMLMarkupsFiducialStorageNode::SetMarkupFromString failed:"
  ...
  if (markupsNode->GetNthControlPoint(pointIndex) == nullptr)
    {
    vtkGenericWarningMacro("vtkDMMLMarkupsFiducialStorageNode::SetMarkupFromString failed: could not get/add control point");
    return false;
    }
  ...
  else if (this->GetCoordinateSystem() == vtkDMMLStorageNode::CoordinateSystemLPS)
    {
    markupsNode->SetNthControlPointPosition(pointIndex, -xyz[0], -xyz[1], xyz[2]);
    }
...
//----------------------------------------------------------------------------
std::string vtkDMMLMarkupsFiducialStorageNode::GetPointAsString(vtkDMMLMarkupsNode *markupsNode, int pointIndex)
{
  if (!markupsNode)
    {
    vtkGenericWarningMacro("vtkDMMLMarkupsFiducialStorageNode::GetMarkupAsString failed: invalid markupsnode");
    return "";
    }
  ...
  if (this->GetCoordinateSystem() == vtkDMMLStorageNode::CoordinateSystemLPS)
    {
    xyz[0] = -xyz[0];
    xyz[1] = -xyz[1];
    }
  else if (this->GetCoordinateSystem() != vtkDMMLStorageNode::CoordinateSystemRAS)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLMarkupsFiducialStorageNode::ReadDataInternal(vtkDMMLNode *refNode)
{
  if (!refNode)
    {
  ...
  if (fullName.empty())
    {
    vtkErrorMacro("vtkDMMLMarkupsFiducialStorageNode: File name not specified");
    return 0;
    }
  ...
  std::string ext = vtkDMMLStorageNode::GetLowercaseExtensionFromFileName(fullName);
  ...
  if (fstr.is_open())
    ...
    std::vector<char> lineBuff(vtkDMMLMarkupsFiducialStorageNode::GetMaximumLineLength());
    ...
    while (fstr.good())
      {
      fstr.getline(line, vtkDMMLMarkupsFiducialStorageNode::GetMaximumLineLength());
      ...
      if (line[0] == '#')
        ...
        if (name == "Markups fiducial file version")
          ...
          {
          int coordinateSystemFlag = vtkDMMLMarkupsStorageNode::GetCoordinateSystemFromString(value.c_str());
          ...
          if (version.size() == 0)
            ...
            if (parseAsAnnotationFiducial)
              ...
              if (!printedVersionWarning)
                {
                vtkWarningMacro("Have an unversioned file, assuming Cjyx 3 format .fcsv");
...
//----------------------------------------------------------------------------
int vtkDMMLMarkupsFiducialStorageNode::WriteDataInternal(vtkDMMLNode *refNode)
{
  std::string fullName = this->GetFullNameFromFileName();
  if (fullName.empty())
    {
  ...
  // put down a header
  of << "# Markups fiducial file version = " << Cjyx_VERSION << endl;
  of << "# CoordinateSystem = " << vtkDMMLMarkupsStorageNode::GetCoordinateSystemAsString(this->GetCoordinateSystem()) << endl;
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsFiducialStorageNode::InitializeSupportedReadFileTypes()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsFiducialStorageNode::InitializeSupportedWriteFileTypes()
{
  this->SupportedWriteFileTypes->InsertNextValue("Markups Fiducial CSV (.fcsv)");
}

//---------------------------------------------------------------------------
std::string vtkDMMLMarkupsFiducialStorageNode::ConvertStringToStorageFormat(std::string input)
{
...
//---------------------------------------------------------------------------
std::string vtkDMMLMarkupsFiducialStorageNode::ConvertStringFromStorageFormat(std::string input)
{
  std::string output = input;
