...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLMarkupsPlaneNode);

//----------------------------------------------------------------------------
vtkDMMLMarkupsPlaneNode::vtkDMMLMarkupsPlaneNode()
{
  ...
  this->CurveInputPoly->GetPoints()->AddObserver(vtkCommand::ModifiedEvent, this->DMMLCallbackCommand);
  this->ObjectToBaseMatrix->AddObserver(vtkCommand::ModifiedEvent, this->DMMLCallbackCommand);
  this->BaseToNodeMatrix->AddObserver(vtkCommand::ModifiedEvent, this->DMMLCallbackCommand);

  // Setup measurements calculated for this markup type
  vtkNew<vtkDMMLMeasurementArea> areaMeasurement;
  areaMeasurement->SetEnabled(false);
  areaMeasurement->SetName("area");
  areaMeasurement->SetInputDMMLNode(this);
  this->Measurements->AddItem(areaMeasurement);
}

//----------------------------------------------------------------------------
vtkDMMLMarkupsPlaneNode::~vtkDMMLMarkupsPlaneNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::ReadXMLAttributes(const char** atts)
{
  ...
  vtkDMMLReadXMLEnumMacro(sizeMode, SizeMode);
  vtkDMMLReadXMLVectorMacro(size, Size, double, 2);
  vtkDMMLReadXMLFloatMacro(autoSizeScalingFactor, AutoSizeScalingFactor);
  vtkDMMLReadXMLOwnedMatrix4x4Macro(planeTobaseMatrix, ObjectToBaseMatrix); // Backwards compatible with old name
  vtkDMMLReadXMLOwnedMatrix4x4Macro(objectToBaseMatrix, ObjectToBaseMatrix);

  vtkDMMLReadXMLEndMacro();
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::CopyContent(vtkDMMLNode* anode, bool deepCopy/*=true*/)
{
  DMMLNodeModifyBlocker blocker(this);
  vtkDMMLCopyBeginMacro(anode);
  vtkDMMLCopyEnumMacro(PlaneType);
  vtkDMMLCopyIntMacro(MaximumNumberOfControlPoints);
  vtkDMMLCopyIntMacro(RequiredNumberOfControlPoints);
  vtkDMMLCopyEnumMacro(SizeMode);
  vtkDMMLCopyVectorMacro(Size, double, 2);
  vtkDMMLCopyVectorMacro(Normal, double, 3);
  vtkDMMLCopyVectorMacro(Center, double, 3);
  vtkDMMLCopyFloatMacro(AutoSizeScalingFactor);
  vtkDMMLCopyEndMacro();
  ...
  vtkDMMLCopyBeginMacro(anode);
  vtkDMMLCopyOwnedMatrix4x4Macro(ObjectToBaseMatrix);
  vtkDMMLCopyOwnedMatrix4x4Macro(BaseToNodeMatrix);
  vtkDMMLCopyEndMacro();
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::PrintSelf(ostream& os, vtkIndent indent)
{
  Superclass::PrintSelf(os,indent);
  vtkDMMLPrintBeginMacro(os, indent);
  vtkDMMLPrintEnumMacro(SizeMode);
  vtkDMMLPrintVectorMacro(Size, double, 2);
  vtkDMMLPrintFloatMacro(AutoSizeScalingFactor);
  vtkDMMLPrintMatrix4x4Macro(ObjectToBaseMatrix);
  vtkDMMLPrintEndMacro();
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::ApplyTransform(vtkAbstractTransform* transform)
{
  if (!transform)
    {
    return;
...
//----------------------------------------------------------------------------
const char* vtkDMMLMarkupsPlaneNode::GetPlaneTypeAsString(int planeType)
{
  switch (planeType)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLMarkupsPlaneNode::GetPlaneTypeFromString(const char* planeType)
{
  ...
  for (int i = 0; i < vtkDMMLMarkupsPlaneNode::PlaneType_Last; i++)
    {
    if (strcmp(planeType, vtkDMMLMarkupsPlaneNode::GetPlaneTypeAsString(i)) == 0)
      {
...
//----------------------------------------------------------------------------
const char* vtkDMMLMarkupsPlaneNode::GetSizeModeAsString(int sizeMode)
{
  switch (sizeMode)
    {
  case SizeModeAuto:
...
//----------------------------------------------------------------------------
int vtkDMMLMarkupsPlaneNode::GetSizeModeFromString(const char* sizeMode)
{
  if (sizeMode == nullptr)
    {
    // invalid size mode
  ...
  for (int i = 0; i < SizeMode_Last; ++i)
    {
    if (strcmp(vtkDMMLMarkupsPlaneNode::GetSizeModeAsString(i), sizeMode) == 0)
      {
      return i;
      }
    }
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetSizeMode(int sizeMode)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetPlaneType(int planeType)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetNormalPointRequired(bool normalPointRequired)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetNormal(double normal_Node[3])
{
  if (!normal_Node)
    {
...
//----------------------------------------------------------------------------
double* vtkDMMLMarkupsPlaneNode::GetNormal()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetNormalWorld(double normalWorld[3])
{
  if (!normalWorld)
    {
...
//----------------------------------------------------------------------------
double* vtkDMMLMarkupsPlaneNode::GetNormalWorld()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetNormal(const double normal_Node[3])
{
  if (!normal_Node)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetNormal(double x_Node, double y_Node, double z_Node)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetNormalWorld(const double normal_World[3])
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetNormalWorld(double x_World, double y_World, double z_World)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetOrigin(double origin_Node[3])
{
  if (!origin_Node)
    {
...
//----------------------------------------------------------------------------
double* vtkDMMLMarkupsPlaneNode::GetOrigin()
{
  vtkNew<vtkMatrix4x4> objectToNodeMatrix;
  this->GetObjectToNodeMatrix(objectToNodeMatrix);
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetOriginWorld(double origin_World[3])
{
  if (!origin_World)
    {
...
//----------------------------------------------------------------------------
double* vtkDMMLMarkupsPlaneNode::GetOriginWorld()
{
  vtkNew<vtkMatrix4x4> objectToWorldMatrix;
  this->GetObjectToWorldMatrix(objectToWorldMatrix);
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetOrigin(const double origin_Node[3])
{
  if (!origin_Node)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetOrigin(double x_Node, double y_Node, double z_Node)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetOriginWorld(const double origin_World[3])
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetOriginWorld(double x_World, double y_World, double z_World)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::CalculateAxesFromPoints(const double point0[3], const double point1[3], const double point2[3],
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetBaseToNodeMatrix(vtkMatrix4x4* baseToNodeMatrix)
{
  if (!baseToNodeMatrix)
    {
...
//----------------------------------------------------------------------------
vtkMatrix4x4* vtkDMMLMarkupsPlaneNode::GetBaseToNodeMatrix()
{
  return this->BaseToNodeMatrix;
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetObjectToNodeMatrix(vtkMatrix4x4* objectToNodeMatrix)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetObjectToWorldMatrix(vtkMatrix4x4* objectToWorldMatrix)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetBaseToWorldMatrix(vtkMatrix4x4* baseToWorldMatrix)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetAxes(double xAxis_Node[3], double yAxis_Node[3], double zAxis_Node[3])
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetAxesWorld(double xAxis_World[3], double yAxis_World[3], double zAxis_World[3])
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetAxes(const double xAxis_Node[3], const double yAxis_Node[3], const double zAxis_Node[3])
{
  if (!xAxis_Node || !yAxis_Node || !zAxis_Node)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetAxesWorld(const double xAxis_World[3], const double yAxis_World[3], const double zAxis_World[3])
{
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::ProcessDMMLEvents(vtkObject* caller, unsigned long event, void* callData)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetSize(double planeSize[2])
{
...
//----------------------------------------------------------------------------
double* vtkDMMLMarkupsPlaneNode::GetSize()
{
  return this->Size;
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetSize(double x, double y)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetSizeWorld(double size_World[2])
{
  vtkNew<vtkMatrix4x4> objectToWorldMatrix;
  this->GetObjectToWorldMatrix(objectToWorldMatrix);
  vtkNew<vtkTransform> objectToWorldTransform;
...
//----------------------------------------------------------------------------
double* vtkDMMLMarkupsPlaneNode::GetSizeWorld()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetSizeWorld(const double size_World[2])
{
  this->SetSizeWorld(size_World[0], size_World[1]);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetSizeWorld(double sizeX_World, double sizeY_World)
{
  vtkNew<vtkMatrix4x4> objectToWorldMatrix;
  this->GetObjectToWorldMatrix(objectToWorldMatrix);
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::SetPlaneBounds(double x0, double x1, double y0, double y1)
{
...
//----------------------------------------------------------------------------
vtkMatrix4x4* vtkDMMLMarkupsPlaneNode::GetObjectToBaseMatrix()
{
  return this->ObjectToBaseMatrix;
}

//---------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::OnTransformNodeReferenceChanged(vtkDMMLTransformNode* transformNode)
{
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdateInteractionHandleToWorldMatrix()
{
...
//---------------------------------------------------------------------------
double vtkDMMLMarkupsPlaneNode::GetClosestPointOnPlaneWorld(const double posWorld[3], double closestPosWorld[3], bool infinitePlane/*=true*/)
{
  if (!posWorld)
    {
...
//---------------------------------------------------------------------------
vtkDMMLStorageNode* vtkDMMLMarkupsPlaneNode::CreateDefaultStorageNode()
{
  ...
  return vtkDMMLStorageNode::SafeDownCast(
    scene->CreateNodeByClass("vtkDMMLMarkupsPlaneJsonStorageNode"));
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::CreateDefaultDisplayNodes()
{
  if (this->GetDisplayNode() != nullptr &&
    vtkDMMLMarkupsPlaneDisplayNode::SafeDownCast(this->GetDisplayNode()) != nullptr)
    {
  ...
  if (this->GetScene() == nullptr)
    {
    vtkErrorMacro("vtkDMMLMarkupsPlaneNode::CreateDefaultDisplayNodes failed: scene is invalid");
    return;
    }
  vtkDMMLMarkupsPlaneDisplayNode* dispNode = vtkDMMLMarkupsPlaneDisplayNode::SafeDownCast(
    this->GetScene()->AddNewNodeByClass("vtkDMMLMarkupsPlaneDisplayNode"));
  if (!dispNode)
    {
    vtkErrorMacro("vtkDMMLMarkupsPlaneNode::CreateDefaultDisplayNodes failed: scene failed to instantiate a vtkDMMLMarkupsPlaneDisplayNode node");
    return;
    }
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetRASBounds(double bounds[6])
{
  if (!bounds)
    {
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetBounds(double bounds[6])
{
  if (!bounds)
    {
    vtkErrorMacro("Invalid bounds argument");
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetPlaneCornerPoints(vtkPoints* points_Node)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GetPlaneCornerPointsWorld(vtkPoints* points_World)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::CalculatePlaneCornerPoints(vtkPoints * points, double xAxis[3], double yAxis[3], double center[3], double size[2])
{
  if (!points)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::CalculatePlaneBounds(vtkPoints* cornerPoints, double bounds[6])
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdatePlaneFromControlPoints()
{
  if (this->IsUpdatingControlPointsFromPlane || this->IsUpdatingPlaneFromControlPoints)
    {
    return;
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdatePlaneFromPointNormal()
{
  double origin_Node[3] = { 0.0, 0.0, 0.0 };
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdatePlaneFrom3Points()
{
...
//----------------------------------------------------------------------------
bool vtkDMMLMarkupsPlaneNode::GetClosestFitPlaneFromControlPoints(vtkMatrix4x4* closestFitPlane)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdatePlaneSize()
{
  if (this->SizeMode != vtkDMMLMarkupsPlaneNode::SizeModeAuto)
    {
    return;
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdateControlPointsFromPlane()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdateControlPointsFromPointNormal()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdateControlPointsFrom3Points()
{
  if (!this->GetIsPlaneValid())
    {
    return;
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::UpdateControlPointsFromPlaneFit()
{
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GenerateOrthogonalMatrix(vtkMatrix4x4* inputMatrix,
  vtkMatrix4x4* outputMatrix, vtkAbstractTransform* transform/*=nullptr*/, bool applyScaling/*=true*/)
{
  ...
  vtkDMMLMarkupsPlaneNode::GenerateOrthogonalMatrix(xAxis, yAxis, zAxis, origin, outputMatrix, transform, applyScaling);
}

//---------------------------------------------------------------------------
void vtkDMMLMarkupsPlaneNode::GenerateOrthogonalMatrix(double xAxis[3], double yAxis[3], double zAxis[3], double origin[3],
  vtkMatrix4x4* outputMatrix, vtkAbstractTransform* transform/*=nullptr*/, bool applyScaling/*=true*/)
{
