...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLMarkupsROINode);

//----------------------------------------------------------------------------
vtkDMMLMarkupsROINode::vtkDMMLMarkupsROINode()
{
  ...
  this->ObjectToNodeMatrix->AddObserver(vtkCommand::ModifiedEvent, this->DMMLCallbackCommand);

  this->ObjectToWorldMatrix = vtkSmartPointer<vtkMatrix4x4>::New();

  this->InteractionHandleToWorldMatrix->AddObserver(vtkCommand::ModifiedEvent, this->DMMLCallbackCommand);

  // Setup measurements calculated for this markup type
  vtkNew<vtkDMMLMeasurementVolume> volumeMeasurement;
  volumeMeasurement->SetEnabled(false);
  volumeMeasurement->SetName("volume");
  volumeMeasurement->SetInputDMMLNode(this);
...
//----------------------------------------------------------------------------
vtkDMMLMarkupsROINode::~vtkDMMLMarkupsROINode() = default;

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::CopyContent(vtkDMMLNode* anode, bool deepCopy/*=true*/)
{
  DMMLNodeModifyBlocker blocker(this);
  Superclass::CopyContent(anode, deepCopy);

  vtkDMMLCopyBeginMacro(anode);
  vtkDMMLCopyEnumMacro(ROIType);
  vtkDMMLCopyVectorMacro(Size, double, 3);
  vtkDMMLCopyOwnedMatrix4x4Macro(ObjectToNodeMatrix);
  vtkDMMLCopyBooleanMacro(InsideOut);
  vtkDMMLCopyEndMacro();
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::PrintSelf(ostream& os, vtkIndent indent)
{
  Superclass::PrintSelf(os,indent);

  vtkDMMLPrintBeginMacro(os, indent);
  vtkDMMLPrintEnumMacro(ROIType);
  vtkDMMLPrintVectorMacro(Size, double, 3);
  vtkDMMLPrintMatrix4x4Macro(ObjectToNodeMatrix);
  vtkDMMLPrintBooleanMacro(InsideOut);
  vtkDMMLPrintEndMacro();
}

//----------------------------------------------------------------------------
vtkDMMLStorageNode* vtkDMMLMarkupsROINode::CreateDefaultStorageNode()
{
  ...
  return vtkDMMLStorageNode::SafeDownCast(
    scene->CreateNodeByClass("vtkDMMLMarkupsROIJsonStorageNode"));
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::CreateDefaultDisplayNodes()
{
  if (this->GetDisplayNode() != nullptr &&
    vtkDMMLMarkupsROIDisplayNode::SafeDownCast(this->GetDisplayNode()) != nullptr)
    {
  ...
  if (this->GetScene() == nullptr)
    {
    vtkErrorMacro("vtkDMMLMarkupsROINode::CreateDefaultDisplayNodes failed: scene is invalid");
    return;
    }
  vtkDMMLMarkupsROIDisplayNode* dispNode = vtkDMMLMarkupsROIDisplayNode::SafeDownCast(
    this->GetScene()->AddNewNodeByClass("vtkDMMLMarkupsROIDisplayNode"));
  if (!dispNode)
    {
    vtkErrorMacro("vtkDMMLMarkupsROINode::CreateDefaultDisplayNodes failed: scene failed to instantiate a vtkDMMLMarkupsROIDisplayNode node");
    return;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetAndObserveObjectToNodeMatrix(vtkMatrix4x4* objectToNodeMatrix)
{
  ...
  if (this->ObjectToNodeMatrix)
    {
    this->ObjectToNodeMatrix->RemoveObserver(this->DMMLCallbackCommand);
    }
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::ApplyTransform(vtkAbstractTransform* transform)
{
  if (!transform)
    {
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetRASBounds(double bounds[6])
{
  if (!bounds)
    {
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetBounds(double bounds[6])
{
  if (!bounds)
    {
    vtkErrorMacro("Invalid bounds argument");
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GenerateBoxBounds(double bounds[6], double xAxis[3], double yAxis[3], double zAxis[3], double center[3], double size[3])
{
...
//----------------------------------------------------------------------------
const char* vtkDMMLMarkupsROINode::GetROITypeAsString(int roiType)
{
  switch (roiType)
    {
...
//-----------------------------------------------------------
int vtkDMMLMarkupsROINode::GetROITypeFromString(const char* name)
{
  if (name == nullptr)
    {
    // invalid name
  ...
  for (int i = 0; i < vtkDMMLMarkupsROINode::ROIType_Last; i++)
    {
    if (strcmp(name, vtkDMMLMarkupsROINode::GetROITypeAsString(i)) == 0)
      {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetXAxisWorld(double axis_World[3])
{
  this->GetAxisWorld(0, axis_World);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetYAxisWorld(double axis_World[3])
{
  this->GetAxisWorld(1, axis_World);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetZAxisWorld(double axis_World[3])
{
  this->GetAxisWorld(2, axis_World);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetAxisWorld(int axisIndex, double axis_World[3])
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetXAxis(double axis_Node[3])
{
  this->GetAxis(0, axis_Node);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetYAxis(double axis_Node[3])
{
  this->GetAxis(1, axis_Node);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetZAxis(double axis_Node[3])
{
  this->GetAxis(2, axis_Node);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetAxis(int axisIndex, double axis_Node[3])
{
  if (axisIndex < 0 || axisIndex >= 3)
    {
    vtkErrorMacro("Invalid axisIndex. Must be 0, 1, or 2.");
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::OnTransformNodeReferenceChanged(vtkDMMLTransformNode* transformNode)
{
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::ProcessDMMLEvents(vtkObject* caller, unsigned long event, void* callData)
{
  if (caller == this->CurveInputPoly->GetPoints() || caller == this->GetParentTransformNode())
    ...
    vtkDMMLTransformNode::GetTransformBetweenNodes(nullptr, this->GetParentTransformNode(), worldToNode);
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetROIType(int roiType)
{
...
//----------------------------------------------------------------------------
vtkVector3d vtkDMMLMarkupsROINode::GetCenter()
{
  vtkVector3d center(
...
//----------------------------------------------------------------------------
vtkVector3d vtkDMMLMarkupsROINode::GetCenterWorld()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetCenter(double center_Node[3])
{
  if (!center_Node)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetCenterWorld(double center_World[3])
{
  if (!center_World)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetCenterWorld(const double center_World[3])
{
  if (!center_World)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetCenterWorld(double x, double y, double z)
{
  double xyz[3] = { x, y, z };
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetCenter(const double center_Node[3])
{
  if (!center_Node)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetCenter(double x, double y, double z)
{
  double xyz[3] = { x, y, z };
  this->SetCenter(xyz);
}
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetSize(const double center[3])
{
  this->SetSize(center[0], center[1], center[2]);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetSize(double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetSizeWorld(double size_World[3])
{
  if (!size_World)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetSizeWorld(const double size_World[3])
{
  this->SetSizeWorld(size_World[0], size_World[1], size_World[2]);
}

//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetSizeWorld(double x_World, double y_World, double z_World)
{
  vtkNew<vtkGeneralTransform> worldToNodeTransform;
  vtkDMMLTransformNode::GetTransformBetweenNodes(nullptr, this->GetParentTransformNode(), worldToNodeTransform);
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetInsideOut(bool insideOut)
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateImplicitFunction()
{
  ...
  if (!sumFunction || !sumFunctionWorld)
    {
    vtkErrorMacro("vtkDMMLMarkupsROINode::UpdateImplicitFunction: Invalid implicit function");
    return;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateROIFromControlPoints()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateBoxROIFromControlPoints()
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateBoundingBoxROIFromControlPoints()
{
  DMMLNodeModifyBlocker blocker(this);
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateControlPointsFromROI()
{
  if (this->IsUpdatingControlPointsFromROI || this->IsUpdatingROIFromControlPoints)
    {
    return;
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateControlPointsFromBoundingBoxROI()
{
  vtkNew<vtkTransform> nodeToObjectTransform;
  nodeToObjectTransform->SetMatrix(this->ObjectToNodeMatrix);
  nodeToObjectTransform->Inverse();
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateControlPointsFromBoxROI()
{
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateInteractionHandleToWorldMatrix()
{
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::UpdateObjectToWorldMatrix()
{
  vtkNew<vtkGeneralTransform> nodeToWorldTansform;
  vtkDMMLTransformNode::GetTransformBetweenNodes(this->GetParentTransformNode(), nullptr, nodeToWorldTansform);
...
//---------------------------------------------------------------------------
bool vtkDMMLMarkupsROINode::GetXYZ(double center[3])
{
...
//---------------------------------------------------------------------------
int vtkDMMLMarkupsROINode::SetXYZ(double center[3])
{
...
//---------------------------------------------------------------------------
int vtkDMMLMarkupsROINode::SetXYZ(double x, double y, double z)
{
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetRadiusXYZ(double radiusXYZ[3])
{
  this->SetSize(radiusXYZ[0] * 2.0, radiusXYZ[1] * 2.0, radiusXYZ[2] * 2.0);
}

//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::SetRadiusXYZ(double x, double y, double z)
{
  double tempXYZ[3] = { x, y, z };
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetRadiusXYZ(double radiusXYZ[3])
{
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetPlanes(vtkPlanes* planes, bool insideOut/*=false*/)
{
  if (!planes)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetPlanesWorld(vtkPlanes* planes, bool insideOut/*=false*/)
{
  if (!planes)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GetTransformedPlanes(vtkPlanes* planes, bool insideOut/*=false*/)
{
  if (!planes)
    {
...
//----------------------------------------------------------------------------
bool vtkDMMLMarkupsROINode::IsPointInROI(double point_Node[3])
{
  return this->ImplicitFunction->FunctionValue(point_Node) <= 0;
}

//----------------------------------------------------------------------------
bool vtkDMMLMarkupsROINode::IsPointInROIWorld(double point_World[3])
{
  return this->ImplicitFunctionWorld->FunctionValue(point_World) <= 0;
}

//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GenerateOrthogonalMatrix(vtkMatrix4x4* inputMatrix,
  vtkMatrix4x4* outputMatrix, vtkAbstractTransform* transform/*=nullptr*/, bool applyScaling/*=true*/)
{
  ...
  vtkDMMLMarkupsROINode::GenerateOrthogonalMatrix(xAxis, yAxis, zAxis, origin, outputMatrix, transform, applyScaling);
}

//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::GenerateOrthogonalMatrix(double xAxis[3], double yAxis[3], double zAxis[3], double origin[3],
  vtkMatrix4x4* outputMatrix, vtkAbstractTransform* transform/*=nullptr*/, bool applyScaling/*=true*/)
{
...
//---------------------------------------------------------------------------
void vtkDMMLMarkupsROINode::WriteCLI(std::vector<std::string>& commandLine, std::string prefix,
...
//---------------------------------------------------------------------------
bool vtkDMMLMarkupsROINode::GetObjectToNodeMatrixRotated()
{
