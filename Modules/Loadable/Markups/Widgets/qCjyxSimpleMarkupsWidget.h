...
#ifndef __qCjyxSimpleMarkupsWidget_h
#define __qCjyxSimpleMarkupsWidget_h
...
#include "ui_qCjyxSimpleMarkupsWidget.h"
...
qCjyxSimpleMarkupsWidget : public qCjyxWidget
{
  Q_OBJECT
...
public:
  ...
  qCjyxSimpleMarkupsWidget(QWidget *parent=nullptr);
  ~qCjyxSimpleMarkupsWidget() override;

  /// Get the currently selected markups node.
  Q_INVOKABLE vtkDMMLNode* currentNode() const;

  /// \deprecated Use currentNode() instead.
  Q_INVOKABLE vtkDMMLNode* getCurrentNode();
  ...
  Q_INVOKABLE qCjyxMarkupsPlaceWidget* markupsPlaceWidget() const;

  /// Get the markup node selector combo box
  Q_INVOKABLE qDMMLNodeComboBox* markupsSelectorComboBox() const;
...
public slots:
  ...
  /// Set the currently selected markups node to be the active markups node in the Cjyx scene.
  void activate();

  /// Set the currently selected markups node to be the active markups node in the Cjyx scene. The argument \a place is true then also interaction mode is set to place mode.
...
protected slots:
  ...
  void onMarkupsNodeAdded(vtkDMMLNode*);
...
protected:
  QScopedPointer<qCjyxSimpleMarkupsWidgetPrivate> d_ptr;

  virtual void setup();

private:
  Q_DECLARE_PRIVATE(qCjyxSimpleMarkupsWidget);
  Q_DISABLE_COPY(qCjyxSimpleMarkupsWidget);
