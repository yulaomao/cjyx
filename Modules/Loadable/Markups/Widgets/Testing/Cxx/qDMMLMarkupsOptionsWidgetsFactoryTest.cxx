...
#include "qDMMLMarkupsMalformedWidget.h"
...
//------------------------------------------------------------------------------
class qDMMLMarkupsOptionsWidgetsFactoryTester : public QObject
{
  Q_OBJECT
...
void qDMMLMarkupsOptionsWidgetsFactoryTester::
testOptionsWidgetRegistration1()
{
  ...
  factory->registerOptionsWidget(new qDMMLMarkupsCurveSettingsWidget());
  ...
  factory->unregisterOptionsWidget(new qDMMLMarkupsCurveSettingsWidget());
...
testOptionsWidgetRegistration3()
{
  ...
  factory->registerOptionsWidget(new qDMMLMarkupsMalformedWidget());
  QCOMPARE(registeredSignalSpy.count(), 0); // Make sure the signal triggers only 1 time.
  auto registeredWidgets = factory->registeredOptionsWidgetsClassNames();
  QVERIFY(registeredWidgets.length() == 0);

  // Test unregistration of an invalid widget
  factory->unregisterOptionsWidget(new qDMMLMarkupsMalformedWidget());
...
testOptionsWidgetRegistration5()
{
  ...
  // Test unregistration of an invalid widget
  factory->unregisterOptionsWidget(new qDMMLMarkupsAngleMeasurementsWidget);
  QCOMPARE(unregisteredSignalSpy.count(), 0); // Make sure the signal triggers only 1 time.
  registeredWidgets = factory->registeredOptionsWidgetsClassNames();
  QVERIFY(registeredWidgets.length() == 1);
...
//------------------------------------------------------------------------------
CTK_TEST_MAIN(qDMMLMarkupsOptionsWidgetsFactoryTest)
#include "moc_qDMMLMarkupsOptionsWidgetsFactoryTest.cxx"
