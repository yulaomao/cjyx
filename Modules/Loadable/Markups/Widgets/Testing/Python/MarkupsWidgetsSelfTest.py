...
class MarkupsWidgetsSelfTestTest(ScriptedLoadableModuleTest):
    ...
    def section_CreateMarkups(self):
        ...
        self.markupsLogic = cjyx.modules.markups.logic()

        # Create sample markups node
        self.markupsNode1 = cjyx.dmmlScene.GetNodeByID(self.markupsLogic.AddNewFiducialNode())
        self.markupsNode1.SetName(self.sampleMarkupsNodeName1)

        self.markupsNode2 = cjyx.dmmlScene.GetNodeByID(self.markupsLogic.AddNewFiducialNode())
    ...
    def section_SimpleMarkupsWidget(self):
        ...
        simpleMarkupsWidget = cjyx.qCjyxSimpleMarkupsWidget()
        nodeSelector = cjyx.util.findChildren(simpleMarkupsWidget, "MarkupsNodeComboBox")[0]
        self.assertIsNone(simpleMarkupsWidget.interactionNode())
        simpleMarkupsWidget.setDMMLScene(cjyx.dmmlScene)
        ...
        self.assertEqual(simpleMarkupsWidget.interactionNode(), cjyx.app.applicationLogic().GetInteractionNode())
        otherInteractionNode = cjyx.vtkDMMLInteractionNode()
        otherInteractionNode.SetSingletonOff()
        cjyx.dmmlScene.AddNode(otherInteractionNode)
    ...
    def section_MarkupsPlaceWidget(self):
        ...
        placeWidget.placeMultipleMarkups = cjyx.qCjyxMarkupsPlaceWidget.ForcePlaceSingleMarkup
        placeWidget.placeModeEnabled = True
        self.assertFalse(placeWidget.placeModePersistency)

        placeWidget.placeMultipleMarkups = cjyx.qCjyxMarkupsPlaceWidget.ForcePlaceMultipleMarkups
        placeWidget.placeModeEnabled = False
        placeWidget.placeModeEnabled = True
        self.assertTrue(placeWidget.placeModePersistency)

        self.assertEqual(placeWidget.interactionNode(), cjyx.app.applicationLogic().GetInteractionNode())
        otherInteractionNode = cjyx.vtkDMMLInteractionNode()
        otherInteractionNode.SetSingletonOff()
        cjyx.dmmlScene.AddNode(otherInteractionNode)
