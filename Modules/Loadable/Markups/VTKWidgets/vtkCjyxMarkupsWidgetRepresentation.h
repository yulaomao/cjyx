 ...
 * simplified and specialized for optimal use in Cjyx.
 * It state is stored in the associated DMML display node to
 * avoid extra synchronization mechanisms.
 * The representation only observes DMML node changes,
 * it does not directly process any interaction events directly
 * (interaction events are processed by vtkDMMLAbstractWidget,
 * which then modifies DMML nodes).
 *
...
*
 * @sa
 * vtkCjyxMarkupsWidgetRepresentation vtkDMMLAbstractWidget vtkPointPlacer
*/

#ifndef vtkCjyxMarkupsRepresentation_h
#define vtkCjyxMarkupsRepresentation_h
...
class VTK_CJYX_MARKUPS_MODULE_VTKWIDGETS_EXPORT vtkCjyxMarkupsWidgetRepresentation : public vtkDMMLAbstractWidgetRepresentation
{
public:
  enum
  {
  ...
  vtkTypeMacro(vtkCjyxMarkupsWidgetRepresentation, vtkDMMLAbstractWidgetRepresentation);
  ...
  virtual vtkDMMLMarkupsNode::ControlPoint *GetNthControlPoint(int n);

  /// Set/Get the vtkDMMLMarkipsNode connected with this representation
  virtual void SetMarkupsDisplayNode(vtkDMMLMarkupsDisplayNode *markupsDisplayNode);
  virtual vtkDMMLMarkupsDisplayNode* GetMarkupsDisplayNode();
  virtual vtkDMMLMarkupsNode* GetMarkupsNode();
  ...
  /// Return found component type (as vtkDMMLMarkupsDisplayNode::ComponentType).
  ...
  virtual void CanInteract(vtkDMMLInteractionEventData* interactionEventData,
...
protected:
  vtkCjyxMarkupsWidgetRepresentation();
  ~vtkCjyxMarkupsWidgetRepresentation() override;
  ...
  class VTK_CJYX_MARKUPS_MODULE_VTKWIDGETS_EXPORT MarkupsInteractionPipeline
  {
  public:
    MarkupsInteractionPipeline(vtkDMMLAbstractWidgetRepresentation* representation);
    virtual ~MarkupsInteractionPipeline();

    vtkWeakPointer<vtkDMMLAbstractWidgetRepresentation> Representation;
    ...
    /// Type is specified using vtkDMMLMarkupsDisplayNode::ComponentType
  ...
  // Update the size of the interaction handle based on screen size + vtkDMMLMarkupsDisplayNode::InteractionHandleScale parameter.
  ...
  virtual void SetMarkupsNode(vtkDMMLMarkupsNode *markupsNode);
...
private:
  vtkCjyxMarkupsWidgetRepresentation(const vtkCjyxMarkupsWidgetRepresentation&) = delete;
  void operator=(const vtkCjyxMarkupsWidgetRepresentation&) = delete;
};
