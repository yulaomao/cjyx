...
#ifndef __qDMMLSegmentsModel_h
#define __qDMMLSegmentsModel_h
...
class qDMMLSegmentsModelPrivate;
...
///
/// Associated with a vtkDMMLSegmentation node. This model creates one model item
...
class Q_CJYX_MODULE_SEGMENTATIONS_WIDGETS_EXPORT qDMMLSegmentsModel : public QStandardItemModel
{
  Q_OBJECT
  QVTK_OBJECT
  ...
  /// Control in which column the segment color is displayed (qDMMLSegmentsModel::VisibilityRole).
  /// A value of -1 (default) hides the column
  ...
  /// Control in which column the segment status is displayed (qDMMLSegmentsModel::StatusRole).
  Q_PROPERTY(int statusColumn READ statusColumn WRITE setStatusColumn)
  /// Control in which column the segment layer is displayed (qDMMLSegmentsModel::LayerRole).
...
public:
  ...
  qDMMLSegmentsModel(QObject *parent=nullptr);
  ~qDMMLSegmentsModel() override;
  ...
  vtkDMMLSegmentationNode* segmentationNode()const;
  virtual void setSegmentationNode(vtkDMMLSegmentationNode* segmentation);
...
protected:
  qDMMLSegmentsModel(qDMMLSegmentsModelPrivate* pimpl, QObject *parent=nullptr);
  ...
  /// Update a segment in the DMML node using the associated QStandardItem
  virtual void updateSegmentFromItem(QString segmentID, QStandardItem* item );
  /// Update a segment in the DMML node using the associated QStandardItem data
  ...
  /// Rearrange the order of the rows to match the indices of the segments in the DMML node
  void reorderItems();
  ...
  /// Called when the vtkDMMLSegmentationDisplayNode is modified
  virtual void onDisplayNodeModified();

protected:
  QScopedPointer<qDMMLSegmentsModelPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLSegmentsModel);
  Q_DISABLE_COPY(qDMMLSegmentsModel);
};
