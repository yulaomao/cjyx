...
#ifndef __qDMMLSegmentEditorWidget_h
#define __qDMMLSegmentEditorWidget_h
...
class qDMMLSegmentEditorWidgetPrivate;
...
///
/// IMPORTANT: The embedding module is responsible for setting the DMML scene and the
///   management of the \sa vtkDMMLSegmentEditorNode parameter set node.
///   The empty parameter set node should be set before the DMML scene, so that the
...
///
class Q_CJYX_MODULE_SEGMENTATIONS_WIDGETS_EXPORT qDMMLSegmentEditorWidget : public qDMMLWidget
{
  Q_OBJECT
  QVTK_OBJECT
...
public:
  ...
  /// Constructor
  explicit qDMMLSegmentEditorWidget(QWidget* parent = nullptr);
  /// Destructor
  ~qDMMLSegmentEditorWidget() override;

  /// Get the segment editor parameter set node
  Q_INVOKABLE vtkDMMLSegmentEditorNode* dmmlSegmentEditorNode()const;

  /// Get currently selected segmentation DMML node
  Q_INVOKABLE vtkDMMLNode* segmentationNode()const;
  /// Get ID of currently selected segmentation node
  Q_INVOKABLE QString segmentationNodeID()const;
  /// Get currently selected master volume DMML node
  Q_INVOKABLE vtkDMMLNode* masterVolumeNode()const;
  ...
  Q_INVOKABLE qCjyxSegmentEditorAbstractEffect* activeEffect()const;
  /// Set active effect
  /// \sa m_ActiveEffect, activeEffect()
  Q_INVOKABLE void setActiveEffect(qCjyxSegmentEditorAbstractEffect* effect);

  /// Get an effect object by name
  /// \return The effect instance if exists, nullptr otherwise
  Q_INVOKABLE qCjyxSegmentEditorAbstractEffect* effectByName(QString name);
  ...
  /// \return The effect instance if exists, nullptr otherwise
  Q_INVOKABLE qCjyxSegmentEditorAbstractEffect* effectByIndex(int index);
  ...
  /// \sa qDMMLNodeComboBox::addAttribute
...
public slots:
  ...
  void setDMMLSegmentEditorNode(vtkDMMLSegmentEditorNode* newSegmentEditorNode);

  /// Update widget state from the DMML scene
  virtual void updateWidgetFromDMML();

  /// Set segmentation DMML node
  void setSegmentationNode(vtkDMMLNode* node);
  /// Set segmentation DMML node by its ID
  void setSegmentationNodeID(const QString& nodeID);
  /// Set master volume DMML node.
  /// If master volume has multiple scalar components
  /// then only the first scalar component is used.
  void setMasterVolumeNode(vtkDMMLNode* node);
  /// Set master volume DMML node by its ID
...
signals:
  ...
  void masterVolumeNodeChanged(vtkDMMLVolumeNode*);
  ...
  void segmentationNodeChanged(vtkDMMLSegmentationNode*);

protected slots:
  /// Handles changing of current segmentation DMML node
  void onSegmentationNodeChanged(vtkDMMLNode* node);
  /// Handles changing of the current master volume DMML node
  void onMasterVolumeNodeChanged(vtkDMMLNode* node);
...
protected:
  ...
  void updateEffectsSectionFromDMML();
  ...
  qCjyxAbstractModuleWidget* switchToSegmentationsModule();

protected:
  QScopedPointer<qDMMLSegmentEditorWidgetPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLSegmentEditorWidget);
  Q_DISABLE_COPY(qDMMLSegmentEditorWidget);
};
