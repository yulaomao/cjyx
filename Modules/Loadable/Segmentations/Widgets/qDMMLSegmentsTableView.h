...
#ifndef __qDMMLSegmentsTableView_h
#define __qDMMLSegmentsTableView_h
...
class qDMMLSegmentsModel;
class qDMMLSegmentsTableViewPrivate;
class qDMMLSortFilterSegmentsProxyModel;
...
class Q_CJYX_MODULE_SEGMENTATIONS_WIDGETS_EXPORT qDMMLSegmentsTableView : public qDMMLWidget
{
  Q_OBJECT
  QVTK_OBJECT
...
public:
  ...
  /// Constructor
  explicit qDMMLSegmentsTableView(QWidget* parent = nullptr);
  /// Destructor
  ~qDMMLSegmentsTableView() override;

  /// Get segmentation DMML node
  Q_INVOKABLE vtkDMMLNode* segmentationNode();
  ...
  Q_INVOKABLE qDMMLSortFilterSegmentsProxyModel* sortFilterProxyModel()const;
  Q_INVOKABLE qDMMLSegmentsModel* model()const;
...
protected slots:
  ...
  /// Update the widget form the DMML node
  /// Called when the segmentation node is modified
  void updateWidgetFromDMML();

  /// Update the filter parameters in the vtkDMMLSegmentationNode
  void updateDMMLFromFilterParameters();
...
protected:
  QScopedPointer<qDMMLSegmentsTableViewPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLSegmentsTableView);
  Q_DISABLE_COPY(qDMMLSegmentsTableView);
};
