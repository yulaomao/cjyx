...
class SegmentEditorSmoothingEffect(AbstractScriptedSegmentEditorPaintEffect):
    ...
    def clone(self):
        ...
        clonedEffect = effects.qCjyxSegmentEditorScriptedPaintEffect(None)
...
If segments overlap, segment higher in the segments table will have priority. <b>Applied to all visible segments.</b></li>
    ...
    def setupOptionsFrame(self):
        ...
        self.kernelSizeMMSpinBox = cjyx.qDMMLSpinBox()
        self.kernelSizeMMSpinBox.setDMMLScene(cjyx.dmmlScene)
        ...
        self.gaussianStandardDeviationMMSpinBox = cjyx.qDMMLSpinBox()
        self.gaussianStandardDeviationMMSpinBox.setDMMLScene(cjyx.dmmlScene)
        ...
        self.methodSelectorComboBox.connect("currentIndexChanged(int)", self.updateDMMLFromGUI)
        self.kernelSizeMMSpinBox.connect("valueChanged(double)", self.updateDMMLFromGUI)
        self.gaussianStandardDeviationMMSpinBox.connect("valueChanged(double)", self.updateDMMLFromGUI)
        self.jointTaubinSmoothingFactorSlider.connect("valueChanged(double)", self.updateDMMLFromGUI)
        self.applyToAllVisibleSegmentsCheckBox.connect("stateChanged(int)", self.updateDMMLFromGUI)
    ...
    def showStatusMessage(self, msg, timeoutMsec=500):
        cjyx.util.showStatusMessage(msg, timeoutMsec)
    ...
    def onApply(self, maskImage=None, maskExtent=None):
        ...
        try:
            ...
            if smoothingMethod == JOINT_TAUBIN:
                ...
                segmentEditorNode = segmentEditorWidget.dmmlSegmentEditorNode()
    ...
    def clipImage(self, inputImage, maskExtent, margin):
        ...
        clipper.Update()
        clippedImage = cjyx.vtkOrientedImageData()
    ...
    def modifySelectedSegmentByLabelmap(self, smoothedImage, selectedSegmentLabelmap, modifierLabelmap, maskImage, maskExtent):
        if maskImage:
            smoothedClippedSelectedSegmentLabelmap = cjyx.vtkOrientedImageData()
            ...
            fillValue = 1.0
            cjyx.vtkOrientedImageDataResample.ApplyImageMask(smoothedClippedSelectedSegmentLabelmap, maskImage, fillValue, False)
            # set original segment labelmap outside painted region, solid 1 inside painted region
            cjyx.vtkOrientedImageDataResample.ModifyImage(maskImage, selectedSegmentLabelmap,
                                                            cjyx.vtkOrientedImageDataResample.OPERATION_MAXIMUM)
            cjyx.vtkOrientedImageDataResample.ModifyImage(maskImage, smoothedClippedSelectedSegmentLabelmap,
                                                            cjyx.vtkOrientedImageDataResample.OPERATION_MINIMUM)
            ...
            self.scriptedEffect.modifySelectedSegmentByLabelmap(maskImage,
                                                                cjyx.qCjyxSegmentEditorAbstractEffect.ModificationModeSet,
                                                                updateExtent)
        else:
            modifierLabelmap.DeepCopy(smoothedImage)
            self.scriptedEffect.modifySelectedSegmentByLabelmap(modifierLabelmap, cjyx.qCjyxSegmentEditorAbstractEffect.ModificationModeSet)
    ...
    def smoothMultipleSegments(self, maskImage=None, maskExtent=None):
        ...
        mergedImage = cjyx.vtkOrientedImageData()
        ...
        self.scriptedEffect.parameterSetNode().SetOverwriteMode(cjyx.vtkDMMLSegmentEditorNode.OverwriteVisibleSegments)
        for segmentId, labelValue in segmentLabelValues:
            threshold.ThresholdBetween(labelValue, labelValue)
            stencil.Update()
            smoothedBinaryLabelMap = cjyx.vtkOrientedImageData()
            ...
            self.scriptedEffect.modifySegmentByLabelmap(segmentationNode, segmentId, smoothedBinaryLabelMap,
                                                        cjyx.qCjyxSegmentEditorAbstractEffect.ModificationModeSet, False)
    ...
    def paintApply(self, viewWidget):
        ...
        if smoothingMethod == JOINT_TAUBIN:
            ...
            cjyx.util.messageBox("Smoothing brush is not available for 'joint smoothing' method.")
            return

        modifierLabelmap = self.scriptedEffect.defaultModifierLabelmap()
        maskImage = cjyx.vtkOrientedImageData()
