...
class SegmentEditorMaskVolumeEffect(AbstractScriptedSegmentEditorEffect):
    ...
    def helpText(self):
        return """<html>Use the currently selected segment as a mask to blank out regions in a volume.<br> The mask is applied to the master volume by default.<p>
Fill inside and outside operation creates a binary labelmap volume as output, with the inside and outside fill values modifiable.
    ...
    def setupOptionsFrame(self):
        ...
        self.updatingGUIFromDMML = False
        self.visibleIcon = qt.QIcon(":/Icons/Small/CjyxVisible.png")
        self.invisibleIcon = qt.QIcon(":/Icons/Small/CjyxInvisible.png")
        ...
        self.inputVolumeSelector = cjyx.qDMMLNodeComboBox()
        self.inputVolumeSelector.nodeTypes = ["vtkDMMLScalarVolumeNode"]
        ...
        self.inputVolumeSelector.setDMMLScene(cjyx.dmmlScene)
        self.inputVolumeSelector.setToolTip("Volume to mask. Default is current master volume node.")
        self.inputVolumeSelector.connect("currentNodeChanged(vtkDMMLNode*)", self.onInputVolumeChanged)
        ...
        self.outputVolumeSelector = cjyx.qDMMLNodeComboBox()
        self.outputVolumeSelector.nodeTypes = ["vtkDMMLScalarVolumeNode", "vtkDMMLLabelMapVolumeNode"]
        ...
        self.outputVolumeSelector.setDMMLScene(cjyx.dmmlScene)
        self.outputVolumeSelector.setToolTip("Masked output volume. It may be the same as the input volume for cumulative masking.")
        self.outputVolumeSelector.connect("currentNodeChanged(vtkDMMLNode*)", self.onOutputVolumeChanged)
    ...
    def createCursor(self, widget):
        ...
        return cjyx.util.mainWindow().cursor
    ...
    def isVolumeVisible(self, volumeNode):
        ...
        for sliceViewName in sliceViewNames:
            ...
            if volumeNodeID == sliceWidget.dmmlSliceCompositeNode().GetBackgroundVolumeID():
                return True
        return False

    def updateGUIFromDMML(self):
        self.updatingGUIFromDMML = True
        ...
        if operationName in ["FILL_INSIDE", "FILL_OUTSIDE"]:
            ...
            if self.outputVolumeSelector.noneDisplay != "(Create new Labelmap Volume)":
                ...
                self.outputVolumeSelector.nodeTypes = ["vtkDMMLLabelMapVolumeNode", "vtkDMMLScalarVolumeNode"]
    ...
    def deactivate(self):
        ...
        cjyx.util.setSliceViewerLayers(background=self.scriptedEffect.parameterSetNode().GetMasterVolumeNode())
    ...
    def onInputVisibilityButtonClicked(self):
        ...
        if inputVolume is None:
            inputVolume = masterVolume
        if inputVolume:
            cjyx.util.setSliceViewerLayers(background=inputVolume)
    ...
    def onInputVolumeChanged(self):
        self.scriptedEffect.parameterSetNode().SetNodeReferenceID("Mask volume.InputVolume", self.inputVolumeSelector.currentNodeID)
        self.updateGUIFromDMML()  # node reference changes are not observed, update GUI manually
    ...
    def maskVolumeWithSegment(segmentationNode, segmentID, operationMode, fillValues, inputVolumeNode, outputVolumeNode, maskExtent=None):
        ...
        maskVolumeNode = cjyx.modules.volumes.logic().CreateAndAddLabelVolume(inputVolumeNode, "TemporaryVolumeMask")
        if not maskVolumeNode:
            logging.error("maskVolumeWithSegment failed: invalid maskVolumeNode")
            return False

        if not cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentsToLabelmapNode(segmentationNode, segmentIDs, maskVolumeNode, inputVolumeNode):
            logging.error("maskVolumeWithSegment failed: ExportSegmentsToLabelmapNode error")
            cjyx.dmmlScene.RemoveNode(maskVolumeNode.GetDisplayNode().GetColorNode())
            cjyx.dmmlScene.RemoveNode(maskVolumeNode.GetDisplayNode())
            cjyx.dmmlScene.RemoveNode(maskVolumeNode)
            return False

        if maskExtent:
            img = cjyx.modules.segmentations.logic().CreateOrientedImageDataFromVolumeNode(maskVolumeNode)
            img.UnRegister(None)
