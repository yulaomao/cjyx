...
about each operation, hover the mouse over the option and wait for the tooltip to appear.</html>"""
    ...
    def setupOptionsFrame(self):
        ...
        self.minimumSizeSpinBox.connect('valueChanged(int)', self.updateDMMLFromGUI)
    ...
    def splitSegments(self, minimumSize=0, maxNumberOfSegments=0, split=True):
        ...
        islandImage = cjyx.vtkOrientedImageData()
        ...
        with cjyx.util.NodeModify(segmentationNode):
            ...
            cjyx.vtkCjyxSegmentationsModuleLogic.GetAllLabelValues(labelValues, islandImage)
            ...
            threshold.Update()
            emptyLabelmap = cjyx.vtkOrientedImageData()
            ...
            for i in range(labelValues.GetNumberOfTuples()):
                ...
                if i != 0 and split:
                    ...
                    segment.AddRepresentation(cjyx.vtkSegmentationConverter.GetSegmentationBinaryLabelmapRepresentationName(),
                                              selectedSegment.GetRepresentation(cjyx.vtkSegmentationConverter.GetSegmentationBinaryLabelmapRepresentationName()))
                ...
                modificationMode = cjyx.qCjyxSegmentEditorAbstractEffect.ModificationModeAdd
                if i == 0:
                    modificationMode = cjyx.qCjyxSegmentEditorAbstractEffect.ModificationModeSet

                # Create oriented image data from output
                modifierImage = cjyx.vtkOrientedImageData()
                ...
                # We could use a single cjyx.vtkCjyxSegmentationsModuleLogic.ImportLabelmapToSegmentationNode
    ...
    def processInteractionEvents(self, callerInteractor, eventId, viewWidget):
        ...
        if viewWidget.className() != "qDMMLSliceWidget":
            return abortEvent
        ...
        if operationName == ADD_SELECTED_ISLAND:
            inputLabelImage = cjyx.vtkOrientedImageData()
        ...
        try:
            ...
            if operationName == ADD_SELECTED_ISLAND:
                ...
                self.scriptedEffect.modifySelectedSegmentByLabelmap(modifierLabelmap, cjyx.qCjyxSegmentEditorAbstractEffect.ModificationModeAdd)
                ...
                if operationName == KEEP_SELECTED_ISLAND:
                    ...
                    self.scriptedEffect.modifySelectedSegmentByLabelmap(modifierLabelmap, cjyx.qCjyxSegmentEditorAbstractEffect.ModificationModeRemove)
    ...
    def updateGUIFromDMML(self):
        ...
        # qCjyxSegmentEditorAbstractEffect should be improved so that it triggers a cursor update
