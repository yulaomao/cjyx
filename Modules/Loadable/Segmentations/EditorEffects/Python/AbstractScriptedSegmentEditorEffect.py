...
class AbstractScriptedSegmentEditorEffect:
    """ Abstract scripted segment editor effects for effects implemented in python
        ...
        1. Instantiation and registration
          ...
          > scriptedEffect = effects.qCjyxSegmentEditorScriptedEffect(None)
          > scriptedEffect.setPythonSource(MyEffect.filePath)
          > scriptedEffect.self().register()
          If effect name is added to cjyx.modules.segmenteditorscriptedeffectnames
        ...
        2.b. Always call API functions (the ones that are defined in the adaptor
          class qCjyxSegmentEditorScriptedEffect) using the adaptor accessor:
          > self.scriptedEffect.updateGUIFromDMML()
    ...
    def register(self):
        effectFactorySingleton = cjyx.qCjyxSegmentEditorEffectFactory.instance()
    ...
    def setWidgetMinMaxStepFromImageSpacing(self, spinbox, imageData):
        ...
        if not imageData:
            return
        import math
        spinbox.unitAwareProperties &= ~(cjyx.qDMMLSpinBox.MinimumValue | cjyx.qDMMLSpinBox.MaximumValue | cjyx.qDMMLSpinBox.Precision)
