...
#include "qCjyxSegmentEditorPaintEffect_p.h"
...
//-----------------------------------------------------------------------------
// qCjyxSegmentEditorPaintEffectPrivate methods

//-----------------------------------------------------------------------------
qCjyxSegmentEditorPaintEffectPrivate::qCjyxSegmentEditorPaintEffectPrivate(qCjyxSegmentEditorPaintEffect& object)
  : q_ptr(&object)
...
//-----------------------------------------------------------------------------
qCjyxSegmentEditorPaintEffectPrivate::~qCjyxSegmentEditorPaintEffectPrivate()
{
  this->clearBrushPipelines();
}

//-----------------------------------------------------------------------------
BrushPipeline* qCjyxSegmentEditorPaintEffectPrivate::brushForWidget(qDMMLWidget* viewWidget)
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::paintAddPoint(qDMMLWidget* viewWidget, double brushPosition_World[3],
  double* lastBrushPosition_World/*=nullptr*/)
{
  ...
  if (q->integerParameter("BrushPixelMode") || !this->DelayedPaint)
    ...
    qCjyxSegmentEditorAbstractEffect::forceRender(viewWidget); // TODO: repaint all?
    }
}

//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::updateBrushStencil(qDMMLWidget* viewWidget)
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
  Q_UNUSED(viewWidget);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::transformPointsFromWorldToIJK(vtkOrientedImageData* image,
  vtkDMMLSegmentationNode* segmentationNode, vtkPoints* rasPoints, vtkPoints* ijkPoints)
{
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::paintPixel(vtkOrientedImageData* modifierLabelmap, qDMMLWidget* viewWidget, double pixelPosition_World[3])
{
  Q_UNUSED(viewWidget);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::paintPixels(
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::paintBrushes(
  vtkOrientedImageData* modifierLabelmap,
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::scaleDiameter(double scaleFactor)
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::onDiameterUnitsClicked()
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::onQuickDiameterButtonClicked()
{
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::onDiameterValueChanged(double value)
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
  if (q->integerParameter("BrushDiameterIsRelative") == 0)
    {
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::updateAbsoluteBrushDiameter()
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
  if (!q->integerParameter("BrushDiameterIsRelative"))
    {
  ...
  qDMMLSliceWidget* sliceWidget = qobject_cast<qDMMLSliceWidget*>(this->ActiveViewWidget);
  qDMMLThreeDWidget* threeDWidget = qobject_cast<qDMMLThreeDWidget*>(this->ActiveViewWidget);
  if (sliceWidget)
    {
  ...
  if (this->ActiveViewWidget)
    {
    qCjyxSegmentEditorAbstractEffect::scheduleRender(this->ActiveViewWidget);
    }
}

//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::updateBrushModel(qDMMLWidget* viewWidget, double brushPosition_World[3])
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
  ...
  if (!sliceWidget || q->integerParameter("BrushSphere"))
    ...
    double sliceSpacingMm = qCjyxSegmentEditorAbstractEffect::sliceSpacing(sliceWidget);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::updateBrush(qDMMLWidget* viewWidget, BrushPipeline* pipeline)
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::updateBrushes()
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
  // unusedWidgetPipelines will contain those widget pointers that are not in the layout anymore
  QList<qDMMLWidget*> unusedWidgetPipelines = this->BrushPipelines.keys();
  ...
  foreach(QString sliceViewName, layoutManager->sliceViewNames())
    ...
    if (!q->segmentationDisplayableInView(sliceWidget->dmmlSliceNode()))
      {
      continue;
      }
    ...
    qCjyxSegmentEditorAbstractEffect::scheduleRender(sliceWidget);
    }
  for (int threeDViewId = 0; threeDViewId < layoutManager->threeDViewCount(); ++threeDViewId)
    ...
    if (!q->segmentationDisplayableInView(threeDWidget->dmmlViewNode()))
      {
      continue;
      }
    ...
    qCjyxSegmentEditorAbstractEffect::scheduleRender(threeDWidget);
    }

  foreach (qDMMLWidget* viewWidget, unusedWidgetPipelines)
    {
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffectPrivate::clearBrushPipelines()
{
  Q_Q(qCjyxSegmentEditorPaintEffect);

  // On exiting Cjyx, this method (triggered by deactivation of the effect when exiting the
  // module containing the segment editor widget) may be called after the layout was destroyed
  bool removeActors = true;
  if (!qCjyxApplication::application() || !qCjyxApplication::application()->layoutManager())
    {
    removeActors = false;
    }

  QMapIterator<qDMMLWidget*, BrushPipeline*> it(this->BrushPipelines);
  while (it.hasNext())
    {
    it.next();
...
//-----------------------------------------------------------------------------
std::string qCjyxSegmentEditorPaintEffectPrivate::segmentAtPosition(qDMMLWidget* viewWidget, double* ras)
{
  Q_Q(qCjyxSegmentEditorPaintEffect);
  if (!q->parameterSetNode())
    {
  ...
  vtkDMMLSegmentationsDisplayableManager2D* segmentationDisplayableManager2D = vtkDMMLSegmentationsDisplayableManager2D::SafeDownCast(
    sliceWidget->sliceView()->displayableManagerByClassName("vtkDMMLSegmentationsDisplayableManager2D"));
...
//-----------------------------------------------------------------------------
// qCjyxSegmentEditorPaintEffect methods

//----------------------------------------------------------------------------
qCjyxSegmentEditorPaintEffect::qCjyxSegmentEditorPaintEffect(QObject* parent)
 : Superclass(parent)
 , d_ptr( new qCjyxSegmentEditorPaintEffectPrivate(*this) )
{
...
//----------------------------------------------------------------------------
qCjyxSegmentEditorPaintEffect::~qCjyxSegmentEditorPaintEffect() = default;

//---------------------------------------------------------------------------
QIcon qCjyxSegmentEditorPaintEffect::icon()
{
  Q_D(qCjyxSegmentEditorPaintEffect);

  return d->PaintIcon;
}

//---------------------------------------------------------------------------
QString const qCjyxSegmentEditorPaintEffect::helpText()const
{
...
//-----------------------------------------------------------------------------
qCjyxSegmentEditorAbstractEffect* qCjyxSegmentEditorPaintEffect::clone()
{
  return new qCjyxSegmentEditorPaintEffect();
}

//---------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::deactivate()
{
  Q_D(qCjyxSegmentEditorPaintEffect);
...
//---------------------------------------------------------------------------
bool qCjyxSegmentEditorPaintEffectPrivate::brushPositionInWorld(qDMMLWidget* viewWidget, int brushPositionInView[2], double brushPosition_World[3])
{
...
//---------------------------------------------------------------------------
bool qCjyxSegmentEditorPaintEffect::processInteractionEvents(
  ...
  qDMMLWidget* viewWidget )
{
  ...
  if (worldPositionValid && eid == vtkCommand::LeftButtonPressEvent && !anyModifierKeyPressed)
    ...
    QList<qDMMLWidget*> viewWidgets = d->BrushPipelines.keys();
    if (!this->parameterSetNode())
      {
      return false;
      }
    foreach (qDMMLWidget* viewWidget, viewWidgets)
      {
    ...
    if (d->IsPainting)
      ...
      QList<qDMMLWidget*> viewWidgets = d->BrushPipelines.keys();
      foreach(qDMMLWidget* aViewWidget, viewWidgets)
        {
        ...
        qCjyxSegmentEditorAbstractEffect::scheduleRender(aViewWidget);
        }
    ...
    if (worldPositionValid)
      ...
      if (d->IsPainting)
        ...
        if (distance2fromLastInteractionPosition >= d->MinimumPaintPointDistance2)
          ...
          qCjyxSegmentEditorAbstractEffect::forceRender(viewWidget);
          }
        }
      }
    else
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::processViewNodeEvents(
    vtkDMMLAbstractViewNode* callerViewNode,
    unsigned long eid,
    qDMMLWidget* viewWidget)
{
  Q_D(qCjyxSegmentEditorPaintEffect);
  ...
  if (viewWidget == d->ActiveViewWidget)
    ...
    d->updateBrushes();
    qCjyxSegmentEditorAbstractEffect::scheduleRender(d->ActiveViewWidget);
    }
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::setupOptionsFrame()
{
  Q_D(qCjyxSegmentEditorPaintEffect);
  ...
  d->BrushDiameterSpinBox = new qDMMLSpinBox(d->BrushDiameterFrame);
  ...
  QObject::connect(d->BrushSphereCheckbox, SIGNAL(clicked()), this, SLOT(updateDMMLFromGUI()));
  QObject::connect(d->EditIn3DViewsCheckbox, SIGNAL(clicked()), this, SLOT(updateDMMLFromGUI()));
  QObject::connect(d->ColorSmudgeCheckbox, SIGNAL(clicked()), this, SLOT(updateDMMLFromGUI()));
  QObject::connect(d->EraseAllSegmentsCheckbox, SIGNAL(clicked()), this, SLOT(updateDMMLFromGUI()));
  QObject::connect(d->BrushPixelModeCheckbox, SIGNAL(clicked()), this, SLOT(updateDMMLFromGUI()));
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::setDMMLDefaults()
{
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::updateGUIFromDMML()
{
  Q_D(qCjyxSegmentEditorPaintEffect);
  if (!this->active())
    {
  ...
  d->BrushDiameterSpinBox->setDMMLScene(this->scene());
  ...
  if (brushDiameterIsRelative)
    ...
    d->BrushDiameterSpinBox->setUnitAwareProperties(qDMMLSpinBox::Prefix | qDMMLSpinBox::Suffix);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::updateDMMLFromGUI()
{
  Q_D(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::referenceGeometryChanged()
{
...
//-----------------------------------------------------------------------------
double qCjyxSegmentEditorPaintEffect::minimumPaintPointDistance()
{
  Q_D(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
bool qCjyxSegmentEditorPaintEffect::delayedPaint()
{
  Q_D(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::setMinimumPaintPointDistance(double dist)
{
  Q_D(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::setDelayedPaint(bool delayed)
{
  Q_D(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::paintApply(qDMMLWidget* viewWidget)
{
  Q_D(qCjyxSegmentEditorPaintEffect);
  ...
  qCjyxSegmentEditorAbstractEffect::ModificationMode modificationMode;
  if (this->m_AlwaysErase)
    {
    modificationMode = this->integerParameter("EraseAllSegments") ?
      qCjyxSegmentEditorAbstractEffect::ModificationModeRemoveAll : qCjyxSegmentEditorAbstractEffect::ModificationModeRemove;
    }
  else
    {
    modificationMode = this->m_Erase ?
      qCjyxSegmentEditorAbstractEffect::ModificationModeRemove : qCjyxSegmentEditorAbstractEffect::ModificationModeAdd;
    }
...
//-----------------------------------------------------------------------------
QList<int> qCjyxSegmentEditorPaintEffect::paintBrushesIntoLabelmap(vtkOrientedImageData* labelmap, qDMMLWidget* viewWidget)
{
  Q_D(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::clearBrushes()
{
  Q_D(qCjyxSegmentEditorPaintEffect);
  // Rendering the feedback actor with no points will result in an error message that will clutter the log.
  // "No input data"
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorPaintEffect::setColorSmudgeCheckboxVisible(bool show)
{
  Q_D(qCjyxSegmentEditorPaintEffect);
...
//-----------------------------------------------------------------------------
QFrame* qCjyxSegmentEditorPaintEffect::paintOptionsFrame()
{
  Q_D(qCjyxSegmentEditorPaintEffect);
