...
#ifndef __qCjyxSegmentEditorAbstractEffect_h
#define __qCjyxSegmentEditorAbstractEffect_h
...
class qCjyxSegmentEditorAbstractEffectPrivate;

class vtkActor2D;
class vtkDMMLInteractionNode;
class vtkDMMLScene;
class vtkDMMLSegmentEditorNode;
...
class Q_CJYX_SEGMENTATIONS_EFFECTS_EXPORT qCjyxSegmentEditorAbstractEffect : public QObject
{
public:
  Q_OBJECT
...
public:
  ...
  qCjyxSegmentEditorAbstractEffect(QObject* parent = nullptr);
  ~qCjyxSegmentEditorAbstractEffect() override;
...
public:
  ...
  virtual qCjyxSegmentEditorAbstractEffect* clone() = 0;
  ...
  Q_INVOKABLE virtual void modifySegmentByLabelmap(vtkDMMLSegmentationNode* segmentationNode, const char* segmentID,
  ...
  virtual QCursor createCursor(qDMMLWidget* viewWidget);
  ...
  virtual bool processInteractionEvents(vtkRenderWindowInteractor* callerInteractor, unsigned long eid, qDMMLWidget* viewWidget)
  ...
  /// \param eid Event identifier
  /// \param viewWidget Widget of the Cjyx layout view. Can be \sa qDMMLSliceWidget or \sa qDMMLThreeDWidget
  virtual void processViewNodeEvents(vtkDMMLAbstractViewNode* callerViewNode, unsigned long eid, qDMMLWidget* viewWidget)
  ...
  virtual void setDMMLDefaults() = 0;
  ...
  virtual void interactionNodeModified(vtkDMMLInteractionNode* interactionNode);

public slots:
  /// Update user interface from parameter set node
  /// NOTE: Base class implementation needs to be called with the effect-specific implementation
  virtual void updateGUIFromDMML() = 0;

  /// Update parameter set node from user interface
  /// NOTE: Base class implementation needs to be called with the effect-specific implementation
  virtual void updateDMMLFromGUI() = 0;

// Get/set methods
public:
  /// Get segment editor parameter set node
  Q_INVOKABLE vtkDMMLSegmentEditorNode* parameterSetNode();
  /// Set segment editor parameter set node
  Q_INVOKABLE void setParameterSetNode(vtkDMMLSegmentEditorNode* node);

  /// Get DMML scene (from parameter set node)
  Q_INVOKABLE vtkDMMLScene* scene();
  ...
  Q_INVOKABLE void addActor2D(qDMMLWidget* viewWidget, vtkActor2D* actor);

  /// Remove actor from the renderer of the widget.
  Q_INVOKABLE void removeActor2D(qDMMLWidget* viewWidget, vtkActor2D* actor);

  /// Add actor to the renderer of the view widget. The effect is responsible for
  /// removing the actor when the effect is deactivated.
  Q_INVOKABLE void addActor3D(qDMMLWidget* viewWidget, vtkProp3D* actor);

  /// Remove actor from the renderer of the widget.
  Q_INVOKABLE void removeActor3D(qDMMLWidget* viewWidget, vtkProp3D* actor);
  ...
  Q_INVOKABLE void cursorOff(qDMMLWidget* viewWidget);
  /// Restore saved cursor
  Q_INVOKABLE void cursorOn(qDMMLWidget* viewWidget);
...
public:
  ...
  Q_INVOKABLE static vtkRenderWindow* renderWindow(qDMMLWidget* viewWidget);
  /// Get renderer for view widget
  Q_INVOKABLE static vtkRenderer* renderer(qDMMLWidget* viewWidget);
  /// Get node for view widget
  Q_INVOKABLE static vtkDMMLAbstractViewNode* viewNode(qDMMLWidget* viewWidget);

  /// Convert RAS position to XY in-slice position
  static QPoint rasToXy(double ras[3], qDMMLSliceWidget* sliceWidget);
  /// Convert RAS position to XY in-slice position, python accessor method
  Q_INVOKABLE static QPoint rasToXy(QVector3D ras, qDMMLSliceWidget* sliceWidget);
  ...
  static void xyzToRas(double inputXyz[3], double outputRas[3], qDMMLSliceWidget* sliceWidget);
  /// Convert XYZ slice view position to RAS position, python accessor method
  Q_INVOKABLE static QVector3D xyzToRas(QVector3D inputXyz, qDMMLSliceWidget* sliceWidget);
  /// Convert XY in-slice position to RAS position
  static void xyToRas(QPoint xy, double outputRas[3], qDMMLSliceWidget* sliceWidget);
  /// Convert XY in-slice position to RAS position
  static void xyToRas(double xy[2], double outputRas[3], qDMMLSliceWidget* sliceWidget);
  /// Convert XY in-slice position to RAS position, python accessor method
  Q_INVOKABLE static QVector3D xyToRas(QPoint xy, qDMMLSliceWidget* sliceWidget);
  /// Convert XYZ slice view position to image IJK position, \sa xyzToRas
  static void xyzToIjk(double inputXyz[3], int outputIjk[3], qDMMLSliceWidget* sliceWidget, vtkOrientedImageData* image, vtkDMMLTransformNode* parentTransform = nullptr);
  /// Convert XYZ slice view position to image IJK position, python accessor method, \sa xyzToRas
  Q_INVOKABLE static QVector3D xyzToIjk(QVector3D inputXyz, qDMMLSliceWidget* sliceWidget, vtkOrientedImageData* image, vtkDMMLTransformNode* parentTransform = nullptr);
  /// Convert XY in-slice position to image IJK position
  static void xyToIjk(QPoint xy, int outputIjk[3], qDMMLSliceWidget* sliceWidget, vtkOrientedImageData* image, vtkDMMLTransformNode* parentTransform = nullptr);
  /// Convert XY in-slice position to image IJK position
  static void xyToIjk(double xy[2], int outputIjk[3], qDMMLSliceWidget* sliceWidget, vtkOrientedImageData* image, vtkDMMLTransformNode* parentTransform = nullptr);
  /// Convert XY in-slice position to image IJK position, python accessor method
  Q_INVOKABLE static QVector3D xyToIjk(QPoint xy, qDMMLSliceWidget* sliceWidget, vtkOrientedImageData* image, vtkDMMLTransformNode* parentTransform = nullptr);

  Q_INVOKABLE static void forceRender(qDMMLWidget* viewWidget);
  Q_INVOKABLE static void scheduleRender(qDMMLWidget* viewWidget);

  Q_INVOKABLE static double sliceSpacing(qDMMLSliceWidget* sliceWidget);

  Q_INVOKABLE bool segmentationDisplayableInView(vtkDMMLAbstractViewNode* viewNode);
...
protected:
  QScopedPointer<qCjyxSegmentEditorAbstractEffectPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qCjyxSegmentEditorAbstractEffect);
  Q_DISABLE_COPY(qCjyxSegmentEditorAbstractEffect);
};
