...
//-----------------------------------------------------------------------------
class qCjyxSegmentEditorScissorsEffectPrivate: public QObject
{
  Q_DECLARE_PUBLIC(qCjyxSegmentEditorScissorsEffect);
protected:
  qCjyxSegmentEditorScissorsEffect* const q_ptr;
public:
  qCjyxSegmentEditorScissorsEffectPrivate(qCjyxSegmentEditorScissorsEffect& object);
  ~qCjyxSegmentEditorScissorsEffectPrivate() override;
public:
  enum
    {
    ShapeFreeForm,
  ...
  bool updateBrushModel(qDMMLWidget* viewWidget);
  /// Update image stencil from mesh
  bool updateBrushStencil(qDMMLWidget* viewWidget);
  /// Paint brush into segment
  void paintApply(qDMMLWidget* viewWidget);
...
protected:
  ...
  ScissorsPipeline* scissorsPipelineForWidget(qDMMLWidget* viewWidget);
...
public:
  ...
  QMap<qDMMLWidget*, ScissorsPipeline*> ScissorsPipelines;
  ...
  qDMMLSpinBox* sliceCutDepthSpinBox;
  QCheckBox*    applyToAllVisibleSegmentsCheckBox;
};

//-----------------------------------------------------------------------------
qCjyxSegmentEditorScissorsEffectPrivate::qCjyxSegmentEditorScissorsEffectPrivate(qCjyxSegmentEditorScissorsEffect& object)
  : q_ptr(&object)
{
  this->ScissorsIcon = QIcon(":Icons/Medium/CjyxEditCut.png");
...
//-----------------------------------------------------------------------------
qCjyxSegmentEditorScissorsEffectPrivate::~qCjyxSegmentEditorScissorsEffectPrivate()
{
  this->clearScissorsPipelines();
}

//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffectPrivate::clearScissorsPipelines()
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
  QMapIterator<qDMMLWidget*, ScissorsPipeline*> it(this->ScissorsPipelines);
  while (it.hasNext())
    {
    it.next();
    qDMMLWidget* viewWidget = it.key();
...
//-----------------------------------------------------------------------------
ScissorsPipeline* qCjyxSegmentEditorScissorsEffectPrivate::scissorsPipelineForWidget(qDMMLWidget* viewWidget)
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffectPrivate::createGlyph(ScissorsPipeline* pipeline, const vtkVector2i& eventPosition)
{
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffectPrivate::updateGlyphWithNewPosition(ScissorsPipeline* pipeline, const vtkVector2i& eventPosition, bool finalize)
{
  if (pipeline->IsDragging)
    {
...
//-----------------------------------------------------------------------------
bool qCjyxSegmentEditorScissorsEffectPrivate::updateBrushModel(qDMMLWidget* viewWidget)
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
  ...
  vtkDMMLSegmentationNode* segmentationNode = q->parameterSetNode()->GetSegmentationNode();
  if (!segmentationNode)
    {
  ...
  vtkDMMLTransformNode::GetMatrixTransformBetweenNodes(segmentationNode->GetParentTransformNode(), nullptr, segmentationToWorldMatrix.GetPointer());
  ...
  if (sliceWidget)
    {
    vtkDMMLSliceNode* sliceNode = vtkDMMLSliceNode::SafeDownCast(qCjyxSegmentEditorAbstractEffect::viewNode(sliceWidget));
    if (!sliceNode)
      {
    ...
    {
    vtkRenderer* renderer = qCjyxSegmentEditorAbstractEffect::renderer(threeDWidget);
    if (!renderer)
      {
      qCritical() << Q_FUNC_INFO << ": Failed to get renderer";
      return false;
...
//-----------------------------------------------------------------------------
bool qCjyxSegmentEditorScissorsEffectPrivate::updateBrushStencil(qDMMLWidget* viewWidget)
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
  Q_UNUSED(viewWidget);
  ...
  // We don't support painting in non-linearly transformed node (it could be implemented, but would probably slow down things too much)
  // TODO: show a meaningful error message to the user if attempted
  vtkDMMLTransformNode::GetMatrixTransformBetweenNodes(nullptr, segmentationNode->GetParentTransformNode(), worldToSegmentationTransformMatrix.GetPointer());
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffectPrivate::paintApply(qDMMLWidget* viewWidget)
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
  ...
  qCjyxSegmentEditorAbstractEffect::ModificationMode modificationMode = qCjyxSegmentEditorAbstractEffect::ModificationModeAdd;
...
//-----------------------------------------------------------------------------
int qCjyxSegmentEditorScissorsEffectPrivate::ConvertShapeFromString(const QString& shapeStr)
{
...
//-----------------------------------------------------------------------------
QString qCjyxSegmentEditorScissorsEffectPrivate::ConvertShapeToString(int shape)
{
  switch (shape)
    {
...
//-----------------------------------------------------------------------------
int qCjyxSegmentEditorScissorsEffectPrivate::ConvertOperationFromString(const QString& operationStr)
{
...
//-----------------------------------------------------------------------------
QString qCjyxSegmentEditorScissorsEffectPrivate::ConvertOperationToString(int operation)
{
  switch (operation)
    {
...
//-----------------------------------------------------------------------------
int qCjyxSegmentEditorScissorsEffectPrivate::ConvertSliceCutModeFromString(const QString& sliceCutModeStr)
{
...
//-----------------------------------------------------------------------------
QString qCjyxSegmentEditorScissorsEffectPrivate::ConvertSliceCutModeToString(int operation)
{
  switch (operation)
    {
...
//-----------------------------------------------------------------------------
bool qCjyxSegmentEditorScissorsEffectPrivate::operationInside()
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
...
//-----------------------------------------------------------------------------
bool qCjyxSegmentEditorScissorsEffectPrivate::operationErase()
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
  int operation = this->ConvertOperationFromString(q->parameter("Operation"));
...
//-----------------------------------------------------------------------------
int qCjyxSegmentEditorScissorsEffectPrivate::shapeDrawCentered()
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
...
//-----------------------------------------------------------------------------
int qCjyxSegmentEditorScissorsEffectPrivate::shape()
{
  Q_Q(qCjyxSegmentEditorScissorsEffect);
...
//----------------------------------------------------------------------------
qCjyxSegmentEditorScissorsEffect::qCjyxSegmentEditorScissorsEffect(QObject* parent)
 : Superclass(parent)
 , d_ptr( new qCjyxSegmentEditorScissorsEffectPrivate(*this) )
{
...
//----------------------------------------------------------------------------
qCjyxSegmentEditorScissorsEffect::~qCjyxSegmentEditorScissorsEffect() = default;

//---------------------------------------------------------------------------
QIcon qCjyxSegmentEditorScissorsEffect::icon()
{
  Q_D(qCjyxSegmentEditorScissorsEffect);

  return d->ScissorsIcon;
}

//---------------------------------------------------------------------------
QString const qCjyxSegmentEditorScissorsEffect::helpText()const
{
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::setupOptionsFrame()
{
  Q_D(qCjyxSegmentEditorScissorsEffect);
  ...
  d->operationGroup->addButton(d->eraseInsideRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::OperationEraseInside);
  d->operationGroup->addButton(d->eraseOutsideRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::OperationEraseOutside);
  d->operationGroup->addButton(d->fillInsideRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::OperationFillInside);
  d->operationGroup->addButton(d->fillOutsideRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::OperationFillOutside);
  ...
  d->shapeGroup->addButton(d->freeFormRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::ShapeFreeForm);
  d->shapeGroup->addButton(d->circleRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::ShapeCircle);
  d->shapeGroup->addButton(d->rectangleRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::ShapeRectangle);
  ...
  d->sliceCutDepthSpinBox = new qDMMLSpinBox();
  ...
  d->sliceCutDepthSpinBox->setUnitAwareProperties(qDMMLSpinBox::Prefix | qDMMLSpinBox::Suffix | qDMMLSpinBox::Precision | qDMMLSpinBox::MaximumValue);
  ...
  d->sliceCutModeGroup->addButton(d->unlimitedRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::SliceCutModeUnlimited);
  d->sliceCutModeGroup->addButton(d->positiveRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::SliceCutModePositive);
  d->sliceCutModeGroup->addButton(d->negativeRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::SliceCutModeNegative);
  d->sliceCutModeGroup->addButton(d->symmetricRadioButton, qCjyxSegmentEditorScissorsEffectPrivate::SliceCutModeSymmetric);
  ...
  QObject::connect(d->applyToAllVisibleSegmentsCheckBox, SIGNAL(clicked()), this, SLOT(updateDMMLFromGUI()));
}

//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::setDMMLDefaults()
{
  Q_D(qCjyxSegmentEditorScissorsEffect);
  Superclass::setDMMLDefaults();
  this->setParameterDefault("ApplyToAllVisibleSegments", 0);
  this->setParameterDefault("Operation", d->ConvertOperationToString(qCjyxSegmentEditorScissorsEffectPrivate::OperationEraseInside));
  this->setParameterDefault("Shape", d->ConvertShapeToString(qCjyxSegmentEditorScissorsEffectPrivate::ShapeFreeForm));
  this->setParameterDefault("SliceCutMode", d->ConvertSliceCutModeToString(qCjyxSegmentEditorScissorsEffectPrivate::SliceCutModeUnlimited));
  this->setParameterDefault("SliceCutDepthMm", 0);
  this->setParameterDefault("ShapeDrawCentered", 0);
}

//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::updateGUIFromDMML()
{
  Q_D(qCjyxSegmentEditorScissorsEffect);

  Superclass::updateGUIFromDMML();
  ...
  d->shapeDrawCenteredCheckBox->setEnabled(shapeIndex == qCjyxSegmentEditorScissorsEffectPrivate::ShapeCircle ||
                                            shapeIndex == qCjyxSegmentEditorScissorsEffectPrivate::ShapeRectangle);
  ...
  d->sliceCutDepthSpinBox->setDMMLScene(this->scene());
  d->sliceCutDepthSpinBox->setValue(this->doubleParameter("SliceCutDepthMm"));
  d->sliceCutDepthSpinBox->setEnabled(sliceCutModeIndex == qCjyxSegmentEditorScissorsEffectPrivate::SliceCutModeSymmetric);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::setOperation(int operationIndex)
{
  Q_D(qCjyxSegmentEditorScissorsEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::setShape(int shapeIndex)
{
  Q_D(qCjyxSegmentEditorScissorsEffect);
  QString shape = d->ConvertShapeToString(shapeIndex);
  this->setParameter("Shape", shape);
  d->shapeDrawCenteredCheckBox->setEnabled(
    shapeIndex == qCjyxSegmentEditorScissorsEffectPrivate::ShapeCircle
    || shapeIndex == qCjyxSegmentEditorScissorsEffectPrivate::ShapeRectangle);
}

//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::setShapeDrawCentered(int checkState)
{
  Q_D(qCjyxSegmentEditorScissorsEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::setSliceCutMode(int sliceCutModeIndex)
{
  Q_D(qCjyxSegmentEditorScissorsEffect);
...
//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::onSliceCutDepthChanged(double value)
{
  this->setParameter("SliceCutDepthMm", value);
}

//-----------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::updateDMMLFromGUI()
{
  Q_D(qCjyxSegmentEditorScissorsEffect);

  Superclass::updateDMMLFromGUI();
...
//-----------------------------------------------------------------------------
qCjyxSegmentEditorAbstractEffect* qCjyxSegmentEditorScissorsEffect::clone()
{
  return new qCjyxSegmentEditorScissorsEffect();
}

//---------------------------------------------------------------------------
void qCjyxSegmentEditorScissorsEffect::deactivate()
{
  Q_D(qCjyxSegmentEditorScissorsEffect);
...
//---------------------------------------------------------------------------
bool qCjyxSegmentEditorScissorsEffect::processInteractionEvents(
  vtkRenderWindowInteractor* callerInteractor,
  unsigned long eid,
  qDMMLWidget* viewWidget )
{
  Q_D(qCjyxSegmentEditorScissorsEffect);
  ...
  if (eid == vtkCommand::LeftButtonPressEvent && !anyModifierKeyPressed)
    ...
    if (pipeline->IsDragging)
      ...
      if (lines->GetNumberOfCells() > 0 && modifierLabelmap)
        ...
        qCjyxSegmentEditorAbstractEffect::scheduleRender(viewWidget);
        }
      abortEvent = true;
      }
    }
