...
// .NAME vtkCjyxSegmentationsModuleLogic - Logic class for segmentation handling
// .SECTION Description
// This class manages the logic associated with converting and handling
// segmentation node objects.

#ifndef __vtkCjyxSegmentationsModuleLogic_h
#define __vtkCjyxSegmentationsModuleLogic_h

// Cjyx includes
#include "vtkCjyxModuleLogic.h"
#include "vtkCjyxSegmentationsModuleLogicExport.h"
...
class vtkDMMLSegmentationStorageNode;
...
class VTK_CJYX_SEGMENTATIONS_LOGIC_EXPORT vtkCjyxSegmentationsModuleLogic :
  public vtkCjyxModuleLogic
{
public:
  static vtkCjyxSegmentationsModuleLogic *New();
  vtkTypeMacro(vtkCjyxSegmentationsModuleLogic,vtkCjyxModuleLogic);
  void PrintSelf(ostream& os, vtkIndent indent) override;

  /// Get segmentation node containing a segmentation object. As segmentation objects are out-of-DMML
  /// VTK objects, there is no direct link from it to its parent node, so must be found from the DMML scene.
  ...
  static vtkDMMLSegmentationNode* GetSegmentationNodeForSegmentation(vtkDMMLScene* scene, vtkSegmentation* segmentation);

  /// Get segmentation node containing a given segment. As segments are out-of-DMML
  /// VTK objects, there is no direct link from it to its parent node, so must be found from the DMML scene.
  /// \param scene DMML scene
  ...
  static vtkDMMLSegmentationNode* GetSegmentationNodeForSegment(vtkDMMLScene* scene, vtkSegment* segment, std::string& segmentId);
  ...
  vtkDMMLSegmentationNode* LoadSegmentationFromFile(const char* filename, bool autoOpacities = true, const char* nodeName=nullptr,
    vtkDMMLColorTableNode* colorTableNode=nullptr);

  /// Create labelmap volume DMML node from oriented image data.
  ...
  /// \param labelmapVolumeNode Labelmap volume to be populated with the oriented image data. The volume node must exist and be added to the DMML scene.
  /// \return Success flag
  static bool CreateLabelmapVolumeFromOrientedImageData(vtkOrientedImageData* orientedImageData, vtkDMMLLabelMapVolumeNode* labelmapVolumeNode);

  /// Create volume DMML node from oriented image data. Display node is not created.
  ...
  static bool CopyOrientedImageDataToVolumeNode(vtkOrientedImageData* orientedImageData,
    vtkDMMLVolumeNode* volumeNode, bool shallowCopy = true, bool shiftImageDataExtentToZeroStart = true);
  ...
  static vtkOrientedImageData* CreateOrientedImageDataFromVolumeNode(vtkDMMLScalarVolumeNode* volumeNode, vtkDMMLTransformNode* outputParentTransformNode = nullptr);
  ...
  static int DoesLabelmapContainSingleLabel(vtkDMMLLabelMapVolumeNode* labelmapVolumeNode);
  ...
  /// Create segment from labelmap volume DMML node. The contents are set as binary labelmap representation in the segment.
  ...
  static vtkSegment* CreateSegmentFromLabelmapVolumeNode(vtkDMMLLabelMapVolumeNode* labelmapVolumeNode, vtkDMMLSegmentationNode* segmentationNode=nullptr);

  /// Create segment from model DMML node.
  ...
  /// \return Created segment that then can be added to the segmentation if needed. Need to take ownership of the created
  ///   object! For example using vtkSmartPointer<vtkSegment>::Take
  static vtkSegment* CreateSegmentFromModelNode(vtkDMMLModelNode* modelNode, vtkDMMLSegmentationNode* segmentationNode=nullptr);

  /// Utility function for getting the segmentation node for a segmentation or segment subject hierarchy item.
  static vtkDMMLSegmentationNode* GetSegmentationNodeForSegmentSubjectHierarchyItem(vtkIdType segmentShItemID, vtkDMMLScene* scene);

  /// Utility function for getting the segment object for a segment subject hierarchy item
  static vtkSegment* GetSegmentForSegmentSubjectHierarchyItem(vtkIdType segmentShItemID, vtkDMMLScene* scene);

  /// Export segment to representation DMML node.
  ...
  static bool ExportSegmentToRepresentationNode(vtkSegment* segment, vtkDMMLNode* representationNode);
  ...
  static bool ExportSegmentsToModels(vtkDMMLSegmentationNode* segmentationNode,
  ...
  /// \param segmentationNode Segmentation node from which the the segments are exported
  /// \param folderItemId Subject hierarchy folder item ID to export the segments to
  static bool ExportVisibleSegmentsToModels(vtkDMMLSegmentationNode* segmentationNode, vtkIdType folderItemId);

  /// Export all segments into a folder, a model node from each segment
  /// \param segmentationNode Segmentation node from which the the segments are exported
  /// \param folderItemId Subject hierarchy folder item ID to export the segments to
  static bool ExportAllSegmentsToModels(vtkDMMLSegmentationNode* segmentationNode, vtkIdType folderItemId);
  ...
  static bool ExportSegmentsToLabelmapNode(vtkDMMLSegmentationNode* segmentationNode, const std::vector<std::string>& segmentIDs,
    vtkDMMLLabelMapVolumeNode* labelmapNode, vtkDMMLVolumeNode* referenceVolumeNode = nullptr,
    int extentComputationMode = vtkSegmentation::EXTENT_UNION_OF_EFFECTIVE_SEGMENTS, vtkDMMLColorTableNode* colorTableNode = nullptr);
  ...
  /// \param extentComputationMode If referenceVolumeNode is not specified then labelmap extents will be determined based on this value.
  ///   By default, the minimum necessary size is used. Set value to vtkSegmentation::EXTENT_REFERENCE_GEOMETRY to use reference geometry extent.
  static bool ExportSegmentsToLabelmapNode(vtkDMMLSegmentationNode* segmentationNode, vtkStringArray* segmentIDs,
    vtkDMMLLabelMapVolumeNode* labelmapNode, vtkDMMLVolumeNode* referenceVolumeNode = nullptr,
    int extentComputationMode = vtkSegmentation::EXTENT_UNION_OF_EFFECTIVE_SEGMENTS, vtkDMMLColorTableNode* colorTableNode = nullptr);
  ...
  /// \param extentComputationMode If referenceVolumeNode is not specified then labelmap extents will be determined based on this value.
  ///   By default, the minimum necessary size is used. Set value to vtkSegmentation::EXTENT_REFERENCE_GEOMETRY to use reference geometry extent.
  static bool ExportVisibleSegmentsToLabelmapNode(vtkDMMLSegmentationNode* segmentationNode,
    vtkDMMLLabelMapVolumeNode* labelmapNode, vtkDMMLVolumeNode* referenceVolumeNode = nullptr,
  ...
  ///   By default, the minimum necessary size is used. Set value to vtkSegmentation::EXTENT_REFERENCE_GEOMETRY to use reference geometry extent.
  static bool ExportAllSegmentsToLabelmapNode(vtkDMMLSegmentationNode* segmentationNode, vtkDMMLLabelMapVolumeNode* labelmapNode,
    int extentComputationMode = vtkSegmentation::EXTENT_UNION_OF_EFFECTIVE_SEGMENTS);
  ...
  static bool ImportLabelmapToSegmentationNode(vtkDMMLLabelMapVolumeNode* labelmapNode,
  ...
  static bool ImportLabelmapToSegmentationNode(vtkDMMLLabelMapVolumeNode* labelmapNode,
    vtkDMMLSegmentationNode* segmentationNode, vtkStringArray* updatedSegmentIDs);
  ...
  /// \param insertBeforeSegmentId New segments will be inserted before this segment.
  bool ImportLabelmapToSegmentationNodeWithTerminology(vtkDMMLLabelMapVolumeNode* labelmapNode,
  ...
  static bool ImportModelToSegmentationNode(vtkDMMLModelNode* modelNode, vtkDMMLSegmentationNode* segmentationNode, std::string insertBeforeSegmentId = "");

  /// Import models in a folder into the segmentation as segments.
  static bool ImportModelsToSegmentationNode(
    vtkIdType folderItemId, vtkDMMLSegmentationNode* segmentationNode, std::string insertBeforeSegmentId = "" );
  ...
  static void GetLabelValuesFromColorNode(vtkDMMLSegmentationNode* segmentationNode, vtkDMMLColorTableNode* colorTableNode,
  ...
  /// \return Success flag
  static bool ApplyParentTransformToPolyData(vtkDMMLTransformableNode* transformableNode, vtkPolyData* polyData);
  ...
  /// \return Success flag
  static bool GetSegmentRepresentation(vtkDMMLSegmentationNode* segmentationNode, std::string segmentID, std::string representationName, vtkDataObject* segmentRepresentation, bool applyParentTransform=true);
  ...
  /// \return Success flag
  static bool GetSegmentBinaryLabelmapRepresentation(vtkDMMLSegmentationNode* segmentationNode, std::string segmentID, vtkOrientedImageData* imageData, bool applyParentTransform=true);
  ...
  ///   If on, then the oriented image data is in RAS, otherwise in the segmentation node's coordinate frame. On by default
  /// \return Success flag
  static bool GetSegmentClosedSurfaceRepresentation(vtkDMMLSegmentationNode* segmentationNode,
  ...
  static bool SetBinaryLabelmapToSegment(vtkOrientedImageData* labelmap, vtkDMMLSegmentationNode* segmentationNode, std::string segmentID,
  ...
  /// 3dCjyxLabel terminology type attribute. If the terminology context does not contain that attribute, match cannot be made.
  /// \param terminologyContextName Terminology context the entries of which are mapped to the labels imported from the labelmap node
  bool SetTerminologyToSegmentationFromLabelmapNode(vtkDMMLSegmentationNode* segmentationNode,
  ...
  vtkDMMLSegmentationNode* GetDefaultSegmentationNode();
  ...
  /// If status tag is not specified then vtkCjyxSegmentationsModuleLogic::NotStarted is returned.
  static int GetSegmentStatus(vtkSegment* segment);
  /// Set the value of the status tag for the given segment.
  /// If setting of vtkCjyxSegmentationsModuleLogic::NotStarted is requested and the
  ...
  static bool ClearSegment(vtkDMMLSegmentationNode* segmentationNode, std::string segmentID);
  ...
  static bool GetSharedSegmentIDsInMask(vtkDMMLSegmentationNode* segmentationNode, std::string sharedSegmentID, vtkOrientedImageData* mask, const int extent[6],
  ...
  static bool ReconvertAllRepresentations(vtkDMMLSegmentationNode* segmentationNode,
  ...
  /// \return True if the representation was created, False otherwise
  static void CollapseBinaryLabelmaps(vtkDMMLSegmentationNode* segmentationNode, bool forceToSingleLayer);
  ...
  static void GenerateMergedLabelmapInReferenceGeometry(vtkDMMLSegmentationNode* segmentationNode, vtkDMMLVolumeNode* referenceVolumeNode,
  ...
  static bool IsEffectiveExentOutsideReferenceVolume(vtkDMMLVolumeNode* referenceVolumeNode, vtkDMMLSegmentationNode* segmentationNode,
...
protected:
  ...
  /// The method is in this logic so that it does not cause confusion throughout Cjyx
  /// (there is an implementation already in qCjyxSaveDataDialogPrivate::nodeFileInfo and it would be good to keep a central one)
  static std::string GetSafeFileName(std::string originalName);

protected:
  vtkCjyxSegmentationsModuleLogic();
  ~vtkCjyxSegmentationsModuleLogic() override;

  /// Command handling subject hierarchy UID added events
  vtkCallbackCommand* SubjectHierarchyUIDCallbackCommand;

private:
  vtkCjyxSegmentationsModuleLogic(const vtkCjyxSegmentationsModuleLogic&) = delete;
  void operator=(const vtkCjyxSegmentationsModuleLogic&) = delete;
};
