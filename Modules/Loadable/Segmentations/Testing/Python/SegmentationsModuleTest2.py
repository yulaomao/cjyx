...
Shared segment modification behavior is controlled through qCjyxSegmentEditorAbstractEffect::modifySelectedSegmentByLabelmap().
'''
...
class SegmentationsModuleTest2(unittest.TestCase):
    ...
    def test_SegmentationsModuleTest2(self):
        ...
        self.assertIsNotNone(cjyx.modules.segmenteditor)
    ...
    def TestSection_SetupScene(self):
        self.paintEffect = cjyx.modules.segmenteditor.widgetRepresentation().self().editor.effectByName("Paint")
        self.eraseEffect = cjyx.modules.segmenteditor.widgetRepresentation().self().editor.effectByName("Erase")
        self.islandEffect = cjyx.modules.segmenteditor.widgetRepresentation().self().editor.effectByName("Islands")
        self.thresholdEffect = cjyx.modules.segmenteditor.widgetRepresentation().self().editor.effectByName("Threshold")

        self.segmentEditorNode = cjyx.util.getNode("SegmentEditor")
        self.assertIsNotNone(self.segmentEditorNode)

        self.segmentationNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLSegmentationNode")
        self.assertIsNotNone(self.segmentationNode)
        self.segmentEditorNode.SetAndObserveSegmentationNode(self.segmentationNode)

        self.masterVolumeNode = cjyx.util.loadVolume(self.dataDir + '/TinyPatient_CT.nrrd')
    ...
    def checkSegmentVoxelCount(self, segmentIndex, expectedVoxelCount):
        ...
        labelmap = cjyx.vtkOrientedImageData()
    ...
    def TestSection_MarginEffects(self):
        ...
        cjyx.modules.segmenteditor.widgetRepresentation().self().editor.effectByName("Margin")
        ...
        binaryLabelmapRepresentationName = cjyx.vtkSegmentationConverter.GetBinaryLabelmapRepresentationName()
        dataTypes = [
            vtk.VTK_CHAR,
            vtk.VTK_SIGNED_CHAR,
        ...
        for dataType in dataTypes:
            initialLabelmap = cjyx.vtkOrientedImageData()
        ...
        for dataType in dataTypes:
            segment1Labelmap = cjyx.vtkOrientedImageData()
            ...
            segment2Labelmap = cjyx.vtkOrientedImageData()
    ...
    def runMarginEffect(self, segment1, segment2, dataType, overwriteMode):
        ...
        marginEffect = cjyx.modules.segmenteditor.widgetRepresentation().self().editor.effectByName("Margin")
        ...
        if overwriteMode == cjyx.vtkDMMLSegmentEditorNode.OverwriteAllSegments:
