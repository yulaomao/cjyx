...
class SegmentationsModuleTest1(unittest.TestCase):
    ...
    def test_SegmentationsModuleTest1(self):
        # Check for modules
        self.assertIsNotNone(cjyx.modules.segmentations)
    ...
    def TestSection_SetupPathsAndNames(self):
        ...
        self.segmentationsModuleTestDir = cjyx.app.temporaryPath + '/SegmentationsModuleTest'
    ...
    def TestSection_RetrieveInputData(self):
        try:
            cjyx.util.downloadAndExtractArchive(
    ...
    def TestSection_LoadInputData(self):
        # Load into Cjyx
        cjyx.util.loadVolume(self.dataDir + '/TinyPatient_CT.nrrd')
        cjyx.util.loadNodeFromFile(self.dataDir + '/TinyPatient_Structures.seg.vtm', "SegmentationFile", {})

        # Change master representation to closed surface (so that conversion is possible when adding segment)
        self.inputSegmentationNode = cjyx.util.getNode('vtkDMMLSegmentationNode1')
    ...
    def TestSection_MergeLabelmapWithDifferentGeometries(self):
        ...
        self.secondSegmentationNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLSegmentationNode', 'Second')
    ...
    def TestSection_ImportExportSegment(self):
        ...
        bodyModelNode = cjyx.vtkDMMLModelNode()
        bodyModelNode.SetName('BodyModel')
        cjyx.dmmlScene.AddNode(bodyModelNode)

        bodySegment = self.inputSegmentationNode.GetSegmentation().GetSegment(self.bodySegmentName)
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentToRepresentationNode(bodySegment, bodyModelNode)
        ...
        bodyLabelmapNode = cjyx.vtkDMMLLabelMapVolumeNode()
        bodyLabelmapNode.SetName('BodyLabelmap')
        cjyx.dmmlScene.AddNode(bodyLabelmapNode)
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentToRepresentationNode(bodySegment, bodyLabelmapNode)
        self.assertTrue(result)
        ...
        allSegmentsLabelmapNode = cjyx.vtkDMMLLabelMapVolumeNode()
        allSegmentsLabelmapNode.SetName('AllSegmentsLabelmap')
        cjyx.dmmlScene.AddNode(allSegmentsLabelmapNode)
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ExportAllSegmentsToLabelmapNode(self.inputSegmentationNode, allSegmentsLabelmapNode)
        self.assertTrue(result)
        ...
        modelImportSegmentationNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLSegmentationNode', 'ModelImport')
        modelImportSegmentationNode.GetSegmentation().SetMasterRepresentationName(self.closedSurfaceReprName)
        modelSegment = cjyx.vtkCjyxSegmentationsModuleLogic.CreateSegmentFromModelNode(bodyModelNode)
        ...
        multiLabelImportSegmentationNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLSegmentationNode', 'MultiLabelImport')
        multiLabelImportSegmentationNode.GetSegmentation().SetMasterRepresentationName(self.binaryLabelmapReprName)
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ImportLabelmapToSegmentationNode(allSegmentsLabelmapNode, multiLabelImportSegmentationNode)
        self.assertTrue(result)
        ...
        singleLabelImportSegmentationNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLSegmentationNode', 'SingleLabelImport')
        ...
        nullSegment = cjyx.vtkCjyxSegmentationsModuleLogic.CreateSegmentFromLabelmapVolumeNode(allSegmentsLabelmapNode)
        ...
        labelSegment = cjyx.vtkCjyxSegmentationsModuleLogic.CreateSegmentFromLabelmapVolumeNode(allSegmentsLabelmapNode)
        ...
        bodyModelTransformNode = cjyx.vtkDMMLLinearTransformNode()
        cjyx.dmmlScene.AddNode(bodyModelTransformNode)
        ...
        bodyModelNodeTransformed = cjyx.vtkDMMLModelNode()
        bodyModelNodeTransformed.SetName('BodyModelTransformed')
        cjyx.dmmlScene.AddNode(bodyModelNodeTransformed)
        bodySegment = self.inputSegmentationNode.GetSegmentation().GetSegment(self.bodySegmentName)
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentToRepresentationNode(bodySegment, bodyModelNodeTransformed)
        self.assertTrue(result)
        ...
        bodyLabelmapNodeTransformed = cjyx.vtkDMMLLabelMapVolumeNode()
        bodyLabelmapNodeTransformed.SetName('BodyLabelmapTransformed')
        cjyx.dmmlScene.AddNode(bodyLabelmapNodeTransformed)
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentToRepresentationNode(bodySegment, bodyLabelmapNodeTransformed)
        self.assertTrue(result)
        self.assertIsNotNone(bodyLabelmapNodeTransformed.GetParentTransformNode())

        # Create transform node that will be used to transform the tested nodes
        modelTransformedImportSegmentationTransformNode = cjyx.vtkDMMLLinearTransformNode()
        cjyx.dmmlScene.AddNode(modelTransformedImportSegmentationTransformNode)
        ...
        modelTransformedImportSegmentationNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLSegmentationNode', 'ModelImportTransformed')
        ...
        modelSegmentTranformed = cjyx.vtkCjyxSegmentationsModuleLogic.CreateSegmentFromModelNode(bodyModelNodeTransformed, modelTransformedImportSegmentationNode)
        ...
        cjyx.dmmlScene.RemoveNode(bodyModelNode)
        cjyx.dmmlScene.RemoveNode(bodyLabelmapNode)
        cjyx.dmmlScene.RemoveNode(allSegmentsLabelmapNode)
        cjyx.dmmlScene.RemoveNode(modelImportSegmentationNode)
        cjyx.dmmlScene.RemoveNode(multiLabelImportSegmentationNode)
        cjyx.dmmlScene.RemoveNode(singleLabelImportSegmentationNode)
        cjyx.dmmlScene.RemoveNode(bodyModelTransformNode)
        cjyx.dmmlScene.RemoveNode(bodyModelNodeTransformed)
        cjyx.dmmlScene.RemoveNode(bodyLabelmapNodeTransformed)
        cjyx.dmmlScene.RemoveNode(modelTransformedImportSegmentationNode)
    ...
    def TestSection_ImportExportSegment2(self):
        ...
        bodySegment = self.inputSegmentationNode.GetSegmentation().GetSegment(self.bodySegmentName)
        bodyLabelmapNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLLabelMapVolumeNode', 'BodyLabelmap')
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentToRepresentationNode(bodySegment, bodyLabelmapNode)
        self.assertTrue(result)
        bodyImageData = bodyLabelmapNode.GetImageData()
        self.assertIsNotNone(bodyImageData)
        ...
        tumorLabelmapNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLLabelMapVolumeNode', 'TumorLabelmap')
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentToRepresentationNode(tumorSegment, tumorLabelmapNode)
        self.assertTrue(result)
        ...
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ImportLabelmapToSegmentationNode(bodyLabelmapNode, singleLabelImportSegmentationNode, bodySegmentIDArray)
        ...
        result = cjyx.vtkCjyxSegmentationsModuleLogic.ImportLabelmapToSegmentationNode(tumorLabelmapNode, singleLabelImportSegmentationNode, tumorSegmentIDArray)
        self.assertTrue(result)
        self.assertEqual(singleLabelImportSegmentationNode.GetSegmentation().GetNumberOfSegments(), 2)

        bodyLabelmap = cjyx.vtkOrientedImageData()
        ...
        tumorLabelmap = cjyx.vtkOrientedImageData()
        ...
        # Clean up temporary nodes
        cjyx.dmmlScene.RemoveNode(bodyLabelmapNode)
        cjyx.dmmlScene.RemoveNode(tumorLabelmapNode)
        cjyx.dmmlScene.RemoveNode(singleLabelImportSegmentationNode)
    ...
    def TestSection_SubjectHierarchy(self):
        ...
        # Remove segmentation
        cjyx.dmmlScene.RemoveNode(self.inputSegmentationNode)
