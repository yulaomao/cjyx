...
//----------------------------------------------------------------------------
int vtkCjyxVolumeRenderingLogicAddFromFileTest(int argc, char* argv[])
{
  if (argc != 2)
    {
...
int testAddVolumePropertyFromFile(const std::string& temporaryDirectory)
{
  ...
  vtkNew<vtkDMMLVolumePropertyNode> defaultVolumePropertyNode;
  vtkNew<vtkDMMLVolumePropertyStorageNode> volumePropertyStorageNode;
  ...
  vtkDMMLVolumePropertyNode *vpNode = logic->AddVolumePropertyFromFile(fileName.c_str());
  CHECK_NULL(vpNode);
