...
const char vtkDMMLVolumeRenderingDisplayableManagerTest1EventLog[] =
"# StreamVersion 1\n";

//----------------------------------------------------------------------------
int vtkDMMLVolumeRenderingDisplayableManagerTest1(int argc, char* argv[])
{
  if (argc < 2)
    {
  ...
  vtkNew<vtkDMMLMarkupsROINode> markupsROINode;
  ...
  vtkNew<vtkDMMLVolumeRenderingDisplayableManager> vrDisplayableManager;
  vrDisplayableManager->SetDMMLApplicationLogic(applicationLogic);
  vrDisplayableManager->SetDMMLScene(scene);
  ...
  vtkNew<vtkDMMLScalarVolumeDisplayNode> volumeDisplayNode;
  ...
  vrLogic->SetDefaultRenderingMethod("vtkDMMLGPURayCastVolumeRenderingDisplayNode");
  vrLogic->SetDMMLScene(scene);
  vrLogic->SetModuleShareDirectory(moduleShareDirectory);

  vrLogic->CreateDefaultVolumeRenderingNodes(volumeNode.GetPointer());
  vtkDMMLVolumeRenderingDisplayNode* vrDisplayNode = vrLogic->GetFirstVolumeRenderingDisplayNode(volumeNode.GetPointer());
  ...
  if (!disableReplay)
    ...
    if (record)
      ...
      recorder->SetInputString(vtkDMMLVolumeRenderingDisplayableManagerTest1EventLog);
      recorder->Play();
      }
    }
