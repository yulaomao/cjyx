...
//----------------------------------------------------------------------------
void SetupScene(vtkRenderer* renderer, vtkDMMLScene* scene, vtkDMMLApplicationLogic* applicationLogic, vtkDMMLDisplayableManagerGroup* displayableManagerGroup)
{
...
//----------------------------------------------------------------------------
void SetupVolumeNode(vtkDMMLScene* scene, vtkDMMLScalarVolumeNode* volumeNode)
{
  ...
  vtkNew<vtkDMMLVolumePropertyNode> volumePropertyNode;
  scene->AddNode(volumePropertyNode.GetPointer());

  vtkNew<vtkDMMLCPURayCastVolumeRenderingDisplayNode> vrDisplayNode;
...
void vtkChangeImageCallback::Execute(vtkObject* caller, unsigned long , void* )
{
  vtkDMMLDisplayableManagerGroup* group =
    vtkDMMLDisplayableManagerGroup::SafeDownCast(caller);
  if (group)
    {
    if (this->Interactor)
      {
...
//----------------------------------------------------------------------------
int vtkDMMLVolumeRenderingMultiVolumeTest(int vtkNotUsed(argc),
