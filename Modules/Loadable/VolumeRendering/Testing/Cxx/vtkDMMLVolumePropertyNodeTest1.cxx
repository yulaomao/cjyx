...
//---------------------------------------------------------------------------
int vtkDMMLVolumePropertyNodeTest1(int argc, char *[] )
{
  if (argc < 2)
    {
    std::cout << "Usage: vtkDMMLVolumePropertyNodeTest1 scene_file_path.dmml"
              << std::endl;
    return EXIT_FAILURE;
    }

  vtkNew<vtkDMMLVolumePropertyNode> node1;
  ...
  applicationLogic->SetDMMLScene(scene.GetPointer()); // register custom nodes
  vtkNew<vtkCjyxVolumeRenderingLogic> vrLogic;
  vrLogic->SetDMMLScene(scene.GetPointer()); // register custom nodes
  vtkNew<vtkCjyxAnnotationModuleLogic> annLogic;
  annLogic->SetDMMLScene(scene.GetPointer()); // register custom nodes
  vtkNew<vtkDMMLLayoutLogic> layLogic;
  layLogic->SetDMMLScene(scene.GetPointer()); // register custom nodes
...
{
  vtkNew<vtkDMMLScene> scene;
  vtkNew<vtkDMMLVolumePropertyNode> propertyNode;
  ...
  vtkDMMLVolumePropertyNode* propertyNode2 =
    vtkDMMLVolumePropertyNode::SafeDownCast(
    scene2->GetFirstNodeByClass("vtkDMMLVolumePropertyNode"));
...
int piecewiseFunctionFromString()
{
  ...
  vtkDMMLVolumePropertyNode::GetPiecewiseFunctionFromString(s, function);
