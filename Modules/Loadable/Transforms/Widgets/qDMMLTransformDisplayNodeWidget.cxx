...
#include "ui_qDMMLTransformDisplayNodeWidget.h"
...
class qDMMLTransformDisplayNodeWidgetPrivate
  : public Ui_qDMMLTransformDisplayNodeWidget
{
  Q_DECLARE_PUBLIC(qDMMLTransformDisplayNodeWidget);
protected:
  qDMMLTransformDisplayNodeWidget* const q_ptr;
public:
  qDMMLTransformDisplayNodeWidgetPrivate(qDMMLTransformDisplayNodeWidget& object);
  ~qDMMLTransformDisplayNodeWidgetPrivate();
  void init();

  vtkDMMLTransformDisplayNode* TransformDisplayNode;
  vtkColorTransferFunction* ColorTransferFunction;
};

//-----------------------------------------------------------------------------
// qDMMLTransformDisplayNodeWidgetPrivate methods
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidgetPrivate
::init()
{
  Q_Q(qDMMLTransformDisplayNodeWidget);
  this->setupUi(q);
  ...
  QObject::connect(this->RegionNodeComboBox, SIGNAL(currentNodeChanged(vtkDMMLNode*)), q, SLOT(regionNodeChanged(vtkDMMLNode*)));
  ...
  // Glyph Parameters
  QObject::connect(this->GlyphPointsNodeComboBox, SIGNAL(currentNodeChanged(vtkDMMLNode*)), q, SLOT(glyphPointsNodeChanged(vtkDMMLNode*)));
...
//-----------------------------------------------------------------------------
// qDMMLTransformDisplayNodeWidget methods

//-----------------------------------------------------------------------------
qDMMLTransformDisplayNodeWidget
::qDMMLTransformDisplayNodeWidget(QWidget *newParent) :
    Superclass(newParent)
  , d_ptr(new qDMMLTransformDisplayNodeWidgetPrivate(*this))
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  d->init();
}

//-----------------------------------------------------------------------------
qDMMLTransformDisplayNodeWidget
::~qDMMLTransformDisplayNodeWidget() = default;

//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget
::setDMMLTransformNode(vtkDMMLNode* transformNode)
{
  setDMMLTransformNode(vtkDMMLTransformNode::SafeDownCast(transformNode));
}

//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget
::setDMMLTransformNode(vtkDMMLTransformNode* transformNode)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
...
::updateWidgetFromDisplayNode()
{
  ...
  switch (d->TransformDisplayNode->GetVisualizationMode())
    {
    case vtkDMMLTransformDisplayNode::VIS_MODE_GLYPH: d->GlyphToggle->setChecked(true); break;
    case vtkDMMLTransformDisplayNode::VIS_MODE_GRID: d->GridToggle->setChecked(true); break;
    case vtkDMMLTransformDisplayNode::VIS_MODE_CONTOUR: d->ContourToggle->setChecked(true); break;
    }
  ...
  std::vector<double> levelsInWidget=vtkDMMLTransformDisplayNode::ConvertContourLevelsFromString(d->ContourLevelsMm->text().toUtf8());
  std::vector<double> levelsInDMML;
  d->TransformDisplayNode->GetContourLevelsMm(levelsInDMML);
  if (!vtkDMMLTransformDisplayNode::IsContourLevelEqual(levelsInWidget,levelsInDMML))
    {
  ...
  if (colorTransferFunctionInNode)
    {
    if (!vtkDMMLProceduralColorNode::IsColorMapEqual(d->ColorTransferFunction,colorTransferFunctionInNode))
      {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::updateGlyphSourceOptions(int glyphType)
{
  Q_D(qDMMLTransformDisplayNodeWidget);

  if (glyphType == vtkDMMLTransformDisplayNode::GLYPH_TYPE_ARROW)
    {
  ...
  else if (glyphType == vtkDMMLTransformDisplayNode::GLYPH_TYPE_CONE)
    {
    d->GlyphDiameterMmLabel->setVisible(true);
    d->GlyphDiameterMm->setVisible(true);
  ...
  else if (glyphType == vtkDMMLTransformDisplayNode::GLYPH_TYPE_SPHERE)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::regionNodeChanged(vtkDMMLNode* node)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::glyphPointsNodeChanged(vtkDMMLNode* node)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphSpacingMm(double spacing)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphScalePercent(double scale)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphDisplayRangeMm(double min, double max)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphType(int glyphType)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphTipLengthPercent(double length)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphDiameterMm(double diameterMm)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphShaftDiameterPercent(double diameterPercent)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphResolution(double resolution)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGridScalePercent(double scale)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGridSpacingMm(double spacing)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGridLineDiameterMm(double diameterMm)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGridResolutionMm(double resolutionMm)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGridShowNonWarped(bool show)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setContourLevelsMm(QString values_str)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setContourResolutionMm(double resolutionMm)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setContourOpacityPercent(double opacityPercent)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGlyphVisualizationMode(bool activate)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!activate)
    {
  ...
  d->TransformDisplayNode->SetVisualizationMode(vtkDMMLTransformDisplayNode::VIS_MODE_GLYPH);
}

//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setGridVisualizationMode(bool activate)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!activate)
    {
  ...
  d->TransformDisplayNode->SetVisualizationMode(vtkDMMLTransformDisplayNode::VIS_MODE_GRID);
}

//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setContourVisualizationMode(bool activate)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!activate)
    {
  ...
  d->TransformDisplayNode->SetVisualizationMode(vtkDMMLTransformDisplayNode::VIS_MODE_CONTOUR);
}

//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setEditorVisibility(bool enabled)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setEditorTranslationEnabled(bool enabled)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setEditorRotationEnabled(bool enabled)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setEditorScalingEnabled(bool enabled)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setVisibility(bool visible)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setVisibility2d(bool visible)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setVisibility3d(bool visible)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
//-----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::setColorTableNode(vtkDMMLNode* colorTableNode)
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
// ----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::colorUpdateRange()
{
  Q_D(qDMMLTransformDisplayNodeWidget);
...
// ----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::onColorInteractionEvent()
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
// ----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::onColorModifiedEvent()
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
...
// ----------------------------------------------------------------------------
void qDMMLTransformDisplayNodeWidget::updateEditorBounds()
{
  Q_D(qDMMLTransformDisplayNodeWidget);
  if (!d->TransformDisplayNode)
    {
