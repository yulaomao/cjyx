...
//-----------------------------------------------------------------------------
int vtkCjyxTransformLogicTest3(int argc, char * argv [])
{
  if(argc < 2)
    {
  ...
  // argv1 == cube.vtk
  vtkSmartPointer<vtkDMMLModelNode> cube = LoadModelInScene(argv[1], scene);
  vtkSmartPointer<vtkDMMLModelNode> tranformedCube = LoadModelInScene(argv[2], scene);

  std::vector<vtkDMMLDisplayableNode*> nodes;
  bool success = true;
  ...
  vtkCjyxTransformLogic::GetNodesBounds(nodes, bounds);
