...
vtkStandardNewMacro(vtkCjyxTransformLogic);

//----------------------------------------------------------------------------
vtkCjyxTransformLogic::vtkCjyxTransformLogic() = default;

//----------------------------------------------------------------------------
vtkCjyxTransformLogic::~vtkCjyxTransformLogic() = default;

//-----------------------------------------------------------------------------
bool vtkCjyxTransformLogic::hardenTransform(vtkDMMLTransformableNode* transformableNode)
{
...
//----------------------------------------------------------------------------
vtkDMMLTransformNode* vtkCjyxTransformLogic::AddTransform(const char* filename, vtkDMMLScene *scene)
{
  vtkNew<vtkDMMLTransformStorageNode> storageNode;

  if (scene == nullptr)
  {
    vtkErrorMacro("scene == nullptr in vtkCjyxTransformLogic::AddTransform");
    return nullptr;
  }
  ...
  vtkSmartPointer<vtkDMMLTransformNode> tnode;

  storageNode->SetScene(scene);

  vtkNew<vtkDMMLTransformNode> generalTransform;
  ...
  // vtkDMMLTransformNode (anyway, transform classes are not meaningful when transforms
  ...
  case TRANSFORM_LINEAR:
    tnode = vtkSmartPointer<vtkDMMLTransformNode>::Take(vtkDMMLLinearTransformNode::New());
    storageNode->ReadData(tnode.GetPointer());
    break;
  case TRANSFORM_BSPLINE:
    tnode = vtkSmartPointer<vtkDMMLTransformNode>::Take(vtkDMMLBSplineTransformNode::New());
    storageNode->ReadData(tnode.GetPointer());
    break;
  case TRANSFORM_GRID:
    tnode = vtkSmartPointer<vtkDMMLTransformNode>::Take(vtkDMMLGridTransformNode::New());
    storageNode->ReadData(tnode.GetPointer());
    break;
  default:
...
int vtkCjyxTransformLogic::SaveTransform(const char* vtkNotUsed(filename),
  vtkDMMLTransformNode *vtkNotUsed(transformNode))
{
  return 1;
}

//----------------------------------------------------------------------------
const char* vtkCjyxTransformLogic::GetVisualizationDisplacementMagnitudeScalarName()
{
  return "DisplacementMagnitude";
}

//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetTransformedPointSamples(vtkPointSet* outputPointSet,
  vtkDMMLTransformNode* inputTransformNode, vtkMatrix4x4* gridToRAS, int* gridSize,
  bool transformToWorld /* = true */)
{
  ...
  vtkCjyxTransformLogic::GetTransformedPointSamples(outputPointSet, inputTransformNode, samplePositions_RAS.GetPointer(), transformToWorld);
}
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetTransformedPointSamplesOnSlice(vtkPointSet* outputPointSet, vtkDMMLTransformNode* inputTransformNode,
...
//----------------------------------------------------------------------------
bool vtkCjyxTransformLogic::GetTransformedPointSamplesAsMagnitudeImage(vtkImageData* magnitudeImage,
  vtkDMMLTransformNode* inputTransformNode, vtkMatrix4x4* ijkToRAS, bool transformToWorld /* = true */)
{
  if (!magnitudeImage || !inputTransformNode || !ijkToRAS)
  {
    vtkGenericWarningMacro("vtkCjyxTransformLogic::GetTransformedPointSamplesAsMagnitudeImage failed: invalid input");
    return false;
  }
...
//----------------------------------------------------------------------------
vtkDMMLVolumeNode* vtkCjyxTransformLogic::CreateDisplacementVolumeFromTransform(vtkDMMLTransformNode* inputTransformNode,
  vtkDMMLVolumeNode* referenceVolumeNode, bool magnitude/*=true*/, vtkDMMLVolumeNode* existingOutputVolumeNode /* = nullptr */)
{
  if (inputTransformNode == nullptr)
  {
    vtkErrorMacro("vtkCjyxTransformLogic::CreateDisplacementVolumeFromTransform failed: inputs are invalid");
    return nullptr;
  }
  if ((referenceVolumeNode == nullptr || referenceVolumeNode->GetImageData() == nullptr) && (existingOutputVolumeNode == nullptr))
  {
    vtkErrorMacro("vtkCjyxTransformLogic::ConvertToGridTransform failed: either a valid \
  ...
  if (scene == nullptr)
  {
    vtkErrorMacro("vtkCjyxTransformLogic::CreateDisplacementVolumeFromTransform failed: scene invalid");
    return nullptr;
  }

  // Create/get a volume node
  vtkSmartPointer<vtkDMMLVolumeNode> outputVolumeNode;
  ...
  {
    outputVolumeNode = vtkSmartPointer<vtkDMMLScalarVolumeNode>::New();
  ...
  if (referenceVolumeNode != nullptr)
  {
    ...
    if (vtkDMMLTransformNode::GetMatrixTransformBetweenNodes(referenceVolumeNode->GetParentTransformNode(), nullptr /* world */, rasToWorld.GetPointer()))
    {
      vtkMatrix4x4::Multiply4x4(rasToWorld.GetPointer(), ijkToRas.GetPointer(), ijkToRas.GetPointer());
    }
    else
    {
      vtkErrorMacro("vtkCjyxTransformLogic::CreateDisplacementVolumeFromTransform: non-linearly transformed reference volume is not supported." \
  ...
  if (magnitude)
  {
    vtkCjyxTransformLogic::GetTransformedPointSamplesAsMagnitudeImage(outputVolume, inputTransformNode, ijkToRas.GetPointer());
    outputVolumeNode->SetVoxelVectorType(vtkDMMLVolumeNode::VoxelVectorTypeUndefined);
  }
  else
  {
    vtkCjyxTransformLogic::GetTransformedPointSamplesAsVectorImage(outputVolume, inputTransformNode, ijkToRas.GetPointer());
    // This indicates that the voxel values should be transformed to LPS when written to file
    outputVolumeNode->SetVoxelVectorType(vtkDMMLVolumeNode::VoxelVectorTypeSpatial);
  }
  ...
  if (outputVolumeNode->GetDisplayNode() == nullptr)
  {
    ...
    {
      vtkNew<vtkDMMLVectorVolumeDisplayNode> displayNode;
      scene->AddNode(displayNode.GetPointer());
      displayNode->SetAndObserveColorNodeID("vtkDMMLColorTableNodeFileViridis.txt");
      outputVolumeNode->SetAndObserveNthDisplayNodeID(0, displayNode->GetID());
    }
...
//----------------------------------------------------------------------------
vtkDMMLTransformNode* vtkCjyxTransformLogic::ConvertToGridTransform(vtkDMMLTransformNode* inputTransformNode,
  vtkDMMLVolumeNode* referenceVolumeNode /* = nullptr */, vtkDMMLTransformNode* existingOutputTransformNode /* = nullptr */)
{
  if (inputTransformNode == nullptr)
  {
    vtkErrorMacro("vtkCjyxTransformLogic::ConvertToGridTransform failed: inputTransformNode is invalid");
    return nullptr;
  }
  if ((referenceVolumeNode == nullptr || referenceVolumeNode->GetImageData() == nullptr) && (existingOutputTransformNode == nullptr))
  {
    vtkErrorMacro("vtkCjyxTransformLogic::ConvertToGridTransform failed: either a valid reference volume node" \
  ...
  if (scene == nullptr)
  {
    vtkErrorMacro("vtkCjyxTransformLogic::ConvertToGridTransform failed: scene invalid");
    return nullptr;
  }

  // Create/get a grid transform
  vtkSmartPointer<vtkDMMLTransformNode> outputGridTransformNode;
  ...
  {
    outputGridTransformNode = vtkSmartPointer<vtkDMMLTransformNode>::New();
  ...
  if (referenceVolumeNode != nullptr)
  {
    ...
    {
      vtkErrorMacro("vtkCjyxTransformLogic::ConvertToGridTransform: non-linearly transformed reference volume" \
  ...
  vtkCjyxTransformLogic::GetTransformedPointSamplesAsVectorImage(outputVolume, inputTransformNode, ijkToRas.GetPointer(), transformToWorld);

  return outputGridTransformNode.GetPointer();
}

//----------------------------------------------------------------------------
bool vtkCjyxTransformLogic::GetTransformedPointSamplesAsVectorImage(vtkImageData* vectorImage,
  vtkDMMLTransformNode* inputTransformNode, vtkMatrix4x4* ijkToRAS, bool transformToWorld /* = true */)
{
  if (!vectorImage || !inputTransformNode || !ijkToRAS)
  {
    vtkGenericWarningMacro("vtkCjyxTransformLogic::GetTransformedPointSamplesAsVectorImage failed: invalid input");
    return false;
  }
  vtkNew<vtkGeneralTransform> inputTransform;
  if (transformToWorld)
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetTransformedPointSamplesOnRoi(vtkPointSet* pointSet,
  vtkDMMLTransformNode* inputTransformNode, vtkMatrix4x4* roiToRAS,
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetGlyphVisualization3d(vtkPolyData* output,
  vtkDMMLTransformDisplayNode* displayNode, vtkMatrix4x4* roiToRAS, int* roiSize,
  vtkPoints* samplePositions_RAS)
{
  vtkNew<vtkUnstructuredGrid> pointSet;
  vtkDMMLTransformNode* inputTransformNode = vtkDMMLTransformNode::SafeDownCast(displayNode->GetDisplayableNode());
  if (samplePositions_RAS)
    {
  ...
  switch (displayNode->GetGlyphType())
    {
    //Arrows
    case vtkDMMLTransformDisplayNode::GLYPH_TYPE_ARROW:
      {
    ...
    //Cones
    case vtkDMMLTransformDisplayNode::GLYPH_TYPE_CONE:
      {
    ...
    //Spheres
    case vtkDMMLTransformDisplayNode::GLYPH_TYPE_SPHERE:
      {
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetGlyphVisualization2d(vtkPolyData* output,
  vtkDMMLTransformDisplayNode* displayNode, vtkMatrix4x4* sliceToRAS,
  double* fieldOfViewOrigin, double* fieldOfViewSize, vtkPoints* samplePositions_RAS)
{
  ...
  vtkDMMLTransformNode* inputTransformNode = vtkDMMLTransformNode::SafeDownCast(displayNode->GetDisplayableNode());
  vtkCjyxTransformLogic::GetTransformedPointSamplesOnSlice(pointSet.GetPointer(), inputTransformNode, sliceToRAS,
...
//----------------------------------------------------------------------------
int vtkCjyxTransformLogic::GetGridSubdivision(vtkDMMLTransformDisplayNode* displayNode)
{
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::CreateGrid(vtkPolyData* gridPolyData,
  vtkDMMLTransformDisplayNode* displayNode, int numGridPoints[3], vtkPolyData* warpedGrid/*=nullptr*/)
{
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetGridVisualization2d(vtkPolyData* output,
  vtkDMMLTransformDisplayNode* displayNode, vtkMatrix4x4* sliceToRAS,
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetGridVisualization3d(vtkPolyData* output, vtkDMMLTransformDisplayNode* displayNode, vtkMatrix4x4* roiToRAS, int* roiSize)
{
  double pointSpacing = displayNode->GetGridSpacingMm() / GetGridSubdivision(displayNode);
  int numGridPoints[3] = { 0 };
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetContourVisualization2d(vtkPolyData* output,
  vtkDMMLTransformDisplayNode* displayNode, vtkMatrix4x4* sliceToRAS,
  double* fieldOfViewOrigin, double* fieldOfViewSize)
{
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetContourVisualization3d(vtkPolyData* output, vtkDMMLTransformDisplayNode* displayNode, vtkMatrix4x4* roiToRAS, int* roiSize)
{
...
//----------------------------------------------------------------------------
bool vtkCjyxTransformLogic::GetVisualization2d(vtkPolyData* output_RAS,
  vtkDMMLTransformDisplayNode* displayNode, vtkDMMLSliceNode* sliceNode,
  vtkDMMLMarkupsNode* glyphPointsNode /*=nullptr*/)
{
  ...
  if (glyphPointsNode != nullptr)
  {
    ...
    vtkCjyxTransformLogic::GetMarkupsAsPoints(glyphPointsNode, samplePoints_RAS);
  }
...
//----------------------------------------------------------------------------
bool vtkCjyxTransformLogic::GetVisualization2d(vtkPolyData* output, vtkDMMLTransformDisplayNode* displayNode,
...
//----------------------------------------------------------------------------
bool vtkCjyxTransformLogic::GetVisualization3d(vtkPolyData* output, vtkDMMLTransformDisplayNode* displayNode, vtkDMMLNode* regionNode)
{
  ...
  vtkDMMLSliceNode* sliceNode = vtkDMMLSliceNode::SafeDownCast(regionNode);
  vtkDMMLDisplayableNode* displayableNode = vtkDMMLDisplayableNode::SafeDownCast(regionNode);
  ...
  vtkSmartPointer<vtkPoints> samplePoints_RAS;
  vtkDMMLMarkupsNode* glyphPointsNode = vtkDMMLMarkupsNode::SafeDownCast(regionNode);
  if (glyphPointsNode != nullptr)
    {
    samplePoints_RAS = vtkSmartPointer<vtkPoints>::New();
    vtkCjyxTransformLogic::GetMarkupsAsPoints(glyphPointsNode, samplePoints_RAS);
    }
  vtkCjyxTransformLogic::GetVisualization3d(output, displayNode, ijkToRAS.GetPointer(), regionSize_IJK, samplePoints_RAS);
  return true;
}

//----------------------------------------------------------------------------
bool vtkCjyxTransformLogic::GetVisualization3d(vtkPolyData* output, vtkDMMLTransformDisplayNode* displayNode,
...
//----------------------------------------------------------------------------
vtkCjyxTransformLogic::TransformKind vtkCjyxTransformLogic::GetTransformKind(vtkDMMLTransformNode *transformNode)
{
  ...
  vtkDMMLTransformNode::FlattenGeneralTransform(transformList.GetPointer(), generalTransform);
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetMarkupsAsPoints(vtkDMMLMarkupsNode* markupsNode, vtkPoints* samplePoints_RAS)
{
  if (samplePoints_RAS == nullptr)
    {
    vtkGenericWarningMacro("vtkCjyxTransformLogic::GetMarkupsAsPoints failed: invalid samplePoints_RAS");
    return;
    }
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetTransformedNodes(
  ...
  bool recursive)
{
  ...
  scene->GetNodesByClass("vtkDMMLDisplayableNode", nodes);
  for (size_t i = 0; i < nodes.size(); ++i)
    {
    vtkDMMLDisplayableNode* node =
      vtkDMMLDisplayableNode::SafeDownCast(nodes[i]);
    if (node->GetTransformNodeID() &&
      strcmp(transformNode->GetID(), node->GetTransformNodeID()) == 0)
      {
      transformedNodes.push_back(node);

      vtkDMMLTransformNode* childTransformNode =
        vtkDMMLTransformNode::SafeDownCast(node);
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetNodesRASBounds(
...
//----------------------------------------------------------------------------
void vtkCjyxTransformLogic::GetNodesBounds(
  const std::vector<vtkDMMLDisplayableNode*>& nodes,
  double bounds[6])
{
  vtkBoundingBox box;
