...
class AddStorableDataAfterSceneViewTest(ScriptedLoadableModule):
    ...
    It tests Cjyx's functionality after the scene view is restored, is the new storable
...
class AddStorableDataAfterSceneViewTestLogic(ScriptedLoadableModuleLogic):
    ...
    def run(self, enableScreenshots=0):
        ...
        try:
            ...
            cjyx.util.errorDisplay(errorMessage)
...
class AddStorableDataAfterSceneViewTestTest(ScriptedLoadableModuleTest):
    ...
    def test_AddStorableDataAfterSceneViewTest1(self):
        ...
        cjyx.util.delayDisplay("Starting the test")

        #
        # add a markups control point list
        #

        pointList = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLMarkupsFiducialNode')
        ...
        #
        cjyx.util.delayDisplay("Adding a new storable node, after creating a scene view")
        import SampleData
        ...
        #
        cjyx.util.delayDisplay("Restoring the scene view")
        sv.RestoreScene()

        #
        # Is the new storable data still present?
        #
        restoredData = cjyx.dmmlScene.GetNodeByID(mrHeadID)

        # for now, the non scene view storable data is removed
        self.assertIsNone(restoredData)
        cjyx.util.delayDisplay('Success: extra storable node removed with scene view restore')
        ...
        #
        cjyx.util.delayDisplay("Restoring the scene view with check for removed nodes")
        sv.RestoreScene(0)
        ...
        cjyx.util.delayDisplay('Success: extra storable node NOT removed with scene view restore')

        print('Scene error code = ' + str(cjyx.dmmlScene.GetErrorCode()))
        print('\t' + cjyx.dmmlScene.GetErrorMessage())

        cjyx.util.delayDisplay('Test passed!')
