...
struct vtkDMMLSequenceBrowserNode::SynchronizationProperties
  {
...
void vtkDMMLSequenceBrowserNode::SynchronizationProperties::FromString( std::string str )
{
...
std::string vtkDMMLSequenceBrowserNode::SynchronizationProperties::ToString()
{
  std::stringstream ss;
...
//------------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLSequenceBrowserNode);

//----------------------------------------------------------------------------
vtkDMMLSequenceBrowserNode::vtkDMMLSequenceBrowserNode()
...
//----------------------------------------------------------------------------
vtkDMMLSequenceBrowserNode::~vtkDMMLSequenceBrowserNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::WriteXML(ostream& of, int nIndent)
{
  // TODO: Convert to use DMML node macros
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::ReadXMLAttributes(const char** atts)
{
  // TODO: Convert to use DMML node macros
  ...
  while (*atts != nullptr)
    ...
    if (!strcmp(attName, "playbackActive"))
      ...
      if (recordingSamplingMode<0 || recordingSamplingMode >= vtkDMMLSequenceBrowserNode::NumberOfRecordingSamplingModes)
        {
        vtkErrorMacro("Invalid recording sampling mode: " << (attValue ? attValue : "(empty). Using LimitedToPlaybackFrameRate."));
        recordingSamplingMode = vtkDMMLSequenceBrowserNode::SamplingLimitedToPlaybackFrameRate;
        }
      ...
      if (indexDisplayMode<0 || indexDisplayMode >= vtkDMMLSequenceBrowserNode::NumberOfIndexDisplayModes)
        {
        vtkErrorMacro("Invalid index display mode: " << (attValue ? attValue : "(empty). Using IndexDisplayAsIndexValue."));
        indexDisplayMode = vtkDMMLSequenceBrowserNode::IndexDisplayAsIndexValue;
        }
...
void vtkDMMLSequenceBrowserNode::Copy(vtkDMMLNode *anode)
{
  // TODO: Convert to use DMML node macros
  vtkDMMLSequenceBrowserNode* node = vtkDMMLSequenceBrowserNode::SafeDownCast(anode);
  if (!node)
    {
    vtkErrorMacro("Node copy failed: not a vtkDMMLSequenceNode");
    return;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::PrintSelf(ostream& os, vtkIndent indent)
{
  // TODO: Convert to use DMML node macros
  ...
  if (this->SynchronizationPostfixes.empty())
    ...
    for (std::vector< std::string >::iterator rolePostfixIt = this->SynchronizationPostfixes.begin();
      ...
      vtkDMMLSequenceNode* sequenceNode = vtkDMMLSequenceNode::SafeDownCast(this->GetNodeReference(sequenceNodeReferenceRole.c_str()));
      ...
      vtkDMMLNode* proxyNode = vtkDMMLSequenceNode::SafeDownCast(this->GetNodeReference(proxyNodeReferenceRole.c_str()));
...
//----------------------------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::GenerateSynchronizationPostfix()
{
  while (1)
    {
...
//----------------------------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::SetAndObserveMasterSequenceNodeID(const char *sequenceNodeID)
{
  if (!sequenceNodeID)
    {
  ...
  vtkDMMLSequenceNode* newMasterNode = (this->Scene ? vtkDMMLSequenceNode::SafeDownCast(this->Scene->GetNodeByID(sequenceNodeID)) : nullptr);
...
//----------------------------------------------------------------------------
vtkDMMLSequenceNode* vtkDMMLSequenceBrowserNode::GetMasterSequenceNode()
{
  if (this->SynchronizationPostfixes.empty())
    {
    return nullptr;
  ...
  vtkDMMLSequenceNode* node=vtkDMMLSequenceNode::SafeDownCast(this->GetNodeReference(sequenceNodeReferenceRole.c_str()));
  return node;
}

//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::RemoveAllProxyNodes()
{
  bool oldModify=this->StartModify();
  for (std::vector< std::string >::iterator rolePostfixIt=this->SynchronizationPostfixes.begin();
    rolePostfixIt!=this->SynchronizationPostfixes.end(); ++rolePostfixIt)
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::RemoveAllSequenceNodes()
{
  bool oldModify=this->StartModify();
...
//----------------------------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::GetSynchronizationPostfixFromSequence(vtkDMMLSequenceNode* sequenceNode)
{
  if (sequenceNode==nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetSynchronizationPostfixFromSequence failed: sequenceNode is invalid");
    return "";
    }
  for (std::vector< std::string >::iterator rolePostfixIt=this->SynchronizationPostfixes.begin();
    rolePostfixIt!=this->SynchronizationPostfixes.end(); ++rolePostfixIt)
...
//----------------------------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::GetSynchronizationPostfixFromSequenceID(const char* sequenceNodeID)
{
  if (sequenceNodeID == nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetSynchronizationPostfixFromSequenceID failed: sequenceNodeID is invalid");
    return "";
    }
  for (std::vector< std::string >::iterator rolePostfixIt = this->SynchronizationPostfixes.begin();
    rolePostfixIt != this->SynchronizationPostfixes.end(); ++rolePostfixIt)
...
//----------------------------------------------------------------------------
vtkDMMLNode* vtkDMMLSequenceBrowserNode::GetProxyNode(vtkDMMLSequenceNode* sequenceNode)
{
  if (sequenceNode==nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetVirtualOutputNode failed: sequenceNode is invalid");
    return nullptr;
    }
...
//----------------------------------------------------------------------------
vtkDMMLNode* vtkDMMLSequenceBrowserNode::AddProxyNode(vtkDMMLNode* sourceProxyNode, vtkDMMLSequenceNode* sequenceNode, bool copy /* =true */)
{
  if (sequenceNode==nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::AddProxyNode failed: sequenceNode is invalid");
    return nullptr;
    }
  if (this->Scene==nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::AddProxyNode failed: scene is invalid");
    return nullptr;
    }
  ...
  vtkDMMLNode* proxyNode = sourceProxyNode;
  if ( copy )
    {
  ...
  vtkDMMLNode* oldProxyNode=this->GetNodeReference(proxyNodeRef.c_str());
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::GetAllProxyNodes(std::vector< vtkDMMLNode* >& nodes)
{
  nodes.clear();
  for (std::vector< std::string >::iterator rolePostfixIt=this->SynchronizationPostfixes.begin();
    rolePostfixIt!=this->SynchronizationPostfixes.end(); ++rolePostfixIt)
    ...
    vtkDMMLNode* node = this->GetNodeReference(proxyNodeRef.c_str());
    if (node==nullptr)
      {
      continue;
      }
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::GetAllProxyNodes(vtkCollection* nodes)
{
  if (nodes==nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetAllProxyNodes failed: nodes is invalid");
    return;
    }
  std::vector< vtkDMMLNode* > nodesVector;
  this->GetAllProxyNodes(nodesVector);
  nodes->RemoveAllItems();
  for (std::vector< vtkDMMLNode* >::iterator it = nodesVector.begin(); it != nodesVector.end(); ++it)
    {
    nodes->AddItem(*it);
    }
}
...
//----------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::IsProxyNode(const char* nodeId)
{
  return this->IsProxyNodeID(nodeId);
}

//----------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::IsProxyNodeID(const char* nodeId)
{
  std::vector< vtkDMMLNode* > nodesVector;
  this->GetAllProxyNodes(nodesVector);
  for (std::vector< vtkDMMLNode* >::iterator it = nodesVector.begin(); it != nodesVector.end(); ++it)
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::RemoveProxyNode(const std::string& postfix)
{
  bool oldModify=this->StartModify();
  std::string proxyNodeRef=PROXY_NODE_REFERENCE_ROLE_BASE+postfix;
  vtkDMMLNode* proxyNode=this->GetNodeReference(proxyNodeRef.c_str());
  if (proxyNode!=nullptr)
    {
...
//----------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::IsSynchronizedSequenceNode(const char* nodeId, bool includeMasterNode/*=false*/)
{
  return this->IsSynchronizedSequenceNodeID(nodeId, includeMasterNode);
}

//----------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::IsSynchronizedSequenceNode(vtkDMMLSequenceNode* sequenceNode, bool includeMasterNode/*=false*/)
{
  if (sequenceNode == nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::IsSynchronizedSequenceNode failed: sequenceNode is invalid");
    return false;
    }
...
//----------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::IsSynchronizedSequenceNodeID(const char* nodeId, bool includeMasterNode/*=false*/)
{
  if (nodeId==nullptr)
    {
    vtkWarningMacro("vtkDMMLSequenceBrowserNode::IsSynchronizedSequenceNode nodeId is NULL");
    return false;
    }
  for (std::vector< std::string >::iterator rolePostfixIt=this->SynchronizationPostfixes.begin();
    rolePostfixIt!=this->SynchronizationPostfixes.end(); ++rolePostfixIt)
...
//----------------------------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::AddSynchronizedSequenceNode(vtkDMMLSequenceNode* sequenceNode)
{
  if (sequenceNode == nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::AddSynchronizedSequenceNode failed: sequenceNode is invalid");
    return "";
    }
...
//----------------------------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::AddSynchronizedSequenceNode(const char* synchronizedSequenceNodeId)
{
  return this->AddSynchronizedSequenceNodeID(synchronizedSequenceNodeId);
}

//----------------------------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::AddSynchronizedSequenceNodeID(const char* synchronizedSequenceNodeId)
{
  bool oldModify = this->StartModify();
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::RemoveSynchronizedSequenceNode(const char* nodeId)
{
  if (this->Scene==nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::RemoveSynchronizedSequenceNode failed: scene is invalid");
    return;
    }
  if (nodeId==nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::RemoveSynchronizedSequenceNode failed: nodeId is invalid");
    return;
    }
  ...
  vtkWarningMacro("vtkDMMLSequenceBrowserNode::RemoveSynchronizedSequenceNode did nothing, the specified node was not synchronized");
}

//----------------------------------------------------------------------------
int vtkDMMLSequenceBrowserNode::GetNumberOfSynchronizedSequenceNodes(bool includeMasterNode/*=false*/)
{
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::GetSynchronizedSequenceNodes(std::vector< vtkDMMLSequenceNode* > &synchronizedSequenceNodes, bool includeMasterNode/*=false*/)
{
  ...
  for (std::vector< std::string >::iterator rolePostfixIt=this->SynchronizationPostfixes.begin();
    ...
    std::string sequenceNodeRef=SEQUENCE_NODE_REFERENCE_ROLE_BASE+(*rolePostfixIt);
    vtkDMMLSequenceNode* synchronizedNode=vtkDMMLSequenceNode::SafeDownCast(this->GetNodeReference(sequenceNodeRef.c_str()));
...
//----------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::GetSynchronizedSequenceNodes(vtkCollection* synchronizedSequenceNodes, bool includeMasterNode /* =false */)
{
  if (synchronizedSequenceNodes==nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetSynchronizedSequenceNodes failed: synchronizedSequenceNodes is invalid");
    return;
    }
  std::vector< vtkDMMLSequenceNode* > synchronizedDataNodesVector;
  this->GetSynchronizedSequenceNodes(synchronizedDataNodesVector, includeMasterNode);
  synchronizedSequenceNodes->RemoveAllItems();
  for (std::vector< vtkDMMLSequenceNode* >::iterator it = synchronizedDataNodesVector.begin(); it != synchronizedDataNodesVector.end(); ++it)
    {
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SetRecordingActive(bool recording)
{
  // Before activating the recording, set the initial timestamp to be correct
  this->RecordingTimeOffsetSec = vtkTimerLog::GetUniversalTime();
  int numberOfItems = this->GetNumberOfItems();
  if (numberOfItems>0
    && this->GetMasterSequenceNode()->GetIndexType()==vtkDMMLSequenceNode::NumericIndex)
    {
...
//---------------------------------------------------------------------------
int vtkDMMLSequenceBrowserNode::SelectFirstItem()
{
...
//---------------------------------------------------------------------------
int vtkDMMLSequenceBrowserNode::SelectLastItem()
{
...
//---------------------------------------------------------------------------
int vtkDMMLSequenceBrowserNode::SelectNextItem(int selectionIncrement/*=1*/)
{
  int numberOfItems = this->GetNumberOfItems();
  if (numberOfItems == 0)
    {
...
//---------------------------------------------------------------------------
int vtkDMMLSequenceBrowserNode::GetNumberOfItems()
{
  vtkDMMLSequenceNode* sequenceNode = this->GetMasterSequenceNode();
  if (!sequenceNode)
    {
    return 0;
    }
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::ProcessDMMLEvents( vtkObject *caller, unsigned long event, void *callData )
{
  this->vtkDMMLNode::ProcessDMMLEvents( caller, event, callData );

  vtkDMMLNode* modifiedNode = vtkDMMLNode::SafeDownCast(caller);
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SaveProxyNodesState()
{
  ...
  if (continuousRecording)
    ...
    if (this->RecordingSamplingMode == vtkDMMLSequenceBrowserNode::SamplingLimitedToPlaybackFrameRate)
      {
  ...
  int wasModified = this->StartModify();
  std::vector< vtkDMMLSequenceNode* > sequenceNodes;
  this->GetSynchronizedSequenceNodes(sequenceNodes, true);
  bool snapshotAdded = false;
  for (std::vector< vtkDMMLSequenceNode* >::iterator it = sequenceNodes.begin(); it != sequenceNodes.end(); it++)
    {
    vtkDMMLSequenceNode* currSequenceNode = (*it);
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::OnNodeReferenceAdded(vtkDMMLNodeReference* nodeReference)
{
  vtkDMMLNode::OnNodeReferenceAdded(nodeReference);
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::OnNodeReferenceRemoved(vtkDMMLNodeReference* nodeReference)
{
  vtkDMMLNode::OnNodeReferenceRemoved(nodeReference);
  for (std::vector< std::string >::iterator rolePostfixIt = this->SynchronizationPostfixes.begin();
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::FixSequenceNodeReferenceRoleName()
{
  bool oldModify=this->StartModify();
  for (std::vector< std::string >::iterator rolePostfixIt=this->SynchronizationPostfixes.begin();
    rolePostfixIt!=this->SynchronizationPostfixes.end(); ++rolePostfixIt)
...
//---------------------------------------------------------------------------
vtkDMMLSequenceNode* vtkDMMLSequenceBrowserNode::GetSequenceNode(vtkDMMLNode* proxyNode)
{
  if (proxyNode == nullptr)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetSequenceNode failed: virtualOutputDataNode is invalid");
    return nullptr;
    }
  for (std::vector< std::string >::iterator rolePostfixIt = this->SynchronizationPostfixes.begin();
    rolePostfixIt != this->SynchronizationPostfixes.end(); ++rolePostfixIt)
    {
    std::string proxyNodeRef = PROXY_NODE_REFERENCE_ROLE_BASE + (*rolePostfixIt);
    vtkDMMLNode* foundProxyNode = this->GetNodeReference(proxyNodeRef.c_str());
...
//---------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::IsAnySequenceNodeRecording()
{
  for (std::vector< std::string >::iterator rolePostfixIt = this->SynchronizationPostfixes.begin();
    rolePostfixIt != this->SynchronizationPostfixes.end(); ++rolePostfixIt)
    {
...
//---------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::GetRecording(vtkDMMLSequenceNode* sequenceNode)
{
  SynchronizationProperties* syncProps = this->GetSynchronizationPropertiesForSequence(sequenceNode);
  if (!syncProps)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetRecording failed: sequence node is invalid or does not belong to this browser node");
    return false;
    }
...
//---------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::GetPlayback(vtkDMMLSequenceNode* sequenceNode)
{
  SynchronizationProperties* syncProps = this->GetSynchronizationPropertiesForSequence(sequenceNode);
  if (!syncProps)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetPlayback failed: sequence node is invalid or does not belong to this browser node");
    return false;
    }
...
//---------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::GetOverwriteProxyName(vtkDMMLSequenceNode* sequenceNode)
{
  SynchronizationProperties* syncProps = this->GetSynchronizationPropertiesForSequence(sequenceNode);
  if (!syncProps)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetOverwriteProxyName failed: sequence node is invalid or does not belong to this browser node");
    return false;
    }
...
//---------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::GetSaveChanges(vtkDMMLSequenceNode* sequenceNode)
{
  SynchronizationProperties* syncProps = this->GetSynchronizationPropertiesForSequence(sequenceNode);
  if (!syncProps)
    {
    vtkErrorMacro("vtkDMMLSequenceBrowserNode::GetSaveChanges failed: sequence node is invalid or does not belong to this browser node");
    return false;
    }
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SetRecording(vtkDMMLSequenceNode* sequenceNode, bool recording)
{
  ...
  bool modified = false;
  for (std::vector< vtkDMMLSequenceNode* >::iterator it = synchronizedSequenceNodes.begin(); it != synchronizedSequenceNodes.end(); ++it)
    {
    SynchronizationProperties* syncProps = this->GetSynchronizationPropertiesForSequence(*it);
    if (!syncProps)
      {
      vtkErrorMacro("vtkDMMLSequenceBrowserNode::SetRecording failed: sequence node is invalid or does not belong to this browser node");
      continue;
      }
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SetPlayback(vtkDMMLSequenceNode* sequenceNode, bool playback)
{
  std::vector< vtkDMMLSequenceNode* > synchronizedSequenceNodes;
  if (sequenceNode)
    {
  ...
  for (std::vector< vtkDMMLSequenceNode* >::iterator it = synchronizedSequenceNodes.begin(); it != synchronizedSequenceNodes.end(); ++it)
    ...
    if (!syncProps)
      {
      vtkErrorMacro("vtkDMMLSequenceBrowserNode::SetPlayback failed: sequence node is invalid or does not belong to this browser node");
      continue;
      }
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SetOverwriteProxyName(vtkDMMLSequenceNode* sequenceNode, bool overwrite)
{
  std::vector< vtkDMMLSequenceNode* > synchronizedSequenceNodes;
  if (sequenceNode)
    {
  ...
  for (std::vector< vtkDMMLSequenceNode* >::iterator it = synchronizedSequenceNodes.begin(); it != synchronizedSequenceNodes.end(); ++it)
    ...
    if (!syncProps)
      {
      vtkErrorMacro("vtkDMMLSequenceBrowserNode::SetOverwriteProxyName failed: sequence node is invalid or does not belong to this browser node");
      continue;
      }
...
//---------------------------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SetSaveChanges(vtkDMMLSequenceNode* sequenceNode, bool save)
{
  std::vector< vtkDMMLSequenceNode* > synchronizedSequenceNodes;
  if (sequenceNode)
    {
  ...
  for (std::vector< vtkDMMLSequenceNode* >::iterator it = synchronizedSequenceNodes.begin(); it != synchronizedSequenceNodes.end(); ++it)
    ...
    if (!syncProps)
      {
      vtkErrorMacro("vtkDMMLSequenceBrowserNode::SetSaveChanges failed: sequence node is invalid or does not belong to this browser node");
      continue;
      }
...
//-----------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SetRecordingSamplingModeFromString(const char *recordingSamplingModeString)
{
...
//-----------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::GetRecordingSamplingModeAsString()
{
  return vtkDMMLSequenceBrowserNode::GetRecordingSamplingModeAsString(this->RecordingSamplingMode);
}

//-----------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::GetRecordingSamplingModeAsString(int recordingSamplingMode)
{
  switch (recordingSamplingMode)
    {
    case vtkDMMLSequenceBrowserNode::SamplingAll: return "all";
    case vtkDMMLSequenceBrowserNode::SamplingLimitedToPlaybackFrameRate: return "limitedToPlaybackFrameRate";
    default:
      return "";
    }
}
...
//-----------------------------------------------------------
int vtkDMMLSequenceBrowserNode::GetRecordingSamplingModeFromString(const std::string& recordingSamplingModeString)
{
  for (int i = 0; i<vtkDMMLSequenceBrowserNode::NumberOfRecordingSamplingModes; i++)
    {
...
//-----------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SetIndexDisplayFormat(std::string indexDisplayNode)
{
...
//-----------------------------------------------------------
void vtkDMMLSequenceBrowserNode::SetIndexDisplayModeFromString(const char *indexDisplayModeString)
{
...
//-----------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::GetIndexDisplayModeAsString()
{
  return vtkDMMLSequenceBrowserNode::GetIndexDisplayModeAsString(this->IndexDisplayMode);
}

//-----------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::GetIndexDisplayModeAsString(int indexDisplayMode)
{
  switch (indexDisplayMode)
    {
    case vtkDMMLSequenceBrowserNode::IndexDisplayAsIndex: return "[index]";
    case vtkDMMLSequenceBrowserNode::IndexDisplayAsIndexValue: return "[indexValue]";
    default:
      return "";
    }
}
...
//-----------------------------------------------------------
int vtkDMMLSequenceBrowserNode::GetIndexDisplayModeFromString(const std::string& indexDisplayModeString)
{
  for (int i = 0; i<vtkDMMLSequenceBrowserNode::NumberOfIndexDisplayModes; i++)
    {
...
//-----------------------------------------------------------------------------
std::string vtkDMMLSequenceBrowserNode::GetFormattedIndexValue(int index)
{
  ...
  std::string prefix;
  std::string suffix;
  bool argFound = vtkDMMLSequenceBrowserNode::ValidateFormatString(sprintfSpecifier, prefix, suffix, formatString, "fFgGeEs");
  if (!argFound)
    {
    return indexValue;
    }
...
//-----------------------------------------------------------------------------
bool vtkDMMLSequenceBrowserNode::ValidateFormatString(std::string& validatedFormat, std::string& prefix, std::string& suffix,
...
//-----------------------------------------------------------------------------
vtkDMMLSequenceBrowserNode::SynchronizationProperties* vtkDMMLSequenceBrowserNode::GetSynchronizationPropertiesForSequence(vtkDMMLSequenceNode* sequenceNode)
{
  if (!sequenceNode)
    {
    return nullptr;
...
//-----------------------------------------------------------------------------
vtkDMMLSequenceBrowserNode::SynchronizationProperties* vtkDMMLSequenceBrowserNode::GetSynchronizationPropertiesForPostfix(const std::string& rolePostfix)
{
  if (rolePostfix.empty())
    {
    return nullptr;
