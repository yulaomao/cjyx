...
#ifndef __vtkDMMLSequenceBrowserNode_h
#define __vtkDMMLSequenceBrowserNode_h

#include "vtkCjyxSequencesModuleDMMLExport.h"
...
class VTK_CJYX_SEQUENCES_MODULE_DMML_EXPORT vtkDMMLSequenceBrowserNode : public vtkDMMLNode
{
public:
  static vtkDMMLSequenceBrowserNode *New();
  vtkTypeMacro(vtkDMMLSequenceBrowserNode,vtkDMMLNode);
  ...
  vtkDMMLSequenceNode* GetMasterSequenceNode();
  ...
  /// Adds a node for synchronized browsing. Returns the new proxy node postfix.
  std::string AddSynchronizedSequenceNode(vtkDMMLSequenceNode* synchronizedSequenceNode);
  ...
  void GetSynchronizedSequenceNodes(std::vector< vtkDMMLSequenceNode* > &synchronizedDataNodes, bool includeMasterNode=false);
  ...
  bool IsSynchronizedSequenceNode(vtkDMMLSequenceNode* sequenceNode, bool includeMasterNode = false);
  ...
  vtkDMMLNode* AddProxyNode(vtkDMMLNode* sourceProxyNode, vtkDMMLSequenceNode* sequenceNode, bool copy=true);

  /// Get proxy corresponding to a sequence node.
  vtkDMMLNode* GetProxyNode(vtkDMMLSequenceNode* sequenceNode);

  /// Deprecated method!
  vtkDMMLNode* GetVirtualOutputDataNode(vtkDMMLSequenceNode* sequenceNode)
    {
    static bool warningLogged = false;
    if (!warningLogged)
      {
      vtkWarningMacro("vtkDMMLSequenceBrowserNode::GetVirtualOutputDataNode is deprecated, use vtkDMMLSequenceBrowserNode::GetProxyNode method instead");
      warningLogged = true;
      }
  ...
  void SetOverwriteTargetNodeName(bool overwrite)
    ...
    if (!warningLogged)
      {
      vtkWarningMacro("vtkDMMLSequenceBrowserNode::SetOverwriteTargetNodeName is deprecated,"
        << " use vtkDMMLSequenceBrowserNode::SetOverwriteProxyName method instead");
      warningLogged = true;
      }
  ...
  void SetDeepCopyVirtualNodes(bool deepcopy)
    ...
    if (!warningLogged)
      {
      vtkWarningMacro("vtkDMMLSequenceBrowserNode::SetDeepCopyVirtualNodes is deprecated, use vtkDMMLSequenceBrowserNode::SetSaveChanges method instead");
      warningLogged = true;
      }
  ...
  vtkDMMLSequenceNode* GetSequenceNode(vtkDMMLNode* proxyNode);

  void GetAllProxyNodes(std::vector< vtkDMMLNode* > &nodes);
  ...
  void GetAllVirtualOutputDataNodes(vtkCollection* nodes)
    ...
    if (!warningLogged)
      {
      vtkWarningMacro("vtkDMMLSequenceBrowserNode::GetAllVirtualOutputDataNodes is deprecated,"
        << " use vtkDMMLSequenceBrowserNode::GetAllProxyNodes method instead");
      warningLogged = true;
      }
  ...
  bool GetRecording(vtkDMMLSequenceNode* sequenceNode);
  bool GetPlayback(vtkDMMLSequenceNode* sequenceNode);
  /// Overwrite proxy node name with name automatically generated from sequence base name and current item index.
  bool GetOverwriteProxyName(vtkDMMLSequenceNode* sequenceNode);
  ...
  bool GetSaveChanges(vtkDMMLSequenceNode* sequenceNode);

  /// Set the synchronization properties for the given sequence/proxy tuple
  void SetRecording(vtkDMMLSequenceNode* sequenceNode, bool recording);
  void SetPlayback(vtkDMMLSequenceNode* sequenceNode, bool playback);
  void SetOverwriteProxyName(vtkDMMLSequenceNode* sequenceNode, bool overwrite);
  void SetSaveChanges(vtkDMMLSequenceNode* sequenceNode, bool save);

  /// Process DMML node events for recording of the proxy nodes
...
protected:
  ...
  ~vtkDMMLSequenceBrowserNode() override;
  vtkDMMLSequenceBrowserNode(const vtkDMMLSequenceBrowserNode&);
  void operator=(const vtkDMMLSequenceBrowserNode&);
  ...
  void OnNodeReferenceAdded(vtkDMMLNodeReference* nodeReference) override;

  /// Called whenever a node reference is removed
  void OnNodeReferenceRemoved(vtkDMMLNodeReference* nodeReference) override;

  std::string GenerateSynchronizationPostfix();
  std::string GetSynchronizationPostfixFromSequence(vtkDMMLSequenceNode* sequenceNode);
...
protected:
  ...
  int RecordingSamplingMode{vtkDMMLSequenceBrowserNode::SamplingLimitedToPlaybackFrameRate};
  int IndexDisplayMode{vtkDMMLSequenceBrowserNode::IndexDisplayAsIndexValue};
...
private:
  ...
  SynchronizationProperties* GetSynchronizationPropertiesForSequence(vtkDMMLSequenceNode* sequenceNode);
