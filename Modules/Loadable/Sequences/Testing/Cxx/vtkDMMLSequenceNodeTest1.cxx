...
//-----------------------------------------------------------------------------
bool SequenceSortedByIndex(vtkDMMLSequenceNode* seqNode)
{
...
//-----------------------------------------------------------------------------
int vtkDMMLSequenceNodeTest1( int, char * [] )
{
  vtkNew< vtkDMMLSequenceNode > seqNode;
  EXERCISE_ALL_BASIC_DMML_METHODS(seqNode.GetPointer());

  seqNode->SetIndexType(vtkDMMLSequenceNode::NumericIndex);

  // Add data nodes at indices 5, 15, 25, 35, ..., 495
  vtkNew<vtkDMMLTransformNode> dataNode;
...
bool testAddInvalidFile(const char * filePath)
{
  ...
  vtkNew<vtkCjyxTablesLogic> tablesLogic;
  tablesLogic->SetDMMLScene(scene.GetPointer());

  int nodeCount = scene->GetNumberOfNodes();
  vtkDMMLTableNode* table = tablesLogic->AddTable(filePath);
