...
// Note: nodes obtained using vtkDMMLSelectionNode::GetUnitNodes() are expected
...
//-----------------------------------------------------------------------------
int vtkCjyxUnitsLogicTest1( int argc , char * argv[] )
{
  bool res = true;
  res = res && testScene();
...
template <class T> void printNodes(const std::vector<T>& nodes)
{
  for (size_t i = 0; i < nodes.size(); ++i)
    {
    vtkDMMLUnitNode* node = vtkDMMLUnitNode::SafeDownCast(nodes[i]);
    if (!node)
      {
                                      ...
                                      const char* testName)
{
  ...
  unitNodeIDs.emplace_back("vtkDMMLUnitNodeApplicationLength");
  unitNodeIDs.emplace_back("vtkDMMLUnitNodeApplicationArea");
  unitNodeIDs.emplace_back("vtkDMMLUnitNodeApplicationVolume");
  unitNodeIDs.emplace_back("vtkDMMLUnitNodeApplicationTime");
  unitNodeIDs.emplace_back("vtkDMMLUnitNodeApplicationFrequency");
  unitNodeIDs.emplace_back("vtkDMMLUnitNodeApplicationVelocity");
  unitNodeIDs.emplace_back("vtkDMMLUnitNodeApplicationIntensity");
...
bool testScene()
{
  ...
  vtkNew<vtkCjyxUnitsLogic> logic;
  logic->SetDMMLScene(scene.GetPointer());

  // Test scene
  std::vector<vtkDMMLNode*> nodes;
  scene->GetNodesByClass("vtkDMMLUnitNode", nodes);
...
bool testSelectionNode()
{
  ...
  // Test scene
  std::vector<vtkDMMLUnitNode*> nodes;
...
bool testSaveAndReloadScene()
{
  ...
  std::vector<vtkDMMLUnitNode*> unitNodes;
...
bool testGetSIPrefixCoefficient()
{
  ...
  for (std::map<std::string, double>::iterator it = coefficients.begin();
    ...
    double coefficient = vtkCjyxUnitsLogic::GetSIPrefixCoefficient(prefix.c_str());
