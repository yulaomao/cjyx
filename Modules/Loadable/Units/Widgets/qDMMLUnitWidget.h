...
#ifndef __qDMMLUnitWidget_h
#define __qDMMLUnitWidget_h
...
class qDMMLUnitWidgetPrivate;
class vtkCjyxUnitsLogic;

class Q_CJYX_MODULE_UNITS_WIDGETS_EXPORT qDMMLUnitWidget : public qDMMLWidget
{
  Q_OBJECT
  QVTK_OBJECT
...
public:
  ...
  qDMMLUnitWidget(QWidget *parent=nullptr);
  ~qDMMLUnitWidget() override;
  ...
  void setDMMLScene(vtkDMMLScene* unitsLogicScene) override;

  Q_INVOKABLE vtkDMMLNode* currentNode() const;

public slots:
  void setCurrentNode(vtkDMMLNode* unitNode);
  ...
  void setUnitFromPreset(vtkDMMLNode* presetNode);
...
protected:
  QScopedPointer<qDMMLUnitWidgetPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLUnitWidget);
  Q_DISABLE_COPY(qDMMLUnitWidget);
};

Q_DECLARE_OPERATORS_FOR_FLAGS(qDMMLUnitWidget::UnitProperties)
