...
#ifndef __vtkCjyxUnitsLogic_h
#define __vtkCjyxUnitsLogic_h
...
#include "vtkCjyxUnitsModuleLogicExport.h"

// STD includes
#include <map>

/// \brief Cjyx logic for unit manipulation.
///
...
class VTK_CJYX_UNITS_MODULE_LOGIC_EXPORT vtkCjyxUnitsLogic
  : public vtkDMMLAbstractLogic
{
public:
  static vtkCjyxUnitsLogic *New();
  typedef vtkCjyxUnitsLogic Self;
  vtkTypeMacro(vtkCjyxUnitsLogic, vtkDMMLAbstractLogic);
  void PrintSelf(ostream& os, vtkIndent indent) override;

  /// Add unit node to the scene.
  /// Returns nullptr if the logic has no scene.
  vtkDMMLUnitNode* AddUnitNode(const char* name,
  ...
  vtkDMMLScene* GetUnitsScene() const;
  ...
  /// parameter should be provided. For example, in Cjyx, since `length` values
...
protected:
  ...
  /// \sa SaveDefaultUnits(), RestoreDefaultUnits()
  void OnDMMLNodeModified(vtkDMMLNode* modifiedNode) override;
  ...
  virtual void AddBuiltInUnits(vtkDMMLScene* scene);

  /// Overloaded to add the defaults units in the application scene.
  void SetDMMLSceneInternal(vtkDMMLScene* newScene) override;

  /// Register DMML Node classes to Scene.
  /// Gets called automatically when the DMMLScene is attached to this
  /// logic class.
  void RegisterNodes() override;
  virtual void RegisterNodesInternal(vtkDMMLScene* scene);
  ...
  vtkDMMLUnitNode* AddUnitNodeToScene(vtkDMMLScene* scene,
    const char* name,
    const char* quantity = "length",
    const char* prefix = "",
    const char* suffix = "",
  ...
  // Variables
  vtkDMMLScene* UnitsScene;
private:
  vtkCjyxUnitsLogic(const vtkCjyxUnitsLogic&) = delete;
  void operator=(const vtkCjyxUnitsLogic&) = delete;
