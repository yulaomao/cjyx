...
class TablesSelfTestTest(ScriptedLoadableModuleTest):
    ...
    def test_TablesSelfTest_FullTest1(self):
        ...
        self.assertTrue(cjyx.modules.tables)
    ...
    def section_CreateTable(self):
        ...
        tableNode = cjyx.vtkDMMLTableNode()
        cjyx.dmmlScene.AddNode(tableNode)
    ...
    def section_TableProperties(self):
        ...
        tableNode = cjyx.util.getNode(self.sampleTableName)
    ...
    def section_TableWidgetButtons(self):
        ...
        cjyx.util.selectModule('Tables')

        # Make sure subject hierarchy auto-creation is on for this test
        tablesWidget = cjyx.modules.tables.widgetRepresentation()
        ...
        lockTableButton = cjyx.util.findChildren(widget=tablesWidget, name='LockTableButton')[0]
        copyButton = cjyx.util.findChildren(widget=tablesWidget, name='CopyButton')[0]
        pasteButton = cjyx.util.findChildren(widget=tablesWidget, name='PasteButton')[0]
        addRowButton = cjyx.util.findChildren(widget=tablesWidget, name='RowInsertButton')[0]
        deleteRowButton = cjyx.util.findChildren(widget=tablesWidget, name='RowDeleteButton')[0]
        lockFirstRowButton = cjyx.util.findChildren(widget=tablesWidget, name='LockFirstRowButton')[0]
        addColumnButton = cjyx.util.findChildren(widget=tablesWidget, name='ColumnInsertButton')[0]
        deleteColumnButton = cjyx.util.findChildren(widget=tablesWidget, name='ColumnDeleteButton')[0]
        lockFirstColumnButton = cjyx.util.findChildren(widget=tablesWidget, name='LockFirstColumnButton')[0]
        tableView = cjyx.util.findChildren(widget=tablesWidget, name='TableView')[0]
    ...
    def section_CliTableInputOutput(self):
        ...
        inputTableNode = cjyx.vtkDMMLTableNode()
        cjyx.dmmlScene.AddNode(inputTableNode)
        ...
        outputTableNode = cjyx.vtkDMMLTableNode()
        cjyx.dmmlScene.AddNode(outputTableNode)
        ...
        cjyx.cli.run(cjyx.modules.executionmodeltour, None, parameters, wait_for_completion=True)
    ...
    def createDummyTransform(self):
        transformNode = cjyx.vtkDMMLLinearTransformNode()
        cjyx.dmmlScene.AddNode(transformNode)
    ...
    def createDummyVolume(self):
        ...
        volumeNode = cjyx.vtkDMMLScalarVolumeNode()
        volumeNode.SetAndObserveImageData(imageData)
        displayNode = cjyx.vtkDMMLScalarVolumeDisplayNode()
        cjyx.dmmlScene.AddNode(volumeNode)
        cjyx.dmmlScene.AddNode(displayNode)
        volumeNode.SetAndObserveDisplayNodeID(displayNode.GetID())
        displayNode.SetAndObserveColorNodeID('vtkDMMLColorTableNodeGrey')
        return volumeNode
