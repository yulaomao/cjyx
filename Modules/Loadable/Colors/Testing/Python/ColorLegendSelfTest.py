...
class ColorLegendSelfTest(ScriptedLoadableModule):
    ...
    This is a test case for the new vtkCjyxScalarBarActor class.
...
class ColorLegendSelfTestTest(ScriptedLoadableModuleTest):
    ...
    def test_ColorLegendSelfTest1(self):
        ...
        colorWidget = cjyx.modules.colors.widgetRepresentation()
        activeColorNodeSelector = cjyx.util.findChildren(colorWidget, 'ColorTableComboBox')[0]
        self.assertIsNotNone(activeColorNodeSelector)
        activeDisplayableNodeSelector = cjyx.util.findChildren(colorWidget, 'DisplayableNodeComboBox')[0]
        self.assertIsNotNone(activeDisplayableNodeSelector)
        createColorLegendButton = cjyx.util.findChildren(colorWidget, 'CreateColorLegendButton')[0]
        self.assertIsNotNone(createColorLegendButton)
        useCurrentColorsButton = cjyx.util.findChildren(colorWidget, 'UseCurrentColorsButton')[0]
        self.assertIsNotNone(useCurrentColorsButton)
        colorLegendDisplayNodeWidget = cjyx.util.findChildren(colorWidget, 'ColorLegendDisplayNodeWidget')[0]
        self.assertIsNotNone(colorLegendDisplayNodeWidget)
        colorLegendVisibilityCheckBox = cjyx.util.findChildren(colorLegendDisplayNodeWidget, 'ColorLegendVisibilityCheckBox')[0]
        ...
        for ind, n in enumerate(testedColorNodeIndices):
            colorNode = cjyx.dmmlScene.GetNthNodeByClass(n, 'vtkDMMLColorNode')
        ...
        colorLegend = cjyx.modules.colors.logic().GetColorLegendDisplayNode(ctVolumeNode)
        self.assertIsNotNone(colorLegend)

        self.delayDisplay('Exercise color legend updates via DMML', self.delayMs)
        ...
        sliceNodeRed = cjyx.app.layoutManager().sliceWidget('Red').dmmlSliceNode()
        self.assertIsNotNone(sliceNodeRed)
        threeDViewNode = cjyx.app.layoutManager().threeDWidget(0).dmmlViewNode()
        ...
        for sliceName, titleColor in sliceNameColor.items():
            ...
            sliceNode = cjyx.app.layoutManager().sliceWidget(sliceName).dmmlSliceNode()
