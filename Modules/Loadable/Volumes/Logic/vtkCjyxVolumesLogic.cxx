/*=auto=========================================================================
  ...
  Program:   3D Cjyx
  Module:    $RCSfile: vtkCjyxVolumesLogic.cxx,v $
...
//----------------------------------------------------------------------------
// vtkCjyxVolumesLogic methods

//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkCjyxVolumesLogic);
...
//----------------------------------------------------------------------------
ArchetypeVolumeNodeSet DiffusionWeightedVolumeNodeSetFactory(std::string& volumeName, vtkDMMLScene* scene, int options)
{
  ArchetypeVolumeNodeSet nodeSet(scene);

  // set up the dwi node's support nodes
  vtkDMMLDiffusionWeightedVolumeDisplayNode* dwdisplayNode =
      vtkDMMLDiffusionWeightedVolumeDisplayNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLDiffusionWeightedVolumeDisplayNode"));

  vtkDMMLDiffusionWeightedVolumeNode* dwiNode =
      vtkDMMLDiffusionWeightedVolumeNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLDiffusionWeightedVolumeNode", volumeName));
  dwiNode->SetAndObserveDisplayNodeID(dwdisplayNode->GetID());

  vtkDMMLNRRDStorageNode* storageNode =
      vtkDMMLNRRDStorageNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLNRRDStorageNode"));
...
//----------------------------------------------------------------------------
ArchetypeVolumeNodeSet DiffusionTensorVolumeNodeSetFactory(std::string& volumeName, vtkDMMLScene* scene, int options)
{
  ArchetypeVolumeNodeSet nodeSet(scene);

  // set up the tensor node's support nodes
  vtkDMMLDiffusionTensorVolumeDisplayNode* dtdisplayNode =
      vtkDMMLDiffusionTensorVolumeDisplayNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLDiffusionTensorVolumeDisplayNode"));
  ...
  vtkDMMLDiffusionTensorVolumeNode* tensorNode =
      vtkDMMLDiffusionTensorVolumeNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLDiffusionTensorVolumeNode", volumeName));
  tensorNode->SetAndObserveDisplayNodeID(dtdisplayNode->GetID());

  vtkDMMLVolumeArchetypeStorageNode* storageNode =
      vtkDMMLVolumeArchetypeStorageNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLVolumeArchetypeStorageNode"));
  wasModifying = storageNode->StartModify();
  storageNode->SetCenterImage(options & vtkCjyxVolumesLogic::CenterImage);
  storageNode->SetUseOrientationFromFile(!((options & vtkCjyxVolumesLogic::DiscardOrientation) != 0));
  storageNode->SetSingleFile(options & vtkCjyxVolumesLogic::SingleFile);
...
//----------------------------------------------------------------------------
ArchetypeVolumeNodeSet NRRDVectorVolumeNodeSetFactory(std::string& volumeName, vtkDMMLScene* scene, int options)
{
  ArchetypeVolumeNodeSet nodeSet(scene);

  // set up the vector node's support nodes
  vtkDMMLVectorVolumeDisplayNode* vdisplayNode =
      vtkDMMLVectorVolumeDisplayNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLVectorVolumeDisplayNode"));

  vtkDMMLVectorVolumeNode* vectorNode =
      vtkDMMLVectorVolumeNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLVectorVolumeNode", volumeName));
...
//----------------------------------------------------------------------------
ArchetypeVolumeNodeSet ArchetypeVectorVolumeNodeSetFactory(std::string& volumeName, vtkDMMLScene* scene, int options)
{
  ArchetypeVolumeNodeSet nodeSet(scene);
...
//----------------------------------------------------------------------------
ArchetypeVolumeNodeSet LabelMapVolumeNodeSetFactory(std::string& volumeName, vtkDMMLScene* scene, int options)
{
  ArchetypeVolumeNodeSet nodeSet(scene);

  // set up the scalar node's support nodes
  vtkDMMLLabelMapVolumeDisplayNode* lmdisplayNode =
      vtkDMMLLabelMapVolumeDisplayNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLLabelMapVolumeDisplayNode"));

  vtkDMMLLabelMapVolumeNode* scalarNode =
      vtkDMMLLabelMapVolumeNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLLabelMapVolumeNode", volumeName));
...
//----------------------------------------------------------------------------
ArchetypeVolumeNodeSet ScalarVolumeNodeSetFactory(std::string& volumeName, vtkDMMLScene* scene, int options)
{
  ArchetypeVolumeNodeSet nodeSet(scene);

  // set up the scalar node's support nodes
  vtkDMMLScalarVolumeDisplayNode* sdisplayNode =
      vtkDMMLScalarVolumeDisplayNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLScalarVolumeDisplayNode"));

  vtkDMMLScalarVolumeNode* scalarNode =
      vtkDMMLScalarVolumeNode::SafeDownCast(
        nodeSet.Scene->AddNewNodeByClass("vtkDMMLScalarVolumeNode", volumeName));
...
//----------------------------------------------------------------------------
vtkCjyxVolumesLogic::vtkCjyxVolumesLogic()
{
...
//----------------------------------------------------------------------------
vtkCjyxVolumesLogic::~vtkCjyxVolumesLogic() = default;

//----------------------------------------------------------------------------
void vtkCjyxVolumesLogic::ProcessDMMLNodesEvents(vtkObject *vtkNotUsed(caller),
                                            unsigned long event,
                                            void *callData)
{
...
//----------------------------------------------------------------------------
void vtkCjyxVolumesLogic
::SetAndObserveColorToDisplayNode(vtkDMMLDisplayNode * displayNode,
                                  int labelMap, const char* vtkNotUsed(filename))
{
  ...
  vtkDMMLColorLogic* colorLogic = vtkDMMLColorLogic::SafeDownCast(this->GetModuleLogic("Colors"));
...
//----------------------------------------------------------------------------
void vtkCjyxVolumesLogic::InitializeStorageNode(
  vtkDMMLStorageNode * storageNode, const char * filename, vtkStringArray *fileList, vtkDMMLScene * dmmlScene)
{
  bool useURI = false;

  if (dmmlScene == nullptr)
    {
    dmmlScene = this->GetDMMLScene();
    }

  if (dmmlScene && dmmlScene->GetCacheManager())
    {
    useURI = dmmlScene->GetCacheManager()->IsRemoteReference(filename);
    }
  if (useURI)
    {
    ...
    storageNode->SetScene(dmmlScene);
    if (fileList != nullptr)
      {
      // it's a list of uris
  ...
  storageNode->AddObserver(vtkCommand::ProgressEvent,  this->GetDMMLNodesCallbackCommand());
}

//----------------------------------------------------------------------------
vtkDMMLVolumeNode* vtkCjyxVolumesLogic::AddArchetypeVolume(
...
//----------------------------------------------------------------------------
vtkDMMLScalarVolumeNode* vtkCjyxVolumesLogic::AddArchetypeScalarVolume(
    const char* filename, const char* volname, int loadingOptions, vtkStringArray *fileList)
{
  ...
  return vtkDMMLScalarVolumeNode::SafeDownCast(this->AddArchetypeVolume(nodeSetFactoryRegistry, filename, volname, loadingOptions, fileList));
}
    ...
    vtkStringArray *fileList)
{
  if (this->GetDMMLScene() == nullptr)
    {
    vtkErrorMacro("AddArchetypeVolume: Failed to add volume - DMMLScene is null");
    return nullptr;
    }
  ...
  vtkSmartPointer<vtkDMMLVolumeNode> volumeNode;
  ...
  volumeName = this->GetDMMLScene()->GetUniqueNameByString(volumeName.c_str());

  vtkNew<vtkErrorSink> errorSink;

  // set up a mini scene to avoid adding and removing nodes from the main scene
  vtkNew<vtkDMMLScene> testScene;
  // associate default nodes with mini scene
  this->GetDMMLScene()->CopyDefaultNodesToScene(testScene.GetPointer());
  // set it up for remote io, the constructor creates a cache and data io manager
  vtkSmartPointer<vtkDMMLRemoteIOLogic> remoteIOLogic;
  remoteIOLogic = vtkSmartPointer<vtkDMMLRemoteIOLogic>::New();
  if (this->GetDMMLScene()->GetCacheManager())
    {
    // update the temp remote cache dir from the main one
    remoteIOLogic->GetCacheManager()->SetRemoteCacheDirectory(this->GetDMMLScene()->GetCacheManager()->GetRemoteCacheDirectory());
    }
  ...
  dataIOManagerLogic->SetDMMLApplicationLogic(this->GetApplicationLogic());
  dataIOManagerLogic->SetAndObserveDataIOManager(
    remoteIOLogic->GetDataIOManager());

  // and link up everything for the test scene
  this->GetApplicationLogic()->SetDMMLSceneDataIO(testScene.GetPointer(),
  ...
  for (NodeSetFactoryRegistry::const_iterator fit = volumeRegistry.begin();
    ...
    if (labelMap == nodeSet.LabelMap)
      ...
      nodeSet.StorageNode->AddObserver(vtkCommand::ProgressEvent,  this->GetDMMLNodesCallbackCommand());
      ...
      nodeSet.StorageNode->RemoveObservers(vtkCommand::ProgressEvent,  this->GetDMMLNodesCallbackCommand());
  ...
  if (volumeNode != nullptr)
    ...
    this->GetDMMLScene()->AddNode(volumeNode);
...
//----------------------------------------------------------------------------
int vtkCjyxVolumesLogic::SaveArchetypeVolume (const char* filename, vtkDMMLVolumeNode *volumeNode)
{
  ...
  vtkDMMLNRRDStorageNode *storageNode1 = nullptr;
  vtkDMMLVolumeArchetypeStorageNode *storageNode2 = nullptr;
  vtkDMMLStorageNode *storageNode = nullptr;
  vtkDMMLStorageNode *snode = volumeNode->GetStorageNode();

  if (snode != nullptr)
    {
    storageNode2 = vtkDMMLVolumeArchetypeStorageNode::SafeDownCast(snode);
    storageNode1 = vtkDMMLNRRDStorageNode::SafeDownCast(snode);
    }

  bool useURI = false;
  if (this->GetDMMLScene() &&
      this->GetDMMLScene()->GetCacheManager())
    {
  ...
  if (volumeNode->IsA("vtkDMMLDiffusionWeightedVolumeNode") ||
//      volumeNode->IsA("vtkDMMLDiffusionTensorVolumeNode") ||
      volumeNode->IsA("vtkDMMLVectorVolumeNode"))
    {

    if (storageNode1 == nullptr)
      {
      storageNode1 = vtkDMMLNRRDStorageNode::New();
      storageNode1->SetScene(this->GetDMMLScene());
      this->GetDMMLScene()->AddNode(storageNode1);
    ...
    if (storageNode2 == nullptr)
      {
      storageNode2 = vtkDMMLVolumeArchetypeStorageNode::New();
      storageNode2->SetScene(this->GetDMMLScene());
      this->GetDMMLScene()->AddNode(storageNode2);
...
//----------------------------------------------------------------------------
vtkDMMLLabelMapVolumeNode* vtkCjyxVolumesLogic
::CreateAndAddLabelVolume(vtkDMMLVolumeNode *volumeNode, const char *name)
{
  return this->CreateAndAddLabelVolume(this->GetDMMLScene(), volumeNode, name);
}

//----------------------------------------------------------------------------
vtkDMMLLabelMapVolumeNode *
vtkCjyxVolumesLogic::CreateAndAddLabelVolume(vtkDMMLScene *scene,
                                               ...
                                               const char *name)
{
  ...
  vtkNew<vtkDMMLLabelMapVolumeNode> labelNode;
  ...
  vtkCjyxVolumesLogic::ClearVolumeImageData(labelNode.GetPointer());
...
//----------------------------------------------------------------------------
vtkDMMLLabelMapVolumeNode* vtkCjyxVolumesLogic
::CreateLabelVolume(vtkDMMLVolumeNode *volumeNode,
                    const char *name)
{
...
//----------------------------------------------------------------------------
vtkDMMLLabelMapVolumeNode* vtkCjyxVolumesLogic
::CreateLabelVolume(vtkDMMLScene* scene,
                    vtkDMMLVolumeNode *volumeNode,
                    const char *name)
{
  vtkWarningMacro("Deprecated, please use CreateAndAddLabelVolume instead");
...
//----------------------------------------------------------------------------
vtkDMMLLabelMapVolumeNode *
vtkCjyxVolumesLogic::FillLabelVolumeFromTemplate(vtkDMMLLabelMapVolumeNode *labelNode,
                                                   vtkDMMLVolumeNode *templateNode)
{
  return Self::FillLabelVolumeFromTemplate(this->GetDMMLScene(), labelNode, templateNode);
}

//----------------------------------------------------------------------------
vtkDMMLLabelMapVolumeNode *
vtkCjyxVolumesLogic::FillLabelVolumeFromTemplate(vtkDMMLScene *scene,
                                                   ...
                                                   vtkDMMLVolumeNode *templateNode)
{
  ...
  // Make an image data of the same size and shape as the input volume, but filled with zeros
  vtkCjyxVolumesLogic::ClearVolumeImageData(labelNode);
...
//----------------------------------------------------------------------------
vtkDMMLLabelMapVolumeNode*
vtkCjyxVolumesLogic::CreateLabelVolumeFromVolume(vtkDMMLScene *scene,
                                                   ...
                                                   vtkDMMLVolumeNode *inputVolume)
{
  ...
  vtkDMMLLabelMapVolumeDisplayNode* displayNode =
    vtkDMMLLabelMapVolumeDisplayNode::SafeDownCast(outputVolume->GetVolumeDisplayNode());
  if (displayNode == nullptr)
    {
    displayNode = vtkDMMLLabelMapVolumeDisplayNode::SafeDownCast(scene->AddNewNodeByClass("vtkDMMLLabelMapVolumeDisplayNode"));
...
//----------------------------------------------------------------------------
vtkDMMLScalarVolumeNode* vtkCjyxVolumesLogic::CreateScalarVolumeFromVolume(
  vtkDMMLScene *scene, vtkDMMLScalarVolumeNode *outputVolume, vtkDMMLVolumeNode *inputVolume)
{
...
//----------------------------------------------------------------------------
void
vtkCjyxVolumesLogic::ClearVolumeImageData(vtkDMMLVolumeNode *volumeNode)
{
  if (volumeNode == nullptr)
    {
    return;
...
//----------------------------------------------------------------------------
std::string
vtkCjyxVolumesLogic::CheckForLabelVolumeValidity(vtkDMMLScalarVolumeNode *volumeNode,
                                                   vtkDMMLLabelMapVolumeNode *labelNode)
{
  ...
  if (!volumeNode || !labelNode)
    ...
    {
    if (vtkDMMLLabelMapVolumeNode::SafeDownCast(labelNode)==nullptr)
      {
      warnings << "Label node is not of type vtkDMMLLabelMapVolumeNode\n";
      }
    else
      {
...
//----------------------------------------------------------------------------
void vtkCjyxVolumesLogic::SetCompareVolumeGeometryEpsilon(double epsilon)
{
  vtkDebugMacro("vtkCjyxVolumesLogic setting "
...
//----------------------------------------------------------------------------
std::string
vtkCjyxVolumesLogic::CompareVolumeGeometry(vtkDMMLScalarVolumeNode *volumeNode1,
...
vtkCjyxVolumesLogic::CloneVolume(vtkDMMLVolumeNode *volumeNode, const char *name)
{
  return Self::CloneVolume(this->GetDMMLScene(), volumeNode, name);
}

//----------------------------------------------------------------------------
vtkDMMLScalarVolumeNode*
vtkCjyxVolumesLogic::
CloneVolume(vtkDMMLScene *scene, vtkDMMLVolumeNode *volumeNode, const char *name, bool cloneImageData/*=true*/)
{
  return vtkDMMLScalarVolumeNode::SafeDownCast(vtkCjyxVolumesLogic::CloneVolumeGeneric(scene, volumeNode, name, cloneImageData));
}

//----------------------------------------------------------------------------
vtkDMMLVolumeNode*
vtkCjyxVolumesLogic::
CloneVolumeGeneric (vtkDMMLScene *scene, vtkDMMLVolumeNode *volumeNode, const char *name, bool cloneImageData/*=true*/)
{
  if ( scene == nullptr || volumeNode == nullptr )
    {
    vtkGenericWarningMacro("vtkCjyxVolumesLogic::CloneVolumeGeneric failed: invalid scene or input volume node");
    return nullptr;
    }

  // clone the volume node
  vtkSmartPointer<vtkDMMLVolumeNode> clonedVolumeNode;
  clonedVolumeNode.TakeReference(vtkDMMLVolumeNode::SafeDownCast(scene->CreateNodeByClass(volumeNode->GetClassName())));
  ...
  vtkDMMLDisplayNode* originalDisplayNode = volumeNode->GetVolumeDisplayNode();
  vtkSmartPointer<vtkDMMLDisplayNode> clonedDisplayNode;
  if (originalDisplayNode)
    {
    clonedDisplayNode.TakeReference((vtkDMMLDisplayNode*)scene->CreateNodeByClass(originalDisplayNode->GetClassName()));
    }
...
vtkDMMLScalarVolumeNode*
vtkCjyxVolumesLogic::
CloneVolumeWithoutImageData(vtkDMMLScene *scene, vtkDMMLVolumeNode *volumeNode, const char *name)
{
  return vtkCjyxVolumesLogic::CloneVolume(scene, volumeNode, name, /*cloneImageData:*/ false );
}

//----------------------------------------------------------------------------
void vtkCjyxVolumesLogic::PrintSelf(ostream& os, vtkIndent indent)
{
  this->vtkObject::PrintSelf(os, indent);

  os << indent << "vtkCjyxVolumesLogic:             " << this->GetClassName() << "\n";
...
//----------------------------------------------------------------------------
int vtkCjyxVolumesLogic::IsFreeSurferVolume (const char* filename)
{
  if (filename == nullptr)
    {
    return 0;
...
//-------------------------------------------------------------------------
void vtkCjyxVolumesLogic::ComputeTkRegVox2RASMatrix ( vtkDMMLVolumeNode *VNode,
                                                                       vtkMatrix4x4 *M )
{
    double dC, dS, dR;
    double Nc, Ns, Nr;
...
//-------------------------------------------------------------------------
void vtkCjyxVolumesLogic::CenterVolume(vtkDMMLVolumeNode* volumeNode)
{
...
//------------------------------------------------------------------------------
void vtkCjyxVolumesLogic
::GetVolumeCenteredOrigin(vtkDMMLVolumeNode* volumeNode, double* origin)
{
  // WARNING: this code is duplicated in qDMMLVolumeInfoWidget !
  origin[0] = 0.;
  origin[1] = 0.;
  origin[2] = 0.;
...
//-------------------------------------------------------------------------
void vtkCjyxVolumesLogic::TranslateFreeSurferRegistrationMatrixIntoCjyxRASToRASMatrix( vtkDMMLVolumeNode *V1Node,
                                                                       ...
                                                                       vtkMatrix4x4 *RAS2RASMatrix)
{
  if  ( V1Node  && V2Node && FSRegistrationMatrix  && RAS2RASMatrix )
    ...
    //
    // So this is what we'll compute and use in Cjyx instead
...
vtkDMMLScalarVolumeNode*
vtkCjyxVolumesLogic
::ResampleVolumeToReferenceVolume(vtkDMMLVolumeNode* inputVolumeNode,
                                  vtkDMMLVolumeNode* referenceVolumeNode)
{
  ...
  vtkDMMLScene* scene = inputVolumeNode->GetScene();
  if (!scene)
    {
    return nullptr;
    }
  ...
  vtkDMMLScalarVolumeNode* outputVolumeNode = Self::CloneVolumeWithoutImageData(scene,
                                                                                inputVolumeNode,
  ...
  vtkSmartPointer<vtkDMMLTransformNode> inputVolumeNodeTransformNode = vtkDMMLTransformNode::SafeDownCast(
  ...
  vtkSmartPointer<vtkDMMLTransformNode> referenceVolumeNodeTransformNode = vtkDMMLTransformNode::SafeDownCast(
  ...
  if (vtkDMMLTransformNode::IsGeneralTransformLinear(outputVolumeResliceTransform, linearResliceTransform))
    {
  ...
  if (inputVolumeNode->IsA("vtkDMMLLabelMapVolumeNode"))
    {
...
// --------------------------------------------------------------------------
std::vector<std::string> vtkCjyxVolumesLogic::GetVolumeDisplayPresetIDs()
{
...
// --------------------------------------------------------------------------
void vtkCjyxVolumesLogic::InitializeDefaultVolumeDisplayPresets()
{
  if (!this->VolumeDisplayPresets.empty())
    {
    // already initialized
  ...
  if (!fp)
    {
    vtkErrorMacro("vtkCjyxVolumesLogic::InitializeDefaultVolumeDisplayPresets failed: Error opening the file '" << displayPresetsFilename << "'");
    return;
    }
  ...
  if (jsonRoot->ParseStream(fs).HasParseError())
    {
    vtkErrorMacro("vtkCjyxVolumesLogic::InitializeDefaultVolumeDisplayPresets failed: Error parsing the file '" << displayPresetsFilename << "'.");
    fclose(fp);
    }
  fclose(fp);
  std::string errorPrefix = "vtkCjyxVolumesLogic::InitializeDefaultVolumeDisplayPresets failed: Error reading '" + displayPresetsFilename + "'.";
...
// --------------------------------------------------------------------------
vtkCjyxVolumesLogic::VolumeDisplayPreset vtkCjyxVolumesLogic::GetVolumeDisplayPreset(const std::string& presetId)
{
  if (this->VolumeDisplayPresets.empty())
    {
...
// --------------------------------------------------------------------------
bool vtkCjyxVolumesLogic::ApplyVolumeDisplayPreset(vtkDMMLVolumeDisplayNode* displayNode, std::string presetId)
{
  VolumeDisplayPreset preset = this->GetVolumeDisplayPreset(presetId);
  if (!preset.valid)
    {
    vtkErrorMacro("vtkCjyxVolumesLogic::ApplyVolumeDisplayPreset failed: Could not find preset ID " << presetId);
    return false;
    }

  vtkDMMLScalarVolumeDisplayNode* volumeDisplayNode = vtkDMMLScalarVolumeDisplayNode::SafeDownCast(displayNode);
  if (!volumeDisplayNode)
    {
    vtkErrorMacro("vtkCjyxVolumesLogic::ApplyVolumeDisplayPreset failed: Invalid display node");
    return false;
    }
...
// --------------------------------------------------------------------------
std::string vtkCjyxVolumesLogic::GetAppliedVolumeDisplayPresetId(vtkDMMLVolumeDisplayNode* displayNode)
{
  vtkDMMLScalarVolumeDisplayNode* volumeDisplayNode = vtkDMMLScalarVolumeDisplayNode::SafeDownCast(displayNode);
  if (!volumeDisplayNode)
    {
