...
Supported by NA-MIC, NAC, BIRN, NCIGT, and the Cjyx Community. See https://www.slicer.org for details.
"""
...
class SegmentStatisticsWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        self.parameterNodeSelector = cjyx.qDMMLNodeComboBox()
        self.parameterNodeSelector.nodeTypes = ["vtkDMMLScriptedModuleNode"]
        self.parameterNodeSelector.addAttribute("vtkDMMLScriptedModuleNode", "ModuleName", "SegmentStatistics")
        ...
        self.parameterNodeSelector.setDMMLScene(cjyx.dmmlScene)
        ...
        self.segmentationSelector = cjyx.qDMMLNodeComboBox()
        self.segmentationSelector.nodeTypes = ["vtkDMMLSegmentationNode"]
        self.segmentationSelector.addEnabled = False
        self.segmentationSelector.removeEnabled = True
        self.segmentationSelector.renameEnabled = True
        self.segmentationSelector.setDMMLScene(cjyx.dmmlScene)
        ...
        self.scalarSelector = cjyx.qDMMLNodeComboBox()
        self.scalarSelector.nodeTypes = ["vtkDMMLScalarVolumeNode"]
        ...
        self.scalarSelector.setDMMLScene(cjyx.dmmlScene)
        ...
        self.outputTableSelector = cjyx.qDMMLNodeComboBox()
        self.outputTableSelector.noneDisplay = "Create new table"
        self.outputTableSelector.setDMMLScene(cjyx.dmmlScene)
        self.outputTableSelector.nodeTypes = ["vtkDMMLTableNode"]
        ...
        self.scalarSelector.connect('currentNodeChanged(vtkDMMLNode*)', self.onNodeSelectionChanged)
        self.segmentationSelector.connect('currentNodeChanged(vtkDMMLNode*)', self.onNodeSelectionChanged)
        self.outputTableSelector.connect('currentNodeChanged(vtkDMMLNode*)', self.onNodeSelectionChanged)
        self.parameterNodeSelector.connect('currentNodeChanged(vtkDMMLNode*)', self.onNodeSelectionChanged)
        self.parameterNodeSelector.connect('currentNodeChanged(vtkDMMLNode*)', self.onParameterSetSelected)
    ...
    def enter(self):
        ...
        if self.parameterNodeSelector.currentNode() is None:
            ...
            cjyx.dmmlScene.AddNode(parameterNode)
    ...
    def onApply(self):
        ...
        with cjyx.util.tryWithErrorDisplay("Failed to compute results.", waitCursor=True):
            if not self.outputTableSelector.currentNode():
                newTable = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLTableNode")
...
class SegmentStatisticsLogic(ScriptedLoadableModuleLogic):
    ...
    def computeStatistics(self):
        ...
        segmentationNode = cjyx.dmmlScene.GetNodeByID(self.getParameterNode().GetParameter("Segmentation"))
        ...
        try:
            if not segmentationNode.GetParentTransformNode() is None:
                ...
                transformedSegmentationNode = cjyx.vtkDMMLSegmentationNode()
                transformedSegmentationNode.Copy(segmentationNode)
                transformedSegmentationNode.HideFromEditorsOn()
                cjyx.dmmlScene.AddNode(transformedSegmentationNode)
            ...
            if transformedSegmentationNode is not None:
                ...
                cjyx.dmmlScene.RemoveNode(transformedSegmentationNode)
    ...
    def showTable(self, table):
        ...
        """
        currentLayout = cjyx.app.layoutManager().layout
        layoutWithTable = cjyx.modules.tables.logic().GetLayoutWithTable(currentLayout)
        cjyx.app.layoutManager().setLayout(layoutWithTable)
        cjyx.app.applicationLogic().GetSelectionNode().SetActiveTableID(table.GetID())
        cjyx.app.applicationLogic().PropagateTableSelection()
...
class SegmentStatisticsTest(ScriptedLoadableModuleTest):
    ...
    def test_SegmentStatisticsBasic(self):
        ...
        resultsTableNode = cjyx.vtkDMMLTableNode()
        cjyx.dmmlScene.AddNode(resultsTableNode)
        ...
        outputFilename = cjyx.app.temporaryPath + '/SegmentStatisticsTestOutput.csv'
...
class Slicelet:
    """A cjyx slicelet is a module widget that comes up in stand alone mode
    implemented as a python class.
    This class provides common wrapper functionality used by all cjyx modlets.
    """
    ...
    def __init__(self, widgetClass=None):
        ...
        self.addDataButton.connect("clicked()", cjyx.app.ioManager().openAddDataDialog)
        self.loadSceneButton = qt.QPushButton("Load Scene")
        self.buttons.layout().addWidget(self.loadSceneButton)
        self.loadSceneButton.connect("clicked()", cjyx.app.ioManager().openLoadSceneDialog)
