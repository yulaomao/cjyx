...
NA-MIC, NAC, BIRN, NCIGT, and the Cjyx Community.
"""


#
# qCjyxPythonModuleExampleWidget
#
...
class EndoscopyWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        cameraNodeSelector.connect('currentNodeChanged(vtkDMMLNode*)', self.setCameraNode)
        pathFormLayout.addRow("Camera:", cameraNodeSelector)

        # Input fiducials node selector
        inputFiducialsNodeSelector = cjyx.qDMMLNodeComboBox()
        ...
        inputFiducialsNodeSelector.nodeTypes = ['vtkDMMLMarkupsFiducialNode', 'vtkDMMLMarkupsCurveNode',
        ...
        outputPathNodeSelector = cjyx.qDMMLNodeComboBox()
        ...
        outputPathNodeSelector.nodeTypes = ['vtkDMMLModelNode']
        ...
        cameraNodeSelector.setDMMLScene(cjyx.dmmlScene)
        inputFiducialsNodeSelector.setDMMLScene(cjyx.dmmlScene)
        outputPathNodeSelector.setDMMLScene(cjyx.dmmlScene)
    ...
    def setCameraNode(self, newCameraNode):
        ...
        # Update UI
        self.updateWidgetFromDMML()

    def updateWidgetFromDMML(self):
        if self.camera:
    ...
    def flyTo(self, pathPointIndex):
        ...
        # using with CjyxIGT extension's VolumeResliceDriver module.
        import numpy as np
...
class EndoscopyComputePath:
    ...
    def __init__(self, fiducialListNode, dl=0.5):
        ...
        if (self.fids.GetClassName() == "vtkDMMLMarkupsCurveNode"
                or self.fids.GetClassName() == "vtkDMMLMarkupsClosedCurveNode"):
            ...
            cjyx.vtkDMMLMarkupsCurveNode.ResamplePoints(self.fids.GetCurvePointsWorld(), resampledPoints, self.dl, self.fids.GetCurveClosed())
        ...
        if self.fids.GetClassName() == "vtkDMMLAnnotationHierarchyNode":
            # cjyx4 style hierarchy nodes
            ...
            for i in range(self.n):
                ...
                self.p[i] = coords
        elif self.fids.GetClassName() == "vtkDMMLMarkupsFiducialNode":
            # cjyx4 Markups node
...
class EndoscopyPathModel:
    ...
    def __init__(self, path, fiducialListNode, outputPathNode=None, cursorType=None):
        ...
        if not model:
            model = scene.AddNewNodeByClass("vtkDMMLModelNode", scene.GenerateUniqueName("Path-%s" % fids.GetName()))
        ...
        if not cursor:
            ...
            if self.cursorType == "markups":
                # Markups cursor
                cursor = scene.AddNewNodeByClass("vtkDMMLMarkupsFiducialNode", scene.GenerateUniqueName("Cursor-%s" % fids.GetName()))
                ...
                # Model cursor
                cursor = scene.AddNewNodeByClass("vtkDMMLMarkupsModelNode", scene.GenerateUniqueName("Cursor-%s" % fids.GetName()))
                cursor.CreateDefaultDisplayNodes()
        ...
        if not transform:
            transform = scene.AddNewNodeByClass("vtkDMMLLinearTransformNode", scene.GenerateUniqueName("Transform-%s" % fids.GetName()))
