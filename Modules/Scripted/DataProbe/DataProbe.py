...
"""
        ...
        parent.acknowledgementText = """This work is supported by NA-MIC, NAC, NCIGT, NIH U24 CA180918 (PIs Kikinis and Fedorov) and the Cjyx Community."""
        # TODO: need a DataProbe icon
        # parent.icon = qt.QIcon(':Icons/XLarge/CjyxDownloadMRHead.png')
        ...
        if not cjyx.app.commandOptions().noMainWindow:
            cjyx.app.connect("startupCompleted()", self.addView)
    ...
    def addView(self):
        ...
        try:
            ...
            parent = cjyx.util.findChild(mw, "DataProbeCollapsibleWidget")
        except IndexError:
...
class DataProbeInfoWidget:
    ...
    def __init__(self, parent=None):
        ...
        modulePath = cjyx.modules.dataprobe.path.replace("DataProbe.py", "")
        ...
        self.CrosshairNode = cjyx.dmmlScene.GetFirstNodeByClass('vtkDMMLCrosshairNode')
        if self.CrosshairNode:
            self.CrosshairNodeObserverTag = self.CrosshairNode.AddObserver(cjyx.vtkDMMLCrosshairNode.CursorPositionModifiedEvent, self.processEvent)
    ...
    def getPixelString(self, volumeNode, ijk):
        ...
        if volumeNode.IsA("vtkDMMLDiffusionTensorVolumeNode"):
    ...
    def processEvent(self, observee, event):
        ...
        if sliceNode:
            sliceWidget = cjyx.app.layoutManager().sliceWidget(sliceNode.GetName())
            if sliceWidget:
        ...
        # set image
        if (not cjyx.dmmlScene.IsBatchProcessing()) and sliceLogic and hasVolume and self.showImage:
        ...
        if hasattr(self.frame.parent(), 'text'):
            sceneName = cjyx.dmmlScene.GetURL()
            if sceneName != "":
    ...
    def generateViewDescription(self, xyz, ras, sliceNode, sliceLogic):
        ...
        # Note that 'xyz' is unused in the Cjyx implementation but could
        ...
        if sliceNode.GetSliceSpacingMode() == cjyx.vtkDMMLSliceNode.PrescribedSliceSpacingMode:
            spacing = "(%s)" % spacing
    ...
    def generateLayerName(self, cjyxLayerLogic):
        volumeNode = cjyxLayerLogic.GetVolumeNode()
        return "<b>%s</b>" % (self.fitName(volumeNode.GetName()) if volumeNode else "None")

    def generateIJKPixelDescription(self, ijk, cjyxLayerLogic):
        volumeNode = cjyxLayerLogic.GetVolumeNode()
        return f"({ijk[0]:3d}, {ijk[1]:3d}, {ijk[2]:3d})" if volumeNode else ""

    def generateIJKPixelValueDescription(self, ijk, cjyxLayerLogic):
        volumeNode = cjyxLayerLogic.GetVolumeNode()
    ...
    def _createMagnifiedPixmap(self, xyz, inputImageDataConnection, outputSize, crosshairColor, imageZoom=10):
        ...
        if (imin_trunc <= imax_trunc) and (jmin_trunc <= jmax_trunc):
            ...
            if vtkImageFromCanvas:
                qImage = qt.QImage()
                cjyx.qDMMLUtils().vtkImageDataToQImage(vtkImageFromCanvas, qImage)
    ...
    def _createSmall(self):
        ...
        Data Probe frame (lower left of cjyx main window by default)"""
...
class DataProbeWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        dataProbeInstance = cjyx.modules.DataProbeInstance
