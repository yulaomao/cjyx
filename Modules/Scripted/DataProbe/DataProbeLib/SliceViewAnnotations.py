...
import cjyx
from cjyx.util import settingsValue
...
class SliceAnnotations(VTKObservationMixin):
    ...
    def __init__(self, layoutManager=None):
        ...
        if self.layoutManager is None:
            self.layoutManager = cjyx.app.layoutManager()
        ...
        self.addObserver(self.parameterNode, vtk.vtkCommand.ModifiedEvent, self.updateGUIFromDMML)
    ...
    def updateSliceViewFromGUI(self):
        if not self.sliceViewAnnotationsEnabled:
            ...
            self.removeObservers(method=self.updateGUIFromDMML)
            return
    ...
    def extractDICOMValues(self, uid):
        ...
        for tag in tags.keys():
            value = cjyx.dicomDatabase.instanceValue(uid, tag)
            p[tags[tag]] = value
