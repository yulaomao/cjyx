...
class CropVolumeSequenceWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        #
        self.inputSelector = cjyx.qDMMLNodeComboBox()
        self.inputSelector.nodeTypes = ["vtkDMMLSequenceNode"]
        self.inputSelector.addEnabled = False
        self.inputSelector.removeEnabled = False
        self.inputSelector.noneEnabled = False
        self.inputSelector.showHidden = False
        self.inputSelector.showChildNodeTypes = False
        self.inputSelector.setDMMLScene(cjyx.dmmlScene)
        ...
        #
        self.outputSelector = cjyx.qDMMLNodeComboBox()
        self.outputSelector.nodeTypes = ["vtkDMMLSequenceNode"]
        ...
        self.outputSelector.setDMMLScene(cjyx.dmmlScene)
        ...
        #
        self.cropParametersSelector = cjyx.qDMMLNodeComboBox()
        self.cropParametersSelector.nodeTypes = ["vtkDMMLCropVolumeParametersNode"]
        ...
        self.cropParametersSelector.setDMMLScene(cjyx.dmmlScene)
        ...
        self.inputSelector.connect("currentNodeChanged(vtkDMMLNode*)", self.onSelect)
        self.cropParametersSelector.connect("currentNodeChanged(vtkDMMLNode*)", self.onSelect)
    ...
    def onEditCropParameters(self):
        if not self.cropParametersSelector.currentNode():
            cropParametersNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLCropVolumeParametersNode")
            self.cropParametersSelector.setCurrentNode(cropParametersNode)
        if self.inputSelector.currentNode():
            inputVolSeq = self.inputSelector.currentNode()
            seqBrowser = cjyx.modules.sequences.logic().GetFirstBrowserNodeForSequenceNode(inputVolSeq)
        ...
        cjyx.app.openNodeModule(self.cropParametersSelector.currentNode())
...
class CropVolumeSequenceLogic(ScriptedLoadableModuleLogic):
    ...
    def transformForSequence(self, volumeSeq):
        seqBrowser = cjyx.modules.sequences.logic().GetFirstBrowserNodeForSequenceNode(volumeSeq)
        if not seqBrowser:
            return None
    ...
    def run(self, inputVolSeq, outputVolSeq, cropParameters):
        ...
        seqBrowser = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLSequenceBrowserNode")
        ...
        cjyx.modules.sequences.logic().UpdateAllProxyNodes()
        ...
        if outputVolSeq:
            ...
            outputVolume = cjyx.dmmlScene.AddNewNodeByClass(inputVolume.GetClassName())
        ...
        cjyx.modules.sequences.logic().GetBrowserNodesForSequenceNode(outputVolSeq, browserNodesForOutputSequence)
        ...
        try:
            ...
            for seqItemNumber in range(numberOfDataNodes):
                ...
                cjyx.modules.sequences.logic().UpdateProxyNodesFromSequences(seqBrowser)
                cjyx.modules.cropvolume.logic().Apply(cropParameters)
                if outputVolSeq:
            ...
            if outputVolume:
                cjyx.dmmlScene.RemoveNode(outputVolume)
            # Temporary input browser node
            cjyx.dmmlScene.RemoveNode(seqBrowser)
            # Temporary input volume proxy node
            cjyx.dmmlScene.RemoveNode(inputVolume)

            # Move output sequence node in the same browser node as the input volume sequence
            # if not in a sequence browser node already.
            if outputVolSeq:

                if cjyx.modules.sequences.logic().GetFirstBrowserNodeForSequenceNode(outputVolSeq) is None:
                    ...
                    outputVolume.SetAndObserveTransformNodeID(outputVolTransformNodeID)
                    cjyx.util.setSliceViewerLayers(background=outputVolume)
...
class CropVolumeSequenceTest(ScriptedLoadableModuleTest):
    ...
    def test_CropVolumeSequence1(self):
        ...
        sequenceBrowserNode = cjyx.modules.sequences.logic().GetFirstBrowserNodeForSequenceNode(sequenceNode)

        # Set cropping parameters
        croppedSequenceNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLSequenceNode')
        cropVolumeNode = cjyx.dmmlScene.AddNewNodeByClass('vtkDMMLCropVolumeParametersNode')
        ...
        cjyx.modules.cropvolume.logic().FitROIToInputVolume(cropVolumeNode)
