...
class PerformanceTests(ScriptedLoadableModule):
    def __init__(self, parent):
        ...
        parent.helpText = """
    Module to run interactive performance tests on the core of cjyx.
    """
...
class PerformanceTestsWidget(ScriptedLoadableModuleWidget):
    ...
    def reslicing(self, iters=100):
        ...
        import math
        import numpy as np
        sliceNode = cjyx.util.getNode('vtkDMMLSliceNodeRed')
        ...
        resultTableName = cjyx.dmmlScene.GetUniqueNameByString("Reslice performance")
        resultTableNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLTableNode", resultTableName)
        cjyx.util.updateTableFromArray(resultTableNode, renderingTimesSec, "Rendering time [s]")
    ...
    def crosshairJump(self, iters=15):
        ...
        for i in range(iters):
            startTime = time.time()
            cjyx.util.clickAndDrag(firstSliceWidget, button=None, modifiers=['Shift'], start=startPoint, end=endPoint, steps=2)
            cjyx.app.processEvents()
            endTime1 = time.time()
            cjyx.util.clickAndDrag(firstSliceWidget, button=None, modifiers=['Shift'], start=endPoint, end=startPoint, steps=2)
            cjyx.app.processEvents()
            endTime2 = time.time()
    ...
    def webViewFormLoadedCallback(self, ok):
        ...
        element.setAttribute("value", "where can I learn more about this 3D Cjyx program?")
    ...
    def memoryCheck(self):
        ...
        if not hasattr(self, 'sysInfo'):
            self.sysInfo = cjyx.vtkSystemInformation()
...
class sliceLogicTest:
    def __init__(self):
        self.step = 0
        self.sliceLogic = cjyx.vtkDMMLSliceLayerLogic()
        self.sliceLogic.SetDMMLScene(cjyx.dmmlScene)
        self.sliceNode = cjyx.vtkDMMLSliceNode()
        self.sliceNode.SetLayoutName("Black")
        cjyx.dmmlScene.AddNode(self.sliceNode)
