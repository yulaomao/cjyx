...
import CjyxWizard.ExtensionDescription
import CjyxWizard.ExtensionProject
import CjyxWizard.TemplateManager
import CjyxWizard.Utilities
...
def _settingsList(settings, key, convertToAbsolutePaths=False):
    ...
    if convertToAbsolutePaths:
        absolutePaths = []
        for path in value:
            absolutePaths.append(cjyx.app.toCjyxHomeAbsolutePath(path))
...
class ExtensionWizard:
    ...
    def __init__(self, parent):
        ...
        parent.helpText = """
This module provides tools to create and manage extensions from within Cjyx.
"""
...
"""
        ...
        cjyx.app.settingsDialog().addPanel("Extension Wizard", self.settingsPanel)
...
class ExtensionWizardWidget:
    ...
    def setupTemplates(self):
        self.templateManager = CjyxWizard.TemplateManager()
    ...
    def createExtension(self):
        ...
        while dlg.exec_() == qt.QDialog.Accepted:
            ...
            # as destination
            try:
                repo = CjyxWizard.Utilities.getRepo(dlg.destination)
                ...
                if repo is None:
                    ...
                    destination = CjyxWizard.Utilities.localRoot(repo)
                ...
                if not cjyx.util.confirmRetryCloseDisplay("An error occurred while trying to create the extension.",
    ...
    def selectExtension(self, path=None):
        ...
        try:
            repo = CjyxWizard.Utilities.getRepo(path)

            xd = None
            if repo:
                try:
                    xd = CjyxWizard.ExtensionDescription(repo=repo)
                    path = CjyxWizard.Utilities.localRoot(repo)
                except:
            ...
            if not xd:
                xd = CjyxWizard.ExtensionDescription(sourcedir=path)

            xp = CjyxWizard.ExtensionProject(path)

        except:
            cjyx.util.errorDisplay("Failed to open extension '%s'." % path, parent=self.parent.window(),
    ...
    def loadModules(self, path, depth=1):
        ...
        factory = cjyx.app.moduleManager().factoryManager()
        ...
        if len(candidates):
            ...
            if dlg.exec_() == qt.QDialog.Accepted:
                ...
                if dlg.addToSearchPaths:
                    settings = cjyx.app.revisionUserSettings()
                    ...
                    if modified:
                        settings.setValue("Modules/AdditionalPaths", cjyx.app.toCjyxHomeRelativePaths(rawSearchPaths))
                ...
                if len(failed):
                    ...
                    cjyx.util.errorDisplay(text, parent=self.parent.window(), windowTitle="Module loading failed",
                ...
                if not factory.loadModules([module.key for module in modulesToLoad]):
                    ...
                    cjyx.util.errorDisplay(text, parent=self.parent.window(), windowTitle="Error loading module(s)",
    ...
    def createExtensionModule(self):
        ...
        while dlg.exec_() == qt.QDialog.Accepted:
            ...
            try:
                ...
                if not cjyx.util.confirmRetryCloseDisplay("An error occurred while trying to create the module.",
            ...
            try:
                ...
                cjyx.util.errorDisplay(text, parent=self.parent.window(), detailedText=traceback.format_exc(),
                                         standardButtons=qt.QMessageBox.Close, informativeText=detailedInformation)
