...
class EditExtensionMetadataDialog:
    ...
    def accept(self):
        if not len(self.project):
            cjyx.util.errorDisplay("Extension name may not be empty.", windowTitle="Invalid metadata", parent=self.dialog)
            return

        if not len(self.description):
            cjyx.util.errorDisplay("Extension description may not be empty.",
