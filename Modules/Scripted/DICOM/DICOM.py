...
import cjyx
from cjyx.util import settingsValue, toBool
...
# DICOM
#
# This code includes the GUI for the cjyx module.  It is supported
...
"""
        self.parent.icon = qt.QIcon(':Icons/Medium/CjyxLoadDICOM.png')
        ...
        self.browserWidget = None  # CjyxDICOMBrowser instance (ctkDICOMBrowser with additional section for loading the selected items)
    ...
    def setup(self):
        ...
        cjyx.app.connect("startupCompleted()", self.performPostModuleDiscoveryTasks)
        cjyx.app.connect("urlReceived(QString)", self.onURLReceived)

        pluginHandlerSingleton = cjyx.qCjyxSubjectHierarchyPluginHandler.instance()
        pluginHandlerSingleton.registerPlugin(cjyx.qCjyxSubjectHierarchyDICOMPlugin())

        self.viewFactory = cjyx.qCjyxSingletonViewFactory()
        self.viewFactory.setTagName("dicombrowser")
        if cjyx.app.layoutManager() is not None:
            cjyx.app.layoutManager().registerViewFactory(self.viewFactory)
    ...
    def performPostModuleDiscoveryTasks(self):
        ...
        if cjyx.dmmlScene.GetTagByClassName("vtkDMMLScriptedModuleNode") != 'ScriptedModule':
            cjyx.dmmlScene.RegisterNodeClass(vtkDMMLScriptedModuleNode())

        self.initializeDICOMDatabase()

        settings = qt.QSettings()
        if settings.contains('DICOM/RunListenerAtStart') and not cjyx.app.commandOptions().testingEnabled:
            if settings.value('DICOM/RunListenerAtStart') == 'true':
                self.startListener()

        if not cjyx.app.commandOptions().noMainWindow:
            cjyx.util.mainWindow().findChild(qt.QAction, "LoadDICOMAction").setVisible(True)
            # add to the main app file menu
            self.addMenu()
            # add the settings options
            self.settingsPanel = DICOMSettingsPanel()
            cjyx.app.settingsDialog().addPanel("DICOM", self.settingsPanel)
    ...
    def initializeDICOMDatabase(self):
        ...
        cjyx.dicomDatabase = cjyx.app.dicomDatabase()
        ...
        tagsToPrecache = list(cjyx.dicomDatabase.tagsToPrecache)
        for pluginClass in cjyx.modules.dicomPlugins:
            plugin = cjyx.modules.dicomPlugins[pluginClass]()
            tagsToPrecache += list(plugin.tags.values())
        tagsToPrecache = sorted(set(tagsToPrecache))  # remove duplicates
        cjyx.dicomDatabase.tagsToPrecache = tagsToPrecache

        # Try to initialize the database using the location stored in settings
        if cjyx.app.commandOptions().testingEnabled:
            # For automatic tests (use a separate DICOM database for testing)
            cjyx.dicomDatabaseDirectorySettingsKey = 'DatabaseDirectoryTest_' + ctk.ctkDICOMDatabase().schemaVersion()
            databaseDirectory = os.path.join(cjyx.app.temporaryPath,
                                             'temp' + cjyx.app.applicationName + 'DICOMDatabase_' + ctk.ctkDICOMDatabase().schemaVersion())
        else:
            # For production
            cjyx.dicomDatabaseDirectorySettingsKey = 'DatabaseDirectory_' + ctk.ctkDICOMDatabase().schemaVersion()
            settings = qt.QSettings()
            databaseDirectory = settings.value(cjyx.dicomDatabaseDirectorySettingsKey)
            if not databaseDirectory:
                documentsLocation = qt.QStandardPaths.DocumentsLocation
                documents = qt.QStandardPaths.writableLocation(documentsLocation)
                databaseDirectory = os.path.join(documents, cjyx.app.applicationName + "DICOMDatabase")
                settings.setValue(cjyx.dicomDatabaseDirectorySettingsKey, databaseDirectory)
        ...
        cjyx.dicomDatabase.openDatabase(databaseFileName)
        if cjyx.dicomDatabase.isOpen:
            # There is an existing database at the current location
            if cjyx.dicomDatabase.schemaVersionLoaded() != cjyx.dicomDatabase.schemaVersion():
                # Schema does not match, do not use it
                cjyx.dicomDatabase.closeDatabase()

    def startListener(self):

        if not cjyx.dicomDatabase.isOpen:
            logging.error("Failed to start DICOM listener. DICOM database is not open.")
            return False

        if not hasattr(cjyx, 'dicomListener'):
            dicomListener = DICOMLib.DICOMListener(cjyx.dicomDatabase)
        ...
        if not dicomListener.process:
            ...
            return False
        cjyx.dicomListener = dicomListener
        logging.info("DICOM C-Store SCP service started at port " + str(cjyx.dicomListener.port))

    def stopListener(self):
        if hasattr(cjyx, 'dicomListener'):
            logging.info("DICOM C-Store SCP service stopping")
            cjyx.dicomListener.stop()
            del cjyx.dicomListener
    ...
    def addMenu(self):
        ...
        fileMenu = cjyx.util.lookupTopLevelWidget('FileMenu')
        if fileMenu:
    ...
    def onLayoutChanged(self, viewArrangement):
        ...
        if (self.currentViewArrangement != cjyx.vtkDMMLLayoutNode.CjyxLayoutNone and
                self.currentViewArrangement != cjyx.vtkDMMLLayoutNode.CjyxLayoutDicomBrowserView):
        ...
        if self.browserWidget is None:
            return
        mw = cjyx.util.mainWindow()
        dataProbe = mw.findChild("QWidget", "DataProbeCollapsibleWidget") if mw else None
        if self.currentViewArrangement == cjyx.vtkDMMLLayoutNode.CjyxLayoutDicomBrowserView:
            ...
            try:
                inDicomModule = cjyx.modules.dicom.widgetRepresentation().isEntered
            except AttributeError:
                # Cjyx is shutting down
                inDicomModule = False
    ...
    def onBrowserWidgetClosed(self):
        if (self.currentViewArrangement != cjyx.vtkDMMLLayoutNode.CjyxLayoutDicomBrowserView and
                self.currentViewArrangement != cjyx.vtkDMMLLayoutNode.CjyxLayoutNone):
        ...
        if (layoutId == cjyx.vtkDMMLLayoutNode.CjyxLayoutNone
                or layoutId == cjyx.vtkDMMLLayoutNode.CjyxLayoutDicomBrowserView):
            layoutId = qt.QSettings().value("MainWindow/layout", cjyx.vtkDMMLLayoutNode.CjyxLayoutInitialView)

        cjyx.app.layoutManager().setLayout(layoutId)

    def _onModuleAboutToBeUnloaded(self, moduleName):
        # Application is shutting down. Stop the listener.
        if moduleName == "DICOM":
            self.stopListener()
            cjyx.app.moduleManager().disconnect(
                'moduleAboutToBeUnloaded(QString)', self._onModuleAboutToBeUnloaded)
...
class _ui_DICOMSettingsPanel:
    def __init__(self, parent):
        ...
        parent.registerProperty(cjyx.dicomDatabaseDirectorySettingsKey, directoryButton,
        ...
        plugins = cjyx.modules.dicomPlugins
...
class DICOMFileDialog:
    ...
    def __init__(self, qCjyxFileDialog):
        ...
        qCjyxFileDialog.fileType = 'DICOM Directory'
        qCjyxFileDialog.description = 'Load directory into DICOM database'
    ...
    def isMimeDataAccepted(self):
        ...
        more directories"""
        self.directoriesToAdd, _ = DICOMFileDialog.pathsFromMimeData(self.qCjyxFileDialog.mimeData())
        self.qCjyxFileDialog.acceptMimeData(len(self.directoriesToAdd) != 0)
    ...
    def createDefaultDatabase():
        ...
        """
        if cjyx.dicomDatabase and cjyx.dicomDatabase.isOpen:
            # Valid DICOM database already exists
            return True

        # Try to create a database with default settings
        if cjyx.modules.DICOMInstance.browserWidget is None:
            cjyx.util.selectModule('DICOM')
        cjyx.modules.DICOMInstance.browserWidget.dicomBrowser.createNewDatabaseDirectory()
        if cjyx.dicomDatabase and cjyx.dicomDatabase.isOpen:
        ...
        cjyx.util.selectModule('DICOM')
        cjyx.modules.dicom.widgetRepresentation().self().onOpenBrowserWidget()
        cjyx.util.warningDisplay("Could not create a DICOM database with default settings. Please create a new database or"
    ...
    def dropEvent(self):
        ...
        if not DICOMFileDialog.validDirectories(self.directoriesToAdd):
            if not cjyx.util.confirmYesNoDisplay("Import of files that have special (non-ASCII) characters in their names is not supported."
                                                   " It is recommended to move files into a different folder and retry. Try to import from current location anyway?"):
                self.directoriesToAdd = []
                return

        cjyx.util.selectModule('DICOM')
        cjyx.modules.DICOMInstance.browserWidget.dicomBrowser.importDirectories(self.directoriesToAdd)
        self.directoriesToAdd = []
...
class DICOMLoadingByDragAndDropEventFilter(qt.QWidget):
    ...
    def eventFilter(self, object, event):
        """
        Custom event filter for Cjyx Main Window.
    ...
    def dropEvent(self, event):
        ...
        if not DICOMFileDialog.validDirectories(self.directoriesToAdd) or not DICOMFileDialog.validDirectories(self.filesToAdd):
            if not cjyx.util.confirmYesNoDisplay("Import from folders with special (non-ASCII) characters in the name is not supported."
                                                   " It is recommended to move files into a different folder and retry. Try to import from current location anyway?"):
                self.directoriesToAdd = []
                return

        cjyx.modules.DICOMInstance.browserWidget.dicomBrowser.importDirectories(self.directoriesToAdd)
        cjyx.modules.DICOMInstance.browserWidget.dicomBrowser.importFiles(self.filesToAdd)
...
class DICOMWidget(ScriptedLoadableModuleWidget):
    """
    Cjyx module that creates the Qt GUI for interacting with DICOM
    """
    ...
    def setup(self):
        ...
        cjyx.modules.DICOMInstance.performPostModuleDiscoveryTasks()
        ...
        uiWidget = cjyx.util.loadUI(self.resourcePath('UI/DICOM.ui'))
        self.layout.addWidget(uiWidget)
        self.ui = cjyx.util.childWidgetVariables(uiWidget)

        self.browserWidget = DICOMLib.CjyxDICOMBrowser()
        self.browserWidget.objectName = 'CjyxDICOMBrowser'

        cjyx.modules.DICOMInstance.setBrowserWidgetInDICOMLayout(self.browserWidget)

        layoutManager = cjyx.app.layoutManager()
        if layoutManager is not None:
            layoutManager.layoutChanged.connect(self.onLayoutChanged)
            viewArrangement = cjyx.app.layoutManager().layoutLogic().GetLayoutNode().GetViewArrangement()
            self.ui.showBrowserButton.checked = (viewArrangement == cjyx.vtkDMMLLayoutNode.CjyxLayoutDicomBrowserView)
        ...
        self.ui.subjectHierarchyTree.setDMMLScene(cjyx.dmmlScene)
        ...
        if mw:
            try:
                action = cjyx.util.findChildren(mw, name='LoadDICOMAction')[0]
    ...
    def addListenerObservers(self):
        if not hasattr(cjyx, 'dicomListener'):
            return
        if cjyx.dicomListener.process is not None:
            cjyx.dicomListener.process.connect('stateChanged(QProcess::ProcessState)', self.onListenerStateChanged)
        cjyx.dicomListener.fileToBeAddedCallback = self.onListenerToAddFile
        cjyx.dicomListener.fileAddedCallback = self.onListenerAddedFile

    def removeListenerObservers(self):
        if not hasattr(cjyx, 'dicomListener'):
            return
        if cjyx.dicomListener.process is not None:
            cjyx.dicomListener.process.disconnect('stateChanged(QProcess::ProcessState)', self.onListenerStateChanged)
        cjyx.dicomListener.fileToBeAddedCallback = None
        cjyx.dicomListener.fileAddedCallback = None

    def updateGUIFromDMML(self, caller, event):
        pass
    ...
    def onCurrentItemChanged(self, id):
        plugin = cjyx.qCjyxSubjectHierarchyPluginHandler.instance().getOwnerPluginForSubjectHierarchyItem(id)
        if not plugin:
            self.subjectHierarchyCurrentVisibility = False
            return
    ...
    def onOpenBrowserWidget(self):
        cjyx.app.layoutManager().setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutDicomBrowserView)

    def onToggleListener(self, toggled):
        if hasattr(cjyx, 'dicomListener'):
            self.removeListenerObservers()
            cjyx.modules.DICOMInstance.stopListener()
        if toggled:
            cjyx.modules.DICOMInstance.startListener()
            self.addListenerObservers()
        self.onListenerStateChanged()
    ...
    def onListenerStateChanged(self, newState=None):
        ...
        """
        if hasattr(cjyx, 'dicomListener') and cjyx.dicomListener.process is not None:
            newState = cjyx.dicomListener.process.state()
        else:
            newState = 0
        ...
        if newState == 0:
            ...
            if hasattr(cjyx.modules, 'DICOMInstance'):  # custom applications may not have the standard DICOM module
                cjyx.modules.DICOMInstance.stopListener()
        if newState == 1:
            self.ui.listenerStateLabel.text = "starting"
        if newState == 2:
            port = str(cjyx.dicomListener.port) if hasattr(cjyx, 'dicomListener') else "unknown"
    ...
    def onListenerAddedFile(self):
        ...
        """
        newFile = cjyx.dicomListener.lastFileAdded
        if newFile:
            cjyx.util.showStatusMessage("Received DICOM file: %s" % newFile, 1000)
        self.databaseRefreshRequestTimer.start()

    def requestDatabaseRefresh(self):
        logging.debug("Database refresh is requested")
        if cjyx.dicomDatabase.isOpen:
            cjyx.dicomDatabase.databaseChanged()
    ...
    def onToggleServer(self):
        if self.testingServer and self.testingServer.qrRunning():
            ...
            if not self.testingServer:
                ...
                self.exeDir = cjyx.app.cjyxHome
                if cjyx.app.intDir:
                    self.exeDir = self.exeDir + '/' + cjyx.app.intDir
                self.exeDir = self.exeDir + '/../CTK-build/DCMTK-build'

                # TODO: deal with Debug/RelWithDebInfo on windows

                # set up temp dir
                tmpDir = cjyx.app.temporaryPath
            ...
            # with standard naming conventions)
            self.dataDir = cjyx.app.cjyxHome + '/../../Cjyx4/Testing/Data/Input/CTHeadAxialDicom'
    ...
    def onClearDatabase(self):
        patientIds = cjyx.dicomDatabase.patients()
        if len(patientIds) == 0:
            cjyx.util.infoDisplay("DICOM database is already empty.")
        elif not cjyx.util.confirmYesNoDisplay(
        ...
        DICOMLib.clearDatabase(cjyx.dicomDatabase)
