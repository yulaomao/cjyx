...
DICOMExportScalarVolume provides the feature of exporting a cjyx
...
class DICOMExportScalarVolume:
    """Code to export cjyx data to dicom database
    ...
    # def parametersFromStudy(self,studyUID=None):
        ...
        # # TODO: we should install dicom.dic with cjyx and use it to
        ...
        # p = {}
        # if studyUID:
        #     series = cjyx.dicomDatabase.seriesForStudy(studyUID)
        #     # first find a unique series number
        #     for serie in series:
        #         files = cjyx.dicomDatabase.filesForSeries(serie, 1)
        #         if len(files):
        #             cjyx.dicomDatabase.loadFileHeader(files[0])
        #             dump = cjyx.dicomDatabase.headerValue('0020,0011')
        #             try:
        ...
        #         files = cjyx.dicomDatabase.filesForSeries(series[0], 1)
        #         if len(files):
        #             self.referenceFile = files[0]
        #             cjyx.dicomDatabase.loadFileHeader(self.referenceFile)
        #             for tag in tags.keys():
        #                 dump = cjyx.dicomDatabase.headerValue(tag)
        #                 try:
        #                     value = dump[dump.index('[')+1:dump.index(']')]
        #                 except ValueError:
        #                     value = "Unknown"
    ...
    def export(self):
        ...
        if displayNode:
            if displayNode.IsA('vtkDMMLScalarVolumeDisplayNode'):
        ...
        #
        if not hasattr(cjyx.modules, 'createdicomseries'):
            logging.error("CreateDICOMSeries module is not found")
            return False
        dicomWrite = cjyx.modules.createdicomseries
        cliNode = cjyx.cli.run(dicomWrite, None, cliparameters, wait_for_completion=True)
