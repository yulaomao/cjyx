...
to manage DICOM data in the context of cjyx.

This code is cjyx-specific and relies on the cjyx python module
for elements like cjyx.dicomDatabase and cjyx.dmmlScene
...
class CjyxDICOMBrowser(VTKObservationMixin, qt.QWidget):
    ...
    def __init__(self, dicomBrowser=None, parent="mainWindow"):
        ...
        qt.QWidget.__init__(self, cjyx.util.mainWindow() if parent == "mainWindow" else parent)
        ...
        self.dicomBrowser = dicomBrowser if dicomBrowser is not None else cjyx.app.createDICOMBrowserForMainDatabase()
    ...
    def setup(self, showPreview=False):
        ...
        widget repacked into it along with cjyx-specific
        extra widgets
        """
        ...
        self.dicomBrowser.databaseDirectorySettingsKey = cjyx.dicomDatabaseDirectorySettingsKey
        ...
        #
        # action related column (interacting with cjyx)
        #
    ...
    def onDirectoryImported(self):
        ...
        if not hasattr(cjyx.app, 'extensionsManagerModel'):
            # Cjyx may not be built with extensions manager support
            return
    ...
    def promptForExtensions(self):
        ...
        if len(extensionsToOffer) != 0:
            ...
            cjyx.util.infoDisplay(message, parent=self, windowTitle='DICOM')
    ...
    def checkForExtensions(self):
        ...
        modulePath = os.path.dirname(cjyx.modules.dicom.path)
        ...
        sopClassUIDs = set()
        modalities = set()
        for patient in cjyx.dicomDatabase.patients():
            for study in cjyx.dicomDatabase.studiesForPatient(patient):
                for series in cjyx.dicomDatabase.seriesForStudy(study):
                    instance0 = cjyx.dicomDatabase.filesForSeries(series, 1)[0]
                    modality = cjyx.dicomDatabase.fileValue(instance0, modalityTag)
                    sopClassUID = cjyx.dicomDatabase.fileValue(instance0, sopClassUIDTag)
                    modalities.add(modality)
                    sopClassUIDs.add(sopClassUID)

        # 3 - check if data matches
        extensionsManagerModel = cjyx.app.extensionsManagerModel()
    ...
    def getFileListsForRole(self, uidArgument, role):
        fileLists = []
        if role == "Series":
            fileLists.append(cjyx.dicomDatabase.filesForSeries(uidArgument))
        if role == "SeriesUIDList":
            for uid in uidArgument:
                uid = uid.replace("'", "")
                fileLists.append(cjyx.dicomDatabase.filesForSeries(uid))
        if role == "Study":
            series = cjyx.dicomDatabase.seriesForStudy(uidArgument)
            for serie in series:
                fileLists.append(cjyx.dicomDatabase.filesForSeries(serie))
        if role == "Patient":
            studies = cjyx.dicomDatabase.studiesForPatient(uidArgument)
            for study in studies:
                series = cjyx.dicomDatabase.seriesForStudy(study)
                for serie in series:
                    fileList = cjyx.dicomDatabase.filesForSeries(serie)
    ...
    def getLoadablesFromFileLists(self, fileLists):
        ...
        if missingFileCount < allFileCount:
            progressDialog = cjyx.util.createProgressDialog(parent=self, value=0, maximum=100)
    ...
    def loadCheckedLoadables(self):
        ...
        (DICOMLoadable or qCjyxDICOMLoadable) instances that are selected"""
        if self.advancedViewButton.checkState() == 0:
            self.examineForLoading()
            ...
            for loadable in self.loadablesByPlugin[plugin]:
                if hasattr(loadable, 'referencedInstanceUIDs'):
                    ...
                    for instance in loadable.referencedInstanceUIDs:
                        instanceFile = cjyx.dicomDatabase.fileForInstance(instance)
                        if instanceFile != '':
        ...
        automaticallyLoadReferences = int(cjyx.util.settingsValue('DICOM/automaticallyLoadReferences', qt.QMessageBox.InvalidRole))
    ...
    def proceedWithReferencedLoadablesSelection(self):
        ...
        cjyx.app.ioManager().emitNewFileLoaded(loadedFileParameters)
    ...
    def warnUserIfLoadableWarningsAndProceed(self):
        ...
        if warningsInSelectedLoadables:
            ...
            if not cjyx.util.confirmOkCancelDisplay(warning, parent=self, detailedText=details):
                return False
        return True
...
class DICOMLoadableTable(qt.QTableWidget):
    ...
    selectable cjyx data to be made from
    ...
    def __init__(self, parent, width=350, height=100):
        ...
        self.setLoadables([])
        self.configure()
        cjyx.app.connect('aboutToQuit()', self.deleteLater)
