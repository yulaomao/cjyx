...
#ifndef __qCjyxDICOMLoadable_h
#define __qCjyxDICOMLoadable_h
...
class vtkCjyxDICOMLoadable;

/// Container class for objects that can be loaded from DICOM files into Cjyx.
...
class Q_CJYX_MODULE_DICOMLIB_WIDGETS_EXPORT qCjyxDICOMLoadable : public QObject
{
  Q_OBJECT
...
public:
  ...
  qCjyxDICOMLoadable(QObject *parent = nullptr);
  ~qCjyxDICOMLoadable() override;
  ...
  Q_INVOKABLE void copyToVtkLoadable(vtkCjyxDICOMLoadable* vtkLoadable);

  /// Copy values from VTK loadable
  Q_INVOKABLE void copyFromVtkLoadable(vtkCjyxDICOMLoadable* vtkLoadable);

protected:
  QScopedPointer<qCjyxDICOMLoadablePrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qCjyxDICOMLoadable);
  Q_DISABLE_COPY(qCjyxDICOMLoadable);
};
