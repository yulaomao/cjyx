...
{
  Q_DECLARE_PUBLIC(qCjyxDICOMTagEditorWidget);
protected:
  qCjyxDICOMTagEditorWidget* const q_ptr;
public:
  qCjyxDICOMTagEditorWidgetPrivate(qCjyxDICOMTagEditorWidget& object);
  virtual void init();
  void setupUi(QWidget *qCjyxDICOMTagEditorWidget);
  ...
  qCjyxDICOMExportable* exportableForRowIndex(unsigned int row);
...
public:
  QList<qCjyxDICOMExportable*> Exportables;
  vtkDMMLScene* Scene;
  ...
  QMap<unsigned int, qCjyxDICOMExportable*> SeriesTagsHeaderRows;
...
//------------------------------------------------------------------------------
qCjyxDICOMTagEditorWidgetPrivate::qCjyxDICOMTagEditorWidgetPrivate(qCjyxDICOMTagEditorWidget& object)
  : q_ptr(&object)
{
  this->Scene = nullptr;
...
//------------------------------------------------------------------------------
void qCjyxDICOMTagEditorWidgetPrivate::init()
{
  Q_Q(qCjyxDICOMTagEditorWidget);
...
//------------------------------------------------------------------------------
void qCjyxDICOMTagEditorWidgetPrivate::setupUi(QWidget *qCjyxDICOMTagEditorWidget)
{
  if (qCjyxDICOMTagEditorWidget->objectName().isEmpty())
    {
    qCjyxDICOMTagEditorWidget->setObjectName(QString::fromUtf8("qCjyxDICOMTagEditorWidget"));
    }

  // Create layout
  QVBoxLayout* mainLayout = new QVBoxLayout(qCjyxDICOMTagEditorWidget);
  mainLayout->setSpacing(0);
  mainLayout->setContentsMargins(0, 0, 0, 0);

  // Create tags table
  this->TagsTable = new QTableWidget(qCjyxDICOMTagEditorWidget);
...
//------------------------------------------------------------------------------
void qCjyxDICOMTagEditorWidgetPrivate::initializeTagsTable()
{
  // Reset study header
...
//------------------------------------------------------------------------------
unsigned int qCjyxDICOMTagEditorWidgetPrivate::topSeriesHeaderRow()
{
...
//------------------------------------------------------------------------------
qCjyxDICOMExportable* qCjyxDICOMTagEditorWidgetPrivate::exportableForRowIndex(unsigned int row)
{
...
//------------------------------------------------------------------------------
void qCjyxDICOMTagEditorWidgetPrivate::insertTagsTableRow(unsigned int row)
{
  ...
  QMap<unsigned int, qCjyxDICOMExportable*> updatedSeriesTagsHeaderRows;
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
qCjyxDICOMTagEditorWidget::qCjyxDICOMTagEditorWidget(QWidget *parent)
  : QWidget(parent)
  , d_ptr(new qCjyxDICOMTagEditorWidgetPrivate(*this))
{
  Q_D(qCjyxDICOMTagEditorWidget);
  d->setupUi(this);
  d->init();
}

//------------------------------------------------------------------------------
qCjyxDICOMTagEditorWidget::~qCjyxDICOMTagEditorWidget() = default;

//------------------------------------------------------------------------------
void qCjyxDICOMTagEditorWidget::setDMMLScene(vtkDMMLScene* scene)
{
  Q_D(qCjyxDICOMTagEditorWidget);
  d->Scene = scene;
}
...
//------------------------------------------------------------------------------
void qCjyxDICOMTagEditorWidget::clear()
{
  Q_D(qCjyxDICOMTagEditorWidget);
...
//------------------------------------------------------------------------------
QString qCjyxDICOMTagEditorWidget::setExportables(QList<qCjyxDICOMExportable*> exportables)
{
  Q_D(qCjyxDICOMTagEditorWidget);

  if (!d->Scene)
    {
    QString error("Invalid DMML scene");
    qCritical() << Q_FUNC_INFO << ": " << error;
    return error;
    }
  vtkDMMLSubjectHierarchyNode* shNode = vtkDMMLSubjectHierarchyNode::GetSubjectHierarchyNode(d->Scene);
  if (!shNode)
    {
  ...
  foreach (qCjyxDICOMExportable* exportable, d->Exportables)
    {
    vtkIdType seriesItemID = exportable->subjectHierarchyItemID();
    if (seriesItemID == vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
      {
      ...
      if (studyItemID == vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
        {
  ...
  for ( std::vector<std::string>::iterator patientTagIt = patientTagNames.begin();
    ...
    {
    std::string tagAttributeName = vtkDMMLSubjectHierarchyConstants::GetDICOMAttributePrefix() + (*patientTagIt);
  ...
  for (std::vector<std::string>::iterator it = patientItemAttributeNames.begin(); it != patientItemAttributeNames.end(); ++it)
    ...
    if (attributeName.size() < vtkDMMLSubjectHierarchyConstants::GetDICOMAttributePrefix().size())
      {
      continue;
      }

    // Get attribute prefix from attribute name, and tag name (attribute name after prefix) and value
    std::string attributePrefix = attributeName.substr(0, vtkDMMLSubjectHierarchyConstants::GetDICOMAttributePrefix().size());
    QString tagName(attributeName.substr(vtkDMMLSubjectHierarchyConstants::GetDICOMAttributePrefix().size()).c_str());
    ...
    if (!attributePrefix.compare(vtkDMMLSubjectHierarchyConstants::GetDICOMAttributePrefix()))
      {
  ...
  for (std::vector<std::string>::iterator studyTagIt = studyTagNames.begin(); studyTagIt != studyTagNames.end(); ++studyTagIt)
    {
    std::string tagAttributeName = vtkDMMLSubjectHierarchyConstants::GetDICOMAttributePrefix() + (*studyTagIt);
  ...
  foreach (qCjyxDICOMExportable* exportable, d->Exportables)
    ...
    foreach (QString tagName, exportableTagsMap.keys())
      ...
      if ( vtkCjyxSubjectHierarchyModuleLogic::IsPatientTag(tagName.toUtf8().constData())
        || vtkCjyxSubjectHierarchyModuleLogic::IsStudyTag(tagName.toUtf8().constData()) )
        {
        continue;
        }
      ...
      std::string tagAttributeName = vtkDMMLSubjectHierarchyConstants::GetDICOMAttributePrefix()
...
//------------------------------------------------------------------------------
QList<qCjyxDICOMExportable*> qCjyxDICOMTagEditorWidget::exportables()const
{
  Q_D(const qCjyxDICOMTagEditorWidget);
  return d->Exportables;
}

//------------------------------------------------------------------------------
void qCjyxDICOMTagEditorWidget::commitChangesToItems()
{
  Q_D(qCjyxDICOMTagEditorWidget);
  ...
  std::string dicomAttributePrefix = vtkDMMLSubjectHierarchyConstants::GetDICOMAttributePrefix();
  vtkIdType studyItemID = vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID;

  // Commit changes to series
  for (int row=d->topSeriesHeaderRow(); row<d->TagsTable->rowCount(); ++row)
    {
    qCjyxDICOMExportable* exportable = d->exportableForRowIndex(row);
...
//------------------------------------------------------------------------------
void qCjyxDICOMTagEditorWidget::tagsTableCellChanged(int row, int column)
{
  Q_D(qCjyxDICOMTagEditorWidget);
  if (column != 1)
    {
