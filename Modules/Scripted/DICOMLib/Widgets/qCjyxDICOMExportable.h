...
#ifndef __qCjyxDICOMExportable_h
#define __qCjyxDICOMExportable_h
...
class vtkCjyxDICOMExportable;

/// Container class for data that can be exported to DICOM files from Cjyx.
...
class Q_CJYX_MODULE_DICOMLIB_WIDGETS_EXPORT qCjyxDICOMExportable : public QObject
{
  Q_OBJECT
...
public:
  ...
  qCjyxDICOMExportable(QObject *parent = nullptr);
  ~qCjyxDICOMExportable() override;
  ...
  Q_INVOKABLE void copyToVtkExportable(vtkCjyxDICOMExportable* vtkExportable);

  /// Copy values from VTK exportable
  Q_INVOKABLE void copyFromVtkExportable(vtkCjyxDICOMExportable* vtkExportable);

protected:
  QScopedPointer<qCjyxDICOMExportablePrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qCjyxDICOMExportable);
  Q_DISABLE_COPY(qCjyxDICOMExportable);
};
