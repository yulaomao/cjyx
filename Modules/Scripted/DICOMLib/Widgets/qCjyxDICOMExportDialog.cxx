...
#include "ui_qCjyxDICOMExportDialog.h"
...
class qCjyxDICOMExportDialogPrivate : public Ui_qCjyxDICOMExportDialog, public QDialog
{
  Q_DECLARE_PUBLIC(qCjyxDICOMExportDialog);
protected:
  qCjyxDICOMExportDialog* const q_ptr;
public:
  qCjyxDICOMExportDialogPrivate(qCjyxDICOMExportDialog& object);
  ~qCjyxDICOMExportDialogPrivate() override;
public:
  void init();
private:
  vtkDMMLScene* Scene;
  vtkIdType ItemToSelect;
  qCjyxDICOMExportable* SelectedExportable;
};

//-----------------------------------------------------------------------------
qCjyxDICOMExportDialogPrivate::qCjyxDICOMExportDialogPrivate(qCjyxDICOMExportDialog& object)
  : q_ptr(&object)
  , Scene(nullptr)
  , ItemToSelect(vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
  , SelectedExportable(nullptr)
{
}

//-----------------------------------------------------------------------------
qCjyxDICOMExportDialogPrivate::~qCjyxDICOMExportDialogPrivate() = default;

//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialogPrivate::init()
{
  Q_Q(qCjyxDICOMExportDialog);

  // Set up tree view
  qDMMLSubjectHierarchyModel* sceneModel = (qDMMLSubjectHierarchyModel*)this->SubjectHierarchyTreeView->model();
  this->SubjectHierarchyTreeView->setDMMLScene(this->Scene);
...
//-----------------------------------------------------------------------------
// qCjyxDICOMExportDialog methods

//-----------------------------------------------------------------------------
qCjyxDICOMExportDialog::qCjyxDICOMExportDialog(QObject* parent)
  : QObject(parent)
  , d_ptr(new qCjyxDICOMExportDialogPrivate(*this))
{
  Q_D(qCjyxDICOMExportDialog);
  d->setupUi(d);
}

//-----------------------------------------------------------------------------
qCjyxDICOMExportDialog::~qCjyxDICOMExportDialog() = default;

//-----------------------------------------------------------------------------
bool qCjyxDICOMExportDialog::exec(vtkIdType itemToSelect/*=vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID*/)
{
  Q_D(qCjyxDICOMExportDialog);
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::setDMMLScene(vtkDMMLScene* scene)
{
  Q_D(qCjyxDICOMExportDialog);
  d->Scene = scene;
}
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::makeDialogSelections()
{
  Q_D(qCjyxDICOMExportDialog);
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::onCurrentItemChanged(vtkIdType itemID)
{
  Q_UNUSED(itemID)
  Q_D(qCjyxDICOMExportDialog);
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::examineSelectedItem()
{
  Q_D(qCjyxDICOMExportDialog);
  ...
  QMap<QString,QList<qCjyxDICOMExportable*> > exportablesByPlugin;
  foreach (vtkIdType selectedSeriesItemID, selectedSeriesItemIDs)
    ...
    foreach(QVariant exportableVariant, exportablesVariantList)
      {
      qCjyxDICOMExportable* exportable = qobject_cast<qCjyxDICOMExportable*>(
      ...
      if (!exportablesByPlugin.contains(plugin))
        {
        QList<qCjyxDICOMExportable*> firstExportableForPlugin;
  ...
  QMap<double,QList<qCjyxDICOMExportable*> > exportablesByConfidence;
  foreach(QList<qCjyxDICOMExportable*> exportablesForPlugin, exportablesByPlugin)
    {
    // Geometric mean to emphasize larger values
    double meanConfidenceForPlugin = 0.0;
    foreach (qCjyxDICOMExportable* exportable, exportablesForPlugin)
      {
  ...
  foreach (double inverseConfidence, exportablesByConfidence.keys())
    ...
    QList<QList<qCjyxDICOMExportable*> > exportableLists = exportablesByConfidence.values(inverseConfidence);
    foreach(QList<qCjyxDICOMExportable*> exportables, exportableLists)
      {
      ...
      foreach (qCjyxDICOMExportable* exportable, exportables)
        {
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::onExportableSelectedAtRow(int row)
{
  Q_D(qCjyxDICOMExportDialog);
  ...
  QList<qCjyxDICOMExportable*> exportableList;
  ...
  d->DICOMTagEditorWidget->setDMMLScene(d->Scene);
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::onTagEdited()
{
  Q_D(qCjyxDICOMExportDialog);
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::onExportSeriesRadioButtonToggled(bool seriesOn)
{
  Q_D(qCjyxDICOMExportDialog);
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::onExport()
{
  Q_D(qCjyxDICOMExportDialog);
  ...
  // Determine whether output directory is a Cjyx DICOM database
  ...
  if (exportToDatabase)
    ...
    outputFolder.setPath(qCjyxApplication::application()->temporaryPath());
  ...
  if (exportToDatabase)
    ...
    if (exportSuccess)
      {
      ctkDICOMDatabase* dicomDatabase = qCjyxApplication::application()->dicomDatabase();
      if (!dicomDatabase)
        {
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::onSaveTagsCheckBoxToggled(bool on)
{
  Q_D(qCjyxDICOMExportDialog);
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::onExportToFolderCheckBoxToggled(bool on)
{
...
//-----------------------------------------------------------------------------
void qCjyxDICOMExportDialog::showUpdatedDICOMBrowser()
{
  if (!qCjyxApplication::application()->dicomDatabase())
    {
...
//-----------------------------------------------------------------------------
bool qCjyxDICOMExportDialog::exportSeries(const QDir& outputFolder)
{
  Q_D(qCjyxDICOMExportDialog);
  ...
  foreach (qCjyxDICOMExportable* exportable, d->DICOMTagEditorWidget->exportables())
    {
  ...
  exportContext.evalScript(QString(
    "plugin = cjyx.modules.dicomPlugins[%1]()\n"
    "try:\n"
    ...
    "  import traceback\n"
    "  traceback.print_exc()\n")
    .arg(qCjyxCorePythonManager::toPythonStringLiteral(d->DICOMTagEditorWidget->exportables()[0]->pluginClass())));
  QApplication::restoreOverrideCursor();
...
//-----------------------------------------------------------------------------
bool qCjyxDICOMExportDialog::exportEntireScene(const QDir& outputFolder)
{
  Q_D(qCjyxDICOMExportDialog);
  PythonQt::init();
  PythonQtObjectPtr exportContext = PythonQt::self()->getMainModule();
  exportContext.evalScript( QString(
    ...
    "success = exporter.export()\n").arg(qCjyxCorePythonManager::toPythonStringLiteral(outputFolder.absolutePath())) );
