...
class DICOMLoadable:
    ...
    loaded from dicom files into cjyx.
    ...
    NOTE: This class is deprecated, use qCjyxDICOMLoadable
    instead.
    """
...
class DICOMPlugin:
    ...
    def isDetailedLogging(self):
        ...
        """
        return cjyx.util.settingsValue('DICOM/detailedLogging', False, converter=cjyx.util.toBool)
    ...
    def load(self, loadable):
        ...
        the referenced data into DMML nodes
        Virtual: should be overridden by the subclass
        """
        return True
    ...
    def defaultSeriesNodeName(self, seriesUID):
        """Generate a name suitable for use as a dmml node name based
        on the series level data in the database"""
        instanceFilePaths = cjyx.dicomDatabase.filesForSeries(seriesUID, 1)
        if len(instanceFilePaths) == 0:
            return "Unnamed Series"
        seriesDescription = cjyx.dicomDatabase.fileValue(instanceFilePaths[0], self.tags['seriesDescription'])
        seriesNumber = cjyx.dicomDatabase.fileValue(instanceFilePaths[0], self.tags['seriesNumber'])
    ...
    def addSeriesInSubjectHierarchy(self, loadable, dataNode):
        ...
        seriesInstanceUid = cjyx.dicomDatabase.fileValue(firstFile, tags['seriesInstanceUID'])
        shn.SetItemUID(seriesItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMUIDName(), seriesInstanceUid)
        shn.SetItemAttribute(seriesItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMSeriesModalityAttributeName(),
                             cjyx.dicomDatabase.fileValue(firstFile, tags['seriesModality']))
        shn.SetItemAttribute(seriesItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMSeriesNumberAttributeName(),
                             cjyx.dicomDatabase.fileValue(firstFile, tags['seriesNumber']))
        shn.SetItemAttribute(seriesItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMFrameOfReferenceUIDAttributeName(),
                             cjyx.dicomDatabase.fileValue(firstFile, tags['frameOfReferenceUID']))
        # Set instance UIDs
        instanceUIDs = ""
        for file in loadable.files:
            uid = cjyx.dicomDatabase.fileValue(file, tags['instanceUID'])
            if uid == "":
                uid = "Unknown"
            instanceUIDs += uid + " "
        instanceUIDs = instanceUIDs[:-1]  # strip last space
        shn.SetItemUID(seriesItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMInstanceUIDName(), instanceUIDs)
        ...
        shn.SetItemAttribute(seriesItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMReferencedInstanceUIDsAttributeName(),
                             referencedInstanceUIDs)

        # Add series item to hierarchy under the right study and patient items. If they are present then used, if not, then created
        studyInstanceUid = cjyx.dicomDatabase.fileValue(firstFile, tags['studyInstanceUID'])
        patientId = cjyx.dicomDatabase.fileValue(firstFile, tags['patientID'])
        if not patientId:
        ...
        patientItemID = shn.GetItemByUID(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMUIDName(), patientId)
        studyId = cjyx.dicomDatabase.fileValue(firstFile, tags['studyID'])
        studyItemID = shn.GetItemByUID(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMUIDName(), studyInstanceUid)
        cjyx.vtkCjyxSubjectHierarchyModuleLogic.InsertDicomSeriesInHierarchy(shn, patientId, studyInstanceUid, seriesInstanceUid)

        if not patientItemID:
            patientItemID = shn.GetItemByUID(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMUIDName(), patientId)
            if patientItemID:
                # Add attributes for DICOM tags
                patientName = cjyx.dicomDatabase.fileValue(firstFile, tags['patientName'])
                if patientName == '':
                    patientName = 'No name'

                shn.SetItemAttribute(patientItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientNameAttributeName(),
                                     patientName)
                shn.SetItemAttribute(patientItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientIDAttributeName(),
                                     patientId)
                shn.SetItemAttribute(patientItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientSexAttributeName(),
                                     cjyx.dicomDatabase.fileValue(firstFile, tags['patientSex']))
                patientBirthDate = cjyx.dicomDatabase.fileValue(firstFile, tags['patientBirthDate'])
                shn.SetItemAttribute(patientItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientBirthDateAttributeName(),
                                     patientBirthDate)
                shn.SetItemAttribute(patientItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientCommentsAttributeName(),
                                     cjyx.dicomDatabase.fileValue(firstFile, tags['patientComments']))
                # Set item name
        ...
        if not studyItemID:
            ...
            if studyItemID:
                # Add attributes for DICOM tags
                studyDescription = cjyx.dicomDatabase.fileValue(firstFile, tags['studyDescription'])
                if studyDescription == '':
                    studyDescription = 'No study description'

                shn.SetItemAttribute(studyItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyDescriptionAttributeName(),
                                     studyDescription)
                studyDate = cjyx.dicomDatabase.fileValue(firstFile, tags['studyDate'])
                shn.SetItemAttribute(studyItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyInstanceUIDAttributeName(),
                                     studyInstanceUid)
                shn.SetItemAttribute(studyItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyIDAttributeName(),
                                     studyId)
                shn.SetItemAttribute(studyItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyDateAttributeName(),
                                     studyDate)
                shn.SetItemAttribute(studyItemID, cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyTimeAttributeName(),
                                     cjyx.dicomDatabase.fileValue(firstFile, tags['studyTime']))
                # Set item name
