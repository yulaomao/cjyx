...
DICOMExportScene provides the feature of exporting a cjyx
...
for elements like cjyx.dicomDatatabase and cjyx.dmmlScene
...
class DICOMExportScene:
    """Export cjyx scene to dicom database
    """
    ...
    def __init__(self, referenceFile, saveDirectoryPath=None):
        ...
        # Path and filename of the Cjyx Data Bundle DICOM file
        self.sdbFile = None
        ...
        self.imageFile = None
        # Study description string to save in the tags. Default is "Cjyx Scene Export"
        self.studyDescription = None
        # Series description string to save in the tags. Default is "Cjyx Data Bundle"
    ...
    def createDICOMFileForScene(self):
        ...
        - first to a directory using the utility in the dmmlScene
        ...
        if self.saveDirectoryPath is None:
            self.saveDirectoryPath = tempfile.mkdtemp('', 'dicomExport', cjyx.app.temporaryPath)
        ...
        self.sdbFile = os.path.join(self.saveDirectoryPath, "CjyxDataBundle.dcm")
        if self.studyDescription is None:
            self.studyDescription = 'Cjyx Scene Export'
        if self.seriesDescription is None:
            self.seriesDescription = 'Cjyx Data Bundle'
        ...
        if self.imageFile is None:
            ...
            image = ctk.ctkWidgetsUtils.grabWidget(cjyx.util.mainWindow())
        ...
        if not cjyx.dmmlScene.WriteToMRB(self.zipFile, imageReader.GetOutput()):
        ...
        creatorString = "3D Cjyx %s" % zipSizeString
...
""" % (creatorString, len(creatorString), zipSizeString, self.zipFile, zipSize)
        ...
        # cmd = "img2dcm -k 'InstanceNumber=1' -k 'SeriesDescription=Cjyx Data Bundle' -df %s/template.dcm %s %s" % (self.saveDirectoryPath, self.imageFile, self.sdbFile)
        args = [
            '-k', 'InstanceNumber=1',
