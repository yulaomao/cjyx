...
class DICOMProcess:
    ...
    def __init__(self):
        ...
        for path in pathOptions:
            testPath = cjyx.app.cjyxHome + path
...
class DICOMStoreSCPProcess(DICOMProcess):
    ...
    def onStateChanged(self, newState):
        ...
        if stderr and stderr.size():
            cjyx.util.errorDisplay("An error occurred. For further information click 'Show Details...'",
    ...
    def notifyUserAboutRunningStoreSCP(self, pid=None):
        if cjyx.util.confirmYesNoDisplay('There are other DICOM listeners running.\n Do you want to end them?'):
            if os.name == 'nt':
...
class DICOMListener(DICOMStoreSCPProcess):
    ...
    def processStdoutLine(self, line):
        ...
        if tagStart != -1:
            ...
            cjyx.dicomFilePath = dicomFilePath
        ...
        if self.protocol == "DICOMweb":
            ...
            try:
                ...
                if version.parse(dicomweb_client.__version__) < version.parse(minimumDicomwebClientVersion):
                    if not cjyx.util.confirmOkCancelDisplay(f"DICOMweb sending requires installation of dicomweb-client (version {minimumDicomwebClientVersion} or later).\nClick OK to upgrade dicomweb-client and restart the application."):
            ...
            if needInstall:
                ...
                progressDialog = cjyx.util.createProgressDialog(labelText='Upgrading dicomweb-client. This may take a minute...', maximum=0)
                cjyx.app.processEvents()
                cjyx.util.pip_install(f'dicomweb-client>={minimumDicomwebClientVersion}')
                import dicomweb_client
