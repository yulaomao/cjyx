...
def getDatabasePatientUIDByPatientName(name):
    ...
    patients = cjyx.dicomDatabase.patients()
    for patientUID in patients:
        currentName = cjyx.dicomDatabase.nameForPatient(patientUID)
        if currentName == name:
            return patientUID
    return None
...
def getDatabasePatientUIDByPatientID(patientID):
    ...
    for patientUID in patients:
        ...
        studies = cjyx.dicomDatabase.studiesForPatient(patientUID)
        series = [cjyx.dicomDatabase.seriesForStudy(study) for study in studies]
        seriesUIDs = [uid for uidList in series for uid in uidList]
        if len(seriesUIDs) == 0:
            continue
        filePaths = cjyx.dicomDatabase.filesForSeries(seriesUIDs[0], 1)
        if len(filePaths) == 0:
            continue
        firstFile = filePaths[0]
        # Get PatientID from first file
        currentPatientID = cjyx.dicomDatabase.fileValue(cjyx.util.longPath(firstFile), "0010,0020")
...
def loadSeriesByUID(seriesUIDs):
    ...
    for seriesUID in seriesUIDs:
        fileLists.append(cjyx.dicomDatabase.filesForSeries(seriesUID))
    if len(fileLists) == 0:
...
def selectHighestConfidenceLoadables(loadablesByPlugin):
    ...
    for plugin in loadablesByPlugin:
        for loadable in loadablesByPlugin[plugin]:
            seriesUID = cjyx.dicomDatabase.fileValue(loadable.files[0], seriesUIDTag)
...
def loadByInstanceUID(instanceUID):
    ...
    filePath = cjyx.dicomDatabase.fileForInstance(instanceUID)
    seriesUID = cjyx.dicomDatabase.seriesForFile(filePath)
    fileList = cjyx.dicomDatabase.filesForSeries(seriesUID)
...
def clearDatabase(dicomDatabase=None):
    ...
    if dicomDatabase is None:
        dicomDatabase = cjyx.dicomDatabase
...
def openTemporaryDatabase(directory=None):
    ...
    if os.path.isabs(directory):
        ...
        tempDatabaseDir = cjyx.app.temporaryPath + '/' + directory
    ...
    originalDatabaseDir = settings.value(cjyx.dicomDatabaseDirectorySettingsKey)
    settings.setValue(cjyx.dicomDatabaseDirectorySettingsKey, tempDatabaseDir)

    openDatabase(tempDatabaseDir)

    # Clear the entire database
    cjyx.dicomDatabase.initializeDatabase()
...
def closeTemporaryDatabase(originalDatabaseDir, cleanup=True):
    ...
    if cjyx.dicomDatabase.isOpen:
        if cleanup:
            ...
            # import shutil
            # databaseDir = os.path.split(cjyx.dicomDatabase.databaseFilename)[0]
        ...
        logging.error('Unable to close DICOM database ' + cjyx.dicomDatabase.databaseFilename)
    ...
    settings = qt.QSettings()
    settings.setValue(cjyx.dicomDatabaseDirectorySettingsKey, originalDatabaseDir)
...
class TemporaryDICOMDatabase:
    """Context manager to conveniently use temporary DICOM database.
       ...
       DICOM database in the application (cjyx.dicomDatabase).
    """
...
def importDicom(dicomDataDir, dicomDatabase=None, copyFiles=False):
    """ Import DICOM files from folder into Cjyx database
    """
    try:
...
def loadSeriesWithVerification(seriesUIDs, expectedSelectedPlugins=None, expectedLoadedNodes=None):
    ...
    ``expectedLoadedNodes`` example: { 'vtkDMMLScalarVolumeNode':2, 'vtkDMMLSegmentationNode':1 }
    """
    if not cjyx.dicomDatabase.isOpen:
    ...
    if expectedLoadedNodes is not None:
        for nodeType in expectedLoadedNodes.keys():
            nodeCollection = cjyx.dmmlScene.GetNodesByClass(nodeType)
...
def allSeriesUIDsInDatabase(database=None):
    """ Collect all series instance UIDs in a DICOM database (the Cjyx one by default)
    ...
    """
    if database is None:
        database = cjyx.dicomDatabase
    dicomWidget = cjyx.modules.dicom.widgetRepresentation().self()
    allSeriesUIDs = []
...
def seriesUIDsForFiles(files):
    ...
    seriesUIDs = set()
    for file in files:
        seriesUID = cjyx.dicomDatabase.seriesForFile(file)
        if seriesUID != '':
...
class LoadDICOMFilesToDatabase:
    ...
    def __init__(self, url, archiveFilePath=None, dicomDataDir=None, \
        ...
        if archiveFilePath is None:
            ...
            archiveFilePath = cjyx.app.temporaryPath + '/' + fileName
        if dicomDataDir is None:
            directoryName = strftime("%Y%m%d_%H%M%S_", gmtime()) + 'LoadDICOMFilesToDatabase'
            dicomDataDir = cjyx.app.temporaryPath + '/' + directoryName
    ...
    def __enter__(self):
        if cjyx.util.downloadAndExtractArchive(self.url, self.archiveFilePath, \
                                                 self.dicomDataDir, self.expectedNumberOfExtractedFiles,
                                                 checksum=self.checksum):
            dicomFiles = cjyx.util.getFilesInDirectory(self.dicomDataDir)
...
def getSortedImageFiles(filePaths, epsilon=0.01):
    ...
    seriesUID = cjyx.dicomDatabase.fileValue(filePaths[0], tags['seriesUID'])

    if cjyx.dicomDatabase.fileValue(filePaths[0], tags['numberOfFrames']) not in ["", "1"]:
    ...
    for tag in [tags['position'], tags['orientation']]:
        value = cjyx.dicomDatabase.fileValue(filePaths[0], tag)
    ...
    for file in filePaths:
        positionStr = cjyx.dicomDatabase.fileValue(file, tags['position'])
        orientationStr = cjyx.dicomDatabase.fileValue(file, tags['orientation'])
        ...
        for fileN in files[1:]:
            ...
            if abs(spaceError) > epsilon:
                ...
                if acquisitionGeometryRegularizationEnabled:
                    warningText += "  Cjyx will apply a transform to this series trying to regularize the volume. Please use caution.\n"
                else:
...
def refreshDICOMWidget():
    ...
    """
    try:
        cjyx.modules.DICOMInstance.browserWidget.dicomBrowser.dicomTableManager().updateTableViews()
...
def getLoadablesFromFileLists(fileLists, pluginClassNames=None, messages=None, progressCallback=None, pluginInstances=None):
    ...
    """
    detailedLogging = cjyx.util.settingsValue('DICOM/detailedLogging', False, converter=cjyx.util.toBool)
    loadablesByPlugin = {}
    loadEnabled = False
    ...
    if pluginClassNames is None:
        pluginClassNames = list(cjyx.modules.dicomPlugins.keys())
    ...
    for step, pluginClassName in enumerate(pluginClassNames):
        if pluginClassName not in pluginInstances:
            pluginInstances[pluginClassName] = cjyx.modules.dicomPlugins[pluginClassName]()
...
def loadLoadables(loadablesByPlugin, messages=None, progressCallback=None):
    ...
    def onNodeAdded(caller, event, calldata):
        node = calldata
        if not isinstance(node, cjyx.vtkDMMLStorageNode) and not isinstance(node, cjyx.vtkDMMLDisplayNode):
            loadedNodeIDs.append(node.GetID())

    sceneObserverTag = cjyx.dmmlScene.AddObserver(cjyx.vtkDMMLScene.NodeAddedEvent, onNodeAdded)
        ...
        try:
            ...
            for derivedItem in loadable.derivedItems:
                ...
                if progressCallback:
                    ...
                    if cancelled:
                        break
                indexer.addFile(cjyx.dicomDatabase, derivedItem)
        except AttributeError:
    ...
    cjyx.dmmlScene.RemoveObserver(sceneObserverTag)
...
def importFromDICOMWeb(dicomWebEndpoint, studyInstanceUID, seriesInstanceUID=None, accessToken=None):
    ...
    progressDialog = cjyx.util.createProgressDialog(parent=cjyx.util.mainWindow(), value=0, maximum=100)
    try:
        ...
        for seriesIndex, currentSeriesInstanceUID in enumerate(seriesInstanceUIDs):
            ...
            try:
                ...
                alreadyImportedInstances = cjyx.dicomDatabase.instancesForSeries(currentSeriesInstanceUID)
                ...
                outputDirectoryBase = cjyx.dicomDatabase.databaseDirectory + "/DICOMweb"
    ...
    if errors:
        cjyx.util.errorDisplay(f"Errors occurred during DICOMweb import of {len(errors)} series.", detailedText="\n\n".join(errors))
    elif cancelled and (len(seriesImported) < len(seriesInstanceUIDs)):
        cjyx.util.infoDisplay(f"DICOMweb import has been interrupted after completing {len(seriesImported)} out of {len(seriesInstanceUIDs)} series.")

    return seriesImported


def registerCjyxURLHandler():
    """
    Registers file associations and applicationName:// protocol (e.g., Cjyx://)
    with this executable. This allows Kheops (https://demo.kheops.online) open
    images selected in the web browser directly in Cjyx.
    For now, only implemented on Windows.
    """
    if os.name == 'nt':
        launcherPath = qt.QDir.toNativeSeparators(qt.QFileInfo(cjyx.app.launcherExecutableFilePath).absoluteFilePath())
        reg = qt.QSettings(f"HKEY_CURRENT_USER\\Software\\Classes", qt.QSettings.NativeFormat)
        reg.setValue(f"{cjyx.app.applicationName}/.", f"{cjyx.app.applicationName} supported file")
        reg.setValue(f"{cjyx.app.applicationName}/URL protocol", "")
        reg.setValue(f"{cjyx.app.applicationName}/shell/open/command/.", f"\"{launcherPath}\" \"%1\"")
        reg.setValue(f"{cjyx.app.applicationName}/DefaultIcon/.", f"{cjyx.app.applicationName}.exe,0")
        for ext in ['dmml', 'mrb']:
            reg.setValue(f".{ext}/.", f"{cjyx.app.applicationName}")
