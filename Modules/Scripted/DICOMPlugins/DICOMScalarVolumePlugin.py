...
# from DICOM files into DMML nodes.  It follows the DICOM module's
# plugin architecture.
#
...
class DICOMScalarVolumePluginClass(DICOMPlugin):
    ...
    def __init__(self, epsilon=0.01):
        ...
        self.defaultStudyID = 'CJYX10001'  # TODO: What should be the new study ID?
    ...
    def settingsPanelEntry(panel, parent):
        ...
        readersComboBox.toolTip = ("Preferred back end.  Archetype was used by default in Cjyx before June of 2017."
    ...
    def compareVolumeNodes(volumeNode1, volumeNode2):
        """
        Given two dmml volume nodes, return true of the numpy arrays have identical data
        ...
        array1 = cjyx.util.array(volumeNode1.GetID())
        array2 = cjyx.util.array(volumeNode2.GetID())
    ...
    def examineFiles(self, files):
        ...
        seriesUID = cjyx.dicomDatabase.fileValue(files[0], self.tags['seriesUID'])
        ...
        for file in allFilesLoadable.files:
            ...
            for tag in subseriesTags:
                value = cjyx.dicomDatabase.fileValue(file, self.tags[tag])
        ...
        for loadable in loadables:
            ...
            for file in loadable.files:
                if cjyx.dicomDatabase.fileValueExists(file, self.tags['pixelData']):
                    newFiles.append(file)
                if cjyx.dicomDatabase.fileValue(file, self.tags['sopClassUID']) == '1.2.840.10008.5.1.4.1.1.66.4':
                    excludedLoadable = True
                    if 'DICOMSegmentationPlugin' not in cjyx.modules.dicomPlugins:
                        logging.warning('Please install Quantitative Reporting extension to enable loading of DICOM Segmentation objects')
                elif cjyx.dicomDatabase.fileValue(file, self.tags['sopClassUID']) == '1.2.840.10008.5.1.4.1.1.481.3':
                    excludedLoadable = True
                    if 'DicomRtImportExportPlugin' not in cjyx.modules.dicomPlugins:
                        logging.warning('Please install CjyxRT extension to enable loading of DICOM RT Structure Set objects')
            if len(newFiles) > 0 and not excludedLoadable:
                loadable.files = newFiles
                loadable.grayscale = ('MONOCHROME' in cjyx.dicomDatabase.fileValue(newFiles[0], self.tags['photometricInterpretation']))
                ...
                loadable.grayscale = ('MONOCHROME' in cjyx.dicomDatabase.fileValue(loadable.files[0], self.tags['photometricInterpretation']))
                newLoadables.append(loadable)
    ...
    #   by the File->Add Data dialog in the Cjyx GUI.
    ...
    def loadFilesWithArchetype(self, files, name):
        """Load files in the traditional Cjyx manner
    ...
    def loadFilesWithSeriesReader(self, imageIOName, files, name, grayscale=True):
        ...
        name = cjyx.dmmlScene.GenerateUniqueName(name)
        if grayscale:
            volumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLScalarVolumeNode", name)
        else:
            volumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLVectorVolumeNode", name)
        volumeNode.SetAndObserveImageData(imageChangeInformation.GetOutputDataObject(0))
        cjyx.vtkDMMLVolumeArchetypeStorageNode.SetMetaDataDictionaryFromReader(volumeNode, reader)
        volumeNode.SetRASToIJKMatrix(reader.GetRasToIjkMatrix())
        volumeNode.CreateDefaultDisplayNodes()

        cjyx.modules.DICOMInstance.reader = reader
        cjyx.modules.DICOMInstance.imageChangeInformation = imageChangeInformation
    ...
    def setVolumeNodeProperties(self, volumeNode, loadable):
        ...
        if volumeNode:
            ...
            instanceUIDs = ""
            for file in loadable.files:
                uid = cjyx.dicomDatabase.fileValue(file, self.tags['instanceUID'])
                if uid == "":
                    uid = "Unknown"
                instanceUIDs += uid + " "
            ...
            #   [2] https://github.com/Slicer/Slicer/blob/3bfa2fc2b310d41c09b7a9e8f8f6c4f43d3bd1e2/Libs/DMML/Core/vtkDMMLScalarVolumeDisplayNode.h#L172
            #
            try:
                windowCenter = float(cjyx.dicomDatabase.fileValue(file, self.tags['windowCenter']))
                windowWidth = float(cjyx.dicomDatabase.fileValue(file, self.tags['windowWidth']))
            ...
            sopClassUID = cjyx.dicomDatabase.fileValue(file, self.tags['sopClassUID'])
            ...
            if modality == "PT":
                displayNode = volumeNode.GetDisplayNode()
                if displayNode:
                    displayNode.SetAndObserveColorNodeID(cjyx.modules.colors.logic().GetPETColorNodeID(cjyx.vtkDMMLPETProceduralColorNode.PETheat))
    ...
    def load(self, loadable, readerApproach=None):
        ...
        if not readerApproach:
            readerIndex = cjyx.util.settingsValue('DICOM/ScalarVolume/ReaderApproach', 0, converter=int)
        ...
        # third, transfer data from the dicom instances into the appropriate Cjyx data containers
        self.setVolumeNodeProperties(volumeNode, loadable)
    ...
    def examineForExport(self, subjectHierarchyItemID):
        ...
        shn = cjyx.vtkDMMLSubjectHierarchyNode.GetSubjectHierarchyNode(cjyx.dmmlScene)
        dataNode = shn.GetItemDataNode(subjectHierarchyItemID)
        if dataNode is None or not dataNode.IsA('vtkDMMLScalarVolumeNode'):
            return []
    ...
    def export(self, exportables):
        for exportable in exportables:
            ...
            if volumeNode is None or not volumeNode.IsA('vtkDMMLScalarVolumeNode'):
            ...
            tags = {}
            tags['Patient Name'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientNameTagName())
            tags['Patient ID'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientIDTagName())
            tags['Patient Birth Date'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientBirthDateTagName())
            tags['Patient Sex'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientSexTagName())
            tags['Patient Comments'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMPatientCommentsTagName())
            tags['Study ID'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyIDTagName())
            tags['Study Date'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyDateTagName())
            tags['Study Time'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyTimeTagName())
            tags['Study Description'] = exportable.tag(cjyx.vtkDMMLSubjectHierarchyConstants.GetDICOMStudyDescriptionTagName())
    ...
    class AcquisitionModeling:
        """Code for representing and analyzing acquisition properties in cjyx
        ...
        def gridTransformFromCorners(self, volumeNode, sourceCorners, targetCorners):
            ...
            gridTransform = cjyx.vtkDMMLGridTransformNode()
            gridTransform.SetName(cjyx.dmmlScene.GenerateUniqueName(volumeNode.GetName() + ' acquisition transform'))
            cjyx.dmmlScene.AddNode(gridTransform)
            ...
            transform = cjyx.vtkOrientedGridTransform()
            ...
            displacements = cjyx.util.arrayFromGridTransform(gridTransform)
        ...
        def sliceCornersFromDICOM(self, volumeNode):
            ...
            for sliceIndex in range(slices):
                ...
                positionString = cjyx.dicomDatabase.instanceValue(uid, positionTag)
                orientationString = cjyx.dicomDatabase.instanceValue(uid, orientationTag)
                spacingString = cjyx.dicomDatabase.instanceValue(uid, spacingTag)
...
class DICOMScalarVolumePlugin:
    ...
    """
        ...
        try:
            ...
            cjyx.modules.dicomPlugins = {}
        cjyx.modules.dicomPlugins['DICOMScalarVolumePlugin'] = DICOMScalarVolumePluginClass
