...
class DICOMImageSequencePluginClass(DICOMPlugin):
    ...
    def examineFiles(self, files):
        ...
        self.detailedLogging = cjyx.util.settingsValue('DICOM/detailedLogging', False, converter=cjyx.util.toBool)
        ...
        for filePath in files:
            ...
            try:
                sopClassUID = cjyx.dicomDatabase.fileValue(filePath, self.tags['sopClassUID'])
                ...
                # Quick check could not be completed (probably Cjyx DICOM database is not initialized).
                # No problem, we'll try to parse the file and check the SOP class UID then.
                pass

            instanceNumber = cjyx.dicomDatabase.fileValue(filePath, self.tags['instanceNumber'])
            if canBeCineMri and sopClassUID == '1.2.840.10008.5.1.4.1.1.4':  # MR Image Storage
                ...
                cineMriTriggerTimes.add(cjyx.dicomDatabase.fileValue(filePath, self.tags['triggerTime']))
                cineMriImageOrientations.add(cjyx.dicomDatabase.fileValue(filePath, self.tags['orientation']))

            else:
                modality = cjyx.dicomDatabase.fileValue(filePath, self.tags['modality'])
                ...
                if not (instanceNumber in instanceNumberToLoadableIndex.keys()):
                    # new instance number
                    seriesNumber = cjyx.dicomDatabase.fileValue(filePath, self.tags['seriesNumber'])
                    seriesDescription = cjyx.dicomDatabase.fileValue(filePath, self.tags['seriesDescription'])
                    photometricInterpretation = cjyx.dicomDatabase.fileValue(filePath, self.tags['photometricInterpretation'])
                    name = ''
                    if seriesNumber:
                        name = f'{seriesNumber}:'
                    if modality:
    ...
    def addSequenceBrowserNode(self, name, outputSequenceNodes, playbackRateFps, loadable):
        ...
        outputSequenceBrowserNode = cjyx.vtkDMMLSequenceBrowserNode()
        outputSequenceBrowserNode.SetName(cjyx.dmmlScene.GenerateUniqueName(name + ' browser'))
        outputSequenceBrowserNode.SetPlaybackRateFps(playbackRateFps)
        cjyx.dmmlScene.AddNode(outputSequenceBrowserNode)
        ...
        for outputSequenceNode in outputSequenceNodes:
            ...
            if first:
                ...
                cjyx.modules.sequences.setToolBarActiveBrowserNode(outputSequenceBrowserNode)

        # Show sequence browser toolbar
        cjyx.modules.sequences.showSequenceBrowser(outputSequenceBrowserNode)
    ...
    def addSequenceFromImageData(self, imageData, tempFrameVolume, filePath, name, singleFileInLoadable):
        ...
        # Create new sequence
        outputSequenceNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLSequenceNode")
        ...
        outputSequenceStorageNode = cjyx.vtkDMMLVolumeSequenceStorageNode()
        cjyx.dmmlScene.AddNode(outputSequenceStorageNode)
    ...
    def load(self, loadable):
        ...
        if loadable.grayscale:
            tempFrameVolume = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLScalarVolumeNode")
        else:
            tempFrameVolume = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLVectorVolumeNode")
        ...
        for fileIndex, filePath in enumerate(loadable.files):
            ...
            if loadable.singleSequence:
                ...
                tempFrameVolume.SetAttribute('DICOM.instanceUIDs', cjyx.dicomDatabase.instanceForFile(filePath))
                # Save trigger time, because it may be needed for 4D cine-MRI volume reconstruction
                triggerTime = cjyx.dicomDatabase.fileValue(filePath, self.tags['triggerTime'])
                if triggerTime:
        ...
        cjyx.dmmlScene.RemoveNode(tempFrameVolume)
...
class DICOMImageSequencePlugin:
    ...
    """
        ...
        try:
            ...
            cjyx.modules.dicomPlugins = {}
        cjyx.modules.dicomPlugins['DICOMImageSequencePlugin'] = DICOMImageSequencePluginClass
