...
# SelfTests
#
# This code includes the GUI for the cjyx module.
# The purpose is to provide a BIST (https://en.wikipedia.org/wiki/Built-in_self-test)
# framework for cjyx as discussed here: https://na-mic.org/Bug/view.php?id=1922
#
...
The SelfTests module allows developers to provide built-in self-tests (BIST) for cjyx so that users can tell
if their installed version of cjyx are running as designed.
"""
...
"""
        ...
        #
        # cjyx.selfTests is a dictionary of tests that are registered
        ...
        cjyx.selfTests['DMMLSceneExists'] = lambda: cjyx.app.dmmlScene
        cjyx.selfTests['CloseScene'] = ExampleSelfTests.closeScene
...
class SelfTestsWidget(ScriptedLoadableModuleWidget):
    """Cjyx module that creates the Qt GUI for interacting with SelfTests
    ...
    def setup(self):
        ...
        self.logic = SelfTestsLogic(cjyx.selfTests)
        ...
        testKeys = sorted(cjyx.selfTests.keys())
        for test in testKeys:
            self.testButtons[test] = qt.QPushButton(test)
            self.testButtons[test].setToolTip(cjyx.selfTests[test].__doc__)
    ...
    def onRunAll(self):
        ...
        cjyx.util.infoDisplay(self.logic, windowTitle='SelfTests')
...
def SelfTestsTest():
    if hasattr(cjyx, 'selfTests'):
        logic = SelfTestsLogic(list(cjyx.selfTests.keys()))
        logic.run()
    print(logic.results)
