...
class DICOMPatcher(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.helpText = """Fix common issues in DICOM files. This module may help fixing DICOM files that Cjyx fails to import."""
...
class DICOMPatcherWidget(ScriptedLoadableModuleWidget):
    ...
    def onPatchButton(self):
        with cjyx.util.tryWithErrorDisplay("Unexpected error.", waitCursor=True):

            import tempfile
            if not self.outputDirSelector.currentPath:
                self.outputDirSelector.currentPath = tempfile.mkdtemp(prefix="DICOMPatcher-", dir=cjyx.app.temporaryPath)
...
class DICOMPatcherTest(ScriptedLoadableModuleTest):
    ...
    def test_DICOMPatcher1(self):
        ...
        import tempfile
        testDir = tempfile.mkdtemp(prefix="DICOMPatcherTest-", dir=cjyx.app.temporaryPath)
