...
class SegmentEditorWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def setup(self):
        ...
        #
        import qCjyxSegmentationsModuleWidgetsPythonQt
        self.editor = qCjyxSegmentationsModuleWidgetsPythonQt.qDMMLSegmentEditorWidget()
        ...
        self.editor.setDMMLScene(cjyx.dmmlScene)
        ...
        self.effectFactorySingleton = cjyx.qCjyxSegmentEditorEffectFactory.instance()
        self.effectFactorySingleton.connect('effectRegistered(QString)', self.editorEffectRegistered)

        # Connect observers to scene events
        self.addObserver(cjyx.dmmlScene, cjyx.dmmlScene.StartCloseEvent, self.onSceneStartClose)
        self.addObserver(cjyx.dmmlScene, cjyx.dmmlScene.EndCloseEvent, self.onSceneEndClose)
        self.addObserver(cjyx.dmmlScene, cjyx.dmmlScene.EndImportEvent, self.onSceneEndImport)
    ...
    def selectParameterNode(self):
        ...
        segmentEditorNode = cjyx.dmmlScene.GetSingletonNode(segmentEditorSingletonTag, "vtkDMMLSegmentEditorNode")
        if segmentEditorNode is None:
            segmentEditorNode = cjyx.dmmlScene.CreateNodeByClass("vtkDMMLSegmentEditorNode")
            segmentEditorNode.UnRegister(None)
            segmentEditorNode.SetSingletonTag(segmentEditorSingletonTag)
            segmentEditorNode = cjyx.dmmlScene.AddNode(segmentEditorNode)
        if self.parameterSetNode == segmentEditorNode:
            # nothing changed
            return
        self.parameterSetNode = segmentEditorNode
        self.editor.setDMMLSegmentEditorNode(self.parameterSetNode)
    ...
    def getDefaultMasterVolumeNodeID(self):
        ...
        for sliceViewName in layoutManager.sliceViewNames():
            ...
            if not sliceWidget:
                continue
            compositeNode = sliceWidget.dmmlSliceCompositeNode()
    ...
    def enter(self):
        ...
        if self.editor.turnOffLightboxes():
            cjyx.util.warningDisplay('Segment Editor is not compatible with slice viewers in light box mode.'
        ...
        self.selectParameterNode()
        self.editor.updateWidgetFromDMML()
        ...
        if not self.editor.segmentationNodeID():
            segmentationNode = cjyx.dmmlScene.GetFirstNode(None, "vtkDMMLSegmentationNode")
