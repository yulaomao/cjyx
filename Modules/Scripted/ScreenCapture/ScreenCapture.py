...
class ScreenCaptureWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        # Input view selector
        self.viewNodeSelector = cjyx.qDMMLNodeComboBox()
        self.viewNodeSelector.nodeTypes = ["vtkDMMLSliceNode", "vtkDMMLViewNode"]
        ...
        self.viewNodeSelector.setDMMLScene(cjyx.dmmlScene)
        ...
        self.sequenceBrowserNodeSelectorWidget = cjyx.qDMMLNodeComboBox()
        self.sequenceBrowserNodeSelectorWidget.nodeTypes = ["vtkDMMLSequenceBrowserNode"]
        ...
        self.sequenceBrowserNodeSelectorWidget.setDMMLScene(cjyx.dmmlScene)
        ...
        if not self.outputDirSelector.currentPath:
            defaultOutputPath = os.path.abspath(os.path.join(cjyx.app.defaultScenePath, 'CjyxCapture'))
        ...
        self.videoFileNameWidget.text = "CjyxCapture.avi"
        ...
        self.lightboxImageFileNameWidget.text = "CjyxCaptureLightbox.png"
        ...
        self.volumeNodeComboBox = cjyx.qDMMLNodeComboBox()
        self.volumeNodeComboBox.nodeTypes = ["vtkDMMLVectorVolumeNode"]
        ...
        self.volumeNodeComboBox.setDMMLScene(cjyx.dmmlScene)
        ...
        watermarkEnabled = cjyx.util.settingsValue('ScreenCapture/WatermarkEnabled', False, converter=cjyx.util.toBool)
        ...
        self.watermarkPositionWidget.setCurrentText(
            cjyx.util.settingsValue('ScreenCapture/WatermarkPosition', self.logic.watermarkPositionPresets[0]["name"]))
        ...
        try:
            self.watermarkSizeSliderWidget.value = int(cjyx.util.settingsValue('ScreenCapture/WatermarkSize', 100))
        except:
            pass
        ...
        try:
            self.watermarkOpacitySliderWidget.value = int(cjyx.util.settingsValue('ScreenCapture/WatermarkOpacity', 30))
        except:
            pass
        ...
        self.viewNodeSelector.connect("currentNodeChanged(vtkDMMLNode*)", self.updateViewOptions)
        ...
        self.sequenceBrowserNodeSelectorWidget.connect("currentNodeChanged(vtkDMMLNode*)", self.updateViewOptions)
    ...
    def currentViewNodeType(self):
        ...
        if not viewNode:
            return None
        elif viewNode.IsA("vtkDMMLSliceNode"):
            return VIEW_SLICE
        elif viewNode.IsA("vtkDMMLViewNode"):
            return VIEW_3D
        else:
            return None
    ...
    def addLog(self, text):
        ...
        cjyx.app.processEvents()  # force update
    ...
    def updateViewOptions(self):
        ...
        sequencesModuleAvailable = hasattr(cjyx.modules, 'sequences')
    ...
    def onCaptureButton(self):
        ...
        if self.watermarkEnabledCheckBox.checked:
            if self.watermarkPathSelector.currentPath:
                ...
                self.logic.setWatermarkImagePath(self.resourcePath('CjyxWatermark.png'))
        ...
        if videoOutputRequested:
            ...
            if not self.logic.isFfmpegPathValid() and os.name == 'nt':  # TODO: implement download for Linux/MacOS?
                ...
                if cjyx.util.confirmOkCancelDisplay(
                    ...
                    if not self.logic.ffmpegDownload():
                        cjyx.util.errorDisplay("ffmpeg download failed")
            if not self.logic.isFfmpegPathValid():
...
class ScreenCaptureLogic(ScriptedLoadableModuleLogic):
    ...
    def showViewControllers(self, show):
        cjyx.util.setViewControllersVisible(show)
    ...
    def getDownloadedFfmpegDirectory(self):
        return os.path.dirname(cjyx.app.cjyxUserSettingsFilePath) + '/ffmpeg'
    ...
    def unzipFfmpeg(self, filePath, ffmpegTargetDirectory):
        ...
        cjyx.app.applicationLogic().Unzip(filePath, ffmpegTargetDirectory)
    ...
    def ffmpegDownload(self):
        ...
        filePath = cjyx.app.temporaryPath + '/ffmpeg-package-cjyx-01.zip'
    ...
    def getFfmpegPath(self):
        ...
        if settings.contains('General/ffmpegPath'):
            return cjyx.app.toCjyxHomeAbsolutePath(settings.value('General/ffmpegPath'))
        return ''
    ...
    def setFfmpegPath(self, ffmpegPath):
        ...
        settings = qt.QSettings()
        if settings.contains('General/ffmpegPath'):
            if ffmpegPath == cjyx.app.toCjyxHomeAbsolutePath(settings.value('General/ffmpegPath')):
                return
        settings.setValue('General/ffmpegPath', cjyx.app.toCjyxHomeRelativePath(ffmpegPath))
    ...
    def captureImageFromView(self, view, filename=None, transparentBackground=False, volumeNode=None):
        ...
        cjyx.app.processEvents()
        if view:
            if type(view) == cjyx.qDMMLSliceView or type(view) == cjyx.qDMMLThreeDView:
                view.forceRender()
            else:
                view.repaint()
        else:
        ...
        if view is None:
            ...
            # img = ctk.ctkWidgetsUtils.grabWidget(cjyx.app.layoutManager().viewport())

            # Grab the main window and use only the viewport's area
            allViews = cjyx.app.layoutManager().viewport()
            topLeft = allViews.mapTo(cjyx.util.mainWindow(), allViews.rect.topLeft())
            bottomRight = allViews.mapTo(cjyx.util.mainWindow(), allViews.rect.bottomRight())
            ...
            img = ctk.ctkWidgetsUtils.grabWidget(cjyx.util.mainWindow(), qt.QRect(topLeft.x(), topLeft.y(), imageSize.x(), imageSize.y()))
        ...
        if volumeNode is not None:
            if isinstance(volumeNode, cjyx.vtkDMMLVectorVolumeNode):
    ...
    def createVideo(self, frameRate, extraOptions, outputDir, imageFileNamePattern, videoFileName):
        ...
        ffmpegParams = [ffmpegPath,
                        "-nostdin",  # disable stdin (to prevent hang when running Cjyx as background process)
...
class ScreenCaptureTest(ScriptedLoadableModuleTest):
    ...
    def setUp(self):
        ...
        cjyx.app.applicationLogic().PropagateVolumeSelection(1)

        # Show slice and 3D views
        layoutManager = cjyx.app.layoutManager()
        layoutManager.setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutFourUpView)
        for sliceViewNodeId in ['vtkDMMLSliceNodeRed', 'vtkDMMLSliceNodeYellow', 'vtkDMMLSliceNodeGreen']:
            cjyx.dmmlScene.GetNodeByID(sliceViewNodeId).SetSliceVisible(True)

        self.tempDir = cjyx.app.temporaryPath + '/ScreenCaptureTest'
    ...
    def test_SliceSweep(self):
        ...
        viewNode = cjyx.dmmlScene.GetNodeByID('vtkDMMLSliceNodeRed')
    ...
    def test_3dViewRotation(self):
        ...
        viewNode = cjyx.dmmlScene.GetNodeByID('vtkDMMLViewNode1')
        self.assertIsNotNone(viewNode)
    ...
    def test_VolumeNodeUpdate(self):
        ...
        volumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLVectorVolumeNode")
