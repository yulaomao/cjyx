...
from cjyx.util import NodeModify, VTKObservationMixin


@contextmanager
def MyScopedQtPropertySetter(qobject, properties):
    """ Context manager to set/reset properties"""
    # TODO: Move it to cjyx.utils and delete it here.
    previousValues = {}
...
def getNode(nodeID):
    if nodeID is None:
        return None
    return cjyx.dmmlScene.GetNodeByID(nodeID)
...
class VectorToScalarVolumeWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def setup(self):
        ...
        self.parameterNodeSelector.addAttribute("vtkDMMLScriptedModuleNode", "ModuleName", "VectorToScalarVolume")
        ...
        self.inputSelector.nodeTypes = ["vtkDMMLVectorVolumeNode"]
        ...
        self.outputSelector.nodeTypes = ["vtkDMMLScalarVolumeNode"]
        self.outputSelector.hideChildNodeTypes = ["vtkDMMLVectorVolumeNode"]
        ...
        # Connections
        self.parameterNodeSelector.connect('currentNodeChanged(vtkDMMLNode*)', self.setParameterNode)
        self.parameterNodeSelector.connect('currentNodeChanged(vtkDMMLNode*)', self.updateGuiFromDMML)

        # updateParameterNodeFromGui
        self.inputSelector.connect("currentNodeChanged(vtkDMMLNode*)", self.updateParameterNodeFromGui)
        self.outputSelector.connect("currentNodeChanged(vtkDMMLNode*)", self.updateParameterNodeFromGui)
    ...
    def setParameterNode(self, inputParameterNode):
        ...
        if self._parameterNode is not None:
            self.removeObserver(self._parameterNode, vtk.vtkCommand.ModifiedEvent, self.updateGuiFromDMML)
        if inputParameterNode is not None:
            self.addObserver(inputParameterNode, vtk.vtkCommand.ModifiedEvent, self.updateGuiFromDMML)
    ...
    def updateGuiFromDMML(self, caller=None, event=None):
        """
    ...
    def onApply(self):
        ...
        if success:
            ...
            cjyx.app.applicationLogic().PropagateVolumeSelection(0)
...
class VectorToScalarVolumeLogic(ScriptedLoadableModuleLogic):
    ...
    def run(self, parameterNode):
        ...
        if parameterNode is None:
            cjyx.util.errorDisplay('Invalid Parameter Node: None')
            return False
