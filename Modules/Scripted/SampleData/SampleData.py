...
from cjyx.util import computeChecksum, extractAlgoAndDigest, TESTING_DATA_URL
...
def downloadFromURL(uris=None, fileNames=None, nodeNames=None, checksums=None, loadFiles=None,
    ...
    See ``cjyx.dmmlScene.GetCacheManager().GetRemoteCacheDirectory()``

    If not explicitly provided or if set to ``None``, the ``loadFileTypes`` are
    guessed based on the corresponding filename extensions.

    If a given fileName has the ``.mrb`` or ``.dmml`` extension, it will **not** be loaded
...
The SampleData module can be used to download data for working with in cjyx.  Use of this module requires an active network connection.
"""
...
<p>MRHead, CBCT-MR Head, and CT-MR Brain data sets were donated to 3D Cjyx project by the persons visible in the images, to be used without any restrictions.</p>
...
"""
        ...
        if not cjyx.app.commandOptions().noMainWindow:
            cjyx.app.connect("startupCompleted()", self.addMenu)
        ...
        try:
            cjyx.modules.sampleDataSources
        except AttributeError:
            cjyx.modules.sampleDataSources = {}

    def addMenu(self):
        a = qt.QAction('Download Sample Data', cjyx.util.mainWindow())
        ...
        for fileName, nodeName, fileType in zip(fileNames, nodeNames, loadFileType):
            ...
            if fileType is None:
                if nodeName is not None:
                    # TODO: Use method from Cjyx IO logic ?
                    fileType = "VolumeFile"
                else:
...
class SampleDataWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        SampleDataWidget.setCategoriesFromSampleDataSources(self.categoryLayout, cjyx.modules.sampleDataSources, self.logic)
    ...
    def logMessage(self, message, logLevel=logging.DEBUG):
        ...
        doc.setHtml(message)
        cjyx.util.showStatusMessage(doc.toPlainText(), 3000)
        ...
        cjyx.app.processEvents(qt.QEventLoop.ExcludeUserInputEvents)
    ...
    def isCategoryVisible(self, category):
        ...
        if not SampleDataLogic.sampleDataSourcesByCategory(category):
            return False
        return cjyx.util.findChild(self.parent, '%sCollapsibleGroupBox' % category).isVisible()
    ...
    def setCategoryVisible(self, category, visible):
        ...
        if not SampleDataLogic.sampleDataSourcesByCategory(category):
            return
        cjyx.util.findChild(self.parent, '%sCollapsibleGroupBox' % category).setVisible(visible)
...
class SampleDataLogic:
    """Manage the cjyx.modules.sampleDataSources dictionary.
    ...
    def registerCustomSampleDataSource(category='Custom',
        ...
        if category not in cjyx.modules.sampleDataSources:
            cjyx.modules.sampleDataSources[category] = []
        ...
        cjyx.modules.sampleDataSources[category].append(dataSource)
    ...
    def sampleDataSourcesByCategory(category=None):
        ...
        if category is None:
            return cjyx.modules.sampleDataSources
        else:
            return cjyx.modules.sampleDataSources.get(category, [])
    ...
    def isSampleDataSourceRegistered(category, sampleDataSource):
        ...
        return sampleDataSource in cjyx.modules.sampleDataSources.get(category, [])
    ...
    def registerBuiltInSampleDataSources(self):
        ...
        if self.builtInCategoryName not in cjyx.modules.sampleDataSources:
            cjyx.modules.sampleDataSources[self.builtInCategoryName] = []
        for sourceArgument in sourceArguments:
            ...
            if SampleDataLogic.isSampleDataSourceRegistered(self.builtInCategoryName, dataSource):
                continue
            cjyx.modules.sampleDataSources[self.builtInCategoryName].append(dataSource)
    ...
    def downloadFileIntoCache(self, uri, name, checksum=None):
        ...
        destFolderPath = cjyx.dmmlScene.GetCacheManager().GetRemoteCacheDirectory()
    ...
    def downloadFromSource(self, source, maximumAttemptsCount=3):
        ...
        load them into Cjyx if it applies.
            ...
            for attemptsCount in range(maximumAttemptsCount):
                ...
                if loadFileType == 'ZipFile':
                    if loadFile is False:
                        ...
                        break
                    outputDir = cjyx.dmmlScene.GetCacheManager().GetRemoteCacheDirectory() + "/" + os.path.splitext(os.path.basename(filePath))[0]
                    qt.QDir().mkpath(outputDir)
                    if cjyx.util.extractArchive(filePath, outputDir):
                        # Success
    ...
    def sourceForSampleName(self, sampleName):
        ...
        for category in cjyx.modules.sampleDataSources.keys():
            for source in cjyx.modules.sampleDataSources[category]:
    ...
    def loadScene(self, uri, fileProperties={}):
        ...
        success = cjyx.app.coreIOManager().loadNodes('SceneFile', fileProperties)
        if not success:
    ...
    def loadNode(self, uri, name, fileType='VolumeFile', fileProperties={}):
        ...
        success = cjyx.app.coreIOManager().loadNodes(fileType, fileProperties, loadedNodes)
        ...
        for i in range(loadedNodes.GetNumberOfItems()):
            ...
            if not loadedNode.IsA("vtkDMMLStorableNode"):
                continue
            storageNode = loadedNode.GetStorageNode()
            if not storageNode:
                continue
            cjyx.dmmlScene.RemoveNode(storageNode)
...
class SampleDataTest(ScriptedLoadableModuleTest):
    ...
    def runTest(self):
        for test in [
            ...
            self.test_downloadFromSource_loadDMMLFile,
            self.test_downloadFromSource_downloadMRBFile,
            self.test_downloadFromSource_downloadDMMLFile,
    ...
    def test_downloadFromSource_downloadFiles(self):
        ...
        without loading into Cjyx.
        """
        logic = SampleDataLogic()

        sceneMTime = cjyx.dmmlScene.GetMTime()
        ...
        self.assertEqual(sceneMTime, cjyx.dmmlScene.GetMTime())
    ...
    def test_downloadFromSource_loadMRBFile(self):
        ...
        filePaths = logic.downloadFromSource(SampleDataSource(
            ...
            loadFiles=True, fileNames='cjyx4minute.mrb'))
        self.assertEqual(len(filePaths), 1)
        self.assertTrue(os.path.exists(filePaths[0]))
        self.assertTrue(os.path.isfile(filePaths[0]))
        self.assertTrue(sceneMTime < cjyx.dmmlScene.GetMTime())

    def test_downloadFromSource_loadDMMLFile(self):
        logic = SampleDataLogic()
        tempFile = qt.QTemporaryFile(cjyx.app.temporaryPath + "/SampleDataTest-loadSceneFile-XXXXXX.dmml")
        tempFile.open()
      ...
      """).strip())
        ...
        sceneMTime = cjyx.dmmlScene.GetMTime()
        filePaths = logic.downloadFromSource(SampleDataSource(
            uris=self.path2uri(tempFile.fileName()), loadFiles=True, fileNames='scene.dmml'))
        self.assertEqual(len(filePaths), 1)
        self.assertTrue(os.path.exists(filePaths[0]))
        self.assertTrue(os.path.isfile(filePaths[0]))
        self.assertTrue(sceneMTime < cjyx.dmmlScene.GetMTime())
    ...
    def test_downloadFromSource_downloadDMMLFile(self):
        logic = SampleDataLogic()
        tempFile = qt.QTemporaryFile(cjyx.app.temporaryPath + "/SampleDataTest-loadSceneFile-XXXXXX.dmml")
        tempFile.open()
        tempFile.write(textwrap.dedent("""
      ...
      """).strip())
        ...
        sceneMTime = cjyx.dmmlScene.GetMTime()
        filePaths = logic.downloadFromSource(SampleDataSource(
            uris=self.path2uri(tempFile.fileName()), fileNames='scene.dmml'))
        self.assertEqual(len(filePaths), 1)
        self.assertTrue(os.path.exists(filePaths[0]))
        self.assertTrue(os.path.isfile(filePaths[0]))
        self.assertEqual(sceneMTime, cjyx.dmmlScene.GetMTime())
    ...
    def test_downloadFromSource_loadNode(self):
        ...
        self.assertEqual(nodes[0], cjyx.dmmlScene.GetFirstNodeByName("MRHead"))
    ...
    def test_downloadFromSource_loadNodeFromMultipleFiles(self):
        ...
        self.assertEqual(len(nodes), 1)
        self.assertEqual(nodes[0], cjyx.dmmlScene.GetFirstNodeByName("DTIVolume"))
    ...
    def test_downloadFromSource_loadNodesWithLoadFileFalse(self):
        ...
        self.assertEqual(nodes[1], cjyx.dmmlScene.GetFirstNodeByName("CTChest"))
    ...
    def test_categoryVisibility(self):
        cjyx.util.selectModule("SampleData")
        widget = cjyx.modules.SampleDataWidget
    ...
    def test_setCategoriesFromSampleDataSources(self):
        ...
        SampleDataWidget.setCategoriesFromSampleDataSources(widget.categoryLayout, cjyx.modules.sampleDataSources, widget.logic)
        self.assertGreater(widget.categoryLayout.count(), 0)
    ...
    def test_customDownloader(self):
        ...
        cjyx.util.selectModule("SampleData")
        widget = cjyx.modules.SampleDataWidget
        button = cjyx.util.findChild(widget.parent, 'customDownloaderPushButton')
    ...
    def test_categoryForSource(self):
        logic = SampleDataLogic()
        source = cjyx.modules.sampleDataSources[logic.builtInCategoryName][0]
