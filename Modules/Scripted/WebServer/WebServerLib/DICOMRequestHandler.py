...
class DICOMRequestHandler(object):
        ...
        if len(splitPath) == 3:
            ...
            for patient in cjyx.dicomDatabase.patients():
                ...
                for study in cjyx.dicomDatabase.studiesForPatient(patient):
                    ...
                    for serie in series:
                        seriesInstances = cjyx.dicomDatabase.instancesForSeries(serie)
                        numberOfStudyRelatedInstances += len(seriesInstances)
                        if len(seriesInstances) > 0:
                            representativeSeries = serie
                            try:
                                dataset = pydicom.dcmread(cjyx.dicomDatabase.fileForInstance(seriesInstances[0]), stop_before_pixels=True)
                    ...
                    if representativeSeries is None:
                        ...
                        continue
                    instances = cjyx.dicomDatabase.instancesForSeries(representativeSeries)
                    firstInstance = instances[0]
                    dataset = pydicom.dcmread(cjyx.dicomDatabase.fileForInstance(firstInstance), stop_before_pixels=True)
            ...
            for serie in series:
                ...
                for instance in seriesInstances:
                    dataset = pydicom.dcmread(cjyx.dicomDatabase.fileForInstance(instance), stop_before_pixels=True)
    ...
    def handleSeries(self, parsedURL, requestBody):
        ...
        if len(splitPath) == 5:
            ...
            series = cjyx.dicomDatabase.seriesForStudy(studyUID)
            for serie in series:
                instances = cjyx.dicomDatabase.instancesForSeries(serie, 1)
                firstInstance = instances[0]
                dataset = pydicom.dcmread(cjyx.dicomDatabase.fileForInstance(firstInstance), stop_before_pixels=True)
            ...
            seriesInstances = cjyx.dicomDatabase.instancesForSeries(seriesUID)
            for instance in seriesInstances:
                dataset = pydicom.dcmread(cjyx.dicomDatabase.fileForInstance(instance), stop_before_pixels=True)
                jsonDataset = dataset.to_json()
                responseBody += jsonDataset.encode() + b","
    ...
    def handleWADOURI(self, parsedURL, requestBody):
        ...
        path = cjyx.dicomDatabase.fileForInstance(instanceUID)
        fp = open(path, 'rb')
