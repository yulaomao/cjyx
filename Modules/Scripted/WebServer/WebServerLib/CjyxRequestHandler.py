...
class CjyxRequestHandler(object):
    """Implements the Cjyx REST api"""
    ...
    def canHandleRequest(self, uri, requestBody):
        ...
        pathParts = os.path.split(parsedURL.path)  # path is like /cjyx/timeimage
        route = pathParts[0]
        return 0.5 if route.startswith(b'/cjyx') else 0.0

    def handleRequest(self, uri, requestBody):
        """Handle a cjyx api request.
        ...
        try:
            ...
            elif request.find(b'/dmml') == 0:
                responseBody, contentType = self.dmml(request)
            ...
            self.logMessage("Could not handle cjyx command: %s" % request)
    ...
    def exec(self, request, requestBody):
        ...
        example:
    curl -X POST localhost:2016/cjyx/exec --data "cjyx.app.layoutManager().setLayout(cjyx.vtkDMMLLayoutNode.CjyxLayoutOneUpRedSliceView)"
        """
    ...
    def setupDMMLTracking(self):
        ...
        if not hasattr(self, "trackingDevice"):
            """ set up the dmml parts or use existing """
            nodes = cjyx.dmmlScene.GetNodesByName('trackingDevice')
            if nodes.GetNumberOfItems() > 0:
                self.trackingDevice = nodes.GetItemAsObject(0)
                nodes = cjyx.dmmlScene.GetNodesByName('tracker')
                ...
                self.cube.Update()
                # display node
                self.modelDisplay = cjyx.vtkDMMLModelDisplayNode()
                self.modelDisplay.SetColor(1, 1, 0)  # yellow
                cjyx.dmmlScene.AddNode(self.modelDisplay)
                # self.modelDisplay.SetPolyData(self.cube.GetOutputPort())
                # Create model node
                self.trackingDevice = cjyx.vtkDMMLModelNode()
                self.trackingDevice.SetScene(cjyx.dmmlScene)
                ...
                cjyx.dmmlScene.AddNode(self.trackingDevice)
                # tracker
                self.tracker = cjyx.vtkDMMLLinearTransformNode()
                self.tracker.SetName('tracker')
                cjyx.dmmlScene.AddNode(self.tracker)
    ...
    def volumeSelection(self, request):
        ...
        currentIndex = 0
        if currentNodeID:
            nodes = cjyx.util.getNodes('vtkDMML*VolumeNode*')
    ...
    def volumes(self, request, requestBody):
        """
        Returns a json list of dmml volume names and ids
        """
        volumes = []
        dmmlVolumes = cjyx.util.getNodes('vtkDMMLScalarVolumeNode*')
        dmmlVolumes.update(cjyx.util.getNodes('vtkDMMLLabelMapVolumeNode*'))
        for id_ in dmmlVolumes.keys():
            volumeNode = dmmlVolumes[id_]
    ...
    def volume(self, request, requestBody):
        ...
        :param id: is the dmml id of the volume to get or put
        """
        p = urllib.parse.urlparse(request.decode())
        q = urllib.parse.parse_qs(p.query)
        try:
            ...
            volumeID = 'vtkDMMLScalarVolumeNode*'
    ...
    def gridTransforms(self, request, requestBody):
        ...
        """
        gridTransforms = []
        dmmlGridTransforms = cjyx.util.getNodes('vtkDMMLGridTransformNode*')
        for id_ in dmmlGridTransforms.keys():
            gridTransform = dmmlGridTransforms[id_]
    ...
    def gridTransform(self, request, requestBody):
        ...
        try:
            ...
            transformID = 'vtkDMMLGridTransformNode*'
    ...
    def postNRRD(self, volumeID, requestBody):
        ...
        :param volumeID: dmml id of the volume to update (new is created if id is invalid)
        ...
        try:
            node = cjyx.util.getNode(volumeID)
        except cjyx.util.DMMLNodeNotFoundException:
            node = None
        if not node:
            node = cjyx.vtkDMMLScalarVolumeNode()
        ...
        array = cjyx.util.array(node.GetID())
        array[:] = pixels.reshape(array.shape)
        imageData.GetPointData().GetScalars().Modified()

        displayNode = node.GetDisplayNode()
        displayNode.ProcessDMMLEvents(displayNode, vtk.vtkCommand.ModifiedEvent, "")
        # TODO: this could be optional
        cjyx.app.applicationLogic().GetSelectionNode().SetReferenceActiveVolumeID(node.GetID())
        cjyx.app.applicationLogic().PropagateVolumeSelection()

        return b"{'status': 'success'}"

    def getNRRD(self, volumeID):
        """Return a nrrd binary blob with contents of the volume node
        :param volumeID: must be a valid dmml id
        """
        volumeNode = cjyx.util.getNode(volumeID)
        volumeArray = cjyx.util.array(volumeID)

        if volumeNode is None or volumeArray is None:
            self.logMessage('Could not find requested volume')
            return None
        supportedNodes = ["vtkDMMLScalarVolumeNode", "vtkDMMLLabelMapVolumeNode"]
...
""".replace("%%scalarType%%", scalarType).replace("%%sizes%%", sizes).replace("%%directions%%", directions).replace("%%origin%%", origin)
    ...
    def getTransformNRRD(self, transformID):
        ...
        """
        transformNode = cjyx.util.getNode(transformID)
        transformArray = cjyx.util.array(transformID)

        if transformNode is None or transformArray is None:
            self.logMessage('Could not find requested transform')
            return None
        supportedNodes = ["vtkDMMLGridTransformNode", ]
        ...
        # (need to make a copy so as not to change the cjyx transform)
...
""".replace("%%sizes%%", sizes).replace("%%directions%%", directions).replace("%%origin%%", origin)
    ...
    def fiducials(self, request, requestBody):
        ...
        """
        fiducials = {}
        for markupsNode in cjyx.util.getNodesByClass('vtkDMMLMarkupsFiducialNode'):
    ...
    def fiducial(self, request, requestBody):
        ...
        :param id: dmml id of the fiducial list
        ...
        try:
            ...
            fiducialID = 'vtkDMMLMarkupsFiducialNode*'
        try:
        ...
        fiducialNode = cjyx.util.getNode(fiducialID)
    ...
    def accessDICOMwebStudy(self, request, requestBody):
        ...
        Cjyx's dicom database, and load it into the scene.
        ...
        for studyUID in loadedUIDs:
            for seriesUID in cjyx.dicomDatabase.seriesForStudy(studyUID):
                for instance in cjyx.dicomDatabase.instancesForSeries(seriesUID):
                    files.append(cjyx.dicomDatabase.fileForInstance(instance))
    ...
    def dmml(self, request):
        """
        Returns a json list of all the dmml nodes
        """
        p = urllib.parse.urlparse(request.decode())
        q = urllib.parse.parse_qs(p.query)
        return (json.dumps(list(cjyx.util.getNodes('*').keys())).encode()), b'application/json'
    ...
    def screenshot(self, request):
        ...
        screenshot = cjyx.util.mainWindow().grab()
    ...
    def setViewersLayout(layoutName):
        for att in dir(cjyx.vtkDMMLLayoutNode):
            if att.startswith("CjyxLayout") and att.endswith("View"):
                foundLayoutName = att[12:-4]
                if layoutName.lower() == foundLayoutName.lower():
                    layoutId = eval(f"cjyx.vtkDMMLLayoutNode.{att}")
    ...
    def gui(self, request):
        ...
        :param viewersLayout: {fourup, oneup3d, ...} cjyx.vtkDMMLLayoutNode constants (CjyxLayout...View)
        ...
        try:
            ...
            contents = None
        if contents == "viewers":
            cjyx.util.findChild(cjyx.util.mainWindow(), "PanelDockWidget").hide()
            cjyx.util.setStatusBarVisible(False)
            cjyx.util.setMenuBarsVisible(False)
            cjyx.util.setToolbarsVisible(False)
        elif contents == "full":
            cjyx.util.findChild(cjyx.util.mainWindow(), "PanelDockWidget").show()
            cjyx.util.setStatusBarVisible(True)
            cjyx.util.setMenuBarsVisible(True)
            cjyx.util.setToolbarsVisible(True)
        else:
            if contents:
        ...
        if viewersLayout is not None:
            CjyxRequestHandler.setViewersLayout(viewersLayout)
    ...
    def slice(self, request):
        ...
        if copySliceGeometryFrom:
            ...
            # technique from vtkDMMLSliceLinkLogic (TODO: should be exposed as method)
    ...
    def timeimage(self, request=''):
        ...
        cjyx.qDMMLUtils().qImageToVtkImageData(timeImage, vtkTimeImage)
