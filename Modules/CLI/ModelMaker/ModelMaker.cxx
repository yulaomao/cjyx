...
int main(int argc, char * argv[])
{
  ...
  if (ModelSceneFile.size() == 0)
    ...
    sceneFilename = vtksys::SystemTools::GetFilenameWithoutExtension(InputVolume) + std::string(".dmml");
    std::cerr << "********\nERROR: no model scene defined! Using " << sceneFilename << endl;
    std::cerr
    <<
    "WARNING: If you started Model Maker from the Cjyx3 GUI, the models will NOT be loaded automatically.\nYou must use File->Import Scene "
  ...
  vtkNew<vtkDMMLScene> modelScene;


  // load the scene that Cjyx will re-read
  ...
  vtkDMMLNode *                              rnd = modelScene->GetNodeByID(modelHierarchyID);
  vtkSmartPointer<vtkDMMLModelHierarchyNode> rtnd;
  if (!rnd)
    {
    std::cerr << "Error: no model hierarchy node at ID \""
              << modelHierarchyID << "\", creating one" << std::endl;
//      return EXIT_FAILURE;
    rtnd = vtkSmartPointer<vtkDMMLModelHierarchyNode>::New();
    rtnd->SetHideFromEditors(0);
    modelScene->AddNode(rtnd);
    // now get it again as a dmml node so can add things under it
    ...
    rtnd = vtkDMMLModelHierarchyNode::SafeDownCast(rnd);
    }

  // if there's a color based model hierarchy file, import it into the model scene
  vtkDMMLModelHierarchyNode *topColorHierarchyNode = nullptr;
  if (ModelHierarchyFile.length() > 0)
    ...
    vtkDMMLNode * mnode = modelScene->GetNthNodeByClass(1,"vtkDMMLModelHierarchyNode");
    if (mnode != nullptr)
      {
      topColorHierarchyNode = vtkDMMLModelHierarchyNode::SafeDownCast(mnode);
      }
  ...
  if (topColorHierarchyNode != nullptr)
    ...
    std::vector< vtkDMMLHierarchyNode* > children = rtnd->GetChildrenNodes();
  ...
  vtkSmartPointer<vtkDMMLColorTableNode>        colorNode;
  vtkSmartPointer<vtkDMMLColorTableStorageNode> colorStorageNode;
  ...
  const char modelFileHeader[] = "3D Cjyx output. SPACE=LPS"; // models are saved in LPS coordinate system
  ...
  // - strings that start with cjyx: are shared memory references, so they won't exist.
  //   The memory address starts with 0x in linux but not on Windows
  if (InputVolume.find(std::string("cjyx:")) != 0)
    {
    FILE * infile;
  ...
  if (useColorNode)
    {
    colorNode = vtkSmartPointer<vtkDMMLColorTableNode>::New();
    ...
    colorStorageNode = vtkSmartPointer<vtkDMMLColorTableStorageNode>::New();
  ...
  vtkNew<vtkDMMLModelDisplayNode> dnd;
  dnd->SetVisibility(1);
...
*/
  ...
  for(::size_t l = 0; l < loopLabels.size(); l++)
    ...
    if (!skipLabel)
      ...
      if (modelScene.GetPointer() != nullptr)
        ...
        // each model needs a dmml node, a storage node and a display node
        vtkNew<vtkDMMLModelNode> mnode;
        mnode->SetScene(modelScene.GetPointer());
        mnode->SetName(labelName.c_str());

        vtkNew<vtkDMMLModelStorageNode> snode;
        ...
        if (colorName.compare("") != 0)
          {
          dmmlNode = modelScene->GetFirstNodeByName(colorName.c_str());
          }
        // if there's no color hierarchy, or no color name or the dmml node
        // named for the color isn't a model hierarchy node, use a flat hierarchy
        if (topColorHierarchyNode == nullptr ||
            colorName.compare("") == 0 ||
            dmmlNode == nullptr ||
            strcmp(dmmlNode->GetClassName(),"vtkDMMLModelHierarchyNode") != 0)
          {
          vtkNew<vtkDMMLModelHierarchyNode> mhnd;
          ...
          vtkDMMLModelHierarchyNode *colorHierarchyNode = vtkDMMLModelHierarchyNode::SafeDownCast(dmmlNode);
  ...
  if (sceneFilename != "")
    ...
    if (topColorHierarchyNode != nullptr)
      ...
      for (unsigned int i = 0; i < allChildren.size(); i++)
        ...
        if (allChildren[i]->GetAssociatedNodeID() == nullptr &&
          ...
          if (vtkDMMLDisplayableHierarchyNode::SafeDownCast(allChildren[i]) != nullptr)
            {
            vtkDMMLDisplayNode *hierarchyDisplayNode = vtkDMMLDisplayableHierarchyNode::SafeDownCast(allChildren[i])->GetDisplayNode();
    ...
    if (ModelSceneFile.size() == 0)
      {
      std::cout << "\nIf you ran this from Cjyx3's GUI, use File->Import Scene... " << sceneFilename.c_str()
                << " to load your models.\n";
      }
    }
