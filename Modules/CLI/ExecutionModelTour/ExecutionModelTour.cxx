...
int main(int argc, char* argv[])
{
  ...
  vtkDMMLNode* node = scene->GetNodeByID(transform1ID);
  if (node)
    {
    vtkDMMLNode* outNode = scene->GetNodeByID(transform2ID);
  ...
  if (!seedsFile.empty())
    ...
    vtkNew<vtkDMMLMarkupsFiducialStorageNode> fiducialStorageNode;
  ...
  // copy the seeds list
  vtkNew<vtkDMMLMarkupsFiducialNode> copiedFiducialNode;
  ...
  if (!seedsOutFile.empty())
    {
    vtkNew<vtkDMMLMarkupsFiducialStorageNode> outputFiducialStorageNode;
