...
#ifndef __vtkDMMLViewInteractorStyle_h
#define __vtkDMMLViewInteractorStyle_h
...
/// \brief Common base class for processing interaction events in DMML views
///
...
/// are generated here.
class VTK_DMML_DISPLAYABLEMANAGER_EXPORT vtkDMMLViewInteractorStyle :
  public vtkInteractorStyle3D
{
public:
  static vtkDMMLViewInteractorStyle *New();
  vtkTypeMacro(vtkDMMLViewInteractorStyle,vtkInteractorStyle3D);
  ...
  void SetDisplayableManagers(vtkDMMLDisplayableManagerGroup* displayableManagers);

  /// Give a chance to displayable managers to process the event.
  /// It just creates vtkDMMLInteractionEventData and calls
  ...
  /// Give a chance to displayable managers to process the event.
  /// Return true if the event is processed.
  virtual bool DelegateInteractionEventDataToDisplayableManagers(vtkDMMLInteractionEventData* eventData);
...
protected:
  vtkDMMLViewInteractorStyle();
  ~vtkDMMLViewInteractorStyle() override;
  ...
  vtkWeakPointer<vtkDMMLDisplayableManagerGroup> DisplayableManagers;
  vtkDMMLAbstractDisplayableManager* FocusedDisplayableManager;

private:
  vtkDMMLViewInteractorStyle(const vtkDMMLViewInteractorStyle&) = delete;
  void operator=(const vtkDMMLViewInteractorStyle&) = delete;
};
