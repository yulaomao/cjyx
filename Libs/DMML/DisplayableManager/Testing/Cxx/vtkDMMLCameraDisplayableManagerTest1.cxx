...
char vtkDMMLCameraDisplayableManagerTest1EventLog[] =
"# StreamVersion 1\n"
...
//----------------------------------------------------------------------------
int vtkDMMLCameraDisplayableManagerTest1(int argc, char* argv[])
{
  ...
  vtkDMMLCameraDisplayableManager * cameraDM2 =
      vtkDMMLCameraDisplayableManager::SafeDownCast(
          factory->GetDisplayableManagerByClassName("vtkDMMLCameraDisplayableManager"));
  if (cameraDM2 != cameraDM)
    {
    std::cerr << "Failed to retrieve vtkDMMLCameraDisplayableManager using "
        << "GetDisplayableManagerByClassName" << std::endl;
    return EXIT_FAILURE;
    }

  vtkDMMLViewDisplayableManager * viewDM2 =
      vtkDMMLViewDisplayableManager::SafeDownCast(
          factory->GetDisplayableManagerByClassName("vtkDMMLViewDisplayableManager"));
  if (viewDM2 != viewDM)
    {
    std::cerr << "Failed to retrieve vtkDMMLViewDisplayableManager using "
        << "GetDisplayableManagerByClassName" << std::endl;
    return EXIT_FAILURE;
    }
    */
  ...
  vtkSmartPointer<vtkDMMLDisplayableManagerGroup> displayableManagerGroup =
      vtkSmartPointer<vtkDMMLDisplayableManagerGroup>::Take(factory->InstantiateDisplayableManagers(rr.GetPointer()));
  ...
  savedScene += "/vtkDMMLCameraDisplayableManagerTest1_saved.dmml";
  scene->SetVersion("Cjyx4.4.0"); // Force scene version to be the same as in the baseline scene file
  ...
  baselineScene += "/Data/vtkDMMLCameraDisplayableManagerTest1.dmml";
  ...
  if (!disableReplay)
    ...
    if (record)
      ...
      recorder->SetInputString(vtkDMMLCameraDisplayableManagerTest1EventLog);
      recorder->Play();
      }
    }
