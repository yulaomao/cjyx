...
char vtkDMMLThreeDReformatDisplayableManagerTest1EventLog[] =
  "# StreamVersion 1\n"
...
//----------------------------------------------------------------------------
int vtkDMMLThreeDReformatDisplayableManagerTest1(int argc, char* argv[])
{
  ...
  // Add ViewNode
  vtkNew<vtkDMMLViewLogic> viewLogic;
  viewLogic->SetDMMLScene(scene);
  vtkDMMLViewNode* viewNode = viewLogic->AddViewNode("1");
  ...
  vtkNew<vtkDMMLThreeDReformatDisplayableManager> reformatDisplayableManager;
  reformatDisplayableManager->SetDMMLApplicationLogic(applicationLogic);
  displayableManagerGroup->AddDisplayableManager(reformatDisplayableManager.GetPointer());

  // Need to add vtkDMMLCameraDisplayableManager, as this processes camera manipulation events
  vtkNew<vtkDMMLCameraDisplayableManager> cameraDisplayableManager;
  cameraDisplayableManager->SetDMMLApplicationLogic(applicationLogic);
  ...
  // Visible when added
  vtkNew<vtkDMMLSliceNode> sliceNodeRed;
  // TODO: This color should be taken into account, not the layout name
  sliceNodeRed->SetLayoutColor(vtkDMMLAbstractViewNode::GetRedColor());
  sliceNodeRed->SetWidgetVisible(1);

  scene->AddNode(sliceNodeRed.GetPointer());

  // Locked to Camera
  vtkNew<vtkDMMLSliceNode> sliceNodeYellow;
  sliceNodeYellow->SetLayoutColor(vtkDMMLAbstractViewNode::GetYellowColor());
  ...
  // Delayed Visibility
  vtkNew<vtkDMMLSliceNode> sliceNodeGreen;
  sliceNodeGreen->SetLayoutColor(vtkDMMLAbstractViewNode::GetGreenColor());
  ...
  // Deleted slice
  vtkNew<vtkDMMLSliceNode> sliceNodeDeleted;
  ...
  // Imported slice
  vtkNew<vtkDMMLSliceNode> sliceNodeImported;
  ...
  // Restored slice
  vtkNew<vtkDMMLSliceNode> sliceNodeRestored;
  ...
  if (!disableReplay)
    ...
    if (record)
      ...
      recorder->SetInputString(vtkDMMLThreeDReformatDisplayableManagerTest1EventLog);
      recorder->Play();
      }
    }
  ...
  if (record || screenshot)
    ...
    screenshootFilename += "/Baseline/vtkDMMLThreeDReformatDisplayableManagerTest1.png";
  ...
  reformatDisplayableManager->SetDMMLApplicationLogic(nullptr);
