...
#define DMMLDisplayableManagerCxxTests_AUTOINIT 1(DMMLDisplayableManagerCxxTests)
#include <vtkAutoInit.h>
VTK_AUTOINIT(DMMLDisplayableManagerCxxTests)

//----------------------------------------------------------------------------
int vtkDMMLThreeDViewDisplayableManagerFactoryTest1(int vtkNotUsed(argc), char* vtkNotUsed(argv)[])
{
  vtkDMMLThreeDViewDisplayableManagerFactory * factory = vtkDMMLThreeDViewDisplayableManagerFactory::GetInstance();
  if (!factory)
    {
  ...
  // should be the same
  vtkNew<vtkDMMLThreeDViewDisplayableManagerFactory> factoryUsingSmartPointer;
  ...
  bool isRegistrered = factory->IsDisplayableManagerRegistered("vtkDMMLINVALIDManager");
  if (isRegistrered)
    {
    std::cerr << "Line " << __LINE__
  ...
  isRegistrered = factory->IsDisplayableManagerRegistered("vtkDMMLCameraDisplayableManager");
  if (isRegistrered)
    {
    std::cerr << "Line " << __LINE__
        << " - Problem with method factory->IsDisplayableManagerRegistered()" << std::endl;
  ...
  bool success = factory->RegisterDisplayableManager("vtkDMMLINVALIDManager");
  if (success)
    {
    std::cerr << "Line " << __LINE__
  ...
  success = factory->RegisterDisplayableManager("vtkDMMLCameraDisplayableManager");
  if (!success)
    {
    std::cerr << "Line " << __LINE__
        << " - Problem with method factory->RegisterDisplayableManager()" << std::endl;
  ...
  //----------------------------------------------------------------------------
  // Check if IsDisplayableManagerRegistered works as expected
  isRegistrered = factory->IsDisplayableManagerRegistered("vtkDMMLINVALIDManager");
  if (isRegistrered)
    {
    std::cerr << "Line " << __LINE__
        << " - Problem with method factory->IsDisplayableManagerRegistered()" << std::endl;
  ...
  vtkDMMLDisplayableManagerGroup * group =
  ...
  vtkDMMLAbstractDisplayableManager * dm1 =
      group->GetDisplayableManagerByClassName("vtkDMMLViewDisplayableManager");
  if (!dm1)
    {
    std::cerr << "Line " << __LINE__
  ...
  if (!dm1->IsA("vtkDMMLViewDisplayableManager"))
    {
    std::cerr << "Line " << __LINE__
        << " - Problem with group->GetDisplayableManagerByClassName() method" << std::endl;
    std::cerr << "\tExpected: vtkDMMLViewDisplayableManager" << std::endl;
    std::cerr << "\tCurrent: " << dm1->GetClassName() << std::endl;
    }

  vtkDMMLAbstractDisplayableManager * dm2 =
  ...
  else if (!dm2->IsA("vtkDMMLCameraDisplayableManager"))
    {
    std::cerr << "Line " << __LINE__
        << " - Problem with group->GetDisplayableManagerByClassName() method" << std::endl;
    std::cerr << "\tExpected: vtkDMMLCameraDisplayableManager" << std::endl;
    std::cerr << "\tCurrent: " << dm2->GetClassName() << std::endl;
    }

  //----------------------------------------------------------------------------
  // Attempt to UnRegister a DisplayableManager already instantiated in a Group
  success = factory->UnRegisterDisplayableManager("vtkDMMLCameraDisplayableManager");
