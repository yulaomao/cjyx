...
vtkStandardNewMacro(vtkDMMLCameraWidget);

//----------------------------------------------------------------------------------
vtkDMMLCameraWidget::vtkDMMLCameraWidget()
{
  ...
  // Context menu
  this->SetEventTranslation(WidgetStateIdle, vtkDMMLInteractionEventData::RightButtonClickEvent, vtkEvent::NoModifier, WidgetEventMenu);
...
//----------------------------------------------------------------------
void vtkDMMLCameraWidget::CreateDefaultRepresentation()
{
...
//-----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::CanProcessInteractionEvent(vtkDMMLInteractionEventData* eventData, double &distance2)
{
...
//-----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessInteractionEvent(vtkDMMLInteractionEventData* eventData)
{
  if (!this->CameraNode)
    {
    return false;
  ...
  switch (widgetEvent)
    ...
    case WidgetEventCameraRotateToAnterior:
      ...
      this->CameraNode->RotateTo(vtkDMMLCameraNode::Anterior);
      break;
    case WidgetEventCameraRotateToPosterior:
      this->SaveStateForUndo();
      this->CameraNode->RotateTo(vtkDMMLCameraNode::Posterior);
      break;
    case WidgetEventCameraRotateToRight:
      this->SaveStateForUndo();
      this->CameraNode->RotateTo(vtkDMMLCameraNode::Right);
      break;
    case WidgetEventCameraRotateToLeft:
      this->SaveStateForUndo();
      this->CameraNode->RotateTo(vtkDMMLCameraNode::Left);
      break;
    case WidgetEventCameraRotateToSuperior:
      this->SaveStateForUndo();
      this->CameraNode->RotateTo(vtkDMMLCameraNode::Superior);
      break;
    case WidgetEventCameraRotateToInferior:
      this->SaveStateForUndo();
      this->CameraNode->RotateTo(vtkDMMLCameraNode::Inferior);
      break;

    case WidgetEventCameraTranslateBackwardX:
      this->SaveStateForUndo();
      this->CameraNode->TranslateAlong(vtkDMMLCameraNode::X, true); // screen X is towards left
      break;
    case WidgetEventCameraTranslateForwardX:
      this->SaveStateForUndo();
      this->CameraNode->TranslateAlong(vtkDMMLCameraNode::X, false); // screen X is towards left
      break;
    case WidgetEventCameraTranslateBackwardY:
      this->SaveStateForUndo();
      this->CameraNode->TranslateAlong(vtkDMMLCameraNode::Y, false);
      break;
    case WidgetEventCameraTranslateForwardY:
      this->SaveStateForUndo();
      this->CameraNode->TranslateAlong(vtkDMMLCameraNode::Y, true);
      break;
    case WidgetEventCameraTranslateBackwardZ:
      this->SaveStateForUndo();
      this->CameraNode->TranslateAlong(vtkDMMLCameraNode::Z, false);
      break;
    case WidgetEventCameraTranslateForwardZ:
      this->SaveStateForUndo();
      this->CameraNode->TranslateAlong(vtkDMMLCameraNode::Z, true);
      break;

    case WidgetEventCameraRotateCcwX:
      this->SaveStateForUndo();
      this->CameraNode->RotateAround(vtkDMMLCameraNode::R, false);
      break;
    case WidgetEventCameraRotateCwX:
      this->SaveStateForUndo();
      this->CameraNode->RotateAround(vtkDMMLCameraNode::R, true);
      break;
    case WidgetEventCameraRotateCcwY:
      this->SaveStateForUndo();
      this->CameraNode->RotateAround(vtkDMMLCameraNode::A, false);
      break;
    case WidgetEventCameraRotateCwY:
      this->SaveStateForUndo();
      this->CameraNode->RotateAround(vtkDMMLCameraNode::A, true);
      break;
    case WidgetEventCameraRotateCcwZ:
      this->SaveStateForUndo();
      this->CameraNode->RotateAround(vtkDMMLCameraNode::S, false);
      break;
    case WidgetEventCameraRotateCwZ:
      this->SaveStateForUndo();
      this->CameraNode->RotateAround(vtkDMMLCameraNode::S, true);
      break;
  ...
  if (processedEvent)
    ...
    // through the qDMMLThreeDView to the cameraNode
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessMouseMove(vtkDMMLInteractionEventData* eventData)
{
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessStartMouseDrag(vtkDMMLInteractionEventData* eventData)
{
  this->SaveStateForUndo();
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessEndMouseDrag(vtkDMMLInteractionEventData* eventData)
{
  if (this->Renderer && this->Renderer->GetRenderWindow() && this->Renderer->GetRenderWindow()->GetInteractor())
    {
    vtkInteractorStyle* interactorStyle = vtkInteractorStyle::SafeDownCast(this->Renderer->GetRenderWindow()->GetInteractor()->GetInteractorStyle());
...
//----------------------------------------------------------------------------------
void vtkDMMLCameraWidget::PrintSelf(ostream& os, vtkIndent indent)
{
  Superclass::PrintSelf(os,indent);
}

//----------------------------------------------------------------------------------
void vtkDMMLCameraWidget::SetCameraNode(vtkDMMLCameraNode* cameraNode)
{
...
//----------------------------------------------------------------------------------
vtkDMMLCameraNode* vtkDMMLCameraWidget::GetCameraNode()
{
  return this->CameraNode;
}

//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessSetCrosshairBackground(vtkDMMLInteractionEventData* eventData)
{
...
//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessSetCrosshair(vtkDMMLInteractionEventData* eventData)
{
  ...
  vtkDMMLScene* scene = this->GetCameraNode()->GetScene();
  ...
  if (crosshairNode->GetCrosshairBehavior() != vtkDMMLCrosshairNode::NoAction)
    ...
    if (this->GetCameraNode() && this->GetCameraNode()->GetLayoutName())
      ...
      vtkDMMLViewNode* viewNode = vtkDMMLViewNode::SafeDownCast(
        scene->GetSingletonNode(this->GetCameraNode()->GetLayoutName(), "vtkDMMLViewNode"));
      if (viewNode)
        {
    ...
    vtkDMMLSliceNode::JumpAllSlices(scene, worldPos[0], worldPos[1], worldPos[2], viewJumpSliceMode, viewGroup);
    }
...
//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessRotate(vtkDMMLInteractionEventData* eventData)
{
...
//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessSpin(vtkDMMLInteractionEventData* eventData)
{
  if (!this->Renderer || !eventData)
    {
    return false;
...
//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessTranslate(vtkDMMLInteractionEventData* eventData)
{
  if (!this->Renderer || !eventData)
    {
    return false;
...
//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessScale(vtkDMMLInteractionEventData* eventData)
{
  if (!this->Renderer || !eventData)
    {
    return false;
  ...
  // Cjyx: pull mouse towards you to bring models closer (opposite of vtk)
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessTouchGestureStart(vtkDMMLInteractionEventData* vtkNotUsed(eventData))
{
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessTouchGestureEnd(vtkDMMLInteractionEventData* vtkNotUsed(eventData))
{
  this->SetWidgetState(WidgetStateIdle);
  return true;
}
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessTouchCameraSpin(vtkDMMLInteractionEventData* eventData)
{
  vtkCamera* camera = this->GetCamera();
  if (!camera)
    {
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessTouchCameraZoom(vtkDMMLInteractionEventData* eventData)
{
  vtkCamera* camera = this->GetCamera();
  if (!camera)
    {
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessTouchCameraTranslate(vtkDMMLInteractionEventData* eventData)
{
  vtkCamera* camera = this->GetCamera();
...
//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::Dolly(double factor)
{
  if (!this->Renderer)
    {
    return false;
...
//----------------------------------------------------------------------------
vtkCamera* vtkDMMLCameraWidget::GetCamera()
{
  if (this->CameraNode)
    {
...
//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::CameraModifyStart()
{
  if (this->CameraNode)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLCameraWidget::CameraModifyEnd(bool wasModified, bool updateClippingRange, bool updateLights)
{
...
//----------------------------------------------------------------------------
void vtkDMMLCameraWidget::SaveStateForUndo()
{
  if (!this->GetCameraNode() || !this->GetCameraNode()->GetScene())
    {
    return;
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessWidgetMenu(vtkDMMLInteractionEventData* eventData)
{
  if (this->WidgetState != WidgetStateIdle)
    {
    return false;
  ...
  vtkDMMLAbstractViewNode* viewNode = eventData->GetViewNode();
  if (!viewNode)
    {
    return false;
    }
...
//-------------------------------------------------------------------------
bool vtkDMMLCameraWidget::ProcessMaximizeView(vtkDMMLInteractionEventData* eventData)
{
  if (this->WidgetState != WidgetStateIdle)
    {
    return false;
  ...
  vtkDMMLLayoutNode* layoutNode = viewNode->GetMaximizedState(isMaximized, canBeMaximized);
...
//----------------------------------------------------------------------------
bool vtkDMMLCameraWidget::GetTiltLocked()
{
  return this->CameraTiltLocked;
}

//----------------------------------------------------------------------------
void vtkDMMLCameraWidget::SetTiltLocked(bool lockState)
{
