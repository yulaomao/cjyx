...
Program:   3D Cjyx
Module:    $RCSfile: itkDMMLIDImageIO.cxx,v $
...
{
  // if this is a DMML node, then filename will be encoded
  // with a "cjyx" scheme.  There are two possibilities:
  //
  // cjyx:<scene id>/<node id>                  - local cjyx
  // cjyx://<hostname>/<scene id>/<node id>     - remote cjyx
  //
  // What about multiple instances of cjyx on a machine? (ports?)
  //
  ...
  // check that the filename starts with the cjyx3 scheme
  loc = fname.find("cjyx:");
  if (loc == 0)
    {
    this->m_Scheme = std::string(fname.begin(),
                               fname.begin() + std::string("cjyx").size());
    ...
    // can cast down to a DMMLVolumeNode
    //
    vtkDMMLNode *node = scene->GetNodeByID(this->m_NodeID.c_str());

    if (node)
      {
      vtkDMMLVolumeNode *vnode = vtkDMMLVolumeNode::SafeDownCast(node);
...
::ReadImageInformation()
{
  ...
  if (node)
    ...
    if (node->GetImageData() == nullptr)
      {
      itkWarningMacro("itkDMMLIDImageIO: ReadImageInformation: node image data is null.");
      return;
      }
    if (node->GetImageData()->GetDimensions() == nullptr)
      {
      itkWarningMacro("itkDMMLIDImageIO: ReadImageInformation: node image data has null dimensions");
      return;
      }
    ...
    if (vtkDMMLDiffusionTensorVolumeNode::SafeDownCast(node) == nullptr)
      {
    ...
    else if (vtkDMMLDiffusionTensorVolumeNode::SafeDownCast(node) != nullptr)
      {
      // tensor at each voxel
      this->SetPixelType(DIFFUSIONTENSOR3D);
      dataType = node->GetImageData()->GetPointData()->GetTensors()->GetDataType();
      }
    else if (vtkDMMLDiffusionWeightedVolumeNode::SafeDownCast(node) != nullptr)
      {
      // raw DWI
      this->SetPixelType(VECTOR);
      dataType = node->GetImageData()->GetScalarType();
      }
    else if (vtkDMMLDiffusionImageVolumeNode::SafeDownCast(node) != nullptr)
      {
    ...
    if (vtkDMMLDiffusionImageVolumeNode::SafeDownCast(node) != nullptr)
      {
      vtkDMMLDiffusionImageVolumeNode *di
        = vtkDMMLDiffusionImageVolumeNode::SafeDownCast(node);
      ...
      vtkDMMLDiffusionWeightedVolumeNode *dw
        = vtkDMMLDiffusionWeightedVolumeNode::SafeDownCast(node);
...
::GetOwnBuffer()
{
  ...
  throw( "DMML Node does not contain image data." );
...
void
DMMLIDImageIO
::WriteImageInformation(vtkDMMLVolumeNode *node, vtkImageData *img,
...
{
  vtkDMMLVolumeNode *node = this->FileNameToVolumeNodePtr( m_FileName.c_str() );
  if (!node)
    {
    return;
    }
  ...
  for (int i = 0; i < node->GetNumberOfDisplayNodes(); ++i)
    {
    vtkDMMLDisplayNode* displayNode = node->GetNthDisplayNode(i);
    if (displayNode)
      {
...
//----------------------------------------------------------------------------
void DMMLIDImageIO::RequestModified(vtkDMMLNode* modifiedObject)
{
...
//      }
  ...
  // itkNRRDImageIO or itkDMMLIDImageIO
...
void
DMMLIDImageIO
::SetDWNodeValues(vtkDMMLDiffusionWeightedVolumeNode *dw,
...
//      }
  ...
  // DMMLIDImageIO, just like it is for NRRDImageIO
  std::string bValueKey = "DWMRI_b-value";
  ...
  // MetaDataDictionary in DMMLIDImageIO, just like for NRRDImageIO
  std::string diffusionGradientKey = "DWMRI_gradient_";
...
void
DMMLIDImageIO
::SetDTNodeValues(vtkDMMLDiffusionImageVolumeNode *di,
                  MetaDataDictionary &dict)
{
  // Measurement frame
  std::string measurementFrameKey = "NRRD_measurement frame";
