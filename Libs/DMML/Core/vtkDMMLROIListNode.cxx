...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLROIListNode);


//----------------------------------------------------------------------------
vtkDMMLROIListNode::vtkDMMLROIListNode()
{
  this->ROIList = vtkCollection::New();
  this->TextScale = 4.5;
  this->Visibility = 1;
  this->Color[0]=0.4; this->Color[1]=1.0; this->Color[2]=1.0;
  // from cjyx 2: 1.0 0.5 0.5"
...
//----------------------------------------------------------------------------
void vtkDMMLROIListNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLROIListNode::ReadXMLAttributes(const char** atts)
{
  ...
  while (*atts != nullptr)
    ...
    if (!strcmp(attName, "TextScale"))
      ...
      while (IDPtr != nullptr)
        ...
        vtkDMMLROINode *newROINode = this->GetNthROINode(ROIIndex);
        ...
        if (newROINode != nullptr)
          ...
          newROINode->ReadXMLString(IDPtr);
            ...
            vtkErrorMacro ("ERROR making a new DMML ROINode!\n");
          }
        newROINode = nullptr;
...
//----------------------------------------------------------------------------
void vtkDMMLROIListNode::Copy(vtkDMMLNode * vtkNotUsed(anode))
{
  //Fill in
  return;
}
...
//----------------------------------------------------------------------------
void vtkDMMLROIListNode::PrintSelf(ostream& os, vtkIndent indent)
{
  //int idx
...
//-----------------------------------------------------------
void vtkDMMLROIListNode::UpdateScene(vtkDMMLScene *scene)
{
...
//-----------------------------------------------------------
void vtkDMMLROIListNode::UpdateReferences()
{
...
//----------------------------------------------------------------------------
int vtkDMMLROIListNode::GetNumberOfROIs()
{
    return this->ROIList->vtkCollection::GetNumberOfItems();
}

//----------------------------------------------------------------------------
int vtkDMMLROIListNode::AddROI()
{
  if ( !this->Scene )
    {
  ...
  // create a vtkDMMLROI and return the ROI number for later
  // access
  vtkDMMLROINode * ROINode = vtkDMMLROINode::New();
...
//----------------------------------------------------------------------------
void vtkDMMLROIListNode::RemoveROI(int i)
{
  this->ROIList->vtkCollection::RemoveItem(i);
  this->InvokeEvent(vtkDMMLScene::NodeRemovedEvent, nullptr);
  this->Modified();
}

//----------------------------------------------------------------------------
void vtkDMMLROIListNode::RemoveAllROIs()
{
  ...
  for (int r = numPoints - 1; r >= 0; r--)
    ...
    vtkDMMLROIListNode *ROINode = vtkDMMLROIListNode::SafeDownCast(this->ROIList->vtkCollection::GetItemAsObject(r));
...
//---------------------------------------------------------------------------
void vtkDMMLROIListNode::ProcessDMMLEvents ( vtkObject *caller,
                                            unsigned long event,
                                            void *callData )
{
...
//---------------------------------------------------------------------------
void vtkDMMLROIListNode::SetColor(double r, double g, double b)
{
  if (this->Color[0] == r &&
    this->Color[1] == g &&
    this->Color[2] == b)
  ...
  this->InvokeEvent(vtkDMMLROIListNode::DisplayModifiedEvent);
  return;
}

//---------------------------------------------------------------------------
void vtkDMMLROIListNode::SetSelectedColor(double r, double g, double b)
{
...
//---------------------------------------------------------------------------
void vtkDMMLROIListNode::SetColor(double c[3])
{
  // set the color
...
//---------------------------------------------------------------------------
void vtkDMMLROIListNode::SetSelectedColor(double c[3])
{
...
//---------------------------------------------------------------------------
void vtkDMMLROIListNode::SetTextScale(double scale)
{
...
//---------------------------------------------------------------------------
void vtkDMMLROIListNode::SetVisibility(int visible)
{
...
//---------------------------------------------------------------------------
void vtkDMMLROIListNode::SetOpacity(double opacity)
{
...
//----------------------------------------------------------------------------
int vtkDMMLROIListNode::SetNthROIXYZ(int n, double x, double y, double z)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node == nullptr)
    {
  ...
  if (this->VolumeNodeID != nullptr)
    {
    vtkDMMLVolumeNode *volumeNode  = vtkDMMLVolumeNode::SafeDownCast(this->Scene->GetNodeByID(this->VolumeNodeID));
    if (volumeNode)
      {
  ...
  node = nullptr;
  this->InvokeEvent(vtkDMMLROIListNode::ROIModifiedEvent, nullptr);
  return 0;
}

//----------------------------------------------------------------------------
int vtkDMMLROIListNode::SetNthROIIJK(int n, double i, double j, double k)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node == nullptr)
    {
...
//----------------------------------------------------------------------------
double * vtkDMMLROIListNode::GetNthROIXYZ(int n)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node != nullptr)
    {
...
//----------------------------------------------------------------------------
double * vtkDMMLROIListNode::GetNthROIIJK(int n)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node != nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLROIListNode::SetNthROIRadiusXYZ(int n, double Radiusx, double Radiusy, double Radiusz)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node == nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLROIListNode::SetNthROIRadiusIJK(int n, double Radiusi, double Radiusj, double Radiusk)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node == nullptr)
    {
...
//----------------------------------------------------------------------------
double * vtkDMMLROIListNode::GetNthROIRadiusXYZ(int n)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node != nullptr)
    {
...
//----------------------------------------------------------------------------
double * vtkDMMLROIListNode::GetNthROIRadiusIJK(int n)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node != nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLROIListNode::SetNthROILabelText(int n, const char *text)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node == nullptr)
    {
...
//----------------------------------------------------------------------------
const char *vtkDMMLROIListNode::GetNthROILabelText(int n)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node != nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLROIListNode::SetNthROISelected(int n, int flag)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node == nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLROIListNode::GetNthROISelected(int n)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node != nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLROIListNode::SetNthROIID(int n, const char * vtkNotUsed(id))
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node == nullptr)
    {
...
//----------------------------------------------------------------------------
const char *vtkDMMLROIListNode::GetNthROIID(int n)
{
  vtkDMMLROINode *node = this->GetNthROINode(n);
  if (node != nullptr)
    {
...
//----------------------------------------------------------------------------
vtkDMMLROINode* vtkDMMLROIListNode::GetNthROINode(int n)
{
  ...
  if (n < 0 || n >= this->ROIList->GetNumberOfItems())
    {
    vtkErrorMacro ("vtkDMMLROIListNode::GetNthROI: index out of bounds, " << n << " is less than zero or more than the number of items: " << this->ROIList->GetNumberOfItems() << endl);
    return nullptr;
    }
  else
    {
    return (vtkDMMLROINode*)this->ROIList->GetItemAsObject(n);
    }
}

//----------------------------------------------------------------------------
void vtkDMMLROIListNode::SetAllVolumeNodeID()
{
...
//----------------------------------------------------------------------------
void vtkDMMLROIListNode::UpdateIJK()
{
  int numROIs = this->GetNumberOfROIs();
  vtkDMMLVolumeNode *VolumeNode  = vtkDMMLVolumeNode::SafeDownCast(this->Scene->GetNodeByID(this->VolumeNodeID));
