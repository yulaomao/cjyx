/*=auto=========================================================================
  ...
  Program:   3D Cjyx
  Module:    $RCSfile: vtkDMMLSliceNode.h,v $
  Date:      $Date: 2006/03/19 17:12:29 $
  Version:   $Revision: 1.3 $

=========================================================================auto=*/

#ifndef __vtkDMMLSliceNode_h
#define __vtkDMMLSliceNode_h
...
/// \brief DMML node for storing a slice through RAS space.
///
...
class VTK_DMML_EXPORT vtkDMMLSliceNode : public vtkDMMLAbstractViewNode
{
  public:
  /// \brief Instantiate a new Slice node without any orientation presets.
  ///
  /// \note To instantiate a vtkDMMLSliceNode with preconfigured
  /// orientation preset matrices (the default presets are: Axial,
  /// Sagittal and Coronal in default), it is necessary to use
  /// vtkDMMLScene::CreateNodeByClass(const char*)
  static vtkDMMLSliceNode *New();
  vtkTypeMacro(vtkDMMLSliceNode,vtkDMMLAbstractViewNode);
  ...
  vtkDMMLCopyContentMacro(vtkDMMLSliceNode);
...
public:
  ...
  /// \param patientRightIsScreenLeft chooses between radiology (default, patient right is left side on the screen)
  /// and neurology (patient right is right side on the screen) view orientation conventions.
  /// \sa vtkDMMLScene::AddDefaultNode(vtkDMMLNode* node)
  /// \sa GetAxialSliceToRASMatrix, GetSagittalSliceToRASMatrix, GetCoronalSliceToRASMatrix
  static void AddDefaultSliceOrientationPresets(vtkDMMLScene* scene, bool patientRightIsScreenLeft=true);
  ...
  /// If jumpMode is set to vtkDMMLSliceNode::DefaultJumpSlice then jump mode set in the slice node will be used.
  /// specified in the slice node will be used.
  static void JumpAllSlices(vtkDMMLScene* scene, double r, double a, double s,
  ...
  void RotateToVolumePlane(vtkDMMLVolumeNode *volumeNode, bool forceSlicePlaneToSingleSlice=true);
...
protected:
  vtkDMMLSliceNode();
  ~vtkDMMLSliceNode() override;
  vtkDMMLSliceNode(const vtkDMMLSliceNode&);
  void operator=(const vtkDMMLSliceNode&);
...
//----------------------------------------------------------------------------
std::vector< std::string > vtkDMMLSliceNode::GetThreeDViewIDs()const
{
