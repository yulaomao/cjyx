...
//------------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLTransformDisplayNode);

//----------------------------------------------------------------------------
vtkDMMLTransformDisplayNode::vtkDMMLTransformDisplayNode()
  :vtkDMMLDisplayNode()
{
...
//----------------------------------------------------------------------------
vtkDMMLTransformDisplayNode::~vtkDMMLTransformDisplayNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::ReadXMLAttributes(const char** atts)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::CopyContent(vtkDMMLNode* anode, bool deepCopy/*=true*/)
{
  DMMLNodeModifyBlocker blocker(this);
  Superclass::CopyContent(anode, deepCopy);
  vtkDMMLTransformDisplayNode *node = vtkDMMLTransformDisplayNode::SafeDownCast(anode);
  if (!node)
    {
    return;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//---------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::ProcessDMMLEvents ( vtkObject *caller, unsigned long event, void *callData )
{
  if (caller!=nullptr
    && (event==vtkCommand::ModifiedEvent || event==vtkDMMLTransformableNode::TransformModifiedEvent)
  ...
  else this->Superclass::ProcessDMMLEvents(caller, event, callData);
}

//----------------------------------------------------------------------------
vtkDMMLNode* vtkDMMLTransformDisplayNode::GetRegionNode()
{
  return this->GetNodeReference(RegionReferenceRole);
}

//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::SetAndObserveRegionNode(vtkDMMLNode* node)
{
  this->SetAndObserveNthNodeReferenceID(RegionReferenceRole, 0, node ? node->GetID() : nullptr);
}

//----------------------------------------------------------------------------
vtkDMMLNode* vtkDMMLTransformDisplayNode::GetGlyphPointsNode()
{
  return this->GetNodeReference(GlyphPointsReferenceRole);
}

//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::SetAndObserveGlyphPointsNode(vtkDMMLNode* node)
{
  this->SetAndObserveNthNodeReferenceID(GlyphPointsReferenceRole, 0, node ? node->GetID() : nullptr);
}

//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::SetContourLevelsMm(double* values, int size)
{
  this->ContourLevelsMm.clear();
...
//----------------------------------------------------------------------------
double* vtkDMMLTransformDisplayNode::GetContourLevelsMm()
{
...
//----------------------------------------------------------------------------
unsigned int vtkDMMLTransformDisplayNode::GetNumberOfContourLevels()
{
  return this->ContourLevelsMm.size();
}

//----------------------------------------------------------------------------
const char* vtkDMMLTransformDisplayNode::ConvertVisualizationModeToString(int modeIndex)
{
  switch (modeIndex)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLTransformDisplayNode::ConvertVisualizationModeFromString(const char* modeString)
{
  ...
  for (int modeIndex=0; modeIndex<VIS_MODE_LAST; modeIndex++)
    {
    if (strcmp(modeString, vtkDMMLTransformDisplayNode::ConvertVisualizationModeToString(modeIndex))==0)
      {
      return modeIndex;
      }
    }
...
//----------------------------------------------------------------------------
const char* vtkDMMLTransformDisplayNode::ConvertGlyphTypeToString(int modeIndex)
{
  switch (modeIndex)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLTransformDisplayNode::ConvertGlyphTypeFromString(const char* modeString)
{
  if (modeString==nullptr)
    {
    return -1;
  ...
  for (int modeIndex=0; modeIndex<GLYPH_TYPE_LAST; modeIndex++)
    {
    if (strcmp(modeString,vtkDMMLTransformDisplayNode::ConvertGlyphTypeToString(modeIndex))==0)
      {
      return modeIndex;
      }
    }
...
//----------------------------------------------------------------------------
std::string vtkDMMLTransformDisplayNode::GetContourLevelsMmAsString()
{
  return ConvertContourLevelsToString(this->ContourLevelsMm);
}

//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::SetContourLevelsMmFromString(const char* str)
{
...
//----------------------------------------------------------------------------
std::vector<double> vtkDMMLTransformDisplayNode::ConvertContourLevelsFromString(const char* str)
{
  return vtkDMMLTransformDisplayNode::StringToDoubleVector(str);
}

//----------------------------------------------------------------------------
std::vector<double> vtkDMMLTransformDisplayNode::StringToDoubleVector(const char* str)
{
...
//----------------------------------------------------------------------------
std::string vtkDMMLTransformDisplayNode::ConvertContourLevelsToString(const std::vector<double>& levels)
{
  return vtkDMMLTransformDisplayNode::DoubleVectorToString(&(levels[0]), levels.size());
}

//----------------------------------------------------------------------------
std::string vtkDMMLTransformDisplayNode::DoubleVectorToString(const double* values, int numberOfValues)
{
  std::stringstream ss;
...
//----------------------------------------------------------------------------
bool vtkDMMLTransformDisplayNode::IsContourLevelEqual(const std::vector<double>& levels1, const std::vector<double>& levels2)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::GetContourLevelsMm(std::vector<double> &levels)
{
  levels=this->ContourLevelsMm;
}

//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::SetDefaultColors()
{
  if (!this->GetScene())
    {
    vtkErrorMacro("vtkDMMLTransformDisplayNode::SetDefaultColors failed: scene is not set");
    return;
    }

  // Create and set a new color table node
  vtkNew<vtkDMMLProceduralColorNode> colorNode;
...
//----------------------------------------------------------------------------
vtkColorTransferFunction* vtkDMMLTransformDisplayNode::GetColorMap()
{
  vtkDMMLProceduralColorNode* colorNode=vtkDMMLProceduralColorNode::SafeDownCast(GetColorNode());
  if (colorNode==nullptr
    ...
    colorNode=vtkDMMLProceduralColorNode::SafeDownCast(this->GetColorNode());
    if (colorNode==nullptr)
      {
      vtkErrorMacro("vtkDMMLTransformDisplayNode::GetColorMap failed: could not create default color node");
      return nullptr;
      }
    }
  vtkColorTransferFunction* colorMap=colorNode->GetColorTransferFunction();
...
//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::SetColorMap(vtkColorTransferFunction* newColorMap)
{
  int oldModified=this->StartModify();
  vtkDMMLProceduralColorNode* colorNode=vtkDMMLProceduralColorNode::SafeDownCast(this->GetColorNode());
  if (colorNode==nullptr)
    {
    // We don't have a color node or it is not the right type
    this->SetDefaultColors();
  ...
  if (colorNode!=nullptr)
    ...
    if (!vtkDMMLProceduralColorNode::IsColorMapEqual(colorNode->GetColorTransferFunction(),newColorMap))
      {
    ...
    {
    vtkErrorMacro("vtkDMMLTransformDisplayNode::SetColorMap failed: could not create default color node");
    }
  this->EndModify(oldModified);
}

//----------------------------------------------------------------------------
void vtkDMMLTransformDisplayNode::UpdateEditorBounds()
{
  this->InvokeEvent(vtkDMMLTransformDisplayNode::TransformUpdateEditorBoundsEvent);
}
