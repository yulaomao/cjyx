...
//----------------------------------------------------------------------------
const double vtkDMMLSegmentationDisplayNode::SEGMENT_COLOR_NO_OVERRIDE = -1.0;

static const char* SegmentColorGeneratorSourceColorNodeReferenceRole = "segmentColorGenerator";

//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLSegmentationDisplayNode);

//----------------------------------------------------------------------------
vtkDMMLSegmentationDisplayNode::vtkDMMLSegmentationDisplayNode()
{
...
//----------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::WriteXML(ostream& of, int nIndent)
{
  ...
  for (SegmentDisplayPropertiesMap::iterator propIt = this->SegmentationDisplayProperties.begin();
    ...
    {
    of << vtkDMMLNode::URLEncodeString(propIt->first.c_str())
...
//----------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::ReadXMLAttributes(const char** atts)
{
  ...
  while (*atts != nullptr)
    ...
    if (!strcmp(attName, "PreferredDisplayRepresentationName2D"))
      ...
      while (std::getline(segmentsDisplayProperties, segmentDisplayPropertiesString, '|'))
        ...
        this->SetSegmentDisplayProperties(vtkDMMLNode::URLDecodeString(id.c_str()), props);
        }
      }
    }
...
//----------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::CopyContent(vtkDMMLNode* anode, bool deepCopy/*=true*/)
{
  DMMLNodeModifyBlocker blocker(this);
  Superclass::CopyContent(anode, deepCopy);

  vtkDMMLSegmentationDisplayNode* node = vtkDMMLSegmentationDisplayNode::SafeDownCast(anode);
  if (!node)
    {
    return;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//---------------------------------------------------------------------------
bool vtkDMMLSegmentationDisplayNode::GetSegmentDisplayProperties(std::string segmentId, SegmentDisplayProperties &properties)
{
  this->UpdateSegmentList();
  ...
  if (propsIt == this->SegmentationDisplayProperties.end())
    {
    vtkWarningMacro("vtkDMMLSegmentationDisplayNode::GetSegmentDisplayProperties: no display properties are found for segment ID="<<segmentId<<", return default");
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentDisplayProperties(std::string segmentId, SegmentDisplayProperties &properties)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentId);
  bool modified = false;
...
//---------------------------------------------------------------------------
vtkVector3d vtkDMMLSegmentationDisplayNode::GetSegmentColor(std::string segmentID)
{
...
//---------------------------------------------------------------------------
bool vtkDMMLSegmentationDisplayNode::GetSegmentColor(std::string segmentID, double* color)
{
  if (!color)
    {
  ...
  // Get segment
  vtkDMMLSegmentationNode* segmentationNode = vtkDMMLSegmentationNode::SafeDownCast(this->GetDisplayableNode());
...
//---------------------------------------------------------------------------
bool vtkDMMLSegmentationDisplayNode::GetSegmentColor(std::string segmentID, double &r, double &g, double &b)
{
  double color[3] = {vtkSegment::SEGMENT_COLOR_INVALID[0], vtkSegment::SEGMENT_COLOR_INVALID[1], vtkSegment::SEGMENT_COLOR_INVALID[2]};
...
//---------------------------------------------------------------------------
vtkVector3d vtkDMMLSegmentationDisplayNode::GetSegmentOverrideColor(std::string segmentID)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentID);
  if (propsIt == this->SegmentationDisplayProperties.end())
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentOverrideColor(std::string segmentID, double r, double g, double b)
{
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentOverrideColor(std::string segmentID, vtkVector3d overrideColor)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::UnsetSegmentOverrideColor(std::string segmentID)
{
  this->SetSegmentOverrideColor(segmentID, SEGMENT_COLOR_NO_OVERRIDE, SEGMENT_COLOR_NO_OVERRIDE, SEGMENT_COLOR_NO_OVERRIDE);
}

//---------------------------------------------------------------------------
bool vtkDMMLSegmentationDisplayNode::GetSegmentVisibility(std::string segmentID)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentID);
  if (propsIt == this->SegmentationDisplayProperties.end())
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentVisibility(std::string segmentID, bool visible)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetAllSegmentsVisibility(bool visible)
{
...
//---------------------------------------------------------------------------
bool vtkDMMLSegmentationDisplayNode::GetSegmentVisibility3D(std::string segmentID)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentID);
  if (propsIt == this->SegmentationDisplayProperties.end())
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentVisibility3D(std::string segmentID, bool visible)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetAllSegmentsVisibility3D(bool visible, bool changeVisibleSegmentsOnly /* = false */)
{
  std::vector<std::string> segmentIDs;
...
//---------------------------------------------------------------------------
bool vtkDMMLSegmentationDisplayNode::GetSegmentVisibility2DFill(std::string segmentID)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentID);
  if (propsIt == this->SegmentationDisplayProperties.end())
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentVisibility2DFill(std::string segmentID, bool visible)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetAllSegmentsVisibility2DFill(bool visible, bool changeVisibleSegmentsOnly /* = false */)
{
  std::vector<std::string> segmentIDs;
  this->GetSegmentIDs(segmentIDs, changeVisibleSegmentsOnly);
  for (std::vector<std::string>::iterator segmentIDIt = segmentIDs.begin(); segmentIDIt != segmentIDs.end(); ++segmentIDIt)
...
//---------------------------------------------------------------------------
bool vtkDMMLSegmentationDisplayNode::GetSegmentVisibility2DOutline(std::string segmentID)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentID);
  if (propsIt == this->SegmentationDisplayProperties.end())
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentVisibility2DOutline(std::string segmentID, bool visible)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetAllSegmentsVisibility2DOutline(bool visible, bool changeVisibleSegmentsOnly /* = false */)
{
  std::vector<std::string> segmentIDs;
  this->GetSegmentIDs(segmentIDs, changeVisibleSegmentsOnly);
  for (std::vector<std::string>::iterator segmentIDIt = segmentIDs.begin(); segmentIDIt != segmentIDs.end(); ++segmentIDIt)
...
//---------------------------------------------------------------------------
double vtkDMMLSegmentationDisplayNode::GetSegmentOpacity3D(std::string segmentID)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentID);
  if (propsIt == this->SegmentationDisplayProperties.end())
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentOpacity3D(std::string segmentID, double opacity)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetAllSegmentsOpacity3D(double opacity, bool changeVisibleSegmentsOnly /* = false */)
{
  std::vector<std::string> segmentIDs;
  this->GetSegmentIDs(segmentIDs, changeVisibleSegmentsOnly);
  for (std::vector<std::string>::iterator segmentIDIt = segmentIDs.begin(); segmentIDIt != segmentIDs.end(); ++segmentIDIt)
...
//---------------------------------------------------------------------------
double vtkDMMLSegmentationDisplayNode::GetSegmentOpacity2DFill(std::string segmentID)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentID);
  if (propsIt == this->SegmentationDisplayProperties.end())
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentOpacity2DFill(std::string segmentID, double opacity)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetAllSegmentsOpacity2DFill(double opacity, bool changeVisibleSegmentsOnly /* = false */)
{
  std::vector<std::string> segmentIDs;
  this->GetSegmentIDs(segmentIDs, changeVisibleSegmentsOnly);
  for (std::vector<std::string>::iterator segmentIDIt = segmentIDs.begin(); segmentIDIt != segmentIDs.end(); ++segmentIDIt)
...
//---------------------------------------------------------------------------
double vtkDMMLSegmentationDisplayNode::GetSegmentOpacity2DOutline(std::string segmentID)
{
  this->UpdateSegmentList();
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentID);
  if (propsIt == this->SegmentationDisplayProperties.end())
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentOpacity2DOutline(std::string segmentID, double opacity)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetAllSegmentsOpacity2DOutline(double opacity, bool changeVisibleSegmentsOnly /* = false */)
{
  std::vector<std::string> segmentIDs;
  this->GetSegmentIDs(segmentIDs, changeVisibleSegmentsOnly);
  for (std::vector<std::string>::iterator segmentIDIt = segmentIDs.begin(); segmentIDIt != segmentIDs.end(); ++segmentIDIt)
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentOpacity(std::string segmentID, double opacity)
{
  SegmentDisplayProperties properties;
  this->GetSegmentDisplayProperties(segmentID, properties);
  properties.Opacity3D = opacity;
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetAllSegmentsOpacity(double opacity, bool changeVisibleSegmentsOnly /* = false */)
{
  std::vector<std::string> segmentIDs;
  this->GetSegmentIDs(segmentIDs, changeVisibleSegmentsOnly);
  for (std::vector<std::string>::iterator segmentIDIt = segmentIDs.begin(); segmentIDIt != segmentIDs.end(); ++segmentIDIt)
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentDisplayPropertiesToDefault(const std::string& segmentId)
{
  vtkDMMLSegmentationNode* segmentationNode = vtkDMMLSegmentationNode::SafeDownCast(this->GetDisplayableNode());
  if (!segmentationNode)
    {
    vtkErrorMacro("vtkDMMLSegmentationDisplayNode: No segmentation node associated to this display node");
    return;
    }
  vtkSegmentation* segmentation = segmentationNode->GetSegmentation();
  vtkSegment* segment = segmentation->GetSegment(segmentId);
  if (!segment)
    {
    vtkErrorMacro("vtkDMMLSegmentationDisplayNode: segment not found by id " << segmentId);
    return;
    }

  DMMLNodeModifyBlocker blocker(this);
  DMMLNodeModifyBlocker blocker2(segmentationNode);
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::RemoveSegmentDisplayProperties(std::string segmentId)
{
  SegmentDisplayPropertiesMap::iterator propsIt = this->SegmentationDisplayProperties.find(segmentId);
  if (propsIt != this->SegmentationDisplayProperties.end())
    {
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::ClearSegmentDisplayProperties()
{
  this->SegmentationDisplayProperties.clear();
...
//---------------------------------------------------------------------------
bool vtkDMMLSegmentationDisplayNode::CalculateAutoOpacitiesForSegments()
{
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::SetSegmentColorGeneratorSourceColorNodeID(const char* colorNodeID)
{
  this->SetNodeReferenceID(SegmentColorGeneratorSourceColorNodeReferenceRole, colorNodeID);
}

//---------------------------------------------------------------------------
const char* vtkDMMLSegmentationDisplayNode::GetSegmentColorGeneratorSourceColorNodeID()
{
  return this->GetNodeReferenceID(SegmentColorGeneratorSourceColorNodeReferenceRole);
}

//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::GenerateSegmentColor(double color[3], int colorNumber/*=0*/)
{
  if (!this->Scene)
    {
    vtkErrorMacro("GenerateSegmentColor: Invalid DMML scene!");
    return;
    }
  ...
  if (!colorTableNode)
    ...
    colorTableNode = vtkDMMLColorTableNode::SafeDownCast(
      this->Scene->GetNodeByID("vtkDMMLColorTableNodeFileGenericAnatomyColors.txt"));
    }
  ...
  // vtkDMMLSegmentationNode::AddSegmentDisplayProperties every time a new segment display
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::GenerateSegmentColor(double &r, double &g, double &b)
{
  double color[3] = {0.0, 0.0, 0.0};
...
//---------------------------------------------------------------------------
std::string vtkDMMLSegmentationDisplayNode::GetDisplayRepresentationName3D()
{
  // Get segmentation node
  vtkDMMLSegmentationNode* segmentationNode = vtkDMMLSegmentationNode::SafeDownCast(this->GetDisplayableNode());
  if (!segmentationNode)
...
//---------------------------------------------------------------------------
std::string vtkDMMLSegmentationDisplayNode::GetDisplayRepresentationName2D()
{
  // Get segmentation node
  vtkDMMLSegmentationNode* segmentationNode = vtkDMMLSegmentationNode::SafeDownCast(this->GetDisplayableNode());
  if (!segmentationNode)
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::GetPolyDataRepresentationNames(std::set<std::string> &representationNames)
{
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::GetVisibleSegmentIDs(std::vector<std::string>& segmentIDs)
{
  this->GetSegmentIDs(segmentIDs, true);
}

//---------------------------------------------------------------------------
std::vector<std::string> vtkDMMLSegmentationDisplayNode::GetVisibleSegmentIDs()
{
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::GetSegmentIDs(std::vector<std::string>& segmentIDs, bool visibleSegmentsOnly)
{
  segmentIDs.clear();
  vtkDMMLSegmentationNode* segmentationNode = vtkDMMLSegmentationNode::SafeDownCast(this->GetDisplayableNode());
  if (!segmentationNode)
    {
    vtkErrorMacro("vtkDMMLSegmentationDisplayNode::GetSegmentIDs: No segmentation node is associated to this display node");
    return;
    }
  // Make sure the requested representation exists
  vtkSegmentation* segmentation = segmentationNode->GetSegmentation();
  if (!segmentation)
    {
    vtkErrorMacro("vtkDMMLSegmentationDisplayNode::GetSegmentIDs: No segmentation is associated to this display node");
    return;
    }
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::GetVisibleSegmentIDs(vtkStringArray* segmentIDs)
{
  if (segmentIDs == nullptr)
    {
    vtkErrorMacro("vtkDMMLSegmentationDisplayNode::GetVisibleSegmentIDs failed: invalid segmentIDs");
    return;
    }
...
//---------------------------------------------------------------------------
void vtkDMMLSegmentationDisplayNode::UpdateSegmentList(bool removeUnusedDisplayProperties /*=true*/)
{
  vtkDMMLSegmentationNode* segmentationNode = vtkDMMLSegmentationNode::SafeDownCast(this->GetDisplayableNode());
