...
class VTK_DMML_EXPORT vtkDataIOManager : public vtkObject
{
  public:
  ...
  vtkDataTransfer *AddNewDataTransfer ( vtkDMMLNode *node);
  void AddNewDataTransfer ( vtkDataTransfer *transfer, vtkDMMLNode *node);
  ...
  void QueueRead ( vtkDMMLNode *node );

  ///
  /// Invokes a RemoteWriteEvent on the node
  /// so that logic can take care of scheduling and applying it
  void QueueWrite ( vtkDMMLNode *node );
