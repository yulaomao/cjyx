...
//---------------------------------------------------------------------------
int vtkDMMLLinearTransformNodeEventsTest(int , char * [] )
{
  vtkNew< vtkDMMLLinearTransformNode > linearTransformNode;
  ...
  // Test vtkDMMLLinearTransformNode::SetAndObserveMatrixTransformToParent()
  ...
  if (!callback->GetErrorString().empty() ||
      ...
      callback->GetNumberOfEvents(vtkDMMLTransformNode::TransformModifiedEvent) != 1)
    {
    std::cerr << "vtkDMMLLinearTransformNode::SetAndObserveMatrixTransformToParent failed (1)."
              << callback->GetErrorString().c_str() << " "
              << "Number of ModifiedEvent: " << callback->GetNumberOfModified() << " "
              << "Number of TransformModifiedEvent: "
              << callback->GetNumberOfEvents(vtkDMMLTransformNode::TransformModifiedEvent)
              << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (!callback->GetErrorString().empty() ||
      ...
      callback->GetNumberOfEvents(vtkDMMLTransformNode::TransformModifiedEvent) != 1)
    {
    std::cerr << "vtkDMMLLinearTransformNode::SetAndObserveMatrixTransformToParent failed (2)."
              << callback->GetErrorString().c_str() << " "
              << "Number of ModifiedEvent: " << callback->GetNumberOfModified() << " "
              << "Number of TransformModifiedEvent: "
              << callback->GetNumberOfEvents(vtkDMMLTransformNode::TransformModifiedEvent)
  ...
  if (!callback->GetErrorString().empty() ||
      callback->GetNumberOfModified() != 0 ||
      callback->GetNumberOfEvents(vtkDMMLTransformNode::TransformModifiedEvent) != 0)
    {
