...
int vtkDMMLNonlinearTransformNodeTest1(int argc, char * argv[] )
{
...
int testTransformConsistencyAndCopy(const char *filename, const char* nodeId, const char* expectedTransformType)
{
  ...
  vtkDMMLTransformNode *transformNode = vtkDMMLTransformNode::SafeDownCast(scene->GetNodeByID(nodeId));
  ...
  vtkNew<vtkDMMLTransformNode> transformNodeCopy;
...
int TestBSplineTransform(const char *filename)
{
  CHECK_EXIT_SUCCESS(testTransformConsistencyAndCopy(filename, "vtkDMMLBSplineTransformNode1", "vtkOrientedBSplineTransform"));
  return EXIT_SUCCESS;
}
...
int TestGridTransform(const char *filename)
{
  CHECK_EXIT_SUCCESS(testTransformConsistencyAndCopy(filename, "vtkDMMLGridTransformNode1", "vtkOrientedGridTransform"));
  return EXIT_SUCCESS;
}
...
int TestThinPlateSplineTransform(const char *filename)
{
  CHECK_EXIT_SUCCESS(testTransformConsistencyAndCopy(filename, "vtkDMMLTransformNode1", "vtkThinPlateSplineTransform"));
  return EXIT_SUCCESS;
}
...
int TestCompositeTransformHardenSplit(const char *filename)
{
  ...
  vtkNew<vtkDMMLTransformNode> infoPrinter;

  // Create a composite transform from bspline and a grid transform
  // WORLD
  //  |-- gridTransformNode
  //       |-- bsplineTransformNode
  vtkDMMLBSplineTransformNode *bsplineTransformNode = vtkDMMLBSplineTransformNode::SafeDownCast(
    scene->GetNodeByID("vtkDMMLBSplineTransformNode1"));
  vtkDMMLGridTransformNode *gridTransformNode = vtkDMMLGridTransformNode::SafeDownCast(
    scene->GetNodeByID("vtkDMMLGridTransformNode1"));
...
int TestBSplineLinearCompositeTransformSplit(const char *filename)
{
  ...
  vtkDMMLTransformNode *compositeBsplineTransformNode = vtkDMMLTransformNode::SafeDownCast(
    scene->GetNodeByID("vtkDMMLTransformNode2"));
...
int TestRelativeTransforms(const char *filename)
{
  ...
  vtkDMMLTransformNode *gridTransformNode = vtkDMMLTransformNode::SafeDownCast(
    scene->GetNodeByID("vtkDMMLGridTransformNode1"));
  vtkDMMLTransformNode *bsplineTransformNode = vtkDMMLTransformNode::SafeDownCast(
    scene->GetNodeByID("vtkDMMLBSplineTransformNode1"));
  vtkDMMLTransformNode *compositeTransformNode = vtkDMMLTransformNode::SafeDownCast(
    scene->GetNodeByID("vtkDMMLTransformNode2"));
  vtkDMMLTransformNode *tpsTransformNode = vtkDMMLTransformNode::SafeDownCast(
    scene->GetNodeByID("vtkDMMLTransformNode1"));
  ...
  vtkDMMLTransformNode::FlattenGeneralTransform(compositeToTpsTransformList.GetPointer(), compositeToTps.GetPointer());
...
int TestGetTransform()
{
  ...
  vtkNew<vtkDMMLTransformNode> transformWithToParentSet;
  ...
  vtkNew<vtkDMMLTransformNode> transformWithFromParentSet;
