...
//----------------------------------------------------------------------------
class vtkDMMLVolumeNodeTestHelper1 : public vtkDMMLVolumeNode
{
public:
  // Provide a concrete New.
  static vtkDMMLVolumeNodeTestHelper1 *New();

  vtkTypeMacro(vtkDMMLVolumeNodeTestHelper1,vtkDMMLVolumeNode);

  vtkDMMLNode* CreateNodeInstance() override
    {
    return vtkDMMLVolumeNodeTestHelper1::New();
    }
  const char* GetNodeTagName() override
    {
    return "vtkDMMLVolumeNodeTestHelper1";
    }

  vtkDMMLStorageNode* CreateDefaultStorageNode() override { return vtkDMMLVolumeArchetypeStorageNode::New(); }
};
vtkStandardNewMacro(vtkDMMLVolumeNodeTestHelper1);

//----------------------------------------------------------------------------
int vtkDMMLVolumeNodeTest1(int , char * [])
{
  vtkNew<vtkDMMLVolumeNodeTestHelper1> node1;
  EXERCISE_ALL_BASIC_DMML_METHODS(node1.GetPointer());

  vtkNew<vtkDMMLVolumeNodeTestHelper1> node2;
  ...
  for(std::vector<std::string>::iterator it = scanOrders.begin();
    ...
    {
    ijkToRAS->Identity();
    vtkDMMLVolumeNode::ComputeIJKToRASFromScanOrder(
          (*it).c_str(), spacing, dimensions, /* centerImage= */ false,
          ijkToRAS.GetPointer());

    const char* computedScanOrder =
        vtkDMMLVolumeNode::ComputeScanOrderFromIJKToRAS(ijkToRAS.GetPointer());
