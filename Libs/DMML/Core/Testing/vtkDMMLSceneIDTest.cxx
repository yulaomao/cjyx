  ...
  int vtkNotUsed(argc), char * vtkNotUsed(argv) [] )
{
  ...
  // UniqueIDs
  //---------------------------------------------------------------------------
  if (node->GetID() != std::string("vtkDMMLModelNode1"))
    {
  ...
  vtkNew<vtkDMMLModelNode> node2;
  scene->AddNode(node2.GetPointer());

  if (node2->GetID() != std::string("vtkDMMLModelNode2"))
    {
  ...
  // Test reserved ID
  scene->AddReservedID("vtkDMMLModelNode3");
  vtkNew<vtkDMMLModelNode> node3;
  scene->AddNode(node3.GetPointer());

  if (node3->GetID() != std::string("vtkDMMLModelNode4"))
    {
    std::cerr << __LINE__ << " Node ID failed: "
  ...
  scene->AddReservedID("vtkDMMLModelNode5");
  scene->RemoveReservedIDs();
  vtkNew<vtkDMMLModelNode> node4;
  scene->AddNode(node4.GetPointer());

  if (node4->GetID() != std::string("vtkDMMLModelNode5"))
    {
    std::cerr << __LINE__ << " Node ID failed: "
