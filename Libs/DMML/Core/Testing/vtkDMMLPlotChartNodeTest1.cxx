...
int vtkDMMLPlotChartNodeTest1(int , char * [] )
{
  // Create a PlotChart node
  vtkNew<vtkDMMLScene> scene;
  vtkNew<vtkDMMLPlotChartNode> node;
  EXERCISE_ALL_BASIC_DMML_METHODS(node.GetPointer());
  ...
  vtkSmartPointer< vtkDMMLPlotChartNode > nodeCopy = vtkSmartPointer< vtkDMMLPlotChartNode >::New();
  nodeCopy->Copy(node.GetPointer());

  CHECK_STD_STRING(node->GetName(), nodeCopy->GetName());

  std::cout << "vtkDMMLPlotChartNodeTest1 completed successfully" << std::endl;
  return EXIT_SUCCESS;
}
