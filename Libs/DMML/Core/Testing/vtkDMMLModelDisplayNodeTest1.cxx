...
//---------------------------------------------------------------------------
int vtkDMMLModelDisplayNodeTest1(int , char * [] )
{
  vtkSmartPointer< vtkDMMLModelDisplayNode > node1 = vtkSmartPointer< vtkDMMLModelDisplayNode >::New();
...
int TestScalarRange(vtkPointSet* mesh)
{
  ...
  vtkNew<vtkDMMLModelDisplayNode> mdNode;
  ...
  // test manual change
  mdNode->SetScalarRangeFlag(vtkDMMLModelDisplayNode::UseManualScalarRange);
  ...
  mdNode->SetScalarRangeFlag(vtkDMMLModelDisplayNode::UseDataScalarRange);
  mdNode->GetScalarRange(mdRange);
  CHECK_DOUBLE(mdRange[0], dataRange[0]);
  CHECK_DOUBLE(mdRange[1], dataRange[1]);

  // test data type range
  mdNode->SetScalarRangeFlag(vtkDMMLModelDisplayNode::UseDataTypeScalarRange);
  mdNode->GetScalarRange(mdRange);
                ...
                bool meshTypeIsPolyData)
{
  ...
  vtkNew<vtkDMMLModelDisplayNode> display;
  if (!observeDisplayBeforeAddToScene)
    {
    scene->AddNode(display.GetPointer());
    }

  model->SetAndObserveDisplayNodeID("vtkDMMLModelDisplayNode1");
  if (!observeMeshBeforeObserveDisplay)
    {
    model->SetAndObserveMesh(mesh);
    }
  ...
  if ((meshTypeIsPolyData && displayInputMesh != display->GetInputPolyData()) ||
    ...
    {
    std::cerr << __LINE__ << ": vtkDMMLModelNode::SetAndobserveMesh failed when "
