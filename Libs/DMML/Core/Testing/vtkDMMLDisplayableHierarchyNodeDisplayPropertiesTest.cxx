...
void PopulateScene(vtkDMMLScene* scene, int numberOfLevels);
vtkDMMLModelNode* GetModelNode(vtkDMMLScene* scene, int level);
vtkDMMLDisplayableHierarchyNode* GetHierarchyNode(vtkDMMLScene* scene, int level);
vtkDMMLDisplayableHierarchyNode* GetModelHierarchyNode(vtkDMMLScene* scene, int level);
...
bool TestColors(vtkDMMLScene* scene,
                double (*modelColors)[3],
                double (*hierarchyColors)[3]);
bool TestVisibility(vtkDMMLScene* scene,
...
//---------------------------------------------------------------------------
int vtkDMMLDisplayableHierarchyNodeDisplayPropertiesTest(int vtkNotUsed(argc),
...
void PopulateScene(vtkDMMLScene* scene, int numberOfLevels)
{
  ...
  vtkDMMLDisplayableHierarchyNode* parentHierarchy = nullptr;
  for (int level = 0; level < numberOfLevels; ++level)
    ...
    // Hierarchy
    vtkNew<vtkDMMLDisplayableHierarchyNode> hierarchyNode;
    std::stringstream shn;
    shn << level << " hierarchy";
    hierarchyNode->SetName(shn.str().c_str());
    scene->AddNode(hierarchyNode.GetPointer());

    vtkNew<vtkDMMLModelDisplayNode> hierarchyDisplayNode;
...
vtkDMMLModelNode* GetModelNode(vtkDMMLScene* scene, int level)
{
  return vtkDMMLModelNode::SafeDownCast(
    scene->GetNthNodeByClass(level, "vtkDMMLModelNode"));
}
...
vtkDMMLDisplayableHierarchyNode* GetHierarchyNode(vtkDMMLScene* scene, int level)
{
  return vtkDMMLDisplayableHierarchyNode::SafeDownCast(
    scene->GetNthNodeByClass(2*level + 1, "vtkDMMLDisplayableHierarchyNode"));
}
...
{
  return vtkDMMLDisplayableHierarchyNode::SafeDownCast(
    scene->GetNthNodeByClass(2*level, "vtkDMMLDisplayableHierarchyNode"));
}
...
bool TestSetVisibilityLevel0()
{
  ...
  vtkDMMLDisplayableHierarchyNode* hierarchyNode= GetHierarchyNode(scene.GetPointer(), 0);
...
bool TestSetVisibilityLevel1()
{
  ...
  // Color and visibility shouldn't be changed when collapsing hierarchy
  vtkDMMLDisplayableHierarchyNode* hierarchyNode= GetHierarchyNode(scene.GetPointer(), 1);
  hierarchyNode->GetDisplayNode()->SetVisibility(0);
  bool res = true;
  res = TestColors(scene.GetPointer(), DEFAULT_MODEL_COLORS, DEFAULT_HIERARCHY_COLORS) && res;
...
bool TestCollapseLevels0And1()
{
  ...
  vtkDMMLDisplayableHierarchyNode* hierarchyNode= GetHierarchyNode(scene.GetPointer(), 0);
  hierarchyNode->SetExpanded(0);
  vtkDMMLDisplayableHierarchyNode* subHierarchyNode= GetHierarchyNode(scene.GetPointer(), 1);
                ...
                double (*hierarchyColors)[3])
{
  for (int level = 0; level < LEVEL_COUNT; ++level)
    {
    // Model
    vtkDMMLModelNode* modelNode = GetModelNode(scene, level);
    ...
    vtkDMMLDisplayableHierarchyNode* hierarchyNode = GetHierarchyNode(scene, level);
    assert(hierarchyNode != nullptr);
    vtkDMMLDisplayNode* hierarchyDisplayNode = hierarchyNode->GetDisplayNode();
