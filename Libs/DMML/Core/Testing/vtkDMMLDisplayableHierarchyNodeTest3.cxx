...
static void PrintNames(std::vector< vtkDMMLHierarchyNode *> kids)
{
...
int vtkDMMLDisplayableHierarchyNodeTest3(int , char * [] )
{
  ...
  vtkNew<vtkDMMLModelDisplayNode> hdnode2;
  ...
  std::vector<vtkSmartPointer<vtkDMMLModelNode> > modelNodes;
  std::vector<vtkSmartPointer<vtkDMMLModelDisplayNode> > modelDisplayNodes;
  std::vector<vtkSmartPointer<vtkDMMLModelHierarchyNode> > modelHierarchyNodes;
  unsigned int numModels = 5;
  for (unsigned int m = 0; m < numModels; m++)
    {
    // first set up a hierarchy for this model
    modelHierarchyNodes.push_back(vtkSmartPointer<vtkDMMLModelHierarchyNode>::New());
    std::stringstream ss;
    ss << m;
    ...
    modelNodes.push_back(vtkSmartPointer<vtkDMMLModelNode>::New());
    modelDisplayNodes.push_back(vtkSmartPointer<vtkDMMLModelDisplayNode>::New());
  ...
  std::vector< vtkDMMLHierarchyNode *> immediateChildren = hnode1->GetChildrenNodes();
  ...
  std::vector< vtkDMMLHierarchyNode *> allChildren2;
  ...
  std::vector< vtkDMMLHierarchyNode *> immediateChildren2 = hnode2->GetChildrenNodes();
  ...
  for (unsigned int i = 0; i < numModels; i++)
    {
    vtkSmartPointer<vtkDMMLHierarchyNode> childHierarchy;
  ...
  vtkNew<vtkDMMLModelNode> m6;
  vtkNew<vtkDMMLModelDisplayNode> md6;
  vtkNew<vtkDMMLModelHierarchyNode> mh6;
  ...
  PrintNames(immediateChildren2);
  vtkDMMLHierarchyNode* testInsert = hnode2->GetNthChildNode(3);
  if (!testInsert ||
