/*=auto=========================================================================
  ...
  Program:   3D Cjyx
  Module:    $RCSfile: vtkDMMLTransformStorageNode.h,v $
  Date:      $Date: 2006/03/19 17:12:29 $
  Version:   $Revision: 1.3 $

=========================================================================auto=*/

#ifndef __vtkDMMLTransformStorageNode_h
#define __vtkDMMLTransformStorageNode_h
...
/// \brief DMML node for transform storage on disk.
///
/// Storage nodes has methods to read/write transforms to/from disk.
class VTK_DMML_EXPORT vtkDMMLTransformStorageNode : public vtkDMMLStorageNode
{
  public:
  static vtkDMMLTransformStorageNode *New();
  vtkTypeMacro(vtkDMMLTransformStorageNode,vtkDMMLStorageNode);
...
protected:
  vtkDMMLTransformStorageNode();
  ~vtkDMMLTransformStorageNode() override;
  vtkDMMLTransformStorageNode(const vtkDMMLTransformStorageNode&);
  void operator=(const vtkDMMLTransformStorageNode&);
  ...
  virtual void SetAndObserveTransformFromParentAutoInvert(vtkDMMLTransformNode* transformNode, vtkAbstractTransform *transform);

  /// Read data and set it in the referenced node
  int ReadDataInternal(vtkDMMLNode *refNode) override;

  /// This method is specialized for ITKv3 tfm files as generated
  /// by legacy versions of BRAINSFit (see cjyx issue #3788).
  /// This generates the vtkOrientedBSplineTransform with an
  /// additive bulk transform (as in ITKv3 bspline transform).
  virtual int ReadFromITKv3BSplineTransformFile(vtkDMMLNode *refNode);
  ...
  /// In practice, as of Cjyx 4.4, only HDF5 (.h5)
  ...
  /// and a bspline component. (see cjyx issue #3788).
  virtual int ReadFromTransformFile(vtkDMMLNode *refNode);
  ...
  /// in LPS coordinate system. When the transform is read into Cjyx, the
  /// displacement vectors are converted to RAS coordinate system.
  virtual int ReadFromImageFile(vtkDMMLNode *refNode);
  ...
  virtual int WriteToTransformFile(vtkDMMLNode* refNode);

  /// Write displacement field transform from a 3-component scalar image.
  /// The 3 scalar components in Cjyx specify the displacement
  ...
  virtual int WriteToImageFile(vtkDMMLNode* refNode);
