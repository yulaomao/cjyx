...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLSelectionNode.cxx,v $
...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLSelectionNode);

//----------------------------------------------------------------------------
vtkDMMLSelectionNode::vtkDMMLSelectionNode()
{
...
//----------------------------------------------------------------------------
vtkDMMLSelectionNode::~vtkDMMLSelectionNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::WriteXML(ostream& of, int nIndent)
{
  Superclass::WriteXML(of, nIndent);
  vtkDMMLWriteXMLBeginMacro(of);
  vtkDMMLWriteXMLStringMacro(activePlaceNodeClassName, ActivePlaceNodeClassName);
  vtkDMMLWriteXMLEndMacro();
}

//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::ReadXMLAttributes(const char** atts)
{
  ...
  vtkDMMLReadXMLStringMacro(activePlaceNodeClassName, ActivePlaceNodeClassName);
...
void vtkDMMLSelectionNode::Copy(vtkDMMLNode *anode)
{
  int disabledModify = this->StartModify();
  Superclass::Copy(anode);

  vtkDMMLCopyBeginMacro(anode);
  vtkDMMLCopyStringMacro(ActivePlaceNodeClassName);
  vtkDMMLCopyEndMacro();
...
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  vtkDMMLPrintStringMacro(ActivePlaceNodeClassName);
...
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::AddNewPlaceNodeClassNameToList(const char *newClassName, const char *resource, const char *iconName)
{
  ...
  if (index == -1)
    ...
    this->InvokeEvent(vtkDMMLSelectionNode::PlaceNodeClassNameListModifiedEvent);
    }
  else
    {
...
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::RemovePlaceNodeClassNameFromList(const char *className)
{
...
//----------------------------------------------------------------------------
std::string vtkDMMLSelectionNode::GetPlaceNodeClassNameByIndex(int n)
{
...
//----------------------------------------------------------------------------
std::string vtkDMMLSelectionNode::GetPlaceNodeResourceByIndex(int n)
{
  std::string resource;
...
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::GetUnitNodes(std::vector<vtkDMMLUnitNode*>& units)
{
  ...
  for (NodeReferencesType::const_iterator it = this->NodeReferences.begin();
    ...
    if (it->first.compare(0, strlen(UNIT_NODE_REFERENCE_ROLE), UNIT_NODE_REFERENCE_ROLE) == 0 &&
      ...
      vtkDMMLNodeReference * reference = it->second[0];
      if (reference)
        {
        units.push_back(
          vtkDMMLUnitNode::SafeDownCast(reference->GetReferencedNode()));
        }
      }
    }
}
...
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::GetUnitNodeIDs(std::vector<const char*>& quantities,
...
//----------------------------------------------------------------------------
std::string vtkDMMLSelectionNode::GetPlaceNodeIconNameByIndex(int n)
{
  std::string iconName;
...
//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetUnitNodeID(const char* quantity)
{
...
//----------------------------------------------------------------------------
vtkDMMLUnitNode* vtkDMMLSelectionNode::GetUnitNode(const char* quantity)
{
  std::string safeQuantity = quantity ? quantity : "";
  std::string referenceRole = UNIT_NODE_REFERENCE_ROLE + safeQuantity;
  return vtkDMMLUnitNode::SafeDownCast(this->GetNodeReference(referenceRole.c_str()));
}

//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetUnitNodeID(const char* quantity, const char* id)
{
  std::string safeQuantity = quantity ? quantity : "";
  std::string referenceRole = UNIT_NODE_REFERENCE_ROLE + safeQuantity;
  ...
  if (this->GetMTime() > mTime)
    ...
    this->InvokeEvent(vtkDMMLSelectionNode::UnitModifiedEvent, &safeQuantity);
    }
}

//---------------------------------------------------------------------------
void vtkDMMLSelectionNode::ProcessDMMLEvents(vtkObject *caller,
                                             unsigned long event,
                                             void *callData)
{
  this->Superclass::ProcessDMMLEvents(caller, event, callData);

  vtkDMMLUnitNode* unitNode = vtkDMMLUnitNode::SafeDownCast(caller);
  if (unitNode && event == vtkCommand::ModifiedEvent)
    {
    std::string quantity =
      unitNode->GetQuantity() ? unitNode->GetQuantity() : "";
    this->InvokeEvent(
      vtkDMMLSelectionNode::UnitModifiedEvent, &quantity);
    }
}

//----------------------------------------------------------------------------
int vtkDMMLSelectionNode::PlaceNodeClassNameInList(std::string className)
{
  for (unsigned int i = 0; i < this->PlaceNodeClassNameList.size(); ++i)
    {
...
//----------------------------------------------------------------------------
std::string vtkDMMLSelectionNode::GetPlaceNodeResourceByClassName(std::string className)
{
  std::string resource;
...
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetReferenceActivePlaceNodeClassName (const char *className)
{
  ...
  this->InvokeEvent(vtkDMMLSelectionNode::ActivePlaceNodeClassNameChangedEvent);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActiveVolumeID()
{
  return this->GetNodeReferenceID(ACTIVE_VOLUME_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActiveVolumeID(const char* id)
{
  this->SetNodeReferenceID(ACTIVE_VOLUME_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetSecondaryVolumeID()
{
  return this->GetNodeReferenceID(SECONDARY_VOLUME_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetSecondaryVolumeID(const char* id)
{
  this->SetNodeReferenceID(SECONDARY_VOLUME_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActiveLabelVolumeID()
{
  return this->GetNodeReferenceID(ACTIVE_LABEL_VOLUME_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActiveLabelVolumeID(const char* id)
{
  this->SetNodeReferenceID(ACTIVE_LABEL_VOLUME_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActivePlaceNodeID()
{
  return this->GetNodeReferenceID(ACTIVE_PLACE_NODE_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActivePlaceNodeID(const char* id)
{
  ...
  this->InvokeEvent(vtkDMMLSelectionNode::ActivePlaceNodeIDChangedEvent);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActiveROIListID()
{
  return this->GetNodeReferenceID(ACTIVE_ROI_LIST_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActiveROIListID(const char* id)
{
  this->SetNodeReferenceID(ACTIVE_ROI_LIST_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActiveCameraID()
{
  return this->GetNodeReferenceID(ACTIVE_CAMERA_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActiveCameraID(const char* id)
{
  this->SetNodeReferenceID(ACTIVE_CAMERA_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActiveTableID()
{
  return this->GetNodeReferenceID(ACTIVE_TABLE_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActiveTableID(const char* id)
{
  this->SetNodeReferenceID(ACTIVE_TABLE_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActiveViewID()
{
  return this->GetNodeReferenceID(ACTIVE_VIEW_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActiveViewID(const char* id)
{
  this->SetNodeReferenceID(ACTIVE_VIEW_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActiveLayoutID()
{
  return this->GetNodeReferenceID(ACTIVE_LAYOUT_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActiveLayoutID(const char* id)
{
  this->SetNodeReferenceID(ACTIVE_LAYOUT_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
const char* vtkDMMLSelectionNode::GetActivePlotChartID()
{
  return this->GetNodeReferenceID(ACTIVE_PLOT_CHART_REFERENCE_ROLE);
}
//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActivePlotChartID(const char* id)
{
  this->SetNodeReferenceID(ACTIVE_PLOT_CHART_REFERENCE_ROLE, id);
}

//----------------------------------------------------------------------------
void vtkDMMLSelectionNode::SetActivePlaceNodePlacementValid(bool valid)
{
  ...
  this->InvokeEvent(vtkDMMLSelectionNode::ActivePlaceNodePlacementValidEvent);
}

//----------------------------------------------------------------------------
bool vtkDMMLSelectionNode::GetActivePlaceNodePlacementValid()
{
