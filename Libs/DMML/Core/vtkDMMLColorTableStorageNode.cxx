...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLColorTableStorageNode.cxx,v $
...
//------------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLColorTableStorageNode);

//----------------------------------------------------------------------------
vtkDMMLColorTableStorageNode::vtkDMMLColorTableStorageNode()
{
...
//----------------------------------------------------------------------------
vtkDMMLColorTableStorageNode::~vtkDMMLColorTableStorageNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLColorTableStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
  vtkDMMLStorageNode::PrintSelf(os,indent);
}

//----------------------------------------------------------------------------
bool vtkDMMLColorTableStorageNode::CanReadInReferenceNode(vtkDMMLNode* refNode)
{
  return refNode->IsA("vtkDMMLColorTableNode");
}

//----------------------------------------------------------------------------
int vtkDMMLColorTableStorageNode::ReadDataInternal(vtkDMMLNode *refNode)
{
  std::string fullName = this->GetFullNameFromFileName();

  // cast the input node
  vtkDMMLColorTableNode *colorNode =
    vtkDMMLColorTableNode::SafeDownCast(refNode);
  ...
  if (fstr.is_open())
    ...
    for (unsigned int i = 0; i < lines.size(); i++)
      ...
      b = b / 255.0;
      a = a / 255.0;
      // if the name has ticks around it, from copying from a dmml file, trim
      // them off the string
...
//----------------------------------------------------------------------------
int vtkDMMLColorTableStorageNode::WriteDataInternal(vtkDMMLNode *refNode)
{
  std::string fullName = this->GetFullNameFromFileName();
  if (fullName.empty())
    {
    vtkErrorMacro("vtkDMMLColorTableStorageNode: File name not specified");
    return 0;
    }

  // cast the input node
  vtkDMMLColorTableNode *colorNode = nullptr;
  if ( refNode->IsA("vtkDMMLColorTableNode") )
    {
    colorNode = dynamic_cast <vtkDMMLColorTableNode *> (refNode);
    }
...
//----------------------------------------------------------------------------
void vtkDMMLColorTableStorageNode::InitializeSupportedReadFileTypes()
{
...
//----------------------------------------------------------------------------
void vtkDMMLColorTableStorageNode::InitializeSupportedWriteFileTypes()
{
