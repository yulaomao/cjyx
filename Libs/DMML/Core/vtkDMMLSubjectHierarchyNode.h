...
// .NAME vtkDMMLSubjectHierarchyNode
// .SECTION Description
// DMML node to represent subject hierarchy tree
//

#ifndef __vtkDMMLSubjectHierarchyNode_h
#define __vtkDMMLSubjectHierarchyNode_h
...
/// \brief DMML node to represent a complete subject hierarchy tree
///
...
///   It is not singleton in either the common or the DMML sense, instead, the subject hierarchy logic
...
///
class VTK_DMML_EXPORT vtkDMMLSubjectHierarchyNode : public vtkDMMLNode
{
public:
  ...
  // Attribute name to identify subject hierarchy 2.0 nodes in scene DMML files
  ...
  enum
  {
    ...
    /// Use vtkDMMLSubjectHierarchyNode::ShowItemsInView or qCjyxSubjectHierarchyPluginHandler::showItemsInView
  ...
  struct SubjectHierarchyItemsShowInViewRequestedEventData
    ...
    vtkDMMLAbstractViewNode* viewNode = nullptr;
    };

public:
  static vtkDMMLSubjectHierarchyNode *New();
  vtkTypeMacro(vtkDMMLSubjectHierarchyNode,vtkDMMLNode);
...
// Get/Set methods
public:
  /// Get the (practically) singleton subject hierarchy node from DMML scene.
  /// Merges subject hierarchy nodes if multiple found, and returns the merged one.
  static vtkDMMLSubjectHierarchyNode* GetSubjectHierarchyNode(vtkDMMLScene* scene);
  ...
  vtkDMMLNode* GetItemDataNode(vtkIdType itemID);
  ...
  /// Typically called after setting an attribute indicating a desired role for the item.
  void RequestOwnerPluginSearch(vtkDMMLNode* dataNode);
  ...
  void SetItemDataNode(vtkIdType itemID, vtkDMMLNode* dataNode);
...
public:
  ...
  /// \param dataNode Associated data DMML node
  /// \return ID of the item in the hierarchy that was assigned automatically when adding
  vtkIdType CreateItem(vtkIdType parentItemID, vtkDMMLNode* dataNode, const char* ownerPluginName=nullptr);
  ...
  /// \param level Level of the hierarchy item. It will be stored as attribute (\sa vtkDMMLSubjectHierarchyConstants)
  ...
  /// \return Success flag
  bool ReparentItemByDataNode(vtkIdType itemID, vtkDMMLNode* newParentNode);
...
public:
  ...
  /// Get subject hierarchy item associated to a data DMML node
  ...
  vtkIdType GetItemByDataNode(vtkDMMLNode* dataNode);
  ...
  vtkDMMLNode* GetParentDataNode(vtkDMMLNode* dataNode, bool recursive=false);
...
public:
  ...
  vtkDMMLDisplayNode* GetDisplayNodeForItem(vtkIdType itemID);
  ...
  bool IsAnyNodeInBranchTransformed(vtkIdType itemID, bool includeParentItem=true, vtkDMMLTransformNode* exceptionNode=nullptr);
  ...
  static vtkDMMLSubjectHierarchyNode* ResolveSubjectHierarchy(vtkDMMLScene* scene);

  /// Show items in selected view (used for drag&drop of subject hierarchy items into the viewer)
  void ShowItemsInView(vtkIdList* itemIDs, vtkDMMLAbstractViewNode* viewNode);
...
protected:
  vtkDMMLSubjectHierarchyNode();
  ~vtkDMMLSubjectHierarchyNode() override;
  vtkDMMLSubjectHierarchyNode(const vtkDMMLSubjectHierarchyNode&);
  void operator=(const vtkDMMLSubjectHierarchyNode&);
