...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLScene.h,v $
Date:      $Date: 2006/03/17 15:10:09 $
Version:   $Revision: 1.18 $

=========================================================================auto=*/

#ifndef __vtkDMMLScene_h
#define __vtkDMMLScene_h
...
/// \brief A set of DMML Nodes that supports serialization and undo/redo.
///
/// vtkDMMLScene represents and provides methods to manipulate a list of
/// DMML objects. The list is core and duplicate entries are not prevented.
//
/// \sa vtkDMMLNode
/// \sa vtkCollection
class VTK_DMML_EXPORT vtkDMMLScene : public vtkObject
{
  ///
  /// make the vtkDMMLSceneViewNode a friend since it has internal vtkDMMLScene
  /// so that it can call protected methods, for example UpdateNodeIDs()
  /// but that's the only class that is allowed to do so
  friend class vtkDMMLSceneViewNode;

public:
  static vtkDMMLScene *New();
  vtkTypeMacro(vtkDMMLScene, vtkObject);
  ...
  int Connect(vtkDMMLMessageCollection* userMessages = nullptr);
  ...
  /// If userMessages is not nullptr then the method may add messages to it about issues
  /// encountered during the operation.
  int Import(vtkDMMLMessageCollection* userMessages=nullptr);

  /// Save scene into URL
  /// If userMessages is not nullptr then the method may add messages to it about issues
  /// encountered during the operation.
  /// Returns nonzero on success
  int Commit(const char* url=nullptr, vtkDMMLMessageCollection* userMessages=nullptr);
  ...
  vtkDMMLNode* CreateNodeByClass(const char* className);
  ...
  /// wrapped with: <code>\#if DMML_APPLICATION_SUPPORT_VERSION < 0x0X0Y0Z</code> and <code>\#endif</code>
  /// where X is the major version of Cjyx scene to support, Y the minor and
  /// Z the patch version.
  ///
  /// \sa CreateNodeByClass(), GetClassNameByTag()
  void RegisterNodeClass(vtkDMMLNode* node, const char* tagName);

  /// \brief Utility function to RegisterNodeClass(), the node tag name is used when
  /// registering the node.
  ///
  /// \sa RegisterNodeClass(vtkDMMLNode* node, const char* tagName)
  void RegisterNodeClass(vtkDMMLNode* node);
  ...
  ///
  /// \sa RegisterNodeClass(vtkDMMLNode* node), GetTypeDisplayNameByClassName
  ...
  void AddDefaultNode(vtkDMMLNode* node);
  vtkDMMLNode* GetDefaultNodeByClass(const char* className);
  ...
  /// \brief Add a node to the scene and send vtkDMMLScene::NodeAboutToBeAddedEvent,
  /// vtkDMMLScene::NodeAddedEvent and vtkDMMLScene::SceneModified events.
  ///
  /// A unique ID (e.g. "vtkDMMLModeNode1", "vtkDMMLScalarVolumeNode4") is
  /// generated and set to the node. If the node has no name
  /// (i.e. vtkDMMLNode::GetName() == 0), a unique name is given to the node
  ...
  /// added but its properties are copied (c.f. vtkDMMLNode::CopyWithScene())
  /// into the already existing singleton node. That node is then returned.
  vtkDMMLNode* AddNode(vtkDMMLNode *nodeToAdd);
  ...
  ///
  /// The method calls CreateNodeByClass(), vtkDMMLNode::SetName() and AddNode().
  ///
  ...
  ///
  /// \sa CreateNodeByClass(), vtkDMMLNode::SetName(), AddNode()
  /// \sa AddDefaultNode()
  /// \sa GenerateUniqueName()
  vtkDMMLNode* AddNewNodeByClass(std::string className, std::string nodeBaseName = "");
  ...
  ///
  /// The method calls CreateNodeByClass(), vtkDMMLNode::SetName(), vtkDMMLNode::SetID(), and AddNode().
  /// \sa AddNewNodeByClass(), CreateNodeByClass(), vtkDMMLNode::SetName(), vtkDMMLNode::SetID(), AddNode()
  vtkDMMLNode* AddNewNodeByClassWithID(std::string className, std::string nodeBaseName, std::string nodeID);

  /// Add a copy of a node to the scene.
  vtkDMMLNode* CopyNode(vtkDMMLNode *n);

  /// \brief Invoke a vtkDMMLScene::NodeAddedEvent.
  ///
  /// This function is used after adding a bunch of nodes with AddNodeNoNotify()
  void NodeAdded(vtkDMMLNode *n);
  void NodeAdded() {this->NodeAdded(nullptr);}

  /// Remove a path from the list.
  void RemoveNode(vtkDMMLNode *n);

  /// \brief Determine whether a particular node is present.
  ///
  /// Returns its position in the list.
  int IsNodePresent(vtkDMMLNode *n);
  ...
  /// Use other methods instead (GetNodes(), GetNodesByClass(), etc.)
  /// or traverse collection returned by GetNodes() using a collection iterator.
  vtkDMMLNode *GetNextNodeByClass(const char* className);

  /// Get nodes having the specified name
  vtkCollection *GetNodesByName(const char* name);
  vtkDMMLNode *GetFirstNodeByName(const char* name);
  ...
  /// visible in editors.
  vtkDMMLNode *GetFirstNode(const char* byName = nullptr, const char* byClass = nullptr,
                            const int* byHideFromEditors = nullptr,
                            bool exactNameMatch = true);

  /// Get node given a unique ID
  vtkDMMLNode *GetNodeByID(const char* name);
  vtkDMMLNode *GetNodeByID(std::string name);
  ...
  vtkDMMLNode* GetNthNode(int n);

  /// Get n-th node of a specified class in the scene
  vtkDMMLNode* GetNthNodeByClass(int n, const char* className );
  /// Convenience function for getting 0-th node of a specified class in the scene
  vtkDMMLNode* GetFirstNodeByClass(const char* className);
  ...
  vtkDMMLNode* GetSingletonNode(const char* singletonTag, const char* className);
  ...
  /// Return 0 if such node can't be found in the scene.
  vtkDMMLNode* GetSingletonNode(vtkDMMLNode* n);
  ...
  vtkDMMLNode * GetNthRegisteredNodeClass(int n);

  /// \brief Return True if \a className is a registered node.
  ///
  /// \sa RegisterNodeClass(vtkDMMLNode* node)
  ...
  vtkDMMLNode* InsertAfterNode(vtkDMMLNode *item, vtkDMMLNode *newItem);
  /// insert a node in the scene before a specified node
  vtkDMMLNode* InsertBeforeNode(vtkDMMLNode *item, vtkDMMLNode *newItem);
  ...
  void SaveStateForUndo(std::vector<vtkDMMLNode *> nodes);
  ...
  void AddReferencedNodeID(const char *id, vtkDMMLNode *refrencingNode);
  bool IsNodeReferencingNodeID(vtkDMMLNode* referencingNode, const char* id);
  ...
  vtkDMMLNode* GetNthReferencingNode(int n);
  /// Get the ReferencedID component of the n-th ReferencedID-ReferencingNode pair.
  /// Only for testing and debugging.
  const char* GetNthReferencedID(int n);

  void RemoveReferencedNodeID(const char *id, vtkDMMLNode *refrencingNode);

  void RemoveNodeReferences(vtkDMMLNode *node);

  void RemoveReferencesToNode(vtkDMMLNode *node);

  /// \brief Notify nodes about node ID changes.
  ///
  /// vtkDMMLNode::UpdateReferenceID() is called for all the nodes that refer
  ...
  void CopyNodeReferences(vtkDMMLScene *scene);

  void CopyNodeChangedIDs(vtkDMMLScene *scene);
  ...
  /// get the new id of the node that is different from one in the dmml file
  ...
  vtkCollection* GetReferencedNodes(vtkDMMLNode *node, bool recursive=true);

  /// Get vector of nodes containing references to an input node
  void GetReferencingNodes(vtkDMMLNode* referencedNode, std::vector<vtkDMMLNode *> &referencingNodes);
  ...
  void GetReferencedSubScene(vtkDMMLNode *node, vtkDMMLScene* newScene);
  ...
  /// The scene is in \link vtkDMMLScene::BatchProcessState BatchProcessState \endlink
  ...
  /// and EndState().
  /// When the scene is in \link vtkDMMLScene::BatchProcessState BatchProcessState \endlink mode, observers can
  /// ignore the events \link vtkDMMLScene::NodeAddedEvent NodeAddedEvent \endlink
  /// and \link vtkDMMLScene::NodeRemovedEvent NodeRemovedEvent \endlink
  /// to only synchronize with the scene when the scene is no longer in
  /// \link vtkDMMLScene::BatchProcessState BatchProcessState \endlink
  /// (\link vtkDMMLScene::EndBatchProcessEvent EndBatchProcessEvent \endlink being fired).
  ///
  /// The call <code>scene->Connect("myScene.dmml");</code> that closes and
  /// import a scene will fire the events:
  ///
  /// * \link vtkDMMLScene::StartBatchProcessEvent StartBatchProcessEvent \endlink,
  /// * \link vtkDMMLScene::StartCloseEvent StartCloseEvent \endlink,
  ///
  /// * \link vtkDMMLScene::NodeAboutToBeRemovedEvent NodeAboutToBeRemovedEvent \endlink,
  /// * \link vtkDMMLScene::NodeRemovedEvent NodeRemovedEvent \endlink,
  /// * \link vtkDMMLScene::ProgressCloseEvent ProgressCloseEvent \endlink,
  /// * \link vtkDMMLScene::ProgressBatchProcessEvent ProgressBatchProcessEvent \endlink,
  /// * ...
  /// * \link vtkDMMLScene::EndCloseEvent EndCloseEvent \endlink,
  /// * \link vtkDMMLScene::StartImportEvent StartImportEvent \endlink,
  ///
  /// * \link vtkDMMLScene::NodeAboutToBeAddedEvent NodeAboutToBeAddedEvent \endlink,
  /// * \link vtkDMMLScene::NodeAddedEvent NodeAddedEvent \endlink,
  /// * \link vtkDMMLScene::ProgressImportEvent ProgressImportEvent \endlink,
  /// * \link vtkDMMLScene::ProgressBatchProcessEvent ProgressBatchProcessEvent \endlink,
  /// * ...
  /// * \link vtkDMMLScene::EndImportEvent EndImportEvent \endlink,
  /// * \link vtkDMMLScene::EndBatchProcessEvent EndBatchProcessEvent \endlink
  enum StateType
    {
  ...
  /// state, e.g. \link vtkDMMLScene::StartImportEvent StartImportEvent \endlink
  /// if \a state is \link vtkDMMLScene::ImportState ImportState \endlink.
  /// If the state is \link vtkDMMLScene::BatchProcessState BatchProcessState \endlink,
  /// \link vtkDMMLScene::CloseState CloseState \endlink,
  /// \link vtkDMMLScene::ImportState ImportState \endlink
  /// or \link vtkDMMLScene::RestoreState RestoreState \endlink
  /// and if the scene is not already in a \link vtkDMMLScene::BatchProcessState BatchProcessState \endlink
  /// state, it also fires the event \link vtkDMMLScene::StartBatchProcessEvent StartBatchProcessEvent \endlink.
  ///
  /// Example:
  /// \code
  ...
  /// e.g. vtkDMMLScene::EndImportEvent if state is \link vtkDMMLScene::ImportState ImportState \endlink.
  ///
  ...
  enum SceneEventType
    ...
    NewSceneEvent = 66030,
    MetadataAddedEvent = 66032, // ### Cjyx 4.5: Simplify - Do not explicitly set for backward compat. See issue #3472
  ...
  void CopyRegisteredNodesToScene(vtkDMMLScene *scene);

  /// Copies all singleton nodes into the parameter scene.
  void CopySingletonNodesToScene(vtkDMMLScene *scene);

  /// Copies all default nodes into the parameter scene. Does not remove existing default nodes.
  void CopyDefaultNodesToScene(vtkDMMLScene *scene);
  ...
  /// \brief Write the scene to a DMML scene bundle (.mrb) file.
  /// If thumbnail image is provided then it is saved in the scene's root folder.
  /// If userMessages is not nullptr then the method may add messages to it about issues
  /// encountered during the operation.
  /// Returns false if the save failed
  bool WriteToMRB(const char* filename, vtkImageData* thumbnail=nullptr, vtkDMMLMessageCollection* userMessages=nullptr);

  /// \brief Read the scene from a DMML scene bundle (.mrb) file
  /// If userMessages is not nullptr then the method may add messages to it about issues
  /// encountered during the operation.
  bool ReadFromMRB(const char* fullName, bool clear=false, vtkDMMLMessageCollection* userMessages = nullptr);
  ...
  /// If userMessages is not nullptr then the method may add messages to it about issues
  /// encountered during the operation.
  static std::string UnpackCjyxDataBundle(const char* sdbFilePath, const char* temporaryDirectory, vtkDMMLMessageCollection* userMessages=nullptr);
  ...
  /// encountered during the operation.
  /// Returns false if the save failed
  bool SaveSceneToCjyxDataBundleDirectory(const char* sdbDir, vtkImageData* thumbnail=nullptr, vtkDMMLMessageCollection* userMessages=nullptr);
...
protected:
  ...
  vtkDMMLScene();
  ~vtkDMMLScene() override;

  void PushIntoUndoStack();
  void PushIntoRedoStack();

  void CopyNodeInUndoStack(vtkDMMLNode *node);
  void CopyNodeInRedoStack(vtkDMMLNode *node);

  /// Add a node to the scene without invoking a vtkDMMLScene::NodeAddedEvent event.
  ///
  /// \warning Use with extreme caution as it might unsynchronize observer.
  vtkDMMLNode* AddNodeNoNotify(vtkDMMLNode *n);

  /// Get referenced nodes
  /// \param recursive All referenced nodes recursively if true,
  ///   only directly referenced nodes if false. Default is true.
  void AddReferencedNodes(vtkDMMLNode *node, vtkCollection *refNodes, bool recursive=true);
  ...
  /// Handle vtkDMMLScene::DeleteEvent: clear the scene.
  static void SceneCallback(vtkObject *caller, unsigned long eid, void *clientData, void *callData);

  std::string GenerateUniqueID(vtkDMMLNode* node);
  ...
  /// Return a unique name for a DMML node. It uses the node tag as the base.
  std::string GenerateUniqueName(vtkDMMLNode* node);
  ...
  void AddNodeID(vtkDMMLNode *node);
  ...
  NodeReferencesType::iterator FindNodeReference(const char* referencedId, vtkDMMLNode* referencingNode);

  /// Clean up elements of the undo/redo stack beyond the maximum size
  void TrimUndoStack();

  /// Reserve all node reference ids for a node
  void ReserveNodeReferenceIDs(vtkDMMLNode* node);
  ...
  virtual void SetSubjectHierarchyNode(vtkDMMLSubjectHierarchyNode*);
  ...
  /// If userMessages is not nullptr then the method may add messages to it about issues
  /// encountered during the operation.
  bool SaveStorableNodeToCjyxDataBundleDirectory(vtkDMMLStorableNode* storableNode, std::string& dataDir,
  ...
  std::vector< vtkDMMLNode* > RegisteredNodeClasses;
  ...
  std::map< std::string, vtkSmartPointer<vtkDMMLNode> > NodeIDs;
  ...
  std::map< std::string, vtkSmartPointer<vtkDMMLNode> > DefaultNodes;
...
private:
  ...
  vtkDMMLScene(const vtkDMMLScene&);   // Not implemented
  void operator=(const vtkDMMLScene&); // Not implemented

  /// If userMessages is not nullptr then the method may add messages to it about issues
  /// encountered during the operation.
  /// Returns nonzero on success.
  int LoadIntoScene(vtkCollection* scene, vtkDMMLMessageCollection* userMessages=nullptr);
...
//------------------------------------------------------------------------------
bool vtkDMMLScene::IsBatchProcessing()const
{
  return (this->GetStates() & vtkDMMLScene::BatchProcessState)
         == vtkDMMLScene::BatchProcessState;
}

//------------------------------------------------------------------------------
bool vtkDMMLScene::IsClosing()const
{
  return (this->GetStates() & vtkDMMLScene::CloseState)
         == vtkDMMLScene::CloseState;
}

//------------------------------------------------------------------------------
bool vtkDMMLScene::IsImporting()const
{
  return (this->GetStates() & vtkDMMLScene::ImportState)
         == vtkDMMLScene::ImportState;
}

//------------------------------------------------------------------------------
bool vtkDMMLScene::IsRestoring()const
{
  return (this->GetStates() & vtkDMMLScene::RestoreState)
         == vtkDMMLScene::RestoreState;
}

//------------------------------------------------------------------------------
bool vtkDMMLScene::IsUndoing()const
{
  return (this->GetStates() & vtkDMMLScene::UndoState)
    == vtkDMMLScene::UndoState;
}

//------------------------------------------------------------------------------
bool vtkDMMLScene::IsRedoing()const
{
  return (this->GetStates() & vtkDMMLScene::RedoState)
    == vtkDMMLScene::RedoState;
}
