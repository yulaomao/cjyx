...
{
  //--- Find the DMML node that points to this file in cache.
  ...
  int nnodes = this->DMMLScene->GetNumberOfNodesByClass ( "vtkDMMLStorableNode" );
  vtkDMMLStorableNode *node;
  std::string uri;
  for ( int n=0; n < nnodes; n++ )
    {
    node = vtkDMMLStorableNode::SafeDownCast ( this->DMMLScene->GetNthNodeByClass (n, "vtkDMMLStorableNode" ));
    if ( node != nullptr )
      {
