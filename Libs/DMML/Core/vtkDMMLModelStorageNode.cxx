/*=auto=========================================================================
  ...
  Program:   3D Cjyx
  Module:    $RCSfile: vtkDMMLModelStorageNode.cxx,v $
...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLModelStorageNode);

//----------------------------------------------------------------------------
vtkDMMLModelStorageNode::vtkDMMLModelStorageNode()
{
  this->DefaultWriteFileExtension = "vtk";
  this->CoordinateSystem = vtkDMMLStorageNode::CoordinateSystemLPS;
}

//----------------------------------------------------------------------------
vtkDMMLModelStorageNode::~vtkDMMLModelStorageNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLModelStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
  vtkDMMLStorageNode::PrintSelf(os,indent);
}

//----------------------------------------------------------------------------
void vtkDMMLModelStorageNode::ReadXMLAttributes(const char** atts)
{
  ...
  this->CoordinateSystem = vtkDMMLStorageNode::CoordinateSystemRAS;
...
//----------------------------------------------------------------------------
void vtkDMMLModelStorageNode::WriteXML(ostream& of, int nIndent)
{
...
//---------------------------------------------------------------------------
const char* vtkDMMLModelStorageNode::GetCoordinateSystemAsString(int id)
{
  return vtkDMMLStorageNode::GetCoordinateSystemTypeAsString(id);
}

//-----------------------------------------------------------
int vtkDMMLModelStorageNode::GetCoordinateSystemFromString(const char* name)
{
  return vtkDMMLStorageNode::GetCoordinateSystemTypeFromString(name);
}

//----------------------------------------------------------------------------
bool vtkDMMLModelStorageNode::CanReadInReferenceNode(vtkDMMLNode *refNode)
{
  return refNode->IsA("vtkDMMLModelNode");
}

//----------------------------------------------------------------------------
int vtkDMMLModelStorageNode::ReadDataInternal(vtkDMMLNode *refNode)
{
  ...
  vtkDMMLModelNode *modelNode = dynamic_cast <vtkDMMLModelNode *> (refNode);
  if (!modelNode)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLModelStorageNode::ReadDataInternal",
  ...
  try
    ...
    if (extension == std::string(".g") || extension == std::string(".byu"))
      ...
      coordinateSystemInFileHeader = vtkDMMLModelStorageNode::GetCoordinateSystemFromFileHeader(reader->GetHeader());
      }
      ...
      meshFromFile = reader->GetOutput();
      this->GetUserMessages()->SetObservedObject(nullptr);
      coordinateSystemInFileHeader = vtkDMMLModelStorageNode::GetCoordinateSystemFromFieldData(meshFromFile);
      }
      ...
      for (int commentIndex = 0; commentIndex < comments->GetNumberOfValues(); commentIndex++)
        {
        coordinateSystemInFileHeader = vtkDMMLModelStorageNode::GetCoordinateSystemFromFileHeader(comments->GetValue(commentIndex));
      ...
      meshFromFile = reader->GetOutput();
      this->GetUserMessages()->SetObservedObject(nullptr);
      coordinateSystemInFileHeader = vtkDMMLModelStorageNode::GetCoordinateSystemFromFileHeader(reader->GetComment());
      }
  ...
  if (coordinateSystemInFileHeader >= 0)
    ...
    vtkInfoMacro("ReadDataInternal (" << (this->ID ? this->ID : "(unknown)") << "): File "
      ...
      << vtkDMMLStorageNode::GetCoordinateSystemTypeAsString(this->CoordinateSystem) << ".");
    }

  vtkSmartPointer<vtkPointSet> meshToSetInNode;
  if (this->CoordinateSystem == vtkDMMLStorageNode::CoordinateSystemRAS)
    {
    ...
    vtkDMMLModelStorageNode::ConvertBetweenRASAndLPS(meshFromFile, meshToSetInNode);
    }
  ...
  if (modelNode->GetMesh() != nullptr)
    ...
    for (int i=0; i<modelNode->GetNumberOfDisplayNodes(); ++i)
      ...
      if (displayNode && displayNode->GetScalarRangeFlag() == vtkDMMLDisplayNode::UseDataScalarRange)
        {
...
//----------------------------------------------------------------------------
int vtkDMMLModelStorageNode::WriteDataInternal(vtkDMMLNode *refNode)
{
  vtkDMMLModelNode *modelNode = vtkDMMLModelNode::SafeDownCast(refNode);

  std::string fullName = this->GetFullNameFromFileName();
  if (fullName.empty())
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLModelStorageNode::WriteDataInternal",
  ...
  const std::string coordinateSystemStr = vtkDMMLStorageNode::GetCoordinateSystemTypeAsString(this->CoordinateSystem);
  ...
  if (this->CoordinateSystem == vtkDMMLStorageNode::CoordinateSystemRAS)
    ...
    {
    // transform from RAS to LPS
    if (modelNode->GetMeshType() == vtkDMMLModelNode::PolyDataMeshType)
      {
    ...
    vtkDMMLModelStorageNode::ConvertBetweenRASAndLPS(modelNode->GetMesh(), meshToWrite);
    }

  bool success = true;

  if (extension == ".vtk" &&
    (modelNode->GetMeshType() == vtkDMMLModelNode::PolyDataMeshType
    || modelNode->GetMeshType() == vtkDMMLModelNode::UnstructuredGridMeshType))
    {
    ...
    if (modelNode->GetMeshType() == vtkDMMLModelNode::PolyDataMeshType)
      ...
      // version 5.1 is not compatible with earlier Cjyx versions (VTK < 9) and most other software
    ...
    // In the future (when Cjyx switches to VTK8) array metadata may be used instead of separate field data.
    ...
    if (!fieldData->GetAbstractArray(coordinateSystemTag.c_str()))
      ...
      {
      vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLModelStorageNode::WriteDataInternal",
    ...
    vtkDMMLDisplayNode* displayNode = modelNode->GetDisplayNode();
    if (displayNode)
      {
  ...
  if (success && this->GetUserMessages()->GetNumberOfMessagesOfType(vtkCommand::ErrorEvent)==0
    ...
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLModelStorageNode::WriteDataInternal",
      "Failed to write model " << (this->ID ? this->ID : "(unknown)")
      << ": Output file " << fullName.c_str() << " has not been created.");
    success = false;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLModelStorageNode::InitializeSupportedReadFileTypes()
{
...
//----------------------------------------------------------------------------
void vtkDMMLModelStorageNode::InitializeSupportedWriteFileTypes()
{
  vtkDMMLModelNode* modelNode = this->GetAssociatedDataNode();
  ...
  if (!modelNode || modelNode->GetMeshType() == vtkDMMLModelNode::UnstructuredGridMeshType)
    {
...
//----------------------------------------------------------------------------
vtkDMMLModelNode* vtkDMMLModelStorageNode::GetAssociatedDataNode()
{
  if (!this->GetScene())
    {
    return nullptr;
  ...
  unsigned int numberOfNodes = this->GetScene()->GetNodesByClass("vtkDMMLModelNode", nodes);
  for (unsigned int nodeIndex=0; nodeIndex<numberOfNodes; nodeIndex++)
    {
    vtkDMMLModelNode* node = vtkDMMLModelNode::SafeDownCast(nodes[nodeIndex]);
    if (node)
      {
      const char* storageNodeID = node->GetStorageNodeID();
      if (storageNodeID && !strcmp(storageNodeID, this->ID))
        {
        return vtkDMMLModelNode::SafeDownCast(node);
        }
      }
    }
...
//----------------------------------------------------------------------------
void vtkDMMLModelStorageNode::ConvertBetweenRASAndLPS(vtkPointSet* inputMesh, vtkPointSet* outputMesh)
{
  if (!inputMesh || !outputMesh)
    {
    vtkGenericWarningMacro("vtkDMMLModelStorageNode::ConvertBetweenRASAndLPS: invalid input or output mesh");
    return;
    }
...
//----------------------------------------------------------------------------
int vtkDMMLModelStorageNode::GetCoordinateSystemFromFileHeader(const char* header)
{
  if (!header)
    {
    return -1;
...
//----------------------------------------------------------------------------
int vtkDMMLModelStorageNode::GetCoordinateSystemFromFieldData(vtkPointSet* mesh)
{
  if (!mesh)
    {
    return -1;
