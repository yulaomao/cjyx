...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLSceneViewNode.cxx,v $
...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLSceneViewNode);

//----------------------------------------------------------------------------
vtkDMMLSceneViewNode::vtkDMMLSceneViewNode()
{
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::WriteNodeBodyXML(ostream& of, int nIndent)
{
  if (!this->SnapshotScene)
    {
    return;
  ...
  for (int n=0; n < this->SnapshotScene->GetNodes()->GetNumberOfItems(); n++)
    {
    vtkDMMLNode* node = (vtkDMMLNode*)this->SnapshotScene->GetNodes()->GetItemAsObject(n);
    if (node && !node->IsA("vtkDMMLSceneViewNode") && node->GetSaveWithScene())
      {
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::ReadXMLAttributes(const char** atts)
{
  ...
  if (this->GetID())
    ...
    {
    screenCaptureFilename += "vtkDMMLSceneViewNodeNoID";
    }
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::ProcessChildNode(vtkDMMLNode *node)
{
  ...
  if (this->SnapshotScene == nullptr)
    {
    this->SnapshotScene = vtkDMMLScene::New();
    }
...
void vtkDMMLSceneViewNode::Copy(vtkDMMLNode *anode)
{
  Superclass::Copy(anode);
  vtkDMMLSceneViewNode *snode = (vtkDMMLSceneViewNode *) anode;

  this->SetScreenShot(vtkDMMLSceneViewNode::SafeDownCast(anode)->GetScreenShot());
  this->SetScreenShotType(vtkDMMLSceneViewNode::SafeDownCast(anode)->GetScreenShotType());
  this->SetSceneViewDescription(vtkDMMLSceneViewNode::SafeDownCast(anode)->GetSceneViewDescription());
  ...
  if ( snode->SnapshotScene != nullptr )
    ...
    for (n=0; n < snode->SnapshotScene->GetNodes()->GetNumberOfItems(); n++)
      {
      node = (vtkDMMLNode*)snode->SnapshotScene->GetNodes()->GetItemAsObject(n);
      if (node)
        {
        node->SetScene(this->SnapshotScene);
        this->SnapshotScene->AddNodeID(node);
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::PrintSelf(ostream& os, vtkIndent indent)
{
  Superclass::PrintSelf(os,indent);
}

//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::UpdateScene(vtkDMMLScene *scene)
{
...
void vtkDMMLSceneViewNode::UpdateStoredScene()
{
  ...
  for (n=0; n<nnodesSanpshot; n++)
    {
    node  = vtkDMMLNode::SafeDownCast(this->SnapshotScene->GetNodes()->GetItemAsObject(n));
    if (node)
      {
      node->SetAddToSceneNoModify(0);
      }
  ...
  for (n=0; n<nnodesSanpshot; n++)
    {
    node  = vtkDMMLNode::SafeDownCast(this->Nodes->GetNodes()->GetItemAsObject(n));
    if (node)
      {
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::StoreScene()
{
  if (this->Scene == nullptr)
    {
    return;
  ...
  int nnodes = this->GetScene()->GetNodesByClass("vtkDMMLStorableNode", nodes);
  for (int i = 0; i < nnodes; ++i)
    {
    vtkDMMLStorableNode *storableNode = vtkDMMLStorableNode::SafeDownCast(nodes[i]);
    if (storableNode)
      {
  ...
  for (int n=0; n < this->Scene->GetNumberOfNodes(); n++)
    {
    vtkDMMLNode *node = this->Scene->GetNthNode(n);
    if (this->IncludeNodeInSceneView(node) &&
        node->GetSaveWithScene() )
      {
      vtkSmartPointer<vtkDMMLNode> newNode = vtkSmartPointer<vtkDMMLNode>::Take(node->CreateNodeInstance());
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::AddMissingNodes()
{
  if (this->Scene == nullptr)
    {
  ...
  std::map<std::string, vtkDMMLNode*> snapshotMap;
  ...
  for (sceneNodes->InitTraversal(it);
    ...
    {
    std::map<std::string, vtkDMMLNode*>::iterator iter = snapshotMap.find(std::string(node->GetID()));
...
//----------------------------------------------------------------------------
bool vtkDMMLSceneViewNode::RestoreScene(bool removeNodes)
{
  if (this->Scene == nullptr)
    {
  ...
  this->Scene->StartState(vtkDMMLScene::RestoreState);
  ...
  std::stack<vtkSmartPointer<vtkDMMLNode> > removedNodes;
  for (sceneNodes->InitTraversal(it);
       (node = vtkDMMLNode::SafeDownCast(sceneNodes->GetNextItemAsObject(it))) ;)
    {
    std::map<std::string, vtkDMMLNode*>::iterator iter = snapshotMap.find(std::string(node->GetID()));
    ...
    if (iter == snapshotMap.end() &&
        ...
        node->GetSaveWithScene())
      {
      removedNodes.push(vtkSmartPointer<vtkDMMLNode>(node));
      }
    }
  while(!removedNodes.empty())
    {
    vtkDMMLNode* nodeToRemove = removedNodes.top().GetPointer();
    ...
    if (isNodeInScene)
      ...
      if (removeNodes)
        ...
        this->Scene->EndState(vtkDMMLScene::RestoreState);
        // signal that there is an error state
        vtkErrorMacro("Unable to restore scene, data in main Cjyx scene that is not included in the scene view");
        return false;
        }
      }
    }

  std::vector<vtkDMMLNode *> addedNodes;
  for (n=0; n < numNodesInSceneView; n++)
    {
    node = vtkDMMLNode::SafeDownCast(this->SnapshotScene->GetNodes()->GetItemAsObject(n));
    if (node)
      ...
      if (this->IncludeNodeInSceneView(node))
        {
        vtkDMMLNode *snode = this->Scene->GetNodeByID(node->GetID());

        if (snode)
          {
          snode->SetScene(this->Scene);
          // to prevent copying of default info if not stored in snapshot
          DMMLNodeModifyBlocker blocker(snode);
          snode->Copy(node);
  ...
  for(n=0; n<addedNodes.size(); n++)
    ...
    //this->Scene->InvokeEvent(vtkDMMLScene::NodeAddedEvent, addedNodes[n] );
    }
...
//----------------------------------------------------------------------------
vtkDMMLScene* vtkDMMLSceneViewNode::GetStoredScene()
{
  return this->SnapshotScene;
}

//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::SetAbsentStorageFileNames()
{
  if (this->Scene == nullptr)
    {
    return;
  ...
  for (n=0; n<numNodesInSceneView; n++)
    ...
    if (node)
      ...
      vtkDMMLStorageNode *snode = vtkDMMLStorageNode::SafeDownCast(node);
      if (snode)
        {
        vtkDMMLNode *node1 = this->Scene->GetNodeByID(snode->GetID());
        if (node1)
          {
          vtkDMMLStorageNode *snode1 = vtkDMMLStorageNode::SafeDownCast(node1);
          if (snode1)
            {
...
//----------------------------------------------------------------------------
vtkDMMLStorageNode* vtkDMMLSceneViewNode::CreateDefaultStorageNode()
{
  ...
  return vtkDMMLStorageNode::SafeDownCast(
    scene->CreateNodeByClass("vtkDMMLSceneViewStorageNode"));
}

//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::SetSceneViewDescription(const vtkStdString& newDescription)
{
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::SetScreenShot(vtkImageData* newScreenShot)
{
  this->StorableModifiedTime.Modified();
...
//----------------------------------------------------------------------------
void vtkDMMLSceneViewNode::SetScreenShotType(int newScreenShotType)
{
...
//----------------------------------------------------------------------------
int vtkDMMLSceneViewNode::GetNodesByClass(const char *className, std::vector<vtkDMMLNode *> &nodes)
{
  if (!this->SnapshotScene)
    {
    return 0;
...
//------------------------------------------------------------------------------
vtkCollection* vtkDMMLSceneViewNode::GetNodesByClass(const char *className)
{
  if (!this->SnapshotScene)
    {
    return nullptr;
...
//----------------------------------------------------------------------------
bool vtkDMMLSceneViewNode::IncludeNodeInSceneView(vtkDMMLNode *node)
{
  if (!node)
    {
    return false;
  ...
  if (node->IsA("vtkDMMLSceneViewNode") ||
      node->IsA("vtkDMMLSceneViewStorageNode") ||
      node->IsA("vtkDMMLSnapshotClipNode") ||
      (node->IsA("vtkDMMLStorableNode") && !node->IsA("vtkDMMLCameraNode")) )
    {
    includeInView = false;
    }

  // check for scene view hierarchy nodes
  else if (node->IsA("vtkDMMLHierarchyNode"))
    {
    ...
    else if (node->GetID() && !strncmp(node->GetID(), "vtkDMMLHierarchyNodeSceneViewTopLevel", 37))
      {
      includeInView = false;
      }
    // backward compatibility: is it an old top level node?
      ...
      {
      vtkSmartPointer<vtkDMMLHierarchyNode> hnode = vtkDMMLHierarchyNode::SafeDownCast(node);
      if (hnode)
        {
        // backward compatibility: non tagged hierarchy nodes
        // check for hierarchy nodes that point to scene view nodes
        if (hnode->GetAssociatedNode() && hnode->GetAssociatedNode()->IsA("vtkDMMLSceneViewNode"))
          {
          includeInView = false;
          }
        // getting the node might fail if it's an erroneously included hierarchy
        // node in a scene view node being restored
        else if (hnode->GetAssociatedNodeID() && !strncmp(hnode->GetAssociatedNodeID(), "vtkDMMLSceneViewNode", 20))
          {
          includeInView = false;
          }
        }
...
void vtkDMMLSceneViewNode::SetSceneViewRootDir( const char* name)
{
  if (!this->SnapshotScene)
    {
    return;
