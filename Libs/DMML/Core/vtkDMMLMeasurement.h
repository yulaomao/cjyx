...
#ifndef __vtkDMMLMeasurement_h
#define __vtkDMMLMeasurement_h
...
/// \sa vtkCodedEntry
///
class VTK_DMML_EXPORT vtkDMMLMeasurement : public vtkObject
{
public:
  ...
  vtkTypeMacro(vtkDMMLMeasurement, vtkObject);
  void PrintSelf(ostream& os, vtkIndent indent) override;

  /// Create a new instance of this measurement type.
  VTK_NEWINSTANCE
  virtual vtkDMMLMeasurement* CreateInstance() const = 0;
  ...
  virtual void Copy(vtkDMMLMeasurement* source);

  /// Perform calculation on InputDMMLNode and store the result internally.
  ...
  /// If QuantityName is specified then a valid InputDMMLNode must be set, too, because the scene is accessed
  /// via the InputDMMLNode.
  ...
  /// Set input DMML node used for calculating the measurement \sa Execute
  void SetInputDMMLNode(vtkDMMLNode* node);
  /// Get input DMML node used for calculating the measurement \sa Execute
  vtkDMMLNode* GetInputDMMLNode();

protected:
  vtkDMMLMeasurement();
  ~vtkDMMLMeasurement() override;
  vtkDMMLMeasurement(const vtkDMMLMeasurement&);
  void operator=(const vtkDMMLMeasurement&);

  /// Helper function to get unit node from the scene based on quantity name.
  vtkDMMLUnitNode* GetUnitNode(const char* quantityName);
...
protected:
  ...
  /// DMML node used to calculate the measurement \sa Execute
  vtkWeakPointer<vtkDMMLNode> InputDMMLNode;
};
