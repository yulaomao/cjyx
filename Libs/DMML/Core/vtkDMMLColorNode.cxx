...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLColorNode.cxx,v $
...
//------------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLColorNode);

//----------------------------------------------------------------------------
vtkDMMLColorNode::vtkDMMLColorNode()
{
  this->SetName("");
...
//----------------------------------------------------------------------------
void vtkDMMLColorNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLColorNode::ReadXMLAttributes(const char** atts)
{
...
//----------------------------------------------------------------------------
vtkLookupTable * vtkDMMLColorNode::GetLookupTable()
{
...
//----------------------------------------------------------------------------
vtkScalarsToColors * vtkDMMLColorNode::GetScalarsToColors()
{
...
void vtkDMMLColorNode::Copy(vtkDMMLNode *anode)
{
  int disabledModify = this->StartModify();

  Superclass::Copy(anode);
  vtkDMMLColorNode *node = (vtkDMMLColorNode *) anode;
...
//----------------------------------------------------------------------------
void vtkDMMLColorNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkDMMLColorNode::UpdateScene(vtkDMMLScene *scene)
{
...
//---------------------------------------------------------------------------
void vtkDMMLColorNode::ProcessDMMLEvents ( vtkObject *caller,
                                           unsigned long event,
                                           void *callData )
{
  Superclass::ProcessDMMLEvents(caller, event, callData);
/*
  vtkDMMLColorDisplayNode *dnode = this->GetDisplayNode();
  if (dnode != nullptr && dnode == vtkDMMLColorDisplayNode::SafeDownCast(caller) &&
      event ==  vtkCommand::ModifiedEvent)
    {
        this->InvokeEvent(vtkDMMLColorNode::DisplayModifiedEvent, nullptr);
    }
*/
  return;
}
...
//---------------------------------------------------------------------------
const char * vtkDMMLColorNode::GetTypeAsString()
{
...
//---------------------------------------------------------------------------
void vtkDMMLColorNode::SetTypeToUser()
{
  this->SetType(this->User);
}

//---------------------------------------------------------------------------
void vtkDMMLColorNode::SetTypeToFile()
{
  this->SetType(this->File);
}

//---------------------------------------------------------------------------
void vtkDMMLColorNode::SetType(int type)
{
  if (this->Type == type)
    {
  ...
  this->InvokeEvent(vtkDMMLColorNode::TypeModifiedEvent);
}

//---------------------------------------------------------------------------
void vtkDMMLColorNode::SetNamesFromColors()
{
...
//---------------------------------------------------------------------------
bool vtkDMMLColorNode::SetNameFromColor(int index)
{
...
//---------------------------------------------------------------------------
bool vtkDMMLColorNode::HasNameFromColor(int index)
{
...
//---------------------------------------------------------------------------
const char *vtkDMMLColorNode::GetColorName(int ind)
{
  ...
  if (ind < 0 || ind >= (int)this->Names.size())
    {
    vtkDebugMacro("vtkDMMLColorNode::GetColorName: index " << ind << " is out of range 0 - " << this->Names.size());
    return "invalid";
    }
...
//---------------------------------------------------------------------------
int vtkDMMLColorNode::GetColorIndexByName(const char *name)
{
  if (name == nullptr)
    {
    vtkErrorMacro("vtkDMMLColorNode::GetColorIndexByName: need a non-null name as argument");
    return -1;
    }
...
//---------------------------------------------------------------------------
std::string vtkDMMLColorNode::GetColorNameWithoutSpaces(int ind, const char *subst)
{
...
//---------------------------------------------------------------------------
std::string vtkDMMLColorNode::GetColorNameAsFileName(int colorIndex, const char *subst)
{
...
//---------------------------------------------------------------------------
int vtkDMMLColorNode::SetColorName(int ind, const char *name)
{
...
//---------------------------------------------------------------------------
int vtkDMMLColorNode::SetColorNameWithSpaces(int ind, const char *name, const char *subst)
{
...
//---------------------------------------------------------------------------
int vtkDMMLColorNode::GetNumberOfColors()
{
  return static_cast<int>(this->Names.size());
}

//---------------------------------------------------------------------------
bool vtkDMMLColorNode::GetColor(int vtkNotUsed(index), double vtkNotUsed(color)[4])
{
  return false;
}

//---------------------------------------------------------------------------
void vtkDMMLColorNode::Reset(vtkDMMLNode* vtkNotUsed(defaultNode))
{
  // don't need to call reset on color nodes, as all but the User color table
  // node are static, and that's taken care of in the vtkDMMLColorTableNode
  //Superclass::Reset(defaultNode);
}

//---------------------------------------------------------------------------
bool vtkDMMLColorNode::GetModifiedSinceRead()
{
...
//---------------------------------------------------------------------------
vtkLookupTable* vtkDMMLColorNode::CreateLookupTableCopy()
{
