/*=auto=========================================================================
  ...
  Program:   3D Cjyx
  Module:    $RCSfile: vtkDMMLNode.h,v $
  Date:      $Date: 2006/03/19 17:12:29 $
  Version:   $Revision: 1.18 $

=========================================================================auto=*/

#ifndef __vtkDMMLNode_h
#define __vtkDMMLNode_h
...
#ifndef vtkSetDMMLObjectMacro
#define vtkSetDMMLObjectMacro(node,value)  {this->DMMLObserverManager->SetObject ( vtkObjectPointer( &(node)), (value) );};
#endif

/// Can be used to set any vtkObject, not only vtkDMMLNodes.
#ifndef vtkSetAndObserveDMMLObjectMacro
#define vtkSetAndObserveDMMLObjectMacro(node,value)  {this->DMMLObserverManager->SetAndObserveObject ( vtkObjectPointer( &(node)), (value) );};
#endif

#ifndef vtkSetAndObserveDMMLObjectMacroNoWarning
#define vtkSetAndObserveDMMLObjectMacroNoWarning(node,value)  {this->DMMLObserverManager->SetAndObserveObject ( vtkObjectPointer( &(node)), (value), 0.0, false /* no warning */ );};
#endif

#ifndef vtkSetAndObserveDMMLObjectEventsMacro
#define vtkSetAndObserveDMMLObjectEventsMacro(node,value,events)  {this->DMMLObserverManager->SetAndObserveObjectEvents ( vtkObjectPointer( &(node)), (value), (events));};
#endif

#ifndef vtkSetAndObserveDMMLObjectEventsMacroNoWarning
#define vtkSetAndObserveDMMLObjectEventsMacroNoWarning(node,value,events)  {this->DMMLObserverManager->SetAndObserveObjectEvents ( vtkObjectPointer( &(node)), (value), (events), nullptr /* priorities */, false /* no warning */);};
#endif

#ifndef vtkObserveDMMLObjectMacro
#define vtkObserveDMMLObjectMacro(node)  {this->DMMLObserverManager->ObserveObject ( (node) );};
#endif

#ifndef vtkObserveDMMLObjectEventsMacro
#define vtkObserveDMMLObjectEventsMacro(node, events)  {this->DMMLObserverManager->AddObjectEvents ( (node), (events) );};
#endif

#ifndef vtkUnObserveDMMLObjectMacro
#define vtkUnObserveDMMLObjectMacro(node)  {this->DMMLObserverManager->RemoveObjectEvents ( (node) );};
#endif
...
#ifndef vtkDMMLNodeNewMacro
#define vtkDMMLNodeNewMacro(newClass) \
  vtkStandardNewMacro(newClass); \
  vtkDMMLNode* newClass::CreateNodeInstance() \
  { \
    return newClass::New(); \
  }
#endif
...
//
#ifndef vtkDMMLCopyContentMacro
#define vtkDMMLCopyContentMacro(thisClassName) \
    void CopyContent(vtkDMMLNode* node, bool deepCopy=true) override; \
...
#ifndef vtkDMMLCopyContentDefaultMacro
#define vtkDMMLCopyContentDefaultMacro(thisClassName) \
    bool HasCopyContent() const override \
    { \
    return strcmp(#thisClassName, this->GetClassNameInternal()) == 0; \
    }
...
/// \brief Abstract Superclass for all specific types of DMML nodes.
///
/// This node encapsulates the functionality common to all types of DMML nodes.
/// This includes member variables for ID, Description, and Options,
/// as well as member functions to Copy() and Write().
class VTK_DMML_EXPORT vtkDMMLNode : public vtkObject
{
  /// make the vtkDMMLScene a friend so that AddNodeNoNotify can call
  /// SetID, but that's the only class that is allowed to do so
    friend class vtkDMMLScene;
    friend class vtkDMMLSceneViewNode;

public:
  vtkTypeMacro(vtkDMMLNode,vtkObject);
  void PrintSelf(ostream& os, vtkIndent indent) override;

  /// \brief Create instance of the default node. Like New only virtual.
  ///
  /// \note Subclasses should implement this method
  virtual vtkDMMLNode* CreateNodeInstance() = 0;
  ...
  virtual void ProcessChildNode(vtkDMMLNode *){};
  ...
  virtual void UpdateScene(vtkDMMLScene *)
  {
  ...
  /// Subclasses should implement this method.
  /// Call this method in the subclass implementation.
  virtual void Copy(vtkDMMLNode *node);
  ...
  /// in all parent classes by adding vtkDMMLCopyContentMacro(ClassName) to the class headers.
  virtual void CopyContent(vtkDMMLNode* node, bool deepCopy=true);

  /// \brief Copy the references of the node into this.
  ///
  /// Existing references will be replaced if found in node, or removed if not
  /// in node.
  virtual void CopyReferences(vtkDMMLNode* node);
  ...
  /// manually **after** calling CopyWithScene(vtkDMMLNode*) using
  /// vtkDMMLScene::AddNode(vtkDMMLNode*).
  /// Only one vtkCommand::ModifiedEvent is invoked, after the copy is fully completed.
  ///
  /// \bug Calling vtkDMMLScene::AddNode(vtkDMMLNode*) **before**
  /// CopyWithScene(vtkDMMLNode*) is **NOT** supported, it will unsynchronize
  /// the node internal caches.
  /// See [#4078](https://github.com/Slicer/Slicer/issues/4078)
  ///
  /// \sa vtkDMMLScene::AddNode(vtkDMMLNode*)
  void CopyWithScene(vtkDMMLNode *node);
  ...
  /// \sa SetSelectable()
  /// \sa SetSingletonTag()
  virtual void Reset(vtkDMMLNode* defaultNode);
  ...
  /// vtkDMMLModelNode then it must be enabled for vtkDMMLModelDisplayNode
  /// and vtkDMMLModelStorageNode as well).
  vtkGetMacro(UndoEnabled, bool);
  vtkSetMacro(UndoEnabled, bool);
  vtkBooleanMacro(UndoEnabled, bool);

  /// Propagate events generated in dmml.
  virtual void ProcessDMMLEvents ( vtkObject *caller, unsigned long event, void *callData );
  ...
  vtkGetMacro(InDMMLCallbackFlag, int);
  void SetInDMMLCallbackFlag (int flag) {
    this->InDMMLCallbackFlag = flag;
  }
  ...
  /// Singleton, and with IDs set to vtkDMMLSelectionNodeSingleton and
  /// vtkDMMLInteractionNodeSingleton.
  ...
  /// of Editor and a node ID of vtkDMMLScriptedModuleNodeEditor.
  ...
  /// to an ID of vtkDMMLColorTableNodeSingletonNewName.
  /// The existing DMML nodes don't always use these conventions but are kept unchanged
  /// for backward compatibility.
  /// \sa vtkDMMLScene::BuildID
  ...
  virtual vtkDMMLScene* GetScene();

  /// \brief This method is for internal use only.
  /// Use AddNode method of vtkDMMLScene to add a node to the scene.
  ///
  /// Internally calls SetSceneReferences()
  /// \sa SetSceneReferences()
  virtual void SetScene(vtkDMMLScene* scene);
  ...
  /// vtkDMMLTransformableNode::TransformModifiedEvent must be observed as well.
  ...
  ///
  /// Important: attributes that vtkDMMLNode::ReadXMLAttributes method receives are
  ...
  /// Optionally a DMML attribute name for storing the reference in the dmml scene file can be specified.
  /// For example "displayNodeRef". If omitted the DMML attribute name will be the same as the role.
  ...
  /// If referenceRole is "unit/" and dmmlAttributeName is "UnitRef", then the generated
  /// DMML attribute names for a node reference role of "unit/length" will be "lengthUnitRef".
  ...
  /// \sa GetReferenceNodeFromDMMLAttributeName()
  void AddNodeReferenceRole(const char *referenceRole, const char *dmmlAttributeName=nullptr, vtkIntArray *events=nullptr);

  /// \brief Set a reference to a node with specified nodeID from this node for a specific \a referenceRole.
  vtkDMMLNode* SetNodeReferenceID(const char* referenceRole, const char* referencedNodeID);

  /// Convenience method that adds a \a referencedNodeID at the end of the list.
  vtkDMMLNode* AddNodeReferenceID(const char* referenceRole , const char* referencedNodeID);

  /// \brief Set a N-th reference from this node with specified
  /// \a referencedNodeID for a specific \a referenceRole.
  vtkDMMLNode* SetNthNodeReferenceID(const char* referenceRole, int n, const char* referencedNodeID);
  ...
  vtkDMMLNode* SetAndObserveNodeReferenceID(const char* referenceRole , const char* referencedNodeID, vtkIntArray *events=nullptr);

  /// \brief Add and observe a reference node from this node for a specific
  /// \a referenceRole.
  ///
  /// Observe Modified event by default, optionally takes array of events.
  vtkDMMLNode* AddAndObserveNodeReferenceID(const char* referenceRole , const char* referencedNodeID, vtkIntArray *events=nullptr);
  ...
  vtkDMMLNode* SetAndObserveNthNodeReferenceID(const char* referenceRole, int n, const char *referencedNodeID, vtkIntArray *events=nullptr);
  ...
  /// \brief Get referenced DMML node for a specific \a referenceRole.
  ///
  ...
  vtkDMMLNode* GetNthNodeReference(const char* referenceRole, int n);

  /// Utility function that returns the first referenced node.
  /// \sa GetNthNodeReference(int), GetNodeReferenceID()
  vtkDMMLNode* GetNodeReference(const char* referenceRole);
  ...
  void GetNodeReferences(const char* referenceRole, std::vector<vtkDMMLNode*> &nodes);
...
protected:
  ...
  class VTK_DMML_EXPORT vtkDMMLNodeReference : public vtkObject
  {
  public:
    vtkTypeMacro(vtkDMMLNodeReference,vtkObject);
    static vtkDMMLNodeReference *New();
  ...
  public:
    ...
    void SetReferencingNode(vtkDMMLNode* node);
    vtkDMMLNode* GetReferencingNode() const;

    void SetReferencedNode(vtkDMMLNode* node);
    vtkDMMLNode* GetReferencedNode() const;

  protected:
    vtkDMMLNodeReference();
    ~vtkDMMLNodeReference() override;

    vtkDMMLNodeReference(const vtkDMMLNodeReference&);
    void operator=(const vtkDMMLNodeReference&);

    /// Name of the reference role
    char*     ReferenceRole;

    /// Points to this DMML node (that added the reference)
    vtkWeakPointer<vtkDMMLNode> ReferencingNode;
    /// The referenced node that is actually observed now
    vtkSmartPointer<vtkDMMLNode> ReferencedNode;
  ...
  ~vtkDMMLNode() override;
  vtkDMMLNode(const vtkDMMLNode&);
  void operator=(const vtkDMMLNode&);

  /// a shared set of functions that call the
  /// virtual ProcessDMMLEvents
  static void DMMLCallback( vtkObject *caller,
  ...
  /// name found in a DMML scene file. Return 0 otherwise.
  ///
  /// \note AddNodeReference() must be called prior.
  ///
  /// \sa GetDMMLAttributeNameFromReferenceRole(), AddNodeReference(),
  /// ReadXMLAttributes(), WriteXML()
  virtual const char* GetReferenceRoleFromDMMLAttributeName(const char* attName);

  /// \brief Return the dmml attribute name (if found) associated with a reference
  /// role. Return 0 otherwise.
  ///
  /// \note AddNodeReference() must be called prior.
  ///
  /// \sa GetReferenceRoleFromDMMLAttributeName(), AddNodeReference(),
  /// ReadXMLAttributes(), WriteXML()
  virtual const char* GetDMMLAttributeNameFromReferenceRole(const char* refRole);
  ...
  vtkDMMLNode* UpdateNodeReferenceEventObserver(vtkDMMLNode *oldReferencedNode, vtkDMMLNode *newReferencedNode,
    vtkIntArray *newEvents, vtkDMMLNodeReference* referenceToIgnore);
  ...
  void GetUpdatedReferencedNodeEventList(int& oldReferencedNodeUseCount, int& newReferencedNodeUseCount,
    ...
    vtkDMMLNode* oldReferencedNode, vtkDMMLNode* newReferencedNode,
    vtkDMMLNodeReference* referenceToIgnore, vtkIntArray* newEvents);
  ...
  virtual void OnNodeReferenceAdded(vtkDMMLNodeReference *reference)
  {
    this->InvokeEvent(vtkDMMLNode::ReferenceAddedEvent, reference);
  }

  /// Called when a referenced node pointer is modified.
  virtual void OnNodeReferenceModified(vtkDMMLNodeReference *reference)
  {
    this->InvokeEvent(vtkDMMLNode::ReferenceModifiedEvent, reference);
  }

  /// Called when a referenced node pointer is removed (set to nullptr).
  virtual void OnNodeReferenceRemoved(vtkDMMLNodeReference *reference)
  {
    this->InvokeEvent(vtkDMMLNode::ReferenceRemovedEvent, reference);
  }
  ...
  /// Holders for DMML callbacks
  vtkCallbackCommand *DMMLCallbackCommand;
  ...
  vtkWeakPointer<vtkDMMLScene> Scene;
  ...
  vtkObserverManager *DMMLObserverManager;

  /// NodeReferences is a map that stores vector of references for each referenceRole,
  /// the referenceRole can be any unique string, for example "display", "transform" etc.
  typedef std::vector< vtkSmartPointer<vtkDMMLNodeReference> > NodeReferenceListType;
  typedef std::map< std::string, NodeReferenceListType > NodeReferencesType;
  NodeReferencesType NodeReferences;

  std::map< std::string, std::string> NodeReferenceDMMLAttributeNames;
...
/// \brief Safe replacement of DMML node start/end modify.
///
/// DMMLNodeModifyBlocker can be used wherever you would otherwise use
...
///
class VTK_DMML_EXPORT DMMLNodeModifyBlocker
{
public:
  vtkWeakPointer<vtkDMMLNode> Node;
  int WasModifying;
  DMMLNodeModifyBlocker(vtkDMMLNode* node)
  {
    this->Node = node;
    if (this->Node)
      {
