...
#ifndef __vtkDMMLMessageCollection_h
#define __vtkDMMLMessageCollection_h
...
/// vtkDMMLStorableNode.  A substantially similar vtkMessageCollection
/// is planned for vtkAddon.  When that is released, this will be
/// replaced by that.
class VTK_DMML_EXPORT vtkDMMLMessageCollection : public vtkObject
{
public:
  vtkTypeMacro(vtkDMMLMessageCollection,vtkObject);
  void PrintSelf(ostream& os, vtkIndent indent) override;
  static vtkDMMLMessageCollection* New();
  ...
  virtual void AddMessages(vtkDMMLMessageCollection* source, const std::string& prefix=std::string());

  /// Clear the message vector.
  virtual void ClearMessages();

  /// Copy all messages from another collection.
  virtual void DeepCopy(vtkDMMLMessageCollection* source);
...
protected:
  ...
  ~vtkDMMLMessageCollection() override;
  vtkDMMLMessageCollection(const vtkDMMLMessageCollection&);
  void operator=(const vtkDMMLMessageCollection&);

  ///
  /// A helper class that describes a single message
  struct VTK_DMML_EXPORT Message
    {
