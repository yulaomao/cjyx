...
//------------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLTableNode);

//----------------------------------------------------------------------------
vtkDMMLTableNode::vtkDMMLTableNode()
{
  this->Table = nullptr;
  this->Schema = nullptr;
  this->Locked = false;
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::ReadXMLAttributes(const char** atts)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::CopyContent(vtkDMMLNode* anode, bool deepCopy/*=true*/)
{
  DMMLNodeModifyBlocker blocker(this);
  Superclass::CopyContent(anode, deepCopy);

  vtkDMMLTableNode* node = vtkDMMLTableNode::SafeDownCast(anode);
  if (!node)
    {
    return;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::ProcessDMMLEvents( vtkObject *caller, unsigned long event, void *callData )
{
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//---------------------------------------------------------------------------
vtkDMMLStorageNode* vtkDMMLTableNode::CreateDefaultStorageNode()
{
  ...
  return vtkDMMLStorageNode::SafeDownCast(
    scene->CreateNodeByClass("vtkDMMLTableStorageNode"));
}

//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetAndObserveTable(vtkTable* table)
{
  if (table==this->Table)
    {
    return;
    }
  vtkSetAndObserveDMMLObjectMacro(this->Table, table);
  this->StorableModifiedTime.Modified();
  this->Modified();
}

//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetAndObserveSchema(vtkTable* schema)
{
  if (schema == this->Schema)
    {
    return;
    }
  vtkSetAndObserveDMMLObjectMacro(this->Schema, schema);
  this->StorableModifiedTime.Modified();
  this->Modified();
}

//----------------------------------------------------------------------------
vtkAbstractArray* vtkDMMLTableNode::AddColumn(vtkAbstractArray* column)
{
  ...
  if (!this->Table)
    ...
    if (!this->Table)
      {
      vtkErrorMacro("vtkDMMLTableNode::AddColumn failed: failed to add VTK table");
      return nullptr;
      }
    }
...
//----------------------------------------------------------------------------
int vtkDMMLTableNode::GetColumnIndex(const char* columnName)
{
  if (!this->Table)
    {
    return -1;
...
//----------------------------------------------------------------------------
int vtkDMMLTableNode::GetColumnIndex(const std::string &columnName)
{
  return this->GetColumnIndex(columnName.c_str());
}

//----------------------------------------------------------------------------
int vtkDMMLTableNode::GetColumnIndex(vtkAbstractArray* column)
{
  if (column == nullptr)
    {
    return -1;
...
//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetColumnName(int columnIndex)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::CopyAllColumnProperties(const std::string& sourceColumnName, const std::string& targetColumnName)
{
  if (this->Schema == nullptr)
    {
    vtkWarningMacro("vtkDMMLTableNode::CopyAllColumnProperties failed: schema is empty");
    return;
    }
  ...
  if (rowIndexTarget < 0)
    ...
    if (nameColumn == nullptr)
      {
      vtkErrorMacro("vtkDMMLTableNode::CopyAllColumnProperties failed: schema is invalid");
      return;
      }
...
//----------------------------------------------------------------------------
bool vtkDMMLTableNode::RenameColumn(int columnIndex, const char* newNamePtr)
{
  std::string newName = (newNamePtr ? newNamePtr : "");
  if (newName.empty())
    {
    vtkErrorMacro("vtkDMMLTableNode::RenameColumn failed: invalid new name");
    return false;
    }
  if (!this->Table)
    {
    vtkErrorMacro("vtkDMMLTableNode::RenameColumn failed: invalid table");
    return false;
    }
  if (columnIndex<0 || columnIndex >= this->Table->GetNumberOfColumns())
    {
    vtkErrorMacro("vtkDMMLTableNode::RenameColumn failed: invalid column index: " << columnIndex);
    return false;
    }
...
//----------------------------------------------------------------------------
bool vtkDMMLTableNode::RemoveColumn(int columnIndex)
{
  if (!this->Table)
    {
    vtkErrorMacro("vtkDMMLTableNode::RemoveColumn failed: invalid table");
    return false;
    }
  if (columnIndex<0 || columnIndex>=this->Table->GetNumberOfColumns())
    {
    vtkErrorMacro("vtkDMMLTableNode::RemoveColumn failed: invalid column index: "<<columnIndex);
    return false;
    }
...
//----------------------------------------------------------------------------
bool vtkDMMLTableNode::RemoveAllColumns()
{
  if (!this->Table)
    {
    vtkErrorMacro("vtkDMMLTableNode::RemoveAllColumns failed: invalid table");
    return false;
    }
  bool wasModified = this->StartModify();
  this->SetAndObserveSchema(nullptr);
...
//----------------------------------------------------------------------------
int vtkDMMLTableNode::AddEmptyRow()
{
  if (!this->Table)
    {
    vtkErrorMacro("vtkDMMLTableNode::AddEmptyRow failed: invalid table");
    return -1;
    }
  int tableWasModified = this->StartModify();
  if (this->Table->GetNumberOfColumns()==0)
    {
    vtkDebugMacro("vtkDMMLTableNode::AddEmptyRow called for an empty table. Add an empty column first.");
    this->AddColumn();
    }
...
//----------------------------------------------------------------------------
bool vtkDMMLTableNode::RemoveRow(int rowIndex)
{
  if (!this->Table)
    {
    vtkErrorMacro("vtkDMMLTableNode::RemoveRow failed: invalid table");
    return false;
    }
  if (this->Table->GetNumberOfColumns()==0)
    {
    vtkErrorMacro("vtkDMMLTableNode::RemoveRow failed: no columns are defined");
    return false;
    }
    if (rowIndex<0 || rowIndex>=this->Table->GetNumberOfRows())
    {
    vtkErrorMacro("vtkDMMLTableNode::RemoveRow failed: invalid row index: "<<rowIndex);
    return false;
    }
...
//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetCellText(int rowIndex, int columnIndex)
{
  if (!this->Table)
    {
    vtkErrorMacro("vtkDMMLTableNode::GetCellText failed: invalid table");
    return "";
    }
  if (columnIndex<0 || columnIndex>=this->Table->GetNumberOfColumns())
    {
    vtkErrorMacro("vtkDMMLTableNode::GetCellText failed: invalid column index "<<columnIndex);
    return "";
    }
  if (rowIndex<0 || rowIndex>=this->Table->GetNumberOfRows())
    {
    vtkErrorMacro("vtkDMMLTableNode::GetCellText failed: invalid row index: "<<rowIndex);
    return "";
    }
...
//----------------------------------------------------------------------------
bool vtkDMMLTableNode::SetCellText(int rowIndex, int columnIndex, const char* text)
{
  if (!this->Table)
    {
    vtkErrorMacro("vtkDMMLTableNode::SetCellText failed: invalid table");
    return false;
    }
  if (columnIndex<0 || columnIndex>=this->Table->GetNumberOfColumns())
    {
    vtkErrorMacro("vtkDMMLTableNode::SetCellText failed: invalid column index "<<columnIndex);
    return false;
    }
  if (rowIndex<0 || rowIndex>=this->Table->GetNumberOfRows())
    {
    vtkErrorMacro("vtkDMMLTableNode::SetCellText failed: invalid row index: "<<rowIndex);
    return false;
    }
...
//----------------------------------------------------------------------------
int vtkDMMLTableNode::GetNumberOfRows()
{
  if (!this->Table)
    {
    return 0;
...
//----------------------------------------------------------------------------
int vtkDMMLTableNode::GetNumberOfColumns()
{
  if (!this->Table)
    {
    return 0;
...
//----------------------------------------------------------------------------
vtkIdType vtkDMMLTableNode::GetPropertyRowIndex(const std::string& columnName)
{
  if (this->Schema == nullptr)
    {
    return -1;
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::GetAllColumnPropertyNames(vtkStringArray* propertyNames)
{
  if (propertyNames == nullptr)
    {
    vtkErrorMacro("vtkDMMLTableNode::GetAllColumnPropertyNames failed: invalid propertyNames");
    return;
    }
...
//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetColumnProperty(int columnIndex, const std::string& propertyName)
{
  if (this->Table == nullptr)
    {
    return "";
...
//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetColumnProperty(const std::string& columnName, const std::string& propertyName)
{
  if (propertyName == SCHEMA_COLUMN_NAME)
    {
    vtkErrorMacro("vtkDMMLTableNode::GetColumnProperty failed: reserved propertyName: " << propertyName);
    return "";
    }
  if (propertyName == SCHEMA_COLUMN_TYPE)
    {
    return vtkDMMLTableNode::GetValueTypeAsString(this->GetColumnType(columnName));
    }
  if (propertyName == SCHEMA_COMPONENT_NAMES)
    {
    return vtkDMMLTableNode::GetComponentNamesAsString(this->GetComponentNames(columnName));
    }
...
//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetColumnPropertyInternal(const std::string& columnName, const std::string& propertyName)
{
  if (this->Schema == nullptr)
    {
    return "";
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetColumnProperty(int columnIndex, const std::string& propertyName, const std::string& propertyValue)
{
  if (this->Table == nullptr)
    {
    return;
  ...
  if (columnName == nullptr)
    {
    vtkErrorMacro("vtkDMMLTableNode::SetColumnProperty failed: invalid column index");
    return;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetColumnProperty(const std::string& columnName, const std::string& propertyName, const std::string& propertyValue)
{
  if (propertyName == SCHEMA_COLUMN_NAME)
    {
    vtkErrorMacro("vtkDMMLTableNode::SetColumnProperty failed: reserved propertyName: " << propertyName);
    return;
    }
  if (propertyName == SCHEMA_COLUMN_TYPE)
    {
    this->SetColumnType(columnName, vtkDMMLTableNode::GetValueTypeFromString(propertyValue));
    return;
    }
  if (propertyName == SCHEMA_COMPONENT_NAMES)
    {
  ...
  if (propertyName == COMPONENT_COUNT_PROPERTY)
    {
    vtkErrorMacro("vtkDMMLTableNode::SetColumnProperty failed : property is read-only " << COMPONENT_COUNT_PROPERTY);
    return;
    }

  if (propertyName.empty())
    {
    vtkErrorMacro("vtkDMMLTableNode::SetColumnProperty failed: property name is invalid");
    return;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetColumnPropertyInternal(const std::string& columnName, const std::string& propertyName, const std::string& propertyValue)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::RemoveColumnProperty(const std::string& propertyName)
{
  if (propertyName == SCHEMA_COLUMN_NAME || propertyName == SCHEMA_COLUMN_TYPE
    ...
    {
    vtkErrorMacro("vtkDMMLTableNode::RemoveColumnProperty failed: reserved propertyName: " << propertyName);
    return;
    }
  if (this->Schema == nullptr)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::RemoveAllColumnProperties(int columnIndex)
{
  if (this->Table == nullptr)
    {
    return;
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::RemoveAllColumnProperties(const std::string& columnName)
{
  if (this->Schema == nullptr)
    {
    return;
...
//----------------------------------------------------------------------------
int vtkDMMLTableNode::GetColumnValueTypeFromSchema(const std::string& columnName)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetColumnLongName(const std::string& columnName, const std::string& longName)
{
  this->SetColumnProperty(columnName, SCHEMA_COLUMN_LONG_NAME, longName);
}

//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetColumnLongName(const std::string& columnName)
{
  return this->GetColumnProperty(columnName, SCHEMA_COLUMN_LONG_NAME);
}

//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetColumnDescription(const std::string& columnName, const std::string& description)
{
  this->SetColumnProperty(columnName, SCHEMA_COLUMN_DESCRIPTION, description);
}

//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetColumnDescription(const std::string& columnName)
{
  return this->GetColumnProperty(columnName, SCHEMA_COLUMN_DESCRIPTION);
}

//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetColumnNullValue(const std::string& columnName, const std::string& description)
{
  this->SetColumnProperty(columnName, SCHEMA_COLUMN_NULL_VALUE, description);
}

//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetColumnNullValue(const std::string& columnName)
{
  return this->GetColumnProperty(columnName, SCHEMA_COLUMN_NULL_VALUE);
}

//----------------------------------------------------------------------------
void vtkDMMLTableNode::SetColumnUnitLabel(const std::string& columnName, const std::string& unitLabel)
{
  this->SetColumnProperty(columnName, SCHEMA_COLUMN_UNIT_LABEL, unitLabel);
}

//----------------------------------------------------------------------------
std::string  vtkDMMLTableNode::GetColumnUnitLabel(const std::string& columnName)
{
  return this->GetColumnProperty(columnName, SCHEMA_COLUMN_UNIT_LABEL);
}

//----------------------------------------------------------------------------
int vtkDMMLTableNode::GetValueTypeFromString(std::string valueTypeStr)
{
  ...
  for (int typeId = 0; typeId < MAX_TYPE_ID; typeId++)
    {
    if (valueTypeStr == vtkDMMLTableNode::GetValueTypeAsString(typeId))
      {
      return typeId;
      }
    }
...
//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetValueTypeAsString(int valueType)
{
  return vtkImageScalarTypeNameMacro(valueType);
}

//----------------------------------------------------------------------------
vtkIdType vtkDMMLTableNode::InsertNextBlankRowWithNullValues(vtkTable* table)
{
  if (table == nullptr)
    {
    vtkErrorMacro("vtkDMMLTableNode::InsertNextBlankRowWithNullValues failed: invalid table");
    return -1;
    }
...
//----------------------------------------------------------------------------
const char* vtkDMMLTableNode::GetDefaultColumnName()
{
  return SCHEMA_DEFAULT_COLUMN_NAME;
}

//----------------------------------------------------------------------------
bool vtkDMMLTableNode::SetDefaultColumnType(const std::string& type, const std::string& nullValue /* ="" */)
{
  int valueType = this->GetValueTypeFromString(type);
  if (valueType == VTK_VOID)
    {
    vtkErrorMacro("vtkDMMLTableNode::SetDefaultColumnType failed: Unknown column value type: " << type
...
//----------------------------------------------------------------------------
bool vtkDMMLTableNode::SetColumnType(const std::string& columnName, int valueType)
{
  ...
  if (columnIndex < 0)
    ...
    if (columnName != SCHEMA_DEFAULT_COLUMN_NAME)
      {
      vtkErrorMacro("vtkDMMLTableNode::SetColumnType failed: column by this name does not exist: " << columnName);
      return false;
      }
    this->SetColumnPropertyInternal(SCHEMA_DEFAULT_COLUMN_NAME, SCHEMA_COLUMN_TYPE, vtkDMMLTableNode::GetValueTypeAsString(valueType));
    return true;
    }
...
//----------------------------------------------------------------------------
int vtkDMMLTableNode::GetColumnType(const std::string& columnName)
{
  if (columnName == SCHEMA_DEFAULT_COLUMN_NAME)
    {
    int valueTypeId = this->GetColumnValueTypeFromSchema(SCHEMA_DEFAULT_COLUMN_NAME);
...
//----------------------------------------------------------------------------
const std::vector<std::string> vtkDMMLTableNode::GetComponentNamesFromString(const std::string& componentNamesString)
{
...
//----------------------------------------------------------------------------
std::string vtkDMMLTableNode::GetComponentNamesAsString(const std::vector<std::string>& componentNames)
{
  std::stringstream ss;
...
//----------------------------------------------------------------------------
bool vtkDMMLTableNode::SetComponentNames(const std::string& columnName, const std::vector<std::string>& componentNames)
{
...
//----------------------------------------------------------------------------
std::vector<std::string> vtkDMMLTableNode::GetComponentNames(const std::string& columnName)
{
  vtkAbstractArray* column = nullptr;
  if (this->Table && columnName != SCHEMA_DEFAULT_COLUMN_NAME)
    {
  ...
  return vtkDMMLTableNode::GetComponentNamesFromArray(column);
}

//----------------------------------------------------------------------------
std::vector<std::string> vtkDMMLTableNode::GetComponentNamesFromSchema(const std::string& columnName)
{
  return this->GetComponentNamesFromString(this->GetColumnPropertyInternal(columnName, SCHEMA_COLUMN_TYPE));
}

//----------------------------------------------------------------------------
std::vector<std::string> vtkDMMLTableNode::GetComponentNamesFromArray(vtkAbstractArray* array)
{
  std::vector<std::string> componentNames;
  if (!array)
    {
