...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLTransformStorageNode.cxx,v $
...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLTransformStorageNode);

bool vtkDMMLTransformStorageNode::RegisterInverseTransformTypesCompleted = false;

//----------------------------------------------------------------------------
vtkDMMLTransformStorageNode::vtkDMMLTransformStorageNode()
{
  ...
  // are instantiated.
  if (!vtkDMMLTransformStorageNode::RegisterInverseTransformTypesCompleted)
    {
    vtkITKTransformConverter::RegisterInverseTransformTypes();
    vtkDMMLTransformStorageNode::RegisterInverseTransformTypesCompleted = true;
    }
}

//----------------------------------------------------------------------------
vtkDMMLTransformStorageNode::~vtkDMMLTransformStorageNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLTransformStorageNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLTransformStorageNode::ReadXMLAttributes(const char** atts)
{
...
void vtkDMMLTransformStorageNode::Copy(vtkDMMLNode *anode)
{
  if (!anode)
    {
    return;
  ...
  vtkDMMLTransformStorageNode *node = vtkDMMLTransformStorageNode::SafeDownCast(anode);
...
//----------------------------------------------------------------------------
void vtkDMMLTransformStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
bool vtkDMMLTransformStorageNode::CanReadInReferenceNode(vtkDMMLNode *refNode)
{
  return refNode->IsA("vtkDMMLTransformNode");
}

//----------------------------------------------------------------------------
int vtkDMMLTransformStorageNode::ReadFromITKv3BSplineTransformFile(vtkDMMLNode *refNode)
{
  ...
  vtkDMMLTransformNode *transformNode = vtkDMMLTransformNode::SafeDownCast(refNode);
  if (transformNode==nullptr)
    {
    vtkErrorMacro("vtkDMMLTransformStorageNode::ReadFromITKv3BSplineTransformFile failed: expected a transform node as input");
    return 0;
    }
  ...
  if (transformNode->GetReadAsTransformToParent())
    ...
    // transform to a Cjyx modeling transform)
    bsplineVtk->Inverse();
...
//----------------------------------------------------------------------------
int vtkDMMLTransformStorageNode::ReadFromImageFile(vtkDMMLNode *refNode)
{
  vtkDMMLTransformNode *tn = vtkDMMLTransformNode::SafeDownCast(refNode);
  if (tn==nullptr)
    {
    vtkErrorMacro("vtkDMMLTransformStorageNode::ReadGridTransform failed: expected a transform node as input");
    return 0;
    }
...
//----------------------------------------------------------------------------
int vtkDMMLTransformStorageNode::ReadFromTransformFile(vtkDMMLNode *refNode)
{
  vtkDMMLTransformNode *transformNode = vtkDMMLTransformNode::SafeDownCast(refNode);
  if (transformNode == nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLTransformStorageNode::ReadDataInternal(vtkDMMLNode *refNode)
{
  std::string fullName =  this->GetFullNameFromFileName();
  if (fullName.empty())
    {
...
//----------------------------------------------------------------------------
int vtkDMMLTransformStorageNode::WriteToTransformFile(vtkDMMLNode *refNode)
{
  vtkDMMLTransformNode *transformNode = vtkDMMLTransformNode::SafeDownCast(refNode);
  if (transformNode == nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLTransformStorageNode::WriteToImageFile(vtkDMMLNode *refNode)
{
  vtkDMMLTransformNode *transformNode = vtkDMMLTransformNode::SafeDownCast(refNode);
  if (transformNode == nullptr)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLTransformStorageNode::WriteDataInternal(vtkDMMLNode *refNode)
{
  std::string fullName =  this->GetFullNameFromFileName();
  if (fullName.empty())
    {
    vtkErrorMacro("vtkDMMLTransformNode write data failed: file name not specified");
    return 0;
    }
  vtkDMMLTransformNode* transformNode = vtkDMMLTransformNode::SafeDownCast(refNode);
  if (transformNode == nullptr)
    {
    vtkErrorMacro("vtkDMMLTransformNode write data failed: invalid transform node");
    return 0;
    }
  if (IsImageFile(fullName))
    {
...
//----------------------------------------------------------------------------
void vtkDMMLTransformStorageNode::InitializeSupportedWriteFileTypes()
{
...
//----------------------------------------------------------------------------
bool vtkDMMLTransformStorageNode::IsImageFile(const std::string &filename)
{
  // determine file type
...
//----------------------------------------------------------------------------
void vtkDMMLTransformStorageNode::SetAndObserveTransformFromParentAutoInvert(vtkDMMLTransformNode* transformNode, vtkAbstractTransform *transform)
{
  ...
  vtkDMMLTransformNode::FlattenGeneralTransform(sourceTransformList.GetPointer(), transform);
