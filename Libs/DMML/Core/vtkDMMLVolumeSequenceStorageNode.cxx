...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLVolumeSequenceStorageNode);

//----------------------------------------------------------------------------
vtkDMMLVolumeSequenceStorageNode::vtkDMMLVolumeSequenceStorageNode() = default;

//----------------------------------------------------------------------------
vtkDMMLVolumeSequenceStorageNode::~vtkDMMLVolumeSequenceStorageNode() = default;

//----------------------------------------------------------------------------
bool vtkDMMLVolumeSequenceStorageNode::CanReadInReferenceNode(vtkDMMLNode *refNode)
{
  return refNode->IsA("vtkDMMLSequenceNode");
}

//----------------------------------------------------------------------------
int vtkDMMLVolumeSequenceStorageNode::ReadDataInternal(vtkDMMLNode* refNode)
{
  ...
  vtkDMMLSequenceNode* volSequenceNode = dynamic_cast<vtkDMMLSequenceNode*>(refNode);
  if (!volSequenceNode)
    {
    vtkErrorMacro(<< "vtkDMMLVolumeSequenceStorageNode::ReadDataInternal : not a Sequence node.");
    return 0;
    }
  ...
  if (fullName == std::string(""))
    {
    vtkErrorMacro(<< "vtkDMMLVolumeSequenceStorageNode::ReadDataInternal : File name not specified");
    return 0;
    }
  ...
  if (!reader->CanReadFile(fullName.c_str()))
    {
    vtkDebugMacro("vtkDMMLVolumeSequenceStorageNode: This is not a nrrd file");
    return 0;
    }
  ...
  for ( KeyVector::iterator kit = keys.begin(); kit != keys.end(); ++kit)
    ...
    if (*kit == "axis 0 index type")
      ...
      for (std::istringstream indexValueList(reader->GetHeaderValue((*kit).c_str()));
        ...
        indexValues.push_back(vtkDMMLNode::URLDecodeString(indexValue.c_str()));
        }
#else
    if (*kit == "axis 0 index type")
      {
  ...
  if (!readAsMultipleImagesOn)
    ...
    if (imageData == nullptr || imageData->GetPointData()==nullptr || imageData->GetPointData()->GetScalars() == nullptr)
      {
      vtkErrorMacro("vtkDMMLVolumeSequenceStorageNode::ReadDataInternal: invalid image data");
      return 0;
      }
  ...
  vtkDebugMacro(<< " vtkDMMLVolumeSequenceStorageNode::ReadDataInternal: Starting reading sequence. ");
...
#endif
    // Cjyx expects normalized image position and spacing
    frameVoxels->SetOrigin(0, 0, 0);
    frameVoxels->SetSpacing(1, 1, 1);
    vtkNew<vtkDMMLScalarVolumeNode> frameVolume;
#ifdef NRRD_CHUNK_IO_AVAILABLE
  ...
  vtkDebugMacro(<< " vtkDMMLVolumeSequenceStorageNode::ReadDataInternal: sequence successfully read. ");
...
//----------------------------------------------------------------------------
bool vtkDMMLVolumeSequenceStorageNode::CanWriteFromReferenceNode(vtkDMMLNode *refNode)
{
  vtkDMMLSequenceNode* volSequenceNode = vtkDMMLSequenceNode::SafeDownCast(refNode);
  ...
  vtkDMMLVolumeNode* firstFrameVolume = vtkDMMLVolumeNode::SafeDownCast(volSequenceNode->GetNthDataNode(0));
  ...
  for (int frameIndex = 1; frameIndex<numberOfFrameVolumes; frameIndex++)
    {
    vtkDMMLVolumeNode* currentFrameVolume = vtkDMMLVolumeNode::SafeDownCast(volSequenceNode->GetNthDataNode(frameIndex));
    if (currentFrameVolume == nullptr)
      {
      vtkDebugMacro("vtkDMMLVolumeSequenceStorageNode::CanWriteFromReferenceNode: only volume nodes can be written (frame "<<frameIndex<<")");
    ...
    if (!vtkAddonMathUtilities::MatrixAreEqual(currentVolumeIjkToRas, firstVolumeIjkToRas))
      {
      vtkDebugMacro("vtkDMMLVolumeSequenceStorageNode::CanWriteFromReferenceNode: IJK to RAS matrix is not the same in all frames"
    ...
    for (int i = 0; i < 6; i++)
      ...
      if (firstFrameVolumeExtent[i] != currentFrameVolumeExtent[i])
        {
        vtkDebugMacro("vtkDMMLVolumeSequenceStorageNode::CanWriteFromReferenceNode: extent mismatch (frame " << frameIndex << ")");
    ...
    if (currentFrameVolumeScalarType != firstFrameVolumeScalarType)
      {
      vtkDebugMacro("vtkDMMLVolumeSequenceStorageNode::CanWriteFromReferenceNode: scalar type mismatch (frame " << frameIndex << ")");
    ...
    if (currentFrameVolumeNumberOfComponents != firstFrameVolumeNumberOfComponents)
      {
      vtkDebugMacro("vtkDMMLVolumeSequenceStorageNode::CanWriteFromReferenceNode: number of components mismatch (frame " << frameIndex << ")");
...
//----------------------------------------------------------------------------
int vtkDMMLVolumeSequenceStorageNode::WriteDataInternal(vtkDMMLNode *refNode)
{
  vtkDMMLSequenceNode* volSequenceNode = vtkDMMLSequenceNode::SafeDownCast(refNode);
  if (volSequenceNode == nullptr)
    {
    vtkDebugMacro(<< "vtkDMMLVolumeSequenceStorageNode::WriteDataInternal: Do not recognize node type " << refNode->GetClassName());
  ...
  if (numberOfFrameVolumes > 0)
    {
    vtkDMMLVolumeNode* frameVolume = vtkDMMLVolumeNode::SafeDownCast(volSequenceNode->GetNthDataNode(0));
  ...
  for (int frameIndex=0; frameIndex<numberOfFrameVolumes; frameIndex++)
    {
    vtkDMMLVolumeNode* frameVolume = vtkDMMLVolumeNode::SafeDownCast(volSequenceNode->GetNthDataNode(frameIndex));
    if (frameVolume==nullptr)
      {
      vtkDebugMacro(<< "vtkDMMLVolumeSequenceStorageNode::WriteDataInternal: Data node "<<frameIndex<<" is not a volume");
      this->GetUserMessages()->AddMessage(vtkCommand::ErrorEvent, std::string("Only volume sequence can be written in this format."));
      return 0;
      }
    vtkNew<vtkMatrix4x4> currentVolumeIjkToRas;
    ...
    || currentFrameVolumeScalarType != frameVolumeScalarType)
      {
      vtkDebugMacro(<< "vtkDMMLVolumeSequenceStorageNode::WriteDataInternal: Data node "<<frameIndex<<" size or scalar type mismatch ("
...
#if Cjyx_VERSION_MAJOR > 4 || (Cjyx_VERSION_MAJOR == 4 && Cjyx_VERSION_MINOR >= 9)
  ...
  if (numberOfFrameVolumes > 0)
    ...
    for (int frameIndex = 0; frameIndex < numberOfFrameVolumes; frameIndex++)
      ...
      // Encode string to make sure there are no spaces in the serialized index value (space is used as separator)
      ssIndexValues << vtkDMMLNode::URLEncodeString(volSequenceNode->GetNthIndexValue(frameIndex).c_str());
      }
    writer->SetAttribute(axisValues, ssIndexValues.str());
  }

  // pass down all DMML attributes to NRRD
  ...
  int writeFlag = 1;
  vtkDebugMacro(<< " vtkDMMLVolumeSequenceStorageNode::WriteDataInternal: Starting writing sequence. ");
  for (int frameIndex = 0; frameIndex < numberOfFrameVolumes; ++frameIndex)
    {
    ...
    if (!vtkAddonMathUtilities::MatrixAreEqual(currentVolumeIjkToRas, firstVolumeIjkToRas))
      {
      vtkDebugMacro("vtkDMMLVolumeSequenceStorageNode::vtkDMMLVolumeSequenceStorageNode: IJK to RAS matrix is not the same in all frames"
        << " (first frame: " << vtkAddonMathUtilities::ToString(firstVolumeIjkToRas)
        << ", frame " << frameIndex << ": " << vtkAddonMathUtilities::ToString(firstVolumeIjkToRas) << ")");
      this->GetUserMessages()->AddMessage(vtkCommand::ErrorEvent, std::string("Geometry of all volumes in the sequence must be the same."));
      return 0;
    ...
    if (frameVolume->GetImageData() == nullptr)
      {
      vtkDebugMacro(<< "vtkDMMLVolumeSequenceStorageNode::WriteDataInternal: "
  ...
  vtkDebugMacro(<< " vtkDMMLVolumeSequenceStorageNode::WriteDataInternal: sequence successfully written. ");
  return writeFlag;
}

//----------------------------------------------------------------------------
void vtkDMMLVolumeSequenceStorageNode::InitializeSupportedReadFileTypes()
{
...
//----------------------------------------------------------------------------
void vtkDMMLVolumeSequenceStorageNode::InitializeSupportedWriteFileTypes()
{
...
//----------------------------------------------------------------------------
const char* vtkDMMLVolumeSequenceStorageNode::GetDefaultWriteFileExtension()
{
  return "seq.nrrd";
}
