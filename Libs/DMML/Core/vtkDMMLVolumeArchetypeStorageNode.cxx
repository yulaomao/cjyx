...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLVolumeArchetypeStorageNode.cxx,v $
...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLVolumeArchetypeStorageNode);

int ConvertVoxelVectorTypeDMMLToVTKITK(int vtkitk)
{
  switch (vtkitk)
    {
    case vtkITKImageWriter::VoxelVectorTypeUndefined: return vtkDMMLVolumeNode::VoxelVectorTypeUndefined;
    case vtkITKImageWriter::VoxelVectorTypeSpatial: return vtkDMMLVolumeNode::VoxelVectorTypeSpatial;
    case vtkITKImageWriter::VoxelVectorTypeColorRGB: return vtkDMMLVolumeNode::VoxelVectorTypeColorRGB;
    case vtkITKImageWriter::VoxelVectorTypeColorRGBA: return vtkDMMLVolumeNode::VoxelVectorTypeColorRGBA;
    default:
      return vtkDMMLVolumeNode::VoxelVectorTypeUndefined;
    }
}

int ConvertVoxelVectorTypeVTKITKToDMML(int vtkitk)
{
  switch (vtkitk)
    {
    case vtkDMMLVolumeNode::VoxelVectorTypeUndefined: return vtkITKImageWriter::VoxelVectorTypeUndefined;
    case vtkDMMLVolumeNode::VoxelVectorTypeSpatial: return vtkITKImageWriter::VoxelVectorTypeSpatial;
    case vtkDMMLVolumeNode::VoxelVectorTypeColorRGB: return vtkITKImageWriter::VoxelVectorTypeColorRGB;
    case vtkDMMLVolumeNode::VoxelVectorTypeColorRGBA: return vtkITKImageWriter::VoxelVectorTypeColorRGBA;
    default:
      return vtkDMMLVolumeNode::VoxelVectorTypeUndefined;
    }
}
...
//----------------------------------------------------------------------------
vtkDMMLVolumeArchetypeStorageNode::vtkDMMLVolumeArchetypeStorageNode()
{
  this->CenterImage = 0;
  this->SingleFile  = 0;
...
//----------------------------------------------------------------------------
vtkDMMLVolumeArchetypeStorageNode::~vtkDMMLVolumeArchetypeStorageNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLVolumeArchetypeStorageNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLVolumeArchetypeStorageNode::ReadXMLAttributes(const char** atts)
{
...
void vtkDMMLVolumeArchetypeStorageNode::Copy(vtkDMMLNode *anode)
{
  int disabledModify = this->StartModify();

  Superclass::Copy(anode);
  vtkDMMLVolumeArchetypeStorageNode *node = (vtkDMMLVolumeArchetypeStorageNode *) anode;
...
//----------------------------------------------------------------------------
void vtkDMMLVolumeArchetypeStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
bool vtkDMMLVolumeArchetypeStorageNode::CanReadInReferenceNode(vtkDMMLNode *refNode)
{
  return refNode->IsA("vtkDMMLScalarVolumeNode") ||
         refNode->IsA("vtkDMMLVectorVolumeNode" );
}

//----------------------------------------------------------------------------
bool vtkDMMLVolumeArchetypeStorageNode::CanWriteFromReferenceNode(vtkDMMLNode *refNode)
{
  return refNode->IsA("vtkDMMLScalarVolumeNode");
}
...
vtkDMMLVolumeArchetypeStorageNode::InstantiateVectorVolumeReader(const std::string& fullName)
{
#ifdef DMML_USE_vtkTeem
  //
...
//----------------------------------------------------------------------------
void ApplyImageSeriesReaderWorkaround(vtkDMMLVolumeArchetypeStorageNode * storageNode,
                                      ...
                                      const std::string& fullName)
{
  ...
  //
  std::string fileExt=vtkDMMLStorageNode::GetLowercaseExtensionFromFileName(fullName);
...
//----------------------------------------------------------------------------
int vtkDMMLVolumeArchetypeStorageNode::ReadDataInternal(vtkDMMLNode *refNode)
{
  ...
  if (fullName.empty())
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLVolumeArchetypeStorageNode::ReadDataInternal",
  ...
  vtkDMMLScalarVolumeNode * volNode = vtkDMMLScalarVolumeNode::SafeDownCast(refNode);
  if (volNode == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLVolumeArchetypeStorageNode::ReadDataInternal",
      "Reference node is expected to be a vtkDMMLScalarVolumeNode");
    return 0;
    }

  vtkSmartPointer<vtkITKArchetypeImageSeriesReader> reader;

  if (refNode->IsA("vtkDMMLVectorVolumeNode"))
    {
    reader.TakeReference(this->InstantiateVectorVolumeReader(fullName));
    }
  else if (refNode->IsA("vtkDMMLDiffusionTensorVolumeNode"))
    {
  ...
  reader->AddObserver( vtkCommand::ProgressEvent,  this->DMMLCallbackCommand);
  ...
  if (volNode->IsA("vtkDMMLDiffusionTensorVolumeNode"))
    {
  ...
  if (!volNode->IsA("vtkDMMLVectorVolumeNode")
      && !volNode->IsA("vtkDMMLDiffusionTensorVolumeNode")
  ...
  vtkDMMLVolumeArchetypeStorageNode::SetMetaDataDictionaryFromReader(volNode, reader);
  ...
  int voxelVectorType = ConvertVoxelVectorTypeVTKITKToDMML(reader->GetVoxelVectorType());
  volNode->SetVoxelVectorType(voxelVectorType);

  // If voxel values store spatial vectors then we need to convert from LPS to RAS
  if (voxelVectorType == vtkDMMLVolumeNode::VoxelVectorTypeSpatial
    && outputImage->GetNumberOfScalarComponents() == 3)
    {
  ...
  if (volNode->IsA("vtkDMMLDiffusionTensorVolumeNode"))
    {
    vtkDMMLDiffusionTensorVolumeNode* dtvn = vtkDMMLDiffusionTensorVolumeNode::SafeDownCast(volNode);
...
//----------------------------------------------------------------------------
int vtkDMMLVolumeArchetypeStorageNode::WriteDataInternal(vtkDMMLNode *refNode)
{
  int result = 1;

  vtkDMMLVolumeNode *volNode = vtkDMMLVolumeNode::SafeDownCast(refNode);
  if (!volNode)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLVolumeArchetypeStorageNode::WriteDataInternal",
  ...
  if (volNode->GetVoxelVectorType() == vtkDMMLVolumeNode::VoxelVectorTypeSpatial)
    {
    if (volNode->GetImageData()->GetNumberOfScalarComponents() != 3)
      {
      vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLVolumeArchetypeStorageNode::WriteDataInternal",
  ...
  if (!moveSucceeded)
    ...
    writer->SetVoxelVectorType(ConvertVoxelVectorTypeDMMLToVTKITK(voxelVectorType));
    // If voxel values store spatial vectors then we need to convert from LPS to RAS
    bool writeVoxelValuesAsLps = (
      voxelVectorType == vtkDMMLVolumeNode::VoxelVectorTypeSpatial
...
//----------------------------------------------------------------------------
void vtkDMMLVolumeArchetypeStorageNode::InitializeSupportedWriteFileTypes()
{
...
//----------------------------------------------------------------------------
std::string vtkDMMLVolumeArchetypeStorageNode::UpdateFileList(vtkDMMLNode *refNode, bool move)
{
  // test whether refNode is a valid node to hold a volume
  if (!refNode->IsA("vtkDMMLScalarVolumeNode") )
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLVolumeArchetypeStorageNode::UpdateFileList",
      "Reference node is not a vtkDMMLVolumeNode");
    return "";
    }

  vtkDMMLVolumeNode *volNode = vtkDMMLScalarVolumeNode::SafeDownCast(refNode);
...
//  std::vector<std::string> pathComponents;
  ...
  */
  ...
  if (!vtksys::SystemTools::RemoveADirectory(tempDir.c_str()))
    {
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLVolumeArchetypeStorageNode::UpdateFileList",
...
//----------------------------------------------------------------------------
void vtkDMMLVolumeArchetypeStorageNode::ConfigureForDataExchange()
{
  this->UseCompressionOff();
}

//----------------------------------------------------------------------------
void vtkDMMLVolumeArchetypeStorageNode::SetMetaDataDictionaryFromReader(vtkDMMLVolumeNode *volNode, vtkITKArchetypeImageSeriesReader *reader)
{
  if (volNode && reader)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLVolumeArchetypeStorageNode::ConvertSpatialVectorVoxelsBetweenRasLps(vtkImageData* imageData)
{
  ...
  if (imageData->GetScalarType() == VTK_DOUBLE)
    ...
    {
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLVolumeArchetypeStorageNode::ConvertSpatialVectorVoxelsBetweenRasLps",
