...
//----------------------------------------------------------------------------
const vtkIdType vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID = 0;
const std::string vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_SEPARATOR = std::string("|");
const std::string vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_NAME_VALUE_SEPARATOR = std::string("^");
const std::string vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_VERSION_ATTRIBUTE_NAME = std::string("SubjectHierarchyVersion");

//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLSubjectHierarchyNode);
...
public:
  ...
  void WriteXML(ostream& of, int indent, vtkDMMLNode* xmlEncoderNode);
...
public:
  ...
  /// This number is used to reference to an item from outside the DMML node
  vtkIdType ID;

  /// Pointer to the data node associated to this subject hierarchy node
  vtkWeakPointer<vtkDMMLNode> DataNode;
  ...
  static std::map<vtkDMMLNode*, vtkSubjectHierarchyItem*> DataNodeCache;
...
public:
  ...
  /// \param dataNode Associated data DMML node. Name of the data node provides the name of the item
  /// \return ID of the item in the hierarchy that was assigned automatically when adding
  vtkIdType AddToTree(vtkSubjectHierarchyItem* parent, vtkDMMLNode* dataNode);

  /// Add hierarchy item to tree under parent, specifying basic properties
  /// \param parent Parent item pointer under which this item is inserted
  /// \param name Name of the item
  /// \param level Level string of the item (\sa vtkDMMLSubjectHierarchyConstants). It will be added as a special attribute
...
public:
  ...
  /// Find child by associated data DMML node
  /// \param dataNode Data DMML node to find
  /// \param recursive Flag whether to find only direct children (false) or in the whole branch (true). True by default
  /// \return Item if found, nullptr otherwise
  vtkSubjectHierarchyItem* FindChildByDataNode(vtkDMMLNode* dataNode, bool recursive=true);
...
public:
  ...
  ///   (e.g. value of vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelStudy()).
...
vtkIdType vtkSubjectHierarchyItem::NextSubjectHierarchyItemID = vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID + 1;

std::map<vtkIdType, vtkSubjectHierarchyItem*> vtkSubjectHierarchyItem::ItemCache = std::map<vtkIdType, vtkSubjectHierarchyItem*>();
std::map<vtkDMMLNode*, vtkSubjectHierarchyItem*> vtkSubjectHierarchyItem::DataNodeCache = std::map<vtkDMMLNode*, vtkSubjectHierarchyItem*>();
...
vtkSubjectHierarchyItem::vtkSubjectHierarchyItem()
  : ID(vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
  , DataNode(nullptr)
  , Name("")
  , OwnerPluginName("")
  , TemporaryID(vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
  , TemporaryDataNodeID("")
  , TemporaryParentItemID(vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
{
  this->Children.clear();
...
//---------------------------------------------------------------------------
vtkIdType vtkSubjectHierarchyItem::AddToTree(vtkSubjectHierarchyItem* parent, vtkDMMLNode* dataNode)
{
  ...
  this->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAddedEvent, this);
...
vtkIdType vtkSubjectHierarchyItem::AddToTree(vtkSubjectHierarchyItem* parent, std::string name, std::string level, int positionUnderParent/*=-1*/)
{
  ...
  this->Name = name;
  this->Attributes[vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName()] = level;
...
void vtkSubjectHierarchyItem::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Parent: "
    << (this->Parent ? this->Parent->ID : vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
...
void vtkSubjectHierarchyItem::ReadXMLAttributes(const char** atts)
{
  ...
  while (*atts != nullptr)
    ...
    if (!strcmp(attName, "id"))
      {
      this->ID = vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID;
    ...
    if (!strcmp(attName, "expanded"))
      ...
      this->UIDs.clear();
      size_t itemSeparatorPosition = valueStr.find(vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_SEPARATOR);
      while (itemSeparatorPosition != std::string::npos)
        {
        std::string itemStr = valueStr.substr(0, itemSeparatorPosition);
        size_t nameValueSeparatorPosition = itemStr.find(vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_NAME_VALUE_SEPARATOR);

        std::string name = itemStr.substr(0, nameValueSeparatorPosition);
        std::string value = itemStr.substr(nameValueSeparatorPosition + vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_NAME_VALUE_SEPARATOR.size());
        this->UIDs[name] = value;

        valueStr = valueStr.substr(itemSeparatorPosition + vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_SEPARATOR.size());
        itemSeparatorPosition = valueStr.find(vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_SEPARATOR);
        }
      if (!valueStr.empty() )
        {
        std::string itemStr = valueStr.substr(0, itemSeparatorPosition);
...
//---------------------------------------------------------------------------
void vtkSubjectHierarchyItem::WriteXML(ostream& of, int nIndent, vtkDMMLNode* xmlEncoderNode)
{
  if (!xmlEncoderNode)
    {
  ...
  //TODO: Storing the parent is necessary if we want to avoid adding extra features to vtkDMMLParser.
  ...
  if (this->TemporaryParentItemID)
    ...
    {
    of << " parent=\"" << (this->Parent ? this->Parent->ID : vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID) << "\"";
    }
  ...
  if (this->UIDs.size() > 0)
    ...
    for (std::map<std::string, std::string>::iterator uidIt = this->UIDs.begin(); uidIt != this->UIDs.end(); ++uidIt)
      ...
      of << xmlEncoderNode->XMLAttributeEncodeString(uidIt->first)
         << vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_NAME_VALUE_SEPARATOR
         << xmlEncoderNode->XMLAttributeEncodeString(uidIt->second)
         << vtkDMMLSubjectHierarchyNode::SUBJECTHIERARCHY_SEPARATOR;
      }
    of << "\"";
    }
...
void vtkSubjectHierarchyItem::DeepCopy(vtkSubjectHierarchyItem* item, bool copyChildren/*=true*/)
{
  ...
  if (item->TemporaryParentItemID)
    ...
    {
    this->TemporaryParentItemID = (item->Parent ? item->Parent->ID : vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID);
    }
...
bool vtkSubjectHierarchyItem::IsVirtualBranchParent()
{
  return !this->GetAttribute(
    vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyVirtualBranchAttributeName() ).empty();
}
...
//---------------------------------------------------------------------------
vtkSubjectHierarchyItem* vtkSubjectHierarchyItem::FindChildByDataNode(vtkDMMLNode* dataNode, bool recursive/*=true*/)
{
  if (!dataNode)
    {
    return nullptr;
...
void vtkSubjectHierarchyItem::GetDataNodesInBranch(vtkCollection* dataNodeCollection, const char* childClass/*=nullptr*/)
{
  ...
  vtkDMMLNode* currentDataNode = this->DataNode.GetPointer();
  if (currentDataNode)
    {
...
bool vtkSubjectHierarchyItem::Reparent(vtkSubjectHierarchyItem* newParentItem)
{
  ...
  this->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemReparentedEvent, this);
...
bool vtkSubjectHierarchyItem::RemoveChild(vtkSubjectHierarchyItem* item)
{
  ...
  // Remove child
  this->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAboutToBeRemovedEvent, item);
  ...
  // Invoke events
  this->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemRemovedEvent, item);
  this->Modified();
...
bool vtkSubjectHierarchyItem::RemoveChild(vtkIdType itemID)
{
  ...
  // Remove child
  this->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAboutToBeRemovedEvent, removedItem.GetPointer());
  ...
  // Invoke events
  this->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemRemovedEvent, removedItem.GetPointer());
  this->Modified();
  ...
  for (childIDIt=childIDs.begin(); childIDIt!=childIDs.end(); ++childIDIt)
    ...
    for (childIt=this->Children.begin(); childIt!=this->Children.end(); ++childIt)
      ...
      if (childIt->GetPointer()->ID == (*childIDIt))
        ...
        childItem->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemReparentedEvent, childItem);
        break;
        }
      }
    }
...
void vtkSubjectHierarchyItem::RemoveAllChildren()
{
  ...
  while (childIDs.size())
    ...
    for (childIt=childIDs.begin(); childIt!=childIDs.end(); ++childIt)
      {
      if ((*childIt) == vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
        {
...
void vtkSubjectHierarchyItem::SetUID(std::string uidName, std::string uidValue)
{
  ...
  this->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemUIDAddedEvent, this);
  this->Modified();
}
...
void vtkSubjectHierarchyItem::SetAttribute(std::string attributeName, std::string attributeValue)
{
  ...
  this->InvokeEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemOwnerPluginSearchRequested, this);
  this->Modified();
}
...
std::string vtkSubjectHierarchyItem::GetAttributeFromAncestor(std::string attributeName, std::string level)
{
  ...
  while (currentItem && currentItem->Parent)
    ...
    if (!currentItem)
      ...
      && currentItem->GetAttribute(vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName()).compare(level) )
      {
      continue;
      }
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
class vtkDMMLSubjectHierarchyNode::vtkInternal
{
public:
  vtkInternal(vtkDMMLSubjectHierarchyNode* external);
  ~vtkInternal();
  ...
  void CopyAsUnresolved(vtkDMMLSubjectHierarchyNode* otherShNode);
...
private:
  vtkDMMLSubjectHierarchyNode* External;
};
...
//---------------------------------------------------------------------------
vtkDMMLSubjectHierarchyNode::vtkInternal::vtkInternal(vtkDMMLSubjectHierarchyNode* external)
...
//---------------------------------------------------------------------------
vtkDMMLSubjectHierarchyNode::vtkInternal::~vtkInternal()
{
  if (this->SceneItem)
    {
...
//---------------------------------------------------------------------------
vtkSubjectHierarchyItem* vtkDMMLSubjectHierarchyNode::vtkInternal::FindItemByID(vtkIdType itemID)
{
...
//---------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::vtkInternal::ResolveUnresolvedItems()
{
  ...
  // Get scene
  vtkDMMLScene* scene = this->External->GetScene();
  if (!scene)
    {
    vtkErrorWithObjectMacro(this->External, "ResolveUnresolvedItems: Invalid DMML scene");
    return false;
    }
  ...
  vtkIdType unresolvedSceneItemID = vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID;
  for ( vtkSubjectHierarchyItem::ChildVector::iterator itemIt=this->UnresolvedItems->Children.begin();
    ...
    {
    if (!itemIt->GetPointer()->GetAttribute(vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName()).compare("Scene"))
      {
  ...
  while (this->UnresolvedItems->Children.size())
    ...
    for ( vtkSubjectHierarchyItem::ChildVector::iterator itemIt=this->UnresolvedItems->Children.begin();
      ...
      item->TemporaryParentItemID = vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID;
      ...
      item->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAddedEvent, this->External->ItemEventCallbackCommand);
      ...
      if (dataNode)
        ...
        {
        idMap[item->TemporaryID] = item->AddToTree(parentItem, item->Name, item->GetAttribute(vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName()));
        }
      ...
      item->TemporaryID = vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID;
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::vtkInternal::AddItemObservers(vtkSubjectHierarchyItem* item)
{
  if (!item)
    {
    return;
  ...
  if (!item->HasObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAddedEvent, this->External->ItemEventCallbackCommand))
    {
    item->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAddedEvent, this->External->ItemEventCallbackCommand);
    }
  if (!item->HasObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAboutToBeRemovedEvent, this->External->ItemEventCallbackCommand))
    {
    item->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAboutToBeRemovedEvent, this->External->ItemEventCallbackCommand);
    }
  if (!item->HasObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemRemovedEvent, this->External->ItemEventCallbackCommand))
    {
    item->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemRemovedEvent, this->External->ItemEventCallbackCommand);
    }
  if (!item->HasObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemUIDAddedEvent, this->External->ItemEventCallbackCommand))
    {
    item->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemUIDAddedEvent, this->External->ItemEventCallbackCommand);
    }
  if (!item->HasObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemOwnerPluginSearchRequested, this->External->ItemEventCallbackCommand))
    {
    item->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemOwnerPluginSearchRequested, this->External->ItemEventCallbackCommand);
    }
  if (!item->HasObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemReparentedEvent, this->External->ItemEventCallbackCommand))
    {
    item->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemReparentedEvent, this->External->ItemEventCallbackCommand);
    }
  ...
  vtkDMMLNode* dataNode = item->DataNode;
  if (dataNode)
    {
    ...
    if (!dataNode->HasObserver(vtkDMMLTransformableNode::TransformModifiedEvent, this->External->ItemEventCallbackCommand))
      {
      dataNode->AddObserver(vtkDMMLTransformableNode::TransformModifiedEvent, this->External->ItemEventCallbackCommand);
      }
    if (!dataNode->HasObserver(vtkDMMLDisplayableNode::DisplayModifiedEvent, this->External->ItemEventCallbackCommand))
      {
      dataNode->AddObserver(vtkDMMLDisplayableNode::DisplayModifiedEvent, this->External->ItemEventCallbackCommand);
      }
    }
}

//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::vtkInternal::CopyAsUnresolved(vtkDMMLSubjectHierarchyNode* otherShNode)
{
  if (!otherShNode)
    {
    return;
...
//----------------------------------------------------------------------------
// vtkDMMLSubjectHierarchyNode members
//----------------------------------------------------------------------------
vtkDMMLSubjectHierarchyNode::vtkDMMLSubjectHierarchyNode()
{
  ...
  this->ItemEventCallbackCommand->SetCallback(vtkDMMLSubjectHierarchyNode::ItemEventCallback);

  // Connect scene item to events related to removal (the parent item invokes these events, not the removed item itself)
  this->Internal->SceneItem->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemAboutToBeRemovedEvent, this->ItemEventCallbackCommand);
  this->Internal->SceneItem->AddObserver(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemRemovedEvent, this->ItemEventCallbackCommand);
}
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::PrintSelf(ostream& os, vtkIndent indent)
{
  Superclass::PrintSelf(os,indent);

  os << indent << "SceneItemID: " << this->Internal->SceneItemID << "\n";
  os << indent << "UnresolvedItemsID: "
    << (this->Internal->UnresolvedItems ? this->Internal->UnresolvedItems->ID : vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID) << "\n";
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::PrintItem(vtkIdType itemID, ostream& os, vtkIndent indent)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
vtkDMMLSubjectHierarchyNode* vtkDMMLSubjectHierarchyNode::ResolveSubjectHierarchy(vtkDMMLScene* scene)
{
  std::vector<vtkDMMLNode*> shNodesInScene;
  scene->GetNodesByClass("vtkDMMLSubjectHierarchyNode", shNodesInScene);

  if (shNodesInScene.size() == 0)
    {
    vtkSmartPointer<vtkDMMLSubjectHierarchyNode> newShNode = vtkSmartPointer<vtkDMMLSubjectHierarchyNode>::New();
    newShNode->SetName("SubjectHierarchy");
    scene->AddNode(newShNode);

    vtkDebugWithObjectMacro(newShNode.GetPointer(), "vtkDMMLSubjectHierarchyNode::ResolveSubjectHierarchy: "
      "New subject hierarchy node created as none was found in the scene");
    return newShNode;
    }

  // Return subject hierarchy node if there is only one
  vtkDMMLSubjectHierarchyNode* firstShNode = nullptr;
  firstShNode = vtkDMMLSubjectHierarchyNode::SafeDownCast(shNodesInScene[0]);
  if (!firstShNode)
    {
    vtkErrorWithObjectMacro( scene,
      "vtkDMMLSubjectHierarchyNode::ResolveSubjectHierarchy: Invalid first subject hierarchy node" );
    return firstShNode;
    }
  if (shNodesInScene.size() == 1)
    ...
    if ( ! ( scene->IsBatchProcessing() || scene->IsImporting() || scene->IsRestoring() || scene->IsClosing()
      ...
      vtkErrorWithObjectMacro( scene,
        "vtkDMMLSubjectHierarchyNode::ResolveSubjectHierarchy: Failed to resolve unresolved subject "
  ...
  std::vector<vtkDMMLSubjectHierarchyNode*> mergedShNodes;
  for (std::vector<vtkDMMLNode*>::iterator shNodeIt=shNodesInScene.begin()+1; shNodeIt!=shNodesInScene.end(); ++shNodeIt)
    {
    vtkDMMLSubjectHierarchyNode* currentShNode = vtkDMMLSubjectHierarchyNode::SafeDownCast(*shNodeIt);
    ...
    if (!firstShNode->Internal->ResolveUnresolvedItems())
      ...
      std::vector<vtkDMMLNode*> allShNodes;
      scene->GetNodesByClass("vtkDMMLSubjectHierarchyNode", allShNodes);
      for (std::vector<vtkDMMLNode*>::iterator allShNodeIt=allShNodes.begin(); allShNodeIt!=allShNodes.end(); ++allShNodeIt)
        {
      ...
      for (std::vector<std::string>::iterator allShNodeIDIt = allShNodeIDs.begin(); allShNodeIDIt != allShNodeIDs.end(); ++allShNodeIDIt)
        {
        vtkDMMLNode* node = scene->GetNodeByID(*allShNodeIDIt);
        if (node)
          {
      ...
      vtkErrorWithObjectMacro( scene,
        "vtkDMMLSubjectHierarchyNode::ResolveSubjectHierarchy: Failed to merge subject hierarchy nodes, re-building subject hierarchy from scratch" );
      return vtkDMMLSubjectHierarchyNode::ResolveSubjectHierarchy(scene);
      }
    }
  // Remove merged subject hierarchy nodes from the scene
  for (std::vector<vtkDMMLSubjectHierarchyNode*>::iterator mergedShIt=mergedShNodes.begin(); mergedShIt!=mergedShNodes.end(); ++mergedShIt)
    {
...
//----------------------------------------------------------------------------
const char* vtkDMMLSubjectHierarchyNode::GetNodeTagName()
{
  return "SubjectHierarchy";
}

//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::ReadItemFromXML(const char** atts)
{
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::WriteXML(ostream& of, int nIndent)
{
  // Add attribute to unambiguously identify subject hierarchy 2.0 node in scene DMML files
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::WriteNodeBodyXML(ostream& of, int indent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::Copy(vtkDMMLNode *anode)
{
  ...
  vtkDMMLSubjectHierarchyNode* otherShNode = vtkDMMLSubjectHierarchyNode::SafeDownCast(anode);
...
//----------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetSceneItemID()
{
  return this->Internal->SceneItemID;
}

//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemDataNode(vtkIdType itemID, vtkDMMLNode* dataNode)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
vtkDMMLNode* vtkDMMLSubjectHierarchyNode::GetItemDataNode(vtkIdType itemID)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemName(vtkIdType itemID, std::string name)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
std::string vtkDMMLSubjectHierarchyNode::GetItemName(vtkIdType itemID)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemLevel(vtkIdType itemID, std::string level)
{
  if (!itemID)
    {
  ...
  if (item->GetAttribute(vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName()).compare(level))
    {
    item->SetAttribute(vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName(), level);
    }
}

//----------------------------------------------------------------------------
std::string vtkDMMLSubjectHierarchyNode::GetItemLevel(vtkIdType itemID)
{
  if (!itemID)
    {
  ...
  return item->GetAttribute(vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName());
}

//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemOwnerPluginName(vtkIdType itemID, std::string ownerPluginName)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
std::string vtkDMMLSubjectHierarchyNode::GetItemOwnerPluginName(vtkIdType itemID)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemExpanded(vtkIdType itemID, bool expanded)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::GetItemExpanded(vtkIdType itemID)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
int vtkDMMLSubjectHierarchyNode::GetItemPositionUnderParent(vtkIdType itemID)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetItemByPositionUnderParent(vtkIdType parentItemID, int position)
{
  if (!parentItemID)
    {
    vtkWarningMacro("GetItemPositionUnderParent: Invalid item ID given");
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemUID(vtkIdType itemID, std::string uidName, std::string uidValue)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
std::string vtkDMMLSubjectHierarchyNode::GetItemUID(vtkIdType itemID, std::string uidName)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemAttribute(vtkIdType itemID, std::string attributeName, std::string attributeValue)
{
  if (!itemID)
    {
...
//---------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::RemoveItemAttribute(vtkIdType itemID, std::string attributeName)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
std::string vtkDMMLSubjectHierarchyNode::GetItemAttribute(vtkIdType itemID, std::string attributeName)
{
  if (!itemID)
    {
...
//---------------------------------------------------------------------------
std::vector<std::string> vtkDMMLSubjectHierarchyNode::GetItemAttributeNames(vtkIdType itemID)
{
  if (!itemID)
    {
...
//---------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::HasItemAttribute(vtkIdType itemID, std::string attributeName)
{
  if (!itemID)
    {
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::ItemModified(vtkIdType itemID)
{
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::RequestOwnerPluginSearch(vtkIdType itemID)
{
  // Do not propagate event on invalid item ID
  if (!itemID)
    {
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::RequestOwnerPluginSearch(vtkDMMLNode* dataNode)
{
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::CreateItem(vtkIdType parentItemID, vtkDMMLNode* dataNode, const char* ownerPluginName)
{
  if (!dataNode)
    {
  ...
  if (itemID != INVALID_ITEM_ID)
    ...
    // for supported data DMML nodes (supported = there is a plugin that can "own" it)
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::CreateHierarchyItem(vtkIdType parentItemID, std::string name, std::string level, int positionUnderParent/*=-1*/)
{
  vtkSubjectHierarchyItem* parentItem = this->Internal->FindItemByID(parentItemID);
  if (!parentItem)
    {
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::CreateSubjectItem(vtkIdType parentItemID, std::string name)
{
  return this->CreateHierarchyItem(parentItemID, name, vtkDMMLSubjectHierarchyConstants::GetDICOMLevelPatient());
}

//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::CreateStudyItem(vtkIdType parentItemID, std::string name)
{
  return this->CreateHierarchyItem(parentItemID, name, vtkDMMLSubjectHierarchyConstants::GetDICOMLevelStudy());
}

//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::CreateFolderItem(vtkIdType parentItemID, std::string name)
{
  return this->CreateHierarchyItem(parentItemID, name, vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelFolder());
}

//----------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::RemoveItem(vtkIdType itemID, bool removeDataNode/*=true*/, bool recursive/*=true*/)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::RemoveItemChildren(vtkIdType itemID, bool removeDataNodes/*=true*/, bool recursive/*=true*/)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::RemoveAllItems(bool removeDataNode/*=false*/)
{
  this->RemoveItem(this->Internal->SceneItemID, removeDataNode, true);
}

//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemParent(vtkIdType itemID, vtkIdType parentItemID, bool enableCircularCheck/*=true*/)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetItemParent(vtkIdType itemID)
{
  if (!itemID)
    {
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::GetItemChildren(vtkIdType itemID, std::vector<vtkIdType>& childIDs, bool recursive/*=false*/)
{
  childIDs.clear();
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::GetItemChildren(vtkIdType itemID, vtkIdList* childIDs, bool recursive/*=false*/)
{
  if (!childIDs)
    {
...
//----------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::ReparentItemByDataNode(vtkIdType itemID, vtkDMMLNode* newParentNode)
{
  vtkSubjectHierarchyItem* item = this->Internal->SceneItem->FindChildByID(itemID);
  if (!item)
    {
  ...
  if (!newParentItem)
    {
    vtkErrorMacro("ReparentItem: Failed to find subject hierarchy item by data DMML node " << newParentNode->GetName());
    return false;
    }
...
//----------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::MoveItem(vtkIdType itemID, vtkIdType beforeItemID)
{
  vtkSubjectHierarchyItem* item = this->Internal->SceneItem->FindChildByID(itemID);
  if (!item)
    {
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetItemByUID(const char* uidName, const char* uidValue)
{
  if (!uidName || !uidValue)
    {
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetItemByUIDList(const char* uidName, const char* uidValue)
{
  if (!uidName || !uidValue)
    {
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetItemByDataNode(vtkDMMLNode* dataNode)
{
  if (!dataNode)
    {
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetItemByName(std::string name)
{
  if (name.empty())
    {
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::GetItemsByName(std::string name, vtkIdList* foundItemIds, bool contains/*=false*/)
{
  if (!foundItemIds)
    {
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetItemChildWithName(vtkIdType parentItemID, std::string name, bool recursive/*=false*/)
{
  if (!parentItemID)
    {
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::GetDataNodesInBranch(vtkIdType itemID, vtkCollection* dataNodeCollection, const char* childClass/*=nullptr*/)
{
  if (!itemID)
    {
...
//---------------------------------------------------------------------------
vtkDMMLNode* vtkDMMLSubjectHierarchyNode::GetParentDataNode(vtkDMMLNode* dataNode, bool recursive/*=false*/)
{
  vtkIdType itemID = this->GetItemByDataNode(dataNode);
  if (itemID == INVALID_ITEM_ID)
    {
  ...
  vtkDMMLNode* parentDataNode = this->GetItemDataNode(parentItemID);
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetDisplayVisibilityForBranch(vtkIdType itemID, int visible)
{
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::SetItemDisplayVisibility(vtkIdType itemID, int visible)
{
  ...
  vtkDMMLNode* dataNode = this->GetItemDataNode(itemID);
  vtkDMMLDisplayableNode* displayableNode = vtkDMMLDisplayableNode::SafeDownCast(dataNode);
...
//---------------------------------------------------------------------------
int vtkDMMLSubjectHierarchyNode::GetDisplayVisibilityForBranch(vtkIdType itemID)
{
...
//---------------------------------------------------------------------------
int vtkDMMLSubjectHierarchyNode::GetItemDisplayVisibility(vtkIdType itemID)
{
  vtkDMMLNode* dataNode = this->GetItemDataNode(itemID);
  vtkDMMLDisplayableNode* displayableNode = vtkDMMLDisplayableNode::SafeDownCast(dataNode);
  vtkDMMLDisplayNode* displayNode = vtkDMMLDisplayNode::SafeDownCast(dataNode);
...
//---------------------------------------------------------------------------
vtkDMMLDisplayNode* vtkDMMLSubjectHierarchyNode::GetDisplayNodeForItem(vtkIdType itemID)
{
  vtkDMMLNode* dataNode = this->GetItemDataNode(itemID);
  vtkDMMLDisplayableNode* displayableNode = vtkDMMLDisplayableNode::SafeDownCast(dataNode);
  vtkDMMLDisplayNode* displayNode = vtkDMMLDisplayNode::SafeDownCast(dataNode);
...
//---------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::IsItemLevel(vtkIdType itemID, std::string level)
{
  if (!itemID)
    {
  ...
  return !item->GetAttribute(vtkDMMLSubjectHierarchyConstants::GetSubjectHierarchyLevelAttributeName()).compare(level);
}

//---------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::IsItemVirtualBranchParent(vtkIdType itemID)
{
  if (!itemID)
    {
...
//---------------------------------------------------------------------------
std::string vtkDMMLSubjectHierarchyNode::GetAttributeFromItemAncestor(vtkIdType itemID, std::string attributeName, std::string level)
{
  vtkSubjectHierarchyItem* item = this->Internal->SceneItem->FindChildByID(itemID);
  if (!item)
    {
...
//---------------------------------------------------------------------------
vtkIdType vtkDMMLSubjectHierarchyNode::GetItemAncestorAtLevel(vtkIdType itemID, std::string level)
{
  vtkSubjectHierarchyItem* item = this->Internal->SceneItem->FindChildByID(itemID);
  if (!item)
    {
...
//---------------------------------------------------------------------------
bool vtkDMMLSubjectHierarchyNode::IsAnyNodeInBranchTransformed(
  vtkIdType itemID, bool includeParentItem/*=true*/, vtkDMMLTransformNode* exceptionNode/*=nullptr*/)
{
  ...
  vtkDMMLTransformableNode* parentTransformableNode = nullptr;
  if (item->DataNode)
    {
    parentTransformableNode = vtkDMMLTransformableNode::SafeDownCast(item->DataNode);
  ...
  this->GetDataNodesInBranch(itemID, childTransformableNodes.GetPointer(), "vtkDMMLTransformableNode");
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::DeserializeUIDList(std::string uidListString, std::vector<std::string>& deserializedUIDList)
{
...
//---------------------------------------------------------------------------
std::vector<vtkIdType> vtkDMMLSubjectHierarchyNode::GetItemsReferencedFromItemByDICOM(vtkIdType itemID)
{
  ...
  std::string referencedInstanceUIDsAttribute = item->GetAttribute(
    vtkDMMLSubjectHierarchyConstants::GetDICOMReferencedInstanceUIDsAttributeName() );
  ...
  for (std::vector<std::string>::iterator uidIt=referencedSopInstanceUids.begin(); uidIt!=referencedSopInstanceUids.end(); ++uidIt)
    ...
    if (referencedItems.empty())
      ...
      vtkSubjectHierarchyItem* referencedItem = this->Internal->SceneItem->FindChildByUIDList(
        vtkDMMLSubjectHierarchyConstants::GetDICOMInstanceUIDName(), (*uidIt) );
      if (referencedItem)
        {
      ...
      for (std::vector<vtkSubjectHierarchyItem*>::iterator itemIt=referencedItems.begin(); itemIt!=referencedItems.end(); ++itemIt)
        ...
        std::string uids = (*itemIt)->GetUID(vtkDMMLSubjectHierarchyConstants::GetDICOMInstanceUIDName());
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::GetItemsReferencedFromItemByDICOM(vtkIdType itemID, vtkIdList* referencedIdList)
{
  if (!referencedIdList)
    {
...
//---------------------------------------------------------------------------
std::vector<vtkIdType> vtkDMMLSubjectHierarchyNode::GetItemsReferencingItemByDICOM(vtkIdType itemID)
{
  ...
  std::string uidsString = item->GetUID(vtkDMMLSubjectHierarchyConstants::GetDICOMInstanceUIDName());
  if (uidsString.empty())
    {
  ...
  for (std::vector<vtkIdType>::iterator itemIt=allItemIDs.begin(); itemIt!=allItemIDs.end(); ++itemIt)
    ...
    std::string referencedUids = currentItem->GetAttribute(vtkDMMLSubjectHierarchyConstants::GetDICOMReferencedInstanceUIDsAttributeName());
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::GetItemsReferencingItemByDICOM(vtkIdType itemID, vtkIdList* referencingIdList)
{
...
//---------------------------------------------------------------------------
std::string vtkDMMLSubjectHierarchyNode::GenerateUniqueItemName(std::string name)
{
  std::vector<vtkIdType> foundItemIDs;
...
//---------------------------------------------------------------------------
int vtkDMMLSubjectHierarchyNode::GetNumberOfItems()
{
...
//---------------------------------------------------------------------------
int vtkDMMLSubjectHierarchyNode::GetNumberOfItemChildren(vtkIdType itemID, bool recursive/*=false*/)
{
  if (!itemID)
    {
...
//---------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::ItemEventCallback(vtkObject* caller, unsigned long eid, void* clientData, void* callData)
{
  vtkDMMLSubjectHierarchyNode* self = reinterpret_cast<vtkDMMLSubjectHierarchyNode*>(clientData);
  ...
  switch (eid)
    ...
    case vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemUIDAddedEvent:
    case vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemOwnerPluginSearchRequested:
    ...
    case vtkCommand::ModifiedEvent:
      ...
      vtkDMMLNode* dataNode = vtkDMMLNode::SafeDownCast(caller);
      if (item)
        {
        // Propagate item modified event
        self->InvokeCustomModifiedEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemModifiedEvent, (void*)&item->ID);
        self->Modified(); // Indicate that the content of the subject hierarchy node has changed, so it needs to be saved
        }
      else if (dataNode)
        {
        ...
        if (itemID != vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
          {
          self->InvokeCustomModifiedEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemModifiedEvent, (void*)&itemID);
          }
        }
      }
      break;
    ...
    case vtkDMMLTransformableNode::TransformModifiedEvent:
      ...
      if (dataNode)
        ...
        if (itemID != vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
          {
          self->InvokeCustomModifiedEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemTransformModifiedEvent, (void*)&itemID);
          }
        }
      }
      break;
    ...
    case vtkDMMLDisplayableNode::DisplayModifiedEvent:
      ...
      if (dataNode)
        ...
        if (itemID != vtkDMMLSubjectHierarchyNode::INVALID_ITEM_ID)
          {
          self->InvokeCustomModifiedEvent(vtkDMMLSubjectHierarchyNode::SubjectHierarchyItemDisplayModifiedEvent, (void*)&itemID);
          }
        }
      }
      break;
    ...
    default:
      vtkErrorWithObjectMacro(self, "vtkDMMLSubjectHierarchyNode::ItemEventCallback: Unknown event ID " << eid);
      return;
    }
}

//---------------------------------------------------------------------------
vtkDMMLSubjectHierarchyNode* vtkDMMLSubjectHierarchyNode::GetSubjectHierarchyNode(vtkDMMLScene* scene)
{
  if (scene == nullptr)
    {
    vtkGenericWarningMacro("vtkDMMLSubjectHierarchyNode::GetSubjectHierarchyNode: Invalid scene given");
    return nullptr;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLSubjectHierarchyNode::ShowItemsInView(vtkIdList* itemIDs, vtkDMMLAbstractViewNode* viewNode)
{
  ...
  // The event will be processed by qCjyxSubjectHierarchyPluginHandler
}
