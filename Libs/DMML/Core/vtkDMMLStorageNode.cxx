...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLStorageNode.cxx,v $
...
//----------------------------------------------------------------------------
vtkCxxSetObjectMacro(vtkDMMLStorageNode, URIHandler, vtkURIHandler)

//----------------------------------------------------------------------------
vtkDMMLStorageNode::vtkDMMLStorageNode()
{
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::WriteXML(ostream& of, int nIndent)
{
  ...
  if (this->FileName != nullptr)
    ...
    of << " fileName=\"" << vtkDMMLNode::URLEncodeString(name.c_str()) << "\"";
    ...
    */
    if (this->GetScene() && this->IsFilePathRelative(this->FileName))
      ...
      if (absFilePath)
        ...
        {
        vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "WriteXML",
          "Unable to convert relative file path to absolute, still using " << this->FileName);
        }
      }
    }
  for (int i = 0; i < this->GetNumberOfFileNames(); i++)
    ...
    of << " fileListMember" << i << "=\"" << vtkDMMLNode::URLEncodeString(name.c_str()) << "\"";
    if (this->GetScene() && this->IsFilePathRelative(this->GetNthFileName(i)))
      {
      // go back to absolute
      ...
      if (absFilePath)
        ...
        {
        vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "WriteXML",
          "Unable to convert relative file path to absolute, still using " << this->GetNthFileName(i));
        }
      }
    }
  ...
  if (this->URI != nullptr)
    {
    of << " uri=\"" << vtkDMMLNode::URLEncodeString(this->URI) << "\"";
    }
  for (int i = 0; i < this->GetNumberOfURIs(); i++)
    {
    of << " uriListMember" << i << "=\"" << vtkDMMLNode::URLEncodeString(this->GetNthURI(i)) << "\"";
    }
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::ReadXMLAttributes(const char** atts)
{
  ...
  while (*atts != nullptr)
    ...
    if (!strcmp(attName, "fileName"))
      {
      std::string filename = vtkDMMLNode::URLDecodeString(attValue);
      if (filename.empty())
        {
    ...
    if (!strncmp(attName, "fileListMember", 14))
      ...
      if (vtksys::SystemTools::FileExists(collapsedFullPath.c_str(), true) == false &&
        ...
        {
        vtkDebugMacro("ReadXMLAttributes: File list member " << filename << " is relative, but is not found relative to the dmml root directory. " << collapsedFullPath << " was not found. Trying to find it relative to the archetype: " << this->GetFileName());
        ...
        if (vtksys::SystemTools::FileExists(secondCollapsedFullPath.c_str(), true) == false)
          {
          vtkWarningMacro("ReadXMLAttributes: file list member " << filename << " is relative, but not found relative to the dmml root directory. Trying to find the path relative to the archetype doesn't work either: " << secondCollapsedFullPath << " not found, going back to using path from dmml root dir of " << collapsedFullPath);
          }
        else
          {
          vtkWarningMacro("ReadXMLAttributes: file list member " << filename << " is relative, but not found relative to the dmml root directory: " << collapsedFullPath << " is invalid. Found the file relative to the archetype, so using path  " <<  secondCollapsedFullPath);
      ...
      {
      std::string uri = vtkDMMLNode::URLDecodeString(attValue);
...
void vtkDMMLStorageNode::Copy(vtkDMMLNode *anode)
{
  int disabledModify = this->StartModify();

  Superclass::Copy(anode);
  vtkDMMLStorageNode *node = (vtkDMMLStorageNode *) anode;
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::ProcessDMMLEvents ( vtkObject *vtkNotUsed(caller), unsigned long event, void *callData )
{
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::StageReadData ( vtkDMMLNode *refNode )
{
  ...
  if ( this->Scene )
    ...
    // of data. But,
    // commented out for now because CLI modules are using DMML to write
    // data in another thread, causing GUI to crash.
//    this->Scene->InvokeEvent (vtkDMMLScene::ImportProgressFeedbackEvent );
    }
  ...
  if (refNode == nullptr)
    {
    vtkDebugMacro("StageReadData: input dmml node is null, returning.");
    return;
    }
  ...
  if (this->Scene == nullptr)
    {
    vtkDebugMacro("StageReadData: Cannot get dmml scene, unable to get remote file handlers.");
    return;
    }
  ...
  if (iomanager != nullptr)
    ...
    if (this->GetReadState() == this->Idle)
      ...
      if (this->URIHandler != nullptr)
        ...
        {
        vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::StageReadData",
          "Unable to get a URI handler for " << this->URI << ", resetting stage to idle");
        this->SetReadStateIdle();
        return;
        }
    ...
    {
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::StageReadData",
      "No IO Manager on the scene");
    }
  vtkDebugMacro("StageReadData: done");
}

//----------------------------------------------------------------------------
void vtkDMMLStorageNode::StageWriteData ( vtkDMMLNode *refNode )
{
  if ( this->Scene )
    {
    // this event is being detected by GUI to provide feedback during load
    ...
    // commented out for now because CLI modules are using DMML to write
    // data in another thread, causing GUI to crash.
//    this->Scene->InvokeEvent (vtkDMMLScene::SaveProgressFeedbackEvent );
    }
  ...
  if (this->Scene == nullptr)
    {
    vtkDebugMacro("StageWriteData: Cannot get dmml scene, unable to get remote file handlers.");
    return;
    }
  if (refNode == nullptr)
    {
    vtkDebugMacro("StageWriteData: input dmml node is null, returning.");
    return;
    }
   ...
   if (iomanager != nullptr)
     ...
     if (this->GetWriteState() == this->Idle)
       ...
       if (this->URIHandler == nullptr)
         {
         vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::StageWriteData",
           "Unable to get a URI handler for " << this->URI << ", resetting stage to idle");
         return;
         }
     ...
     {
     vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::StageWriteData",
       "StageWriteData: No IO Manager on the scene");
     }
}

//----------------------------------------------------------------------------
const char * vtkDMMLStorageNode::GetStateAsString(int state)
{
...
//----------------------------------------------------------------------------
std::string vtkDMMLStorageNode::GetFullNameFromFileName()
{
  return this->GetFullNameFromNthFileName(-1);
}

//----------------------------------------------------------------------------
std::string vtkDMMLStorageNode::GetFullNameFromNthFileName(int n)
{
...
//----------------------------------------------------------------------------
int vtkDMMLStorageNode::SupportedFileType(const char *fileName)
{
...
//----------------------------------------------------------------------------
int vtkDMMLStorageNode::FileNameIsInList(const char *fileName)
{
  if (fileName == nullptr)
    {
    return 0;
...
//----------------------------------------------------------------------------
unsigned int vtkDMMLStorageNode::AddFileName( const char* filename )
{
  if (!filename)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::AddFileName",
      "Cannot add a null file name");
    return 0;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::ResetFileNameList( )
{
  this->FileNameList.resize( 0 );
}

//----------------------------------------------------------------------------
const char * vtkDMMLStorageNode::GetNthFileName(int n) const
{
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::ResetNthFileName(int n, const char* fileName)
{
  if (fileName == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ResetNthFileName",
      "ResetNthFileName: given file name is null (n = " << n << ").");
    return;
    }
  if (n < 0)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ResetNthFileName",
      "ResetNthFileName: invalid file name number (n = " << n << ").");
    return;
    }
  else if (n >= this->GetNumberOfFileNames())
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ResetNthFileName",
      "ResetNthFileName: file name number " << n << " not already set.");
    return;
    }
...
//----------------------------------------------------------------------------
unsigned int vtkDMMLStorageNode::AddURI( const char* uri )
{
  if (uri == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::AddURI",
      "Cannot add a null URI.");
    return 0;
    }
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::ResetURIList( )
{
  this->URIList.resize( 0 );
}

//----------------------------------------------------------------------------
const char * vtkDMMLStorageNode::GetNthURI(int n)
{
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::ResetNthURI(int n, const char* uri)
{
  if (uri == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ResetNthURI",
      "ResetNthURI: given URI is null (n = " << n << ").");
    return;
    }
  if (n >= 0 && this->GetNumberOfURIs() >= n)
    ...
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ResetNthURI",
      "ResetNthURI: URI number " << n << " not already set.");
    }
}

//----------------------------------------------------------------------------
void vtkDMMLStorageNode::SetDataDirectory(const char *dataDirName)
{
  if (dataDirName == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::SetDataDirectory",
      "Input directory name is null.");
    return;
    }
  if (this->GetFileName() == nullptr)
    {
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::SetDataDirectory",
      "File name is null, no reason to reset data directory.");
    return;
    }
  // reset the filename
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::SetURIPrefix(const char *uriPrefix)
{
  if (uriPrefix == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::SetURIPrefix",
      "Input URI prefix is null.");
    return;
    }
  vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::SetURIPrefix",
    "Setting URI prefix is not implemented yet.");
  // reset the uri
...
//----------------------------------------------------------------------------
vtkStringArray* vtkDMMLStorageNode::GetSupportedReadFileTypes()
{
...
//----------------------------------------------------------------------------
vtkStringArray* vtkDMMLStorageNode::GetSupportedWriteFileTypes()
{
...
//------------------------------------------------------------------------------
const char* vtkDMMLStorageNode::GetDefaultWriteFileExtension()
{
...
//------------------------------------------------------------------------------
void vtkDMMLStorageNode::GetFileExtensionsFromFileTypes(vtkStringArray* inputFileTypes, vtkStringArray* outputFileExtensions)
{
  if (inputFileTypes == nullptr || outputFileExtensions == nullptr)
    {
    vtkErrorMacro("vtkDMMLStorageNode::GetSupportedReadFileExtensions failed: invalid inputs");
    return;
    }
...
//------------------------------------------------------------------------------
std::string vtkDMMLStorageNode::GetSupportedFileExtension(const char* fileName /* =nullptr */, bool includeReadable /* =true */, bool includeWriteable /* =true */)
{
...
//------------------------------------------------------------------------------
void vtkDMMLStorageNode::SetDefaultWriteFileExtension(const char* ext)
{
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::InitializeSupportedReadFileTypes()
{
...
//----------------------------------------------------------------------------
void vtkDMMLStorageNode::InitializeSupportedWriteFileTypes()
{
  this->SupportedWriteFileTypes->Reset();
  this->SupportedWriteFileTypes->SetNumberOfTuples(0);
}
...
//------------------------------------------------------------------------------
int vtkDMMLStorageNode::IsFilePathRelative(const char * filepath)
{
  if (filepath == nullptr)
    {
...
//------------------------------------------------------------------------------
const char *vtkDMMLStorageNode::GetAbsoluteFilePath(const char *inputPath)
{
  ...
  if (!this->GetScene() ||
      !this->GetScene()->GetRootDirectory())
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::GetAbsoluteFilePath",
      "Relative path " << inputPath << " is specified but no scene or root directory to find it from.");
    return nullptr;
    }
...
//------------------------------------------------------------------------------
void vtkDMMLStorageNode::InvalidateFile()
{
  this->StoredTime->Delete();
  this->StoredTime = vtkTimeStamp::New();
}
...
//------------------------------------------------------------------------------
vtkTimeStamp vtkDMMLStorageNode::GetStoredTime()
{
  return *this->StoredTime;
}

//----------------------------------------------------------------------------
bool vtkDMMLStorageNode::CanReadInReferenceNode(vtkDMMLNode *refNode)
{
  // Must be reimplemented in subclass;
  return refNode->IsA("vtkDMMLStorableNode");
}

//----------------------------------------------------------------------------
bool vtkDMMLStorageNode::CanWriteFromReferenceNode(vtkDMMLNode *refNode)
{
  return this->CanReadInReferenceNode(refNode);
}

//------------------------------------------------------------------------------
int vtkDMMLStorageNode::ReadData(vtkDMMLNode* refNode, bool temporary)
{
  if (refNode == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ReadData",
      "Cannot read data into a null node.");
    return 0;
    }
  ...
  if (this->GetFileName() == nullptr && this->GetURI() == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ReadData",
      "Both filename and uri are null.");
    return 0;
    }
  ...
  if ( this->GetReadState() != this->TransferDone )
    ...
    vtkWarningToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ReadData",
      "ReadData: read state is pending, remote download hasn't finished yet");
    return 0;
    }
  ...
  vtkDMMLStorableNode* storableNode = vtkDMMLStorableNode::SafeDownCast(refNode);
  int success = this->ReadDataInternal(refNode);
  if (!success)
    {
    // failed
    std::string storableNodeID = "(null)";
    std::string storableNodeName = "(null)";
    if (storableNode)
      {
      if (storableNode->GetID())
        {
        storableNodeID = storableNode->GetID();
        }
      if (storableNode->GetName())
        {
        storableNodeName = storableNode->GetName();
        }
      }
    std::string location;
    if (this->GetURI())
      {
      location += std::string(" URI='") + this->GetURI() + "'";
      }
    if (this->GetFileName())
      {
      location += std::string(" filename='") + this->GetFileName() + "'";
      }
    if (location.empty())
      {
      location += " - no URI or filename is specified";
      }
    else
      {
    ...
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::ReadData",
      "Failed to read node " << storableNodeName << " (" << storableNodeID << ")" << location);
...
//------------------------------------------------------------------------------
int vtkDMMLStorageNode::WriteData(vtkDMMLNode* refNode)
{
  this->WriteState = this->Idle;
  if (refNode == nullptr)
    {
    vtkErrorToMessageCollectionMacro(this->GetUserMessages(), "vtkDMMLStorageNode::WriteData",
      "Cannot write " << (this->GetID() ? this->GetID() : "(null)") << ": input node is null");
    return 0;
    }
...
//------------------------------------------------------------------------------
int vtkDMMLStorageNode::ReadDataInternal(vtkDMMLNode* vtkNotUsed(refNode))
{
  return 0;
}

//------------------------------------------------------------------------------
int vtkDMMLStorageNode::WriteDataInternal(vtkDMMLNode* vtkNotUsed(refNode))
{
  return 0;
}

//------------------------------------------------------------------------------
std::string vtkDMMLStorageNode::GetLowercaseExtensionFromFileName(const std::string& filename)
{
...
//------------------------------------------------------------------------------
std::string vtkDMMLStorageNode::GetFileNameWithoutExtension(const char* filePath /* =nullptr */)
{
...
//------------------------------------------------------------------------------
void vtkDMMLStorageNode::UpdateCompressionPresets()
{
}

//------------------------------------------------------------------------------
int vtkDMMLStorageNode::GetNumberOfCompressionPresets()
{
...
//------------------------------------------------------------------------------
std::vector<std::string> vtkDMMLStorageNode::GetCompressionPresetDisplayNames()
{
  this->UpdateCompressionPresets();
...
//------------------------------------------------------------------------------
std::string vtkDMMLStorageNode::GetCompressionParameterFromDisplayName(const std::string& name)
{
  this->UpdateCompressionPresets();
  std::vector<CompressionPreset>::iterator compressionPresetIt;
  for (compressionPresetIt = this->CompressionPresets.begin(); compressionPresetIt != this->CompressionPresets.end(); ++compressionPresetIt)
...
//------------------------------------------------------------------------------
std::string vtkDMMLStorageNode::GetDisplayNameFromCompressionParameter(const std::string& parameter)
{
  this->UpdateCompressionPresets();
  std::vector<CompressionPreset>::iterator compressionPresetIt;
  for (compressionPresetIt = this->CompressionPresets.begin(); compressionPresetIt != this->CompressionPresets.end(); ++compressionPresetIt)
...
//------------------------------------------------------------------------------
const std::vector<vtkDMMLStorageNode::CompressionPreset> vtkDMMLStorageNode::GetCompressionPresets()
{
  this->UpdateCompressionPresets();
...
//----------------------------------------------------------------------------
vtkDMMLStorableNode* vtkDMMLStorageNode::GetStorableNode()
{
  if (this->Scene == nullptr)
    {
    return nullptr;
  ...
  for (sceneNodes->InitTraversal(it);
    ...
    vtkDMMLStorableNode* storableNode =
      vtkDMMLStorableNode::SafeDownCast(node);
...
//-----------------------------------------------------------
const char* vtkDMMLStorageNode::GetCoordinateSystemTypeAsString(int id)
{
  switch (id)
    {
...
//-----------------------------------------------------------
int vtkDMMLStorageNode::GetCoordinateSystemTypeFromString(const char* name)
{
  if (name == nullptr)
    {
    // invalid name
