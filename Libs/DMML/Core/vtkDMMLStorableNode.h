/*=auto=========================================================================
  ...
  Program:   3D Cjyx
  Module:    $RCSfile: vtkDMMLStorableNode.h,v $
  Date:      $Date: 2006/03/19 17:12:28 $
  Version:   $Revision: 1.6 $

=========================================================================auto=*/

#ifndef __vtkDMMLStorableNode_h
#define __vtkDMMLStorableNode_h
...
/// \brief DMML node to represent a 3D surface model.
///
...
class VTK_DMML_EXPORT vtkDMMLStorableNode : public vtkDMMLNode
{
public:
  vtkTypeMacro(vtkDMMLStorableNode,vtkDMMLNode);
  ...
  vtkDMMLCopyContentMacro(vtkDMMLStorableNode);
  ...
  /// It's an informatics metadata mechanism so that Cjyx knows what kinds
  ...
  void SetCjyxDataType ( const char *type );
  const char *GetCjyxDataType ();
  ...
  vtkDMMLStorageNode* GetNthStorageNode(int n);
  vtkDMMLStorageNode* GetStorageNode();
  ...
  virtual vtkDMMLStorageNode* CreateDefaultStorageNode();
  ...
  /// Default value: "vtkDMMLSequenceStorageNode"
  ...
  /// If the method is not overwritten by subclass then it creates vtkDMMLSequenceStorageNode,
  /// which saves the sequence in a DMML scene bundle file.
  virtual vtkDMMLStorageNode* CreateDefaultSequenceStorageNode();
 ...
 protected:
  vtkDMMLStorableNode();
  ~vtkDMMLStorableNode() override;
  vtkDMMLStorableNode(const vtkDMMLStorableNode&);
  void operator=(const vtkDMMLStorableNode&);

  static const char* StorageNodeReferenceRole;
  static const char* StorageNodeReferenceDMMLAttributeName;

  virtual const char* GetStorageNodeReferenceRole();
  virtual const char* GetStorageNodeReferenceDMMLAttributeName();

  vtkTagTable *UserTagTable;

  ///
  /// CjyxDataType records the kind of storage node that
  /// holds the data. Set in each subclass.
  std::string CjyxDataType;
  ...
  /// vtkDMMLStorageNode::GetStoredTime()
  ...
  /// that is stored on disk, not in the DMML scene: e.g. points and cells for a
