...
Program:   3D Cjyx
Module:    $RCSfile: vtkDMMLNRRDStorageNode.cxx,v $
...
//----------------------------------------------------------------------------
vtkDMMLNodeNewMacro(vtkDMMLNRRDStorageNode);

//----------------------------------------------------------------------------
vtkDMMLNRRDStorageNode::vtkDMMLNRRDStorageNode()
{
  this->CenterImage = 0;
...
//----------------------------------------------------------------------------
vtkDMMLNRRDStorageNode::~vtkDMMLNRRDStorageNode() = default;

//----------------------------------------------------------------------------
void vtkDMMLNRRDStorageNode::WriteXML(ostream& of, int nIndent)
{
...
//----------------------------------------------------------------------------
void vtkDMMLNRRDStorageNode::ReadXMLAttributes(const char** atts)
{
...
void vtkDMMLNRRDStorageNode::Copy(vtkDMMLNode *anode)
{
  int disabledModify = this->StartModify();

  Superclass::Copy(anode);
  vtkDMMLNRRDStorageNode *node = (vtkDMMLNRRDStorageNode *) anode;
...
//----------------------------------------------------------------------------
void vtkDMMLNRRDStorageNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
bool vtkDMMLNRRDStorageNode::CanReadInReferenceNode(vtkDMMLNode *refNode)
{
  return refNode->IsA("vtkDMMLScalarVolumeNode") ||
         refNode->IsA("vtkDMMLVectorVolumeNode" ) ||
         refNode->IsA("vtkDMMLDiffusionWeightedVolumeNode") ||
         refNode->IsA("vtkDMMLDiffusionTensorVolumeNode");
}

//----------------------------------------------------------------------------
int vtkDMMLNRRDStorageNode::ReadDataInternal(vtkDMMLNode *refNode)
{
  vtkDMMLVolumeNode *volNode = nullptr;

  if ( refNode->IsA("vtkDMMLDiffusionTensorVolumeNode") )
    {
    volNode = dynamic_cast <vtkDMMLDiffusionTensorVolumeNode *> (refNode);
    }
  else if ( refNode->IsA("vtkDMMLDiffusionWeightedVolumeNode") )
    {
    volNode = dynamic_cast <vtkDMMLDiffusionWeightedVolumeNode *> (refNode);
    }
  else if ( refNode->IsA("vtkDMMLVectorVolumeNode") )
    {
    volNode = dynamic_cast <vtkDMMLVectorVolumeNode *> (refNode);
    }
  else if ( refNode->IsA("vtkDMMLScalarVolumeNode") )
    {
    volNode = dynamic_cast <vtkDMMLScalarVolumeNode *> (refNode);
    }
  else if ( refNode->IsA("vtkDMMLVolumeNode") )
    {
    // Generic case used for any VolumeNode. Used when Extensions add
    // node types that are subclasses of VolumeNode.
    volNode = dynamic_cast<vtkDMMLVolumeNode *>(refNode);
    }
  else
    {
  ...
  if ( refNode->IsA("vtkDMMLDiffusionTensorVolumeNode") )
    ...
    if ( ! (reader->GetPointDataType() == vtkDataSetAttributes::TENSORS))
      {
      vtkErrorMacro("ReadData: DMMLVolumeNode does not match file kind");
      return 0;
      }
    }
  else if ( refNode->IsA("vtkDMMLDiffusionWeightedVolumeNode"))
  ...
  vtkMatrix4x4 *mat2;
  if ( refNode->IsA("vtkDMMLTensorVolumeNode") )
    {
    ...
    if (mat2 == nullptr)
      ...
      {
      //dynamic_cast <vtkDMMLTensorVolumeNode *> (volNode)->SetMeasurementFrameMatrix(mat2);
      (vtkDMMLTensorVolumeNode::SafeDownCast(volNode))->SetMeasurementFrameMatrix(mat2);
      }
    }
  if ( refNode->IsA("vtkDMMLDiffusionWeightedVolumeNode") )
    {
    mat2 = reader->GetMeasurementFrameMatrix();
    if (mat2 == nullptr)
      {
      ...
      {
      //dynamic_cast <vtkDMMLTensorVolumeNode *> (volNode)->SetMeasurementFrameMatrix(mat2);
      (vtkDMMLDiffusionWeightedVolumeNode::SafeDownCast(volNode))->SetMeasurementFrameMatrix(mat2);
      }
    }
  ...
  if ( refNode->IsA("vtkDMMLDiffusionWeightedVolumeNode") )
    ...
    if (!this->ParseDiffusionInformation(reader.GetPointer(), grad.GetPointer(), bvalue.GetPointer()))
      {
      vtkErrorMacro("vtkDMMLDiffusionWeightedVolumeNode: Cannot parse Diffusion Information");
      return 0;
      }
    dynamic_cast <vtkDMMLDiffusionWeightedVolumeNode *> (volNode)->SetDiffusionGradients(grad.GetPointer());
    dynamic_cast <vtkDMMLDiffusionWeightedVolumeNode *> (volNode)->SetBValues(bvalue.GetPointer());
    }
...
//----------------------------------------------------------------------------
int vtkDMMLNRRDStorageNode::WriteDataInternal(vtkDMMLNode *refNode)
{
  vtkDMMLVolumeNode *volNode = nullptr;
  ...
  if ( refNode->IsA("vtkDMMLDiffusionTensorVolumeNode") )
    {
    volNode = vtkDMMLDiffusionTensorVolumeNode::SafeDownCast(refNode);
    if (volNode)
      {
      ((vtkDMMLDiffusionTensorVolumeNode *) volNode)->GetMeasurementFrameMatrix(mf.GetPointer());
      }
    }
  else if ( refNode->IsA("vtkDMMLDiffusionWeightedVolumeNode") )
    {

    volNode = vtkDMMLDiffusionWeightedVolumeNode::SafeDownCast(refNode);
    if (volNode)
      {
      ((vtkDMMLDiffusionWeightedVolumeNode *) volNode)->GetMeasurementFrameMatrix(mf.GetPointer());
      grads = ((vtkDMMLDiffusionWeightedVolumeNode *) volNode)->GetDiffusionGradients();
      bValues = ((vtkDMMLDiffusionWeightedVolumeNode *) volNode)->GetBValues();
      }
    }
  else if ( refNode->IsA("vtkDMMLVectorVolumeNode") )
    {
    volNode = vtkDMMLVectorVolumeNode::SafeDownCast(refNode);
    if (volNode)
      {
      ((vtkDMMLVectorVolumeNode *) volNode)->GetMeasurementFrameMatrix(mf.GetPointer());
      }
    }
  else if ( refNode->IsA("vtkDMMLScalarVolumeNode") )
    {
    volNode = vtkDMMLScalarVolumeNode::SafeDownCast(refNode);
    }
  else if ( refNode->IsA("vtkDMMLVolumeNode") )
    {
    // Generic case used for any VolumeNode. Used when Extensions add
    // node types that are subclasses of VolumeNode.
    volNode = vtkDMMLVolumeNode::SafeDownCast(refNode);
    }
  else
    {
...
bool parse_gradient_key(std::string key, size_t &grad_number, size_t &gradkey_pad_width, std::string err)
{
  ...
  // note: Cjyx no longer supports NRRDs with DWMRI_NEX_ keys. This was removed
  //       from Dicom2Nrrd in the following commit:
  //           Cjyx3/4 SVN: r26101, git-svn: 63a18f7d6900a
...
//----------------------------------------------------------------------------
int vtkDMMLNRRDStorageNode::ParseDiffusionInformation(
...
//----------------------------------------------------------------------------
void vtkDMMLNRRDStorageNode::InitializeSupportedReadFileTypes()
{
...
//----------------------------------------------------------------------------
void vtkDMMLNRRDStorageNode::InitializeSupportedWriteFileTypes()
{
...
//----------------------------------------------------------------------------
int vtkDMMLNRRDStorageNode::GetGzipCompressionLevelFromCompressionParameter(std::string compressionParameter)
{
...
//----------------------------------------------------------------------------
void vtkDMMLNRRDStorageNode::ConfigureForDataExchange()
{
