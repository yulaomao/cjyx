...
#ifndef __vtkDMMLSegmentationStorageNode_h
#define __vtkDMMLSegmentationStorageNode_h
...
/// \brief DMML node for segmentation storage on disk.
///
...
class VTK_DMML_EXPORT vtkDMMLSegmentationStorageNode : public vtkDMMLStorageNode
{
#ifdef SUPPORT_4D_SPATIAL_NRRD
...
public:
  static vtkDMMLSegmentationStorageNode *New();
  vtkTypeMacro(vtkDMMLSegmentationStorageNode, vtkDMMLStorageNode);
...
protected:
  ...
  vtkDMMLSegmentationNode* GetAssociatedDataNode();

  /// Write data from a referenced node
  int WriteDataInternal(vtkDMMLNode *refNode) override;

  /// Write binary labelmap representation to file
  virtual int WriteBinaryLabelmapRepresentation(vtkDMMLSegmentationNode* segmentationNode, std::string path);

  /// Write a poly data representation to file
  virtual int WritePolyDataRepresentation(vtkDMMLSegmentationNode* segmentationNode, std::string path);
  ...
  virtual int ReadBinaryLabelmapRepresentation(vtkDMMLSegmentationNode* segmentationNode, std::string path);

#ifdef SUPPORT_4D_SPATIAL_NRRD
  /// Read binary labelmap representation from 4D spatial nrrd file - obsolete
  virtual int ReadBinaryLabelmapRepresentation4DSpatial(vtkDMMLSegmentationNode* segmentationNode, std::string path);
#endif

  /// Read a poly data representation to file
  virtual int ReadPolyDataRepresentation(vtkDMMLSegmentationNode* segmentationNode, std::string path);
  ...
  static std::string GetSegmentColorAsString(vtkDMMLSegmentationNode* segmentationNode, const std::string& segmentId);
...
protected:
  vtkDMMLSegmentationStorageNode();
  ~vtkDMMLSegmentationStorageNode() override;

private:
  vtkDMMLSegmentationStorageNode(const vtkDMMLSegmentationStorageNode&) = delete;
  void operator=(const vtkDMMLSegmentationStorageNode&) = delete;
};
