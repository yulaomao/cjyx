...
#ifndef __vtkDMMLCoreTestingMacros_h
#define __vtkDMMLCoreTestingMacros_h
...
#define CHECK_NODE_IN_SCENE_BY_ID(scene, nodeID, expected) \
  { \
  if (!vtkDMMLCoreTestingUtilities::CheckNodeInSceneByID(__LINE__, (scene), (nodeID), (expected))) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_NODE_ID_AND_NAME(node, expectedID, expectedName) \
  { \
  if (!vtkDMMLCoreTestingUtilities::CheckNodeIdAndName(__LINE__, (node), (expectedID), (expectedName))) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define EXERCISE_BASIC_OBJECT_METHODS( node )                                        \
  {                                                                                  \
  int result = vtkDMMLCoreTestingUtilities::ExerciseBasicObjectMethods(node);        \
...
/// Tests all applicable common DMML node methods
#define EXERCISE_ALL_BASIC_DMML_METHODS( node )                                      \
  CHECK_EXIT_SUCCESS(vtkDMMLCoreTestingUtilities::ExerciseAllBasicDMMLMethods(node));

//---------------------------------------------------------------------------
// Deprecated macros, for backward compatibility only
// (className is no longer needed; usually EXERCISE_ALL_BASIC_DMML_METHODS can
// be used instead of all the macros below)

#define EXERCISE_BASIC_DMML_METHODS( className, node )                               \
  CHECK_EXIT_SUCCESS(vtkDMMLCoreTestingUtilities::ExerciseBasicDMMLMethods(node));

#define EXERCISE_BASIC_STORABLE_DMML_METHODS( className, node )                      \
  CHECK_EXIT_SUCCESS(vtkDMMLCoreTestingUtilities::ExerciseBasicStorableDMMLMethods(node));

#define EXERCISE_BASIC_TRANSFORMABLE_DMML_METHODS( className, node )                 \
  CHECK_EXIT_SUCCESS(vtkDMMLCoreTestingUtilities::ExerciseBasicTransformableDMMLMethods(node));

#define EXERCISE_BASIC_DISPLAYABLE_DMML_METHODS( className, node )                   \
  CHECK_EXIT_SUCCESS(vtkDMMLCoreTestingUtilities::ExerciseBasicDisplayableDMMLMethods(node));

#define EXERCISE_BASIC_DISPLAY_DMML_METHODS( className, node )                       \
  CHECK_EXIT_SUCCESS(vtkDMMLCoreTestingUtilities::ExerciseBasicDisplayDMMLMethods(node));

#define EXERCISE_BASIC_STORAGE_DMML_METHODS( className, node )                       \
  CHECK_EXIT_SUCCESS(vtkDMMLCoreTestingUtilities::ExerciseBasicStorageDMMLMethods(node));

#define EXERCISE_BASIC_TRANSFORM_DMML_METHODS( className, node )                     \
  CHECK_EXIT_SUCCESS(vtkDMMLCoreTestingUtilities::ExerciseBasicTransformDMMLMethods(node));

// Need to include vtkDMMLCoreTestingUtilities.h here because
// vtkDMMLCoreTestingUtilities use some of the macros above, too.
