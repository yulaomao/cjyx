...
//----------------------------------------------------------------------------
int vtkDMMLSliceLayerLogicTest(int , char * [] )
{
  // Test setting volume node and then scene
  {
    vtkNew<vtkDMMLSliceLayerLogic> logic;
    ...
    TEST_SET_GET_VALUE(logic, DMMLScene, DMMLScene.GetPointer());
  }
...
bool testDTIPipeline()
{
  ...
  vtkNew<vtkImageReslice> recjyx;
  recjyx->GenerateStencilOutputOn();
  recjyx->SetNumberOfThreads(1);
  recjyx->SetInputConnection(tensorsToScalars->GetOutputPort());
  recjyx->SetOutputExtentToDefault();
  recjyx->SetOutputDimensionality(2);

  vtkNew<vtkAssignAttribute> scalarsToTensors;
  scalarsToTensors->Assign(vtkDataSetAttributes::SCALARS,
                           vtkDataSetAttributes::TENSORS,
                           vtkAssignAttribute::POINT_DATA);
  scalarsToTensors->SetInputConnection(recjyx->GetOutputPort());
