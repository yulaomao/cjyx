...
//-----------------------------------------------------------------------------
int vtkDMMLApplicationLogicTest1(int argc, char *argv [])
{
  if (argc != 2)
    {
...
int SliceLogicsTest()
{
  vtkNew<vtkDMMLApplicationLogic> appLogic;
...
int SliceOrientationPresetInitializationTest()
{
...

  {
    ...
    vtkDMMLSliceNode * defaultSliceNode =
        vtkDMMLSliceNode::SafeDownCast(scene->GetDefaultNodeByClass("vtkDMMLSliceNode"));
...
int TemporaryPathTest()
{
  ...
  DMMLAppLogic->SetTemporaryPath(nullptr); // Test nullptr
  if (strlen(DMMLAppLogic->GetTemporaryPath()) != 0)
    {
    std::string temporaryEmptyString;
    std::cerr << "Line " << __LINE__ << " - Problem with SetTemporaryPath using NULL" << "\n"
              << "\ttemporaryPath:" << DMMLAppLogic->GetTemporaryPath() << "\n"
  ...
  TestDataType data;
  data.push_back("/tmp/cjyx");
  data.push_back("/tmp/cjyx/test");
  data.push_back("/tmp/cjyx/test"); // Test same path as before
  data.push_back("c:\\path\\to\\temp\\cjyx");
  ...
  for(TestDataType::size_type rowIdx = 0; rowIdx < data.size(); ++rowIdx)
    ...
    DMMLAppLogic->SetTemporaryPath(temporaryPath.c_str());
    if (DMMLAppLogic->GetTemporaryPath() != temporaryPath)
      {
...
int AddModuleLogicTest()
{
  ...
  {
    vtkNew<vtkDMMLAbstractLogic> moduleLogic;
  ...
  {
    const vtkDMMLAbstractLogic* retval = appLogic->GetModuleLogic((module_name+"a").c_str());
  ...
  // Passing nullptr to GetModuleLogic should trigger an error and return nullptr
  {
    ...
    const vtkDMMLAbstractLogic* retval = appLogic->GetModuleLogic(nullptr);
  ...
  // Removing an already registered module logic should succeed
  {
    ...
    const vtkDMMLAbstractLogic* retval = appLogic->GetModuleLogic((module_name).c_str());
    CHECK_NULL(retval);
  }
