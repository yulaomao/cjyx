...
int vtkDMMLDisplayableHierarchyLogicTest1(int , char * [] )
{
  vtkNew<vtkDMMLScene> scene;
  vtkDMMLDisplayableHierarchyLogic* displayableHierarchyLogic = vtkDMMLDisplayableHierarchyLogic::New();
  displayableHierarchyLogic->SetDebug(true);

  displayableHierarchyLogic->SetDMMLScene(scene.GetPointer());
  ...
  vtkDMMLModelNode *m1 = vtkDMMLModelNode::New();
  scene->AddNode(m1);
  vtkDMMLModelNode *m2 = vtkDMMLModelNode::New();
  scene->AddNode(m2);
  ...
  vtkDMMLHierarchyNode* h1 = vtkDMMLHierarchyNode::GetAssociatedHierarchyNode(scene.GetPointer(), m1->GetID());
  if (!h1)
    {
  ...
  vtkDMMLHierarchyNode* h2 = vtkDMMLHierarchyNode::GetAssociatedHierarchyNode(scene.GetPointer(), m2->GetID());
  if (!h2)
    {
  ...
  bool retval = displayableHierarchyLogic->DeleteHierarchyNodeAndChildren(vtkDMMLDisplayableHierarchyNode::SafeDownCast(h2));
  if (!retval)
    {
