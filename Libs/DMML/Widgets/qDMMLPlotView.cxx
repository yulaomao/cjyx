...
//--------------------------------------------------------------------------
// qDMMLPlotViewPrivate methods

//---------------------------------------------------------------------------
qDMMLPlotViewPrivate::qDMMLPlotViewPrivate(qDMMLPlotView& object)
  : q_ptr(&object)
{
  this->DMMLScene = nullptr;
  this->DMMLPlotViewNode = nullptr;
  this->DMMLPlotChartNode = nullptr;
  //this->PinButton = 0;
//  this->PopupWidget = 0;
  this->UpdatingWidgetFromDMML = false;
}

//---------------------------------------------------------------------------
qDMMLPlotViewPrivate::~qDMMLPlotViewPrivate() = default;

//---------------------------------------------------------------------------
void qDMMLPlotViewPrivate::init()
{
  Q_Q(qDMMLPlotView);
  ...
  qvtkConnect(q->chart(), vtkCommand::InteractionEvent, q, SLOT(updateDMMLChartAxisRangeFromWidget()));
...
//---------------------------------------------------------------------------
void qDMMLPlotViewPrivate::setDMMLScene(vtkDMMLScene* newScene)
{
...
// --------------------------------------------------------------------------
vtkDMMLPlotSeriesNode* qDMMLPlotViewPrivate::plotSeriesNodeFromPlot(vtkPlot* plot)
{
  if (plot == nullptr)
    {
    return nullptr;
  ...
  vtkDMMLPlotSeriesNode* plotSeriesNode = vtkDMMLPlotSeriesNode::SafeDownCast(this->dmmlScene()->GetNodeByID(plotSeriesNodeID.toUtf8().constData()));
...
// --------------------------------------------------------------------------
void qDMMLPlotViewPrivate::adjustRangeForLogScale(double range[2], double computedLimit[2])
{
...
// --------------------------------------------------------------------------
vtkSmartPointer<vtkPlot> qDMMLPlotViewPrivate::updatePlotFromPlotSeriesNode(vtkDMMLPlotSeriesNode* plotSeriesNode, vtkPlot* existingPlot)
{
  if (plotSeriesNode == nullptr)
    {
    return nullptr;
    }
  vtkDMMLTableNode* tableNode = plotSeriesNode->GetTableNode();
  ...
  switch (plotType)
    {
    case vtkDMMLPlotSeriesNode::PlotTypeScatter:
    case vtkDMMLPlotSeriesNode::PlotTypeLine:
      if (!existingPlot || !existingPlot->IsA("vtkPlotLine"))
        {
        newPlot = vtkSmartPointer<vtkPlotLine>::New();
        }
      break;
    case vtkDMMLPlotSeriesNode::PlotTypeBar:
    case vtkDMMLPlotSeriesNode::PlotTypeScatterBar:
  ...
  if (newPlot->GetPen())
    ...
    if (plotType == vtkDMMLPlotSeriesNode::PlotTypeBar)
      {
      ...
      {
      case vtkDMMLPlotSeriesNode::LineStyleNone: lineStyleVtk = vtkPen::NO_PEN; break;
      case vtkDMMLPlotSeriesNode::LineStyleSolid: lineStyleVtk = vtkPen::SOLID_LINE; break;
      case vtkDMMLPlotSeriesNode::LineStyleDash: lineStyleVtk = vtkPen::DASH_LINE; break;
      case vtkDMMLPlotSeriesNode::LineStyleDot: lineStyleVtk = vtkPen::DOT_LINE; break;
      case vtkDMMLPlotSeriesNode::LineStyleDashDot: lineStyleVtk = vtkPen::DASH_DOT_LINE; break;
      case vtkDMMLPlotSeriesNode::LineStyleDashDotDot: lineStyleVtk = vtkPen::DASH_DOT_DOT_LINE; break;
      default:
  ...
  if (plotLine)
    ...
    switch (plotSeriesNode->GetMarkerStyle())
      {
      case vtkDMMLPlotSeriesNode::MarkerStyleNone: markerStyleVtk = VTK_MARKER_NONE; break;
      case vtkDMMLPlotSeriesNode::MarkerStyleCross: markerStyleVtk = VTK_MARKER_CROSS; break;
      case vtkDMMLPlotSeriesNode::MarkerStylePlus: markerStyleVtk = VTK_MARKER_PLUS; break;
      case vtkDMMLPlotSeriesNode::MarkerStyleSquare: markerStyleVtk = VTK_MARKER_SQUARE; break;
      case vtkDMMLPlotSeriesNode::MarkerStyleCircle: markerStyleVtk = VTK_MARKER_CIRCLE; break;
      case vtkDMMLPlotSeriesNode::MarkerStyleDiamond: markerStyleVtk = VTK_MARKER_DIAMOND; break;
      default:
...
// --------------------------------------------------------------------------
void qDMMLPlotViewPrivate::startProcessing()
{
}

//
// --------------------------------------------------------------------------
void qDMMLPlotViewPrivate::endProcessing()
{
  this->updateWidgetFromDMML();
}

// --------------------------------------------------------------------------
vtkDMMLScene* qDMMLPlotViewPrivate::dmmlScene()
{
  return this->DMMLScene;
}

// --------------------------------------------------------------------------
void qDMMLPlotViewPrivate::onPlotChartNodeChanged()
{
  vtkDMMLPlotChartNode *newPlotChartNode = nullptr;

  if (this->DMMLScene && this->DMMLPlotViewNode && this->DMMLPlotViewNode->GetPlotChartNodeID())
    {
    newPlotChartNode = vtkDMMLPlotChartNode::SafeDownCast
      (this->DMMLScene->GetNodeByID(this->DMMLPlotViewNode->GetPlotChartNodeID()));
    }

  this->qvtkReconnect(this->DMMLPlotChartNode, newPlotChartNode,
    vtkCommand::ModifiedEvent, this, SLOT(updateWidgetFromDMML()));

  this->DMMLPlotChartNode = newPlotChartNode;

  this->updateWidgetFromDMML();
}

// --------------------------------------------------------------------------
void qDMMLPlotViewPrivate::RecalculateBounds()
{
  Q_Q(qDMMLPlotView);
  ...
  q->updateDMMLChartAxisRangeFromWidget();
}

// --------------------------------------------------------------------------
void qDMMLPlotViewPrivate::switchInteractionMode()
{
  Q_Q(qDMMLPlotView);
  ...
  int interactionMode = this->DMMLPlotViewNode->GetInteractionMode();
  interactionMode = (interactionMode + 1) % vtkDMMLPlotViewNode::InteractionMode_Last;
  this->DMMLPlotViewNode->SetInteractionMode(interactionMode);
}

// --------------------------------------------------------------------------
void qDMMLPlotViewPrivate::emitSelection()
{
  Q_Q(qDMMLPlotView);
  ...
  const char *PlotChartNodeID = this->DMMLPlotViewNode->GetPlotChartNodeID();

  vtkDMMLPlotChartNode* plotChartNode = vtkDMMLPlotChartNode::SafeDownCast
    (this->DMMLScene->GetNodeByID(PlotChartNodeID));
  if (!plotChartNode)
    {
    return;
    }

  vtkNew<vtkStringArray> dmmlPlotSeriesIDs;
  ...
  for (int plotIndex = 0; plotIndex < q->chart()->GetNumberOfPlots(); plotIndex++)
    ...
    if (selection->GetNumberOfValues() > 0)
      ...
      vtkDMMLPlotSeriesNode* plotSeriesNode = this->plotSeriesNodeFromPlot(plot);
      if (plotSeriesNode)
        {
        // valid plot data node found
        dmmlPlotSeriesIDs->InsertNextValue(plotSeriesNode->GetID());
        }
      }
    }
  ...
  // emit the signal
  emit q->dataSelected(dmmlPlotSeriesIDs.GetPointer(), selectionCol.GetPointer());
}

// --------------------------------------------------------------------------
void qDMMLPlotViewPrivate::updateWidgetFromDMML()
{
  Q_Q(qDMMLPlotView);

  if (!this->DMMLScene || !this->DMMLPlotViewNode
    || !q->isEnabled() || !q->chart() || !q->chart()->GetLegend())
    {
    return;
    }

  if (this->UpdatingWidgetFromDMML)
    {
    return;
    }
  this->UpdatingWidgetFromDMML = true;

  // Set interaction mode
  int interactionMode = this->DMMLPlotViewNode->GetInteractionMode();
  switch (interactionMode)
  {
  case vtkDMMLPlotViewNode::InteractionModePanView:
    ...
    break;
  case vtkDMMLPlotViewNode::InteractionModeSelectPoints:
    q->chart()->SetClickActionToButton(vtkChart::SELECT, vtkContextMouseEvent::LEFT_BUTTON);
    ...
    q->chart()->SetActionToButton(vtkChart::ZOOM_AXIS, vtkContextMouseEvent::RIGHT_BUTTON);
    break;
  case vtkDMMLPlotViewNode::InteractionModeFreehandSelectPoints:
    q->chart()->SetClickActionToButton(vtkChart::SELECT, vtkContextMouseEvent::LEFT_BUTTON);
    q->chart()->SetActionToButton(vtkChart::SELECT_POLYGON, vtkContextMouseEvent::LEFT_BUTTON);
    q->chart()->SetActionToButton(vtkChart::PAN, vtkContextMouseEvent::MIDDLE_BUTTON);
    q->chart()->SetActionToButton(vtkChart::ZOOM_AXIS, vtkContextMouseEvent::RIGHT_BUTTON);
    break;
  case vtkDMMLPlotViewNode::InteractionModeMovePoints:
  ...
  const char *plotChartNodeID = this->DMMLPlotViewNode->GetPlotChartNodeID();
  vtkDMMLPlotChartNode* plotChartNode = vtkDMMLPlotChartNode::SafeDownCast(this->DMMLScene->GetNodeByID(plotChartNodeID));
  if (!plotChartNode)
    {
  ...
  q->chart()->SetDragPointAlongX(this->DMMLPlotViewNode->GetEnablePointMoveAlongX()
    && plotChartNode->GetEnablePointMoveAlongX());
  q->chart()->SetDragPointAlongY(this->DMMLPlotViewNode->GetEnablePointMoveAlongY()
  ...
  vtkSmartPointer<vtkCollection> allPlotSeriesNodesInScene = vtkSmartPointer<vtkCollection>::Take
    (this->dmmlScene()->GetNodesByClass("vtkDMMLPlotSeriesNode"));
  ...
  std::set< vtkDMMLPlotSeriesNode* > plotSeriesNodesNotToAdd;
  ...
  for (std::vector<std::string>::iterator it = plotSeriesNodesIDs.begin(); it != plotSeriesNodesIDs.end(); ++it)
    {
    vtkDMMLPlotSeriesNode* plotSeriesNode = vtkDMMLPlotSeriesNode::SafeDownCast(this->dmmlScene()->GetNodeByID(it->c_str()));
  ...
  for (unsigned int axisIndex = 0; axisIndex < numberOfAxisIDs; ++axisIndex)
    ...
    bool dmmlLogScale = false;
    bool dmmlRangeAuto = true;
    double dmmlRange[2] = { 0.0, 1.0 };
    if (axisID == vtkAxis::BOTTOM || axisID == vtkAxis::TOP)
      ...
      dmmlLogScale = plotChartNode->GetXAxisLogScale();
      dmmlRangeAuto = plotChartNode->GetXAxisRangeAuto();
      plotChartNode->GetXAxisRange(dmmlRange);
      }
      ...
      dmmlLogScale = plotChartNode->GetYAxisLogScale();
      dmmlRangeAuto = plotChartNode->GetYAxisRangeAuto();
      plotChartNode->GetYAxisRange(dmmlRange);
      }

    if (dmmlRangeAuto)
      {
      // Automatic range
      if (axis->GetLogScale() != dmmlLogScale)
        {
        double unscaledRange[2] = { 0.0, 1.0 };
        double limit[2] = { -vtkMath::Inf(), vtkMath::Inf() };
        axis->GetUnscaledRange(unscaledRange);
        axis->SetLogScale(dmmlLogScale);
        if (dmmlLogScale)
          {
      ...
      if (dmmlLogScale)
        {
        this->adjustRangeForLogScale(dmmlRange, limit);
        }
      axis->SetUnscaledMinimumLimit(limit[0]);
      axis->SetUnscaledMaximumLimit(limit[1]);
      axis->SetUnscaledRange(dmmlRange);
      axis->Update();
      }
...
// --------------------------------------------------------------------------
// qDMMLPlotView methods

// --------------------------------------------------------------------------
qDMMLPlotView::qDMMLPlotView(QWidget* _parent) : Superclass(_parent)
  , d_ptr(new qDMMLPlotViewPrivate(*this))
{
  Q_D(qDMMLPlotView);
  d->init();
}
...
//------------------------------------------------------------------------------
void qDMMLPlotView::setDMMLScene(vtkDMMLScene* newScene)
{
  Q_D(qDMMLPlotView);
  ...
  if (d->DMMLPlotViewNode && newScene != d->DMMLPlotViewNode->GetScene())
    {
    this->setDMMLPlotViewNode(nullptr);
    }
...
//---------------------------------------------------------------------------
void qDMMLPlotView::setDMMLPlotViewNode(vtkDMMLPlotViewNode* newPlotViewNode)
{
  Q_D(qDMMLPlotView);
  if (d->DMMLPlotViewNode == newPlotViewNode)
    {
    return;
    }

  // connect modified event on PlotViewNode to updating the widget
  d->qvtkReconnect(d->DMMLPlotViewNode, newPlotViewNode,
    vtkDMMLPlotViewNode::PlotChartNodeChangedEvent, d, SLOT(updateWidgetFromDMML()));
  ...
  d->qvtkReconnect(d->DMMLPlotViewNode, newPlotViewNode,
    vtkDMMLPlotViewNode::PlotChartNodeChangedEvent, d, SLOT(onPlotChartNodeChanged()));

  // cache the PlotViewNode
  d->DMMLPlotViewNode = newPlotViewNode;
...
//---------------------------------------------------------------------------
vtkDMMLPlotViewNode* qDMMLPlotView::dmmlPlotViewNode()const
{
  Q_D(const qDMMLPlotView);
  return d->DMMLPlotViewNode;
}

//---------------------------------------------------------------------------
vtkDMMLScene* qDMMLPlotView::dmmlScene()const
{
  Q_D(const qDMMLPlotView);
  return d->DMMLScene;
}
...
//---------------------------------------------------------------------------
QSize qDMMLPlotView::sizeHint()const
{
...
// --------------------------------------------------------------------------
void qDMMLPlotView::keyPressEvent(QKeyEvent *event)
{
  Q_D(qDMMLPlotView);
...
// --------------------------------------------------------------------------
void qDMMLPlotView::keyReleaseEvent(QKeyEvent *event)
{
  Q_D(qDMMLPlotView);
  this->Superclass::keyPressEvent(event);
}
...
// --------------------------------------------------------------------------
void qDMMLPlotView::fitToContent()
{
  Q_D(qDMMLPlotView);
  d->RecalculateBounds();
  // Repaint the chart scene
...
// --------------------------------------------------------------------------
void qDMMLPlotView::RemovePlotSelections()
{
  if (!this->chart())
    {
    return;
...
// --------------------------------------------------------------------------
void qDMMLPlotView::updateDMMLChartAxisRangeFromWidget()
{
  Q_D(qDMMLPlotView);
  ...
  if (d->UpdatingWidgetFromDMML)
    {
    return;
    }
  int wasModified = d->DMMLPlotChartNode->StartModify();
  // Setting Axes
  ...
  if (bottomAxis)
    ...
    d->DMMLPlotChartNode->SetXAxisRange(range);
    }
  ...
  if (leftAxis)
    ...
    d->DMMLPlotChartNode->SetYAxisRange(range);
    }
  d->DMMLPlotChartNode->EndModify(wasModified);
}

// ----------------------------------------------------------------------------
void qDMMLPlotView::saveAsSVG(const QString & fileName)
{
