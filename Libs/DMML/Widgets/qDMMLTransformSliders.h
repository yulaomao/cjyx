...
#ifndef __qDMMLTransformSliders_h
#define __qDMMLTransformSliders_h
...
class vtkMatrix4x4;
class qDMMLTransformSlidersPrivate;

class QDMML_WIDGETS_EXPORT qDMMLTransformSliders : public qDMMLWidget
{
  Q_OBJECT
  QVTK_OBJECT
...
public:
  ...
  explicit qDMMLTransformSliders(QWidget* parent);
  ~qDMMLTransformSliders() override;
...
protected slots:
  ...
  ///
  /// Triggered upon DMML node transform modified events
  void onDMMLTransformNodeModified(vtkObject* caller);

protected:
  QScopedPointer<qDMMLTransformSlidersPrivate> d_ptr;
  ...
  void updateRangeFromTransform(vtkDMMLTransformNode* transformNode);
...
private:
  Q_DECLARE_PRIVATE(qDMMLTransformSliders);
  Q_DISABLE_COPY(qDMMLTransformSliders);
};
