...
//------------------------------------------------------------------------------
qDMMLItemDelegate::qDMMLItemDelegate(QObject *parent)
...
//------------------------------------------------------------------------------
qDMMLItemDelegate::~qDMMLItemDelegate()
{
...
//------------------------------------------------------------------------------
bool qDMMLItemDelegate::isColor(const QModelIndex& index)const
{
  ...
  if (editData.isNull() &&
    ...
    return true;
            ...
            index.data(qDMMLColorModel::ColorRole).type() == QVariant::Color )
    {
    return true;
    }
  return false;
...
//------------------------------------------------------------------------------
int qDMMLItemDelegate::colorRole(const QModelIndex& index)const
{
  QVariant decorationData = index.data(Qt::DecorationRole);
  QVariant colorData = index.data(qDMMLColorModel::ColorRole);
  if (decorationData.type() == QVariant::Color)
    ...
    {
    return qDMMLColorModel::ColorRole;
    }
  return -1;
}

//------------------------------------------------------------------------------
bool qDMMLItemDelegate::is0To1Value(const QModelIndex& index)const
{
  QVariant editData = index.data(Qt::EditRole);
...
//------------------------------------------------------------------------------
void qDMMLItemDelegate::setEditorData(QWidget *editor,
                                      const QModelIndex &index) const
{
  if (this->isColor(index))
    {
...
//------------------------------------------------------------------------------
void qDMMLItemDelegate::setModelData(QWidget *editor, QAbstractItemModel *model,
                                   const QModelIndex &index) const
{
  if (this->isColor(index))
    {
...
//------------------------------------------------------------------------------
void qDMMLItemDelegate::commitSenderData()
{
...
//------------------------------------------------------------------------------
void qDMMLItemDelegate::commitAndClose()
{
  QWidget* editor = qobject_cast<QWidget*>(this->sender());
  emit commitData(editor);
...
//------------------------------------------------------------------------------
bool qDMMLItemDelegate::eventFilter(QObject *object, QEvent *event)
{
