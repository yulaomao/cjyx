...
#ifndef __qDMMLTreeView_h
#define __qDMMLTreeView_h
...
class qDMMLTreeViewPrivate;
class vtkDMMLNode;
class vtkDMMLScene;

/// \todo Rename to qDMMLSceneTreeView
/// In debug mode, pressing the '!' key on the view switches of
/// qDMMLSortFilterProxyModel::filterType, it can be useful to debug the scene model
/// and filters applied to them.
class QDMML_WIDGETS_EXPORT qDMMLTreeView : public QTreeView
{
  Q_OBJECT
  QVTK_OBJECT
  /// This property controls what type of scene representation
  /// (which qDMMLSceneModel implementation/subclass)is used to populate
  /// the nodes of the scene.
  /// Some built-in model types are available :
  /// - "" -> qDMMLSceneModel
  /// - "Transform" -> qDMMLSceneTransformModel
  /// - "Displayable" -> qDMMLSceneDisplayableModel
  ...
  /// \sa qDMMLSceneModel::listenNodeModifiedEvent
  Q_PROPERTY(qDMMLSceneModel::NodeTypes listenNodeModifiedEvent READ listenNodeModifiedEvent WRITE setListenNodeModifiedEvent)
  ...
  /// list will be hidden.
  /// For example, a value of "qDMMLSceneDisplayableModel", "vtkDMMLModelNode"
  ...
  /// \sa qDMMLSortFilterProxyModel::nodeTypes
  ...
  /// vtkDMMLNode::HideFromEditors set to true are visible in the view.
  /// False by default (only nodes with HideFromEditors = 0 are visible)
  /// \sa vtkDMMLNode::GetHideFromEditors()
  ...
  /// Hidden by default.
  /// Don't use qDMMLSortFilterProxyModel::HideNodesUnaffiliatedWithNodeID if
...
public:
  ...
  qDMMLTreeView(QWidget *parent=nullptr);
  ~qDMMLTreeView() override;
  ...
  vtkDMMLNode* currentNode()const;

  /// Could be "", "Transform", "Displayable" or the type
  /// associated with the scene model added with qDMMLTreeView::setSceneModel()
  /// \sa setSceneModel()
  QString sceneModelType()const;

  /// Set the node types on the scene model.
  /// \sa listenNodeModifiedEvent qDMMLSceneModel::setListenNodeModifiedEvent
  void setListenNodeModifiedEvent(qDMMLSceneModel::NodeTypes listen);
  /// Get the node types of the scene model.
  /// \sa qDMMLSceneModel::listenNodeModifiedEvent
  qDMMLSceneModel::NodeTypes listenNodeModifiedEvent()const;

  /// Customize the model
  void setSceneModel(qDMMLSceneModel* newSceneModel, const QString& modelType);
  /// Customize the filter model
  void setSortFilterProxyModel(qDMMLSortFilterProxyModel* newFilterModel);
  ...
  Q_INVOKABLE qDMMLSceneModel* sceneModel()const;
...
public slots:
  ...
  void setRootNode(vtkDMMLNode* root);
  ...
  /// \sa setHideAll(), qDMMLSortFilterProxyModel::FilterType
  inline void setShowAll(bool);
  /// Bypass all the filters on the view and hide all the nodes.
  /// \sa showAll(), setDontHideAll(), qDMMLSortFilterProxyModel::FilterType
...
signals:
  ...
  void editNodeRequested(vtkDMMLNode* node);
...
protected:
  qDMMLTreeView(qDMMLTreeViewPrivate* pimpl, QWidget *parent=nullptr);

protected:
  QScopedPointer<qDMMLTreeViewPrivate> d_ptr;
...
private:
  Q_DECLARE_PRIVATE(qDMMLTreeView);
  Q_DISABLE_COPY(qDMMLTreeView);
};

// --------------------------------------------------------------------------
void qDMMLTreeView::setShowHidden(bool enable)
{
  this->sortFilterProxyModel()->setShowHidden(enable);
}

// --------------------------------------------------------------------------
bool qDMMLTreeView::showHidden()const
{
  return this->sortFilterProxyModel()->showHidden();
}

// --------------------------------------------------------------------------
void qDMMLTreeView::setShowAll(bool show)
{
  this->sortFilterProxyModel()->setShowAll(show);
}

// --------------------------------------------------------------------------
void qDMMLTreeView::setHideAll(bool hide)
{
  this->sortFilterProxyModel()->setHideAll(hide);
}
#include <QDebug>
// --------------------------------------------------------------------------
void qDMMLTreeView::setDontHideAll(bool dontHide)
{
