...
#ifndef __qDMMLUtils_h
#define __qDMMLUtils_h
...
class QDMML_WIDGETS_EXPORT qDMMLUtils : public QObject
{
  Q_OBJECT;
public:
  typedef qDMMLUtils Self;
  qDMMLUtils(QObject* parent = nullptr);
  ~qDMMLUtils() override;
  ...
  ///
  Q_INVOKABLE static void getTransformInCoordinateSystem(vtkDMMLNode* transformNode, bool global,
    vtkTransform* transform);
  Q_INVOKABLE static void getTransformInCoordinateSystem(vtkDMMLTransformNode* transformNode,
    bool global, vtkTransform* transform);

  /// Retrieve the number of visible view node associated with \a scene
  Q_INVOKABLE static int countVisibleViewNode(vtkDMMLScene* scene);
