...
#include "ui_qDMMLVolumeInfoWidget.h"
...
//------------------------------------------------------------------------------
class qDMMLVolumeInfoWidgetPrivate: public Ui_qDMMLVolumeInfoWidget
{
  Q_DECLARE_PUBLIC(qDMMLVolumeInfoWidget);

protected:
  qDMMLVolumeInfoWidget* const q_ptr;

public:
  qDMMLVolumeInfoWidgetPrivate(qDMMLVolumeInfoWidget& object);
  void init();

  vtkWeakPointer<vtkDMMLVolumeNode> VolumeNode;
};

//------------------------------------------------------------------------------
qDMMLVolumeInfoWidgetPrivate::qDMMLVolumeInfoWidgetPrivate(qDMMLVolumeInfoWidget& object)
  : q_ptr(&object)
{
}

//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidgetPrivate::init()
{
  Q_Q(qDMMLVolumeInfoWidget);
  this->setupUi(q);
...
//------------------------------------------------------------------------------
qDMMLVolumeInfoWidget::qDMMLVolumeInfoWidget(QWidget *_parent)
  : Superclass(_parent)
  , d_ptr(new qDMMLVolumeInfoWidgetPrivate(*this))
{
  Q_D(qDMMLVolumeInfoWidget);
  d->init();
}

//------------------------------------------------------------------------------
qDMMLVolumeInfoWidget::~qDMMLVolumeInfoWidget() = default;


//------------------------------------------------------------------------------
vtkDMMLVolumeNode* qDMMLVolumeInfoWidget::volumeNode()const
{
  Q_D(const qDMMLVolumeInfoWidget);
  return d->VolumeNode;
}

//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setVolumeNode(vtkDMMLNode* node)
{
  this->setVolumeNode(vtkDMMLVolumeNode::SafeDownCast(node));
}

//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setVolumeNode(vtkDMMLVolumeNode* volumeNode)
{
  Q_D(qDMMLVolumeInfoWidget);
  qvtkReconnect(d->VolumeNode, volumeNode, vtkCommand::ModifiedEvent,
                this, SLOT(updateWidgetFromDMML()));
  qvtkReconnect(d->VolumeNode, volumeNode, vtkDMMLVolumeNode::ImageDataModifiedEvent,
                this, SLOT(updateWidgetFromDMML()));
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setDataTypeEditable(bool enable)
{
  Q_D(qDMMLVolumeInfoWidget);
...
//------------------------------------------------------------------------------
bool qDMMLVolumeInfoWidget::isDataTypeEditable()const
{
  Q_D(const qDMMLVolumeInfoWidget);
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::updateWidgetFromDMML()
{
  Q_D(qDMMLVolumeInfoWidget);
  ...
  d->ScanOrderComboBox->setCurrentIndex(d->ScanOrderComboBox->findData(
    vtkDMMLVolumeNode::ComputeScanOrderFromIJKToRAS(mat.GetPointer())));
  ...
  vtkDMMLStorageNode* storageNode = d->VolumeNode->GetStorageNode();
  d->FileNameLineEdit->setText(storageNode ? storageNode->GetFileName() : "");

  vtkDMMLScalarVolumeNode *scalarNode = vtkDMMLScalarVolumeNode::SafeDownCast( d->VolumeNode );
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setImageSpacing(double* spacing)
{
  Q_D(qDMMLVolumeInfoWidget);
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setImageOrigin(double* origin)
{
  Q_D(qDMMLVolumeInfoWidget);
  if (d->VolumeNode == nullptr)
    {
...
//------------------------------------------------------------------------------
bool qDMMLVolumeInfoWidget::isCentered()const
{
  Q_D(const qDMMLVolumeInfoWidget);
  if (!d->VolumeNode)
    {
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::center()
{
  Q_D(qDMMLVolumeInfoWidget);
  if (d->VolumeNode)
    {
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setScanOrder(int index)
{
  Q_D(qDMMLVolumeInfoWidget);
  if (d->VolumeNode == nullptr)
    {
  ...
  if (vtkDMMLVolumeNode::ComputeIJKToRASFromScanOrder(
    scanOrder.toUtf8(),
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setNumberOfScalars(int number)
{
  Q_D(qDMMLVolumeInfoWidget);
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setScalarType(int index)
{
  Q_D(qDMMLVolumeInfoWidget);
  vtkImageData* imageData = d->VolumeNode ? d->VolumeNode->GetImageData() : nullptr;
  if (imageData == nullptr)
...
//------------------------------------------------------------------------------
void qDMMLVolumeInfoWidget::setWindowLevelFromPreset(QListWidgetItem *presetItem)
{
  Q_D(qDMMLVolumeInfoWidget);
  vtkDMMLScalarVolumeNode *scalarNode = vtkDMMLScalarVolumeNode::SafeDownCast( d->VolumeNode );
  vtkDMMLScalarVolumeDisplayNode *displayNode = scalarNode ? scalarNode->GetScalarVolumeDisplayNode() : nullptr;
