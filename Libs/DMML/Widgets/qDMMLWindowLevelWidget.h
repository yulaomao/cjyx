...
#ifndef __qDMMLWindowLevelWidget_h
#define __qDMMLWindowLevelWidget_h
...
class QDMML_WIDGETS_EXPORT qDMMLWindowLevelWidget
  : public qDMMLVolumeWidget
{
  Q_OBJECT
...
public:
  /// Constructors
  typedef qDMMLVolumeWidget Superclass;
  explicit qDMMLWindowLevelWidget(QWidget* parentWidget = nullptr);
  ~qDMMLWindowLevelWidget() override;
...
protected:
  ...
  void updateWidgetFromDMMLDisplayNode() override;

  /// Special initial updates that are done only when switching between volumes.
  void updateWidgetFromDMMLVolumeNode() override;

private:
  Q_DECLARE_PRIVATE(qDMMLWindowLevelWidget);
  Q_DISABLE_COPY(qDMMLWindowLevelWidget);
};
