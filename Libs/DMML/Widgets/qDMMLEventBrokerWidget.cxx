...
#include "qDMMLWidgetsConfigure.h"
...
//------------------------------------------------------------------------------
qDMMLEventBrokerWidgetPrivate::qDMMLEventBrokerWidgetPrivate()
{
  this->ConnectionsTreeWidget = nullptr;
}

//------------------------------------------------------------------------------
vtkObservation* qDMMLEventBrokerWidgetPrivate::observation(QTreeWidgetItem* item)const
{
...
//------------------------------------------------------------------------------
QTreeWidgetItem* qDMMLEventBrokerWidgetPrivate::itemFromSubject(vtkObject* subject)const
{
  if (!subject)
    {
    return nullptr;
...
//------------------------------------------------------------------------------
QTreeWidgetItem* qDMMLEventBrokerWidgetPrivate::itemFromEvent(QTreeWidgetItem* subjectItem, unsigned long long event)const
{
  if (!subjectItem)
    {
    return nullptr;
...
//------------------------------------------------------------------------------
QTreeWidgetItem* qDMMLEventBrokerWidgetPrivate::itemFromObservation(vtkObservation* observation)const
{
  if (!observation)
    {
    return nullptr;
...
//------------------------------------------------------------------------------
void qDMMLEventBrokerWidgetPrivate::setObjectToItem(QTreeWidgetItem* item, vtkObject* object)const
{
  item->setData(NameColumn, Qt::UserRole,
    QVariant::fromValue(reinterpret_cast<long long>(object)));
  vtkDMMLNode* node = vtkDMMLNode::SafeDownCast(object);
  if (node)
    {
    // vtkDMMLNode
...
//------------------------------------------------------------------------------
vtkObject* qDMMLEventBrokerWidgetPrivate::objectFromItem(QTreeWidgetItem* item)const
{
  if (!item)
    {
    return nullptr;
...
//------------------------------------------------------------------------------
void qDMMLEventBrokerWidgetPrivate::showItem(QTreeWidgetItem* item)
{
  if (!item)
    {
    return;
...
//------------------------------------------------------------------------------
void qDMMLEventBrokerWidgetPrivate::addObservation(vtkObservation* observation)
{
  // Observation
...
//------------------------------------------------------------------------------
void qDMMLEventBrokerWidgetPrivate::setupUi(QWidget* parentWidget)
{
...
//------------------------------------------------------------------------------
qDMMLEventBrokerWidget::qDMMLEventBrokerWidget(QWidget* parentWidget)
  : Superclass(parentWidget)
  , d_ptr(new qDMMLEventBrokerWidgetPrivate)
{
  Q_D(qDMMLEventBrokerWidget);

  d->setupUi(this);
}

//------------------------------------------------------------------------------
qDMMLEventBrokerWidget::~qDMMLEventBrokerWidget() = default;

//------------------------------------------------------------------------------
void qDMMLEventBrokerWidget::refresh()
{
  Q_D(qDMMLEventBrokerWidget);
...
//------------------------------------------------------------------------------
void qDMMLEventBrokerWidget::onItemChanged(QTreeWidgetItem* item, int column)
{
  Q_D(qDMMLEventBrokerWidget);
...
//------------------------------------------------------------------------------
void qDMMLEventBrokerWidget::onCurrentItemChanged(QTreeWidgetItem* currentItem)
{
  Q_D(qDMMLEventBrokerWidget);
...
//------------------------------------------------------------------------------
void qDMMLEventBrokerWidget::resetElapsedTimes()
{
  vtkEventBroker* eventBroker = vtkEventBroker::GetInstance();
  for (int i = 0; i < eventBroker->GetNumberOfObservations(); ++i)
    {
...
//------------------------------------------------------------------------------
void qDMMLEventBrokerWidget::expandElapsedTimeItems()
{
  Q_D(qDMMLEventBrokerWidget);
  this->refresh();
  vtkEventBroker* eventBroker = vtkEventBroker::GetInstance();
