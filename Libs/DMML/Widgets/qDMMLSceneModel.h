...
#ifndef __qDMMLSceneModel_h
#define __qDMMLSceneModel_h
...
class qDMMLSceneModelPrivate;

/// qDMMLSceneModel has at least 1 column with the scene as a top level item.
...
///
///  - Scene
///    |- ViewNode        vtkDMMLViewNode1
///    |- CameraNode      vtkDMMLCameraNode1
///    ...
///    |- my_volume.nrrd  vtkDMMLScalarVolumeNode1
///    |- Add new node
/// \endverbatim
...
class QDMML_WIDGETS_EXPORT qDMMLSceneModel : public QStandardItemModel
{
  Q_OBJECT
  QVTK_OBJECT
...
public:
  ...
  qDMMLSceneModel(QObject *parent=nullptr);
  ~qDMMLSceneModel() override;
  ...
  /// 0 by default
  Q_INVOKABLE virtual void setDMMLScene(vtkDMMLScene* scene);
  Q_INVOKABLE vtkDMMLScene* dmmlScene()const;

  /// 0 until a valid scene is set
  QStandardItem* dmmlSceneItem()const;

  /// invalid until a valid scene is set
  QModelIndex dmmlSceneIndex()const;

  /// Return the vtkDMMLNode associated to the node index.
  /// 0 if the node index is not a DMML node (i.e. vtkDMMLScene, extra item...)
  inline vtkDMMLNode* dmmlNodeFromIndex(const QModelIndex &nodeIndex)const;
  vtkDMMLNode* dmmlNodeFromItem(QStandardItem* nodeItem)const;
  QModelIndex indexFromNode(vtkDMMLNode* node, int column = 0)const;
  // Utility function
  QStandardItem* itemFromNode(vtkDMMLNode* node, int column = 0)const;
  // Return all the QModelIndexes (all the columns) for a given node
  QModelIndexList indexes(vtkDMMLNode* node)const;

  /// Option that activates the expensive listening of the vtkDMMLNode Modified
  /// events. When listening, the signal itemDataChanged() is fired when a
  /// vtkDMMLNode is modified.
  ...
  virtual vtkDMMLNode* parentNode(vtkDMMLNode* node)const;
  ...
  virtual bool         canBeAChild(vtkDMMLNode* node)const;
  /// Must be reimplemented in derived classes
  /// Returns false in qDMMLSceneModel
  virtual bool         canBeAParent(vtkDMMLNode* node)const;
  /// Must be reimplemented in derived classes.
  /// Doesn't reparent and returns false by qDMMLSceneModel
  virtual bool         reparent(vtkDMMLNode* node, vtkDMMLNode* newParent);
  /// Utility method that returns true if \a child has \a parent as parent,
  /// grandparent, great grandparent etc...
  /// \sa isAffiliatedNode()
  bool isParentNode(vtkDMMLNode* child, vtkDMMLNode* parent)const;
  /// Utility method that returns true if 2 nodes are child/parent for each
  /// other. It can be grandchild, great grand child...
  /// \sa isParentNode()
  bool isAffiliatedNode(vtkDMMLNode* nodeA, vtkDMMLNode* nodeB)const;

  /// Observe node and update item when the node is modified.
  /// \sa listenNodeModifiedEvent
  virtual void observeNode(vtkDMMLNode* node);

protected slots:

  virtual void onDMMLSceneNodeAboutToBeAdded(vtkDMMLScene* scene, vtkDMMLNode* node);
  virtual void onDMMLSceneNodeAboutToBeRemoved(vtkDMMLScene* scene, vtkDMMLNode* node);
  virtual void onDMMLSceneNodeAdded(vtkDMMLScene* scene, vtkDMMLNode* node);
  virtual void onDMMLSceneNodeRemoved(vtkDMMLScene* scene, vtkDMMLNode* node);

  virtual void onDMMLSceneAboutToBeImported(vtkDMMLScene* scene);
  virtual void onDMMLSceneImported(vtkDMMLScene* scene);
  virtual void onDMMLSceneAboutToBeClosed(vtkDMMLScene* scene);
  virtual void onDMMLSceneClosed(vtkDMMLScene* scene);
  virtual void onDMMLSceneStartBatchProcess(vtkDMMLScene* scene);
  virtual void onDMMLSceneEndBatchProcess(vtkDMMLScene* scene);

  void onDMMLSceneDeleted(vtkDMMLScene* scene);

  void onDMMLNodeModified(vtkObject* node);
  ...
  void onDMMLNodeIDChanged(vtkObject* node, void* callData);
...
signals:
  ...
  /// a drag and drop
  void aboutToReparentByDragAndDrop(vtkDMMLNode* node, vtkDMMLNode* newParent);

  ///  This signal is sent after a user dragged and dropped a Node in the
  /// qDMMLTreeView
  void reparentedByDragAndDrop(vtkDMMLNode* node, vtkDMMLNode* newParent);
...
protected:
  qDMMLSceneModel(qDMMLSceneModelPrivate* pimpl, QObject *parent=nullptr);

  virtual void updateScene();
  virtual void populateScene();
  virtual QStandardItem* insertNode(vtkDMMLNode* node);
  virtual QStandardItem* insertNode(vtkDMMLNode* node, QStandardItem* parent, int row = -1);

  virtual bool isANode(const QStandardItem* item)const;
  virtual QFlags<Qt::ItemFlag> nodeFlags(vtkDMMLNode* node, int column)const;
  ...
  /// nodeFlags
  virtual void updateItemFromNode(QStandardItem* item, vtkDMMLNode* node, int column);

  /// To reimplement if you want custom display of the QStandardItem from
  /// the DMML node.
  /// Example:
  /// \code
  /// void MySceneModel::updateItemDataFromNode(QStandardItem* item, vtkDMMLNode* node, int column)
  /// if (column == 3)
  ///   {
  ...
  /// nodeFlags
  virtual void updateItemDataFromNode(QStandardItem* item, vtkDMMLNode* node, int column);
  ...
  /// nodeFlags
  virtual void updateNodeFromItem(vtkDMMLNode* node, QStandardItem* item);
  /// To reimplement if you want to propagate user changes on QStandardItem
  /// into the DMML node.
  /// Example:
  /// <code>
  /// void MySceneModel::updateNodeFromItemData(vtkDMMLNode* node, QStandardItem* item)
  /// if (column == 3)
  ///   {
  ...
  /// nodeFlags
  virtual void updateNodeFromItemData(vtkDMMLNode* node, QStandardItem* item);

  /// Update the items associated with the node and uid.
  void updateNodeItems(vtkDMMLNode* node, const QString& uid);
...
protected:
  QScopedPointer<qDMMLSceneModelPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLSceneModel);
  Q_DISABLE_COPY(qDMMLSceneModel);
};
Q_DECLARE_METATYPE(qDMMLSceneModel::NodeTypes)

void printStandardItem(QStandardItem* item, const QString& offset);

// -----------------------------------------------------------------------------
vtkDMMLNode* qDMMLSceneModel::dmmlNodeFromIndex(const QModelIndex &nodeIndex)const
{
  return this->dmmlNodeFromItem(this->itemFromIndex(nodeIndex));
}
