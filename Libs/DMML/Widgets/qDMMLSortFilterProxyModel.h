...
#ifndef __qDMMLSortFilterProxyModel_h
#define __qDMMLSortFilterProxyModel_h
...
class QDMML_WIDGETS_EXPORT qDMMLSortFilterProxyModel : public QSortFilterProxyModel
{
  Q_OBJECT
  QVTK_OBJECT
  ...
  /// If \a showChildNodeTypes is false and \a nodeTypes is vtkDMMLVolumeNode
  /// then vtkDMMLScalarVolumeNode are not visible.
  /// True by default.
  ...
  /// e.g.: nodeTypes = vtkDMMLVolumeNode, showChildNodeTypes = true,
  /// hideChildNodeTypes = vtkDMMLDiffusionWeightedVolumeNode
  /// -> all the nodes of type vtkDMMLScalarVolumeNode, vtkDMMLTensorVolumeNode,
  /// vtkDMMLDiffusionImageVolumeNode... (but not vtkDMMLDiffusionWeightedVolumeNode)
  /// will be visible.
...
public:
  ...
  qDMMLSortFilterProxyModel(QObject *parent=nullptr);
  ~qDMMLSortFilterProxyModel() override;

  /// Retrieve the associated DMML scene
  vtkDMMLScene* dmmlScene()const;

  /// Retrieve the DMML scene index
  Q_INVOKABLE QModelIndex dmmlSceneIndex()const;

  /// Retrieve the associated vtkDMMLNode
  Q_INVOKABLE vtkDMMLNode* dmmlNodeFromIndex(const QModelIndex& index)const;

  /// Retrieve an index for a given vtkDMMLNode
  Q_INVOKABLE QModelIndex indexFromDMMLNode(vtkDMMLNode* node, int column = 0)const;
  ...
  /// Give more control over the types of DMML node you want to force
...
public slots:
  ...
  // TODO Add setDMMLScene() to propagate to the scene model
protected:
  ...
  virtual AcceptType filterAcceptsNode(vtkDMMLNode* node)const;

  QStandardItem* sourceItem(const QModelIndex& index)const;

protected:
  QScopedPointer<qDMMLSortFilterProxyModelPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qDMMLSortFilterProxyModel);
  Q_DISABLE_COPY(qDMMLSortFilterProxyModel);
};
