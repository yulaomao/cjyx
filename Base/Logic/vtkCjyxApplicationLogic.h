...
==========================================================================*/
///  vtkCjyxApplicationLogic - the main logic to manage the application
///
/// The Main entry point for the cjyx3 application.
/// -- manages the connection to the dmml scene
/// -- manages the creation of Views and Slices (logic only)
/// -- serves as central point for dispatching events
/// There is a corresponding vtkCjyxApplicationGUI class that provides
/// a user interface to this class by observing this class.
//

#ifndef __vtkCjyxApplicationLogic_h
#define __vtkCjyxApplicationLogic_h
...
class VTK_CJYX_BASE_LOGIC_EXPORT vtkCjyxApplicationLogic
  : public vtkDMMLApplicationLogic
{
  public:

  /// The Usual vtk class functions
  static vtkCjyxApplicationLogic *New();
  vtkTypeMacro(vtkCjyxApplicationLogic, vtkDMMLApplicationLogic);
  void PrintSelf(ostream& os, vtkIndent indent) override;

  /// Update the data IO, local and remote, with the new scene
  /// For stand alone applications, follow the set up steps in
  /// qCjyxCoreApplicationPrivate::initDataIO() to set up the
  /// remote IO logic and data manager logic and then call this
  /// method to hook them into the scene.
  /// \sa qCjyxCoreApplicationPrivate::initDataIO()
  /// \sa vtkDMMLRemoteIOLogic::AddDataIOToScene()
  void SetDMMLSceneDataIO(vtkDMMLScene *scene,
  ...
  enum RequestEvents
    {
      RequestModifiedEvent = vtkDMMLApplicationLogic::RequestInvokeEvent + 1,
  ...
  /// \note On macOS extensions are installed in the "<Cjyx_EXTENSIONS_DIRBASENAME>-<cjyxRevision>"
  /// folder being a sub directory of the application dir, an extra test is performed to make sure the
  /// tested filePath doesn't belong to that "<Cjyx_EXTENSIONS_DIRBASENAME>-<cjyxRevision>" folder.
  ...
  /// Get Cjyx-X.Y share directory associated with module located in \a filePath
  static std::string GetModuleCjyxXYShareDirectory(const std::string& filePath);

  /// Get Cjyx-X.Y lib directory associated with module located in \a filePath
  static std::string GetModuleCjyxXYLibDirectory(const std::string& filePath);
...
protected:
  ...
  vtkCjyxApplicationLogic();
  ~vtkCjyxApplicationLogic() override;
  ...
  /// can be set via an environment variable CJYX_BACKGROUND_THREAD_PRIORITY.
...
private:
  vtkCjyxApplicationLogic(const vtkCjyxApplicationLogic&);
  void operator=(const vtkCjyxApplicationLogic&);
