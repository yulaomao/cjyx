...
#ifndef __vtkCjyxApplicationLogicRequests_h
#define __vtkCjyxApplicationLogicRequests_h
...
public:
  ...
  virtual void Execute(vtkCjyxApplicationLogic*) {};
...
public:
  ...
  void Execute(vtkCjyxApplicationLogic* appLogic) override
  {
    // This method needs to read the data into the specific type of node and set up an
    // appropriate storage and display node.

    vtkDMMLNode *nd = appLogic->GetDMMLScene()->GetNodeByID(m_TargetNode.c_str());
    ...
    vtkDMMLCommandLineModuleNode *clp = vtkDMMLCommandLineModuleNode::SafeDownCast(nd);
#endif

    bool useURI = appLogic->GetDMMLScene()->GetCacheManager()->IsRemoteReference(m_Filename.c_str());

    vtkDMMLStorableNode *storableNode = vtkDMMLStorableNode::SafeDownCast(nd);
    if (storableNode)
      {
      int numStorageNodes = storableNode->GetNumberOfStorageNodes();
      for (int n = 0; n < numStorageNodes; n++)
        {
        vtkDMMLStorageNode *testStorageNode = storableNode->GetNthStorageNode(n);
        if (testStorageNode)
          {
    ...
    if (m_DeleteFile)
     {
      ...
      if (m_Filename.find("cjyx:") != std::string::npos)
        {
        removed = 1;
        }
      else
    ...
    vtkDMMLDisplayableNode *displayableNode =
      vtkDMMLDisplayableNode::SafeDownCast(nd);
    if (displayableNode)
      {
    ...
    if (m_DisplayData)
      {
      if (vtkDMMLLabelMapVolumeNode::SafeDownCast(nd) != nullptr)
        {
        appLogic->GetSelectionNode()->SetActiveLabelVolumeID(m_TargetNode.c_str());
        appLogic->PropagateVolumeSelection();
        }
      else if (vtkDMMLScalarVolumeNode::SafeDownCast(nd) != nullptr)
        {
        appLogic->GetSelectionNode()->SetActiveVolumeID(m_TargetNode.c_str());
        // make sure win/level gets calculated
        vtkDMMLDisplayNode* displayNode = vtkDMMLScalarVolumeNode::SafeDownCast(nd)->GetDisplayNode();
        if (displayNode)
          {
      ...
      else if (vtkDMMLTableNode::SafeDownCast(nd) != nullptr)
        {
...
public:
  ...
  void Execute(vtkCjyxApplicationLogic* appLogic) override
  {
    if (m_SourceNodes.size() != m_TargetNodes.size())
      ...
      appLogic->GetDMMLScene()->SetURL(m_Filename.c_str());
      appLogic->GetDMMLScene()->Import();
    ...
    appLogic->GetDMMLScene()->StartState(vtkDMMLScene::ImportState);

    while (sit != m_SourceNodes.end())
      {
      vtkDMMLNode *source = miniscene->GetNodeByID((*sit).c_str());
      vtkDMMLNode *target = appLogic->GetDMMLScene()->GetNodeByID( (*tit).c_str() );
      ...
      if (source && target)
        ...
        vtkDMMLStorableNode *storableTarget = vtkDMMLStorableNode::SafeDownCast(target);
        if ( storableTarget )
          {
        ...
        // module ran).
        vtkDMMLModelHierarchyNode *smhnd
          = vtkDMMLModelHierarchyNode::SafeDownCast(source);
        vtkDMMLModelHierarchyNode *tmhnd
          = vtkDMMLModelHierarchyNode::SafeDownCast(target);
        if (smhnd && tmhnd)
          {
          // get the model node and display node BEFORE we add nodes to
          // the target scene
          vtkDMMLModelNode *smnd = smhnd->GetModelNode();
          vtkDMMLDisplayNode *sdnd = smhnd->GetDisplayNode();
          ...
          if (smnd)
            ...
            vtkDMMLDisplayNode *sdnd1 = smnd->GetDisplayNode();

            vtkDMMLNode *tmodel = appLogic->GetDMMLScene()->CopyNode(smnd);
            vtkDMMLStorableNode::SafeDownCast(tmodel)->SetAndObserveStorageNodeID(nullptr);
            vtkDMMLModelNode *mnd = vtkDMMLModelNode::SafeDownCast( tmodel );
            tmhnd->SetModelNodeID( mnd->GetID() );

            if (sdnd1)
              {
              vtkDMMLNode *tdnd = appLogic->GetDMMLScene()->CopyNode(sdnd1);
          ...
          if (sdnd)
            {
            vtkDMMLNode *dnd = appLogic->GetDMMLScene()->CopyNode(sdnd);
          ...
          std::vector<vtkDMMLModelHierarchyNode *> childNodesThatNeedParentsIDsRemapped;
          for (int n=0;
               n<miniscene->GetNumberOfNodesByClass("vtkDMMLModelHierarchyNode");
               n++)
            {
            vtkDMMLModelHierarchyNode * mhnd = vtkDMMLModelHierarchyNode
            ...
            if (mhnd)
              ...
              if (ssit == m_SourceNodes.end())
                ...
                if (strcmp(mhnd->GetTopParentNode()->GetID(), smhnd->GetID()) == 0)
                  ...
                  // to the target scene
                  vtkDMMLModelNode *smnd1 = mhnd->GetModelNode();
                  vtkDMMLDisplayNode *sdnd1 = mhnd->GetDisplayNode();

                  vtkDMMLNode *tchild = appLogic->GetDMMLScene()->CopyNode(mhnd);
                  ...
                  vtkDMMLModelHierarchyNode *tcmhd
                    = vtkDMMLModelHierarchyNode::SafeDownCast( tchild );
                  ...
                  if (smnd1)
                    ...
                    // get display node BEFORE we add nodes to the target scene
                    vtkDMMLDisplayNode *sdnd2 = smnd1->GetDisplayNode();

                    vtkDMMLNode *tmodel = appLogic->GetDMMLScene()->CopyNode(smnd1);
                    vtkDMMLStorableNode::SafeDownCast(tmodel)->SetAndObserveStorageNodeID(nullptr);
                    vtkDMMLModelNode *mnd =vtkDMMLModelNode::SafeDownCast(tmodel);
                    ...
                    if (sdnd2)
                      {
                      vtkDMMLNode *tdnd = appLogic->GetDMMLScene()->CopyNode(sdnd2);
                      mnd->SetAndObserveDisplayNodeID( tdnd->GetID() );
                      }
                    }
    ...
    appLogic->GetDMMLScene()->EndState(vtkDMMLScene::ImportState);
...
public:
  ...
  void Execute(vtkCjyxApplicationLogic* appLogic) override
  {
    ...
    vtkDMMLNode *siblingNode = scene->GetNodeByID(m_SubjectHierarchySiblingNode);
    vtkDMMLNode *updatedNode = scene->GetNodeByID(m_UpdatedNode);
    vtkDMMLSubjectHierarchyNode *shnd = vtkDMMLSubjectHierarchyNode::GetSubjectHierarchyNode(scene);
...
public:
  ...
  void Execute(vtkCjyxApplicationLogic* appLogic) override
  {
    ...
    vtkDMMLNode *referencingNode = scene->GetNodeByID(m_ReferencingNode);
    vtkDMMLNode *referencedNode = scene->GetNodeByID(m_ReferencedNode);
...
#endif // __vtkCjyxApplicationLogicRequests_h
