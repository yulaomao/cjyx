...
typedef std::pair< vtkDataTransfer *, vtkDMMLNode * > TransferNodePair;
  ...
  unsigned long event, void *callData)
{
  ...
  if ( (node != nullptr) && (event == vtkDataIOManager::RemoteReadEvent ) )
    {
    vtkDebugMacro("ProcessDMMLEvents: calling queue read on the node " << node->GetID());
    this->QueueRead ( node );
    node->InvokeEvent ( vtkDataIOManager::RefreshDisplayEvent );
    }
  else if ( (node != nullptr) && (event == vtkDataIOManager::RemoteWriteEvent ) )
    {
    vtkDebugMacro("ProcessDMMLEvents: calling queue write on the node " << node->GetID());
...
//----------------------------------------------------------------------------
void vtkDataIOManagerLogic::AddNewDataTransfer ( vtkDataTransfer *dt, vtkDMMLNode *node )
{
...
//----------------------------------------------------------------------------
int vtkDataIOManagerLogic::QueueRead ( vtkDMMLNode *node )
{
//  this->DebugOn();
  ...
  vtkDMMLStorableNode *dnode = vtkDMMLStorableNode::SafeDownCast ( node );
  if ( dnode == nullptr )
    {
    vtkErrorMacro("QueueRead: unable to cast input dmml node " << node->GetID() << " to a storable node");
    return 0;
    }
  ...
  for (int n = 0; n < numStorageNodes; n++)
    {
    if (dnode->GetNthStorageNode(n)->GetReadState() == vtkDMMLStorageNode::Scheduled)
      {
  ...
  //--- This test has been done in DMML (DataIOManager), but with asynchIO,
  ...
  if ( (cm->GetCurrentCacheSize()*1000000.0) >= bufsize )
    ...
    //--- to download new data, but that Cjyx is loading
  ...
  //--- the resulting dmml call will trigger an event
...
//----------------------------------------------------------------------------
int vtkDataIOManagerLogic::QueueWrite ( vtkDMMLNode *node )
{
    //--- do some node nullchecking first.
  if ( node == nullptr )
    {
  ...
  if ( dnode == nullptr )
    {
    vtkErrorMacro("QueueWrite: unable to cast input dmml node " << node->GetID() << " to a storable node");
    return 0;
    }
  ...
  for (int i = 0; i < dnode->GetNumberOfStorageNodes(); i++)
    {
    if (dnode->GetNthStorageNode(i)->GetWriteState() == vtkDMMLStorageNode::Scheduled)
      {
      storageNodeIndex = i;
...
void vtkDataIOManagerLogic::ApplyTransfer( void *clientdata )
{
  ...
  vtkDMMLNode *node = this->GetDMMLScene()->GetNodeByID ((dt->GetTransferNodeID() ));
  if ( node == nullptr )
    {
    vtkErrorMacro("ApplyTransfer: can't get dmml node from transfer node id " << dt->GetTransferNodeID());
    return;
    }
  ...
  if ( dt->GetTransferType() == vtkDataTransfer::RemoteDownload  )
     ...
     if ( handler != nullptr && source != nullptr && dest != nullptr )
      ...
      if ( asynchIO && dt->GetTransferStatus() == vtkDataTransfer::Pending)
        ...
        for (int i = 0; i < storableNode->GetNumberOfStorageNodes(); i++)
          {
          if (storableNode->GetNthStorageNode(i)->GetReadState() == vtkDMMLStorageNode::Transferring &&
        ...
        vtkDMMLStorageNode *storageNode = storableNode->GetNthStorageNode(storageNodeIndex);
        if ( !storageNode )
          {
    ...
    if ( handler != nullptr && source != nullptr && dest != nullptr )
      ...
      if ( asynchIO && dt->GetTransferStatus() == vtkDataTransfer::Pending)
        ...
        for (int i = 0; i < storableNode->GetNumberOfStorageNodes(); i++)
          {
          if (storableNode->GetNthStorageNode(i)->GetWriteState() == vtkDMMLStorageNode::Scheduled)
            {
            storageNodeIndex = i;
            }
          }
