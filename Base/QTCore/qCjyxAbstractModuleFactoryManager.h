...
#ifndef __qCjyxAbstractModuleFactoryManager_h
#define __qCjyxAbstractModuleFactoryManager_h
...
/// Loading modules into cjyx happens in multiple steps:
/// 1) module factories must be registered into the factory manager:
///   qCjyxModuleFactoryManager* factoryManager = app->moduleManager()->factoryManager();
///   factoryManager->registerFactory(new qCjyxLoadableModuleFactory);
///   ...
/// 2) directories where the modules to load are located must be passed to the factory manager
///   factoryManager->addSearchPath(app->cjyxHome() + "/" +  Cjyx_QTSCRIPTEDMODULES_LIB_DIR + "/" );
///   factoryManager->addSearchPath(app->cjyxHome() + "/" + Cjyx_CLIMODULES_LIB_DIR + "/" );
///   ...
/// 3) Optionally:
...
class Q_CJYX_BASE_QTCORE_EXPORT qCjyxAbstractModuleFactoryManager : public QObject
{
  Q_OBJECT
  ...
  /// manually.
  ///
  /// \todo In qCjyxAbstractModuleFactoryManager, should the module search recursively descend \a searchPaths
...
public:
  typedef ctkAbstractFileBasedFactory<qCjyxAbstractCoreModule> qCjyxFileBasedModuleFactory;
  typedef ctkAbstractFactory<qCjyxAbstractCoreModule> qCjyxModuleFactory;

  typedef QObject Superclass;
  qCjyxAbstractModuleFactoryManager(QObject * newParent = nullptr);

  /// Destructor, Deallocates resources
  /// Unregister (and delete) all registered factories.
  ~qCjyxAbstractModuleFactoryManager() override;
  ...
  /// (e.g. in ~qCjyxAbstractModuleFactoryManager())
  ///
  /// Example:
  ///
  /// \code{.cpp}
  /// qCjyxAbstractModuleFactoryManager factoryManager;
  /// factoryManager.registerFactory(new qCjyxCoreModuleFactory);
  /// factoryManager.registerFactory(new qCjyxLoadableModuleFactory);
  /// \endcode
  ///
  ...
  void registerFactory(qCjyxModuleFactory* factory, int priority = 0);
  void unregisterFactory(qCjyxModuleFactory* factory);
  ...
  Q_INVOKABLE qCjyxAbstractCoreModule* moduleInstance(const QString& moduleName)const;
  ...
  /// \sa qCjyxAbstractCoreModule::dependencies(), moduleDependees()
  QStringList dependentModules(const QString& module)const;

  /// Return the list of modules that depend on \a module.
  /// Note that the list can contain unloaded modules.
  /// \sa dependentModules(), qCjyxAbstractCoreModule::dependencies()
...
protected:
  QScopedPointer<qCjyxAbstractModuleFactoryManagerPrivate> d_ptr;

  void registerModules(const QString& directoryPath);

  /// Instantiate a module given its \a name
  qCjyxAbstractCoreModule* instantiateModule(const QString& name);

  /// Uninstantiate a module given its \a moduleName
  virtual void uninstantiateModule(const QString& moduleName);

private:
  Q_DECLARE_PRIVATE(qCjyxAbstractModuleFactoryManager);
  Q_DISABLE_COPY(qCjyxAbstractModuleFactoryManager);
};

//-----------------------------------------------------------------------------
void qCjyxAbstractModuleFactoryManager::addSearchPaths(const QStringList& paths)
{
  this->setSearchPaths(this->searchPaths() << paths);
}

//-----------------------------------------------------------------------------
void qCjyxAbstractModuleFactoryManager::addSearchPath(const QString& path)
{
  this->setSearchPaths(this->searchPaths() << path);
}

//-----------------------------------------------------------------------------
void qCjyxAbstractModuleFactoryManager::removeSearchPaths(const QStringList& paths)
{
...
//-----------------------------------------------------------------------------
void qCjyxAbstractModuleFactoryManager::removeSearchPath(const QString& path)
{
...
//-----------------------------------------------------------------------------
void qCjyxAbstractModuleFactoryManager::addModuleToIgnore(const QString& moduleName)
{
...
//-----------------------------------------------------------------------------
void qCjyxAbstractModuleFactoryManager::removeModuleToIgnore(const QString& moduleName)
{
  QStringList modules = this->modulesToIgnore();
