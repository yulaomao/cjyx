...
#ifndef __qCjyxCoreCommandOptions_h
#define __qCjyxCoreCommandOptions_h
...
class Q_CJYX_BASE_QTCORE_EXPORT qCjyxCoreCommandOptions : public ctkCommandLineParser
{
  Q_OBJECT
  Q_PROPERTY(bool displayHelpAndExit READ displayHelpAndExit CONSTANT)
  Q_PROPERTY(bool ignoreCjyxRC READ ignoreCjyxRC CONSTANT)
...
public:
  ...
  qCjyxCoreCommandOptions();
  ~qCjyxCoreCommandOptions() override;

  /// Convenient method allowing to parse arguments
  bool parse(const QStringList& arguments);

  /// Return True if cjyx should display help and exit
  bool displayHelpAndExit()const;

  /// Return True if the ignore rest argument has been passed
  bool ignoreRest() const;

  /// Return True if the loading of CjyxRC should be skipped
  bool ignoreCjyxRC()const;

  /// Return path of the python script to execute after cjyx is loaded
  QString pythonScript()const;

  /// Return extra python script
  QString extraPythonScript()const;

  /// Set extra python script to be executed after cjyx is loaded
  void setExtraPythonScript(const QString& newExtraPythonScript);

  /// Return python code snippet to execute after cjyx is loaded
  QString pythonCode()const;

  /// Return True if Cjyx should execute python code or script and exit
  ...
  /// Return True if cjyx should display version and exit
  bool displayVersionAndExit()const;

  /// Return True if cjyx should display program path and exit
  bool displayProgramPathAndExit()const;

  /// Return True if cjyx should display home path and exit
  bool displayHomePathAndExit()const;

  /// Return True if cjyx should display settings path and exit
  bool displaySettingsPathAndExit()const;

  /// Return True if cjyx should display temporary path and exit
  bool displayTemporaryPathAndExit()const;

  /// Return True if cjyx should display a message and exit.
  ...
  /// Return True if cjyx should display application information in the terminal.
  bool displayApplicationInformation() const;

  /// Return True if cjyx should display details regarding the module discovery process
  bool verboseModuleDiscovery()const;

  /// Return True if cjyx should display information at startup
  bool verbose()const;

  /// Return True if cjyx shouldn't catch messages printed to the terminal.
  bool disableMessageHandlers()const;

  /// Return True if cjyx shouldn't print messages on the terminal.
  /// Messages won't be displayed on either stdout or stderr and will only be captured
  /// in the error log model.
  /// \sa qCjyxCoreApplication::errorLogModel()
  /// \sa ctkErrorLogModel::setTerminalOutputs()
  bool disableTerminalOutputs()const;

  /// Return a value indicating if cjyx settings should be disabled.
  ///
  ...
  /// Return True if cjyx is in testing mode.
  /// Typically set when running unit tests:
  ///  ./Cjyx --testing --launch ./bin/qCjyxXXXTests ...
  /// \note Using this option implies 'disable-settings' option.
  /// \sa settingsDisabled()
  bool isTestingEnabled()const;

#ifdef Cjyx_USE_PYTHONQT
  /// Return True if cjyx has no python infrastructure initialized.
...
protected:
  ...
  /// \sa qCjyxCoreCommandOptions::parse(const QStringList&)
  QHash<QString, QVariant> parsedArgs() const;

protected:
  QScopedPointer<qCjyxCoreCommandOptionsPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qCjyxCoreCommandOptions);
  Q_DISABLE_COPY(qCjyxCoreCommandOptions);
