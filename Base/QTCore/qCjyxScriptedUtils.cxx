...
//-----------------------------------------------------------------------------
bool qCjyxScriptedUtils::loadSourceAsModule(const QString& moduleName,
                                       ...
                                       PyObject * local_dict)
{
  ...
  if (fileName.endsWith(".py"))
    ...
    pyRes = PyRun_String(
          ...
          .arg(qCjyxCorePythonManager::toPythonStringLiteral(fileName))
          .arg(qCjyxCorePythonManager::toPythonStringLiteral(moduleName)).toUtf8(),
...
//-----------------------------------------------------------------------------
bool qCjyxScriptedUtils::setModuleAttribute(const QString& moduleName,
...
//-----------------------------------------------------------------------------
qCjyxPythonCppAPI::qCjyxPythonCppAPI() = default;

//-----------------------------------------------------------------------------
qCjyxPythonCppAPI::~qCjyxPythonCppAPI()
{
  if (this->PythonSelf)
    {
...
//-----------------------------------------------------------------------------
QString qCjyxPythonCppAPI::objectName()const
{
  return this->ObjectName;
}

//-----------------------------------------------------------------------------
void qCjyxPythonCppAPI::setObjectName(const QString& name)
{
  this->ObjectName = name;
}

//-----------------------------------------------------------------------------
void qCjyxPythonCppAPI::declareMethod(int id, const char* name)
{
  if (!name)
    {
    return;
...
//-----------------------------------------------------------------------------
PyObject* qCjyxPythonCppAPI::instantiateClass(QObject* cpp, const QString& className, PyObject* classToInstantiate)
{
  PyObject * wrappedThis = PythonQt::self()->priv()->wrapQObject(cpp);
  if (!wrappedThis)
    {
    PythonQt::self()->handleError();
    qCritical() << "qCjyxPythonCppAPI::instantiateClass"
                << " - [" << this->ObjectName << "]"
...
//-----------------------------------------------------------------------------
PyObject * qCjyxPythonCppAPI::callMethod(int id, PyObject * arguments)
{
...
//-----------------------------------------------------------------------------
PyObject* qCjyxPythonCppAPI::pythonSelf()const
{
  ...
  // cjyx.modules.endoscopy.widgetRepresentation().self()
