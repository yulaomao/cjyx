...
//-----------------------------------------------------------------------------
vtkDMMLScene* qCjyxCoreIOManagerPrivate::currentScene()const
{
...
QStringList qCjyxCoreIOManager::allWritableFileExtensions()const
{
  ...
  for (int i = 0; i < numRegisteredNodeClasses; ++i)
    {
    vtkDMMLNode *dmmlNode = d->currentScene()->GetNthRegisteredNodeClass(i);
    if (dmmlNode && dmmlNode->IsA("vtkDMMLStorageNode"))
      {
      vtkDMMLStorageNode* snode = vtkDMMLStorageNode::SafeDownCast(dmmlNode);
      if (snode)
        {
...
qCjyxIOOptions* qCjyxCoreIOManager::fileOptions(const QString& readerDescription)const
{
  ...
  reader->setDMMLScene(d->currentScene());
  ...
  foreach(qCjyxFileWriter* writer, d->Writers)
    ...
    if (writer->canWriteObject(object))
      ...
      if (writer->extensions(object).contains(extension))
        {
        writer->setDMMLScene(d->currentScene());
        bestWriter = writer;
        }
      }
    }
...
//-----------------------------------------------------------------------------
QString qCjyxCoreIOManager::completeCjyxWritableFileNameSuffix(vtkDMMLStorableNode *node)const
{
  vtkDMMLStorageNode* storageNode = node->GetStorageNode();
  if (!storageNode)
    {
...
//-----------------------------------------------------------------------------
bool qCjyxCoreIOManager::loadScene(const QString& fileName, bool clear, vtkDMMLMessageCollection* userMessages/*=nullptr*/)
{
...
//-----------------------------------------------------------------------------
bool qCjyxCoreIOManager::loadFile(const QString& fileName, vtkDMMLMessageCollection* userMessages/*=nullptr*/)
{
  qCjyxIO::IOProperties properties;
  properties["fileName"] = fileName;
                                     ...
                                     vtkDMMLMessageCollection* userMessages/*=nullptr*/)
{
  ...
  if (loadedNodes)
    ...
    foreach(const QString& node, nodes)
      {
      vtkDMMLNode* loadedNode = d->currentScene()->GetNodeByID(node.toUtf8());
      if (!loadedNode)
        {
...
//-----------------------------------------------------------------------------
vtkDMMLNode* qCjyxCoreIOManager::loadNodesAndGetFirst(qCjyxIO::IOFileType fileType,
  const qCjyxIO::IOProperties& parameters, vtkDMMLMessageCollection* userMessages/*=nullptr*/)
{
  vtkNew<vtkCollection> loadedNodes;
  this->loadNodes(fileType, parameters, loadedNodes.GetPointer(), userMessages);

  vtkDMMLNode* node = vtkDMMLNode::SafeDownCast(loadedNodes->GetItemAsObject(0));
  Q_ASSERT(node);
...
//-----------------------------------------------------------------------------
vtkDMMLStorageNode* qCjyxCoreIOManager::createAndAddDefaultStorageNode(
    vtkDMMLStorableNode* node)
{
  if (!node)
    {
...
void qCjyxCoreIOManager::addDefaultStorageNodes()
{
  ...
  for (int i = 0; i < numNodes; ++i)
    {
    vtkDMMLStorableNode* storableNode = vtkDMMLStorableNode::SafeDownCast(d->currentScene()->GetNthNode(i));
    if (!storableNode)
      {
      continue;
      }
    ...
    std::string fileBaseName = vtkDMMLApplicationLogic::PercentEncode(storableNode->GetName());
...
)
{
  ...
  vtkNew<vtkDMMLScene> temporaryScene;
  ...
  for (const auto& parameters : parameterMaps)
    ...
    vtkDMMLStorableNode* storableNode = vtkDMMLStorableNode::SafeDownCast(d->currentScene()->GetNodeByID(nodeID.toUtf8()));
    if (!storableNode)
      {
      if (userMessages)
        {
    ...
    vtkDMMLStorableNode* temporaryStorableNode = vtkDMMLStorableNode::SafeDownCast(temporaryScene->AddNewNodeByClass(storableNode->GetClassName()));
    ...
    vtkDMMLTransformableNode* nodeAsTransformable = vtkDMMLTransformableNode::SafeDownCast(storableNode);
    if (hardenTransforms && nodeAsTransformable)
      {
      vtkDMMLTransformableNode* temporaryNodeAsTransformable = vtkDMMLTransformableNode::SafeDownCast(temporaryStorableNode);
      ...
      vtkDMMLTransformNode* parentTransform = nodeAsTransformable->GetParentTransformNode();
      if (parentTransform)
        {
        ...
        vtkDMMLTransformNode* compositeTransformNode =
          vtkDMMLTransformNode::SafeDownCast(temporaryScene->AddNewNodeByClass("vtkDMMLTransformNode"));
