...
//-----------------------------------------------------------------------------
int qCjyxCLIModuleTest1(int argc, char * argv[])
{
  ...
  moduleFactoryManager->registerFactory(new qCjyxCLILoadableModuleFactory);
  ...
  if (!moduleNames.contains(cliModuleName))
    {
    std::cerr << "Line " << __LINE__ << " - Problem with qCjyxCLILoadableModuleFactory"
  ...
  qCjyxAbstractCoreModule * module = moduleManager->module("CLI4Test");
  if (!module)
    {
    std::cerr << "Line " << __LINE__
