...
#ifndef qCjyxSingletonViewFactory_h
#define qCjyxSingletonViewFactory_h
...
/// New view widgets can be registered by registering a new qCjyxSingletonViewFactory, and then setting the widget and tag using setWidget(QWidget*) and
/// setTagName(QString).
...
class Q_CJYX_BASE_QTGUI_EXPORT qCjyxSingletonViewFactory : public ctkLayoutViewFactory
{
  Q_OBJECT
public:
  typedef ctkLayoutViewFactory Superclass;
  qCjyxSingletonViewFactory(QObject* parent=nullptr);
  ~qCjyxSingletonViewFactory() override;
...
protected:
  QScopedPointer<qCjyxSingletonViewFactoryPrivate> d_ptr;
...
private:
  Q_DECLARE_PRIVATE(qCjyxSingletonViewFactory)
};
