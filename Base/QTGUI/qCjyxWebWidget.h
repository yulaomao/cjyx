...
#ifndef __qCjyxWebWidget_h
#define __qCjyxWebWidget_h
...
#include "vtkCjyxConfigure.h" // For Cjyx_USE_PYTHONQT_WITH_OPENSSL

class QNetworkReply;
class qCjyxWebWidgetPrivate;
class QUrl;
class QWebChannel;
...
class Q_CJYX_BASE_QTGUI_EXPORT qCjyxWebWidget
  : public QWidget
{
  Q_OBJECT
  ...
  friend class qCjyxWebEnginePage;
public:
  /// Superclass typedef
  typedef QWidget Superclass;

  /// Constructor
  explicit qCjyxWebWidget(QWidget* parent = nullptr);

  /// Destructor
  ~qCjyxWebWidget() override;
...
protected:
  qCjyxWebWidget(qCjyxWebWidgetPrivate* pimpl, QWidget* parent = nullptr);
  QScopedPointer<qCjyxWebWidgetPrivate> d_ptr;
...
private:
  Q_DECLARE_PRIVATE(qCjyxWebWidget);
  Q_DISABLE_COPY(qCjyxWebWidget);
};
