...
#ifndef __qCjyxModulesMenu_h
#define __qCjyxModulesMenu_h
...
///
/// qCjyxModulesMenu supports a tree hierarchy of modules (based on
/// module->category() )
class Q_CJYX_BASE_QTGUI_EXPORT qCjyxModulesMenu: public QMenu
{
  Q_OBJECT
  ...
  /// qCjyxModulesMenu share the same QActions. When a module QAction is
  /// fired from a menu, all the qCjyxModulesMenu would make it the current
  /// module. When duplicateActions is true, the QActions populating the menu are
  /// duplicates from the original module QAction. That way the qCjyxModulesMenu
  /// behaves independently from the other qCjyxModulesMenus.
...
public:
  ...
  /// toolbar area)
  qCjyxModulesMenu(const QString& title, QWidget* parent = nullptr);
  qCjyxModulesMenu(QWidget* parent = nullptr);
  ~qCjyxModulesMenu() override;
  ...
  qCjyxModuleManager* moduleManager()const;
...
public slots:
  ...
  ///
  /// \sa qCjyxAbstractCoreModule::category()
  /// \sa qCjyxAbstractCoreModule::action()
  /// \sa qCjyxAbstractCoreModule::isHidden()
  ...
  void addModule(qCjyxAbstractCoreModule*);
  ...
  ///
  /// Return true if the module was found and removed.
  bool removeModule(qCjyxAbstractCoreModule*);
...
protected:
  QScopedPointer<qCjyxModulesMenuPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qCjyxModulesMenu);
  Q_DISABLE_COPY(qCjyxModulesMenu);
};

//---------------------------------------------------------------------------
void qCjyxModulesMenu::addModules(const QStringList& moduleNames)
{
...
//---------------------------------------------------------------------------
void qCjyxModulesMenu::removeModules(const QStringList& moduleNames)
{
  foreach(const QString& moduleName, moduleNames)
    {
