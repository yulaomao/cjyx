...
#ifndef __qCjyxApplication_h
#define __qCjyxApplication_h
...
class QMainWindow;
class qCjyxApplicationPrivate;
class qCjyxCommandOptions;
class qCjyxIOManager;
#ifdef Cjyx_USE_PYTHONQT
class qCjyxPythonManager;
#endif
...
class Q_CJYX_BASE_QTGUI_EXPORT qCjyxApplication : public qCjyxCoreApplication
{
  Q_OBJECT
public:

  typedef qCjyxCoreApplication Superclass;
  qCjyxApplication(int &argc, char **argv);
  ~qCjyxApplication() override;

  /// Return a reference to the application singleton
  static qCjyxApplication* application();
  ...
  /// Get commandOptions
  Q_INVOKABLE qCjyxCommandOptions* commandOptions()const;

  /// Get IO Manager
  Q_INVOKABLE qCjyxIOManager* ioManager();

#ifdef Cjyx_USE_PYTHONQT
  /// Get Python Manager
  Q_INVOKABLE qCjyxPythonManager * pythonManager();
  ...
  Q_INVOKABLE qCjyxLayoutManager* layoutManager()const;
  Q_INVOKABLE void setLayoutManager(qCjyxLayoutManager* layoutManager);
  ...
  QString nodeModule(vtkDMMLNode* node, double *confidence=nullptr)const;
  ...
  ///
  /// \sa qCjyxCoreCommandOptions::displayApplicationInformation()
...
public slots:
  ...
  /// \note qCjyxApplication is a temporary host for the function as it should be
  /// moved into a DataManager where module can register new node
  /// types/modules
  void openNodeModule(vtkDMMLNode* node, QString role=QString(), QString context=QString());
  ...
  /// \sa qDMMLLayoutManager::setRenderPaused()
  ...
  /// Override the qCjyxCoreApplication implementation to also show error messages in a popup window.
  bool loadFiles(const QStringList& filePaths, vtkDMMLMessageCollection* userMessages = nullptr) override;
...
signals:
  ...
  ///
  /// \sa qCjyxAppMainWindow::initialWindowShown()
...
private:
  Q_DECLARE_PRIVATE(qCjyxApplication);
  Q_DISABLE_COPY(qCjyxApplication);
};

/// Apply the Cjyx palette to the \c palette
/// Note also that the palette parameter is passed by reference and will be
/// updated using the native paletter and applying Cjyx specific properties.
void  Q_CJYX_BASE_QTGUI_EXPORT qCjyxApplyPalette(QPalette& palette);
