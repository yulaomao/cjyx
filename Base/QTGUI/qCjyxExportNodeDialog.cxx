...
#include "qCjyxExportNodeDialog_p.h"
...
//-----------------------------------------------------------------------------
NodeTypeWidgetSet::NodeTypeWidgetSet(QWidget* parent, vtkDMMLStorableNode* storableNode, vtkDMMLScene* scene)
  : prototypeNode{storableNode}
{
  ...
  if (storageNode)
    ...
    if (!defaultStorageNodeClassName.empty())
      {
      vtkSmartPointer<vtkDMMLNode> defaultStorageNodeAsNode = vtkSmartPointer<vtkDMMLNode>::Take(
        scene->CreateNodeByClass(defaultStorageNodeClassName.c_str())
      );
      vtkDMMLStorageNode* defaultStorageNode = vtkDMMLStorageNode::SafeDownCast(defaultStorageNodeAsNode);
      extensionInStorageNode = defaultStorageNode->GetDefaultWriteFileExtension();
      }
    }

  // (Checking that storageNode is not null allows us to dodge a warning from completeCjyxWritableFileNameSuffix)
  QString currentExtension = storageNode ?
    coreIOManager->completeCjyxWritableFileNameSuffix(storableNode) : QString(".");
...
bool NodeTypeWidgetSet::updateOptionsWidget()
{
  ...
  if (iterator != this->formatToOptionsWidget.end())
    ...
    qCjyxIOOptions* options = coreIOManager->fileWriterOptions(this->prototypeNode, this->formatText());
    ...
    qCjyxFileWriterOptionsWidget* newOptionsWidget = dynamic_cast<qCjyxFileWriterOptionsWidget*>(options);
    if (!newOptionsWidget)
      {
      delete options;
...
//-----------------------------------------------------------------------------
void NodeTypeWidgetSet::changeCurrentOptionsWidget(qCjyxFileWriterOptionsWidget* newOptionsWidget)
{
...
//-----------------------------------------------------------------------------
QString qCjyxExportNodeDialogPrivate::forceFileNameExtension(const QString& fileName, const QString& extension, vtkDMMLNode* node)
{
  qCjyxCoreIOManager* coreIOManager = qCjyxCoreApplication::application()->coreIOManager();
...
//-----------------------------------------------------------------------------
QString qCjyxExportNodeDialogPrivate::defaultFilename(vtkDMMLNode* node, QString extension)
{
  ...
  const QString safeNodeName = qCjyxCoreIOManager::forceFileNameValidCharacters(unsafeNodeName);
  return forceFileNameExtension(safeNodeName, extension, node);
}

//-----------------------------------------------------------------------------
template <typename T>
bool qCjyxExportNodeDialogPrivate::setDifferenceIsNonempty(const QList<T>& a, const QList<T>& b)
{
  for (const T & item : a)
    {
    if (!b.contains(item))
...
//-----------------------------------------------------------------------------
bool qCjyxExportNodeDialogPrivate::layoutWidgetsAllInvisible(const QLayout* layout, const QWidget* relativeTo)
{
...
//-----------------------------------------------------------------------------
qCjyxExportNodeDialogPrivate::qCjyxExportNodeDialogPrivate(QWidget* parentWidget)
    ...
    ProtectFilenameLineEdit{false}
{
  ...
  connect(this->RecursiveChildrenCheckBox, &QCheckBox::stateChanged, this, &qCjyxExportNodeDialogPrivate::onNodeInclusionCheckboxStateChanged);
  connect(this->IncludeChildrenCheckBox, &QCheckBox::stateChanged, this, &qCjyxExportNodeDialogPrivate::onNodeInclusionCheckboxStateChanged);
  connect(this->IncludeChildrenCheckBox, &QCheckBox::stateChanged, this, &qCjyxExportNodeDialogPrivate::onIncludeChildrenCheckBoxStateChanged);
...
//-----------------------------------------------------------------------------
qCjyxExportNodeDialogPrivate::~qCjyxExportNodeDialogPrivate() = default;

//-----------------------------------------------------------------------------
const QList<vtkDMMLStorableNode*> & qCjyxExportNodeDialogPrivate::nodeList() const
{
...
//-----------------------------------------------------------------------------
bool qCjyxExportNodeDialogPrivate::setup(
  vtkDMMLScene* scene,
  const QList<vtkDMMLStorableNode*>& nodesNonrecursive,
  const QList<vtkDMMLStorableNode*>& nodesRecursive,
  vtkDMMLStorableNode* selectedNode,
...
)
{
  ...
  this->DirectoryPathLineEdit->setCurrentPath(
    this->LastUsedDirectory.isEmpty() ? this->DMMLScene->GetRootDirectory() : this->LastUsedDirectory
  );
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::onNodeInclusionCheckboxStateChanged(int state)
{
  Q_UNUSED(state);
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::onIncludeChildrenCheckBoxStateChanged(int state)
{
  Q_UNUSED(state);
  this->updatePreserveHierarchyCheckBox();
}
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::updateHardenTransformCheckBox()
{
  bool someNodeHasTransform = false;
  for (vtkDMMLStorableNode* node : this->nodeList())
    {
    vtkDMMLTransformableNode* nodeAsTransformable = vtkDMMLTransformableNode::SafeDownCast(node);
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::updatePreserveHierarchyCheckBox()
{
...
//-----------------------------------------------------------------------------
bool qCjyxExportNodeDialogPrivate::populateNodeTypeWidgetSets()
{
  ...
  for (vtkDMMLStorableNode* node : this->nodeList())
    ...
    if (!this->NodeTypeToNodeTypeWidgetSet.contains(nodeType))
      ...
      try
        {
        NodeTypeWidgetSet* nodeTypeWidgetSet = new NodeTypeWidgetSet(this, node, this->DMMLScene);
        this->NodeTypeToNodeTypeWidgetSet[nodeType] = nodeTypeWidgetSet;
        connect(nodeTypeWidgetSet->exportFormatComboBox, &QComboBox::currentTextChanged, this, &qCjyxExportNodeDialogPrivate::formatChangedSlot);
        }
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::adjustTabbingOrder()
{
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::accept()
{
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::saveWidgetStates()
{
...
//-----------------------------------------------------------------------------
bool qCjyxExportNodeDialogPrivate::exportNodes()
{
  ...
  // Build savingParameterMaps, the list that will be passed to qCjyxCoreIOManager::exportNodes
  QList<qCjyxIO::IOProperties> savingParameterMaps;
  ...
  for (vtkDMMLStorableNode* node : this->nodeList())
    ...
    if (fileInfo.exists() && !replaceYesToAll)
      ...
      qCjyxFileWriterOptionsWidget* options = nodeTypeWidgetSet->optionsWidget;
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::formatChangedSlot()
{
...
//-----------------------------------------------------------------------------
QString qCjyxExportNodeDialogPrivate::recommendedFilename(vtkDMMLStorableNode* node) const
{
  if (!this->FilenameLineEdit->isEnabled())
    {
...
//-----------------------------------------------------------------------------
QDir qCjyxExportNodeDialogPrivate::getSubjectHierarchyBasedDirectory(vtkDMMLStorableNode* node) const
{
  ...
  for (int i = pathList.size()-1; i >=0; --i)
    {
    pathListSanitizedReversed.push_back(qCjyxCoreIOManager::forceFileNameValidCharacters(pathList[i]));
    }
...
//-----------------------------------------------------------------------------
vtkDMMLStorableNode* qCjyxExportNodeDialogPrivate::theOnlyNode() const
{
  if (this->nodeList().size()!=1)
    {
...
//-----------------------------------------------------------------------------
NodeTypeWidgetSet* qCjyxExportNodeDialogPrivate::getNodeTypeWidgetSetSafe(NodeTypeWidgetSet::NodeType nodeType, bool logError) const
{
...
//-----------------------------------------------------------------------------
NodeTypeWidgetSet* qCjyxExportNodeDialogPrivate::theOnlyNodeTypeWidgetSet() const
{
  if (this->NodeTypesInDialog.size()!=1)
    {
...
//-----------------------------------------------------------------------------
QComboBox* qCjyxExportNodeDialogPrivate::theOnlyExportFormatComboBox() const
{
  if (this->NodeTypesInDialog.size()!=1)
    {
    qCritical() << Q_FUNC_INFO << "failed: Expected exactly one node type.";
...
//-----------------------------------------------------------------------------
void qCjyxExportNodeDialogPrivate::onFilenameEditingFinished()
{
  ...
  QString strippedFileName = qCjyxCoreIOManager::forceFileNameValidCharacters(this->FilenameLineEdit->text());
...
//-----------------------------------------------------------------------------
qCjyxExportNodeDialog::qCjyxExportNodeDialog(QObject* parentObject)
  : qCjyxFileDialog(parentObject)
  , d_ptr(new qCjyxExportNodeDialogPrivate(nullptr))
{
}

//-----------------------------------------------------------------------------
qCjyxExportNodeDialog::~qCjyxExportNodeDialog() = default;

//-----------------------------------------------------------------------------
qCjyxIO::IOFileType qCjyxExportNodeDialog::fileType()const
{
  return QString("GenericNodeExport");
}

//-----------------------------------------------------------------------------
QString qCjyxExportNodeDialog::description()const
{
  return tr("Export an individual node");
}

//-----------------------------------------------------------------------------
qCjyxFileDialog::IOAction qCjyxExportNodeDialog::action()const
{
  return qCjyxFileDialog::Write;
}

//-----------------------------------------------------------------------------
bool qCjyxExportNodeDialog::exec(const qCjyxIO::IOProperties& properties)
{
  Q_D(qCjyxExportNodeDialog);
  ...
  for (const QVariant & childID : childIdsNonrecursive)
    ...
    vtkDMMLStorableNode* n = vtkDMMLStorableNode::SafeDownCast(scene->GetNodeByID(childIDString.toUtf8().constData()));
    if (n)
      {
