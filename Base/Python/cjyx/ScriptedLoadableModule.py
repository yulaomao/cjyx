...
This work is supported by NA-MIC, NAC, BIRN, NCIGT, and the Cjyx Community. See <a>https://www.slicer.org</a> for details.
...
"""
        ...
        try:
            ...
            cjyx.selfTests = {}
        cjyx.selfTests[self.moduleName] = self.runTest
    ...
    def getDefaultModuleDocumentationLink(self, docPage=None):
        ...
        link to the module's documentation in current Cjyx version's documentation.
        The text is "For more information see the online documentation."
        If docPage is not specified then the link points to URL returned by :func:`cjyx.app.moduleDocumentationUrl`.
        """
        if docPage:
            url = cjyx.app.documentationBaseUrl + docPage
        else:
            url = cjyx.app.moduleDocumentationUrl(self.moduleName)
...
class ScriptedLoadableModuleWidget:
    def __init__(self, parent=None):
        ...
        self.developerMode = cjyx.util.settingsValue('Developer/DeveloperMode', False, converter=cjyx.util.toBool)
        if not parent:
            self.parent = cjyx.qDMMLWidget()
            self.parent.setLayout(qt.QVBoxLayout())
            self.parent.setDMMLScene(cjyx.dmmlScene)
        else:
            self.parent = parent
    ...
    def resourcePath(self, filename):
        scriptedModulesPath = os.path.dirname(cjyx.util.modulePath(self.moduleName))
        return os.path.join(scriptedModulesPath, 'Resources', filename)

    def cleanup(self):
        """Override this function to implement module widget specific cleanup.

        It is invoked when the signal `qCjyxModuleManager::moduleAboutToBeUnloaded(QString)`
    ...
    def setupDeveloperSection(self):
        ...
        if os.path.isfile(moduleUiFileName):
            ...
            self.editModuleUiButton.connect('clicked()', lambda filename=moduleUiFileName: cjyx.util.startQtDesigner(moduleUiFileName))

        # restart Cjyx button
        # (use this during development, but remove it when delivering
        #  your module to users)
        self.restartButton = qt.QPushButton("Restart Cjyx")
        self.restartButton.toolTip = "Restart Cjyx"
        self.restartButton.name = "ScriptedLoadableModuleTemplate Restart"
        self.restartButton.connect('clicked()', cjyx.app.restart)
    ...
    def onReload(self):
        ...
        cjyx.util.reloadScriptedModule(self.moduleName)
    ...
    def onReloadAndTest(self, **kwargs):
        ...
        passing ``kwargs``.
        """
        with cjyx.util.tryWithErrorDisplay("Reload and Test failed."):
            self.onReload()
            test = cjyx.selfTests[self.moduleName]
            test(msec=int(cjyx.app.userSettings().value("Developer/SelfTestDisplayMessageDelay")), **kwargs)

    def onEditSource(self):
        filePath = cjyx.util.modulePath(self.moduleName)
...
class ScriptedLoadableModuleLogic:
    ...
    def getParameterNode(self):
        ...
        if self.isSingletonParameterNode:
            parameterNode = cjyx.dmmlScene.GetSingletonNode(self.moduleName, "vtkDMMLScriptedModuleNode")
            if parameterNode:
            ...
            numberOfScriptedModuleNodes = cjyx.dmmlScene.GetNumberOfNodesByClass("vtkDMMLScriptedModuleNode")
            for nodeIndex in range(numberOfScriptedModuleNodes):
                parameterNode = cjyx.dmmlScene.GetNthNodeByClass(nodeIndex, "vtkDMMLScriptedModuleNode")
                if parameterNode.GetAttribute("ModuleName") == self.moduleName:
                    return parameterNode
        # no parameter node was found for this module, therefore we add a new one now
        parameterNode = cjyx.dmmlScene.AddNode(self.createParameterNode())
        return parameterNode
    ...
    def createParameterNode(self):
        ...
        The node is of vtkDMMLScriptedModuleNode class. Module name is added as an attribute to allow filtering
        ...
        """
        if cjyx.dmmlScene is None:
            return

        node = cjyx.dmmlScene.CreateNodeByClass("vtkDMMLScriptedModuleNode")
        ...
        node.SetName(cjyx.dmmlScene.GenerateUniqueName(self.moduleName))
        return node
...
class ScriptedLoadableModuleTest(unittest.TestCase):
    ...
    def delayDisplay(self, message, requestedDelay=None, msec=None):
        ...
        and displayed to user by cjyx.util.errorDisplay function.
        """
        ...
        cjyx.util.delayDisplay(message, msec)
    ...
    def takeScreenshot(self, name, description, type=-1):
        ...
        :param type: which viewport to capture. If not specified then captures the entire window.
            Valid values: cjyx.qDMMLScreenShotDialog.FullLayout,
            cjyx.qDMMLScreenShotDialog.ThreeD, cjyx.qDMMLScreenShotDialog.Red,
            cjyx.qDMMLScreenShotDialog.Yellow, cjyx.qDMMLScreenShotDialog.Green.
        """
        ...
        widget = 0
        if type == cjyx.qDMMLScreenShotDialog.FullLayout:
            # full layout
            widget = lm.viewport()
        elif type == cjyx.qDMMLScreenShotDialog.ThreeD:
            # just the 3D window
            widget = lm.threeDWidget(0).threeDView()
        elif type == cjyx.qDMMLScreenShotDialog.Red:
            # red slice window
            widget = lm.sliceWidget("Red")
        elif type == cjyx.qDMMLScreenShotDialog.Yellow:
            # yellow slice window
            widget = lm.sliceWidget("Yellow")
        elif type == cjyx.qDMMLScreenShotDialog.Green:
            # green slice window
            widget = lm.sliceWidget("Green")
        else:
            # default to using the full window
            widget = cjyx.util.mainWindow()
            # reset the type so that the node is set correctly
            type = cjyx.qDMMLScreenShotDialog.FullLayout

        # grab and convert to vtk image data
        qimage = ctk.ctkWidgetsUtils.grabWidget(widget)
        imageData = vtk.vtkImageData()
        cjyx.qDMMLUtils().qImageToVtkImageData(qimage, imageData)

        annotationLogic = cjyx.modules.annotations.logic()
