...
def createNode(cliModule, parameters=None):
    """Creates a new vtkDMMLCommandLineModuleNode for a specific module, with
    optional parameters"""
    if not cliModule:
        return None
...
def setNodeParameters(node, parameters):
    """Sets parameters for a vtkDMMLCommandLineModuleNode given a dictionary
    ...
    For images, geometry, points and regions, provide a vtkDMMLNode
    """
    import cjyx
    if not node:
        return None
    ...
    for key, value in parameters.items():
        ...
        elif isinstance(value, cjyx.vtkDMMLNode):
...
def run(module, node=None, parameters=None, wait_for_completion=False, delete_temporary_files=True, update_display=True):
    ...
    # import cjyx.util
    # widget = cjyx.util.getModuleGui(module)
    # if not widget:
