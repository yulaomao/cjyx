...
"""Base URL for downloading data from Cjyx Data Store. Data store contains atlases,
...
def exit(status=EXIT_SUCCESS):
    ...
    from cjyx import app
    # Prevent automatic application exit (for example, triggered by starting Cjyx
...
def _readCMakeCache(var):
    ...
    try:
        with open(os.path.join(app.cjyxHome, "CMakeCache.txt")) as cache:
            for line in cache:
...
def sourceDir():
    """Location of the Cjyx source directory.

    :type: :class:`str` or ``None``

    This provides the location of the Cjyx source directory, if Cjyx is being
    run from a CMake build directory. If the Cjyx home directory does not
    contain a ``CMakeCache.txt`` (e.g. for an installed Cjyx), the property
    will have the value ``None``.
    """
    return _readCMakeCache('Cjyx_SOURCE_DIR')


def startupEnvironment():
    """Returns the environment without the Cjyx specific values.
...
def lookupTopLevelWidget(objectName):
    """Loop over all top level widget associated with 'cjyx.app' and
...
def mainWindow():
    """Get main window widget (qCjyxMainWindow object)

    :return: main window widget, or ``None`` if there is no main window
    """
    try:
        mw = lookupTopLevelWidget('qCjyxMainWindow')
    except RuntimeError:
...
def findChildren(widget=None, name="", text="", title="", className=""):
    ...
    If no widget is provided, cjyx.util.mainWindow() is used.
...
def startQtDesigner(args=None):
    ...
    return cjyx.app.launchDesigner(cmdLineArguments)
...
def childWidgetVariables(widget):
    ...
    Example::
      ...
      uiWidget = cjyx.util.loadUI(myUiFilePath)
      self.ui = cjyx.util.childWidgetVariables(uiWidget)
      self.ui.inputSelector.setDMMLScene(cjyx.dmmlScene)
      self.ui.outputSelector.setDMMLScene(cjyx.dmmlScene)
...
def addParameterEditWidgetConnections(parameterEditWidgets, updateParameterNodeFromGUI):
      ...
      class SurfaceToolboxWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
        ...
        def setup(self):
          ...
          self.parameterEditWidgets = [
            ...
            ...]
          cjyx.util.addParameterEditWidgetConnections(self.parameterEditWidgets, self.updateParameterNodeFromGUI)

        def updateGUIFromParameterNode(self, caller=None, event=None):
          if self._parameterNode is None or self._updatingGUIFromParameterNode:
            return
          self._updatingGUIFromParameterNode = True
          cjyx.util.updateParameterEditWidgetsFromNode(self.parameterEditWidgets, self._parameterNode)
        ...
        def updateParameterNodeFromGUI(self, caller=None, event=None):
          ...
          cjyx.util.updateNodeFromParameterEditWidgets(self.parameterEditWidgets, self._parameterNode)
    ...
    for (widget, parameterName) in parameterEditWidgets:
        ...
        elif widgetClassName == "qDMMLNodeComboBox":
...
def updateParameterEditWidgetsFromNode(parameterEditWidgets, parameterNode):
    """ Update widgets from values stored in a vtkDMMLScriptedModuleNode.

    The function is useful for implementing updateGUIFromParameterNode.

    Note: Only a few widget classes are supported now. More will be added later. Report any missing classes at discourse.cjyx.org.
...
def updateNodeFromParameterEditWidgets(parameterEditWidgets, parameterNode):
    """ Update vtkDMMLScriptedModuleNode from widgets.
...
def setSliceViewerLayers(background='keep-current', foreground='keep-current', label='keep-current',
    ...
    def _nodeID(nodeOrID):
        nodeID = nodeOrID
        if isinstance(nodeOrID, cjyx.vtkDMMLNode):
            nodeID = nodeOrID.GetID()
        return nodeID

    num = cjyx.dmmlScene.GetNumberOfNodesByClass('vtkDMMLSliceCompositeNode')
    for i in range(num):
        sliceViewer = cjyx.dmmlScene.GetNthNodeByClass(i, 'vtkDMMLSliceCompositeNode')
    ...
    if rotateToVolumePlane:
        if background != 'keep-current':
            volumeNode = cjyx.dmmlScene.GetNodeByID(_nodeID(background))
        elif foreground != 'keep-current':
            volumeNode = cjyx.dmmlScene.GetNodeByID(_nodeID(foreground))
        elif label != 'keep-current':
            volumeNode = cjyx.dmmlScene.GetNodeByID(_nodeID(label))
        else:
            volumeNode = None
        if volumeNode:
            layoutManager = cjyx.app.layoutManager()
            for sliceViewName in layoutManager.sliceViewNames():
                layoutManager.sliceWidget(sliceViewName).dmmlSliceNode().RotateToVolumePlane(volumeNode)

    if fit:
        layoutManager = cjyx.app.layoutManager()
        if layoutManager is not None:
            sliceLogics = layoutManager.dmmlSliceLogics()
...
def setToolbarsVisible(visible, ignore=None):
    ...
    try:
        import cjyx
        cjyx.modules.sequences.autoShowToolBar = visible
    except:
...
def loadNodeFromFile(filename, filetype, properties={}, returnNode=False):
    ...
    """
    from cjyx import app, vtkDMMLMessageCollection
    from vtk import vtkCollection
    properties['fileName'] = filename

    loadedNodesCollection = vtkCollection()
    userMessages = vtkDMMLMessageCollection()
...
def openAddSegmentationDialog():
    from cjyx import app, qCjyxFileDialog
    return app.coreIOManager().openDialog('SegmentationFile', qCjyxFileDialog.Read)
...
def openAddShaderPropertyDialog():
    from cjyx import app, qCjyxFileDialog
    return app.coreIOManager().openDialog('ShaderPropertyFile', qCjyxFileDialog.Read)
...
def saveNode(node, filename, properties={}):
    ...
    method 'qCjyxCoreIOManager::fileWriterFileType(vtkObject*)'. This can be done
...
def saveScene(filename, properties={}):
    ...
    as a DMML file, MRB file or directory.

    If filename ends with '.dmml', the scene is saved as a single file
    without associated data.

    If filename ends with '.mrb', the scene is saved as a DMML bundle (Zip
    archive with scene and data files).

    In every other case, the scene is saved in the directory
    specified by 'filename'. Both DMML scene file and data
...
def exportNode(node, filename, properties={}, world=False):
    ...
    """
    from cjyx import app, vtkDataFileFormatHelper, vtkDMMLMessageCollection
    nodeIDs = [node.GetID()]
    fileNames = [filename]
...
def getModuleWidget(module):
    ...
    if isinstance(widgetRepr, cjyx.qCjyxScriptedLoadableModuleWidget):
...
def getModuleLogic(module):
    ...
    if isinstance(module, str):
        module = getModule(module)
    if isinstance(module, cjyx.qCjyxScriptedLoadableModule):
        try:
...
def modulePath(moduleName):
    ...
    """
    import cjyx  # noqa: F401
    return eval('cjyx.modules.%s.path' % moduleName.lower())
...
def reloadScriptedModule(moduleName):
    ...
    * Call ``setup()`` function
    * Update ``cjyx.modules.<moduleName>Widget`` attribute
    """
    import imp, sys, os
    import cjyx
    ...
    parent = eval('cjyx.modules.%s.widgetRepresentation()' % moduleName.lower())
    ...
    # application exit)
    if hasattr(cjyx.modules, widgetName):
        widget = getattr(cjyx.modules, widgetName)
        widget.cleanup()

        if hasattr(widget, '_onModuleAboutToBeUnloaded'):
            cjyx.app.moduleManager().disconnect('moduleAboutToBeUnloaded(QString)', widget._onModuleAboutToBeUnloaded)
    ...
    widget.setup()
    setattr(cjyx.modules, widgetName, widget)
...
def resetThreeDViews():
    ...
    import cjyx
    cjyx.app.layoutManager().resetThreeDViews()


def resetSliceViews():
    """Reset focal view around volumes"""
    import cjyx
    manager = cjyx.app.layoutManager().resetSliceViews()
...
class DMMLNodeNotFoundException(Exception):
    """Exception raised when a requested DMML node was not found."""
    pass
...
def getNodes(pattern="*", scene=None, useLists=False):
    ...
    """
    import cjyx, collections, fnmatch
    nodes = collections.OrderedDict()
    if scene is None:
        scene = cjyx.dmmlScene
...
def getNode(pattern="*", index=0, scene=None):
    ...
    associated with ``cjyx.dmmlScene``.

    :raises DMMLNodeNotFoundException: if no node is found
     that matches the specified pattern.
    """
    nodes = getNodes(pattern, scene)
    if not nodes:
        raise DMMLNodeNotFoundException("could not find nodes in the scene by name or id '%s'" % (pattern if (isinstance(pattern, str)) else ""))
...
def getNodesByClass(className, scene=None):
    ...
    if scene is None:
        scene = cjyx.dmmlScene
    nodes = cjyx.dmmlScene.GetNodesByClass(className)
    nodes.UnRegister(cjyx.dmmlScene)
    nodeList = []
    nodes.InitTraversal()
...
def getFirstNodeByName(name, className=None):
    """Get the first DMML node that name starts with the specified name.
...
class NodeModify:
    """Context manager to conveniently compress dmml node modified event."""
...
class RenderBlocker:
    ...
    Code blocks such as::
      ...
      try:
        cjyx.app.pauseRender()
        # Do things
      finally:
        cjyx.app.resumeRender()

    Can be written as::

      with cjyx.util.RenderBlocker():
        # Do things
...
def array(pattern="", index=0):
    ...
    DMML node that matches the pattern.
    ...
    import cjyx
    if isinstance(node, cjyx.vtkDMMLVolumeNode):
        return arrayFromVolume(node)
    elif isinstance(node, cjyx.vtkDMMLModelNode):
        return arrayFromModelPoints(node)
    elif isinstance(node, cjyx.vtkDMMLGridTransformNode):
        return arrayFromGridTransform(node)
    elif isinstance(node, cjyx.vtkDMMLMarkupsNode):
        return arrayFromMarkupsControlPoints(node)
    elif isinstance(node, cjyx.vtkDMMLTransformNode):
...
def arrayFromVolume(volumeNode):
    ...
    """
    scalarTypes = ['vtkDMMLScalarVolumeNode', 'vtkDMMLLabelMapVolumeNode']
    vectorTypes = ['vtkDMMLVectorVolumeNode', 'vtkDMMLMultiVolumeNode', 'vtkDMMLDiffusionWeightedVolumeNode']
    tensorTypes = ['vtkDMMLDiffusionTensorVolumeNode']
...
def arrayFromModelPolyIds(modelNode):
    ...
    Typically in Cjyx n(j) will always be 3 because a model node's
...
def arrayFromSegmentBinaryLabelmap(segmentationNode, segmentId, referenceVolumeNode=None):
    ...
    if not referenceVolumeNode:
        referenceVolumeNode = segmentationNode.GetNodeReference(cjyx.vtkDMMLSegmentationNode.GetReferenceImageGeometryReferenceRole())
        if not referenceVolumeNode:
    ...
    labelmapVolumeNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLLabelMapVolumeNode", "__temp__")
    try:
        if not cjyx.modules.segmentations.logic().ExportSegmentsToLabelmapNode(segmentationNode, segmentIds, labelmapVolumeNode, referenceVolumeNode):
            raise RuntimeError("Export of segment failed.")
        narray = cjyx.util.arrayFromVolume(labelmapVolumeNode)
    finally:
        cjyx.dmmlScene.RemoveNode(labelmapVolumeNode)
...
def updateSegmentBinaryLabelmapFromArray(narray, segmentationNode, segmentId, referenceVolumeNode=None):
    ...
    labelmapVolumeNode = cjyx.modules.volumes.logic().CreateAndAddLabelVolume(referenceVolumeNode, "__temp__")
    try:
        ...
        segmentIds = vtk.vtkStringArray()
        segmentIds.InsertNextValue(segmentId)
        if not cjyx.modules.segmentations.logic().ImportLabelmapToSegmentationNode(labelmapVolumeNode, segmentationNode, segmentIds):
...
def updateVolumeFromArray(volumeNode, narray):
    ...
    # Probably this could be fixed in VTK or Cjyx but for now just reject it.
    ...
    # (same notifications as in vtkDMMLVolumeNode.SetImageDataConnection)
    import cjyx
    volumeNode.StorableModified()
    volumeNode.Modified()
    volumeNode.InvokeEvent(cjyx.vtkDMMLVolumeNode.ImageDataModifiedEvent, volumeNode)
...
def addVolumeFromArray(narray, ijkToRAS=None, name=None, nodeClassName=None):
    ...
    :param nodeClassName: type of created volume, default: ``vtkDMMLScalarVolumeNode``.
      Use ``vtkDMMLLabelMapVolumeNode`` for labelmap volume, ``vtkDMMLVectorVolumeNode`` for vector volume.
    ...
    Example::
      ...
      import numpy as np
      volumeNode = cjyx.util.addVolumeFromArray(np.zeros((30, 40, 50)))

    Example::

      # create labelmap volume filled with voxel value of 120
      import numpy as np
      volumeNode = cjyx.util.addVolumeFromArray(np.ones((30, 40, 50), 'int8') * 120,
        np.diag([0.2, 0.2, 0.5, 1.0]), nodeClassName="vtkDMMLLabelMapVolumeNode")
    """
    import cjyx
    from vtk import vtkMatrix4x4
    ...
    if nodeClassName is None:
        nodeClassName = "vtkDMMLScalarVolumeNode"

    volumeNode = cjyx.dmmlScene.AddNewNodeByClass(nodeClassName, name)
...
def updateTableFromArray(tableNode, narrays, columnNames=None):
    ...
    Example::
      ...
      tableNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLTableNode")
...
def dataframeFromTable(tableNode):
    ...
    try:
        ...
        raise ImportError("Failed to convert to pandas dataframe. Please install pandas by running `cjyx.util.pip_install('pandas')`")
...
def toVTKString(text):
    ...
    .. deprecated:: 4.11.0
      Since now VTK assumes that all strings are in UTF-8 and all strings in Cjyx are UTF-8, too,
...
def tempDirectory(key='__CjyxTemp__', tempDir=None, includeDateTime=True):
    ...
    import qt, cjyx
    if not tempDir:
        tempDir = qt.QDir(cjyx.app.temporaryPath)
    if includeDateTime:
...
def delayDisplay(message, autoCloseMsec=1000, parent=None, **kwargs):
    ...
    If ``0 <= autoCloseMsec < 400`` then only ``cjyx.app.processEvents()`` is called.
    ...
    # (after creating and closing a messagebox, hovering over the mouse on Cjyx icon, moving up the
...
def infoDisplay(text, windowTitle=None, parent=None, standardButtons=None, **kwargs):
    ...
    If there is no main window, or if the application is running in testing mode (``cjyx.app.testingEnabled() == True``),
...
def confirmOkCancelDisplay(text, windowTitle=None, parent=None, **kwargs):
    ...
    When the application is running in testing mode (``cjyx.app.testingEnabled() == True``),
    the popup is skipped and True ("Ok") is returned, with a message being logged to indicate this.
    """
    import qt, cjyx, logging
    if not windowTitle:
        windowTitle = cjyx.app.applicationName + " confirmation"
...
def _messageDisplay(logLevel, text, testingReturnValue, mainWindowNeeded=False, parent=None, windowTitle=None, **kwargs):
    ...
    :param parent: The message box parent; by default it is set to main window by cjyx.util.messageBox
    ...
    if not windowTitle:
        windowTitle = cjyx.app.applicationName + " " + logLevelString
    if cjyx.app.testingEnabled():
...
def messageBox(text, parent=None, **kwargs):
    ...
    For example::
      ...
      cjyx.util.messageBox("Some message", dontShowAgainSettingsKey = "MainWindow/DontShowSomeMessage")
    ...
    if cjyx.app.testingEnabled():
        ...
        cjyx.util.delayDisplay(text, autoCloseMsec=3000, parent=parent, **kwargs)
        return testingReturnValue
...
def displayPythonShell(display=True):
    ...
    .. code-block:: python
      ...
      with cjyx.util.displayPythonShell():
        cjyx.util.pip_install('nibabel')
...
class WaitCursor:
    ...
    .. code-block:: python
      ...
      n = 2
      with cjyx.util.MessageDialog(f'Sleeping for {n} seconds...'):
        with cjyx.util.WaitCursor():
          time.sleep(n)
...
class MessageDialog:
    def __init__(self, message, show=True, logLevel=None):
        ...
        When the application is running in testing mode (``cjyx.app.testingEnabled() == True``), the message box is skipped.
        ...
        self.show = show and not cjyx.app.testingEnabled()
...
def tryWithErrorDisplay(message=None, show=True, waitCursor=False):
    ...
    .. code-block:: python
      ...
      with cjyx.util.tryWithErrorDisplay("Risky operation failed."):
        risky()
    """
    try:
        if waitCursor:
        ...
        if waitCursor:
            cjyx.app.restoreOverrideCursor()
        if show and not cjyx.app.testingEnabled():
...
def clickAndDrag(widget, button='Left', start=(10, 10), end=(10, 40), steps=20, modifiers=[]):
    """Send synthetic mouse events to the specified widget (qDMMLSliceWidget or qDMMLThreeDView)
...
def plot(narray, xColumnIndex=-1, columnNames=None, title=None, show=True, nodes=None):
    ...
    .. code-block:: python
      ...
      chartNode.GetNthPlotSeriesNode(0).SetPlotType(cjyx.vtkDMMLPlotSeriesNode.PlotTypeScatterBar)
    ...
    if chartNode is None:
        chartNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLPlotChartNode")
    if title is not None:
    ...
    for columnIndex in range(numberOfColumns):
        ...
        if len(seriesNodes) > seriesIndex:
            ...
            seriesNode = cjyx.dmmlScene.AddNewNodeByClass("vtkDMMLPlotSeriesNode")
    ...
    # Show plot in layout
    if show:
        cjyx.modules.plots.logic().ShowChartInLayout(chartNode)
...
def launchConsoleProcess(args, useStartupEnvironment=True, updateEnvironment=None, cwd=None):
    ...
    :param useStartupEnvironment: launch the process in the original environment as the original Cjyx process
...
def _executePythonModule(module, args):
    """Execute a Python module as a script in Cjyx's Python environment.

    Internally python -m is called with the module name and additional arguments.

    :raises RuntimeError: in case of failure
    """
    # Determine pythonCjyxExecutablePath
    try:
        from cjyx import app  # noqa: F401
        # If we get to this line then import from "app" is succeeded,
        # which means that we run this function from Cjyx Python interpreter.
        # PythonCjyx is added to PATH environment variable in Cjyx
        # therefore shutil.which will be able to find it.
        import shutil
        pythonCjyxExecutablePath = shutil.which('PythonCjyx')
        if not pythonCjyxExecutablePath:
            raise RuntimeError("PythonCjyx executable not found")
    except ImportError:
        # Running from console
        import os
        import sys
        pythonCjyxExecutablePath = os.path.dirname(sys.executable) + "/PythonCjyx"
        if os.name == 'nt':
            pythonCjyxExecutablePath += ".exe"

    commandLine = [pythonCjyxExecutablePath, "-m", module, *args]
...
def pip_install(requirements):
    ...
    Example: calling from Cjyx GUI

    .. code-block:: python

      pip_install("tensorflow keras scikit-learn ipywidgets")

    Example: calling from PythonCjyx console

    .. code-block:: python

      from cjyx.util import pip_install
...
def pip_uninstall(requirements):
    ...
    .. code-block:: python
      ...
      from cjyx.util import pip_uninstall
