...
set(PYTHON_STDLIB_SUBDIR lib/python${Cjyx_REQUIRED_PYTHON_VERSION_DOT})
...
if((NOT DEFINED PYTHON_INCLUDE_DIR
   ...
   OR NOT DEFINED PYTHON_EXECUTABLE) AND NOT Cjyx_USE_SYSTEM_${proj})

  set(python_SOURCE_DIR "${CMAKE_BINARY_DIR}/Python-${Cjyx_REQUIRED_PYTHON_VERSION}")
  ...
  ExternalProject_Add(python-source
    URL ${_download_${Cjyx_REQUIRED_PYTHON_VERSION}_url}
    URL_MD5 ${_download_${Cjyx_REQUIRED_PYTHON_VERSION}_md5}
  ...
  if(UNIX)
    ...
    set(cjyx_PYTHON_SHARED_LIBRARY_DIR ${python_DIR}/lib)
    set(PYTHON_INCLUDE_DIR ${python_DIR}/include/python${Cjyx_REQUIRED_PYTHON_VERSION_DOT}${Cjyx_REQUIRED_PYTHON_ABIFLAGS})
    set(PYTHON_LIBRARY ${python_DIR}/lib/libpython${Cjyx_REQUIRED_PYTHON_VERSION_DOT}${Cjyx_REQUIRED_PYTHON_ABIFLAGS}.${python_IMPORT_SUFFIX})
    set(PYTHON_EXECUTABLE ${python_DIR}/bin/PythonCjyx)
    set(cjyx_PYTHON_REAL_EXECUTABLE ${python_DIR}/bin/python)
  elseif(WIN32)
    set(cjyx_PYTHON_SHARED_LIBRARY_DIR ${python_DIR}/bin)
    set(PYTHON_INCLUDE_DIR ${python_DIR}/include)
    set(PYTHON_LIBRARY ${python_DIR}/libs/python${Cjyx_REQUIRED_PYTHON_VERSION_MAJOR}${Cjyx_REQUIRED_PYTHON_VERSION_MINOR}.lib)
    set(PYTHON_EXECUTABLE ${python_DIR}/bin/PythonCjyx.exe)
    set(cjyx_PYTHON_REAL_EXECUTABLE ${python_DIR}/bin/python.exe)
  else()
  ...
  if(NOT Cjyx_USE_SYSTEM_python)
    ExternalProject_Add_Step(${proj} configure_python_launcher
      COMMAND ${CMAKE_COMMAND}
        ...
        -DPYTHON_REAL_EXECUTABLE:FILEPATH=${cjyx_PYTHON_REAL_EXECUTABLE}
        -DPYTHON_SHARED_LIBRARY_DIR:PATH=${cjyx_PYTHON_SHARED_LIBRARY_DIR}
        -DPYTHON_SITE_PACKAGES_SUBDIR:STRING=${PYTHON_SITE_PACKAGES_SUBDIR}
        -DPYTHON_STDLIB_SUBDIR:STRING=${PYTHON_STDLIB_SUBDIR}
        -DCjyx_BIN_DIR:PATH=${Cjyx_BIN_DIR}
        -DCjyx_BINARY_DIR:PATH=${Cjyx_BINARY_DIR}
        -DCjyx_BINARY_INNER_SUBDIR:STRING=${Cjyx_BINARY_INNER_SUBDIR}
        -DCjyx_LIB_DIR:PATH=${Cjyx_LIB_DIR}
        -DCjyx_SHARE_DIR:PATH=${Cjyx_SHARE_DIR}
        -DCjyx_SOURCE_DIR:PATH=${Cjyx_SOURCE_DIR}
        -DCjyx_REVISION:STRING=${Cjyx_REVISION}
        -DCjyx_ORGANIZATION_DOMAIN:STRING=${Cjyx_ORGANIZATION_DOMAIN}
        -DCjyx_ORGANIZATION_NAME:STRING=${Cjyx_ORGANIZATION_NAME}
        -DCjyx_MAIN_PROJECT_APPLICATION_NAME:STRING=${Cjyx_MAIN_PROJECT_APPLICATION_NAME}
        -DCJYX_REVISION_SPECIFIC_USER_SETTINGS_FILEBASENAME:STRING=${CJYX_REVISION_SPECIFIC_USER_SETTINGS_FILEBASENAME}
        -P ${Cjyx_SOURCE_DIR}/SuperBuild/python_configure_python_launcher.cmake
      COMMAND ${CMAKE_COMMAND}
    ...
    # Note: Install rules for PythonCjyxLauncherSettingsToInstall.ini and PythonCjyx executable
      ...
      if(_configure_lsb_release_wrapper)
        ExternalProject_Add_Step(${proj} configure_lsb_release_wrapper
          COMMAND ${CMAKE_COMMAND}
            ...
            -DPYTHON_REAL_EXECUTABLE:FILEPATH=${cjyx_PYTHON_REAL_EXECUTABLE}
            -P ${Cjyx_SOURCE_DIR}/SuperBuild/python_configure_lsb_release_wrapper.cmake
          DEPENDEES install
          )
  ...
  #-----------------------------------------------------------------------------
  # Cjyx Launcher setting specific to build tree
  ...
  #-----------------------------------------------------------------------------
  # Cjyx Launcher setting specific to install tree
  ...
  # This is required to support the case when Cjyx is built within
  # a virtual environment. More specifically, since Cjyx links against
