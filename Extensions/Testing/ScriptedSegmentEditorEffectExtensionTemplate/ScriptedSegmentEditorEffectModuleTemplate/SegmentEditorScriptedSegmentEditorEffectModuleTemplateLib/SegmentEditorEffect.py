...
"""
    ...
    def setupOptionsFrame(self):
        ...
        # Object scale slider
        self.objectScaleMmSlider = cjyx.qDMMLSliderWidget()
        self.objectScaleMmSlider.setDMMLScene(cjyx.dmmlScene)
        self.objectScaleMmSlider.quantity = "length"  # get unit, precision, etc. from DMML unit node
        ...
        self.objectScaleMmSlider.connect('valueChanged(double)', self.updateDMMLFromGUI)
    ...
    def onApply(self):
        ...
        masterVolumeNode = cjyx.vtkDMMLScalarVolumeNode()
        cjyx.dmmlScene.AddNode(masterVolumeNode)
        masterVolumeNode.SetAndObserveTransformNodeID(segmentationNode.GetTransformNodeID())
        cjyx.vtkCjyxSegmentationsModuleLogic.CopyOrientedImageDataToVolumeNode(self.scriptedEffect.masterVolumeImageData(), masterVolumeNode)
        # Generate merged labelmap of all visible segments, as the filter expects a single labelmap with all the labels.
        mergedLabelmapNode = cjyx.vtkDMMLLabelMapVolumeNode()
        cjyx.dmmlScene.AddNode(mergedLabelmapNode)
        cjyx.vtkCjyxSegmentationsModuleLogic.ExportSegmentsToLabelmapNode(segmentationNode, visibleSegmentIds, mergedLabelmapNode, masterVolumeNode)

        # Run segmentation algorithm
        import SimpleITK as sitk
        import sitkUtils
        # Read input data from Cjyx into SimpleITK
        labelImage = sitk.ReadImage(sitkUtils.GetCjyxITKReadWriteAddress(mergedLabelmapNode.GetName()))
        backgroundImage = sitk.ReadImage(sitkUtils.GetCjyxITKReadWriteAddress(masterVolumeNode.GetName()))
        ...
        # Write result from SimpleITK to Cjyx. This currently performs a deep copy of the bulk data.
        sitk.WriteImage(labelImage, sitkUtils.GetCjyxITKReadWriteAddress(mergedLabelmapNode.GetName()))
        ...
        cjyx.vtkCjyxSegmentationsModuleLogic.ImportLabelmapToSegmentationNode(mergedLabelmapNode, segmentationNode, visibleSegmentIds)
        cjyx.dmmlScene.RemoveNode(masterVolumeNode)
        cjyx.dmmlScene.RemoveNode(mergedLabelmapNode)
