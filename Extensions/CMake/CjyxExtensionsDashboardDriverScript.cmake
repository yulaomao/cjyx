...
# Helper function used to extract cjyx revision within a header file. The revision is
# expected to be defined using the format '#define Cjyx_REVISION "abcd123456"'
function(_get_cjyx_revision headerfile varname)
  set(_macroname Cjyx_REVISION)
...
# Defaults
#-----------------------------------------------------------------------------
if(DEFINED Cjyx_RELEASE_TYPE)
  if("${Cjyx_RELEASE_TYPE}" MATCHES "^E.*")
    set(Cjyx_RELEASE_TYPE "Experimental")
  elseif("${Cjyx_RELEASE_TYPE}" MATCHES "^P.*")
    set(Cjyx_RELEASE_TYPE "Preview")
  elseif("${Cjyx_RELEASE_TYPE}" MATCHES "^S.*")
    set(Cjyx_RELEASE_TYPE "Stable")
  else()
    message(FATAL_ERROR "Invalid value for Cjyx_RELEASE_TYPE [${Cjyx_RELEASE_TYPE}]")
  endif()
else()
  set(Cjyx_RELEASE_TYPE "Experimental")
endif()
...
if(NOT DEFINED CTEST_BINARY_DIRECTORY)
  list(APPEND expected_variables
    ...
    Cjyx_DIRECTORY_IDENTIFIER
    )
  set(CTEST_BINARY_DIRECTORY "${DASHBOARDS_DIR}/${EXTENSION_DASHBOARD_SUBDIR}/${EXTENSION_DIRECTORY_BASENAME}-${Cjyx_DIRECTORY_IDENTIFIER}-E${testing_suffix}-b")
endif()
list(APPEND expected_variables CTEST_BINARY_DIRECTORY)

file(WRITE "${CTEST_BINARY_DIRECTORY} - CjyxExtensions-build-${CTEST_BUILD_NAME}-${SCRIPT_MODE}${testing_long_suffix}-${EXTENSIONS_INDEX_BRANCH}.txt" "Generated by ${CTEST_SCRIPT_NAME}")
...
if(NOT DEFINED EXTENSIONS_BUILDSYSTEM_SOURCE_DIRECTORY)
  ...
  set(EXTENSIONS_BUILDSYSTEM_SOURCE_DIRECTORY "${Cjyx_SOURCE_DIR}/Extensions/CMake")
endif()
...
setIfNotDefined(CTEST_DROP_SITE "cjyx.cdash.org")
...
setIfNotDefined(Cjyx_UPLOAD_EXTENSIONS TRUE)
...
if(NOT DEFINED CDASH_PROJECT_NAME)
  ...
  if("${Cjyx_RELEASE_TYPE}" STREQUAL "Stable")
    set(CDASH_PROJECT_NAME  "CjyxStable")
  endif()
endif()
...
if(NOT EXTENSIONS_BUILDSYSTEM_TESTING)
  set(Cjyx_EXTENSION_DESCRIPTION_DIR "${CTEST_BINARY_DIRECTORY}/ExtensionsIndex")
endif()
list(APPEND variables Cjyx_EXTENSION_DESCRIPTION_DIR)
...
# Used in CjyxExtensionPackageAndUploadTarget CMake module
...
if(NOT EXTENSIONS_BUILDSYSTEM_TESTING)
  ...
  get_filename_component(src_dir ${Cjyx_EXTENSION_DESCRIPTION_DIR} REALPATH)
  ...
  if(NOT EXISTS "${Cjyx_EXTENSION_DESCRIPTION_DIR}/.git")
    _write_gitclone_script(
      ...
      ${Cjyx_EXTENSION_DESCRIPTION_DIR} # source_dir
  ...
  # Retrieve revision associated with Cjyx build tree
  set(cjyx_version_header "${Cjyx_DIR}/vtkCjyxVersionConfigure.h")
  if(NOT EXISTS "${cjyx_version_header}")
    message(FATAL_ERROR "Aborting build. Could not find 'vtkCjyxVersionConfigure.h' in Cjyx_DIR [${Cjyx_DIR}].")
  endif()
  _get_cjyx_revision("${cjyx_version_header}" Cjyx_REVISION)
  message(STATUS "Cjyx_REVISION:${Cjyx_REVISION}")
  set(Cjyx_PREVIOUS_REVISION ${Cjyx_REVISION})
...
macro(run_ctest)
  ...
  set(cjyx_build_in_progress FALSE)
  set(build_in_progress_file ${Cjyx_DIR}/BUILD_IN_PROGRESS)
  if(EXISTS ${build_in_progress_file})
    set(cjyx_build_in_progress TRUE)
  endif()

  if(cjyx_build_in_progress)
    message("Skipping run_ctest() - Cjyx build in PROGRESS")
  else()
    ...
    if(NOT EXTENSIONS_BUILDSYSTEM_TESTING AND run_ctest_with_update)
      ctest_update(SOURCE "${Cjyx_EXTENSION_DESCRIPTION_DIR}" RETURN_VALUE FILES_UPDATED)
    endif()
...
Cjyx_EXTENSION_DESCRIPTION_DIR:PATH=${Cjyx_EXTENSION_DESCRIPTION_DIR}")
      endif()
...
Cjyx_EXTENSIONS_TRACK_QUALIFIER:STRING=${EXTENSIONS_TRACK_QUALIFIER}
GIT_EXECUTABLE:FILEPATH=${CTEST_GIT_COMMAND}
Cjyx_DIR:PATH=${Cjyx_DIR}
Cjyx_UPLOAD_EXTENSIONS:BOOL=${Cjyx_UPLOAD_EXTENSIONS}
${OPTIONAL_CACHE_CONTENT}
${ADDITIONAL_CMAKECACHE_OPTION}
")
    endif()

    _get_cjyx_revision("${Cjyx_DIR}/vtkCjyxVersionConfigure.h" Cjyx_WC_REVISION)
    set(cjyx_source_updated FALSE)
    if(NOT "${Cjyx_PREVIOUS_WC_REVISION}" STREQUAL "${Cjyx_WC_REVISION}")
      set(cjyx_source_updated TRUE)
    endif()

    message(STATUS "FILES_UPDATED ................: ${FILES_UPDATED}")
    message(STATUS "force_build ..................: ${force_build}")
    message(STATUS "Cjyx_PREVIOUS_WC_REVISION ..: ${Cjyx_PREVIOUS_WC_REVISION}")
    message(STATUS "Cjyx_WC_REVISION ...........: ${Cjyx_WC_REVISION}")

    if(FILES_UPDATED GREATER 0 OR force_build OR cjyx_source_updated)

      set(Cjyx_PREVIOUS_WC_REVISION ${Cjyx_WC_REVISION})
      set(force_build FALSE)
