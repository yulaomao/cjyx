...
#
#  Cjyx_EXT_METADATA_NAMES: List of metadata names (e.g SCM, SCMURL, ...)
#
#  Cjyx_EXT_REQUIRED_METADATA_NAMES: List of required metadata names
#
#  Cjyx_EXT_OPTIONAL_METADATA_NAMES: List of optional metadata names
#
#  And for each metadata:
#
#    Cjyx_EXT_METADATA_<name>_REQUIRED: Set to 1 if mandatory.
#
#    Cjyx_EXT_<name>_DEFAULT          : Default value if any.
#
#    Cjyx_EXT_<name>_DESCRIPTION      : Describe the metadata.
#
...
#
#  cjyx_extension_metadata_set_defaults(<prefix>):
#
#  For each optional metadata, set variables "<prefix>_<name>"
#  initialized to "Cjyx_EXT_<name>_DEFAULT" if "<prefix>_<name>"
#  was not already defined.
#

set(Cjyx_EXT_METADATA_NAMES )

macro(_define_metadata name description required default)
  set(Cjyx_EXT_METADATA_${name}_REQUIRED    "${required}")
  set(Cjyx_EXT_METADATA_${name}_DEFAULT     "${default}")
  set(Cjyx_EXT_METADATA_${name}_DESCRIPTION "${description}")
  list(APPEND Cjyx_EXT_METADATA_NAMES ${name})
endmacro()
...
set(Cjyx_EXT_REQUIRED_METADATA_NAMES )
set(Cjyx_EXT_OPTIONAL_METADATA_NAMES )
foreach(name IN LISTS Cjyx_EXT_METADATA_NAMES)
  # Sanity check
  foreach(var IN ITEMS "REQUIRED" "DEFAULT" "DESCRIPTION")
    set(varname "Cjyx_EXT_METADATA_${name}_${var}")
    if(NOT DEFINED "${varname}")
      message(FATAL_ERROR "Variable ${varname} is not defined")
    endif()
  endforeach()
  if(Cjyx_EXT_METADATA_${name}_REQUIRED)
    list(APPEND Cjyx_EXT_REQUIRED_METADATA_NAMES ${name})
  else()
    list(APPEND Cjyx_EXT_OPTIONAL_METADATA_NAMES ${name})
  endif()
endforeach()

macro(cjyx_extension_metadata_set_defaults prefix)
  foreach(name IN LISTS Cjyx_EXT_OPTIONAL_METADATA_NAMES)
    if("${${prefix}_${name}}" STREQUAL "")
      set(${prefix}_${name} "${Cjyx_EXT_METADATA_${name}_DEFAULT}")
    endif()
  endforeach()
endmacro()
...
function(cjyx_extension_description_spec_defaults_test)

  # Check metadata count
  list(LENGTH Cjyx_EXT_METADATA_NAMES metadata_count)
  list(LENGTH Cjyx_EXT_REQUIRED_METADATA_NAMES required_metadata_count)
  list(LENGTH Cjyx_EXT_OPTIONAL_METADATA_NAMES optional_metadata_count)
  ...
  foreach(name IN LISTS required optonal)
    foreach(varsuffix IN ITEMS "REQUIRED" "DEFAULT" "DESCRIPTION")
      set(expected_defined_varname "Cjyx_EXT_METADATA_${name}_${varsuffix}")
...
if(TEST_cjyx_extension_description_spec_defaults_test)
  cjyx_extension_description_spec_defaults_test()
endif()

function(cjyx_extension_metadata_set_defaults_test)
  ...
  cjyx_extension_metadata_set_defaults("foo")

  # Check variable already set are not overridden
  if(NOT foo_ENABLED STREQUAL "0")
    message(FATAL_ERROR "Problem with cjyx_extension_metadata_set_defaults")
  endif()
  ...
  cjyx_extension_metadata_set_defaults("bar")
...
if(TEST_cjyx_extension_metadata_set_defaults_test)
  cjyx_extension_metadata_set_defaults_test()
endif()
