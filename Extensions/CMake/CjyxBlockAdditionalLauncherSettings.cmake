...
if(NOT TARGET ConfigureAdditionalLauncherSettings AND _configure_additional_launcher_settings)
  ...
  if (EXTENSION_DEPENDS)
    ...
    foreach(dep ${EXTENSION_DEPENDS})
      ...
      if (NOT "${dep}" STREQUAL "NA")
        ...
        if (NOT ${dep}_FOUND)
          message(WARNING "Dependent extension ${dep} cannot be found by CMake find_package(), therefore paths variables cannot be imported from this extension. The problem can be resolved by generating ${dep}Config.cmake by adding include(\${Cjyx_EXTENSION_GENERATE_CONFIG}) to the top-level CMakeLists.txt of the dependent extension.")
        endif()
      endif()
    endforeach()
  endif()

  set(Cjyx_ADDITIONAL_LAUNCHER_SETTINGS_FILE ${CMAKE_CURRENT_BINARY_DIR}/AdditionalLauncherSettings.ini)
  set(Cjyx_ADDITIONAL_LAUNCHER_SETTINGS "--launcher-additional-settings" ${Cjyx_ADDITIONAL_LAUNCHER_SETTINGS_FILE})

  # Add '--launcher-additional-settings' to launch command
  list(FIND Cjyx_LAUNCH_COMMAND "--launch" launch_index)
  list(INSERT Cjyx_LAUNCH_COMMAND ${launch_index} ${Cjyx_ADDITIONAL_LAUNCHER_SETTINGS})

  list(FIND SEM_LAUNCH_COMMAND "--launch" launch_index)
  list(INSERT SEM_LAUNCH_COMMAND ${launch_index} ${Cjyx_ADDITIONAL_LAUNCHER_SETTINGS})
  ...
  set(EXTENSION_LIBRARY_PATHS_BUILD
    ${CMAKE_BINARY_DIR}/${Cjyx_CLIMODULES_LIB_DIR}/\${CMAKE_CFG_INTDIR}
    ${CMAKE_BINARY_DIR}/${Cjyx_QTLOADABLEMODULES_LIB_DIR}/\${CMAKE_CFG_INTDIR}
    )
  ...
  if(DEFINED EXTENSION_SUPERBUILD_BINARY_DIR)
    list(APPEND EXTENSION_LIBRARY_PATHS_BUILD
      ${EXTENSION_SUPERBUILD_BINARY_DIR}/${Cjyx_THIRDPARTY_LIB_DIR}/\${CMAKE_CFG_INTDIR}
      )
  endif()
  ...
  set(EXTENSION_PATHS_BUILD
    ${CMAKE_BINARY_DIR}/${Cjyx_CLIMODULES_BIN_DIR}/\${CMAKE_CFG_INTDIR}
    ${CMAKE_BINARY_DIR}/${Cjyx_THIRDPARTY_BIN_DIR}/\${CMAKE_CFG_INTDIR}
    )

  # Third party libraries may live at the superbuild top level.
  if(DEFINED EXTENSION_SUPERBUILD_BINARY_DIR)
    list(APPEND EXTENSION_PATHS_BUILD
      ${EXTENSION_SUPERBUILD_BINARY_DIR}/${Cjyx_THIRDPARTY_BIN_DIR}/\${CMAKE_CFG_INTDIR}
      )
  endif()
  ...
  set(EXTENSION_PYTHONPATH_BUILD
    ...
    ${CMAKE_BINARY_DIR}/${Cjyx_QTLOADABLEMODULES_LIB_DIR}/\${CMAKE_CFG_INTDIR}
    ${CMAKE_BINARY_DIR}/${Cjyx_QTLOADABLEMODULES_PYTHON_LIB_DIR}
    )
  # Third party libraries may live at the superbuild top level.
  if(DEFINED EXTENSION_SUPERBUILD_BINARY_DIR)
...
${_extension_paths_${envvar}}")
  ...
  "file(WRITE ${Cjyx_ADDITIONAL_LAUNCHER_SETTINGS_FILE}
\"[General]
...
")
  ...
  add_custom_command(
    ...
    OUTPUT
      ${Cjyx_ADDITIONAL_LAUNCHER_SETTINGS_FILE}
