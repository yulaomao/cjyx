...
class ExtensionWizard:
    ...
    def contribute(self, args):
        ...
        try:
            ...
            upstreamRepo = GithubHelper.getRepo(gh, name="Cjyx/ExtensionsIndex")
    ...
    def _execute(self, args):
        # Set up arguments
        parser = argparse.ArgumentParser(description="Cjyx Wizard",
        ...
        parser.add_argument("--target", metavar="VERSION", default="master",
                            help="version of Cjyx for which the extension"
        ...
        if not haveGit() and (args.publish or args.contribute or args.name):
            ...
            Consider re-building Cjyx with SSL support or downloading
            Cjyx from https://download.slicer.org
            """ % option))
        ...
        candidateBuiltInTemplatePaths = [
            ...
            os.path.join(scriptPath, "..", "..", "..", "share",  # Run from install
                         "Cjyx-%s.%s" % tuple(__version_info__[:2]),
                         "Wizard", "Templates")
        ]
