...
def update_external_project_python_packages(packages_to_update, directory, cpython_tag):
    ...
    for package_name, (current_version, latest_version) in packages_to_update.items():
        ...
        if package_name in ["vtk", "simpleitk"]:
            continue  # Cjyx python wraps VTK and SimpleITK instead of installing the official python package from PyPI
...
if __name__ == '__main__':
    ...
    parser.description = """
        ...
        For updating Cjyx python packages for https://github.com/Slicer/Slicer/tree/master,
        run using PythonCjyx.exe from the latest Cjyx preview.
    ...
    if not search_directory:
        # Assume script is in cloned Cjyx repo, so choose the SuperBuild directory to search
