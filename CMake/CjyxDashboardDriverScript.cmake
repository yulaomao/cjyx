...
if(WIN32)
  ...
  # when creating packages to avoid popping up a console window when running Cjyx.
  if(NOT DEFINED Cjyx_BUILD_WIN32_CONSOLE)
    set(Cjyx_BUILD_WIN32_CONSOLE ON)
  endif()
  list(APPEND expected_variables Cjyx_BUILD_WIN32_CONSOLE)
  if(NOT DEFINED Cjyx_BUILD_WIN32_CONSOLE_LAUNCHER)
    if(WITH_PACKAGES)
      set(Cjyx_BUILD_WIN32_CONSOLE_LAUNCHER OFF)
    else()
      set(Cjyx_BUILD_WIN32_CONSOLE_LAUNCHER ON)
    endif()
  endif()
  list(APPEND expected_variables Cjyx_BUILD_WIN32_CONSOLE_LAUNCHER)
endif()

if(NOT DEFINED Cjyx_USE_VTK_DEBUG_LEAKS)
  list(APPEND expected_variables Cjyx_RELEASE_TYPE)
  set(Cjyx_USE_VTK_DEBUG_LEAKS ON)
  if("${Cjyx_RELEASE_TYPE}" STREQUAL "Stable")
    set(Cjyx_USE_VTK_DEBUG_LEAKS OFF)
  endif()
endif()
list(APPEND expected_variables Cjyx_USE_VTK_DEBUG_LEAKS)
...
if(NOT DEFINED CTEST_BUILD_NAME)
  ...
  if(NOT Cjyx_BUILD_CLI)
    set(name "${name}-NoCLI")
  endif()
  if(WIN32)
    if(NOT Cjyx_BUILD_WIN32_CONSOLE_LAUNCHER)
      set(name "${name}-NoConsole")
    endif()
  endif()
  if(NOT Cjyx_USE_VTK_DEBUG_LEAKS)
...
if(NOT DEFINED CTEST_SOURCE_DIRECTORY OR NOT DEFINED CTEST_BINARY_DIRECTORY)
  list(APPEND expected_variables
    DASHBOARDS_DIR
    Cjyx_DASHBOARD_SUBDIR
    Cjyx_DIRECTORY_BASENAME
    Cjyx_DIRECTORY_IDENTIFIER
    )
  if(NOT DEFINED CTEST_SOURCE_DIRECTORY)
    set(CTEST_SOURCE_DIRECTORY "${DASHBOARDS_DIR}/${Cjyx_DASHBOARD_SUBDIR}/${Cjyx_DIRECTORY_BASENAME}-${Cjyx_DIRECTORY_IDENTIFIER}")
  endif()
  if(NOT DEFINED CTEST_BINARY_DIRECTORY)
    set(CTEST_BINARY_DIRECTORY "${DASHBOARDS_DIR}/${Cjyx_DASHBOARD_SUBDIR}/${Cjyx_DIRECTORY_BASENAME}-${Cjyx_DIRECTORY_IDENTIFIER}-build")
  endif()
endif()
...
setIfNotDefined(CJYX_PACKAGE_MANAGER_URL "https://slicer-packages.kitware.com")
...
# Used in CjyxPackageAndUploadTarget CMake module
...
macro(run_ctest)
  ...
  set(build_in_progress_file ${CTEST_BINARY_DIRECTORY}/Cjyx-build/BUILD_IN_PROGRESS)
...
CMAKE_OSX_DEPLOYMENT_TARGET:STRING=${CMAKE_OSX_DEPLOYMENT_TARGET}")
    ...
    if(DEFINED Cjyx_BUILD_WIN32_CONSOLE)
      set(OPTIONAL_CACHE_CONTENT "${OPTIONAL_CACHE_CONTENT}
Cjyx_BUILD_WIN32_CONSOLE:BOOL=${Cjyx_BUILD_WIN32_CONSOLE}")
    endif()

    if(DEFINED Cjyx_BUILD_WIN32_CONSOLE_LAUNCHER)
    set(OPTIONAL_CACHE_CONTENT "${OPTIONAL_CACHE_CONTENT}
Cjyx_BUILD_WIN32_CONSOLE_LAUNCHER:BOOL=${Cjyx_BUILD_WIN32_CONSOLE_LAUNCHER}")
  endif()

    if(DEFINED Cjyx_USE_PYTHONQT)
      set(OPTIONAL_CACHE_CONTENT "${OPTIONAL_CACHE_CONTENT}
Cjyx_USE_PYTHONQT:BOOL=${Cjyx_USE_PYTHONQT}")
    endif()

    if(DEFINED Cjyx_BUILD_CLI)
      set(OPTIONAL_CACHE_CONTENT "${OPTIONAL_CACHE_CONTENT}
Cjyx_BUILD_CLI:BOOL=${Cjyx_BUILD_CLI}")
    endif()
...
Cjyx_RELEASE_TYPE:STRING=${Cjyx_RELEASE_TYPE}
Cjyx_USE_VTK_DEBUG_LEAKS:BOOL=${Cjyx_USE_VTK_DEBUG_LEAKS}
...
")
  ...
  if(FILES_UPDATED GREATER 0 OR force_build)
    ...
    #-----------------------------------------------------------------------------
    set(cjyx_build_dir "${CTEST_BINARY_DIRECTORY}/Cjyx-build")
    ...
    if(WITH_PACKAGES AND (run_ctest_with_packages OR run_ctest_with_upload))
      ...
      if(build_errors GREATER "0")
        ...
        include(CjyxCTestUploadURL)

        message(STATUS "Packaging and uploading Cjyx to packages server ...")
        set(package_list)
        if(run_ctest_with_packages)
          ctest_build(
            TARGET packageupload
            BUILD ${cjyx_build_dir}
            APPEND
            )
          ctest_submit(PARTS Build)
        endif()
        ...
        if(run_ctest_with_upload)
          message(STATUS "Uploading Cjyx package URL ...")

          file(STRINGS ${cjyx_build_dir}/PACKAGES.txt package_list)

          foreach(p ${package_list})
            get_filename_component(package_name "${p}" NAME)
            message(STATUS "Uploading URL to [${package_name}] on CDash")
            cjyx_ctest_upload_url(
              ALGO "SHA512"
              DOWNLOAD_URL_TEMPLATE "${CJYX_PACKAGE_MANAGER_URL}/api/v1/file/hashsum/%(algo)/%(hash)/download"
              FILEPATH ${p}
              )
            if(run_ctest_submit)
    ...
    if(WITH_COVERAGE AND CTEST_COVERAGE_COMMAND AND run_ctest_with_coverage)
      ...
      if(EXISTS ${cjyx_build_dir}/CMakeFiles/TargetDirectories.txt)
        file(READ ${cjyx_build_dir}/CMakeFiles/TargetDirectories.txt cjyx_build_coverage_dirs)
        file(APPEND "${CTEST_BINARY_DIRECTORY}/CMakeFiles/TargetDirectories.txt" "${cjyx_build_coverage_dirs}")
      endif()
      ctest_coverage(BUILD "${cjyx_build_dir}")
      if(run_ctest_submit)
    ...
    if(WITH_MEMCHECK AND CTEST_MEMORYCHECK_COMMAND AND run_ctest_with_memcheck)
        ctest_memcheck(BUILD "${cjyx_build_dir}")
        if(run_ctest_submit)
