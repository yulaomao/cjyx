...
function(cjyxInstallLibrary)
  ...
  cmake_parse_arguments(_cjyxInstallLibrary
    "${options}"
    "${oneValueArgs}"
    "${multiValueArgs}"
    ${ARGN}
    )
  if(_cjyxInstallLibrary_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "Unknown keywords given to cjyxInstallLibrary(): \"${_cjyxInstallLibrary_UNPARSED_ARGUMENTS}\"")
  endif()

  if(NOT WIN32)

    set(install_permissions)
    if(DEFINED _cjyxInstallLibrary_PERMISSIONS)
      set(install_permissions PERMISSIONS ${_cjyxInstallLibrary_PERMISSIONS})
    endif()

    set(strip_configs "Release")
    if(DEFINED _cjyxInstallLibrary_STRIP_CONFIGURATIONS)
      set(strip_configs ${_cjyxInstallLibrary_STRIP_CONFIGURATIONS})
    endif()

    get_filename_component(dir_tmp ${_cjyxInstallLibrary_FILE} PATH)
    # Note: Library symlinks are always named "lib.*.dylib" on mac or "lib.so.*" on linux
    get_filename_component(lib_dir ${_cjyxInstallLibrary_FILE} PATH)
    get_filename_component(lib_name ${_cjyxInstallLibrary_FILE} NAME_WE)
    install(DIRECTORY ${lib_dir}/
      DESTINATION ${_cjyxInstallLibrary_DESTINATION} COMPONENT ${_cjyxInstallLibrary_COMPONENT}
    ...
    if(_cjyxInstallLibrary_STRIP)
      cjyxStripInstalledLibrary(
        FILES "${_cjyxInstallLibrary_DESTINATION}/${lib_name}.so"
        COMPONENT ${_cjyxInstallLibrary_COMPONENT}
...
function(cjyxStripInstalledLibrary)
  set(options
    )
  set(oneValueArgs
    COMPONENT
  ...
  cmake_parse_arguments(_cjyxStripInstalledLibrary
    "${options}"
    "${oneValueArgs}"
    "${multiValueArgs}"
    ${ARGN}
    )
  if(_cjyxStripInstalledLibrary_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "Unknown keywords given to cjyxStripInstalledLibrary(): \"${_cjyxStripInstalledLibrary_UNPARSED_ARGUMENTS}\"")
  endif()

  # Sanity checks
  if(NOT _cjyxStripInstalledLibrary_FILES AND NOT _cjyxStripInstalledLibrary_PATTERN)
  ...
  foreach(var ${expected_defined_vars})
    if(NOT DEFINED _cjyxStripInstalledLibrary_${var})
  ...
  # Defaults
  set(configs "Release")
  if(DEFINED _cjyxStripInstalledLibrary_CONFIGURATIONS)
    set(configs ${_cjyxStripInstalledLibrary_CONFIGURATIONS})
  endif()
  ...
  if(UNIX AND NOT APPLE)
    if("${CMAKE_BUILD_TYPE}" IN_LIST configs)
      ...
      set(dollar "$")
      if(_cjyxStripInstalledLibrary_FILES)
        foreach(file IN LISTS _cjyxStripInstalledLibrary_FILES)
          install(
  ...
  execute_process(COMMAND \"${CMAKE_STRIP}\" \"${dollar}ENV{DESTDIR}${dollar}{CMAKE_INSTALL_PREFIX}/${file}\")"
            COMPONENT ${_cjyxStripInstalledLibrary_COMPONENT}
            )
        endforeach()
      endif()
      if(_cjyxStripInstalledLibrary_PATTERN)
        install(
          CODE "file(
  GLOB_RECURSE libraries FOLLOW_SYMLINKS
  LIST_DIRECTORIES false
  RELATIVE ${dollar}ENV{DESTDIR}${dollar}{CMAKE_INSTALL_PREFIX}
  ${dollar}ENV{DESTDIR}${dollar}{CMAKE_INSTALL_PREFIX}/${_cjyxStripInstalledLibrary_PATTERN}
  )
